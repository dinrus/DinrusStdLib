...
bool PopUpTable::Key(dword key, i32 n) {
	switch(key) {
	case K_ENTER:
	case K_ALT_DOWN:
		DoClose();
...
void PopUpTable::PopUp(Ctrl *owner, i32 x, i32 top, i32 bottom, i32 width) {
	if(inpopup)
		return;
	inpopup++;
	DoClose();
	i32 h = AddFrameSize(width, min(droplines * GetLineCy(), GetTotalCy())).cy;
...
void PopUpTable::PopUp(Ctrl *owner, i32 width)
{
