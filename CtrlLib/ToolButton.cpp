...
Bar::Item& ToolButton::Text(tukk txt)
{
	Txt newtext;
...
ToolButton& ToolButton::Label(tukk text, i32 _kind)
{
	if(kind != _kind) {
		kind = _kind;
		Refresh();
...
ToolButton& ToolButton::Label(tukk text)
{
	Label(text, kind == NOLABEL ? RIGHTLABEL : kind);
	return *this;
}

ToolButton& ToolButton::Kind(i32 _kind)
{
	if(kind != _kind) {
		kind = _kind;
		Refresh();
...
Bar::Item& ToolButton::Image(const DRX::Image& img_)
{
	drx::Image m = img_;
	if(!m.IsSame(img)) {
		img = m;
...
Bar::Item& ToolButton::Tip(tukk tip)
{
	if(tiptext != tip) {
		tiptext = tip;
		UpdateTip();
...
Bar::Item& ToolButton::Help(tukk help)
{
	HelpLine(help);
	return *this;
}

Bar::Item& ToolButton::Topic(tukk help)
{
	HelpTopic(help);
	return *this;
}

Bar::Item& ToolButton::Description(tukk desc)
{
...
Image CachedContrast(const Image& m, i32 d)
{
	if(d == 0)
		return m;
	sCachedContrast cr;
...
//		w.DrawRect(sz, checked && !HasMouse() ? Blend(SColorFace, SColorLight) : SColorFace);
	...
	DRX::Image img = CachedContrast(image, style->contrast[li]);
	if(!IsEnabled())
...
Txt ToolButton::GetDesc() const
{
	return tiptext;
}

i32 ToolButton::OverPaint() const
{
