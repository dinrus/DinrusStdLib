...
i32 Splitter::ClientToPos(Point p) const
{
	return minmax(vert ? 10000 * p.y / GetSize().cy : 10000 * p.x / GetSize().cx, 0, 9999);
}

i32 Splitter::PosToClient(i32 pos) const
{
	Size sz = GetSize();
	i32 sa = vert ? sz.cy : sz.cx;
	i32 a = sa * pos / 10000;
#ifdef TARGET_COCOA // workaround issue with splitter bar completely in "resize arrow" area
...
void Splitter::Layout() {
	...
	i32 lw = chstyle->width >> 1;
	i32 rw = chstyle->width - lw;

	i32 i = 0;
	for(Ctrl *q = GetFirstChild(); q; q = q->GetNext()) {
		if(!q->InFrame()) {
			i32 lo = i > 0 ? PosToClient(pos[i - 1]) + rw : 0;
			i32 hi = i < count ? PosToClient(pos[i]) - lw : vert ? sz.cy : sz.cx;
			q->Show();
			if(vert)
...
i32 Splitter::GetSplitWidth() const
{
...
void Splitter::Paint(Draw& w) {
	...
	const Val *ch = vert ? chstyle->vert : chstyle->horz;
	if(style < 0)
		for(i32 i = 0; i < pos.GetCount(); i++) {
			i32 p = PosToClient(pos[i]) - (chstyle->width >> 1);
...
i32 Splitter::FindIndex(Point client) const {
	i32 best = -1;
	i32 maxdist = chstyle->width;
	for(i32 i = 0; i < pos.GetCount(); i++) {
		i32 dist = abs((vert ? client.y : client.x) - PosToClient(pos[i]));
		if(dist <= maxdist) {
			best = i;
			maxdist = abs(dist);
		}
...
i32 Splitter::GetMins(i32 i) const
{
	i32 min1 = (i < mins.GetCount() ? mins[i] : 0);
	i32 min2 = 0;
	i32 cx = IsVert() ? GetSize().cy : GetSize().cx;
	if(cx)
		min2 = (i < minpx.GetCount() ? minpx[i] : 0) * 10000 / cx;
	return max(min1, min2);
}

Splitter& Splitter::SetPos(i32 p, i32 i) {
	i32 l = (i > 0 && i - 1 < pos.GetCount() ? pos[i - 1] : 0) + GetMins(i);
	i32 h = (i + 1 < pos.GetCount() ? pos[i + 1] : 10000) - GetMins(i + 1);
...
void Splitter::Insert(i32 ii, Ctrl& pane)
{
	if(ii >= GetCount())
		Add(pane);
	else {
