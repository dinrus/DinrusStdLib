...
i32  SplitterFrame::BoundSize()
{
	i32 maxsize = max(0, (type == LEFT || type == RIGHT ? parentsize.cx : parentsize.cy) - sizemin);
...
void SplitterFrame::Paint(Draw& draw)
{
	...
	const Val *ch = horz ? style->vert : style->horz;
...
SplitterFrame& SplitterFrame::Set(Ctrl& c, i32 _size, i32 _type)
{
	while(GetFirstChild())
