...
void Wcpy(char16 *t, const Txt& s, i32 sz)
{
	Vec<char16> w = ToSystemCharsetW(s);
	if(w.GetCount() > sz) {
		w.SetCount(sz - 1);
		w.Add(0);
	}
	...
	if(visible) {
		...
		if (Status == FALSE) {
			...
			if(ErrorCode == ERROR_SUCCESS || ErrorCode == ERROR_TIMEOUT) {
				for(i32 retry = 0; retry < 60; retry++) {
					Sleep(50);
...
void TrayIcon::Message(i32 type, tukk title, tukk text, i32 timeout)
{
	nid.uFlags = 0x10;
...
TrayIcon& TrayIcon::Tip(tukk text)
{
	tip = text;
	Notify(NIM_MODIFY);
	return *this;
...
LRESULT TrayIcon::WindowProc(UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	if(message == UM_TASKBAR_)
		...
		case WM_RBUTTONDOWN:
			::SetForegroundWindow(nid.hWnd);
			MenuBar::Execute(NULL, OTVET(DoMenu), GetMousePos());
			return TRUE;
