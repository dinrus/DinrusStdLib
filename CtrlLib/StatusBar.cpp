...
InfoCtrl::InfoCtrl()
{
	...
	right = false;
	defaulttext = t_("Нет активных операций");
	Set(Null);
	Transparent();
}
...
void InfoCtrl::Paint(Draw& w)
{
	...
	if(temp) {
		...
		r.right = 2;
		for(i32 i = 0; i < tab.GetCount() && r.right < sz.cx; i++) {
			const Tab& t = tab[i];
			r.left = r.right;
			if(t.width < 0)
...
i32 InfoCtrl::GetTabOffset(i32 t) const
{
	Size sz = GetSize();
	Rect r = sz;
	r.right = 2;
...
i32 InfoCtrl::GetRealTabWidth(i32 tabi, i32 width) const
{
	i32 cx = GetSize().cx;
	i32 x = 2;
	for(i32 i = 0; i < tabi; i++) {
		const Tab& t = tab[i];
		if(t.width < 0)
			x = max(x, cx + t.width);
		else {
...
{
public:
	void Paint(Draw& w, const Rect& r, const Val& q, Color ink, Color paper, dword s) const {
		if(IsType<Image>(q)) {
			Image img = q;
			Size sz = img.GetSize();
...
void InfoCtrl::Set(i32 _tab, const PaintRect& info, i32 width)
{
	Tab& t = tab.At(_tab);
	t.width = width;
	t.info = info;
...
void InfoCtrl::Set(i32 tab, const Val& info, i32 width)
{
...
void InfoCtrl::Set(const Val& info)
{
	tab.Clear();
	Set(0, Nvl(info, (Val)defaulttext), INT_MAX);
}
...
void InfoCtrl::Temporary(const PaintRect& info, i32 timeout)
{
	temptime.KillSet(timeout, OTVET(EndTemporary));
	temp = info;
	Refresh();
}

void InfoCtrl::Temporary(const Val& info, i32 timeout)
{
	Temporary(PaintRect(Single<VCenterDisplay>(), info), timeout);
}

InfoCtrl& InfoCtrl::Left(i32 w)
{
	right = false;
	Width(w);
...
StatusBar& StatusBar::Height(i32 _cy)
{
	cy = _cy;
	RefreshParentLayout();
	return *this;
...
public:
	virtual void Paint(Draw& w, const Rect& r, const Val& q, Color ink, Color paper, dword s) const;
};

void ProgressDisplayCls::Paint(Draw& w, const Rect& _r, const Val& q,
                               Color ink, Color paper, dword s) const
{
	...
	r.Deflate(2);
	i32 pos = minmax(i32((double)q * r.Width() / 1000), 0, r.Width());
	if(pos) {
...
ProgressInfo& ProgressInfo::Set(i32 _pos, i32 _total)
{
	pos = _pos;
	total = _total;
