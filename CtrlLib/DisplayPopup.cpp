...
Vec<DisplayPopup::Pop *>& DisplayPopup::Pop::all()
{
	static Vec<DisplayPopup::Pop *> all;
	return all;
}
...
bool DisplayPopup::StateHook(Ctrl *, i32 reason)
{
...
bool DisplayPopup::MouseHook(Ctrl *, bool, i32, Point, i32, dword)
{
	SyncAll();
	return false;
}
...
void DisplayPopup::Pop::Set(Ctrl *_ctrl, const Rect& _item,
                            const Val& _value, const Display *_display,
                            Color _ink, Color _paper, dword _style, i32 _margin)
{
	if(!GUI_ToolTips())
		return;
...
void DisplayPopup::Set(Ctrl *ctrl, const Rect& item, const Val& v, const Display *display, Color ink, Color paper, dword style, i32 margin)
{
	if(!popup) {
		popup.Create();
