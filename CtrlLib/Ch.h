...
Image MakeButton(i32 radius, const Image& face, double border_width, Color border_color = Null, dword corner = 0xff);
Image MakeButton(i32 radius, Color face, double border_width, Color border_color = Null, dword corner = 0xff);

Image Hot3(const Image& m); // Adds resizing hotspots at 1/3
Image ChHot(const Image& m, i32 n = 2); // Adds resizing hotspots DPI(n)
Color AvgColor(const Image& m, const Rect& rr);
Color AvgColor(const Image& m, i32 margin = 0);
Color AvgColor(const Image& m, RGBA bg, const Rect& rr);
Color AvgColor(const Image& m, RGBA bg, i32 margin = 0);
...
Image WithLeftLine(const Image& m, Color c, i32 w = DPI(1));
Image WithRightLine(const Image& m, Color c, i32 w = DPI(1));
Image WithTopLine(const Image& m, Color c, i32 w = DPI(1));
Image WithBottomLine(const Image& m, Color c, i32 w = DPI(1));
...
#ifdef _DEBUG
void  SetChameleonSample(const Val& m, bool once = true);
