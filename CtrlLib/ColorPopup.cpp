...
Txt FormatColor(Color c)
{
	if(IsNull(c))
		return "Null";
	for(i32 i = 0; i < __countof(s_colors); i++)
...
i32 ColorPopUp::GetColorCount() const
{
	return 18 + scolors * 18 + 2 * 18 + hints * 18 + 216;
}

Color ColorPopUp::GetColor(i32 i) const
{
	if(!scolors)
		i += 18;
	if(i < 36)
...
void ColorPopUp::DrawFilledFrame(Draw &w, i32 x, i32 y, i32 cx, i32 cy, Color fcol, Color bcol)
{
...
void ColorPopUp::Paint(Draw& w)
{
	...
	i32 i = 0;
	for(;;) {
		for(i32 x = 0; x < 18 * DPI(16); x += DPI(16)) {
			if(i >= GetColorCount()) {
				if(!norampwheel) {
...
i32 ColorPopUp::Get(Point p)
{
	if(p.y >= GetCy())
		return 999;
	if(withvoid) {
		i32 y0 = StdFont().Info().GetHeight() + DPI(4);
		if(p.y < y0)
			return 997;
		p.y -= y0;
	}
...
bool ColorPopUp::Key(dword key, i32 count)
{
	if(key == K_ESCAPE)	{
		Close();
		WhenCancel();
...
ColorPopUp::ColorPopUp()
{
	...
	ramp <<= OTVET(Ramp);
	wheel <<= OTVET(Wheel);
	ramp.WhenLeftDouble = wheel.WhenLeftDouble = OTVET(Select);
	BackPaint();
