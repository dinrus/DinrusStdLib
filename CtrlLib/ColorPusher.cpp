...
void ColorPusher::Paint(Draw& w)
{
	...
	i32 ty = (sz.cy - StdFont().Info().GetHeight()) / 2;
	if(IsNull(color))
	...
	if(color == VoidColor)
		...
		auto DrawColor = [&](i32 x, i32 y, i32 cx, i32 cy) {
...
bool ColorPusher::Key(dword key, i32)
{
	if(key == K_SPACE) {
		Drop();
		return true;
...
void ColorPusher::SetData(const Val& v)
{
	Color c = v;
	if(color != c) {
		color = c;
...
ColorPusher::ColorPusher()
{
	...
	colors.WhenSelect = OTVET(AcceptColors);
	colors.WhenCancel = OTVET(CloseColors);
	colors.WhenAction = OTVET(NewColor);
	colors.Hints();
