...
Txt Pusher::GetDesc() const
{
	return label;
}

Pusher& Pusher::SetLabel(tukk atext) {
...
i32 Pusher::GetVisualState() const
{
...
void Button::PaintButton(Draw& w, const Rect& r, const Button::Style& st, i32 visualstate, bool focus,
                         const Txt& label, Font font, const Image& img,
                         bool monoimg, i32 accesskey, bool visibleaccesskeys, bool disabled)
{
	DrawLabel dl;
	dl.text = label;
...
bool Button::Key(dword key, i32 w) {
...
void SpinButtons::FrameLayout(Rect& r)
{
	...
	Size sz = r.Size();
	i32 h = r.Height();
	i32 h7 = min(sz.cx / 2, style->width);
...
void Option::Paint(Draw& w) {
	...
	i32 ix = 0;
	i32 d = tsz.cy >> 1;
	i32 ty = (sz.cy - tsz.cy) / 2;
	i32 iy = (tsz.cy - isz.cy) / 2 + ty;
	...
	i32 q = GetVisualState();
	i32 g = (!notnull || threestate) && IsNull(option) ? CtrlsImg::I_O2
...
void   Option::SetData(const Val& data) {
	Set(IsTxt(data) ? Txt(data) == "1" ? 1
	                                         : IsNull(data) ? (notnull ? 0 : i32(Null)) : 0
	                   : notnull ? Nvl((i32)data, 0) : (i32)data);
}

Val  Option::GetData() const {
	return (i32) option;
}
...
Option& Option::Set(i32 b)
{
	b = IsNull(b) ? Null : (i32)(bool)b;
	if(b != option) {
		option = b;
		Update();
		RefreshPush();
...
void  ButtonOption::Paint(Draw& w) {
	...
	if(VisibleAccessKeys() && IsEnabled())
		dl.accesskey = accesskey;
	i32 i = !IsShowEnabled() ? CTRL_DISABLED :
	         push ? CTRL_PRESSED :
...
void  ButtonOption::SetData(const Val& v)
{
	Set(IsNull(v) ? 0 : IsTxt(v) ? Txt(v) == "1" : (i32)v);
}

Val ButtonOption::GetData() const
{
	return (i32)Get();
}
...
ButtonOption& ButtonOption::SetLabel(const Txt& text) {
...
Val DataPusher::GetData() const
{
	return data;
}

void DataPusher::Set(const Val& value)
{
	if(value != data) {
		data = value;
		Refresh();
...
void DataPusher::SetData(const Val& value)
{
	if(value != data) {
		data = value;
		UpdateRefresh();
...
DataPusher& DataPusher::NullText(tukk text, Font fnt, Color ink)
{
	WTxt txt = text;
...
DataPusher& DataPusher::NullText(tukk text, Color ink)
{
...
void DataPusher::Paint(Draw& w)
{
	...
	if(IsNull(data) && !IsNull(nulltext)) {
		i32 cy = nullfont.Info().GetHeight();
		w.DrawText(rc.left, rc.top + (rc.GetHeight() - cy) / 2, nulltext, nullfont, nullink);
	}
}

void DataPusher::SetDataAction(const Val& value)
{
	SetData(value);
	UpdateActionRefresh();
}
