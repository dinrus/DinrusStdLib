...
public:
	...
	StaticText& SetVAlign(i32 align);
	...
	StaticText& SetImage(const Image& img, i32 spc = 0);
	StaticText& SetText(tukk text);
	...
	i32         GetAlign() const                        { return Nvl(GetIntAttr(ATTR_ALIGN), ALIGN_LEFT); }
	i32         GetVAlign() const                       { return Nvl(GetIntAttr(ATTR_VALIGN), ALIGN_CENTER); }
	Image       GetImage() const                        { return GetAttr<Image>(ATTR_IMAGE); }

	StaticText& operator=(tukk s)                { SetText(s); return *this; }
...
public:
	Label& SetText(tukk text);
	Label& SetLabel(tukk lbl);

	Label& operator=(tukk s)                     { SetText(s); return *this; }
...
public:
	...
	static Val LabelBoxLook;

public:
	static void  SetLook(const Val& v) { LabelBoxLook = v; }
	static Val GetLook()               { return LabelBoxLook; }

	LabelBox&    SetColor(Color c)       { color = c; return *this; }

	LabelBox();
	virtual ~LabelBox();
	LabelBox& operator=(tukk s)   { SetText(s); return *this; }
};

void PaintLabelBox(Draw& w, Size sz, Color color, i32 d);
...
public:
	StaticRect& Background(const Val& chvalue);
	StaticRect& Color(class Color c)                   { Background(c); return *this; }

	Val GetBackground() const                        { return bg; }
...
public:
	...
	SeparatorCtrl& Margin(i32 l, i32 r);
	SeparatorCtrl& Margin(i32 w)                { return Margin(w, w); }
	SeparatorCtrl& SetSize(i32 w);
