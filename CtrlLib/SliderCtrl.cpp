...
i32  SliderCtrl::HoVe(i32  x, i32  y) const
{
	return IsVert() ? y : x;
}

i32& SliderCtrl::HoVeR(i32& x, i32& y) const
{
	return IsVert() ? y : x;
}
...
bool SliderCtrl::Key(dword key, i32 repcnt)
{
	if(IsEditable())
		switch(key) {
		case K_LEFT:
...
void SliderCtrl::LeftDown(Point pos, dword keyflags)
{
	if(!IsEditable())
		return;
	SetWantFocus();
	i32 thumb = SliderToClient(value);
...
{
	if(HasCapture()) {
		i32 n = ClientToSlider(HoVe(pos.x, pos.y));
		if(n != value) {
			value = n;
			UpdateActionRefresh();
		}
...
void SliderCtrl::SetData(const Val& v)
{
	i32 i = v;
	if(!IsNull(i))
		i = minmax(i, Min(), Max() );
...
Val SliderCtrl::GetData() const
{
	return value;
}

SliderCtrl& SliderCtrl::MinMax(i32 _min, i32 _max)
{
	if(min != _min || max != _max) {
		min = _min;
		max = _max;
		if(!IsNull(value)) {
			i32 v = minmax(value, Min(), Max());
			if(value != v) {
				value = v;
				Update();
			}
...
i32 SliderCtrl::SliderToClient(i32 v) const
{
	if(IsNull(v))
		return Null;
	v = minmax(v, Min(), Max());
...
i32 SliderCtrl::ClientToSlider(i32 p) const
{
