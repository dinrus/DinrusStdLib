...
void ShowToolTip()
{
	...
	if(!GUI_ToolTips())
		return;
	if(tipctrl) {
		Txt text = tipctrl->GetTip();
		LLOG("-> showing tip: " << text << " tipctrl: " << DRX::Name(tipctrl));
		...
		if(text.GetCount() && top && (top->IsForeground() || top->IsPopUp())) {
			...
			showmode = true;
			KillTimeCallback((uk )EndShowMode);
			return;
		}
		LLOG("-> background / empty text, top = " << DRX::Name(top));
	}
	SetTimeCallback(500, callback(EndShowMode), (uk )EndShowMode);
}
...
void SyncToolTip(Ctrl *ctrl)
{
	...
	if(ctrl != tipctrl || ctrl && ctrl->GetTip() != AppToolTip().Get()) {
		LLOG("ToolTipHook / ctrl change " << DRX::Name(ctrl) << " -> " << DRX::Name(ctrl));
		tipctrl = ctrl;
		KillTimeCallback((uk )SyncToolTip);
		if(showmode)
			ShowToolTip();
		else {
			LLOG("ToolTipHook -> SetTimeCallback");
			SetTimeCallback(GUI_ToolTipDelay(), callback(ShowToolTip), (uk )SyncToolTip);
		}
	}
}

bool ToolTipHook(Ctrl *ctrl, bool inframe, i32 event, Point p, i32 zdelta, dword keyflags)
{
	if(event == Ctrl::MOUSEMOVE && ctrl != &AppToolTip())
	{
		SyncToolTip(Ctrl::GetVisibleChild(ctrl, p, inframe));
		LLOG("ToolTipHook MOUSEMOVE " << DRX::Name(ctrl) << " inframe: " << inframe);
	}
	if(event == Ctrl::MOUSELEAVE) {
		LLOG("ToolTipHook MOUSELEAVE " << DRX::Name(ctrl));
		CloseToolTip();
		KillTimeCallback((uk )EndShowMode);
		SetTimeCallback(1000, callback(EndShowMode), (uk )EndShowMode);
	}
...
bool ToolTipStateHook(Ctrl *ctrl, i32 reason)
{
...
public:
	...
	QTFPopUp& Width(i32 _width)                   { width = _width; return *this; }
	QTFPopUp& MaxHeight(i32 _maxheight)           { maxheight = _maxheight; return *this; }
...
void QTFPopUp::PopUp(Ctrl *parent) {
	...
	i32 cy = min(maxheight, GetHeight(r.Width()) + maxheight - r.Height());
