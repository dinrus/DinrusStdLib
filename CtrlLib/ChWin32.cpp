...
#define DLIHEADER   <drx/CtrlLib/XPTheme.dli>
...
struct XpElement : Moveable<XpElement> {
	...
	hash_t GetHashVal() const { return widget ^ part ^ state; }

	XpElement() { contentm = whista = false; }
};

static HANDLE xp_widget_handle[XP_COUNT];
static VecMap<XpElement, i32> xp_margin;
static VecMap<XpElement, i32> xp_opaque;

void XpClear()
{
	for(i32 i = 0; i < XP_COUNT; i++)
...
Image XpImage0(i32 widget, i32 part, i32 state, Color color, Size sz)
{
...
Image XpImage(i32 widget, i32 part, i32 state, Color color = Null, Size sz = Null)
{
	Image m = XpImage0(widget, part, state, color, sz);
	return sEmulateDarkTheme ? DarkTheme(m) : m;
}

bool XpIsAvailable(i32 widget, i32 part, i32 state)
{
	HANDLE theme = XpWidget(widget);
	if(!theme)
		return false;
...
i32 XpMargin(const XpElement& e)
{
	i32 q = xp_margin.Find(e);
	if(q >= 0)
		return xp_margin[q];
...
Color XpColor(i32 widget, i32 part, i32 state, i32 type)
{
	COLORREF cref = 0;
	HANDLE theme = XpWidget(widget);
	if(!theme)
		return Null;
	i32 r = XpTheme().GetThemeColor(theme, part, state, type, &cref);
	return r == S_OK ? sAdjust(Color::FromCR(cref)) : Null;
}

i32 XpInt(i32 widget, i32 part, i32 state, i32 type)
{
	HANDLE theme = XpWidget(widget);
	if(!theme)
		return Null;
	i32 n = Null;
	i32 r = XpTheme().GetThemeInt(theme, part, state, type, &n);
	return r == S_OK ? n : Null;
}

i32 XpBool(i32 widget, i32 part, i32 state, i32 type)
{
	HANDLE theme = XpWidget(widget);
	if(!theme)
		return Null;
	BOOL flag = false;
	i32 r = XpTheme().GetThemeBool(theme, part, state, type, &flag);
	return r == S_OK ? flag : Null;
}

void SetXpImages(i32 uii, i32 n, i32 widget, i32 part, i32 state)
{
	for(i32 i = 0; i < n; i++) {
...
void SetXpColors(Color *color, i32 n, i32 widget, i32 part, i32 state, i32 type)
{
	i32 ii = 0;
	for(i32 i = 0; i < n; i++) {
...
Val XpLookFn(Draw& w, const Rect& rect, const Val& v, i32 op, Color)
{
	if(IsTypeRaw<XpElement>(v)) {
		const XpElement& e = ValTo<XpElement>(v);
...
void Win32Look(Val *ch, i32 count, i32 widget, i32 part, i32 state = 1, bool contentm = false)
{
...
void Win32Look(Val& ch, i32 widget, i32 part, i32 state = 1, bool contentm = false)
{
	Win32Look(&ch, 1, widget, part, state, contentm);
}

Txt XpThemeInfo(LPCWSTR pszPropertyName)
{
...
{
	Txt s = GetWinRegTxt("AppsUseLightTheme", "Software\\Microsoft\\Windows\\CurrentVersion\\Themes\\Personalize", HKEY_CURRENT_USER);
	bool b = s.GetCount() && *s == 0;
	return GetEnv("DINRUS_DARKMODE__") == "1" ? !b : b;
}

void ChHostSkin()
{
	if(Ctrl::IsUHDEnabled()) {
		HRESULT (STDAPICALLTYPE *SetProcessDpiAwareness)(i32);
	...
	Txt name = FromSystemCharsetW(ncm.lfMenuFont.lfFaceName);
	i32 height = abs((i32)ncm.lfMenuFont.lfHeight);
	i32 q = Font::FindFaceNameIndex(name);
	...
	if(XpWidget(XP_BUTTON)) {
		...
		if(vista_aero) {
			...
			for(i32 i = 0; i < 4; i++) {
				i32 efs = i + 1;
				i32 ebsx = max(2, XpInt(XP_EDIT, efp, efs, 2403/*TMT_BORDERSIZE*/));
				i32 ebsy = max(1, XpInt(XP_EDIT, efp, efs, 2403/*TMT_BORDERSIZE*/));
		...
		i32 ebs = max(2, XpInt(XP_LISTVIEW, 0, 1, 2403/*TMT_BORDERSIZE*/));
		...
		for(i32 i = 0; i < 4; i++) {
			{
				...
				s.hthumb[i] = ChLookWith(RawToVal(e), XpImage(XP_SCROLLBAR, SBP_GRIPPERHORZ, 1));
				e.part = SBP_THUMBBTNVERT;
				s.vthumb[i] = ChLookWith(RawToVal(e), XpImage(XP_SCROLLBAR, SBP_GRIPPERVERT, 1));
			}
			Color paper = i == 3 ? SColorFace : SColorPaper;
			Image m = XpImage(XP_COMBOBOX, CP_DROPDOWNBUTTON, CBXS_NORMAL + i, paper, Size(32, 32));
			Image mm = m;
			Size isz = m.GetSize();
			i32 cbs = XpInt(XP_COMBOBOX, CP_DROPDOWNBUTTON, CBXS_NORMAL + i, 2403/*TMT_BORDERSIZE*/);
			if(cbs == 0) {
				Image h = HorzSymm(m);
