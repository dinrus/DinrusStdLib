...
Val MenuBar::GetBackground() const
{
...
void MenuBar::SetupRestoreFocus()
{
	...
	LLOG("SetupRestoreFocus -> " << DRX::Name(restorefocus));
}
...
void MenuBar::Deactivate()
{
	...
	if(IsOpen())
		SetTimeCallback(0, OTVET(PostDeactivate), TIMEID_STOP);
}
...
bool MenuKeyHook(Ctrl *ctrl, dword key, i32 count)
{
...
bool MenuMouseHook(Ctrl *ctrl, bool inframe, i32 event, Point p,
...
bool MenuStateHook(Ctrl *ctrl, i32 reason)
{
...
bool MenuBar::Key(dword key, i32 count)
{
	LLOG("KEY " << GetKeyDesc(key));
	bool horz = IsChild();
	if((horz ? key == K_RIGHT : key == K_DOWN)) {
		Ctrl *ctrl = GetFocusChildDeep();
		LLOG("MenuBar::Key(" << key << ") -> IterateFocusForward for " << DRX::Name(ctrl) << ", pane " << DRX::Name(&pane));
		if(HasMouseDeep())
	...
	if((horz ? key == K_LEFT : key == K_UP)) {
		Ctrl *ctrl = GetFocusChildDeep();
		LLOG("MenuBar::Key(" << key << ") -> IterateFocusBackward for " << DRX::Name(ctrl) << ", pane " << DRX::Name(&pane));
		if(HasMouseDeep())
			GetMouseCtrl()->Refresh();
...
{
	SetTimeCallback(400, OTVET(SubmenuClose), TIMEID_SUBMENUCLOSE);
}
...
void MenuBar::Post(Event<Bar&> bar)
{
	...
	SetTimeCallback(0, OTVET1(Set, bar), TIMEID_POST);
}
...
void MenuBar::PopUp(Ctrl *owner, Point p, Size rsz)
{
	...
	restorefocus = GetFocusCtrl();
	LLOG("PopUp " << DRX::Name(this) << " set restorefocus:" << DRX::Name(restorefocus));
	DistributeAccessKeys();
...
{
	static Vec<Ctrl *> ows; // Used to prevent another open local menu for single owner to be opened (repeated right-click)
	i32 level = ows.GetCount();
...
i32 MenuBar::GetStdHeight(Font font)
{
...
MenuBar::~MenuBar()
{
	lock++;
	LLOG("~MenuBar " << Name());
	LLOG("Parent menu " << DRX::Name(parentmenu));
	if(parentmenu)
		parentmenu->SetActiveSubmenu(NULL, NULL);
	LLOG("~MenuBar 1");
}
