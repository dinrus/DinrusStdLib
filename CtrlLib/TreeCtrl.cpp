...
TreeCtrl::TreeCtrl()
{
	...
	sb.Box(sb_box);
	WhenLeftDouble = OTVET(StdLeftDouble);
}
...
TreeCtrl::Node::Node(const Image& img, const Val& v)
{
	Init();
	image = img;
	key = value = v;
...
TreeCtrl::Node::Node(const Image& img, const Val& v, const Val& t)
{
	Init();
	image = img;
	key = v;
...
TreeCtrl::Node::Node(const Val& v)
{
	Init();
	key = value = v;
}

TreeCtrl::Node::Node(const Val& v, const Val& t)
{
	Init();
	key = v;
	value = t;
...
TreeCtrl::Node::Node(const Image& img, Ctrl& ctrl, i32 cx, i32 cy)
{
	Init();
	SetCtrl(ctrl);
	image = img;
...
// 2008-04-08 mrjt
Size   TreeCtrl::Item::GetValSize(const Display *treedisplay) const
{
...
{
	Size sz = GetValSize(treedisplay);
...
void   TreeCtrl::SetRoot(const Image& img, Val v)
{
	SetRoot(TreeCtrl::Node(img, v).CanOpen());
}

void   TreeCtrl::SetRoot(const Image& img, Val v, Val t)
{
	SetRoot(TreeCtrl::Node(img, v, t).CanOpen());
}

void   TreeCtrl::SetRoot(const Image& img, Ctrl& ctrl, i32 cx, i32 cy)
{
	SetRoot(TreeCtrl::Node(img, ctrl, cx, cy).CanOpen());
}

i32    TreeCtrl::Insert(i32 parentid, i32 i, const TreeCtrl::Node& n)
{
	KillEdit();
	i32 id;
	if(freelist >= 0) {
...
i32    TreeCtrl::Add(i32 parentid, const TreeCtrl::Node& n)
{
	return Insert(parentid, item[parentid].child.GetCount(), n);
}

i32    TreeCtrl::Insert(i32 parentid, i32 i)
{
	return Insert(parentid, i, TreeCtrl::Node());
}

i32    TreeCtrl::Add(i32 parentid)
{
	return Add(parentid, TreeCtrl::Node());
}

i32    TreeCtrl::Insert(i32 parentid, i32 i, const Image& img, Val v, bool withopen)
{
	return Insert(parentid, i, TreeCtrl::Node(img, v).CanOpen(withopen));
}

i32    TreeCtrl::Insert(i32 parentid, i32 i, const Image& img, Val v, Val t, bool withopen)
{
	return Insert(parentid, i, TreeCtrl::Node(img, v, t).CanOpen(withopen));
}

i32 TreeCtrl::Insert(i32 parentid, i32 i, const Image& img, Val key, const Txt& value, bool withopen)
{
	return Insert(parentid, i, img, key, (Val)value, withopen);
}

i32 TreeCtrl::Insert(i32 parentid, i32 i, const Image& img, Val key, tukk value, bool withopen)
{
	return Insert(parentid, i, img, key, (Val)value, withopen);
}

i32    TreeCtrl::Add(i32 parentid, const Image& img, Val v, bool withopen)
{
	return Add(parentid, TreeCtrl::Node(img, v).CanOpen(withopen));
}

i32    TreeCtrl::Add(i32 parentid, const Image& img, Val v, Val t, bool withopen)
{
	return Add(parentid, TreeCtrl::Node(img, v, t).CanOpen(withopen));
}

i32 TreeCtrl::Add(i32 parentid, const Image& img, Val key, const Txt& value, bool withopen)
{
	return Add(parentid, img, key, (Val)value, withopen);
}

i32 TreeCtrl::Add(i32 parentid, const Image& img, Val key, tukk value, bool withopen)
{
	return Add(parentid, img, key, (Val)value, withopen);
}

i32    TreeCtrl::Insert(i32 parentid, i32 i, const Image& img, Ctrl& ctrl, i32 cx, i32 cy, bool withopen)
{
	return Insert(parentid, i, TreeCtrl::Node(img, ctrl, cx, cy).CanOpen(withopen));
}

i32    TreeCtrl::Add(i32 parentid, const Image& img, Ctrl& ctrl, i32 cx, i32 cy, bool withopen)
{
	return Add(parentid, TreeCtrl::Node(img, ctrl, cx, cy).CanOpen(withopen));
}

Val  TreeCtrl::Get(i32 id) const
{
	const Item& m = item[id];
	return m.ctrl ? m.ctrl->GetData() : m.key;
}

Val  TreeCtrl::GetVal(i32 id) const
{
	const Item& m = item[id];
	return m.ctrl ? m.ctrl->GetData() : m.value;
}

void  TreeCtrl::Set(i32 id, Val v)
{
	Item& m = item[id];
	if(m.ctrl)
		m.ctrl->SetData(v);
...
void  TreeCtrl::Set(i32 id, Val k, Val v)
{
	Item& m = item[id];
	if(m.ctrl)
		m.ctrl->SetData(v);
...
void TreeCtrl::SetVal(const Val& v)
{
	i32 id = GetCursor();
	Set(id, Get(id), v);
}

void TreeCtrl::SetDisplay(i32 id, const Display& display)
{
	item[id].SetDisplay(display);
	Dirty(id);
}

const Display& TreeCtrl::GetDisplay(i32 id) const
{
...
bool TreeCtrl::IsValid(i32 id) const
{
	return id >= 0 && id < item.GetCount() && !item[id].free;
}

void   TreeCtrl::SetNode(i32 id, const TreeCtrl::Node& n)
{
...
void   TreeCtrl::RemoveChildren(i32 id)
{
	Item& m = item[id];
	for(i32 i = 0; i < m.child.GetCount(); i++)
		RemoveSubtree(m.child[i]);
	m.child.Clear();
	Dirty(id);
}

i32 TreeCtrl::GetChildIndex(i32 id, i32 childid) const
{
	for(i32 i = 0; i < GetChildCount(id); i++)
		if(GetChild(id, i) == childid)
			return i;
	return -1;
}

void   TreeCtrl::RemoveSubtree(i32 id)
{
	Item& m = item[id];
	if(m.sel)
		selectcount--;
...
void   TreeCtrl::Remove(i32 id)
{
	ASSERT(id > 0);
	i32 pi = item[id].parent;
...
void   TreeCtrl::Remove(const Vec<i32>& id)
{
...
void TreeCtrl::RemoveCtrls(i32 itemi)
{
	Item& m = item[itemi];
	if(m.ctrl)
		m.ctrl->Remove();
...
void TreeCtrl::ReLine(i32 itemi, i32 level, Size& sz)
{
	i32 ii = line.GetCount();
	Line& l = line.Add();
	l.level = level;
	l.itemi = itemi;
	l.y = sz.cy;
...
void TreeCtrl::SyncCtrls(bool add, Ctrl *restorefocus)
{
	...
	Size sz = GetSize();
	i32 y0 = 0;
	for(i32 i = noroot; i < line.GetCount(); i++) {
		const Line& l = line[i];
		Item& m = item[l.itemi];
		i32 mcy = l.y - y0;
		y0 = l.y;
...
bool TreeCtrl::IsOpen(i32 id) const
{
	return item[id].isopen;
}

Vec<i32> TreeCtrl::GetOpenIds() const
{
	Vec<i32> r;
	for(i32 id = 0; id < item.GetCount(); id++)
		if(!item[id].free && item[id].isopen)
			r.Add(id);
	return r;
}

void TreeCtrl::OpenIds(const Vec<i32>& ids)
{
	for(i32 id : ids)
		if(IsValid(id))
			Open(id);
...
void TreeCtrl::Open(i32 id, bool open)
{
	Item& m = item[id];
...
void TreeCtrl::OpenDeep(i32 id, bool open)
{
	Open(id, open);
	Item& m = item[id];
	for(i32 i = 0; i < m.child.GetCount(); i++)
...
void TreeCtrl::MakeVisible(i32 id)
{
	id = GetParent(id);
	while(id >= 0) {
		Open(id);
...
i32 TreeCtrl::FindLine(i32 y) const
{
	i32 l = FindUpperBound(line, y, LineLess());
	return l > 0 ? l - 1 : 0;
}

void TreeCtrl::RefreshLine(i32 i, i32 ex)
{
	SyncTree();
	if(i >= 0) {
		Size sz = GetSize();
		i32 y = line[i].y - sb.GetY();
		Refresh(0, y - ex, sz.cx, item[line[i].itemi].GetSize(display).cy + 2 * ex);
		SyncInfo();
	}
}

void TreeCtrl::RefreshItem(i32 id, i32 ex)
{
	SyncTree();
...
i32  TreeCtrl::GetItemAtLine(i32 i)
{
	SyncTree();
	return line[i].itemi;
}

i32  TreeCtrl::GetLineAtItem(i32 id)
{
	SyncTree();
	return item[id].linei;
}

i32  TreeCtrl::GetLineCount()
{
	SyncTree();
	return line.GetCount();
}

void TreeCtrl::ScrollIntoLine(i32 i)
{
	sb.ScrollIntoY(line[i].y, item[line[i].itemi].GetSize(display).cy);
}

void TreeCtrl::CenterLine(i32 i)
{
	i32 top = line[i].y;
	i32 bottom = top + item[line[i].itemi].GetSize(display).cy;
...
void TreeCtrl::MoveCursorLine(i32 c, i32 incr)
{
	i32 cnt = line.GetCount();
	if (!incr) return;
	else if (c < 0) c = cnt-1;
	else if (c >= cnt) c = 0;
...
void TreeCtrl::SetCursorLine(i32 i, bool sc, bool sel, bool cb)
{
	if(nocursor)
		return;
	if(sel && multiselect) {
...
void TreeCtrl::SetCursorLine(i32 i)
{
	SetCursorLine(i, true, true, true);
}

void TreeCtrl::SetCursorLineSync(i32 i)
{
	if(nocursor)
		return;
	if(i != cursor) {
		...
		if(i < 0)
			...
			cursor = minmax(i, (i32)noroot, line.GetCount() - 1);
		RefreshLine(cursor);
...
void TreeCtrl::SetCursor(i32 id, bool sc, bool sel, bool cb)
{
	while(id > 0) {
...
void TreeCtrl::SetCursor(i32 id)
{
	SetCursor(id, true, true, true);
}

i32  TreeCtrl::GetCursor() const
{
	return cursor >= 0 ? line[cursor].itemi : -1;
}

Val  TreeCtrl::Get() const
{
	return IsCursor() ? Get(GetCursor()) : Val();
}

Val  TreeCtrl::GetVal() const
{
	return IsCursor() ? GetVal(GetCursor()) : Val();
}

i32    TreeCtrl::Find(Val key)
{
	for(i32 i = 0; i < item.GetCount(); i++)
...
bool   TreeCtrl::FindSetCursor(Val key)
{
	i32 q = Find(key);
	if(q < 0)
		return false;
...
bool TreeCtrl::IsSel(i32 id) const
{
	return multiselect ? IsSelected(id) : GetCursor() == id;
}

bool TreeCtrl::IsSelDeep(i32 id) const
{
	return IsSel(id) || id && IsSelDeep(GetParent(id));
}

void TreeCtrl::SelectOne0(i32 id, bool sel, bool cb)
{
	if(!multiselect)
		return;
...
void TreeCtrl::SelectOne(i32 id, bool sel)
{
	SelectOne0(id, sel, true);
}

void TreeCtrl::ShiftSelect(i32 l1, i32 l2)
{
	if(!multiselect)
		return;
	if(l1 > l2)
		DRX::Swap(l1, l2);
	for(i32 i = 0; i < line.GetCount(); i++)
...
Rect TreeCtrl::GetValRect(const Line& l) const
{
	const Item& m = item[l.itemi];
	Size msz = m.GetSize(display);
	Size isz = m.image.GetSize();
	Size vsz = m.GetValSize(display);
	Point org = sb;
	i32 x = levelcx + l.level * levelcx + isz.cx - org.x + m.margin;
...
void TreeCtrl::DoClick(Point p, dword flags, bool down, bool canedit)
{
	...
	if(p.y + org.y > sb.GetTotal().cy)
		return;
	i32 i = FindLine(p.y + org.y);
	const Line& l = line[i];
	i32 itemx = levelcx + l.level * levelcx - org.x;
	i32 x = itemx - (levelcx >> 1);
	...
	if(p.x > x - DPI(6) && p.x < x + DPI(6)) {
		...
		auto DoEdit = [&](i32 qq) {
			...
			if(cursor == qq && qq >= 0 && cursor > 0 && !HasCapture() && WhenEdited && !(flags & (K_SHIFT|K_CTRL)) &&
			   GetValRect(line[qq]).Contains(p) && canedit)
...
void TreeCtrl::SyncInfo()
{
	...
	if((HasMouse() || info.HasMouse()) && popupex && !IsEdit()) {
		...
		if(i < line.GetCount()) {
			...
			const Line& l = line[i];
			const Item& m = item[l.itemi];
			i32 x = levelcx + l.level * levelcx - org.x + m.image.GetSize().cx;
			Size csz = m.GetCtrlSize();
			if(m.ctrl && !highlight_ctrl)
				x += csz.cx;
			Rect r = RectC(x, l.y - org.y, sz.cx - x, m.GetValSize(display).cy + 2 * m.margin);
			if(r.Contains(p)) {
				Color fg, bg;
				dword st;
...
void TreeCtrl::LeftDouble(Point p, dword flags)
{
	...
	i32 i = FindLine(p.y + org.y);
	const Line& l = line[i];
	i32 x = levelcx + l.level * levelcx - org.x - (levelcx >> 1) - org.x;
...
const Display *TreeCtrl::GetStyle(i32 i, Color& fg, Color& bg, dword& st)
{
	const Line& l = line[i];
	const Item& m = item[l.itemi];
	st = 0;
...
void TreeCtrl::Paint(Draw& w)
{
	...
	i32 levelcx2 = levelcx >> 1;
	i32 start = multiroot ? 1 : 0;
	for(i32 i = start; i < line.GetCount(); i++) {
		Line& l = line[i];
		if(l.ll >= 0) {
			i32 yl = line[i].y + item[l.itemi].GetSize(display).cy - org.y;
			i32 yh = line[l.ll].y + item[line[l.ll].itemi].GetSize(display).cy / 2 - org.y;
			if(yh >= 0 && yl < sz.cy) {
				i32 x = levelcx + levelcx * l.level + levelcx2 - org.x;
				w.DrawRect(x, yl, 1, yh - yl, SColorShadow);
			}
		}
	}
	Rect dri = Null;
	for(i32 i = FindLine(org.y); i < line.GetCount(); i++) {
		Line& l = line[i];
		const Item& m = item[l.itemi];
		Size msz = m.GetSize(display);
		Size isz = m.image.GetSize();
...
void TreeCtrl::MouseWheel(Point, i32 zdelta, dword keyflags)
{
	if(keyflags & K_SHIFT)
		sb.WheelX(zdelta);
	else
...
bool TreeCtrl::Tab(i32 d)
{
	...
	if(m.ctrl && m.ctrl->HasFocusDeep())
		return false;
	for(i32 i = cursor + d; i >= 0 && i < line.GetCount(); i += d) {
		Item& m = item[line[i].itemi];
		if(m.ctrl && m.ctrl->SetWantFocus())
			return true;
	}
...
bool TreeCtrl::Key(dword key, i32)
{
	SyncTree();
	...
	default: {
			if(accel && key >= ' ' && key < K_CHAR_LIM) {
				...
				for(;;) {
					...
					if(l == GetCursorLine())
						break;
					Val v = GetVal(line[l].itemi);
					WTxt w;
					if(IsTxt(v)) w = v;
					else w = StdFormat(v).ToWTxt();
					word first = w[0];
				...
				i32 ln = (exact_line >= 0 ? exact_line : upper_line >= 0 ? upper_line : ascii_line);
				if(ln < 0)
					BeepExclamation();
				else {
					i32 id = GetItemAtLine(ln);
					Open(id);
					SetCursor(id);
				}
				return true;
...
struct TreeCtrl::SortOrder {
	...
	const ValPairOrder  *pairorder;
	bool                   byvalue;

	bool operator()(i32 a, i32 b) const {
		return pairorder ? (*pairorder)(tree->Get(a), tree->GetVal(a), tree->Get(b), tree->GetVal(b))
		                 : byvalue ? (*order)(tree->GetVal(a), tree->GetVal(b))
...
void TreeCtrl::Sort0(i32 id, const ValOrder& order, bool byvalue)
{
	SortOrder so;
	so.tree = this;
	so.order = &order;
	so.byvalue = byvalue;
	DRX::Sort(item[id].child, so);
}

void TreeCtrl::Sort(i32 id, const ValOrder& order, bool byvalue)
{
	SyncTree();
...
void TreeCtrl::SortDeep0(i32 id, const ValOrder& order, bool byvalue)
{
	Sort0(id, order, byvalue);
	Item& m = item[id];
	for(i32 i = 0; i < m.child.GetCount(); i++)
...
void TreeCtrl::SortDeep(i32 id, const ValOrder& order, bool byvalue)
{
	SyncTree();
...
void TreeCtrl::Sort(i32 id, i32 (*compare)(const Val& v1, const Val& v2), bool byvalue)
{
	Sort(id, FnValOrder(compare), byvalue);
}

void TreeCtrl::SortDeep(i32 id, i32 (*compare)(const Val& v1, const Val& v2), bool byvalue)
{
	SortDeep(id, FnValOrder(compare), byvalue);
}

void TreeCtrl::SortByVal(i32 id, const ValOrder& order)
{
	Sort(id, order, true);
}

void TreeCtrl::SortDeepByVal(i32 id, const ValOrder& order)
{
	SortDeep(id, order, true);
}

void TreeCtrl::SortByVal(i32 id, i32 (*compare)(const Val& v1, const Val& v2))
{
	Sort(id, compare, true);
}

void TreeCtrl::SortDeepByVal(i32 id, i32 (*compare)(const Val& v1, const Val& v2))
{
	SortDeep(id, compare, true);
}

void TreeCtrl::Sort0(i32 id, const ValPairOrder& order)
{
	SortOrder so;
	so.tree = this;
	so.pairorder = &order;
...
void TreeCtrl::Sort(i32 id, const ValPairOrder& order)
{
	SyncTree();
	Sort0(id, order);
	Dirty(id);
...
void TreeCtrl::SortDeep0(i32 id, const ValPairOrder& order)
{
	Sort0(id, order);
	Item& m = item[id];
	for(i32 i = 0; i < m.child.GetCount(); i++)
...
void TreeCtrl::SortDeep(i32 id, const ValPairOrder& order)
{
	SyncTree();
	SortDeep0(id, order);
	Dirty(id);
...
void TreeCtrl::Sort(i32 id, i32 (*compare)(const Val& k1, const Val& v1,
                                           const Val& k2, const Val& v2))
{
	SortDeep(id, FnValPairOrder(compare));
}

void TreeCtrl::SortDeep(i32 id, i32 (*compare)(const Val& k1, const Val& v1,
                                               const Val& k2, const Val& v2))
{
	SortDeep(id, FnValPairOrder(compare));
}

void  TreeCtrl::SetData(const Val& data)
{
	FindSetCursor(data);
}

Val TreeCtrl::GetData() const
{
	return Get();
}
...
void TreeCtrl::GatherSel(i32 id, Vec<i32>& sel) const
{
	if(IsSel(id))
		sel.Add(id);
	const Item& m = item[id];
...
{
	Size sz = GetSize();
	for(i32 i = FindLine(sb.GetY()); i < line.GetCount(); i++) {
		Line& l = line[i];
		i32 y = l.y - sb.GetY();
		if(y > sz.cy)
			break;
...
void TreeCtrl::SetOption(i32 id)
{
}
...
void TreeCtrl::DnD(i32 itemid, i32 insert)
{
...
bool TreeCtrl::DnDInserti(i32 ii, PasteClip& d, bool bottom)
{
	if(ii >= 0 && ii < line.GetCount()) {
		i32 itemi = line[ii].itemi;
		i32 parent = GetParent(itemi);
		i32 childi = GetChildIndex(parent, itemi);
		i32 ins = -1;
		if(bottom) {
...
void TreeCtrl::DragAndDrop(Point p, PasteClip& d)
{
	...
	if(py < sb.GetTotal().cy) {
		...
		if(l.itemi && WhenDropInsert) {
			i32 y = l.y;
			i32 cy = item[l.itemi].GetSize(display).cy;
...
void TreeCtrl::AdjustAction(i32 parent, PasteClip& d)
{
	if(IsDragAndDropSource() && IsSelDeep(parent) && d.GetAction() == DND_MOVE)
		d.SetAction(DND_COPY);
}

Vec<i32> TreeCtrl::InsertDrop(i32 parent, i32 ii, const TreeCtrl& src, PasteClip& d)
{
	TreeCtrl copy;
	Vec<i32> sel = src.GetSel();
	for(i32 i = 0; i < sel.GetCount(); i++)
		Copy(copy, 0, i, src, sel[i]);
	Vec<i32> did;
	for(i32 i = 0; i < copy.GetChildCount(0); i++) {
	...
	for(i32 i = 0; i < did.GetCount(); i++)
		SelectOne(did[i], true);
	return did;
}

Vec<i32> TreeCtrl::InsertDrop(i32 parent, i32 ii, PasteClip& d)
{
	return InsertDrop(parent, ii, GetInternal<TreeCtrl>(d), d);
}

void TreeCtrl::Swap(i32 id1, i32 id2)
{
	SyncTree();
	Item& i1 = item[id1];
	Item& i2 = item[id2];
	for(i32 i = 0; i < i1.child.GetCount(); i++)
		item[i1.child[i]].parent = id2;
	for(i32 i = 0; i < i2.child.GetCount(); i++)
...
void TreeCtrl::SwapChildren(i32 parentid, i32 i1, i32 i2)
{
	SyncTree();
	Item& parent = item[parentid];
	DRX::Swap(parent.child[i1], parent.child[i2]);
	Dirty(parentid);
}

void OptionTree::SetRoot(const Image& img, Option& opt, tukk text)
{
	if(text)
		opt.SetLabel(text);
...
void OptionTree::SetRoot(Option& option, tukk text)
{
	SetRoot(Null, option, text);
}

void OptionTree::SetRoot(const Image& img, tukk text)
{
	SetRoot(img, aux[0], text);
}

void OptionTree::SetRoot(tukk text)
{
	SetRoot(Null, aux[0], text);
}

i32 OptionTree::Add(i32 parentid, const Image& img, Option& opt, tukk text)
{
	return Insert(parentid, GetChildCount(parentid), img, opt, text);
}

i32 OptionTree::Insert(i32 parentid, i32 i, const Image& img, Option& opt, tukk text)
{
	if(text)
		opt.SetLabel(text);
	i32 id = TreeCtrl::Insert(parentid, i, img, opt);
	option.At(id, NULL) = &opt;
	opt.NoNotNull().BlackEdge();
	opt <<= OTVET1(SetOption, id);
	SetOption(id);
	return id;
}


i32 OptionTree::Insert(i32 parentid, i32 i, const Image& img, tukk text)
{
	return Insert(parentid, i, img, aux.Add().NoNotNull().Set(Get(parentid)), text);
}

i32 OptionTree::Insert(i32 parentid, i32 i, tukk text)
{
	return Insert(parentid, i, aux.Add().NoNotNull().Set(Get(parentid)), text);
}


i32 OptionTree::Add(i32 parentid, const Image& img, tukk text)
{
	return Insert(parentid, GetChildCount(parentid), img, text);
}

i32 OptionTree::Add(i32 parentid, Option& opt, tukk text)
{
	return Insert(parentid, GetChildCount(parentid), opt, text);
}

i32 OptionTree::Insert(i32 parentid, i32 i, Option& opt, tukk text)
{
	return Insert(parentid, i, Null, opt.NoNotNull(), text);
}

i32 OptionTree::Add(i32 parentid, tukk text)
{
	return Insert(parentid, GetChildCount(parentid), text);
}

void OptionTree::SetLabel(i32 id, tukk text)
{
	Node n = GetNode(id);
...
void OptionTree::SetChildren(i32 id, bool b)
{
	for(i32 i = 0; i < GetChildCount(id); i++) {
		i32 chid = GetChild(id, i);
...
void OptionTree::SetOption(i32 id)
{
...
i32 Copy(TreeCtrl& dst, i32 did, i32 i, const TreeCtrl& src, i32 id)
{
	TreeCtrl::Node x = src.GetNode(id);
	x.ctrl = NULL;
	did = dst.Insert(did, i, x);
	dst.Open(did, src.IsOpen(id));
	for(i32 i = 0; i < src.GetChildCount(id); i++)
