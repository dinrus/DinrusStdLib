topic "Класс FileList (СписокФайлов)";
[i448;a25;kKO9;2 $$1,0#37138531426314131252341829483380:class]
[l288;2 $$2,0#27521748481378242620020725143825:desc]
[0 $$3,0#96390100711032703541132217272105:end]
[H6;0 $$4,0#05600065144404261032431302351956:begin]
[i448;a25;kKO9;2 $$5,0#37138531426314131252341829483370:item]
[l288;a4;*@5;1 $$6,6#70004532496200323422659154056402:requirement]
[l288;i1121;b17;O9;~~~.1408;2 $$7,0#10431211400427159095818037425705:param]
[i448;b42;O9;2 $$8,8#61672508125594000341940100500538:tparam]
[b42;2 $$9,9#13035079074754324216151401829390:normal]
[2 $$0,0#00000000000000000000000000000000:Default]
[{_} 
[ {{10000@3 [s0;%RU-RU [*@7;4 Класс ][*@(229)4 FileList (СписокФайлов)]]}}&]
[s3; &]
[s1;:FileList`:`:class: [@(0.0.255)3 class][3 _][*3 FileList][3 _:_][@(0.0.255)3 public][3 _][*@3;3 C
olumnList][3 , ][@(0.0.255)3 private][3 _][*@3;3 Display]&]
[s2;%RU-RU &]
[s2;%RU-RU 
@@image:2240&1320
()
&]
[s2;%RU-RU &]
[s9;%RU-RU Этот класс является специализацией 
класса ColumnList, направленной на отображение 
списков файлов и подобных им элементов. 
Список состоит из структур File, элементы 
которых позволяют соответствующим 
образом описать и отобразить отдельные 
файлы.&]
[s3;%RU-RU &]
[s0; &]
[ {{10000F(128)G(128)@1 [s0;%RU-RU [* Список Публичных Членов]]}}&]
[s3;%RU-RU &]
[s5;:FileList`:`:WhenRename: [_^Callback2^ Callback2][@(0.0.255) <const]_String[@(0.0.255) `&
], [@(0.0.255) const]_String[@(0.0.255) `&>]_[* WhenRename]&]
[s2;%RU-RU Этот обрвыз вызывается, когда 
пользователь переименовывает файл. 
Первый аргумент `- это исходное filename 
(`"имя файла`"), второй `- новое.&]
[s3; &]
[s4; &]
[s5;:FileList`:`:StartEdit`(`): [@(0.0.255) void]_[* StartEdit]()&]
[s2;%RU-RU Запускает редактирование текущего 
элемента.&]
[s3; &]
[s4; &]
[s5;:FileList`:`:EndEdit`(`): [@(0.0.255) void]_[* EndEdit]()&]
[s2;%RU-RU Завершает редактирование элемента.&]
[s3; &]
[s4; &]
[s5;:FileList`:`:IsEdit`(`)const: [@(0.0.255) bool]_[* IsEdit]()_[@(0.0.255) const]&]
[s2;%RU-RU True, если редактирование активно.&]
[s3; &]
[s4; &]
[s5;:FileList`:`:OkEdit`(`): [@(0.0.255) void]_[* OkEdit]()&]
[s2;%RU-RU Завершает редактирование и вызывает 
WhenRename, что на самом деле переименовывает 
файл.&]
[s2;%RU-RU &]
[s3;%RU-RU &]
[s4; &]
[s5;:FileList`:`:SetIcon`(int`,const Image`&`): [@(0.0.255) void]_[* SetIcon]([@(0.0.255) i
nt]_[*@3 ii], [@(0.0.255) const]_[_^Image^ Image][@(0.0.255) `&]_[*@3 icon])&]
[s2;%RU-RU Заменяет иконку файлового элемента.&]
[s3;%RU-RU &]
[s4; &]
[s5;:Upp`:`:FileList`:`:Add`(const Upp`:`:FileList`:`:File`&`): [@(0.0.255) void]_[* Add](
[@(0.0.255) const]_[_^Upp`:`:FileList`:`:File^ File][@(0.0.255) `&]_[*@3 f])&]
[s5;:FileList`:`:Add`(const String`&`,const Image`&`,Font`,Color`,bool`,int64`,Time`,Color`,const String`&`,Font`,Value`,Color`,bool`,bool`): [@(0.0.255) v
oid]_[* Add]([@(0.0.255) const]_[_^String^ String][@(0.0.255) `&]_[*@3 name], 
[@(0.0.255) const]_[_^Image^ Image][@(0.0.255) `&]_[*@3 icon]_`=_Null, 
[_^Font^ Font]_[*@3 font]_`=_StdFont(), [_^Color^ Color]_[*@3 ink]_`=_SColorText(), 
[@(0.0.255) bool]_[*@3 isdir]_`=_[@(0.0.255) false], [_^int64^ int64]_[*@3 length]_`=_[@3 0],
 [_^Time^ Time]_[*@3 time]_`=_Null, [_^Color^ Color]_[*@3 extink]_`=_Null, 
[@(0.0.255) const]_[_^String^ String][@(0.0.255) `&]_[*@3 desc]_`=_Null, 
[_^Font^ Font]_[*@3 descfont]_`=_Null, [_^Value^ Value]_[*@3 data]_`=_Null, 
[_^Color^ Color]_[*@3 underline]_`=_Null, [@(0.0.255) bool]_[*@3 unixexe]_`=_[@(0.0.255) fa
lse], [@(0.0.255) bool]_[*@3 hidden]_`=_[@(0.0.255) false])&]
[s2;%RU-RU Добавляет новый файловый элемент 
в конец списка.&]
[s3;%RU-RU &]
[s4; &]
[s5;:FileList`:`:Get`(int`)const: [@(0.0.255) const]_[_^FileList`:`:File^ File][@(0.0.255) `&
]_[* Get]([@(0.0.255) int]_[*@3 i])_[@(0.0.255) const]&]
[s2;%RU-RU Возвращает структуру файлового 
элемента по указанному индексу.&]
[s3; &]
[s4; &]
[s5;:FileList`:`:operator`[`]`(int`)const: [@(0.0.255) const]_[_^FileList`:`:File^ File][@(0.0.255) `&
]_[* operator`[`]]([@(0.0.255) int]_[*@3 i])_[@(0.0.255) const]&]
[s2;%RU-RU То же, что и Get([%-*@3 i]).&]
[s3; &]
[s4; &]
[s5;:FileList`:`:Insert`(int`,const String`&`,const Image`&`,Font`,Color`,bool`,int64`,Time`,Color`,const String`&`,Font`,Value`,Color`,bool`,bool`): [@(0.0.255) v
oid]_[* Insert]([@(0.0.255) int]_[*@3 ii], [@(0.0.255) const]_[_^String^ String][@(0.0.255) `&
]_[*@3 name], [@(0.0.255) const]_[_^Image^ Image][@(0.0.255) `&]_[*@3 icon]_`=_Null, 
[_^Font^ Font]_[*@3 font]_`=_StdFont(), [_^Color^ Color]_[*@3 ink]_`=_SColorText(), 
[@(0.0.255) bool]_[*@3 isdir]_`=_[@(0.0.255) false], [_^int64^ int64]_[*@3 length]_`=_[@3 0],
 [_^Time^ Time]_[*@3 time]_`=_Null, [_^Color^ Color]_[*@3 extink]_`=_Null, 
[@(0.0.255) const]_[_^String^ String][@(0.0.255) `&]_[*@3 desc]_`=_Null, 
[_^Font^ Font]_[*@3 descfont]_`=_Null, [_^Value^ Value]_[*@3 data]_`=_Null, 
[_^Color^ Color]_[*@3 underline]_`=_Null, [@(0.0.255) bool]_[*@3 unixexe]_`=_[@(0.0.255) fa
lse], [@(0.0.255) bool]_[*@3 hidden]_`=_[@(0.0.255) false])&]
[s2;%RU-RU Вставляет новый файловый элемент 
по указанной позиции.&]
[s3;%RU-RU &]
[s4; &]
[s5;:FileList`:`:Set`(int`,const String`&`,const Image`&`,Font`,Color`,bool`,int64`,Time`,Color`,const String`&`,Font`,Value`,Color`,bool`,bool`): [@(0.0.255) v
oid]_[* Set]([@(0.0.255) int]_[*@3 ii], [@(0.0.255) const]_[_^String^ String][@(0.0.255) `&]_
[*@3 name], [@(0.0.255) const]_[_^Image^ Image][@(0.0.255) `&]_[*@3 icon]_`=_Null, 
[_^Font^ Font]_[*@3 font]_`=_StdFont(), [_^Color^ Color]_[*@3 ink]_`=_SColorText(), 
[@(0.0.255) bool]_[*@3 isdir]_`=_[@(0.0.255) false], [_^int64^ int64]_[*@3 length]_`=_[@3 0],
 [_^Time^ Time]_[*@3 time]_`=_Null, [_^Color^ Color]_[*@3 extink]_`=_Null, 
[@(0.0.255) const]_[_^String^ String][@(0.0.255) `&]_[*@3 desc]_`=_Null, 
[_^Font^ Font]_[*@3 descfont]_`=_Null, [_^Value^ Value]_[*@3 data]_`=_Null, 
[_^Color^ Color]_[*@3 underline]_`=_Null, [@(0.0.255) bool]_[*@3 unixexe]_`=_[@(0.0.255) fa
lse], [@(0.0.255) bool]_[*@3 hidden]_`=_[@(0.0.255) false])&]
[s2;%RU-RU Заменяет информацию о файловом 
элементе по указанной позиции.&]
[s3;%RU-RU &]
[s4; &]
[s5;:FileList`:`:Set`(int`,const FileList`:`:File`&`): [@(0.0.255) void]_[* Set]([@(0.0.255) i
nt]_[*@3 ii], [@(0.0.255) const]_[_^FileList`:`:File^ File][@(0.0.255) `&]_[*@3 f])&]
[s2;%RU-RU Заменяет информацию о файловом 
элементе по указанной позиции.&]
[s3;%RU-RU &]
[s4; &]
[s5;:FileList`:`:GetCurrentName`(`)const: [_^String^ String]_[* GetCurrentName]()_[@(0.0.255) c
onst]&]
[s2;%RU-RU Возвращает имя файла элемента 
с курсором. Если курсора нет, возвращается 
пустой String.&]
[s3; &]
[s4; &]
[s5;:FileList`:`:Find`(const char`*`): [@(0.0.255) int]_[* Find]([@(0.0.255) const]_[@(0.0.255) c
har]_`*[*@3 name])&]
[s2;%RU-RU Индекс элемента с указанным именем 
[%-*@3 name] или отрицательное значение, 
если отсутствует в списке. Поиск линеен.&]
[s3; &]
[s4; &]
[s5;:FileList`:`:FindSetCursor`(const char`*`): [@(0.0.255) bool]_[* FindSetCursor]([@(0.0.255) c
onst]_[@(0.0.255) char]_`*[*@3 name])&]
[s2;%RU-RU Если элемент с именем [%-*@3 name].есть 
в списке, устанавливает на него курсор 
и возвращает true, в противном случае 
возвращает false.&]
[s3; &]
[s4; &]
[s5;:FileList`:`:Sort`(const FileList`:`:Order`&`): [@(0.0.255) void]_[* Sort]([@(0.0.255) c
onst]_[_^FileList`:`:Order^ Order][@(0.0.255) `&]_[*@3 order])&]
[s2;%RU-RU Сортирует список. Order содержит 
единичный виртуальный метод для упорядочива
ния структур File.&]
[s3; &]
[s4; &]
[s5;:FileList`:`:IconWidth`(int`): [_^FileList^ FileList][@(0.0.255) `&]_[* IconWidth]([@(0.0.255) i
nt]_[*@3 w])&]
[s2;%RU-RU Устанавливает пространство, резервируем
ое для отрисовки иконки файла.&]
[s3; &]
[s4; &]
[s5;:FileList`:`:GetIconWidth`(`)const: [@(0.0.255) int]_[* GetIconWidth]()_[@(0.0.255) con
st]&]
[s2;%RU-RU Возвращает значение, установленное 
посредством IconWidth.&]
[s3; &]
[s4; &]
[s5;:FileList`:`:Renaming`(bool`): [_^FileList^ FileList][@(0.0.255) `&]_[* Renaming]([@(0.0.255) b
ool]_[*@3 b]_`=_[@(0.0.255) true])&]
[s2;%RU-RU В режиме переименования, пользователю 
разрешается редактировать имя элемента 
(при помещении курсора мыши над элементом 
и удерживании кнопки). Обрвыз WhenRename 
вызывается, когда пользователь принимает 
новое имя.&]
[s3; &]
[s4; &]
[s5;:FileList`:`:JustName`(bool`): [_^FileList^ FileList][@(0.0.255) `&]_[* JustName]([@(0.0.255) b
ool]_[*@3 b]_`=_[@(0.0.255) true])&]
[s2;%RU-RU В режиме JustName, если имя файла содержит 
символы [* `\] или [* /], в качестве имени 
файла отображается только часть, 
находящаяся за последним из таких 
символов.&]
[s3;%RU-RU &]
[s4; &]
[s5;:FileList`:`:AccelKey`(bool`): [_^FileList^ FileList][@(0.0.255) `&]_[* AccelKey]([@(0.0.255) b
ool]_[*@3 b]_`=_[@(0.0.255) true])&]
[s2;%RU-RU AccelKey активирует поиск файла, начинающийс
я по нажатии клавиши (сверяется первая 
буква имени файла).&]
[s3;%RU-RU &]
[s4; &]
[s5;:FileList`:`:SelectDir`(bool`): [@(0.0.255) void]_[* SelectDir]([@(0.0.255) bool]_[*@3 b]_
`=_[@(0.0.255) true])&]
[s2;%RU-RU В режиме SelectDir, можно выделять 
директории (important for multiselecting directories 
in FileSel).&]
[s0;%RU-RU &]
[s0;%RU-RU &]
[s0; &]
[ {{10000@3 [s0;%RU-RU [*@(229)4 Структура FileList`::File]]}}&]
[s3;%RU-RU &]
[s1;:FileList`:`:File`:`:struct: [@(0.0.255)3 struct][3 _][*3 File]&]
[s9;%RU-RU Структура, используемая для хранения 
информации об элементе. Некоторые 
элементы, такие как длина файла, фактически 
не напрямую используются FileList, а предоставля
ются только для упрощения использования 
FileList в его первичных приложениях 
(напр., файлвыборках).&]
[s3;%RU-RU &]
[s0;%RU-RU &]
[ {{10000F(128)G(128)@1 [s0;%RU-RU [* Список Публичных Членов]]}}&]
[s3; &]
[s5;:FileList`:`:File`:`:isdir: [@(0.0.255) bool]_[* isdir]&]
[s2;%RU-RU Если true, элемент содержит директорию.&]
[s3;%RU-RU &]
[s4; &]
[s5;:FileList`:`:File`:`:unixexe: [@(0.0.255) bool]_[* unixexe]&]
[s2;%RU-RU Если true, элемент в POSIX имеет право 
исполнения.&]
[s3; &]
[s4; &]
[s5;:FileList`:`:File`:`:hidden: [@(0.0.255) bool]_[* hidden]&]
[s2;%RU-RU Если true, элемент скрыт.&]
[s3; &]
[s4; &]
[s5;:FileList`:`:File`:`:icon: [_^Image^ Image]_[* icon]&]
[s2;%RU-RU Иконка файла.&]
[s3;%RU-RU &]
[s4; &]
[s5;:FileList`:`:File`:`:name: [_^String^ String]_[* name]&]
[s2;%RU-RU Имя файла.&]
[s3;%RU-RU &]
[s4; &]
[s5;:FileList`:`:File`:`:font: [_^Font^ Font]_[* font]&]
[s2;%RU-RU Шрифт, используемый для показа 
имени файла.&]
[s3;%RU-RU &]
[s4; &]
[s5;:FileList`:`:File`:`:ink: [_^Color^ Color]_[* ink]&]
[s2;%RU-RU Цвет, используемый для показа 
имени файла.&]
[s3;%RU-RU &]
[s4; &]
[s5;:FileList`:`:File`:`:length: [_^int64^ int64]_[* length]&]
[s2;%RU-RU Длина файла.&]
[s3;%RU-RU &]
[s4; &]
[s5;:FileList`:`:File`:`:time: [_^Time^ Time]_[* time]&]
[s2;%RU-RU Время последней записи.&]
[s3;%RU-RU &]
[s4; &]
[s5;:FileList`:`:File`:`:extink: [_^Color^ Color]_[* extink]&]
[s2;%RU-RU Цвет, используемый для показа 
расширения файла.&]
[s3;%RU-RU &]
[s4; &]
[s5;:FileList`:`:File`:`:desc: [_^String^ String]_[* desc]&]
[s2;%RU-RU Дополнительное описание файла, 
показываемое вслед за именем файла.&]
[s3;%RU-RU &]
[s4; &]
[s5;:FileList`:`:File`:`:descfont: [_^Font^ Font]_[* descfont]&]
[s2;%RU-RU Шрифт, используемый для дополнительного
 описания.&]
[s3;%RU-RU &]
[s4; &]
[s5;:FileList`:`:File`:`:operator const String`&`(`)const: [* operator_const_String`&](
)_[@(0.0.255) const]&]
[s2;%RU-RU Возвращает имя.&]
[s3;%RU-RU &]
[s0;%RU-RU &]
[s0; [@7$(0.128.128)+150 Процедуры с FileList]&]
[s3; &]
[s5;:Load`(FileList`&`,const String`&`,const char`*`,bool`,Callback3`<bool`,const String`&`,Image`&`>`,FileSystemInfo`&`,const String`&`,bool`): [@(0.0.255) b
ool]_[* Load]([_^FileList^ FileList][@(0.0.255) `&]_[*@3 list], [@(0.0.255) const]_[_^String^ S
tring][@(0.0.255) `&]_[*@3 dir], [@(0.0.255) const]_[@(0.0.255) char]_`*[*@3 patterns], 
[@(0.0.255) bool]_[*@3 dirs]_`=_[@(0.0.255) false], [_^Callback3^ Callback3][@(0.0.255) <bo
ol], [@(0.0.255) const]_[_^String^ String][@(0.0.255) `&], Image[@(0.0.255) `&>]_[*@3 WhenI
con]_`=_CNULL, [_^FileSystemInfo^ FileSystemInfo][@(0.0.255) `&]_[*@3 filesystem]_`=_St
dFileSystemInfo(), [@(0.0.255) const]_[_^String^ String][@(0.0.255) `&]_[*@3 search]_`=_[_^String^ S
tring](), [@(0.0.255) bool]_[*@3 hidden]_`=_[@(0.0.255) true])&]
[s2;%RU-RU Эта функция грузит список [%-*@3 list] 
с файловыми элементами, расположенными 
в директории [%-*@3 dir]. Образцы [%-*@3 patterns] 
содержат допустимые файловые маски, 
разделённые точкой с запятой(напр., 
`"`*.cpp;`*.h`"). Если [%-*@3 dirs] равен false, файлы 
в списке отображаются цветом `"disabled`" 
(useful for directory selection). [%-*@3 WhenIcon] задаёт 
присваивание иконок для отдельных 
имён файлов (первый булев параметр 
равен true для директорий). [%-*@3 filesystem] 
определяет интерфейс файловой системы.&]
[s3;%RU-RU &]
[s4; &]
[s5;:GetDriveImage`(char`): [_^Image^ Image]_[* GetDriveImage]([@(0.0.255) char]_[*@3 drive`_
style])&]
[s2;%RU-RU Возвращает зависящую от ОС иконку 
[%-*@3 drive`_style].&]
[s3;%RU-RU &]
[s4; &]
[s5;:Upp`:`:Sort`(Upp`:`:FileList`&`,int`): [@(0.0.255) void]_[* Sort]([_^Upp`:`:FileList^ F
ileList][@(0.0.255) `&]_[*@3 list], [@(0.0.255) int]_[*@3 kind])&]
[s0;l288;%RU-RU Сортирует список файлов [%-*@3 list] 
по критерию [%-*@3 kind]. [%-*@3 kind] может быть 
одним из FILELISTSORT`_NAME, FILELISTSORT`_EXT, FILELISTSORT`_TIME, 
в необязательном сочетании с FILELISTSORT`_DESCENDING 
(используется оператор `|).&]
[s3;%RU-RU &]
[s0; ]]