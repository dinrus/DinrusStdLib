topic "Класс TabCtrl (ВкладкаКтрл)";
[i448;a25;kKO9;2 $$1,0#37138531426314131252341829483380:class]
[l288;2 $$2,0#27521748481378242620020725143825:desc]
[0 $$3,0#96390100711032703541132217272105:end]
[H6;0 $$4,0#05600065144404261032431302351956:begin]
[i448;a25;kKO9;2 $$5,0#37138531426314131252341829483370:item]
[l288;a4;*@5;1 $$6,6#70004532496200323422659154056402:requirement]
[l288;i1121;b17;O9;~~~.1408;2 $$7,0#10431211400427159095818037425705:param]
[i448;b42;O9;2 $$8,8#61672508125594000341940100500538:tparam]
[b42;2 $$9,9#13035079074754324216151401829390:normal]
[2 $$0,0#00000000000000000000000000000000:Default]
[{_}%RU-RU 
[ {{10000@3 [s0;%- [*@(229)4 Класс TabCtrl (ВкладкаКтрл)]]}}&]
[s4; &]
[s1;:TabCtrl`:`:class:%- [@(0.0.255)3 class][3 _][*3 TabCtrl][3 _:_][@(0.0.255)3 public][3 _][*@3;3 C
trl]&]
[s2; Этот виджет можно использовать для 
организации контента диалогового 
окна путём разделения его на отдельные 
страницы, переключаемые кликами на 
вкладках`-заголовочниках.&]
[s0; &]
[s2; 
@@image:2155&1443
()
&]
[s0; &]
[s2; Каждая вкладка ВкладкаКтрла может 
содержать, но не обязательно содержит, 
подчинённый виджет (как правило, панель 
подчинённого диалогового окна).&]
[s2; Значение ВкладкаКтрла (доступное 
через GetData() / SetData() или соответствующий 
оператор`-сокращение) является индексом 
активной вкладки.&]
[s2; TabCtrl отвечает на вызовы методов SetReadOnly 
/ SetEditable блокированием/разблокированием 
себя в активной вкладке, относительно 
действий пользователя.&]
[s3; &]
[ {{10000F(128)G(128)@1 [s0; [* Список Публичных Членов]]}}&]
[s4;%- &]
[s5;:TabCtrl`:`:WhenSet:%- [_^Callback^ Callback]_[* WhenSet]&]
[s2; Этот обрвыз происходит при изменении 
активной вкладки.&]
[s3; &]
[s4;%- &]
[s5;:TabCtrl`:`:Add`(`):%- [_^TabCtrl`:`:Item^ TabCtrl`::Item][@(0.0.255) `&]_[* Add]()&]
[s2; Добавить новую вкладку в TabCtrl. Возвращает 
ссылку на [^`:`:TabCtrl`:`:Item^ TabCtrl`::Item], которую 
можно использовать для настройки 
свойств вкладки.&]
[s3; &]
[s4;%- &]
[s5;:TabCtrl`:`:Add`(const char`*`):%- [_^TabCtrl`:`:Item^ TabCtrl`::Item][@(0.0.255) `&]_
[* Add]([@(0.0.255) const]_[@(0.0.255) char]_`*[*@3 text])&]
[s2; То же, что и Add(), но также устанавливает 
текст [%-*@3 text] вкладки.&]
[s3; &]
[s4;%- &]
[s5;:TabCtrl`:`:Add`(const Image`&`,const char`*`):%- [_^TabCtrl`:`:Item^ TabCtrl`::Item
][@(0.0.255) `&]_[* Add]([@(0.0.255) const]_[_^Image^ Image][@(0.0.255) `&]_[*@3 m], 
[@(0.0.255) const]_[@(0.0.255) char]_`*[*@3 text])&]
[s2; Добавляет вкладку и устанавливает 
её рисунок и ярлык.&]
[s3; &]
[s4;%- &]
[s5;:TabCtrl`:`:Add`(Ctrl`&`,const char`*`):%- [_^TabCtrl`:`:Item^ TabCtrl`::Item][@(0.0.255) `&
]_[* Add]([_^Ctrl^ Ctrl][@(0.0.255) `&]_[*@3 slave], [@(0.0.255) const]_[@(0.0.255) char]_`*[*@3 t
ext])&]
[s2; Добавляет вкладку с подчинённым 
виджетом [%-*@3 slave] и ярлыком [%-*@3 text].&]
[s3; &]
[s4;%- &]
[s5;:TabCtrl`:`:Add`(Ctrl`&`,const Image`&`,const char`*`):%- [_^TabCtrl`:`:Item^ TabCt
rl`::Item][@(0.0.255) `&]_[* Add]([_^Ctrl^ Ctrl][@(0.0.255) `&]_[*@3 slave], 
[@(0.0.255) const]_[_^Image^ Image][@(0.0.255) `&]_[*@3 m], [@(0.0.255) const]_[@(0.0.255) ch
ar]_`*[*@3 text])&]
[s2; Добавляет вкладку с подчинённым 
виджетом [%-*@3 slave], рисунком и ярлыком 
[%-*@3 text].&]
[s3; &]
[s4;%- &]
[s5;:TabCtrl`:`:Insert`(int`):%- [_^TabCtrl`:`:Item^ TabCtrl`::Item][@(0.0.255) `&]_[* Inser
t]([@(0.0.255) int]_[*@3 i])&]
[s2; [%- Вставляет новую вкладку в TabCtrl, в 
позиции ][%-*@3 i][%- . Возвращает ссылку 
на TabCtrl`::Item], которую можно использовать 
для настройки свойств вкладки.&]
[s3; &]
[s4;%- &]
[s5;:TabCtrl`:`:Insert`(int`,const char`*`):%- [_^TabCtrl`:`:Item^ TabCtrl`::Item][@(0.0.255) `&
]_[* Insert]([@(0.0.255) int]_[*@3 i], [@(0.0.255) const]_[@(0.0.255) char]_`*[*@3 text])&]
[s2;%- Вставляет новую вкладку в TabCtrl, в 
позиции [*@3 i], и заодно устанавливает 
текст вкладки в [*@3 text].&]
[s3; &]
[s4;%- &]
[s5;:TabCtrl`:`:Insert`(int`,const Image`&`,const char`*`):%- [_^TabCtrl`:`:Item^ TabCt
rl`::Item][@(0.0.255) `&]_[* Insert]([@(0.0.255) int]_[*@3 i], [@(0.0.255) const]_[_^Image^ Im
age][@(0.0.255) `&]_[*@3 m], [@(0.0.255) const]_[@(0.0.255) char]_`*[*@3 text])&]
[s2;%- Вставляет новую вкладку в TabCtrl, в 
позиции [*@3 i], и заодно устанавливает 
текст вкладки в [*@3 text] и рисунок в [*@3 m].&]
[s3; &]
[s4;%- &]
[s5;:TabCtrl`:`:Insert`(int`,Ctrl`&`,const char`*`):%- [_^TabCtrl`:`:Item^ TabCtrl`::Ite
m][@(0.0.255) `&]_[* Insert]([@(0.0.255) int]_[*@3 i], [_^Ctrl^ Ctrl][@(0.0.255) `&]_[*@3 slave
], [@(0.0.255) const]_[@(0.0.255) char]_`*[*@3 text])&]
[s2;%- Вставляет новую вкладку в TabCtrl, в 
позиции [*@3 i], а также устанавливает 
его подчинённый виджет в [*@3 slave], а текст 
в значение [*@3 text].&]
[s3; &]
[s4;%- &]
[s5;:TabCtrl`:`:Insert`(int`,Ctrl`&`,const Image`&`,const char`*`):%- [_^TabCtrl`:`:Item^ T
abCtrl`::Item][@(0.0.255) `&]_[* Insert]([@(0.0.255) int]_[*@3 i], [_^Ctrl^ Ctrl][@(0.0.255) `&
]_[*@3 slave], [@(0.0.255) const]_[_^Image^ Image][@(0.0.255) `&]_[*@3 m], 
[@(0.0.255) const]_[@(0.0.255) char]_`*[*@3 text])&]
[s2; [%- Вставляет новую вкладку в TabCtrl, в 
позиции ][%-*@3 i]. [%-*@3 slave] `- подчинённое 
диалоговое окно, [%-*@3 m] `- показываемый 
во вкладке рисунок, а [%-*@3 text] `- ярлык 
вкладки.&]
[s3; &]
[s4;%- &]
[s5;:TabCtrl`:`:Remove`(int`):%- [@(0.0.255) void]_[* Remove]([@(0.0.255) int]_[*@3 i])&]
[s2; Удаляет вкладку с индексом [%-*@3 i].&]
[s3; &]
[s4;%- &]
[s5;:TabCtrl`:`:GetCount`(`)const:%- [@(0.0.255) int]_[* GetCount]()_[@(0.0.255) const]&]
[s2; Возвращает число вкладок в TabCtrl.&]
[s3; &]
[s4;%- &]
[s5;:TabCtrl`:`:GetTab`(Point`)const:%- [@(0.0.255) int]_[* GetTab]([_^Point^ Point]_[*@3 p])
_[@(0.0.255) const]&]
[s2; Возвращает индекс вкладки, расположенной 
в позиции [%-*@3 p].&]
[s3; &]
[s4;%- &]
[s5;:TabCtrl`:`:GetItem`(int`):%- [_^TabCtrl`:`:Item^ Item][@(0.0.255) `&]_[* GetItem]([@(0.0.255) i
nt]_[*@3 i])&]
[s2; Возвращает ссылку на вкладку [%-*@3 i].&]
[s3; &]
[s4;%- &]
[s5;:TabCtrl`:`:GetItem`(int`)const:%- [@(0.0.255) const]_[_^TabCtrl`:`:Item^ Item][@(0.0.255) `&
]_[* GetItem]([@(0.0.255) int]_[*@3 i])_[@(0.0.255) const]&]
[s2; Возвращает константную ссылку на 
вкладку [%-*@3 i].&]
[s3; &]
[s4;%- &]
[s5;:TabCtrl`:`:Set`(int`):%- [@(0.0.255) void]_[* Set]([@(0.0.255) int]_[*@3 i])&]
[s2; Устанавливает вкладку [*@3 i ]в качестве 
активной. Также можно использовать 
SetData (или operator<<`=).&]
[s3; &]
[s4;%- &]
[s5;:TabCtrl`:`:Get`(`)const:%- [@(0.0.255) int]_[* Get]()_[@(0.0.255) const]&]
[s2; Возвращает индекс активной вкладки. 
Также можно использовать GetData (или 
operator`~).&]
[s3; &]
[s4;%- &]
[s5;:TabCtrl`:`:Find`(const Ctrl`&`)const:%- [@(0.0.255) int]_[* Find]([@(0.0.255) const]_[_^Ctrl^ C
trl][@(0.0.255) `&]_[*@3 slave])_[@(0.0.255) const]&]
[s2; Находит индекс вкладки [%-*@3 slave]. Если 
[%-*@3 slave] отсутствует в TabCtrl, возвращает 
`-1.&]
[s3; &]
[s4;%- &]
[s5;:TabCtrl`:`:Set`(Ctrl`&`):%- [@(0.0.255) void]_[* Set]([_^Ctrl^ Ctrl][@(0.0.255) `&]_[*@3 s
lave])&]
[s2;%- [%RU-RU Устанавливает вкладку с подчинённым 
виджетом ][*@3 slave][%RU-RU  в качестве активной. 
Если] [*@3 slave] отсутствует в TabCtrl, ничего 
не происходит.&]
[s3; &]
[s4;%- &]
[s5;:TabCtrl`:`:IsAt`(Ctrl`&`):%- [@(0.0.255) bool]_[* IsAt]([_^Ctrl^ Ctrl][@(0.0.255) `&]_[*@3 s
lave])&]
[s2; Возвращает true, если [%-*@3 slave] представляет 
собой активную вкладку.&]
[s3; &]
[s4;%- &]
[s5;:TabCtrl`:`:Insert`(Ctrl`&`):%- [_^TabCtrl`:`:Item^ TabCtrl`::Item][@(0.0.255) `&]_[* In
sert]([_^Ctrl^ Ctrl][@(0.0.255) `&]_[*@3 before`_slave])&]
[s5;:TabCtrl`:`:Insert`(Ctrl`&`,const char`*`):%- [_^TabCtrl`:`:Item^ TabCtrl`::Item][@(0.0.255) `&
]_[* Insert]([_^Ctrl^ Ctrl][@(0.0.255) `&]_[*@3 before`_slave], [@(0.0.255) const]_[@(0.0.255) c
har]_`*[*@3 text])&]
[s5;:TabCtrl`:`:Insert`(Ctrl`&`,const Image`&`,const char`*`):%- [_^TabCtrl`:`:Item^ Ta
bCtrl`::Item][@(0.0.255) `&]_[* Insert]([_^Ctrl^ Ctrl][@(0.0.255) `&]_[*@3 before`_slave], 
[@(0.0.255) const]_[_^Image^ Image][@(0.0.255) `&]_[*@3 m], [@(0.0.255) const]_[@(0.0.255) ch
ar]_`*[*@3 text])&]
[s5;:TabCtrl`:`:Insert`(Ctrl`&`,Ctrl`&`,const char`*`):%- [_^TabCtrl`:`:Item^ TabCtrl`:
:Item][@(0.0.255) `&]_[* Insert]([_^Ctrl^ Ctrl][@(0.0.255) `&]_[*@3 before`_slave], 
[_^Ctrl^ Ctrl][@(0.0.255) `&]_[*@3 slave], [@(0.0.255) const]_[@(0.0.255) char]_`*[*@3 text])
&]
[s5;:TabCtrl`:`:Insert`(Ctrl`&`,Ctrl`&`,const Image`&`,const char`*`):%- [_^TabCtrl`:`:Item^ T
abCtrl`::Item][@(0.0.255) `&]_[* Insert]([_^Ctrl^ Ctrl][@(0.0.255) `&]_[*@3 before`_slave], 
[_^Ctrl^ Ctrl][@(0.0.255) `&]_[*@3 slave], [@(0.0.255) const]_[_^Image^ Image][@(0.0.255) `&]_
[*@3 m], [@(0.0.255) const]_[@(0.0.255) char]_`*[*@3 text])&]
[s2; Вставляет новую вкладку перед вкладкой, 
содержащей [%-*@3 before`_slave]. Если [%-*@3 before`_slave] 
отсутствует в TabCtrl, новая вкладка 
добавляется в конце.&]
[s3; &]
[s4;%- &]
[s5;:TabCtrl`:`:Remove`(Ctrl`&`):%- [@(0.0.255) void]_[* Remove]([_^Ctrl^ Ctrl][@(0.0.255) `&
]_[*@3 slave])&]
[s2;%- [%RU-RU Удаляет вкладку с подчинённым 
окном ][*@3 slave][%RU-RU . Если] [*@3 slave] отсутствует 
в TabCtrl, ничего не происходит.&]
[s3; &]
[s4;%- &]
[s5;:TabCtrl`:`:GoNext`(`):%- [@(0.0.255) void]_[* GoNext]()&]
[s2; Устанавливает следующую вкладку 
в качестве активной. Если эта вкладка 
последняя, то устанавливается первая.&]
[s3; &]
[s4;%- &]
[s5;:TabCtrl`:`:GoPrev`(`):%- [@(0.0.255) void]_[* GoPrev]()&]
[s2; Устанавливает предыдущую вкладку 
в качестве активной. Если эта вкладка 
первая, то устанавливается последняя.&]
[s3; &]
[s4;%- &]
[s5;:TabCtrl`:`:ComputeSize`(Size`):%- [_^Size^ Size]_[* ComputeSize]([_^Size^ Size]_[*@3 pan
e])&]
[s2; Вычисляет требуемый наименьший размер 
TabCtrl, чтобы его хватало на клиентскую 
область размером [%-*@3 pane].&]
[s3; &]
[s4;%- &]
[s5;:Upp`:`:TabCtrl`:`:ComputeSize`(`):%- [_^Upp`:`:Size^ Size]_[* ComputeSize]()&]
[s2; Вычисляет требуемый наименьший размер 
TabCtrl, достаточный для клиентской области, 
в которой помещаются все подчинённые 
виджеты, руководствуясь данными от 
функции GetMinSize подчинённых виджетов.&]
[s3;%- &]
[s4;%- &]
[s5;:TabCtrl`:`:Add`(Ctrl`&`):%- [@(0.0.255) void]_[* Add]([_^Ctrl^ Ctrl][@(0.0.255) `&]_[*@3 c
])&]
[s2; Добавляет виджет [%-*@3 c] в клиентскую 
область. В этом случае виджет не принадлежит
 ни одной из вкладок и их переключение 
на него не влияет.&]
[s3; &]
[s4;%- &]
[s5;:TabCtrl`:`:NoAccept`(bool`):%- [_^TabCtrl^ TabCtrl][@(0.0.255) `&]_[* NoAccept]([@(0.0.255) b
ool]_[*@3 ac]_`=_[@(0.0.255) true])&]
[s2; Если активен, метод TabCtrl, Accept, не вызывает 
метод Accept ни у одного из подчинённых 
виджетов.&]
[s3; &]
[s4;%- &]
[s5;:TabCtrl`:`:AcceptCurrent`(bool`):%- [_^TabCtrl^ TabCtrl][@(0.0.255) `&]_[* AcceptCurre
nt]([@(0.0.255) bool]_[*@3 ac]_`=_[@(0.0.255) true])&]
[s2; Если AcceptCurrent активен (дефолтно неактивен), 
метод TabCtrl, Accept, принимает только текущий 
подчинённый виджет, вместо всех их. 
Возвращает `*this.&]
[s3; &]
[s4;%- &]
[s5;:TabCtrl`:`:AcceptAll`(`):%- [_^TabCtrl^ TabCtrl][@(0.0.255) `&]_[* AcceptAll]()&]
[s2; То же, что и AcceptCurrent(false).&]
[s3; &]
[s4;%- &]
[s5;:TabCtrl`:`:SetStyle`(const TabCtrl`:`:Style`&`):%- [_^TabCtrl^ TabCtrl][@(0.0.255) `&
]_[* SetStyle]([@(0.0.255) const]_[_^TabCtrl`:`:Style^ Style][@(0.0.255) `&]_[*@3 s])&]
[s2; Устанавливает стиль контрола вкладки 
в TabCtrl`::Style [%-*@3 s].&]
[s3; &]
[s4;%- &]
[s5;:TabCtrl`:`:Reset`(`):%- [@(0.0.255) void]_[* Reset]()&]
[s2; Сбрасывает TabCtrl в дефолтные настройки.&]
[s3; &]
[s4;%- &]
[s5;:TabCtrl`:`:StyleDefault`(`):%- [@(0.0.255) static] [@(0.0.255) const]_[_^TabCtrl`:`:Style^ S
tyle][@(0.0.255) `&]_[* StyleDefault]()&]
[s2; Возвращает дефолтный стиль контрола 
вкладки.&]
[s3;%- &]
[s0; &]
[s0; &]
[s0;%- &]
[ {{10000@3 [s0;%- [*@(229)4 Класс TabCtrl`::Item]]}}&]
[s3; &]
[s1;:TabCtrl`:`:Item`:`:class:%- [@(0.0.255)3 class][3 _][*3 Item]&]
[s9; Этот гнездовой класс представляет 
свойства индивидуальных вкладок 
в TabCtrl.&]
[s3; &]
[s0; &]
[ {{10000F(128)G(128)@1 [s0; [* Список Публичных Методов]]}}&]
[s3;%- &]
[s5;:TabCtrl`:`:Item`:`:Text`(const String`&`):%- [_^TabCtrl`:`:Item^ Item][@(0.0.255) `&
]_[* Text]([@(0.0.255) const]_[_^String^ String][@(0.0.255) `&]_[*@3 `_text])&]
[s2; Устанавливает ярлык вкладки в текст 
[%-*@3 `_text]. Возвращает `*this.&]
[s3; &]
[s4;%- &]
[s5;:TabCtrl`:`:Item`:`:Picture`(PaintRect`):%- [_^TabCtrl`:`:Item^ Item][@(0.0.255) `&]_
[* Picture]([_^PaintRect^ PaintRect]_[*@3 d])&]
[s2; Устанавливает [%-*@3 d] как видимый контент 
вкладки. Возвращает `*this.&]
[s3; &]
[s4;%- &]
[s5;:TabCtrl`:`:Item`:`:SetImage`(const Image`&`):%- [_^TabCtrl`:`:Item^ Item][@(0.0.255) `&
]_[* SetImage]([@(0.0.255) const]_[_^Image^ UPP`::Image][@(0.0.255) `&]_[*@3 `_im])&]
[s2; Устанавливает рисунок в левой части 
ярлыка вкладки[%- .] Возвращает `*this.&]
[s3; &]
[s4;%- &]
[s5;:TabCtrl`:`:Item`:`:SetCtrl`(Ctrl`*`):%- [_^TabCtrl`:`:Item^ Item][@(0.0.255) `&]_[* Se
tCtrl]([_^Ctrl^ Ctrl]_`*[*@3 `_ctrl])&]
[s2; Устанавливает контрол [%-*@3 ctrl], находящийся 
внутри вкладки header. Возвращает `*this.&]
[s3; &]
[s4;%- &]
[s5;:TabCtrl`:`:Item`:`:SetCtrl`(Ctrl`&`):%- [_^TabCtrl`:`:Item^ Item][@(0.0.255) `&]_[* Se
tCtrl]([_^Ctrl^ Ctrl][@(0.0.255) `&]_[*@3 c])&]
[s2; То же, что и SetCtrl(`&[%-*@3 c]). Возвращает 
`*this.&]
[s3; &]
[s4;%- &]
[s5;:TabCtrl`:`:Item`:`:Slave`(Ctrl`*`):%- [_^TabCtrl`:`:Item^ Item][@(0.0.255) `&]_[* Slav
e]([_^Ctrl^ Ctrl]_`*[*@3 `_slave])&]
[s2; Устанавливает виджет [%-*@3 slave]. Возвращает 
`*this.&]
[s3; &]
[s4;%- &]
[s5;:TabCtrl`:`:Item`:`:Key`(dword`):%- [_^TabCtrl`:`:Item^ Item][@(0.0.255) `&]_[* Key]([_^dword^ d
word]_[*@3 `_key])&]
[s2; Устанавливает ключ [%-*@3 key], активирующий 
вкладку. Возвращает `*this.&]
[s3; &]
[s4;%- &]
[s5;:TabCtrl`:`:Item`:`:Enable`(bool`):%- [_^TabCtrl`:`:Item^ Item][@(0.0.255) `&]_[* Enabl
e]([@(0.0.255) bool]_[*@3 `_en]_`=_[@(0.0.255) true])&]
[s2; Активирует/дезактивирует вкладку. 
Возвращает `*this.&]
[s3; &]
[s4;%- &]
[s5;:TabCtrl`:`:Item`:`:Disable`(`):%- [_^TabCtrl`:`:Item^ Item][@(0.0.255) `&]_[* Disable](
)&]
[s2; То же, что и Enable(false).&]
[s3; &]
[s4;%- &]
[s5;:TabCtrl`:`:Item`:`:IsEnabled`(`)const:%- [@(0.0.255) bool]_[* IsEnabled]()_[@(0.0.255) c
onst]&]
[s2; Возвращает true, если вкладка активирована.&]
[s3; &]
[s4;%- &]
[s5;:TabCtrl`:`:Item`:`:GetSlave`(`):%- [_^Ctrl^ Ctrl]_`*[* GetSlave]()&]
[s5;:TabCtrl`:`:Item`:`:GetSlave`(`)const:%- [@(0.0.255) const]_[_^Ctrl^ Ctrl]_`*[* GetSlav
e]()_[@(0.0.255) const]&]
[s2; Возвращает виджет slave.&]
[s3;%- &]
[s4;%- &]
[s5;:TabCtrl`:`:Item`:`:GetCtrl`(`):%- [_^Ctrl^ Ctrl]_`*[* GetCtrl]()&]
[s5;:TabCtrl`:`:Item`:`:GetCtrl`(`)const:%- [@(0.0.255) const]_[_^Ctrl^ Ctrl]_`*[* GetCtrl](
)_[@(0.0.255) const]&]
[s2; Возвращает контрол, расположенный 
внутри вкладки header.&]
[s3;%- &]
[s4;%- &]
[s5;:Upp`:`:TabCtrl`:`:Item`:`:GetText`(`)const:%- [_^Upp`:`:String^ String]_[* GetText](
)_[@(0.0.255) const]&]
[s2; Возвращает текст, установленный 
посредством Text.&]
[s3;%- &]
[s4;%- &]
[s5;:Upp`:`:TabCtrl`:`:Item`:`:GetPicture`(`)const:%- [_^Upp`:`:PaintRect^ PaintRect]_[* G
etPicture]()_[@(0.0.255) const]&]
[s2; Возвращает PaintRect, установленный посредством
 Picture или SetImage.&]
[s3;%- &]
[s0; ]]