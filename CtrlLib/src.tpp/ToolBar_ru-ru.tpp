topic "Класс ToolBar (БрусИнструментов)";
[i448;a25;kKO9;2 $$1,0#37138531426314131252341829483380:class]
[l288;2 $$2,0#27521748481378242620020725143825:desc]
[0 $$3,0#96390100711032703541132217272105:end]
[H6;0 $$4,0#05600065144404261032431302351956:begin]
[i448;a25;kKO9;2 $$5,0#37138531426314131252341829483370:item]
[l288;a4;*@5;1 $$6,6#70004532496200323422659154056402:requirement]
[l288;i1121;b17;O9;~~~.1408;2 $$7,0#10431211400427159095818037425705:param]
[i448;b42;O9;2 $$8,8#61672508125594000341940100500538:tparam]
[b42;2 $$9,9#13035079074754324216151401829390:normal]
[2 $$0,0#00000000000000000000000000000000:Default]
[{_} 
[ {{10000@3 [s0;%RU-RU [*@(229)4 Класс ToolBar (БрусИнструментов)]]}}&]
[s3; &]
[s1;:ToolBar`:`:class: [@(0.0.255)3 class][3 _][*3 ToolBar][3 _:_][@(0.0.255)3 public][3 _][*@3;3 Ba
rCtrl]&]
[s0;%RU-RU &]
[s2;%RU-RU 
@@image:2300&162
()
&]
[s0;%RU-RU &]
[s9;%RU-RU Этот класс обеспечивает `"панелями 
инструментов`" `- tool`-bars.&]
[s3;%RU-RU &]
[s0; &]
[ {{10000F(128)G(128)@1 [s0;%RU-RU [* Список Публичных Методов]]}}&]
[s3; &]
[s5;:ToolBar`:`:Set`(Callback1`<Bar`&`>`): [@(0.0.255) void]_[* Set]([_^topic`:`/`/Core`/src`/Callbacks`$en`-us`#Callback1`:`:class^ C
allback1][@(0.0.255) <]Bar[@(0.0.255) `&>]_[*@3 bar])&]
[s2;%RU-RU Устанавливает новый контент в 
этот ToolBar. Контент всегда представляется 
как процедура класса Bar `- элементы 
никогда нельзя добавлять прямо (через 
Add). Этот метод нужно вызывать при 
всяком изменении состояния любой 
из кнопок БрусаИнструментов. ToolBar 
содержит умную логику освежения, 
поэтому перерисовываются только 
действительно изменённые элементы.&]
[s3;%RU-RU &]
[s4; &]
[s5;:ToolBar`:`:Post`(Callback1`<Bar`&`>`): [@(0.0.255) void]_[* Post]([_^topic`:`/`/Core`/src`/Callbacks`$en`-us`#Callback1`:`:class^ C
allback1][@(0.0.255) <]Bar[@(0.0.255) `&>]_[*@3 bar])&]
[s2;%RU-RU Декрекирован `- не использовать!.&]
[s3;%RU-RU &]
[s4; &]
[s5;:ToolBar`:`:ButtonMinSize`(Size`): [_^ToolBar^ ToolBar][@(0.0.255) `&]_[* ButtonMinSize
]([_^Size^ Size]_[*@3 sz])&]
[s2;%RU-RU Устанавливает минимальный размер 
стандартной кнопки БрусаИнструментов.&]
[s3;%RU-RU &]
[s4; &]
[s5;:ToolBar`:`:MaxIconSize`(Size`): [_^ToolBar^ ToolBar][@(0.0.255) `&]_[* MaxIconSize]([_^Size^ S
ize]_[*@3 sz])&]
[s2;%RU-RU Устанавливает максимальный размер 
иконки, отображаемой в кнопке БрусаИнструме
нов, в [%-*@3 sz].&]
[s3;%RU-RU &]
[s4; &]
[s5;:ToolBar`:`:StyleDefault`(`): [@(0.0.255) static] [@(0.0.255) const]_[_^ToolBar`:`:Style^ S
tyle][@(0.0.255) `&]_[* StyleDefault]()&]
[s2;%RU-RU Возвращает дефолтный стиль, используемы
й для отрисовки БрусаИнструментов.&]
[s3; &]
[s4; &]
[s5;:ToolBar`:`:SetStyle`(const ToolBar`:`:Style`&`): [_^ToolBar^ ToolBar][@(0.0.255) `&]_
[* SetStyle]([@(0.0.255) const]_[_^ToolBar`:`:Style^ Style][@(0.0.255) `&]_[*@3 s])&]
[s2;%RU-RU Устанавливает визуальный стиль 
в ToolBar`::Style [%-*@3 s].&]
[s3;%RU-RU &]
[s4; &]
[s5;:ToolBar`:`:GetStdHeight`(`): [@(0.0.255) static] [@(0.0.255) int]_[* GetStdHeight]()&]
[s2;%RU-RU Возвращает стандартную высоту 
тулбара, при его использовании в качестве 
фрейма. Это относится только к тулбарам 
с иконками нормального размера.&]
[s3; &]
[s0; &]
[ {{10000@3 [s0;%RU-RU [*@(229)4 Структура ToolBar`::Style]]}}&]
[s3;%RU-RU &]
[s1;:ToolBar`:`:Style`:`:struct: [@(0.0.255)3 struct][3 _][*3 Style][3 _:_][@(0.0.255)3 public][3 _
][*@3;3 ChStyle][@(0.0.255)3 <][*3 Style][@(0.0.255)3 >][3 _]&]
[s9;%RU-RU Эта структура определяет визуальный 
стиль ToolBar`'а.&]
[s3; &]
[s0; ]]