...
bool IsCocoTransparent(const RGBA *s, i32 add, i32 n)
{
	while(n-- > 0) {
		if(s->a > 10)
			return false;
...
Image CocoImg(Color bg, i32 type, i32 value, i32 state)
{
	Size isz(140, 140);
...
Image CocoImg(i32 type, i32 value = 0, i32 state = 0)
{
	Image m[2];
	for(i32 i = 0; i < 2; i++)
		m[i] = CocoImg(i ? Black() : White(), type, value, state);
	Image h = CocoCrop(RecreateAlpha(m[0], m[1]));
	i32 q = h.GetSize().cy / 4;
	SetHotSpots(h, Point(q, q));
	return h;
}

Color CocoColor(i32 k, Color bg = SColorFace())
{
	return AvgColor(CocoImg(bg, COCO_NSCOLOR, k, 0));
}

void SOImages(i32 imli, i32 type, i32 value)
{
	Image h[4];
...
void CocoButton(Image *h, i32 type, i32 value)
{
	h[0] = CocoImg(type, value, CTRL_NORMAL);
	h[1] = CocoImg(type, value, CTRL_HOT); // same as Normal
	h[2] = CocoImg(type, value, CTRL_PRESSED);
...
void CocoButton(Button::Style& s, i32 type, i32 value)
{
	Image h[4];
...
void ChHostSkin()
{
	...
	{
		EditTxt::Style& s = EditTxt::StyleDefault().Write();
	...
	auto nsimg = [](i32 ii) { return CocoImg(COCO_NSIMAGE, ii); };
