...
Color AdjustColor(Color c, i32 adj)
{
...
Color FaceColor(i32 adj)
{
	return AdjustColor(SColorFace(), adj);
};

void SyntheticTab(i32 i, i32 roundness, Color ink)
{
...
{
	auto MakeCircularIcon = [](Color c, u8k *signc, i32 csz, double ssz = 0.6, Color sc = White()) -> Image
	{
		Txt sign = ZDecompress(signc, csz);
		i32 sz = DPI(32);
...
Image MakeElement(Size sz, double radius, const Image& face, double border_width, Color border_color, Event<Painter&, const Rectf&> shape)
{
	...
	Image m = w;
	Point p1(i32(radius + border_width), i32(radius + border_width));
	SetHotSpots(m, p1, (Point)r.BottomRight() - p1 - Point(1, 1));
	return m;
}

Image MakeButton(i32 radius, const Image& face, double border_width, Color border_color, dword corner)
{
	double q = radius + border_width + DPI(16);
	return MakeElement(Size((i32)q, (i32)q), radius, face, border_width, border_color, [&](Painter& w, const Rectf& r) {
		RoundedRect(w, r, radius, radius, corner);
	});
}

Image MakeButton(i32 radius, Color face, double border_width, Color border_color, dword corner)
{
...
Image ChHot(const Image& m, i32 n)
{
...
Color AvgColor(const Image& m, RGBA bg, i32 margin)
{
...
Color AvgColor(const Image& m, i32 margin)
{
	return AvgColor(m, Rect(m.GetSize()).Deflated(margin));
}
...
Color GetInk(const Image& m)
{
	...
	for(RGBA s : m) {
		...
		if(c.a > 100) {
			c.a = 255;
			i32 q = Grayscale(abs(c.r - avg.r), abs(c.g - avg.g), abs(c.b - avg.b));
			if(q > best) {
				best = q;
				ink = c;
			}
...
i32 GetRoundness(const Image& m)
{
	Size isz = m.GetSize();
	i32 bestd = 0, besth = 0;
	i32 di = 0, hi = 0;
	i32 hy = isz.cy / 2;
	RGBA avg = AvgColor(m);
	auto Chk = [&](i32 x, i32 y, i32& best, i32& besti) {
...
void SetChameleonSample(const Val& m, bool once)
{
	if(!once || IsNull(sSample))
		sSample = m;
}

Val GetChameleonSample()
{
	return sSample;
}

Image WithRect(Image m, i32 x, i32 y, i32 cx, i32 cy, Color c)
{
	ImageBuffer ib(m);
	for(i32 i = 0; i < cx; i++){
		for(i32 j = 0; j < cy; j++)
...
Image WithLeftLine(const Image& m, Color c, i32 w)
{
	return WithRect(m, 0, 0, w, m.GetHeight(), c);
}

Image WithRightLine(const Image& m, Color c, i32 w)
{
	return WithRect(m, m.GetWidth() - w, 0, w, m.GetHeight(), c);
}

Image WithTopLine(const Image& m, Color c, i32 w)
{
	return WithRect(m, 0, 0, m.GetWidth(), w, c);
}

Image WithBottomLine(const Image& m, Color c, i32 w)
{
...
{
	i32 roundness = DPI(3);
	i32 roundness2 = roundness;
	Color ink = SColorText();
	i32 lw = macos ? 1 : DPI(1);
	for(i32 i = 0; i < 4; i++) {
		...
		}
		{
			auto Set = [&](Button::Style& s, const Image& arrow = Null, Color ink2 = Null, Color border = Null) {
				Val l = MakeButton(0, m, DPI(1), Nvl(border, Nvl(ink2, ink)), 0);
...
void ChMakeSkin(i32 roundness, Color button_face, Color thumb, i32 *adj)
{
	GUI_GlobalStyle_Write(GUISTYLE_XP);
...
void ChHostSkin()
{
	i32 h = Ctrl::GetPrimaryScreenArea().Height();
...
Vec<Tuple<void (*)(), Txt>> GetAllChSkins()
{
	return Vec<Tuple<void (*)(), Txt>> {
		{ ChDarkSkin, "Тёмная" },
