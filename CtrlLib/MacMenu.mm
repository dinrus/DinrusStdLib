...
{
@public
	drx::Ptr<drx::CocoMenuBar> ptr;
	drx::Event<drx::Bar&>      proc;
}
...
struct CocoMenuBar : public Bar {
	...
	i32       lock = 0;
	bool      dockmenu = false;
	i32       cy = 0; // estimate of height to place the menu correctly
...
CocoMenuBar::Item& CocoMenuBar::Item::Text(tukk text)
{
	Txt h;
	while(*text) {
		if(*text == '&') {
	...
	NSTxt *s = [NSTxt stringWithUTF8Txt:~h];
	nsitem.title = s;
	if(submenu)
...
{
	nsitem.state = check ? NSControlStateValOn : NSControlStateValOff;
	return *this;
}
...
NSMenu *Cocoa_DockMenu() {
	drx::TopWindow *w = drx::TopWindow::GetMenuTopWindow(true);

	if(w && w->WhenDockMenu) {
		static drx::CocoMenuBar bar;
		bar.dockmenu = true;
		bar.Clear();
