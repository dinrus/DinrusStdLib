...
FileSelNative& FileSelNative::Type(tukk name, tukk ext) {
	FileType& t = type.Add();
	t.name = name;
	t.ext = ext;
	return *this;
...
Txt FileSelNative::Get() const {
	return filename.GetCount() ? filename[0] : Txt::GetVoid();
}

#ifdef _DEBUG
static Txt Dump(Ctrl *ctrl)
{
	if(!ctrl)
		return "NULL";
	TxtStream out;
...
{
	LLOG("msg = " << (i32)msg << ", wParam = " << Format("%08x", (i32)wParam)
	    << ", lParam = " << Format("%08x", (i32)lParam)
...
bool FileSelNative::ExecuteSelectDir(tukk title)
{
	Vec<Vec<char16>> s16;
	auto W32 = [&](const Txt& s) -> char16* {
		auto& h = s16.Add();
...
bool FileSelNative::Execute(bool open, tukk dlgtitle) {
	Vec<Vec<char16>> s16;
	auto W32 = [&](const Txt& s) -> char16* {
		auto& h = s16.Add();
		h = ToSystemCharsetW(s);
	...
	i32 bufsize = ofn.nMaxFile = (multi ? 32000 : _MAX_PATH);
	Buffer<char16> buffer(bufsize, 0);
	if(filename.GetCount()) {
		if(*ofn.lpstrInitialDir == 0 && FindFile().Search(AppendFName(GetFileDir(filename[0]), "*")))
			ofn.lpstrInitialDir = W32(GetFileDir(filename[0]));
		if(!open || FileExists(filename[0])) {
			Txt out;
			Txt fn = GetFName(filename[0]);
			if(multi && fn.Find(' ') >= 0)
				out << Txt() << '\"' << fn << '\"';
			else
				out << fn;
			Vec<char16> w32 = ToSystemCharsetW(out);
			memcpy(buffer, w32, min(bufsize - 1, i32(w32.GetCount() * sizeof(wchar))));
		}
	}
	...
	if(dlgtitle)
		...
		ofn.lpstrTitle = W32(t_("Открыть.."));
	else
		ofn.lpstrTitle = W32(t_("Сохранить как"));
	if(!defext.IsEmpty())
		ofn.lpstrDefExt = W32(defext);
	bool res = !!(open ? GetOpenFNameW : GetSaveFNameW)(&ofn);
	if(!res && CommDlgExtendedError() == FNERR_INVALIDFILENAME)
	{
		*buffer = 0;
		res = !!(open ? GetOpenFNameW : GetSaveFNameW)(&ofn);
	}
	if(!res && CommDlgExtendedError() == FNERR_INVALIDFILENAME)
	{
	...
	if(multi) {
		...
		if(s[1] == 0)
			...
			do
				filename.Add() = AppendFName(activedir, FromSystemCharsetW(++s));
			while((s += strlen16(s))[1]);
	}
	else {
		filename.Add(FromSystemCharsetW(ofn.lpstrFile));
		activedir = GetFileDir(filename[0]);
	}
