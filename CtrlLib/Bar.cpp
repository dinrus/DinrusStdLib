...
void BarPane::PaintBar(Draw& w, const SeparatorCtrl::Style& ss, const Val& pane,
                       const Val& iconbar, i32 iconsz)
{
	Size sz = GetSize();
	Rect r = GetSize();
	for(i32 i = 0; i < breakpos.GetCount(); i++)
		if(horz) {
			i32 y = breakpos[i];
			Rect rr = r;
			rr.bottom = y;
...
Size BarPane::LayOut(bool horz, i32 maxsize, bool repos)
{
	auto HoVe = [&](Size& sz)->i32& { return horz ? sz.cx : sz.cy; };
	auto VeHo = [&](Size& sz)->i32& { return horz ? sz.cy : sz.cx; };
	maxsize -= 2 * (horz ? hmargin : vmargin);
	if(repos) {
		this->horz = horz;
		breakpos.Clear();
	}
	i32 bmargin = horz ? vmargin : hmargin;
	Size asz(0, 0);
	VeHo(asz) = bmargin;
	auto q = item.begin();
	while(q != item.end()) {
...
Size BarPane::Repos(bool horz, i32 maxsize)
{
	return LayOut(horz, maxsize, true);
}

Size BarPane::GetPaneSize(bool _horz, i32 maxsize) const
{
...
void BarPane::Add(Ctrl *ctrl, i32 gapsize)
{
	Item& m = item.Add();
	m.ctrl = ctrl;
	m.gapsize = gapsize;
...
Bar::Item& Bar::Item::Text(tukk text)        { return *this; }
Bar::Item& Bar::Item::Key(dword key)                { return *this; }
Bar::Item& Bar::Item::Repeat(bool b)                { return *this; }
Bar::Item& Bar::Item::Image(const DRX::Image& img)  { return *this; }
Bar::Item& Bar::Item::Bold(bool bold)               { return *this; }
Bar::Item& Bar::Item::Enable(bool _enable)          { return *this; }
Bar::Item& Bar::Item::Tip(tukk tip)          { return *this; }
Bar::Item& Bar::Item::Help(tukk help)        { return *this; }
Bar::Item& Bar::Item::Topic(tukk topic)      { return *this; }
Bar::Item& Bar::Item::Description(tukk desc) { return *this; }
Bar::Item& Bar::Item::Check(bool check)             { return *this; }
Bar::Item& Bar::Item::Radio(bool check)             { return *this; }
void       Bar::Item::FinalSync() {}

Bar::Item& Bar::Item::Label(tukk text)
{
...
Bar::Item& Bar::Item::RightLabel(tukk text)
{
	ToolButton *b = dynamic_cast<ToolButton *>(this);
...
{
	AddCtrl(&ctrl, (i32)Null);
}

Bar::Item&  Bar::Add(bool enable, tukk text, const DRX::Image& image, const Callback& callback)
{
	return AddItem(callback).Text(text).Image(image).Enable(enable);
}

Bar::Item&  Bar::Add(bool enable, KeyInfo& (*key)(), const DRX::Image& image, const Callback& callback)
{
	tukk text = (*key)().name;
	if(*text == '\3')
...
Bar::Item&  Bar::Add(tukk text, const DRX::Image& image, const Callback& callback)
{
	return AddItem(callback).Text(text).Image(image);
}
/*
Bar::Item& Bar::Add(const Txt& text, const DRX::Image& image, const Event<> & callback)
{
	return Add(~text, image, callback);
}
*/
Bar::Item&  Bar::Add(KeyInfo& (*key)(), const DRX::Image& image, const Callback& callback)
{
	tukk text = (*key)().name;
	if(*text == '\3')
		text = t_GetLngTxt(text + 1);
...
Bar::Item&  Bar::Add(bool enable, tukk text, const Callback & callback)
{ return IsMenuBar() ? Add(enable, text, Image(), callback) : NilItem(); }

Bar::Item&  Bar::Add(tukk text, const Callback & callback)
...
Bar::Item& Bar::Add(bool enable, tukk text, const DRX::Image& image, const DRX::Function<void ()>& fn)
{
	return Add(enable, text, image, Callback() << fn);
}

Bar::Item& Bar::Add(bool enable, KeyInfo& (*key)(), const DRX::Image& image, const Function<void ()>& fn)
{
	return Add(enable, key, image, Callback() << fn);
}

Bar::Item& Bar::Add(tukk text, const DRX::Image& image, const Function<void ()>& fn)
{
	return Add(text, image, Callback() << fn);
}

/*
Bar::Item& Bar::Add(const Txt& text, const DRX::Image& image, const Function<void ()>& fn)
{
	return Add(text, image, Event<> () << fn);
}
*/

Bar::Item& Bar::Add(KeyInfo& (*key)(), const DRX::Image& image, const Function<void ()>& fn)
{
	return Add(key, image, Callback() << fn);
}

Bar::Item& Bar::Add(bool enable, tukk text, const Function<void ()>& fn)
{
...
Bar::Item& Bar::Add(tukk text, const Function<void ()>& fn)
{
...
void Bar::Break()
{
	AddCtrl(NULL, (i32)Null);
}

void Bar::Gap(i32 size)
{
	AddCtrl(NULL, size);
}
...
void   Bar::MenuGap(i32 size)            { if(IsMenuBar()) Gap(size); }
void   Bar::AddMenu(Ctrl& ctrl)          { if(IsMenuBar()) Add(ctrl); }
void   Bar::AddMenu(Ctrl& ctrl, Size sz) { if(IsMenuBar()) Add(ctrl, sz); }

Bar::Item&  Bar::AddMenu(bool enable, KeyInfo& (*key)(), const DRX::Image& image, const Callback & callback)
{ return IsMenuBar() ? Add(enable, key, image, callback) : NilItem(); }

Bar::Item&  Bar::AddMenu(KeyInfo& (*key)(), const DRX::Image& image, const Callback & callback)
{ return IsMenuBar() ? Add(key, image, callback) : NilItem(); }

Bar::Item&  Bar::AddMenu(bool enable, tukk text, const DRX::Image& image, const Callback & callback)
{ return IsMenuBar() ? Add(enable, text, image, callback) : NilItem(); }

Bar::Item&  Bar::AddMenu(tukk text, const DRX::Image& image, const Callback & callback)
{ return IsMenuBar() ? Add(text, image, callback) : NilItem(); }


Bar::Item& Bar::AddMenu(bool enable, tukk text, const DRX::Image& image, const Function<void ()>& fn)
{
	return AddMenu(enable, text, image, Callback() << fn);
}

Bar::Item& Bar::AddMenu(bool enable, KeyInfo& (*key)(), const DRX::Image& image, const Function<void ()>& fn)
{
	return AddMenu(enable, key, image, Callback () << fn);
}

Bar::Item& Bar::AddMenu(tukk text, const DRX::Image& image, const Function<void ()>& fn)
{
	return AddMenu(text, image, Callback () << fn);
}

Bar::Item& Bar::AddMenu(KeyInfo& (*key)(), const DRX::Image& m, const Function<void ()>& fn)
{
	return AddMenu(key, m, Callback () << fn);
}

Bar::Item& Bar::Add(bool enable, tukk text, const Callback1<Bar&>& proc)
{ return AddSubMenu(proc).Text(text).Enable(enable); }

Bar::Item& Bar::Add(tukk text, const Callback1<Bar&>& proc)
{ return Add(true, text, proc); }

Bar::Item& Bar::Add(bool enable, tukk text, const DRX::Image& image, const Callback1<Bar&>& proc)
{ return Add(enable, text, proc).Image(image); }

Bar::Item& Bar::Add(tukk text, const DRX::Image& image, const Callback1<Bar&>& proc)
{ return Add(text, proc).Image(image); }

Bar::Item& Bar::Sub(bool enable, tukk text, const Function<void (Bar&)>& submenu)
{
	return Add(enable, text, Callback1<Bar&>() << submenu);
}

Bar::Item& Bar::Sub(tukk text, const Function<void (Bar&)>& submenu)
{
	return Add(text, Callback1<Bar&>() << submenu);
}

Bar::Item& Bar::Sub(bool enable, tukk text, const DRX::Image& image, const Function<void (Bar&)>& submenu)
{
	return Add(enable, text, image, Callback1<Bar&>() << submenu);
}

Bar::Item& Bar::Sub(tukk text, const DRX::Image& image, const Function<void (Bar&)>& submenu)
{
...
void   Bar::ToolGap(i32 size)              { if(IsToolBar()) Gap(size); }
...
Bar::Item&  Bar::Add(bool enable, const DRX::Image& image, Event<>  cb)
{ return IsToolBar() ? AddItem(cb).Image(image).Enable(enable) : NilItem(); }

Bar::Item&  Bar::Add(const DRX::Image& image, Event<>  cb)
...
class Bar::ScanKeys : public Bar {
	struct KeyItem : public Bar::Item {
		...
		virtual Item& Text(tukk )             { return *this; }
		...
		virtual Item& Image(const DRX::Image& img)   { return *this;  }
		virtual Item& Enable(bool _enable = true)    { enabled = _enable; return *this; }
		virtual Item& Tip(tukk tip)           { return *this; }
		virtual Item& Help(tukk help)         { return *this; }
		virtual Item& Topic(tukk help)        { return *this; }
		virtual Item& Description(tukk help)  { return *this; }
	};
...
Val BarCtrl::GetBackground() const
{
	return Null;
}
...
void BarCtrl::AddCtrl(Ctrl *ctrl, i32 gapsize)
{
	pane.Add(ctrl, gapsize);
}

BarCtrl& BarCtrl::Align(i32 al) {
	align = al;
	SyncBar();
	return *this;
}
...
void BarCtrl::PaintBar(Draw& w, const SeparatorCtrl::Style& ss, const Val& p, const Val& iconbar, i32 iconsz)
{
...
void LRUList::Select(Txt f, Event<const Txt&> WhenSelect)
{
	WhenSelect(f);
}

void LRUList::operator()(Bar& bar, Event<const Txt&> WhenSelect, i32 count, i32 from)
{
	if(bar.IsMenuBar() && !lru.IsEmpty()) {
		bar.Separator();
		char n = '1';
		for(i32 i = from; i < min(from + count, lru.GetCount()); i++) {
			bar.Add(Txt("&") + n + ' ' + GetFName(lru[i]),
			        OTVET2(Select, lru[i], WhenSelect));
			n = n == '9' ? 'A' : n + 1;
		}
	}
}

void LRUList::NewEntry(const Txt& path)
{
	LruAdd(lru, path, limit);
}

void LRUList::RemoveEntry(const Txt& path)
{
	i32 q = FindIndex(lru, path);
	if(q >= 0)
		lru.Remove(q);
}
