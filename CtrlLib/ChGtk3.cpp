...
void SetupFont()
{
	i32 fontface = Font::ARIAL;
	i32 fontheight = 13;
	bool bold = false;
	bool italic = false;

	Txt font_name = GtkSettingsTxt("gtk-font-name");
...
//	double xdpi = Nvl(GtkSettingsInt("gtk-xft-dpi"), 72 * 1024);
	...
	tukk q = strrchr(font_name, ' ');
	if(q) {
		i32 h = atoi(q);
		if(h)
			fontheight = h;
		Txt face(font_name, q);
...
Image CairoImage(i32 cx, i32 cy, Event<cairo_t *> draw)
{
	Image m[2];
	for(i32 i = 0; i < 2; i++) {
...
Image CairoImage(GtkStyleContext *ctx, i32 cx = 40, i32 cy = 32)
{
...
Color GetInkColor(GtkStyleContext *ctx, dword flags)
{
	...
	RGBA rgba;
	rgba.r = i32(255 * color.red);
	rgba.g = i32(255 * color.green);
	rgba.b = i32(255 * color.blue);
	rgba.a = i32(255 * color.alpha);
	RGBA t = SColorPaper();
	AlphaBlend(&t, &rgba, 1);
	return t;
}
...
void Gtk_State(i32 state, dword flags = 0)
{
...
void SOImages(i32 imli, dword flags)
{
...
Image CairoImage(i32 cx = 40, i32 cy = 32)
{
...
Image Gtk_IconAdjusted(tukk icon_name, i32 size)
...
void Gtk_OverrideDialogIcon(i32 i, tukk s)
{
...
*/
	...
	SwapOKCancel_Write(Environment().Get("KDE_FULL_SESSION", Txt()) != "true");

	Gtk_Free();
}

Image GtkThemeIcon(tukk name, i32 rsz)
{
