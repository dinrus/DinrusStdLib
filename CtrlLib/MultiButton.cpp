...
MultiButton::SubButton& MultiButton::SubButton::SetLabel(tukk text)
{
	label = text;
	Refresh();
	return *this;
...
MultiButton::SubButton& MultiButton::SubButton::Width(i32 w)
{
	cx = w;
	Refresh();
	return *this;
...
MultiButton::SubButton& MultiButton::InsertButton(i32 i)
{
	MultiButtons();
...
void MultiButton::RemoveButton(i32 i)
{
	MultiButtons();
	buttons.Remove(i);
}
...
i32 MultiButton::GetButtonCount() const
{
	if(droppush)
		return 1;
...
MultiButton::SubButton& MultiButton::Button(i32 i) const
{
	if(droppush) {
		static SubButton b;
		b.main = true;
...
const MultiButton::SubButton& MultiButton::GetButton(i32 i) const
{
	return Button(i);
}

MultiButton::SubButton& MultiButton::GetButton(i32 i)
{
	MultiButtons();
	return Button(i);
}
...
{
	for(i32 i = 0; i < GetButtonCount(); i++) {
...
{
	if(b && owner)
		for(i32 i = 0; i < owner->GetButtonCount(); i++)
...
i32 MultiButton::OverPaint() const
{
...
bool MultiButton::Metrics(i32& border, i32& lx, i32 &rx, const Rect& r)
{
...
bool MultiButton::Metrics(i32& border, i32& lx, i32 &rx)
{
	return Metrics(border, lx, rx, GetSize());
}

i32 MultiButton::FindButton(i32 px)
{
	if(IsReadOnly())
		return Null;
...
bool MultiButton::GetPos(SubButton& b, i32& lx, i32& rx, i32& x, i32& cx, i32 px)
{
...
void MultiButton::GetPos(i32 ii, i32& x, i32& cx)
{
	i32 border, lx, rx;
	Metrics(border, lx, rx);
	x = cx = 0;
...
void MultiButton::GetLR(i32& lx, i32& rx)
{
	i32 border;
	Metrics(border, lx, rx);
	i32 x = 0;
...
Rect MultiButton::Paint0(Draw& w, bool getcr)
{
	...
	for(i32 i = 0; i < GetButtonCount(); i++) {
		...
		if(getcr)
			continue;
		bool dopaint = true;
		Val v = b.left ? left ? style->lmiddle[st] : style->left[st]
...
{
	i32 h = FindButton(p.x);
	Txt tip = GetTextAttr(ATTR_TIP);
...
void MultiButton::LeftDown(Point p, dword flags)
{
	...
	if(hl == NONE)
		...
		if(hl == MAIN)
			...
			i32 x, cx;
			i32 border = IsTrivial() ? style->trivialborder : style->border;
			GetPos(hl, x, cx);
...
MultiButton& MultiButton::Set(const Val& v, bool update)
{
	if(value != v) {
		value = v;
		if(update)
...
MultiButton& MultiButton::SetValCy(i32 cy)
{
	if(cy != valuecy) {
		valuecy = cy;
		Refresh();
...
void MultiButton::SetData(const Val& v)
{
	if(value != v) {
		value = v;
		UpdateRefresh();
...
Val MultiButton::GetData() const
{
	return value;
}

void MultiButton::PseudoPush(i32 bi)
{
	hl = bi;
	push = true;
	DoPush(bi);
