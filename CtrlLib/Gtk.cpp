...
bool FileSelNative::Execute0(i32 mode, tukk title)
{
	Ctrl::ReleaseCtrlCapture();
	if(!title)
		title = decode(mode, 1, t_("Открыть"), 0, t_("Сохранить как"), t_("Выбрать папку"));
	...
	GtkWidget *fc = gtk_file_chooser_dialog_new(title, w ? w->gtk() : NULL,
	                                            ...
	                                            t_("Отмена"), GTK_RESPONSE_CANCEL,
	                                            decode(mode, 1, t_("Открыть"), 0, t_("Сохранить"), t_("Выбрать")),
	                                            GTK_RESPONSE_OK,
	                                            NULL);
	...
	if(IsFullPath(ipath)) {
		...
		if(ff)
			...
			gtk_file_chooser_set_current_name(GTK_FILE_CHOOSER(fc), GetFName(ipath));
		}
	}
	else
