...
Bar::Item& MenuItemBase::Text(tukk s)
{
...
Bar::Item& MenuItemBase::Image(const DRX::Image& img)
{
	return *this;
}
...
Bar::Item& MenuItemBase::Tip(tukk s)
{
	return *this;
}

Bar::Item& MenuItemBase::Help(tukk s)
{
	HelpLine(s);
	return *this;
}

Bar::Item& MenuItemBase::Description(tukk s)
{
	Ctrl::Description(s);
	return *this;
}

Bar::Item& MenuItemBase::Topic(tukk s)
{
	HelpTopic(s);
	return *this;
}
...
Txt MenuItemBase::GetDesc() const
{
	return text;
}
...
void DrawMnemonicText(Draw& w, i32 x, i32 y, const Txt& s, Font font, Color color,
                      i32 mnemonic, bool menumark)
{
	i32 apos = HIWORD(mnemonic);
	i32 q;
...
void DrawMenuText(Draw& w, i32 x, i32 y, const Txt& s, Font f, bool enabled,
                  bool hl, i32 mnemonic, Color color, Color hlcolor, bool menumark)
{
	if(enabled)
...
void MenuItemBase::DrawMenuText(Draw& w, i32 x, i32 y, const Txt& s, Font f, bool enabled,
                                bool hl, Color color, Color hlcolor)
{
	DRX::DrawMenuText(w, x, y, s, f, enabled, hl, VisibleAccessKeys() ? accesskey : 0,
	                  color, hlcolor, InOpaqueBar());
}

void MenuItemBase::PaintTopItem(Draw& w, i32 state) {
	Size sz = GetSize();
	Txt text = GetText();
...
Bar::Item& MenuItem::Image(const DRX::Image& img)
{
	licon = img;
	Refresh();
	return *this;
...
MenuItem& MenuItem::RightImage(const DRX::Image& img)
{
	ricon = img;
	Refresh();
	return *this;
...
void MenuItem::Paint(Draw& w)
{
	...
	Size isz = li.GetSize();
	i32 iy = (sz.cy - isz.cy) / 2;
	bool chk = false;
	i32 x = max(Zx(3), (leftgap + textgap - isz.cx) / 2);
	...
	if(!IsEmpty(keydesc)) {
		...
		DRX::DrawMenuText(w, x - isz.cx - Zx(2), (sz.cy - isz.cy) / 2, keydesc, font, isenabled, hl,
...
bool MenuItem::Key(dword key, i32 count)
{
...
void SubMenuItem::MouseEnter(Point, dword)
{
	...
	if(!menu.IsOpen() && isenabled)
		SetTimeCallback(400, OTVET(Pull), TIMEID_PULL);
}
...
i32 SubMenuItem::GetVisualState()
{
...
bool SubMenuItem::Key(dword key, i32 count)
{
...
bool TopSubMenuItem::Key(dword key, i32) {
...
bool TopMenuItem::Key(dword key, i32 count)
{
...
i32 TopMenuItem::GetStdHeight(Font font)
{
