...
StaticText& StaticText::SetAlign(i32 align)
{
...
StaticText& StaticText::SetVAlign(i32 align)
{
...
StaticText& StaticText::SetImage(const Image& img, i32 spc)
{
...
StaticText& StaticText::SetText(tukk s)
{
	text = s;
	Refresh();
	return *this;
...
Label& Label::SetLabel(tukk _text)
{
	Txt text;
...
void LabelBox::Paint(Draw& w)
{
	...
	i32 d = lsz.cy >> 1;
	i32 ty = sz.cy < 2 * Draw::GetStdFontCy() ? (sz.cy - lsz.cy) / 2 : 0;
	DrawLabel l;
	MakeDrawLabel(l);
...
StaticRect& StaticRect::Background(const Val& chvalue)
{
	if(chvalue != bg) {
		bg = chvalue;
		Refresh();
...
SeparatorCtrl& SeparatorCtrl::Margin(i32 l, i32 r)
{
...
SeparatorCtrl& SeparatorCtrl::SetSize(i32 w)
{
	if(w != size) {
		size = w;
		Refresh();
...
void DisplayCtrl::SetData(const Val& v)
{
	pr.SetVal(v);
	Refresh();
}

Val DisplayCtrl::GetData() const
{
	return pr.GetVal();
}
