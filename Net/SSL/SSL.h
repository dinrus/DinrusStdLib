...
public:
	...
	bool     Grow(i32 length);

	Txt      Get() const;
	bool     Set(const Txt& d);
...
public:
	...
	bool     OpenBuffer(tukk data, i32 length);
	bool     CreateBuffer();
	Txt      GetResult() const;
...
public:
	...
	bool     Load(const Txt& data, bool asn1 = false);
	Txt      Save(bool asn1 = false) const;

	Txt      GetSubjectName() const;
	Txt      GetIssuerName() const;
	Date     GetNotBefore() const;
	Date     GetNotAfter() const;
	i32      GetVersion() const;
	Txt      GetSerialNumber() const;
...
public:
	...
	bool     CipherList(tukk list);
	bool     UseCertificate(Txt certificate, Txt private_key, bool cert_asn1 = false);
	void     VerifyPeer(bool verify = true, i32 depth = 2);
