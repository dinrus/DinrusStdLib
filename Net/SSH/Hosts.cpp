...
bool SshHosts::Add(const Txt& host, i32 port, const Info& info, const Txt& comment)
{
	return Add(Format("[%s]:%d", host, port), info, comment);
}

bool SshHosts::Add(const Txt& host, const Info& info, const Txt& comment)
{
	ASSERT(ssh_session);
	Clear();
	bool b = handle &&
...
bool SshHosts::Load(const Txt& filename)
{
	ASSERT(ssh_session);
	Clear();
	file_path = filename;
...
bool SshHosts::SaveAs(const Txt& filename)
{
	ASSERT(ssh_session);
	Clear();
...
SshHosts::Info SshHosts::Check(const Txt& host, i32 port)
{
	ASSERT(ssh_session);
	Clear();
	Info info;
...
Vec<SshHost*> SshHosts::GetHosts()
{
	ASSERT(ssh_session);
	Clear();
	Vec<SshHost*> v;
	SshHost *prev = nullptr, *next = nullptr;
	i32 rc = libssh2_knownhost_get(handle, &prev, nullptr);
	if(rc >= 0) {
		v.Add(prev);
		if(rc == 0)
			while(rc < 1) {
...
bool SshHosts::Error()
{
	...
	i32 rc = libssh2_session_last_error(ssh_session, libmsg, nullptr, 0);
	error.a = rc;
	error.b = Txt(*libmsg);
	LLOG("Failed. " << error.b);
	return false;
