...
SqlId SqlId::Of(tukk of) const
{
	return Txt().Cat() << of << (char)SQLC_OF << ToTxt();
}

SqlId SqlId::Of(SqlId id) const
{
	return id.IsNull() ? ToTxt() : Txt().Cat() << id.ToTxt() << (char)SQLC_OF << ToTxt();
}

void SqlId::PutOf0(Txt& s, const SqlId& b) const
{
	s << ToTxt() << (char)SQLC_OF << ~b;
}

void SqlId::PutOf(Txt& s, const SqlId& b) const
{
	s << (char)SQLC_COMMA;
	PutOf0(s, b);
}
...
SqlId SqlId::As(tukk as) const
{
	return id.IsNull() ? ToTxt() : Txt().Cat() << ToTxt() << (char)SQLC_AS << as;
}
...
SqlId SqlId::operator [] (i32 i) const
{
	return ToTxt() + FormatInt(i);
}

SqlId SqlId::operator&(const SqlId& s) const
{
	return ToTxt() + "$" + s.ToTxt();
}

SqlId SqlId::operator()(const S_info& table) const
{
	Txt x;
	for(i32 i = 0; i < table.GetCount(); i++)
		if(i)
...
Txt SqlS::operator()() const
{
	return Txt().Cat() << '(' << text << ')';
}

Txt SqlS::operator()(i32 at) const
{
	return at > priority ? operator()() : text;
}

Txt SqlS::operator()(i32 at, u8 cond) const
{
	if(at <= priority)
		return text;
	TxtBuf out;
...
void SqlS::Init(const SqlS& a, tukk o, i32 olen, const SqlS& b, i32 pr, i32 prb)
{
	TxtBuf s;
	if(a.priority < pr) {
		s.Cat('(');
...
SqlVal SqlVal::As(tukk as) const {
	SqlVal v;
	v.SetHigh(Txt().Cat() << text << (char)SQLC_AS << '\t' << as << '\t');
	return v;
}
...
SqlVal SqlFunc(tukk name, const SqlVal& a) {
	return SqlVal(Txt().Cat() << name << '(' << ~a << ')', SqlS::FN);
}

SqlVal SqlFunc(tukk n, const SqlVal& a, const SqlVal& b) {
	return SqlVal(Txt(n).Cat() << '(' + ~a << ", " << ~b << ')', SqlS::FN);
}

SqlVal SqlFunc(tukk n, const SqlVal& a, const SqlVal& b, const SqlVal& c) {
	return SqlVal(Txt(n).Cat() << '(' << ~a << ", " << ~b << ", " << ~c << ')', SqlS::FN);
}

SqlVal SqlFunc(tukk n, const SqlVal& a, const SqlVal& b, const SqlVal& c, const SqlVal& d) {
	return SqlVal(Txt(n).Cat() << '(' << ~a << ", " << ~b << ", " << ~c << ", " << ~d << ')', SqlS::FN);
}
...
SqlBool SqlBoolFunc(tukk name, const SqlBool& a) {
	return SqlBool(Txt().Cat() << name << '(' << ~a << ')', SqlS::FN);
}

SqlBool SqlBoolFunc(tukk n, const SqlBool& a, const SqlBool& b) {
	return SqlBool(Txt(n).Cat() << '(' + ~a << ", " << ~b << ')', SqlS::FN);
}

SqlBool SqlBoolFunc(tukk n, const SqlBool& a, const SqlBool& b, const SqlBool& c) {
	return SqlBool(Txt(n).Cat() << '(' << ~a << ", " << ~b << ", " << ~c << ')', SqlS::FN);
}

SqlBool SqlBoolFunc(tukk n, const SqlBool& a, const SqlBool& b, const SqlBool& c, const SqlBool& d) {
	return SqlBool(Txt(n).Cat() << '(' << ~a << ", " << ~b << ", " << ~c << ", " << ~d << ')', SqlS::FN);
}

SqlVal Decode(const SqlVal& exp, const SqlSet& variants) {
	ASSERT(!variants.IsEmpty());
	Vec<SqlVal> v = variants.Split();
	ASSERT(v.GetCount() > 1);
...
SqlVal ConvertAscii(const SqlVal& exp) {
	return SqlVal(SqlCode(MSSQL, Txt().Cat() << "((" << ~exp << ") collate SQL_Latin1_General_Cp1251_CS_AS)")
