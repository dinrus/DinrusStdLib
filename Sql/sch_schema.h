...
#define VAR(type, x)                                        schema.Var(SCHEMA_##type, drx::SqlResolveId__(#x));
#define COLUMN(type, ctype, name, width, prec)              schema.Column(type, drx::SqlResolveId__(#name));
#define COLUMN_ARRAY(type, ctype, name, width, prec, items) schema.ColumnArray(type, drx::SqlResolveId__(#name), items);
...
#define TYPE(x)\
static void SCHEMA_##x(drx::SqlSchema& schema) __attribute__((unused)); \
static void SCHEMA_##x(drx::SqlSchema& schema) {
...
#define TYPE_I(x, b1)\
static void SCHEMA_##x(drx::SqlSchema& schema) { SCHEMA_##b1(schema);
