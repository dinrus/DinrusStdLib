...
bool StdStatementExecutor::Execute(const Txt& stmt)
{
	cursor.Execute(stmt);
	return true;
}
...
struct SQLStatementExecutorClass : StatementExecutor {
	virtual bool Execute(const Txt& stmt) { SQL.Execute(stmt); return true; }
};
...
bool SqlPerformScript(SqlSession& session, Stream& script,
                      Gate<i32, i32> progress_canceled, bool stoponerror)
{
	Txt stmt;
	i32 level = 0;
	bool ok = true;
