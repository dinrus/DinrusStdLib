...
[s24; Пример для этого раздела будет ориентирован
 на использование PostgreSQL. Смотрите 
пакеты примеров работы с SQL, предложенные 
в примерах для Upp, чтобы также научиться 
использовать MySQL и SQLite.&]
[s23; Файл описания схемы Schema (.sch file)&]
[s24; В каждом файле описания схемы приводится 
таблица и выкладка колонок базы данных. 
&]
[s25; Пример Postgresql (`"person`_db.sch`"):&]
[s7; TABLE`_ (PERSON)&]
...
[s7; END`_TABLE&]
[s24; В этой схеме описаны таблицы `'person`' 
и `'employee`', с соотношением [_ суррогатного 
ключа ]1 к 1, где `" employee является person`". 
&]
[s24; Различные типы, упоминаемые в этом 
примере, маппируются в типы SQL. Больше 
информации можно извлечь из заголовков 
исходного кода, касающихся типов 
баз данных. В панном примере все умонинаемые
 типы можно найти в файле `"PostgreSQLSchema.h`" 
из пакета `"PostgreSQL`".&]
[s24; У каждого объявления типа есть  2 
варианта; лдтн с подчерком `"`_`", а другой 
без. Когда используется подчерк, автоматичес
ки создаётся объект [C SqlId, который используетс
я как переменная в файлах`-исходниках]. 
When not used, you must manually define the [C SqlID] object in 
your source. Reference the [_^topic`:`/`/ACWarehouseIO`/sql`_info`/BasicUse`$ru`-ru`#SqlId Objects^ S
