...
struct FieldOperator {
	...
	void Table(tukk name)                           { table = name; }

	virtual void Field(Ref f);
	virtual void Field(tukk name, Ref f);
	virtual void Field(tukk name, Ref f, bool *b); // Ugly fix to resolve bool issue with S_info creation

	FieldOperator& operator()(tukk name, Ref f)     { Field(name, f, NULL); return *this; }
	FieldOperator& operator()(const Txt& name, Ref f)   { Field(name, f, NULL); return *this; }
	FieldOperator& operator()(Id id, Ref f)                { Field(~id, f); return *this; }
	FieldOperator& operator()(tukk name, bool& b);
	FieldOperator& operator()(const Txt& name, bool& b) { return (*this)(~name, b); }
	FieldOperator& operator()(Id id, bool& b)              { return (*this)(~id, b); }
	
	virtual void Width(i32 width); // Only to be used for S_info
...
public:
	SqlCode(u8 cond, const Txt& text);
	SqlCode operator()(u8 cond, const Txt& text);

	Txt operator()(const Txt& text);
	Txt operator()();
};
...
private:
	void PutOf0(Txt& s, const SqlId& b) const;
	void PutOf(Txt& s, const SqlId& b) const;
...
public:
	...
	const Txt& ToTxt() const               { return id.ToTxt(); }
	const Txt& operator~() const              { return ToTxt(); }
	Txt        Quoted() const;

	SqlId         Of(SqlId id) const;
	SqlId         Of(tukk of) const;
	SqlId         As(tukk as) const;
	SqlId         As(SqlId id) const             { return As(~id.ToTxt()); }
	SqlId         operator[](i32 i) const; // deprecated
...
//$+
	SqlId()                                      {}
	SqlId(tukk s) : id(s)                 {}
	SqlId(const Txt& s) : id(s)               {}
	SqlId(const Id& id) : id(id)                 {}
};

typedef SqlId SqlCol; // Deprecated

#define SQLID(x)     const DRX::SqlId x(#x);
#define SQL_ID(n, x) const DRX::SqlId n(#x);
...
protected:
	...
	void   Init(const SqlS& a, tukk o, i32 olen, const SqlS& b, i32 pr, i32 prb);
...
public:
	...
	Txt        operator()() const;
	Txt        operator()(i32 at) const;
	Txt        operator()(i32 at, u8 cond) const;
	const Txt& operator~() const         { return text; }
	bool          IsEmpty() const           { return priority == EMPTY; }

	SqlS()                                  : priority(EMPTY) {}
	SqlS(tukk s, i32 pr)             : text(s), priority(pr) {}
	SqlS(const Txt& s, i32 pr)           : text(s), priority(pr) {}
	
	force_inline SqlS(const SqlS& a, tukk op, const SqlS& b, i32 pr, i32 prb) { Init(a, op, (i32)strlen(op), b, pr, prb); }
	force_inline SqlS(const SqlS& a, tukk op, const SqlS& b, i32 pr)          { Init(a, op, (i32)strlen(op), b, pr, pr); }
};

class SqlVal : public SqlS, Moveable<SqlVal> {
public:
	void SetNull()                          { text = "NULL"; priority = NULLVAL; }
	void SetHigh(const Txt& s)           { text = s; priority = HIGH; }
	bool IsNull() const                     { return priority == NULLVAL; }

	SqlVal As(tukk as) const;
	SqlVal As(const SqlId& id) const;

	SqlVal()                                {}
	SqlVal(const Txt& s, i32 pr)         : SqlS(s, pr) {}
	SqlVal(const SqlS& a, tukk o, const SqlS& b, i32 pa, i32 pb)
		                                    : SqlS(a, o, b, pa, pb) {}
	SqlVal(const SqlS& a, tukk o, const SqlS& b, i32 p)
		                                    : SqlS(a, o, b, p) {}
...
SqlVal operator/(const SqlVal& a, const SqlVal& b);
SqlVal operator%(const SqlVal& a, const SqlVal& b);
SqlVal operator|(const SqlVal& a, const SqlVal& b); // Txt concatenation
...
SqlVal SqlFunc(tukk name, const SqlVal& a);
SqlVal SqlFunc(tukk name, const SqlVal& a, const SqlVal& b);
SqlVal SqlFunc(tukk name, const SqlVal& a, const SqlVal& b, const SqlVal& c);
SqlVal SqlFunc(tukk name, const SqlVal& a, const SqlVal& b, const SqlVal& c, const SqlVal& d);
SqlVal SqlFunc(tukk name, const SqlSet& set);

SqlBool SqlBoolFunc(tukk name, const SqlBool& a);
SqlBool SqlBoolFunc(tukk name, const SqlBool& a, const SqlBool& b);
SqlBool SqlBoolFunc(tukk name, const SqlBool& a, const SqlBool& b, const SqlBool& c);
SqlBool SqlBoolFunc(tukk name, const SqlBool& a, const SqlBool& b, const SqlBool& c, const SqlBool& d);
...
SqlVal Cast(tukk type, const SqlId& a);
...
SqlVal SqlTxt(tukk s);

SqlVal SqlRowNum();

SqlVal SqlArg();

SqlVal SqlBinary(const Txt& data);
...
public:
	...
	SqlVal   AsVal() const                { return SqlVal(text, LOW); }
	SqlVal   As(tukk as) const       { return AsVal().As(as); }
	SqlVal   As(const SqlId& id) const      { return AsVal().As(id); }

	SqlBool(const Txt& s, i32 pr)        : SqlS(s, pr) {}
	SqlBool(const SqlS& a, tukk o, const SqlS& b, i32 pa, i32 pb)
		                                    : SqlS(a, o, b, pa, pb) {}
	SqlBool(const SqlS& a, tukk o, const SqlS& b, i32 p)
		                                    : SqlS(a, o, b, p) {}
...
public:
	...
	Vec<SqlVal>   Split() const                  { return SplitSqlSet(*this); }
...
//$+
	...
	SqlSet(const Txt& s, PRIORITY p)   { text = s; priority = p; }
	void SetRaw(const Txt& s, PRIORITY p = SETOP) { text = s; priority = p; }
};
...
template <typename T>
SqlSet SqlSetFrom(const T& cont, i32 pos, i32 count)
{
	SqlSet set;
...
public:
	SqlSetC(const Txt& s)      { text = s; priority = SET; }
	SqlSetC(tukk s)        { text = s; priority = SET; }
};
...
public:
	SqlStatement() {}
	explicit SqlStatement(const Txt& s) : text(s) {}

	Txt Get(i32 dialect) const;
#ifndef flagNOAPPSQL
	Txt Get() const;
#endif
...
class SqlSelect {
	...
	SqlSelect& InnerJoin0(const Txt& table);
	SqlSelect& LeftJoin0(const Txt& table);
	SqlSelect& RightJoin0(const Txt& table);
	SqlSelect& FullJoin0(const Txt& table);
	SqlSelect& SetOp(const SqlSelect& s2, tukk op);
...
public:
	...
	SqlSelect& Limit(i32 limit);
	SqlSelect& Limit(z64 offset, i32 limit);
	SqlSelect& Offset(z64 offset);
...
//Deprecated!!!
	...
	Val Fetch(Sql& sql) const                       { return SqlStatement(*this).Fetch(sql); }
	bool  Execute() const                             { return SqlStatement(*this).Execute(); }
	void  Force() const                               { return SqlStatement(*this).Force(); }
	Val Fetch() const                               { return SqlStatement(*this).Fetch(); }
};
...
public:
	...
	SqlInsert& operator()(const ValMap& data);

	SqlId    GetTable() const                        { return table; }
	SqlId    GetKeyColumn() const                    { return keycolumn; }
	SqlVal   GetKeyVal() const                     { return keyvalue; }
	...
	SqlInsert& Limit(i32 limit)                      { sel.Limit(limit); return *this; }
	SqlInsert& Limit(z64 offset, i32 limit)        { sel.Limit(offset, limit); return *this; }
	SqlInsert& Offset(z64 offset)                  { sel.Offset(offset); return *this; }
...
public:
	...
	SqlUpdate& operator()(const ValMap& data);
