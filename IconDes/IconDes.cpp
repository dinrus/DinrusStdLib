...
bool IconDes::InImage(i32 x, i32 y)
{
	if(!IsCurrent())
		return false;
...
void IconDes::RefreshPixel(i32 x, i32 y, i32 cx, i32 cy)
{
	Point p = sb;
	x -= p.x;
	y -= p.y;
...
void IconDes::RefreshPixel(Point p, i32 cx, i32 cy)
{
...
void IconDes::MakePaste()
{
	...
	if(paste_mode == PASTE_OPAQUE)
		DRX::Copy(c.image, c.pastepos, c.paste_image, c.paste_image.GetSize());
	else
	if(paste_mode == PASTE_BACK) {
		Image h = c.image;
		DRX::Copy(c.image, c.pastepos, c.paste_image, c.paste_image.GetSize());
		DRX::Over(c.image, Point(0, 0), h, c.image.GetSize());
	}
	else
		DRX::Over(c.image, c.pastepos, Premultiply(c.paste_image), c.paste_image.GetSize());
	MaskSelection();
}
...
void IconDes::DoFill(i32 tolerance)
{
...
void IconDes::SaveUndo()
{
	if(!IsCurrent())
		return;
	Slot& c = Current();
	Vec<ImageIml> undo = UnpackImlData(c.undo);
	i32 maxn = minmax((single_mode ? 4000000 : 400000) / max( i32(c.image.GetLength()), 1), 4, 128);
...
void IconDes::Undo()
{
	...
	if(undo.GetCount() == 0)
		return;
	Vec<ImageIml> redo = UnpackImlData(c.redo);
...
void IconDes::MaskFill(ImageBuffer& ib, RGBA color, i32 mask)
{
	if(!IsCurrent())
		return;
	if(doselection)
