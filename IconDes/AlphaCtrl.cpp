...
void AlphaCtrl::Paint(Draw& w)
{
	...
	i32 a = mask ? maskvalue : alpha;
	i32 x = a == 0   ? DPI(8) - isz.cx / 2 :
...
static void sPaintCol(ImageBuffer& b, i32 x, i32 y, i32 h, RGBA c)
{
	RGBA *t = b[y] + x;
	while(h-- > 0) {
		*t = c;
...
void AlphaCtrl::Set(i32 a)
{
	(mask ? maskvalue : alpha) = a;
	UpdateRefresh();
}

i32 AlphaCtrl::Get() const
{
