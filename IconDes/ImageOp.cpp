...
struct sFloodFill {
	...
	RGBA& At(i32 x, i32 y)         { return ib[y + rc.top][x + rc.left]; }
	bool  Eq(i32 x, i32 y);
	u8& Flag(i32 x, i32 y) { return flag[y * sz.cx + x]; }
	void  Fill(RGBA color, Point pt, const Rect& rc, i32 tolerance_);
	void  Try(i32 x, i32 y);

	sFloodFill(ImageBuffer& ib) : ib(ib) { tolerance = 0; }
};

force_inline
bool sFloodFill::Eq(i32 x, i32 y)
{
	const RGBA& q = At(x, y);
	if(tolerance < 0 && q.a)
...
void sFloodFill::Try(i32 x, i32 y)
{
...
void sFloodFill::Fill(RGBA _color, Point pt, const Rect& _rc, i32 tolerance_)
{
...
{
	RGBA t = mx[1][1];
	i32 na = mx[0][1].a, wa = mx[1][0].a, ea = mx[1][2].a, sa = mx[2][1].a;
	i32 suma = na + wa + ea + sa, half = suma >> 1;
	if(suma > 0) {
