...
void IconShow::Paint(Draw& w)
{
	...
	i32 n = msz.cx ? minmax(sz.cx / isz.cx, 1, __countof(color)) : 1;
	Image m2, m3;
...
void sDrawChRect(Draw& w, i32 x0, i32 y, i32 cx, i32 cy, bool start = false)
{
...
void IconDes::Paint(Draw& w)
{
	...
	if(magnify == 1) {
		...
		AlphaBlend(pb, image, (i32)image.GetLength());
		if(!pastepaint)
			for(i32 y = 0; y < isz.cy; y++) {
	...
	i32 m = isz.cx * magnify + 1;
	Point mpos = magnify * spos;
	for(i32 y = isz.cy * magnify; y >= 0; y -= magnify)
		w.DrawRect(-mpos.x, y - mpos.y, m, 1, Black());
	m = isz.cy * magnify + 1;
	for(i32 x = isz.cx * magnify; x >= 0; x -= magnify)
	...
	i32 my0 = -magnify * spos.y;
	i32 magnify4 = 4 * magnify;
	i32 md = (magnify + 1) / 2;
	i32 mc = (magnify - 1) / 2;
	Image masked = Crop(IconDesImg::Masked(), Size(magnify - 1, magnify - 1));
	for(i32 y0 = 0; y0 < isz.cy; y0 += 4, my0 += magnify4) {
		i32 mx0 = -magnify * spos.x;
		for(i32 x0 = 0; x0 < isz.cx; x0 += 4, mx0 += magnify4) {
			if(w.IsPainting(mx0, my0, magnify4, magnify4)) {
				i32 ex = min(x0 + 4, isz.cx);
				i32 my = my0;
				for(i32 y = y0; y < min(y0 + 4, isz.cy); y++, my += magnify) {
