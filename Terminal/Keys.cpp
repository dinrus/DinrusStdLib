...
bool TerminalCtrl::ProcessKey(dword key, bool ctrlkey, bool altkey, i32 count)
{
	if((key = EncodeCodepoint(key, gsets.Get(key, IsLevel2()))) == DEFAULTCHAR)
		return false;

	if(ctrlkey)
		key = ToAscii((char)key) & 0x1F;
...
bool TerminalCtrl::ProcessVTStyleFunctionKey(const FunctionKey& k, dword modkeys, i32 count)
{
...
bool TerminalCtrl::ProcessPCStyleFunctionKey(const FunctionKey& k, dword modkeys, i32 count)
{
	i32 modifiers = 0;
...
bool TerminalCtrl::VTKey(dword key, i32 count)
{
	const static VecMap<dword, FunctionKey> sFunctionKeyMap = {
	...
	i32 i = sFunctionKeyMap.Find(key & ~keymask);
	if(i < 0)
		return false;
...
bool TerminalCtrl::UDKey(dword key, i32 count)
{
	if(!HasUDK())
		return false;
...
bool TerminalCtrl::NavKey(dword key, i32 count)
{
	if(!keynavigation)
		return false;
...
bool TerminalCtrl::Key(dword key, i32 count)
{
	...
	if(key == K_RETURN) {
		...
		if(!shiftkey && key >= ' ' && key < 65536) {
			...
			case K_CTRL_PERIOD:
				key = '.';
				break;
			#ifndef TARGET_WIN32 // U++ ctrl + period and ctrl + semicolon enumeratos have the same value on Windows (a bug?)
