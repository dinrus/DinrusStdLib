...
SixelStream::SixelStream(ukk data, z64 size)
: MemReadStream(data, size)
, background(true)
{
}

SixelStream::SixelStream(const Txt& data)
...
{
	i32 w = buffer.GetWidth();
	for(i32 i = 0, y = cursor.y * w; i < 6; i++, y += w)
		coords[i] = y;
}
...
static Color sHSLColor(i32 h, i32 s, i32 l)
{
	...
	return Color(
		(i32) clamp((r + m) * 100.0 + 0.5, 0.0, 100.0),
		(i32) clamp((g + m) * 100.0 + 0.5, 0.0, 100.0),
		(i32) clamp((b + m) * 100.0 + 0.5, 0.0, 100.0));
}
...
force_inline
void SixelStream::PaintSixel(i32 c)
{
