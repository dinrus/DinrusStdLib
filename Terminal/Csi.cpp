...
{
	static constexpr tukk VTID_52   = "/Z";
	static constexpr tukk VTID_1XX  = "?6c";
	static constexpr tukk VTID_2XX  = "?62;1;6;8;9;15;17;22";
	static constexpr tukk VTID_3XX  = "?63;1;6;8;9;15;17;22";
	static constexpr tukk VTID_4XX  = "?64;1;6;8;9;15;17;21;22;28";
	static constexpr tukk VTID_UNIT = "!|00000000";
...
void TerminalCtrl::ReportPresentationState(const VTInStream::Sequence& seq)
{
	...
	if(report == 1) {	// DECCIR
		...
		auto Is96Chars = [=] (u8 gx) -> bool
		{
			return CHARSET_ISO8859_1 <= gx && gx <= CHARSET_ISO8859_16;
		};

		auto GetCharsetId = [=] (u8 chrset) -> tukk
		{
...
void TerminalCtrl::HandleWindowOpsRequests(const VTInStream::Sequence& seq)
{
	...
	enum WindowReports : i32 { // P = pixels, C = cells
	...
	if(windowactions && 0 < opcode && opcode < 11) {
		...
		case ACTION_MINIMIZE:
			...
			break;
		case ACTION_MOVE: {
			i32 x = seq.GetInt(3, 0);
			i32 y = seq.GetInt(2, 0);
			WindowMoveRequest(w, x, y);
			break; }
		case ACTION_RESIZE_VIEW: {
			i32 cx = StrInt(seq.GetStr(3));
			i32 cy = StrInt(seq.GetStr(2));
	...
	if(windowreports && 10 < opcode && opcode < 24) {
		...
		case REPORT_WINDOW_TITLE:
			PutOSC(Format("l%s", EncodeDataTxt(w->GetTitle())));
			break;
		default:
...
void TerminalCtrl::WindowMoveRequest(TopWindow *w, i32 x, i32 y)
{
...
void TerminalCtrl::WindowResizeRequest(TopWindow *w, i32 cx, i32 cy)
{
	Rect r = w->GetRect();
	Rect wr = GetWorkArea();
...
void TerminalCtrl::WindowPageResizeRequest(TopWindow *w, i32 cx, i32 cy)
{
...
{
	i32 level = seq.GetInt(1, 0);
	i32 mode  = seq.GetInt(2, -1);
...
{
	i32 srcpage  = seq.GetInt(5);
	i32 destpage = seq.GetInt(8);
...
void TerminalCtrl::ChangeRectAreaAttrs(const VTInStream::Sequence& seq, bool invert)
{
	...
	Vec<Txt> sgrcodes;
	for(i32 i = 4; i < seq.parameters.GetCount(); i++)
