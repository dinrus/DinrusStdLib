...
void TerminalCtrl::ParseOperatingSystemCommands(const VTInStream::Sequence& seq)
{
	...
	case 2:		// Set window title (and icon name)
		WhenTitle(DecodeDataTxt(seq.GetStr(2)).ToTxt());
		break;
...
void TerminalCtrl::ParseiTerm2Graphics(const VTInStream::Sequence& seq)
{
	...
	auto GetVal = [=, this](const Txt& s, i32 p, i32 f) -> i32
	{
		i32 n = ReadInt(s, 0);
		if(!n)
			return n;
	...
	ImageTxt simg(pick(enc));
	
	simg.size.Clear();
	bool show = false;
	
	for(const Txt& s : Split(options.Mid(pos), ';', false)) {
		Txt key, val;
...
void TerminalCtrl::ParseHyperlinks(const VTInStream::Sequence& seq)
{
	...
	constexpr i32k MAX_URI_LENGTH = 2084;

	Txt uri = seq.GetStr(3);
	...
	if(IsNull(uri) || uri.GetLength() > MAX_URI_LENGTH) {
		...
		cellattrs.data = FoldHash(GetHashVal(uri));
		RenderHyperlink(uri);
	}
}
...
void TerminalCtrl::ParseClipboardRequests(const VTInStream::Sequence& seq)
{
	...
	auto CheckInvalidBase64Chars = [](i32 c) -> bool
	{
		return !IsAlNum(c) && c != '=' && c != '+' && c != '/';
	};
	
	Txt params = seq.GetStr(2);	// We don't support multiple clipboard buffers...
	Txt data   = seq.GetStr(3);

	if(IsClipboardReadPermitted() && data.IsEqual("?")) {
		WTxt o = GetWTxt(Selection());
		if(IsNull(o))
			o = GetWTxt(Clipboard());
		PutOSC("52;s0;" + Base64Encode(EncodeDataTxt(o)));
	}
	else
	if(IsClipboardWritePermitted()) {
		if(FindMatch(data, CheckInvalidBase64Chars) < 0) {
			Txt in = Base64Decode(data);
			if(!IsNull(in))
				Copy(DecodeDataTxt(in));
		}
		else
			ClearClipboard();
	}
