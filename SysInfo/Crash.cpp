...
#if defined (flagDEBUG) && defined(TARGET_WIN32)
...
void /*__cdecl*/ CrashHandler::InvalidParameterHandler(const wchar_t* expression, const wchar_t *function,
    const wchar_t* file, u32 line, uintptr_t) {
    if (line == 0)
        Panic("Неверный параметр");
    else
        Panic(Format("Неверный параметр в in %s, функция %s, фвйл %s, строка %d", AsTxt(expression),
            AsTxt(function), AsTxt(file), i32(line)));
}
