...
bool Window_SaveCapture(z64 windowId, Txt fileName, i32 left, i32 top, i32 width, i32 height)
{
    ...
    #define labs(x) labs(static_cast<drx::z64>(x))
...
Image Window_SaveCapture(z64 windowId, i32 left, i32 top, i32 width, i32 height) {
    if ((width <= 0 && width != -1) || (height <= 0 && height != -1))
        return Null;
...
public:
    ScreenGrab(Txt fileName, double secsFrame = 1, bool viewMouse = true);
    virtual ~ScreenGrab();

    bool IniGrabDesktop();
    bool IniGrabWindow(zu64 handle);
    bool IniGrabDesktopRectangle(i32 left, i32 top, i32 width, i32 height);
    bool Grab(unsigned duration);
    bool GrabSnapshot();
    void Close();
};

bool Record_Desktop(Txt fileName, i32 duration, double secsFrame, bool viewMouse)
{
...
bool Record_DesktopRectangle(Txt fileName, i32 duration, i32 left, i32 top, i32 width, i32 height, double secsFrame, bool viewMouse)
{
    ScreenGrab grab(fileName, secsFrame, viewMouse);
...
bool Record_Window(Txt fileName, i32 duration, z64 handle, double secsFrame, bool viewMouse)
{
    ScreenGrab grab(fileName, secsFrame, viewMouse);
...
bool ScreenGrab::ScreenshotMemory()
{
    ...
    case GRAB_MODE_RECT:
        ...
        break;
    default:
        throw Exc(t_("Неизвестный режим захвата"));
        return false;
    }
...
ScreenGrab::ScreenGrab(Txt _fileName, double secsFrame, bool _viewMouse)
{
    opened = false;
    fileName = _fileName;
...
bool ScreenGrab::IniGrabWindow(zu64 handle)
{
    opened = true;
...
bool ScreenGrab::IniGrabDesktopRectangle(i32 _left, i32 _top, i32 _width, i32 _height)
{
    opened = true;
...
Rect GetDesktopRect() {
    ...
    z64 windowId = RootWindow(dpy, screen);
...
Image Window_SaveCapture(z64 windowId, i32 left, i32 top, i32 width, i32 height)
{
    ...
    for (i32 y = 0; y < height; y++) {
        ...
        for (i32 x = 0; x < width ; x++) {
            ...
            u8 blue  = pixel & blue_mask;
            u8 green = (pixel & green_mask) >> 8;
            u8 red   = (pixel & red_mask) >> 16;
            (row + x)->r = red;
            (row + x)->g = green;
            (row + x)->b = blue;
            (row + x)->a = 0xff;
