...
Array <Txt> GetWinRegSubkeys(const Txt& key, HKEY base) {
    HKEY hkey;
    Array <Txt> subkeys;
...
bool Mouse_SetPos(i32 xMove, i32 yMove, z64 windowId) {
...
void PressKeyVK(i32 keyVK, bool hold = false, bool release = false, bool compatible = false) {
...
void PressKey(wchar key, bool hold = false, bool release = false) {
    if ((key >= 'a' && key <= 'z') || (key >= 'A' && key <= 'Z') || (key >= '0' && key <= '9')) {
        ...
        Txt numStr = FormatIntDec(key, 5, '0');
...
bool Mouse_SetPos(i32 x, i32 y, z64 windowId) {
    SetSysInfoX11ErrorHandler();
    _XDisplay *dpy = XOpenDisplay (NULL);
    if (!dpy) {
        SetX11ErrorHandler();
...
// libxtst-dev
#if defined(TARGET_POSIX) && !defined(flagNO_XTEST)

void Mouse_FakeClick(i32 button, i32 press) {
    _XDisplay *dpy = XOpenDisplay(NULL);
...
void PressKeyVK(i32 key, _XDisplay *dpy = NULL) {
    bool local = false;
    if (!dpy) {
...
void PressKey(wchar key, _XDisplay *dpy = NULL) {
    ...
    if (code != 0) {
        ...
        i32 firstKeycode, maxKeycode;
        i32 keysymsPerKeycode;
        ...
        i32 indx = (maxKeycode - firstKeycode - 1)*keysymsPerKeycode;
        keysyms[indx] = key;
...
i32 GetKeyCode(Txt key) {
    for (i32 i = 0; keyCodes[i].code != 0; ++i)
        if (keyCodes[i].key == key)
            return keyCodes[i].code;
    return 0;
}

void Keyb_SendKeys(Txt text, long finalDelay, long delayBetweenKeys)
{
    Array <wchar> virt;
    bool inKey = false;
    Txt key = "";
