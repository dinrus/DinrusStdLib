...
void Parser::ThrowError(const Txt& e)
{
	...
	LLOG("ERROR: (" << GetLine(lex.Pos()) << ") " << e << ", scope: " << current_scope <<
	     ", code:  " << AsCTxt(Txt(lex.Pos(), lex.Pos(i))));
#endif
	throw Error();
}

inline tukk bew(tukk s, tukk t)
{
	while(*s)
		if(*s++ != *t++)
			return NULL;
...
// Txt == Txt comparison is likely faster than Txt == tukk  comparison
static Txt s_operator("operator");
static Txt s_virtual("virtual");
static Txt s_inline("inline");
static Txt s_static("static");

static inline bool sSpaces(Txt& res, tukk & s)
{
	if((u8)*s <= ' ') {
		char c = *s++;
		if(c != '\2' && c != '\x1f') {
...
Txt FnItem(tukk s, tukk pname, tukk qname, const Txt& name, bool oper)
{ // Converts function natural text to (unqualified) item
	...
	while(*s) { // Get the name of function into res
		...
		i32 plvl = 0;
		for(;;) {
			if(*s == '<' && plvl == 0 && !oper) { // resolve template params, like Fn<i32, true>
				res.Cat(*s++);
				lvl++;
			}
			if(*s == '>' && plvl == 0 && !oper) {
...
Txt Purify(tukk s, tukk qname, const Txt& name) {
	Txt res;
	while(*s && (u8)*s <= ' ') s++;
	bool wasid = false;
	bool firstpar = true;
...
Txt Subst(const Txt& s, const Vec<Txt>& tpar)
{
	if(tpar.GetCount() == 0)
		return s;
	Txt r;
...
Txt CleanTp(const Txt& tp)
{
	i32 q = tp.Find('<');
	i32 w = tp.ReverseFind('>');
	if(q < 0 || w < 0) return tp;
	Txt a = TrimLeft(TrimRight(tp.Mid(q + 1, w - q - 1)));
	tukk s = a;
	Txt r;
	while(*s) {
		if(*s == ',') {
...
inline void ScopeCat(Txt& scope, const Txt& s)
{
	if(scope.GetCount())
		scope << s_dblcln;
	scope << s;
...
bool Parser::IsNamespace(const Txt& scope)
{
	i32 l = scope.GetCount();
	return memcmp(~context.ns, ~scope, l) == 0 && findarg(context.ns[l], '\0', ':') >= 0;
}

bool Parser::Key(i32 code)
{
	if(lex == code) {
		++lex;
		return true;
...
i32  Parser::GetLine(tukk p)
{
	i32 pos = i32(p - ~file.text);
	i32 l = 0;
	i32 h = file.linepos.GetCount();
	while(l < h) {
		i32 q = (l + h) / 2;
...
void Parser::Line()
{
	i32 pos = i32(lex.Pos() - ~file.text);
	while(line + 1 < file.linepos.GetCount() && file.linepos[line + 1] <= pos)
		line++;
}

void Parser::Check(bool b, tukk err)
{
	if(!b) ThrowError(err);
}

void Parser::CheckKey(i32 c)
{
	if(!Key(c)) ThrowError(Format("Отсутствует %c", c));
}

Txt Parser::TemplateParams(Txt& param)
{
	Txt r;
	do {
		tukk pos = lex.Pos();
		...
		MergeWith(r, ",", Txt(pos, lex.Pos()));
	}
	while(Key(tk_template));
	return r;
}

Txt Parser::TemplateParams()
{
	Txt dummy;
	return TemplateParams(dummy);
}

Txt Parser::TemplatePnames()
{
	Txt pnames;
...
Txt Parser::ReadOper(bool& castoper) {
	tukk p = lex.Pos();
	Key(tk_operator);
	i32 level = 0;
	if(Key('('))
...
Txt Parser::Name(Txt& name, bool& castoper, bool& oper)
{
	Txt s;
	if(Key(t_dblcolon)) {
		s = "::";
	...
	for(;;) {
		if(lex.IsId()) {
			...
			Txt h = ReadOper(castoper);
			oper = true;
			name << h;
			s << h;
			break;
		}
		if(lex == '<') // void Fn<u8>(); situation
			s << TemplateParams();
		if(Key(t_dblcolon)) {
			s << "::";
			name << "::";
...
Txt Parser::Name(bool& castoper, bool& oper)
{
	Txt h;
...
Txt Parser::TType()
{
	i32 q = FindIndex(context.tparam, lex[0]);
	if(q >= 0) return AsTxt(q);
	return lex.Id();
}

Txt Parser::StructDeclaration(const Txt& tn, const Txt& tp)
{
	i32 t = lex.GetCode(); // t is now struct/class/union
	...
	if(!tp.IsEmpty())
		nn = "template " + tp + " ";
	Txt key = (t == tk_class ? "class" : t == tk_union ? "union" : "struct");
	nn << key << ' ' << name;
	...
	if(Key(':')) {
		...
		bool c = false;
		do {
			Txt access = t == tk_class ? "private" : "public";
...
Txt Parser::ReadType(Decla& d, const Txt& tname, const Txt& tparam)
{ // returns the name of constructor
	if(findarg((i32)lex, tk_struct, tk_class, tk_union) >= 0 && !d.isfriend) {
	...
	if(Key(tk_int) || Key(tk_char) ||
	   Key(tk___int8) || Key(tk___int16) || Key(tk___int32) || Key(tk___z64) || Key(tk___int128) ||
...
void Parser::Declarator(Decl& d, tukk p)
{
	i32 n = RPtr();
	if(n) {
		while(n--) lex.Get();
...
Parser::Decl& Parser::Finish(Decl& d, tukk s)
{
	d.natural = Txt(s, lex.Pos());
	return d;
}

bool Parser::IsParamList(i32 q)
{
	return true;
}
...
Array<Parser::Decl> Parser::Declaration0(bool l0, bool more, const Txt& tname, const Txt& tparam)
{
	Array<Decl> r;
	Decla d;
	tukk p = lex.Pos();
	...
	if(l0) {
		if(lex == tk_SKYLARK && lex[1] == '(' && lex.IsId(2)) {
			...
			a.name = lex.GetId();
			a.function = true;
			a.natural = Txt().Cat() << "void " << a.name << "(Http& http)";
			Decl& p = a.param.Add();
			p.name = "http";
			p.type = "Http";
			p.natural = "Http& http";
		...
		if((lex == tk_RPC_METHOD || lex == tk_RPC_GMETHOD) && lex[1] == '(' && lex.IsId(2)) {
			...
			a.name = lex.GetId();
			a.function = true;
			a.natural = Txt().Cat() << "void " << a.name << "(RpcData& rpc)";
			Decl& p = a.param.Add();
			p.name = "rpc";
			p.type = "RpcData";
	...
	Txt st = ReadType(d, tname, tparam);
	...
	if(lex != ';') // struct/class declaration without defining variable
		do {
			...
			a.natural = natural1 + Txt(p1, lex.Pos());
			p = lex.Pos();
		}
		while(more && Key(','));
	return r;
...
Array<Parser::Decl> Parser::Declaration(bool l0, bool more, const Txt& tname, const Txt& tparam)
{
	if(Key(tk_typedef)) {
...
Txt Parser::Tparam(i32& q)
{
	...
	return Txt(p, lex.Pos(q));
}

Txt NoTemplatePars(const Txt& s)
{
	i32 q = s.Find('<');
...
CppItem& Parser::Item(const Txt& scope, const Txt& using_namespace, const Txt& item,
...
void Parser::AddMacro(i32 lineno, const Txt& macro, i32 kind)
{
	Txt name;
	tukk s = macro;
	while(*s && iscid(*s))
...
Txt Parser::AnonymousName()
{
	i32 lvl = 0;
	for(i32 i = 0; lex[i] != t_eof; i++) {
		if(lex[i] == '{') lvl++;
		else
		if(lex[i] == '}')
			if(--lvl == 0) {
	...
	return "@" + Base64Encode(Txt((tukk )&x, sizeof(x))) + "/" + title;
}

void Parser::AddNamespace(const Txt& n, const Txt& name)
{
	Txt h = "namespace " + n;
...
bool Parser::Scope(const Txt& tp, const Txt& tn) {
	if(Key(tk_namespace)) {
		...
		Context c0;
		c0 <<= context;
		i32 struct_level0 = struct_level;
...
CppItem& Parser::Fn(const Decl& d, const Txt& templ, bool body,
                    const Txt& tname, const Txt& tparam)
{
	Txt param;
	Txt pname;
	Txt ptype;
	for(i32 i = 0; i < d.param.GetCount(); i++) {
	...
	Txt item = FnItem(d.natural, pname, d.name, nm, d.oper);
	Txt scope = context.scope;
	Txt nn;
	tukk s = nn0;
	i32 l = 0;
	while(*s) {
...
bool Parser::UsingNamespace()
{
	if(lex == tk_using && !(lex.IsId(1) && lex[2] == '=')) {
		...
		if(Key(tk_namespace))
			while(lex.IsId()) {
				Vec<Txt> h = Split(context.namespace_using, ';');
				Txt u;
				do {
					u << lex.GetId();
					if(Key(t_dblcolon))
	...
	for(;;) {
		...
		if(lex.IsId(i) || lex[i] == ':' ||
		   findarg(lex[i], tk_bool, tk_signed, tk_unsigned, tk_long, tk_int, tk_short,
		                       tk_char, tk___int8, tk___int16, tk___int32, tk___z64,
...
void Parser::ClassEnum(const Txt& clss)
{
	context.typenames.FindAdd(lex);
	Context cc;
	cc <<= context;
...
void Parser::Do()
{
	...
	if(Key(tk_template)) {
		if(lex.IsId() || lex == tk_class && lex.IsId(1)) {
			...
			Txt tnames;
			Txt tparam = TemplateParams(tnames);
				...
				if(d.name.GetCount()) {
					if(d.function) {
						...
						if(d.type_def)
							ScopeCat(scope, d.name);
						Txt name = d.name;
						i32 member_type = d.s_static ? CLASSVARIABLE : INSTANCEVARIABLE;
						q = d.name.ReverseFind("::");
						if(q >= 0) { // class variable definition like: i32 Ctrl::EventLoop;
...
void  Parser::Do(Stream& in, CppBase& _base, i32 filei_, i32 filetype_,
                 const Txt& title_, Event<i32, const Txt&> _err,
                 ...
                 const Index<Txt>& namespace_using)
{
	...
	Txt n;
	for(i32 i = 0; i < namespace_stack.GetCount(); i++) {
	...
	while(lex != t_eof)
		try {
			try {
				...
				inbody = false;
				struct_level = 0;
				for(i32 i = 0; i < typenames.GetCount(); i++)
...
Vec<Txt> ParserContext::GetNamespaces() const
{
	Vec<Txt> ns;
	Vec<Txt> h = Split(current_scope, ':');
	while(h.GetCount()) {
		ns.Add(Join(h, "::"));
		h.Drop();
	}
