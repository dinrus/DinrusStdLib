...
void Parser::Locals(const Txt& type)
{
	Line();
...
{ // attempt to interpret code as local variable declaration
	...
	if(t == tk_int || t == tk_bool || t == tk_float || t == tk_double || t == tk_void ||
	   ...
	   t == tk_char || t == tk___int8 || t == tk___int16 || t == tk___int32 || t == tk___z64 ||
	   t == tk_auto) {
		q++;
	...
	if(t == tk_decltype && lex[q + 1] == '(') {
		...
		for(;;) {
			...
			if(lex[q] == ')' && --lvl == 0) {
				Locals("@" + Txt(lex.Pos(q0), lex.Pos(q)));
				return true;
			}
			++q;
		}
...
Txt Parser::ResolveAutoType()
{
	Vec<Txt> xp = MakeXP(lex.Pos());
	...
	Index<Txt> s = GetExpressionType(*base, *this, xp);
	i32 i = FindMax(s); // Ugly hack: we are not resolving overloading at all, so just choose stable type if there are more, not Null
	return i < 0 ? Txt() : s[i];
}
