...
Txt Aformat(double x)
{
	return (i32)x == x ? AsTxt(x) : x < 100000 ? Format("%.2f", x) : Format("%g", x);
}
...
Txt FMOP::Save() const
{
	...
	return r + decode(waveform, WAVEFORM_SQUARE, "Q",
	                            ...
	                            WAVEFORM_WHITE, "N",
	                            WAVEFORM_SIN, "",
	                            ~("w" + AsTxt(waveform - WAVEFORM_FIRSTSAMPLE)));
}

tukk  FMOP::Load(tukk s)
{
	auto Get = [&](double& r) {
		try {
			CParser p(s);
	...
	while(*s) {
		...
		case 'w':
			double w;
			Get(w);
			waveform = clamp(WAVEFORM_FIRSTSAMPLE + (i32)w, (i32)WAVEFORM_FIRSTSAMPLE, (i32)WAVEFORM_LASTSAMPLE);
		}
	}
	return s;
}

Txt Sound::Save() const
{
	Txt r;
	r << Aformat(f) << ':';
