...
void SoundGen::FMOPGen::Comp()
{
	...
	wave_tab = waveform_table[clamp(waveform, 0, (i32)WAVEFORM_LASTSAMPLE)];
}

Txt SoundGen::FMOPGen::ToTxt() const
{
	return Txt() << "Громкость: " << volume << ", фаза: " << p << ", Объём ADSR: " << v;
}

force_inline
double SoundGen::FMOPGen::Evaluate(i32 t, double mf, double mod, double& cv)
{
	if(volume == 0)
		return 0;
	if(t >= duration) {
...
void SoundGen::Start(const Sound& s)
{
	...
	lpan = (float)s.pan;
	rpan = (float)(1 - s.pan);
	for(i32 i = 3; i <= 4; i++) // vibrato, tremolo
