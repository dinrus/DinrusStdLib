...
void SvgParser::ResolveGradient(i32 i)
{
	Gradient& g = gradient[i];
	if(g.resolved || g.href.GetCount() < 2)
		return;
	i32 q = gradient.Find(g.href.Mid(1));
	g.resolved = true;
	if(q < 0)
		return;
	ResolveGradient(q);
...
void SvgParser::StartElement(const XmlNode& n)
{
	...
	current = &n;
	bp.Begin();
	bp.Transform(Transform(AsTxt("transform")));
	Style(AsTxt("style"));
	Txt classid = AsTxt("class");
	if(classid.GetCount())
		Style(classes.Get(classid, Txt()));
	for(i32 i = 0; i < n.GetAttrCount(); i++)
		ProcessVal(n.AttrId(i), n.Attr(i));
	closed = false;
}
...
void SvgParser::DoGradient(i32 gi, bool stroke)
{
	State& s = state.Top();
	ResolveGradient(gi);
	Gradient& g = gradient[gi];
	if(g.stop.GetCount()) {
		for(i32 i = 0; i < g.stop.GetCount(); i++)
...
void SvgParser::ParseGradient(const XmlNode& n, bool radial)
{
	...
	auto Dbl = [&](tukk id, double def) { return Nvl(StrDbl(n.Attr(id)), def); };
	g.c.x = Dbl("cx", def5);
	g.c.y = Dbl("cy", def5);
	...
	g.b.x = Dbl("x2", Null);
	g.b.y = Dbl("y2", def);
	g.style = decode(AsTxt("spreadMethod"), "pad", GRADIENT_PAD, "reflect", GRADIENT_REFLECT,
	                 "repeat", GRADIENT_REPEAT, (i32)Null);
	...
	for(const XmlNode& m : n)
		if(m.IsTag("stop")) {
			...
			s.color = clamp(i32(opacity * 255 + 0.5), 0, 255) * color;
...
Rectf GetSvgViewBox(const Txt& v)
{
	Rectf r = Null;
	if(v.GetCount()) {
		try {
...
void SvgParser::Element(const XmlNode& n, i32 depth, bool dosymbols)
{
	...
	if(n.IsTag("text")) {
		...
		auto DoText = [&](const XmlNode& n) {
			Txt text = n.GatherText();
			text.Replace("\n", " ");
			text.Replace("\r", "");
			text.Replace("\t", " ");
			if(text.GetCount()) {
				Font fnt = state.Top().font;
				i32 anchor = state.Top().text_anchor;
				double x = Dbl("x");
				if(anchor) {
...
void SvgParser::Items(const XmlNode& n, i32 depth)
{
	StartElement(n);
	for(const auto& m : n)
		Element(m, depth);
...
bool ParseSVG(Painter& p, tukk svg, Event<Txt, Txt&> resloader, Rectf *boundingbox)
{
	SvgParser sp(p);
...
{
	return RenderSVG(p, svg, Event<Txt, Txt&>());
}
...
Rectf GetSVGBoundingBox(tukk svg)
{
	NilPainter nil;
	Rectf bb;
	if(!ParseSVG(nil, svg, Event<Txt, Txt&>(), &bb))
		return Null;
	return bb;
}
...
Image RenderSVGImage(Size sz, tukk svg, Event<Txt, Txt&> resloader)
{
	...
	Size isz((i32)ceil(iszf.cx), (i32)ceil(iszf.cy));
...
{
	return RenderSVGImage(sz, svg, Event<Txt, Txt&>());
}
