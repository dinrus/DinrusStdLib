...
{
	DRX::Fill(~*ip, color, ip->GetLength());
...
void BufferPainter::RenderPathSegments(LinearPathConsumer *g, const Vec<u8>& path,
...
Buffer<ClippingLine> BufferPainter::RenderPath(double width, Event<One<SpanSource>&> ss, const RGBA& color)
{
	...
	if(width == 0 || (!ss && color.a == 0 && (i32)width >= FILL)) {
	...
	if(co) {
		if((i32) width >= FILL && !ss && !alt && findarg(mode, MODE_ANTIALIASED, MODE_SUBPIXEL) >= 0) {
			for(i32 i = 0; i < path_info->path.GetCount(); i++) {
	...
	bool doclip = (i32) width == CLIP;
	auto fill = [&](CoWork *co) {
		i32 opacity = i32(256 * pathattr.opacity);
		if(!opacity)
			return;
		...
		if((i32) width != ONPATH) {
			if(alt)
				...
				i32 ii = 0;
				for(;;) {
					i32 y = (co ? co->Next() : ii++) + rasterizer.MinY();
	...
	for(const auto& p : path_info->path) {
		...
		if( (i32)width != ONPATH) {
			i32 n = rasterizer.MaxY() - rasterizer.MinY();
			if(n >= 0) {
...
void BufferPainter::FinishPathJob()
{
	...
	fill_job & [=, this] {
		...
		for(i32 i = 0; i < fillcount; i++) {
			...
			j.c = Mul8(j.color, i32(256 * j.attr.opacity));
		}
		auto fill = [&](i32 ymin, i32 ymax) {
			if(subpixel) {
				SubpixelFiller subpixel_filler;
				subpixel_filler.ss = NULL;
				i32 ci = CoWork::GetWorkerIndex();
				...
				for(i32 i = 0; i < fillcount; i++) {
					CoJob& j = cofill[i];
					i32 jymin = max(j.rasterizer.MinY(), ymin);
					i32 jymax = min(j.rasterizer.MaxY(), ymax);
					for(i32 y = jymin; y <= jymax; y++)
