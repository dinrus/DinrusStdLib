...
Color SvgParser::GetTextColor(const Txt& color) {
	struct Svg_color {
		tukk name;
		u8 r, g, b;
	} colors[] = {
	...
	static VecMap<Txt, Color> map;
	ONCELOCK {
		for(i32 i = 0; i < __countof(colors); i++)
...
Color SvgParser::GetColor(const Txt& text_) {
	Txt text = ToLower(text_);
...
Array<double> GetTransformArgs(Txt str, tukk command) {
	Array<double> args;
	i32 pos, endpos;
