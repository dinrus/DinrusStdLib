...
void CubicMinMax(double p1, double p2, double p3, double p4, double& l, double& h) {
	...
	double c2 = 3 * (p4 - p3);
	double r[2];
	i32 n = SolveQuadraticEquation(c2 - c1 + c0, c1 - 2 * c0, c0, r);
	for(i32 i = 0; i < n; i++) {
		double tv = r[i];
		if(tv > 0 && tv < 1) {
...
void BoundsPainter::TextOp(const Pointf& p, wtukk text, Font fnt, i32 n, const double *dx)
{
	Painter::TextOp(p, text, fnt, n, dx);
}

void BoundsPainter::CharacterOp(const Pointf& p, i32 ch, Font fnt)
{
