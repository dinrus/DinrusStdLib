...
void PaintImageBufferPaintingFn(ImageBuffer& ib, const Painting& p, Size sz, Point pos, i32 mode)
{
...
void PaintImageBufferDrawingFn(ImageBuffer& ib, const Drawing& iw, i32 mode)
{
	BufferPainter sw(ib, mode);
	Sizef sz = ib.GetSize();
	Size isz = iw.GetSize();
...
static Image sRenderGlyph(Point at, i32 angle, i32 chr, Font fnt, Color color, Size sz)
{
	ImageBuffer ib(sz);
	BufferPainter sw(ib);
	sw.Clear(RGBAZero());
	sw.DrawText(at.x, at.y, angle, WTxt(chr, 1), fnt, color);
	Image h = ib;
