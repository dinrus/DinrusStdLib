...
void PaintingPainter::FillOp(const RGBA& color1, const RGBA& color2, const Xform2D& transsrc, i32 style)
{
...
void PaintingPainter::FillOp(const Pointf& f, const RGBA& color1, const RGBA& color2, const Xform2D& transsrc, i32 style)
{
...
void PaintingPainter::StrokeOp(double width, const Pointf& f, const RGBA& color1,
                               const RGBA& color2, const Xform2D& transsrc, i32 style)
{
	Put(PAINTING_FILL_RADIAL_X);
	Putf(width);
	Putf(f);
...
void PaintingPainter::FillOp(const Pointf& f, const RGBA& color1,
                             const Pointf& p, double r, const RGBA& color2, i32 style)
{
...
void PaintingPainter::StrokeOp(double width, const Pointf& p1, const RGBA& color1,
                               const Pointf& p2, const RGBA& color2, i32 style)
{
...
void PaintingPainter::CharacterOp(const Pointf& p, i32 ch, Font fnt)
{
...
void PaintingPainter::TextOp(const Pointf& p, wtukk text, Font fnt, i32 n, const double *dx)
{
	Put(PAINTING_TEXT);
	Putf(p);
	Put32(n);
...
void PaintingPainter::LineCapOp(i32 linecap)
{
	Put(PAINTING_LINECAP);
	Put(linecap);
}

void PaintingPainter::LineJoinOp(i32 linejoin)
{
...
void PaintingPainter::DashOp(const Vec<double>& dash, double start)
{
	Put(PAINTING_DASH);
	Put32(dash.GetCount());
	for(i32 i = 0; i < dash.GetCount(); i++)
		Putf(dash[i]);
	Putf(start);
}
