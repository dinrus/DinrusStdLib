...
struct sMakeGlyph : LRUCache<Val, GlyphKey>::Maker {
	GlyphKey gk;

	GlyphKey Key() const     { return gk; }
	i32      Make(Val& v) const {
		GlyphPainter gp;
		gp.move = gp.pos = Null;
		gp.tolerance = gk.tolerance;
		PaintCharacter(gp, Pointf(0, 0), gk.chr, gk.fnt);
		i32 sz = gp.glyph.GetCount() * 4;
		v = RawPickToVal(pick(gp.glyph));
		return sz;
	}
};
...
void ApproximateChar(LinearPathConsumer& t, Pointf at, i32 ch, Font fnt, double tolerance)
{
	...
	INTERLOCKED {
		...
		static LRUCache<Val, GlyphKey> cache;
		cache.Shrink(500000);
		sMakeGlyph h;
		h.gk.fnt = fnt;
		h.gk.chr = ch;
		...
		DDUMP(ValTo< Vec<float> >(v));
		GlyphPainter chp;
		chp.move = chp.pos = Null;
		chp.tolerance = tolerance;
		PaintCharacter(chp, Pointf(0, 0), ch, fnt);
		DDUMP(chp.glyph);
		ASSERT(ValTo< Vec<float> >(v) == chp.glyph);
#endif
	}
	const Vec<float>& g = ValTo< Vec<float> >(v);
	i32 i = 0;
	while(i < g.GetCount()) {
		Pointf p;
		p.x = g[i++];
