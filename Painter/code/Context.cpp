...
void BufferPainter::LineCapOp(i32 linecap)
{
...
void BufferPainter::LineJoinOp(i32 linejoin)
{
...
void BufferPainter::DashOp(const Txt& dash, double start)
{
	Txt key;
	if(start) {
		key.Cat(1);
...
void BufferPainter::DashOp(const Vec<double>& dash, double start)
{
	Txt key;
	RawCat(key, 0);
	RawCat(key, start);
...
void BufferPainter::ColorStop0(Attr& a, double pos, const RGBA& color)
{
	...
	c.color = color;
	c.stop = pos;
	i32 i = FindLowerBound(a.color_stop, c);
...
void BufferPainter::Create(ImageBuffer& ib, i32 mode_)
{
	ip = &ib;
