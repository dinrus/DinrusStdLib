...
void SvgParser::ProcessVal(const Txt& key, const Txt& value_)
{
	State& s = state.Top();
	Txt value = TrimBoth(value_);
...
void SvgParser::Style(tukk style)
{
	Txt key, value;
	for(;;) {
		if(*style == ';' || *style == '\0') {
...
Xform2D SvgParser::Transform(tukk transform)
{
	Xform2D mx;
	try {
		CParser p(transform);
		while(!p.IsEof()) {
			Txt kind = ToLower(p.ReadId());
			Vec<double> r;
			p.Char('(');
