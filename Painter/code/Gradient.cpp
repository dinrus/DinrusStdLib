...
Image BufferPainter::MakeGradient(RGBA color1, Vec<ColorStop>& color_stop, RGBA color2, i32 n)
{
	ImageBuffer ib(n, 1);
	RGBA *t = ib[0];
	i32 l = 0;
	RGBA cl = color1;
	for(i32 i = 0; i <= color_stop.GetCount(); i++) {
		i32 h;
		RGBA ch;
		if(i < color_stop.GetCount()) {
			h = (i32)(color_stop[i].stop * (n - 1));
...
Txt BufferPainter::GradientImageMaker::Key() const
{
	TxtStream ss;
...
Image BufferPainter::MakeGradientCached(RGBA color1, Vec<ColorStop>& color_stop, RGBA color2, i32 n)
{
...
Image BufferPainter::Gradient(const RGBA& color1, const RGBA& color2, i32 n)
{
...
{
	return Gradient(color1, color2, minmax(i32(Distance(p1, p2) * pathattr.mtx.GetScale()), 2, 16384));
}

static dword sLinearStyle(i32 style)
{
...
void BufferPainter::FillOp(const Pointf& p1, const RGBA& color1, const Pointf& p2, const RGBA& color2, i32 style)
{
	Fill(Gradient(color1, color2, p1, p2), p1, p2, sLinearStyle(style));
}

void BufferPainter::FillOp(const RGBA& color1, const RGBA& color2, const Xform2D& transsrc, i32 style)
{
	Fill(Gradient(color1, color2, 500), Xform2D::Scale(1.0 / 500) * transsrc, sLinearStyle(style));
}

void BufferPainter::StrokeOp(double width, const Pointf& p1, const RGBA& color1, const Pointf& p2, const RGBA& color2, i32 style)
{
	Stroke(width, Gradient(color1, color2, p1, p2), p1, p2, sLinearStyle(style));
}

void BufferPainter::StrokeOp(double width, const RGBA& color1, const RGBA& color2, const Xform2D& transsrc, i32 style)
{
