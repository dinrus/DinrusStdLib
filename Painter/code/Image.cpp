...
force_inline
i32 IntAndFraction(f32x4 x, f32x4& fraction)
{
...
i32 Int(f32x4 x)
{
	return (i32)Truncate(x + f32all(8000)) - 8000;
}

struct PainterImageSpanData {
	i32         ax, ay, cx, cy, maxx, maxy;
	u8        style;
	u8        hstyle, vstyle;
	bool        fast;
	bool        fixed;
	...
	PainterImageSpanData(dword flags, const Xform2D& m, const Image& img, bool co, bool imagecache) {
		...
		if(!fast) {
			...
			if(sc.x >= 0.01 && sc.y >= 0.01) {
				nx = (i32)max(1.0, 1.0 / sc.x);
				ny = (i32)max(1.0, 1.0 / sc.y);
			}
		}
		if(nx == 1 && ny == 1)
			xform = Inverse(m);
...
struct PainterImageSpan : SpanSource, PainterImageSpanData {
	...
	const RGBA *Pixel(i32 x, i32 y) { return &image[y][x]; }

	const RGBA *GetPixel(i32 x, i32 y) {
		if(hstyle == FILL_HPAD)
			x = minmax(x, 0, maxx);
		else
	...
	virtual void Get(RGBA *span, i32 x, i32 y, unsigned len)
	{
...
struct PainterImageSpan : SpanSource, PainterImageSpanData {
	...
	RGBA Pixel(i32 x, i32 y) { return image[y][x]; }

	RGBA GetPixel(i32 x, i32 y) {
		if(hstyle == FILL_HPAD)
			x = minmax(x, 0, maxx);
		else
		if(hstyle == FILL_HREFLECT)
