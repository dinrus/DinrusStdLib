force_inline
RGBA Mul8(const RGBA& s, i32 mul)
{
	RGBA t;
	t.r = (mul * s.r) >> 8;
	t.g = (mul * s.g) >> 8;
...
struct SpanSource {
	virtual void Get(RGBA *span, i32 x, i32 y, unsigned len) = 0;
	virtual ~SpanSource() {}
};
...
public:
	...
	void Set(u8k *s, i32 len)     { data = (u8 *)MemoryAlloc(len); memcpy8(data, s, len); }
	void SetFull()                       { ASSERT(!data); data = (u8 *)1; }

	bool IsEmpty() const                 { return !data; }
	bool IsFull() const                  { return data == (u8 *)1; }
	operator u8k*() const         { return data; }
...
protected:
	...
	virtual void   DashOp(const Txt& dash, double start);
...
private:
	...
	struct Attr : Moveable<Attr, SimpleAttr> {
		i32                             mtx_serial; // used to detect changes to speedup preclip
		WithDeepCopy<Vec<ColorStop>> color_stop;
	...
	Vec<Buffer<ClippingLine>> clip;
	Array< ImageBuffer >       mask;
	Vec<Vec<PathLine>>   onpathstack;
	Vec<double>             pathlenstack;
	i32                        mtx_serial = 0;
	ArrayMap<Txt, DashInfo> dashes;
	
	Rectf                      preclip;
	i32                        preclip_mtx_serial = -1;
	bool                       regular;
	...
	struct OnPathTarget : LinearPathConsumer {
		Vec<BufferPainter::PathLine> path;
		Pointf pos;
		double len;
	...
	i32              jobcount, fillcount;
	i32              emptycount; // jobs skipped because of Null color or zero width

	CoWorkNX         fill_job;
	
	template <class T> T& PathAdd(i32 type);
	...
	void             RenderRadial(double width, const Pointf& f, const RGBA& color1,
	                              const Pointf& c, double r, const RGBA& color2,
	                              const Xform2D& m, i32 style);
	void             RenderRadial(double width, const Pointf& f, const RGBA& color1, const RGBA& color2,
	                              const Xform2D& transsrc, i32 style);

	struct           GradientImageMaker;
	static Image     MakeGradient(RGBA color1, Vec<ColorStop>& color_stop, RGBA color2, i32 n);
	static Image     MakeGradientCached(RGBA color1, Vec<ColorStop>& color_stop, RGBA color2, i32 n);
	Image            Gradient(const RGBA& color1, const RGBA& color2, i32 n);
	...
	static void RenderPathSegments(LinearPathConsumer *g, const Vec<u8>& path, const SimpleAttr *attr, double tolerance);
...
public:
	...
	void               Create(ImageBuffer& ib, i32 mode = MODE_ANTIALIASED);
	void               Finish();

	BufferPainter(ImageBuffer& ib, i32 mode = MODE_ANTIALIASED) { Create(ib, mode); }
