...
struct UDropTarget : public IDropTarget {
	...
	STDMETHOD(QueryInterface)(REFIID riid, uk *ppvObj);
...
STDMETHODIMP CLANG_NOTHROW UDropTarget::QueryInterface(REFIID iid, uk * ppv)
{
...
Txt UDropTarget::Get(tukk fmt) const
{
	FORMATETC fmtetc = ToFORMATETC(fmt);
	STGMEDIUM s;
	if(data->GetData(&fmtetc, &s) == S_OK && s.tymed == TYMED_HGLOBAL) {
		tuk val = (tuk )GlobalLock(s.hGlobal);
		Txt data(val, (i32)GlobalSize(s.hGlobal));
...
void UDropTarget::DnD(POINTL pl, bool drop, DWORD *effect, DWORD keys)
{
	...
	*effect = DROPEFFECT_NONE;
	...
	if(d.IsAccepted()) {
		LLOG("DnD accepted, action: " << (i32)d.action);
...
STDMETHODIMP CLANG_NOTHROW UDataObject::QueryInterface(REFIID iid, uk * ppv)
{
...
void SetMedium(STGMEDIUM *medium, const Txt& data)
{
	i32 sz = data.GetCount();
	HGLOBAL hData = GlobalAlloc(0, sz + 4);
	if (hData) {
		tuk ptr = (tuk ) GlobalLock(hData);
		memcpy(ptr, ~data, sz);
		memset(ptr + sz, 0, 4);
		GlobalUnlock(hData);
...
{
	Txt fmt = FromWin32CF(fmtetc->cfFormat);
	ClipData *s = data.FindPtr(fmt);
	if(s) {
		Txt q = s->Render();
		SetMedium(medium, q.GetCount() ? q : sDnDSource ? sDnDSource->GetDropData(fmt) : Txt());
		return S_OK;
	}
...
#ifdef TARGET_WINCE
static i32 CF_PERFORMEDDROPEFFECT = RegisterClipboardFormat(_T("Performed DropEffect"));
#else
static i32 CF_PERFORMEDDROPEFFECT = RegisterClipboardFormat("Performed DropEffect");
#endif
...
STDMETHODIMP CLANG_NOTHROW UDropSource::QueryInterface(REFIID riid, uk *ppvObj)
{
