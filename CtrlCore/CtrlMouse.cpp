...
void Ctrl::LogMouseEvent(tukk f, const Ctrl *ctrl, i32 event, Point p, i32 zdelta, dword keyflags)
{
	if(!Ini::user_log)
		return;
	Txt txt = f;
...
Image Ctrl::FrameMouseEventH(i32 event, Point p, i32 zdelta, dword keyflags)
{
	GuiLock __;
	Ptr<Ctrl> this_ = this;
	for(i32 i = 0; i < mousehook().GetCount(); i++)
...
Image Ctrl::FrameMouseEvent(i32 event, Point p, i32 zdelta, dword keyflags)
{
	return Image::Arrow();
}
...
Image Ctrl::MouseEvent0(i32 event, Point p, i32 zdelta, dword keyflags)
{
	GuiLock __;
	Ptr<Ctrl> this_ = this;
	bool pb = sPropagated;
...
Image Ctrl::MouseEventH(i32 event, Point p, i32 zdelta, dword keyflags)
{
	GuiLock __;
	Ptr<Ctrl> this_ = this;
	for(i32 i = 0; i < mousehook().GetCount(); i++)
...
void Ctrl::MouseWheel(Point p, i32 zd, dword kf)
{
	Ctrl *parent = GetParent();
	if(parent) {
...
void Ctrl::ChildFrameMouseEvent(Ctrl *child, i32 event, Point p, i32 zdelta, dword keyflags)
{
	GuiLock __;
	Ctrl *parent = GetParent();
	if(parent)
...
void Ctrl::ChildMouseEvent(Ctrl *child, i32 event, Point p, i32 zdelta, dword keyflags)
{
	GuiLock __;
	Ctrl *parent = GetParent();
	if(parent)
...
Image Ctrl::MouseEvent(i32 event, Point p, i32 zdelta, dword keyflags)
{
	LLOG("MouseEvent " << DRX::Name(this) << " " << FormatIntHex(event));
	switch(event) {
	case MOUSEENTER:
...
Image Ctrl::MEvent0(i32 e, Point p, i32 zd)
{
	GuiLock __;
...
void    Ctrl::LRepeat() {
	...
	if(repeatTopCtrl && GetMouseLeft()) {
		...
		KillRepeat();
	LLOG("LRepeat " << DRX::Name(mouseCtrl));
}

static i32 sDistMax(Point a, Point b)
{
	return IsNull(a) ? INT_MAX : max(abs(a.x - b.x), abs(a.y - b.y));
}

static i32 sDistMin(Point a, Point b)
{
...
void    Ctrl::LRep() {
	LLOG("LRep");
	DRX::SetTimeCallback(-GetKbdSpeed(), callback(&Ctrl::LRepeat), &mousepos);
}
...
void    Ctrl::RRep() {
	DRX::SetTimeCallback(-GetKbdSpeed(), callback(&Ctrl::RRepeat), &mousepos);
}
...
void    Ctrl::MRep() {
	GuiLock __;
	DRX::SetTimeCallback(-GetKbdSpeed(), callback(&Ctrl::MRepeat), &mousepos);
}
...
void    Ctrl::CheckMouseCtrl() {
	...
	if(mouseCtrl) {
		...
		LLOG("CheckMouseCtrl mouseCtrl " << DRX::Name(mouseCtrl) << " " << r);
...
i32 leftdbltime = Null, rightdbltime = Null, middledbltime = Null;

bool sDblTime(i32 time)
{
	return !IsNull(time) && (i32)msecs() - time < GUI_DblClickTime();
}

Image Ctrl::DispatchMouse(i32 e, Point p, i32 zd) {
	GuiLock __;
	EventLevelDo ___;
	...
	if(e == LEFTDOUBLE) {
		leftdbltime = msecs();
		leftdblpos = p;
		DRX::SetTimeCallback(GetKbdDelay(), callback(&Ctrl::LRep), &mousepos);
		repeatTopCtrl = this;
	}
	if(e == RIGHTDOUBLE) {
		rightdbltime = msecs();
		rightdblpos = p;
		DRX::SetTimeCallback(GetKbdDelay(), callback(&Ctrl::RRep), &mousepos);
		repeatTopCtrl = this;
	}
	if(e == MIDDLEDOUBLE) {
		middledbltime = msecs();
		middledblpos = p;
		DRX::SetTimeCallback(GetKbdDelay(), callback(&Ctrl::MRep), &mousepos);
		repeatTopCtrl = this;
	}
	if(e == LEFTDOWN) {
		LLOG("Ctrl::DispatchMouse: init left repeat for " << DRX::Name(this) << " at " << p);
		DRX::SetTimeCallback(GetKbdDelay(), callback(&Ctrl::LRep), &mousepos);
		DRX::SetTimeCallback(2 * GetKbdDelay(), callback(&Ctrl::LHold), &mousepos);
		leftmousepos = p;
	...
	if(e == RIGHTDOWN) {
		LLOG("Ctrl::DispatchMouse: init right repeat for " << DRX::Name(this) << " at " << p);
		DRX::SetTimeCallback(GetKbdDelay(), callback(&Ctrl::RRep), &mousepos);
		DRX::SetTimeCallback(2 * GetKbdDelay(), callback(&Ctrl::RHold), &mousepos);
		rightmousepos = p;
	...
	if(e == MIDDLEDOWN) {
		LLOG("Ctrl::DispatchMouse: init middle repeat for " << DRX::Name(this) << " at " << p);
		DRX::SetTimeCallback(GetKbdDelay(), callback(&Ctrl::MRep), &mousepos);
		DRX::SetTimeCallback(2 * GetKbdDelay(), callback(&Ctrl::MHold), &mousepos);
		middlemousepos = p;
...
Image Ctrl::DispatchMouseEvent(i32 e, Point p, i32 zd) {
	GuiLock __;
...
AutoWaitCursor::AutoWaitCursor(i32& avg) : WaitCursor(avg >= 0), avg(avg) {
	time0 = msecs();
}
