...
i32 Ctrl::GetChildIndex(const Ctrl *child) const
{
	GuiLock __;
	i32 i = 0;
...
Ctrl * Ctrl::GetIndexChild(i32 ii) const
{
	GuiLock __;
	Ctrl *c = GetFirstChild();
	for(i32 i = 0; i < ii && c; i++)
		c = c->GetNext();
	return c;
}

i32 Ctrl::GetViewChildIndex(const Ctrl *child) const
{
	GuiLock __;
	i32 i = 0;
	for (Ctrl *c = GetFirstChild(); c; c = c->GetNext())
...
i32 Ctrl::GetViewChildCount() const
{
	GuiLock __;
	i32 n = 0;
	for (Ctrl *c = GetFirstChild(); c; c = c->GetNext())
...
Ctrl * Ctrl::GetViewIndexChild(i32 ii) const
{
	GuiLock __;
	i32 i = 0;
	for (Ctrl *c = GetFirstChild(); c; c = c->GetNext())
...
{
	LLOG("IterateFocusFw(" << DRX::Name(ctrl) << ")");
	while(ctrl) {
...
{
	GuiLock __;
	LLOG("IterateFocusForward(" << DRX::Name(ctrl) << ", top " << DRX::Name(top) << ", noframe " << noframe << ", init " << init << ")");
