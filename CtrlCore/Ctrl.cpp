...
static bool StdDisplayErrorFn(const Val& e)
{
    GuiLock __;
    if(!e.IsError())
        return false;
...
{
    static bool (*errfn)(const Val& v) = &StdDisplayErrorFn;
    return errfn;
}
...
Ctrl *Ctrl::LoopCtrl;
i32   Ctrl::LoopLevel;
z64 Ctrl::EventLoopNo;
z64 Ctrl::EndSessionLoopNo;

bool Ctrl::MemoryCheck;

bool Ctrl::painting = false;

void   Ctrl::SetData(const Val&) {}
Val  Ctrl::GetData() const       { return Val(); }

void Ctrl::Paint(Draw& draw)                        {}
i32  Ctrl::OverPaint() const                        { return 0; }
...
bool Ctrl::Key(dword key, i32 count)
{
    return false;
}
...
dword  Ctrl::AccessKeyBit(i32 accesskey)
{
    accesskey &= 255;
    if(accesskey >= 'A' && accesskey <= 'Z')
        return (zu64)2 << (accesskey - 'A');
    return !!accesskey;
}
...
void Ctrl::StateDeep(i32 reason)
{
    GuiLock __;
    if(destroying)
        return;
...
void Ctrl::StateH(i32 reason)
{
    GuiLock __;
    for(i32 i = 0; i < statehook().GetCount(); i++)
...
void Ctrl::DoRemove() {
    ...
    if(HasChildDeep(mouseCtrl) || mouseCtrl == this)
        mouseCtrl = NULL;
    LLOG("DoRemove " << Name() << " focusCtrl: " << DRX::Name(focusCtrl));
    GuiPlatformRemove();
    Ctrl *parent = GetParent();
    if(HasFocusDeep()) {
        ...
        if(destroying) {
            if(parent) {
                LLOG("parent - deferred SetFocus / ChildLostFocus; parent = " << DRX::Name(parent));
            ...
            DoKillFocus(fc, NULL);
            if(parent) {
                LLOG("DoRemove -> SetFocus(" << DRX::Name(parent) << "), focusCtrl = " << DRX::Name(focusCtrl) << ", fc = " << DRX::Name(fc));
                bool b = IsWantFocus();
                NoWantFocus();
...
Ctrl::~Ctrl() {
    ...
    Close();
    KillTimeCallbacks(this, (u8 *) this + sizeof(Ctrl));
    ClearInfo();
    FreeFrames();
}

Vec<Ctrl::MouseHook>& Ctrl::mousehook() { static Vec<Ctrl::MouseHook> h; return h; }
Vec<Ctrl::KeyHook>&   Ctrl::keyhook() { static Vec<Ctrl::KeyHook> h; return h; }
Vec<Ctrl::StateHook>& Ctrl::statehook() { static Vec<Ctrl::StateHook> h; return h; }
...
{
    GuiLock __;
    i32 q = FindIndex(mousehook(), hook);
...
i32  Ctrl::HorzLayoutZoom(i32 cx)
{
    Csizeinit();
...
i32  Ctrl::VertLayoutZoom(i32 cy)
{
    Csizeinit();
    return cy > -16000 ? Csize.cy * cy / Dsize.cy : cy;
}

Size Ctrl::LayoutZoom(i32 cx, i32 cy)
{
...
Font FontZ(i32 face, i32 height)
{
...
{
    if(GetEnv("DINRUS_DISABLE_UHD__") == "1")
        return false;
...
Font StdFontZ(i32 height)   { return FontZ(Font::STDFONT, height); }
Font SansSerifZ(i32 height) { return FontZ(Font::SANSSERIF, height); }
Font SerifZ(i32 height)     { return FontZ(Font::SERIF, height); }
Font MonospaceZ(i32 height) { return FontZ(Font::MONOSPACE, height); }

Font ScreenSansZ(i32 height) { return FontZ(Font::SCREEN_SANS, height); }
Font ScreenSerifZ(i32 height) { return FontZ(Font::SCREEN_SERIF, height); }
Font ScreenFixedZ(i32 height) { return FontZ(Font::SCREEN_FIXED, height); }
Font RomanZ(i32 height) { return FontZ(Font::ROMAN, height); }
Font ArialZ(i32 height) { return FontZ(Font::ARIAL, height); }
Font CourierZ(i32 height) { return FontZ(Font::COURIER, height); }

Txt Ctrl::appname;

void Ctrl::SetAppName(const Txt& nm)
{
    GuiLock __;
    appname = nm;
}
...
Vec<Ctrl *> Ctrl::GetTopWindows()
{
    GuiLock __;
    Vec<Ctrl *> c = GetTopCtrls();
    Vec<Ctrl *> r;
...
{
    GuiLock __;
    Vec<Ctrl *> tc = Ctrl::GetTopCtrls();
    for(i32 i = 0; i < tc.GetCount(); i++)
        tc[i]->Close();
}
...
Vec< Ptr<Ctrl> > DisableCtrls(const Vec<Ctrl *>& ctrl, Ctrl *exclude)
{
    Vec< Ptr<Ctrl> > disabled;
    for(i32 i = 0; i < ctrl.GetCount(); i++) {
        Ctrl *q = ctrl[i];
...
Txt Ctrl::Name0() const {
    GuiLock __;
    Txt s = CppDemangle(typeid(*this).name());
    const Ctrl *q = this;
    Txt path;
    while(q) {
        Txt id = q->GetLayoutId();
        if(id.GetCount())
...
Txt Ctrl::Name(Ctrl *ctrl)
{
    return drx::Name(ctrl);
}
