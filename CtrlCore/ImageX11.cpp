...
{
	i32 q = FoldHash(GetHashVal(c)) % (i32)XRSolidFillCount;
...
Image SystemDraw::X11Cursor(i32 c)
{
	ImageBuffer b(32, 32);
	Image m(b);
	m.SetAuxData(c + 1);
...
uk SystemDraw::CursorX11(const Image& img)
{
	GuiLock __;
	i32 q = (z64)img.GetAuxData() - 1;
	if(q >= 0)
		return (uk )XCreateFontCursor(Xdisplay, q);
	i32 len = img.GetLength();
	Size sz = img.GetSize();
