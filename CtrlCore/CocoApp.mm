...
void CocoInit(i32 argc, tukk *argv, tukk *envptr)
{
	...
	id quitTitle = [@"Quit " stringByAppendingTxt:appName];
	...
	Font::SetFace(0, ToTxt((CFTxtRef)[sysfont familyName]), Font::TTF);
...
{
	GuiLock __;
	return i32(1000 * NSEvent.keyRepeatDelay);
}

i32 Ctrl::GetKbdSpeed()
{
	GuiLock __;
	return i32(1000 * NSEvent.keyRepeatInterval);
}
...
bool Ctrl::ProcessEvent(bool *)
{
	...
	// DLOG("ProcessEvent " << ToTxt(event.description));
...
Rect Ctrl::GetDefaultWindowRect()
{
	...
	static i32 pos = min(sz.cx / 10, 50);
	pos += 10;
	i32 cx = sz.cx * 2 / 3;
	i32 cy = sz.cy * 2 / 3;
...
{
	Tuple<i32, NSSearchPathDir> map[] = {
		{ SF_NSDocumentDir, NSDocumentDir },
		{ SF_NSUserDir, NSUserDir },
		{ SF_NSDesktopDir, NSDesktopDir },
		{ SF_NSDownloadsDir, NSDownloadsDir },
		{ SF_NSMoviesDir, NSMoviesDir },
		{ SF_NSMusicDir, NSMusicDir },
		{ SF_NSPicturesDir, NSPicturesDir },
	};
	
	if(auto *h = FindTuple(map, __countof(map), i)) {
		NSArray * paths = NSSearchPathForDirectoriesInDomains(h->b, NSUserDomainMask, YES);
		if(paths.count)
			return ToTxt([paths objectAtIndex:0]);
	}
