...
static drx::Point coco_mouse_pos;
static bool       coco_mouse_left;
static bool       coco_mouse_right;
static i32        coco_flags;
static drx::Ptr<drx::Ctrl> coco_capture;

drx::Ptr<drx::Ctrl> drx::Ctrl::lastActive;
...
struct MMImp {
	static bool KeyFlags(drx::Ctrl *ctrl, NSEvent *e) {
		bool alt = GetAlt();
		bool ctl = GetCtrl();
		bool sht = GetShift();
		bool opt = GetOption();
	...
	static bool MouseEvent(CocoView *view, NSEvent *e, i32 event, double zd = 0)
	{
		if(!view->ctrl)
			return false;
		Flags(e);
		...
		if(event == Ctrl::MOUSEMOVE) {
			...
			if(coco_capture)
				...
				Vec<Ctrl *> t = Ctrl::GetTopCtrls(); // Find window that contains the mouse, from the top
				for(NSNumber *num in [NSWindow windowNumbersWithOptions:0]) { // All app windows
					NSWindow *win = [NSApp windowWithWindowNumber:[num integerVal]];
					if(win) {
					    i32 q = FindMatch(t, [&](Ctrl *t) { return t->GetNSWindow() == win; });
					    if(q >= 0) {
					        Ctrl *w = t[q];
	...
	static bool MouseDownEvent(CocoView *view, NSEvent *e, i32 button)
	{
		if(!view->ctrl)
			return false;
		drx::Ctrl::lastActive = view->ctrl;
		...
		if(Ctrl::ignoreclick)
			return false;
		static i32 clicktime = msecs() - 100000;
		...
		if(msecs(clicktime) < GUI_DblClickTime() && view == dbl_view &&
		   max(abs(np.x - dbl_pos.x), abs(np.y - dbl_pos.y)) < 3) {
			b = MouseEvent(view, e, button|drx::Ctrl::DOUBLE);
			clicktime = msecs() - 999999;
		}
		else {
			b = MouseEvent(view, e, button|drx::Ctrl::DOWN);
			clicktime = msecs();
		}
		dbl_view = view;
		dbl_pos = np;
	...
	static void Paint(drx::Ctrl *ctrl, drx::SystemDraw& w, const Rect& r)
	{
		if(!ctrl)
			return;
	...
	static bool KeyEvent(drx::Ctrl *ctrl, NSEvent *e, i32 up) {
		if(!ctrl)
			return false;
		Flags(e);
		if(!ctrl->IsEnabled())
			return false;
		drx::dword k = e.keyCode;
		WTxt x = ToWTxt((CFTxtRef)(e.charactersIgnoringModifiers));
		if(x.GetCount() == 1)
			switch(ToUpper(x[0])) {
		...
		if(!up && !(k & (K_CTRL|K_ALT))) {
			WTxt x = ToWTxt((CFTxtRef)(e.characters));
	...
	static void BecomeKey(drx::Ctrl *ctrl)
	{
		if(!ctrl)
			return;
		LLOG("Become key " << drx::Name(ctrl));
		drx::Ctrl::lastActive = ctrl;
		ctrl->ActivateWnd();
		...
		drx::Ctrl::SyncAppIcon();
		drx::TopWindow::SyncMainMenu(false);
		drx::Ctrl::ReleaseCtrlCapture();
	}

	static void ResignKey(drx::Ctrl *ctrl)
	{
		if(!ctrl)
			return;
		LLOG("Resign key " << drx::Name(ctrl));
		ctrl->KillFocusWnd();
		drx::Ctrl::ReleaseCtrlCapture();
	}
	
	static void DoClose(drx::Ctrl *ctrl)
	{
		if(!ctrl)
			return;
		ctrl->MMClose();
	...
	static i32  DnD(drx::Ctrl *ctrl, id<NSDraggingInfo> info, bool paste = false)
	{
		if(!ctrl)
			return false;
		...
		ctrl->DnD(drx::Point(DPI(np.x), DPI(np.y)) + ctrl->GetScreenRect().TopLeft(), clip);
	...
	static void ShowPreedit(Ctrl *ctrl, const WTxt& text)
	{
		if(ctrl)
	...
	static void PreeditText(Ctrl *ctrl, const WTxt& s)
	{
		if(ctrl)
			for(drx::wchar ch : s)
				if(ch >= 32 && ch != 127 && ch != ' ')
...
-(void)drawRect:(NSRect)r {
	if(ctrl) {
		drx::SystemDraw w([[NSGraphicsContext currentContext] CGContext], self);
		drx::MMImp::Paint(ctrl, w, MakeRect(r, drx::DPI(1)));
	}
}

- (void)mouseDown:(NSEvent *)e {
	coco_mouse_left = true;
	if(!drx::MMImp::MouseDownEvent(self, e, drx::Ctrl::LEFT))
		[super mouseDown:e];
}

- (void)mouseUp:(NSEvent *)e {
	coco_mouse_left = false;
	if(!drx::MMImp::MouseEvent(self, e, drx::Ctrl::LEFTUP))
		[super mouseUp:e];
	drx::Ctrl::ReleaseCtrlCapture();
}

- (void)mouseMoved:(NSEvent *)e {
	if(!drx::MMImp::MouseEvent(self, e, drx::Ctrl::MOUSEMOVE))
		[super mouseMoved:e];
}
...
- (void)rightMouseDown:(NSEvent*)e {
	coco_mouse_right = true;
	if(!drx::MMImp::MouseDownEvent(self, e, drx::Ctrl::RIGHT))
		[super rightMouseDown:e];
}

- (void)rightMouseUp:(NSEvent*)e {
	coco_mouse_right = false;
	if(!drx::MMImp::MouseEvent(self, e, drx::Ctrl::RIGHTUP))
		[super rightMouseUp:e];
}

- (void)scrollWheel:(NSEvent *)e {
	if(!drx::MMImp::MouseEvent(self, e, drx::Ctrl::MOUSEWHEEL, [e deltaY]))
		[super scrollWheel:e];
}

- (void)keyDown:(NSEvent *)e {
    [self interpretKeyEvents: [NSArray arrayWithObject: e]];
	if(!drx::MMImp::KeyEvent(ctrl, e, 0))
		[super keyDown:e];
}

- (void)keyUp:(NSEvent *)e {
	if(!drx::MMImp::KeyEvent(ctrl, e, drx::K_KEYUP))
			[super keyUp:e];
}

- (void)flagsChanged:(NSEvent *)e {
	if(!drx::MMImp::KeyFlags(ctrl, e))
		[super flagsChanged:e];
}

- (void)cursorUpdate:(NSEvent *)event {
	drx::MMImp::Flags(event);
	drx::MMImp::DoCursorShape();
}

- (BOOL)windowShouldClose:(NSWindow *)sender {
	if(ctrl->IsEnabled())
		drx::MMImp::DoClose(ctrl);
	return NO;
}

- (void)windowDidResize:(NSNotification *)notification {
	drx::MMCtrl::SyncRect(self);
}
...
- (void)windowDidBecomeKey:(NSNotification *)notification {
	drx::MMImp::BecomeKey(ctrl);
}

- (void)windowDidResignKey:(NSNotification *)notification {
	drx::MMImp::ResignKey(ctrl);
}
...
{
	return drx::MMImp::DnD(ctrl, sender);
}
...
{
	return drx::MMImp::DnD(ctrl, sender, true) != NSDragOperationNone;
}
...
- (void)updateTrackingAreas
{
	...
	drx::Size sz = ctrl->GetScreenRect().GetSize();
...
-(void)insertText:(id)aTxt replacementRange:(NSRange)replacementRange
{
    (void) replacementRange;

    NSTxt* pInsert = [aTxt isMemberOfClass: [NSAttributedTxt class]] ? [aTxt string] : aTxt;

	if(pInsert)
		drx::MMImp::PreeditText(ctrl, drx::ToWTxt(pInsert));
}
...
{
	return drx::MMImp::PreeditRect(ctrl);
}

- (void)setMarkedText:(id)aTxt selectedRange:(NSRange)selRange replacementRange:(NSRange)replacementRange
{
    if(![aTxt isKindOfClass:[NSAttributedTxt class]] )
        aTxt = [[[NSAttributedTxt alloc] initWithTxt:aTxt] autorelease];

	drx::MMImp::ShowPreedit(ctrl, drx::ToWTxt([aTxt string]));
}
...
- (NSAttributedTxt *)attributedSubstringForProposedRange:(NSRange)aRange actualRange:(NSRangePointer)actualRange
{
	(void) aRange;
	(void) actualRange;
	return nil;
