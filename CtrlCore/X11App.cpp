...
bool WaitForEvent(Window w, i32 type, XEvent& event){
	GuiLock __;
	for(i32 i = 0; i < 80; i++) {
...
Txt ReadPropertyData(Window w, Atom property, Atom rtype)
{
	GuiLock __;
		...
		if(type == XA_INCR) {
			...
			for(;;) {
				...
				if(!WaitForEvent(w, PropertyNotify, event))
					break;
				if(event.xproperty.atom == property && event.xproperty.state == PropertyNewVal) {
					Txt x = GetProperty(w, property);
					if(!x.GetLength())
						break;
					r.Cat(x);
					XDeleteProperty(Xdisplay, w, property);
...
Vec<i32> GetPropertyInts(Window w, Atom property, Atom rtype)
{
	GuiLock __;
	Vec<i32> result;
	Txt p = GetProperty(w, property, rtype);
	i64k *ptr = (i64k *)~p;
...
void sPanicMessageBox(tukk title, tukk text)
{
	IGNORE_RESULT(
	...
	i32 x = (DisplayWidth(display, screen) - 600) / 2;
	i32 y = (DisplayHeight(display, screen) - 120) / 2;
	...
	for(;;) {
		...
		case Expose:
			...
			for(;;) {
				tukk e = strchr(b, '\n');
				if(!e) break;
				XDrawTxt(display, win, gc, 20, y, b, e - b);
				y += font_info->max_bounds.ascent + font_info->max_bounds.descent;
				b = e + 1;
			}
			XDrawTxt(display, win, gc, 20, y, b, strlen(b));
			break;
		}
	}
}
...
i32 X11ErrorHandler(XDisplay *, XErrorEvent *error)
{
	...
	e << "\nresource id: " << (i32)error->resourceid << " = " << Format("%0X", (i32)error->resourceid);
...
void Ctrl::InitX11(tukk display)
{
	GuiLock __;
	...
	Vec<i32> nets = GetPropertyInts(Xroot, XAtom("_NET_SUPPORTED"));
	for(i32 i = 0; i < nets.GetCount(); i++)
...
Vec<Rect> FindScreensResolutions()
{
	Vec<Rect> screensResolutions;
	i32 event, error;
...
Vec<Rect> FindScreensStruts()
{
	Vec<Rect> struts;

	Vec<i32> clients = GetPropertyInts(Xroot, XAtom("_NET_CLIENT_LIST"));
	for (i32 i = 0; i < clients.GetCount(); i++) {
		Vec<i32> strut = GetPropertyInts(clients[i], XAtom("_NET_WM_STRUT"));
		if(strut.GetCount() == 4)
...
Rect Ctrl::GetDefaultWindowRect()
{
	...
	static i32 top    = 0;
	if (width == 0 && height == 0) {
		Vec<Rect> screens = FindScreensResolutions();
		if(screens.GetCount()) {
	...
	static i32 pos = min(width / 10, 50);
	pos += 10;
	i32 cx = width * 2 / 3;
	i32 cy = height * 2 / 3;
...
{
	Vec<Rect> workAreas = FindScreensResolutions();
	Vec<Rect> struts    = FindScreensStruts();
	for (i32 i = 0; i < workAreas.GetCount(); i++) {
...
Rect Ctrl::GetVirtualWorkArea()
{
	...
	static Rect r;
	if(r.right == 0) {
		Vec<i32> x = GetPropertyInts(Xroot, XAtom("_NET_WORKAREA"));
		if(x.GetCount())
