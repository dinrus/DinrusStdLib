topic "Реакция на действительное разрешение, поддержка UHD";
[l288;i1120;a17;O9;~~~.1408;2 $$1,0#10431211400427159095818037425705:param]
[a83;*R6 $$2,5#31310162474203024125188417583966:caption]
[H4;b83;*4 $$3,5#07864147445237544204411237157677:title]
[i288;O9;C2 $$4,6#40027414424643823182269349404212:item]
[b42;a42;2 $$5,5#45413000475342174754091244180557:text]
[l288;b17;a17;2 $$6,6#27521748481378242620020725143825:desc]
[l321;C@5;1 $$7,7#20902679421464641399138805415013:code]
[b2503;2 $$8,0#65142375456100023862071332075487:separator]
[*@(0.0.255)2 $$9,0#83433469410354161042741608181528:base]
[C2 $$10,0#37138531426314131251341829483380:class]
[l288;a17;*1 $$11,11#70004532496200323422659154056402:requirement]
[i417;b42;a42;O9;~~~.416;2 $$12,12#10566046415157235020018451313112:tparam]
[b167;C2 $$13,13#92430459443460461911108080531343:item1]
[i288;a42;O9;C2 $$14,14#77422149456609303542238260500223:item2]
[*@2$(0.128.128)2 $$15,15#34511555403152284025741354420178:NewsDate]
[l321;*C$7;2 $$16,16#03451589433145915344929335295360:result]
[l321;b83;a83;*C$7;2 $$17,17#07531550463529505371228428965313:result`-line]
[l160;*C+117 $$18,5#88603949442205825958800053222425:package`-title]
[2 $$19,0#53580023442335529039900623488521:gap]
[C2 $$20,20#70211524482531209251820423858195:class`-nested]
[b50;2 $$21,21#03324558446220344731010354752573:Par]
[2 $$0,0#00000000000000000000000000000000:Default]
[{_}%RU-RU 
[s2; [*@5 Реакция на действительное разрешение, 
поддержка UHD]&]
[s0; Та как координаты всех виджетов всегда 
заданы в пикселях, приложению приходится 
реагировать на действительную настройку 
целевого ГИП, чтобы соответствующим 
образом менять масштабы.&]
[s0; &]
[s0; В U`+`+, базовый фактор масштабирования 
всегда определён размером стандартного 
шрифта, который считывается из хостовой 
платформы (фактор масштабирования 
может регулироваться и самим приложением, 
но это в данном случае не очень уместно).&]
[s0; &]
[s0; Все выкладки прорабатываются в некотором 
искуственно лпределённом, относительно 
небольшом, размере `'базового шрифта`', 
затем масштабируются на основе пропорций 
между стандартным системным шрифтом 
и этим базовым. Также можно масштабировать 
отдельные измерения посредством 
функций Ctrl`::`[Horz`|Vert`]LayoutZoom или [^topic`:`/`/CtrlCore`/src`/Zooming`_ru`-ru^ Z
x/Zy/Zsz].&]
[s0; &]
[s0; При правильном использовании это 
неплохо работает для шрифта ГИП хоста 
размером 12`-22. Когда шрифт ГИП хоста 
увеличивается, начинается проблема 
с рисунками Images (например, иконками 
кнопок), так как они становятся слишком 
маленькими. Это типично для дисплея 
UHD.&]
[s0; &]
[s0; Для решения этой проблемы приходится 
использовать увеличенные рисунки. 
Эта проблема отсутствует для рисунков, 
создаваемых кодом (так как они могут 
легко применять фактор масштабирования 
в своём алгоритме),  но это становится 
проблемой для рисунков из ресурсов 
.iml.&]
[s0; &]
[s0; Для решения этой проблемы у Image есть 
информация о разрешении. Она может 
содержать 3 значения `- Standard, UHD, None. 
Эту информацию можно настроить при 
разработке иконки (кликнуть правой 
кнопкой мыши на названии рисунка,выбрать 
`"Рисунок...`", или дважды кликнуть на 
названии рисунка)&]
[s0;= &]
[s0;= 
@@rawimage:1953&2000
()
&]
[s0; &]
[s0; U`+`+ можно переключить в [^topic`:`/`/Draw`/src`/UHD`_ru`-ru`#Upp`:`:SetUHDMode`(bool`)^ р
ежим `"UHD`"], в котором, если иконка достаётся 
из .iml и она в стандартном разрешении, 
то она увеличивается в 2x (используется 
относительно умный алгоритм, поэтому 
на дисплее UHD она выглядит довольно 
неплохо). Соответственно, в стандартном 
режиме иконки с разрешением UHD уменьшаются. 
Иконки с разрешением `"None`" остаются 
нетронутыми.&]
[s0; &]
[s0; Можно и нужно иметь оба, и стандартный, 
и UHD варианты в одном т том же файле 
.iml. При случае,в варианте UHD [/ name] меняется 
на [/ name]`_`_UHD в коде на C`+`+ (чтобы избежать 
конфликта имён). Также, в режиме UHD, 
UHD вариант используется для переписания 
стандартного.&]
[s0; &]
[s0; UHD, как правило, активируется, когда 
стандартная высота иконки больше, 
чем 22.&]
[s0; &]
[s0; В дополнение к функциям Zx/Zy/Zsz, U`+`+ 
также предоставляет набор [^topic`:`/`/Draw`/src`/UHD`_ru`-ru^ ф
ункций DPI] , который просто уаваивает 
аргументы, когда активен режим UHD, 
`- иногда это лучший подход к масштабированию
 рисунков.&]
[s0; &]
[s0; Тем не менее, некотоорые хостовые 
платформы (Windows) нуждаются в особом 
информировании о том, что приложение 
может справляться с режимом UHD, иначе 
они входят в режим, когда приложение 
отображается в стандартном разрешении, 
но с увеличенными окнами (что выглядит 
просто ужас...). Обычно U`+`+ по умолчанию 
в этом `"HiDPI`" режиме.  Его можно вернуть 
в грубый режим увеличения вызовом 
Ctrl`::SetUHDEnabled(false) в INITIBLOCK.]]