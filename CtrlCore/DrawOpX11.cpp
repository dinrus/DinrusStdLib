...
bool SystemDraw::ClipOp(const Rect& r)
{
	...
	Cloff f = cloff.Top();
	bool ch = false;
	Vec<Rect> newclip = Intersect(clip.Top(), r + actual_offset, ch);
	if(ch) {
		f.clipi = clip.GetCount();
		clip.Add() = pick(newclip);
	}
...
bool SystemDraw::ExcludeClipOp(const Rect& r)
{
	...
	CloneClip();
	Vec<Rect>& cl = clip.Top();
	bool ch = false;
	Vec<Rect> ncl = Subtract(cl, r + actual_offset, ch);
	if(ch) {
		cl = pick(ncl);
		SetClip();
	}
...
bool SystemDraw::IntersectClipOp(const Rect& r)
{
	...
	Vec<Rect>& cl = clip.Top();
	bool ch = false;
	Vec<Rect> ncl = Intersect(cl, r + actual_offset, ch);
	if(ch) {
		cl = pick(ncl);
		SetClip();
	}
	                          ...
	                          i32 width, Color color, Color doxor)
{
	...
	XGetGCVals(Xdisplay, GetGC(), GCForeground | GCFunction, &gcv_old);
	...
	if(vertex_count == 2)
		...
		i32 segment_count = vertex_count - count_count;
...
static void DrawPolyPolyPolygonRaw(SystemDraw& draw, const Point *vertices, i32 vertex_count,
	i32k *subpolygon_counts, i32 subpolygon_count_count, i32k *, i32)
{
	GuiLock __;
	Point offset = draw.GetOffset();
	const Point *in = vertices;
	for(i32 i = 0; i < subpolygon_count_count; i++) {
		i32 n = subpolygon_counts[i];
	...
	Color color, i32 width, Color outline, zu64 pattern, Color doxor)
{
	...
	if(!IsNull(outline))
	{
		...
		Point offset = GetOffset();
		for(i32k *sp = subpolygon_counts, *se = sp + subpolygon_count_count; sp < se; sp++)
		{
...
void SystemDraw::DrawEllipseOp(const Rect& r, Color color, i32 pen, Color pencolor)
{
	GuiLock __;
	SetLineStyle(pen);
	if(!IsNull(color)) {
