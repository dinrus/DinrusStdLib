...
void SetSurface(SystemDraw& w, const Rect& dest, const RGBA *pixels, Size srcsz, Point poff)
{
	...
	cairo_surface_t *surface = cairo_image_surface_create_for_data((u8 *)pixels, CAIRO_FORMAT_ARGB32, dsz.cx, dsz.cy, 4 * dsz.cx);
...
cairo_surface_t *CreateCairoSurface(const Image& img, cairo_surface_t *other)
{
	...
	u8 *a = (u8 *)cairo_image_surface_get_data(surface);
	i32 stride = cairo_format_stride_for_width(fmt, isz.cx);
	for(i32 yy = 0; yy < isz.cy; yy++) {
...
struct ImageSysDataMaker : LRUCache<ImageSysData, z64>::Maker {
	...
	virtual i32    Make(ImageSysData& object) const { object.Init(img, other); return img.GetLength(); }
};
...
void CairoGet(ImageBuffer& b, Size isz, cairo_surface_t *surface, cairo_surface_t *alpha_surface)
{
	cairo_surface_flush(surface);
	u8 *a = (u8 *)cairo_image_surface_get_data(surface);
	i32 stride = cairo_format_stride_for_width(CAIRO_FORMAT_ARGB32, isz.cx);
	RGBA *t = b;
	u8 *aa = NULL;
	if(alpha_surface) {
		cairo_surface_flush(alpha_surface);
		aa = (u8 *)cairo_image_surface_get_data(alpha_surface);
	}
	for(i32 yy = 0; yy < isz.cy; yy++) {
		RGBA *s = (RGBA *)a;
		RGBA *e = s + isz.cx;
