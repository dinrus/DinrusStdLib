...
struct ImageSysDataMaker : LRUCache<ImageSysData, z64>::Maker {
	...
	virtual i32    Make(ImageSysData& object) const { object.Init(img); return img.GetLength(); }
};

static LRUCache<ImageSysData, z64> cg_image_cache;
...
void SystemDraw::SysDrawImageOp(i32 x, i32 y, const Image& img, Color color)
{
	GuiLock __;
...
struct NSImageSysDataMaker : LRUCache<NSImageSysData, z64>::Maker {
	Image img;

	virtual z64  Key() const                        { return img.GetSerialId(); }
	virtual i32    Make(NSImageSysData& object) const { object.Init(img); return img.GetLength(); }
};

static LRUCache<NSImageSysData, z64> nsimage_cache;
...
void ImageDraw::Init(i32 cx, i32 cy)
{
	ib.Create(cx, cy);
...
Image GetIconForFile(tukk path)
{
	...
	NSImage *image;
	CFRef<CFTxtRef> fexe = CFTxtCreateWithCTxt(NULL, path, kCFTxtEncodingUTF8);
	image = [[NSWorkspace sharedWorkspace]iconForFile:(__bridge NSTxt *)~fexe];
/*
	}
	else { // not used any more
		CFRef<CFTxtRef> fext = CFTxtCreateWithCTxt(NULL, path, kCFTxtEncodingUTF8);
		image = *ext == '*' ? [[NSWorkspace sharedWorkspace] iconForFileType:NSFileTypeForHFSTypeCode(kGenericFolderIcon)]
	                        : [[NSWorkspace sharedWorkspace]iconForFileType:(__bridge NSTxt *)~fext];
	}
*/
