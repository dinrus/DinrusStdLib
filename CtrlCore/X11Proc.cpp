...
// 01/12/2007 - END
	...
	case KeyRelease: {
			...
			if(pressed && w.xic) {
				Status status;
				i32 len = Xutf8LookupTxt(w.xic, &event->xkey, buff, sizeof(buff), &keysym, &status);
				buff[len] = 0;
				...
				i32 len = XLookupTxt(&event->xkey, buff, sizeof(buff), &keysym, NULL);
				buff[len] = 0;
				chr = ToUtf32(buff, len)[0];
				if(len > 1)
					wtext = ToUtf32(buff, len);
			...
			for(i32 i = 0; i < __countof(keypadtab); i++) {
			...
			if(GetCtrl() || GetAlt()) // Send Numpad keycodes.
				for(i32 i = 0; i < __countof(numpadtab); i++) {
			...
			if(GetCtrl() || GetAlt()) { // fix Ctrl+Shift+1 etc...
				keysym = decode((i32)event->xkey.keycode,
								0x0a, 0x31,
								0x0b, 0x32,
								0x0c, 0x33,
								0x0d, 0x34,
			...
			if(chr && pressed) {
				...
				for(i32 ii = 1; ii < wtext.GetLength(); ii++)
