...
- (BOOL)canBecomeMainWindow {
	LLOG("canBecomeMainWindow " << drx::Name(ctrl) << ", owner " << drx::Name(ctrl->GetOwner()));
	return active && ctrl && ctrl->IsEnabled() && dynamic_cast<drx::TopWindow *>(~ctrl) && !ctrl->GetOwner();
}
...
static Vec<Ptr<Ctrl>> mmtopctrl; // should work without Ptr, but let us be defensive....
...
uk Ctrl::GetNSWindow() const
{
	return top && GetTop()->coco ? GetTop()->coco->window : NULL;
}

uk Ctrl::GetNSView() const
{
...
void Ctrl::WndDestroy()
{
	...
	DeleteTop();
	popup = isopen = false;
	i32 ii = FindIndex(mmtopctrl, this);
	if(ii >= 0)
		mmtopctrl.Remove(ii);
...
bool Ctrl::IsWndOpen() const {
	GuiLock __;
//	DLOG("IsWndOpen " << drx::Name(this) << ": " << (bool)top);
	return top;
}
...
void TopWindow::SyncTitle()
{
	...
	if(top) {
		...
		CFRef<CFTxtRef> s = CFTxtCreateWithCTxt(NULL, (tukk )~title.ToTxt(), kCFTxtEncodingUTF8);
		[GetTop()->coco->window setTitle:(NSTxt *)~s];
	}
}
