...
static i32 GetParaHeight(const Array<RichPara::Part>& parts)
{
	i32 ht = 0;
	for(i32 i = 0; i < parts.GetCount(); i++) {
		i32 pht = 0;
...
class RTFEncoder {
public:
	RTFEncoder(Stream& stream, const RichText& richtext, u8 charset,
		Size dot_page_size, const Rect& dot_margins, uk context);
...
private:
	...
	void            PutHeader();
	void            PutDocument();
	void            PutTxt(const RichTxt& txt, i32 nesting, i32 dot_width);
	void            PutTable(const RichTable& table, i32 nesting, i32 dot_width);
	void            PutParts(const Array<RichPara::Part>& parts,
		RichPara::CharFormat& base, i32 bpart, i32 boff, i32 epart, i32 eoff);

	void            Begin()                           { stream.Put('{'); }
	void            Begin(tukk cmd)            { Begin(); Command(cmd); }
	void            Begin(tukk cmd, i32 param) { Begin(); Command(cmd, param); }
	void            End()                             { stream.Put('}'); }

	void            Command(tukk cmd);
	void            Command(tukk cmd, i32 param);
	void            Space()                           { stream.Put(' '); }
	void            PutText(wtukk text);
	void            PutObject(const RichObj& object);
	void            PutTabs(const Vec<RichPara::Tab>& tabs);
	void            PutBinHex(u8k *data, i32 count);
	void            PutBinHex(const Txt& s)        { PutBinHex(s, s.GetLength()); }
	void            PutBorder(TxtBuf& rowfmt, i32 wd, Color c, tukk clpar);
	...
	struct Group {
		...
		Group(RTFEncoder *owner, tukk cmd) : owner(owner) { owner->Begin(cmd); }
		Group(RTFEncoder *owner, tukk cmd, i32 param) : owner(owner) { owner->Begin(cmd, param); }
		~Group() { owner->End(); }
		RTFEncoder *owner;
	};
...
private:
	...
	Index<i32>            used_faces;
	enum { SYMBOL_INDEX = 1, WINGDINGS_INDEX = 2 };
	VecMap<Color, i32> used_ink, used_paper;
...
Txt EncodeRTF(const RichText& richtext, u8 charset, Size dot_page_size, const Rect& dot_margins, uk context)
{
	TxtStream out;
...
{
	i32 dot_page_height = iscale(dot_page_width, 1414, 1000);
...
RTFEncoder::RTFEncoder(Stream& stream_, const RichText& richtext_, u8 charset_,
	Size dot_page_size_, const Rect& dot_margins_, uk context_)
: stream(stream_)
, richtext(richtext_)
, charset(charset_)
...
{
	for(i32 i = 0; i < richtext.GetStyleCount(); i++)
...
{
	for(i32 i = 0; i < rt.GetPartCount(); i++) {
		if(rt.IsTable(i)) {
			...
			for(i32 r = 0; r < table.GetRows(); r++)
				for(i32 c = 0; c < table.GetColumns(); c++) {
			...
			for(i32 p = 0; p < para.part.GetCount(); p++) {
...
void RTFEncoder::Command(tukk cmd)
{
	stream.Put('\\');
	stream.Put(cmd);
}

void RTFEncoder::Command(tukk cmd, i32 param)
{
	stream.Put('\\');
	stream.Put(cmd);
...
void RTFEncoder::PutText(wtukk text)
{
	for(; *text; text++)
		if(*text >= 128) {
			if(*text >= 65536) {
				Vec<char16> h = ToUtf16(*text);
				stream << "\\uc0 \\u" << (unsigned)h[0] << "\\uc1 \\u" << (unsigned)h[1];
			}
			else
				stream.Put(Format("\\u%d?", (i16)*text));
		}
		else {
			if(*text == '{' || *text == '}' || *text == '\\')
				stream.Put('\\');
...
void RTFEncoder::PutBinHex(u8k *b, i32 count)
{
	enum { BLOCK = 32 };
	for(i32 l = count; l > 0; l -= BLOCK) {
		stream.PutCrLf();
		for(u8k *e = b + min<i32>(l, BLOCK); b < e; b++) {
...
void RTFEncoder::PutObject(const RichObj& object)
{
	...
	Command("pngblip");
	PutBinHex(PNGEncoder().SaveTxt(object.ToImage(out_size)));
}
...
bool RTFEncoder::PutParaFormat(const RichPara::Format& pf, const RichPara::Format& difpf)
{
	...
	i32 oind = difpf.indent, olm = difpf.lm;
	if(difpf.bullet != RichPara::BULLET_NONE) {
		olm += oind;
		oind = -oind;
	}
	i32 nind = pf.indent, nlm = pf.lm;
...
void RTFEncoder::PutTabs(const Vec<RichPara::Tab>& tabs)
{
...
void RTFEncoder::PutHeader()
{
	...
	i32 fontcpg = Null;
	for(i32 i = __countof(ansicpg); --i >= 0;)
	...
	Command("deff", 0);
	{
		...
		for(i32 i = 0; i < used_faces.GetCount(); i++) {
			Group fnt(this, "f", i);
			i32 fn = used_faces[i];
			i32 cs = fontcpg;
	...
	if(phys_colors.GetCount() > 1) {
		...
		stream.Put(';');
		for(i32 i = 1; i < phys_colors.GetCount(); i++) {
...
void RTFEncoder::PutTable(const RichTable& table, i32 nesting, i32 dot_width)
{
	Vec<i32> vspan_counts;
	const RichTable::Format& tfmt = table.GetFormat();
	Vec<i32> column_pos;
	column_pos.SetCount(tfmt.column.GetCount() + 1);
	column_pos[0] = tfmt.lm;
	i32 sum = 0;
	for(i32 c = 0; c < tfmt.column.GetCount(); c++)
		sum += tfmt.column[c];
	i32 rem_width = dot_width - tfmt.lm - tfmt.rm;
	for(i32 c = 0; c < tfmt.column.GetCount(); c++) {
		i32 part = iscale(tfmt.column[c], rem_width, max(sum, 1));
		sum -= tfmt.column[c];
		rem_width -= part;
...
*/
			...
			i32 fw = (!IsNull(tf.framecolor) ? tf.frame : 0);
			if(isleft) {
				lb = fw;
				lc = tf.framecolor;
			}
		...
		if(!nesting)
			stream.Put(fmtstr);
		for(i32 c = 0; c < cellindex.GetCount(); c++) {
			i32 cx = cellindex[c];
			const RichCell& cell = table[r][cx];
			i32 cell_wd = column_pos[cx + cell.hspan + 1] - column_pos[cx];
...
void RTFEncoder::PutBorder(TxtBuf& rowfmt, i32 wd, Color c, tukk clpar)
{
	if(wd > 0) {
...
void RTFEncoder::PutTxt(const RichTxt& txt, i32 nesting, i32 dot_width)
{
	for(i32 i = 0; i < txt.GetPartCount(); i++) {
		if(txt.IsTable(i))
			PutTable(txt.GetTable(i), nesting, dot_width);
		else {
			const RichPara& para = txt.Get(i, richtext.GetStyles());
			Uuid pstyle = txt.GetParaStyle(i);
			i32 new_para_ht = GetParaHeight(para.part);
			i32 first_part = 0, first_ind = 0;
