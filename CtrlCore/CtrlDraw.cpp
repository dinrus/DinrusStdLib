...
void Ctrl::Refresh(i32 x, i32 y, i32 cx, i32 cy) {
	Refresh(RectC(x, y, cx, cy));
}

void Ctrl::RefreshFrame(i32 x, i32 y, i32 cx, i32 cy) {
...
void  Ctrl::ScrollRefresh(const Rect& r, i32 dx, i32 dy)
{
	sCheckGuiLock();
	GuiLock __; // Beware: Even if we have ThreadHasGuiLock ASSERT, we still can be the main thread!
	LLOG("ScrollRefresh " << r << " " << dx << " " << dy);
	if(!IsOpen() || !IsVisible() || r.IsEmpty()) return;
	i32 tdx = tabs(dx), tdy = tabs(dy);
...
bool Ctrl::AddScroll(const Rect& sr, i32 dx, i32 dy)
{
	GuiLock __;
	Top *top = GetTop();
	if(!top)
		return true;
	for(i32 i = 0; i < top->scroll.GetCount(); i++) {
...
void Ctrl::ScrollCtrl(Top *top, Ctrl *q, const Rect& r, Rect cr, i32 dx, i32 dy)
{
	if(top && r.Intersects(cr)) { // Uno: Contains -> Intersetcs
		...
		if(r.Intersects(cr.Offseted(dx, dy))) { // Uno's suggestion 06/11/26 Contains -> Intersetcs
			...
			m.to = to;
			m.ctrl = q;
			LLOG("ScrollView Add " << DRX::Name(q) << from << " -> " << to);
			return;
		}
		cr &= r;
		if(!cr.IsEmpty()) {
...
void  Ctrl::ScrollView(const Rect& _r, i32 dx, i32 dy)
{
	GuiLock __;
	LLOG("ScrollView " << _r << " " << dx << " " << dy);
#ifdef GUITARGET_NOSCROLL
	LLOG("NOSCROLL");
	Refresh(_r);
#else
...
void  Ctrl::ScrollView(i32 x, i32 y, i32 cx, i32 cy, i32 dx, i32 dy) {
	ScrollView(RectC(x, y, cx, cy), dx, dy);
}

void  Ctrl::ScrollView(i32 dx, i32 dy) {
...
void  Ctrl::SyncScroll()
{
	...
	if(!top)
		return;
	Vec<Scroll> scroll = pick(top->scroll);
	top->scroll.Clear();
	if(IsFullRefresh())
		return;
	for(i32 i = 0; i < scroll.GetCount(); i++) {
		Scroll& sc = scroll[i];
...
struct sDrawLevelCheck {
	...
	void Check() {
		ASSERT_(lvl == w.GetCloffLevel(), "Несовпадение Draw::Begin/End для " + DRX::Name(q));
	}
...
void Ctrl::CtrlPaint(SystemDraw& w, const Rect& clip) {
	...
	LEVELCHECK(w, this);
	LTIMING("CtrlPaint");
	LLOG("=== CtrlPaint " << DRX::Name(this) << ", clip: " << clip << ", rect: " << GetRect() << ", view: " << GetView());
...
inline i32 Area(const Rect& r)
{
	return r.GetHeight() * r.GetWidth();
}

void CombineArea(Vec<Rect>& area, const Rect& r)
{
	LTIMING("CombineArea");
	if(r.IsEmpty()) return;
	i32 ra = Area(r);
	for(i32 i = 0; i < area.GetCount(); i++) {
		Rect ur = r | area[i];
		i32 a = Area(ur);
...
void Ctrl::GatherTransparentAreas(Vec<Rect>& area, SystemDraw& w, Rect r, const Rect& clip)
{
	GuiLock __;
...
void Ctrl::UpdateArea0(SystemDraw& draw, const Rect& clip, i32 backpaint)
{
	GuiLock __;
	LTIMING("UpdateArea");
	LLOG("========== UPDATE AREA " << DRX::Name(this) << ", clip: " << clip << " ==========");
...
void Ctrl::DrawCtrlWithParent(Draw& w, i32 x, i32 y)
{
	GuiLock __;
	Ctrl *parent = GetParent();
	if(parent) {
...
void Ctrl::DrawCtrl(Draw& w, i32 x, i32 y)
{
	GuiLock __;
	w.Offset(x, y);
...
void Ctrl::SyncMoves()
{
	...
	if(!top)
		return;
	for(i32 i = 0; i < top->move.GetCount(); i++) {
	...
	for(i32 i = 0; i < top->scroll_move.GetCount(); i++) {
