...
XEvent ClientMsg(Window src, Atom type, i32 format = 32)
{
	XEvent e;
	Zero(e);
...
struct DnDLoop : LocalLoop {
	...
	void Sync();
	Val  GetData() const { return LocalLoop::GetData(); } // Silence CLANG warning
...
void DnDLoop::Sync()
{
	...
	for(;;) {
		if(XQueryPointer(Xdisplay, tgt, &root, &tgt, &x, &y, &d2, &d2, &d1)) {
			if(!tgt)
				break;
			Vec<i32> x = GetPropertyInts(tgt, XAtom("XdndAware"));
	...
	if(tgt != target) {
		...
		if(target) {
			...
			for(i32 i = 0; i < min(3, fmt.GetCount()); i++)
...
void Ctrl::DropStatusEvent(XEvent *event)
{
	...
	if(event->type == ClientMessage && dndloop && event->xclient.data.l[0] == (i32)dndloop->target) {
		if(event->xclient.message_type == XdndStatus && Xdnd_waiting_status) {
			...
			if(Xdnd_status == DND_NONE)
				Xdnd_status = (event->xclient.data.l[1] & 1) ?
				                 event->xclient.data.l[4] == (i32)XdndActionMove ? DND_MOVE : DND_COPY
				              : DND_NONE;
			Xdnd_waiting_status = false;
		}
		if(event->xclient.message_type == XdndFinished && Xdnd_waiting_finished) {
			...
			if(Xdnd_version == 5)
				Xdnd_status = (event->xclient.data.l[1] & 1) ?
				                 event->xclient.data.l[2] == (i32)XdndActionMove ? DND_MOVE : DND_COPY
				              : DND_NONE;
...
void DnDLoop::Request(XSelectionRequestEvent *se)
{
	...
	if(se->target == XAtom("TARGETS")) {
		...
		Txt d = GetData(XAtomName(se->target));
		if(d.GetCount())
...
void Ctrl::DropEvent(XWindow& w, XEvent *event)
{
	...
	if(event->xclient.message_type == XdndEnter) {
		...
		if(event->xclient.data.l[1] & 1) {
			Vec<i32> v = GetPropertyInts(src, XAtom("XdndTypeList"), XA_ATOM);
	...
	if(event->xclient.message_type == XdndPosition) {
		...
		XdndAction = event->xclient.data.l[4] == (i32)XdndActionMove ? DND_MOVE : DND_COPY;
		DnD(src, false);
	}
	if(event->xclient.message_type == XdndLeave)
		DnDLeave();
	if(event->xclient.message_type == XdndDrop && dndctrl) {
		LLOG("XdndDrop to " << DRX::Name(dndctrl));
		DnD(src, true);
		DnDLeave();
	}
}
