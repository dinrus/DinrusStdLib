...
Rect RectTracker::Track(const Rect& r, i32 _tx, i32 _ty)
{
	rect = r;
	tx = _tx;
	ty = _ty;
...
i32 RectTracker::TrackHorzLine(i32 x0, i32 y0, i32 cx, i32 line)
{
	return Track(RectC(x0, y0, cx, line + 1), -1, ALIGN_BOTTOM).bottom - 1;
}

i32 RectTracker::TrackVertLine(i32 x0, i32 y0, i32 cy, i32 line)
{
	return Track(RectC(x0, y0, line + 1, cy), ALIGN_RIGHT, -1).right - 1;
}

Point RectTracker::TrackLine(i32 x0, i32 y0)
{
...
void RectTracker::MouseMove(Point mp, dword){
	...
	if(tx == ALIGN_CENTER && ty == ALIGN_CENTER) { // move rectangle
		i32 x = org.left - op.x + p.x;
		i32 y = org.top - op.y + p.y;
		...
		if(keepratio) {
			i32 cy = org.Width() ? rect.Width() * org.Height() / org.Width() : 0;
			i32 cx = org.Height() ? rect.Height() * org.Width() / org.Height() : 0;
...
public:
	...
	PointLooper(Ctrl& ctrl, const Vec<Image>& ani, i32 ani_ms)
...
{
	return ani[i32(GetTimeClick() / ani_ms % ani.GetCount())];
}

bool  PointLooper::Key(dword key, i32)
{
	if(key == K_ESCAPE) {
		result = false;
		EndLoop();
