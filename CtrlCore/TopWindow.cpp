...
{
	LLOG("Activate " << Name() << " activefocus = " << DRX::Name(activefocus));
...
{
	LLOG("DeActivate current focus " << DRX::Name(GetFocusCtrl()));
	if(HasFocusDeep())
		activefocus = GetFocusCtrl();
	USRLOG("   DEACTIVATE " + Desc(this));
	LLOG("DeActivate " << Name() << " activefocus = " << DRX::Name(activefocus));
}

void TopWindow::PlaceFocus()
{
	LLOG("PlaceFocus " << drx::Name(this));
	if(activefocus)
...
void TopWindow::Break(i32 ID)
{
	EndLoop(ID);
}

bool TopWindow::AcceptBreak(i32 ID)
{
	if(Accept()) {
		Break(ID);
		return true;
...
void TopWindow::RejectBreak(i32 ID)
{
	Reject();
	Break(ID);
}
...
TopWindow::Abreak *TopWindow::FindAction(i32 ID)
{
	for(i32 i = 0; i < action.GetCount(); i++)
		if(action[i].ID == ID) return &action[i];
	return NULL;
}

TopWindow::Abreak *TopWindow::FindAddAction(i32 ID)
{
...
Callback TopWindow::Breaker(i32 ID)
{
	return callback(FindAddAction(ID), &Abreak::Break);
}

Callback TopWindow::Acceptor(i32 ID)
{
	return callback(FindAddAction(ID), &Abreak::Accept);
}

Callback TopWindow::Rejector(i32 ID)
{
	return callback(FindAddAction(ID), &Abreak::Reject);
}

TopWindow& TopWindow::Breaker(Ctrl& m, i32 ID)
{
	m.WhenAction = Breaker(ID);
	return *this;
}

TopWindow& TopWindow::Acceptor(Ctrl& m, i32 ID)
{
	m.WhenAction = Acceptor(ID);
	return *this;
}

TopWindow& TopWindow::Rejector(Ctrl& m, i32 ID)
{
...
bool TopWindow::Key(dword key, i32 count)
{
...
void GatherWindowTree(Ctrl *w, const Vec<Ctrl *>& ws, Vec<Ctrl *>& es)
{
	if(!w->InLoop())
		es.Add(w);
	for(i32 i = 0; i < ws.GetCount(); i++)
		if(ws[i]->GetOwner() == w)
			GatherWindowTree(ws[i], ws, es);
}

i32  TopWindow::Run(bool appmodal)
{
	GuiLock __;
	LLOG("TopWindow::Run() <- " << typeid(*this).name());
	LLOG("Focus = " << DRX::Name(GetFocusCtrl()));
	if(!IsOpen())
		Open();
	if(!IsVisible()) Show();
	bool pinloop = inloop;
	i32  pexitcode = exitcode;
	exitcode = Null;
	Vec<Ctrl *> es;
	if(appmodal)
		es = GetTopCtrls();
	else {
		Vec<Ctrl *> ws = GetTopCtrls();
		for(i32 i = 0; i < ws.GetCount(); i++)
			if(ws[i]->InLoop())
				es.Add(ws[i]);
	...
	Vec< Ptr<Ctrl> > disabled = DisableCtrls(es, this);
#ifdef _DEBUG
	for(i32 d = 0; d < disabled.GetCount(); d++)
		LLOG("DisableCtrls[" << d << "] = " << DRX::Name(disabled[d]));
	LLOG("Running EventLoop in " << DRX::Name(this));
#endif
	EventLoop(this);
#ifdef _DEBUG
	LLOG("Finished EventLoop in " << DRX::Name(this));
	for(i32 e = 0; e < disabled.GetCount(); e++)
		LLOG("EnableCtrls[" << e << "] = " << DRX::Name(disabled[e]));
#endif
...
#ifdef GUI_WIN
	LLOG("Focus = " << DRX::Name(GetFocusCtrl()) << ", raw " << (uk )::GetFocus());
#endif
	return q;
}

i32  TopWindow::Execute()
{
	i32 m = Run();
	Close();
	return m;
...
TopWindow& TopWindow::Title(const WTxt& _title)
{
	if(title != _title) {
		title = _title;
		SyncTitle();
...
TopWindow& TopWindow::Title(tukk s)
{
	return Title(WTxt(s));
}
