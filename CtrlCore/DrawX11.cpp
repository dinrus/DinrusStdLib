...
dword   (*Xgetpixel)(i32 r, i32 g, i32 b);
...
void   XError(tukk s)
{
	Panic(Txt("Ошибка X11:") + s + " !");
}

static i32 sAcs;

bool sAllocColor(i32 xr, i32 xg, i32 xb)
{
	XColor ce;
	ce.red = xr;
	ce.green = xg;
...
dword GetPseudoColorPixel(i32 r, i32 g, i32 b)
{
...
struct Xshift {
	...
	i32    bits;
	i32    shift;
	dword  Do(i32 c) { return (c >> bits << shift) & mask; }
}
Xred, Xgreen, Xblue;
...
dword GetTrueColorPixel(i32 r, i32 g, i32 b)
{
	return Xred.Do(r) | Xgreen.Do(g) | Xblue.Do(b);
}

inline i32 ssq(i32 x) { return x * x; }
...
//	Xcolormap = (Colormap)GDK().gdk_x11_colormap_get_xcolormap(GDK().gdk_rgb_get_colormap());
	...
	if(v->c_class == TrueColor) {
		...
		u8 *cm = Xmapcolor = new u8[12 * 24 * 12];
		for(i32 r = 0; r < 12; r++)
			for(i32 g = 0; g < 24; g++)
				for(i32 b = 0; b < 12; b++) {
					i32 mind = INT_MAX;
...
void InitX11Draw(tukk dispname)
{
	if(!dispname || !*dispname) {
		i32 f = Environment().Find("DISPLAY");
...
void SystemDraw::SetForeground(Color color)
{
	...
	i32 p = GetXPixel(color.GetR(), color.GetG(), color.GetB());
...
void SystemDraw::SetClip() {
	...
	if(!clip.IsEmpty()) {
		const Vec<Rect>& topclip = clip.Top();
		drawingclip = Rect(0, 0, 0, 0);
		if(!topclip.IsEmpty()) {
			drawingclip = topclip[0];
			for(i32 i = 1; i < topclip.GetCount(); i++)
	...
	if(dw == Xroot) return;
	LTIMING("SetClip");
	DRX::SetClip(gc, xftdraw, clip.Top());
}

void SystemDraw::SetLineStyle(i32 width)
{
	GuiLock __;
...
void SystemDraw::Init(const Vec<Rect>& _clip, Point _offset)
{
	GuiLock __;
	Init();
	clip.Add() <<= _clip;
...
Rect SystemDraw::GetClip() const
{
	...
	const Vec<Rect>& cl = clip[cloff.Top().clipi];
	Rect box(0, 0, 0, 0);
	if(!cl.GetCount()) return box;
	box = cl[0];
	LLOG("cl[0] = " << box);
	for(i32 i = 1; i < cl.GetCount(); i++) {
		LLOG("cl[" << i << "] = " << cl[i]);
		box |= cl[i];
	}
...
SystemDraw::SystemDraw(Drawable _dw, GC _gc, XftDraw *_xftdraw, const Vec<Rect>& _clip)
{
	LLOG("SystemDraw");
	dw = _dw;
	gc = _gc;
