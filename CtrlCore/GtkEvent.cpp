...
Ctrl *Ctrl::GetTopCtrlFromId(i32 id)
{
	i32 q = FindGtkId(id);
	if(q >= 0) {
		Ctrl *p = wins[q].ctrl;
...
gboolean Ctrl::GtkDraw(GtkWidget *widget, cairo_t *cr, gpointer user_data)
{
	...
	if(p) {
		...
		Rect r = RectC((i32)x1, (i32)y1, (i32)ceil(x2 - x1), (i32)ceil(y2 - y1));
		...
		if(list->status == CAIRO_STATUS_SUCCESS && list->num_rectangles < 10) {
			Vec<Rect> clip;
			for(i32 i = 0; i < list->num_rectangles; i++) {
				const cairo_rectangle_t& r = list->rectangles[i];
				clip.Add(Rect((i32)r.x, (i32)r.y, (i32)(r.x + r.width), (i32)(r.y + r.height)));
			}
...
gboolean Ctrl::GtkEvent(GtkWidget *widget, GdkEvent *event, gpointer user_data)
{
...
#ifdef LOG_EVENTS
	Txt ev = "?";
	Tuple2<i32, tukk > *f = FindTuple(xEvent, __countof(xEvent), event->type);
	if(f)
		ev = f->b;
	...
	case GDK_KEY_RELEASE:
		...
		value << (i32) key->keyval << (i32) key->hardware_keycode;
		if(pressed) {
...
i32 Ctrl::DoButtonEvent(GdkEvent *event, bool press)
{
	GdkEventButton *e = (GdkEventButton *)event;
	static i32 mask[] = { GDK_BUTTON1_MASK, GDK_BUTTON2_MASK, GDK_BUTTON3_MASK };
	if(e->button >= 1 && e->button <= 3) {
		i32 m = mask[e->button - 1];
		if(press)
			MouseState |= m;
		else
			MouseState &= ~m;
	...
	return findarg(e->button, 8, 9) >= 0 ? (i32)e->button : (i32)Null;
}
...
Point Ctrl::GetMouseInfo(GdkWindow *win, GdkModifierType& mod)
{
	...
	return s_mousepos; //return Point((i32)SCL(x), (i32)SCL(y));
#else
	gint x, y;
...
void Ctrl::AddEvent(gpointer user_data, i32 type, const Val& value, GdkEvent *event)
{
	if(Events.GetCount() > 50000)
		return;
	GEvent& e = Events.AddTail();
	e.windowid = (u32)(uintptr_t)user_data;
	e.type = type;
	e.value = value;
	GdkModifierType mod;
...
{
	GuiLock __;
	Ctrl *w = GetTopCtrlFromId((u32)(uintptr_t)user_data);
	if(w && w->HasFocusDeep() && focusCtrl && !IsNull(focusCtrl->GetPreedit())) {
		PangoAttrList *attrs;
		gchar *str;
		gint   cursor_pos;
		gtk_im_context_get_preedit_string(context, &str, &attrs, &cursor_pos);
		WTxt text = ToUtf32(str);
		g_free(str);
...
bool Ctrl::DispatchMouseIn(i32 act, i32 zd)
{
	Point p = GetMousePos();
...
Image Ctrl::GtkMouseEvent(i32 action, i32 act, i32 zd)
{
	if(grabpopup && activePopup.GetCount()) {
		for(i32 i = visiblePopup.GetCount(); --i >= 0;)
...
void Ctrl::GtkButtonEvent(i32 action)
{
	i32 act = action;
	i32 button = CurrentEvent.value;
	if(action != MOUSEMOVE)
...
#ifdef LOG_EVENTS
	Txt ev = "?";
	Tuple2<i32, tukk > *f = FindTuple(xEvent, __countof(xEvent), CurrentEvent.type);
	if(f)
		ev = f->b;
	LOG("> PROCESS EVENT " << drx::Name(this) << " " << ev);
	ProcStop tm;
	tm.ev = ev;
#endif
	if(!IsOpen())
	...
	case GDK_KEY_RELEASE:
		...
		if(kv >= 0 && kv < K_CHAR_LIM) {
			...
			if(kv > 256) { // Non-latin keyboard layout should still produce accelerators like Ctrl+C etc...
				static VecMap<i32, i32> hwkv; // convert hw keycode to Latin GTK keyval
				ONCELOCK {
					for(i32 i = 1; i < 256; i++) { // Latin keyvals are in 0..255 range
						GdkKeymapKey *keys;
						gint n_keys;
						if(gdk_keymap_get_entries_for_keyval(gdk_keymap_get_for_display(gdk_display_get_default()), i, &keys, &n_keys)) {
							for(i32 j = 0; j < n_keys; j++)
								if(keys[j].group == 0)
			...
			Tuple2<i32, i32> *x = FindTuple(cv, __countof(cv), kv);
			if(x)
				kv = x->b;
			else {
				if(GetCtrl() || GetAlt()) {// fix Ctrl+Shift+1 etc...
					static VecMap<i32, i32> hwkv;
					ONCELOCK {
						for(i32 k : { GDKEY(0), GDKEY(1), GDKEY(2), GDKEY(3), GDKEY(4),
		...
		break;
	case EVENT_TEXT: {
		WTxt h = CurrentEvent.value;
		for(i32 i = 0; i < h.GetCount(); i++) // TODO: Add compression
			DispatchKey(h[i], 1);
		break;
	}
		...
		while(Events.GetCount() > 1) { // GEvent compression (coalesce autorepeat, mouse moves/wheel, configure)
			...
			if(b.type == a.type && a.windowid == b.windowid && a.state == b.state) {
			    ...
			    if(a.type == GDK_SCROLL)
			        b.value = (i32)b.value + (i32)a.value;
				else
