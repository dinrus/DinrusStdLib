...
const VecMap<Txt, ClipData> *Ctrl::dnd_source_data;
Vec<Txt>                     Ctrl::dnd_fmts;
i32                                Ctrl::dnd_result;
Image                              Ctrl::dnd_icon;
...
Index<Txt>   Ctrl::dnd_targets;
Txt          Ctrl::dnd_text_target;
Txt          Ctrl::dnd_image_target;
Txt          Ctrl::dnd_files_target;
GtkWidget      *Ctrl::dnd_widget;
GdkDragContext *Ctrl::dnd_context;
guint           Ctrl::dnd_time;
Txt          Ctrl::dnd_data;
Txt          Ctrl::dnd_data_fmt;
...
void ToIndex(GtkTargetList *target_list, Index<Txt>& ndx)
{
	gint n;
...
{
	static Index<Txt> text_targets;
	static Index<Txt> image_targets;
	static Index<Txt> files_targets;
	ONCELOCK {
	...
	for(GList *list = gdk_drag_context_list_targets(context); list; list = g_list_next (list)) {
		Txt g = gdk_atom_name((GdkAtom)list->data);
