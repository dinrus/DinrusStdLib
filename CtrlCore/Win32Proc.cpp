...
LRESULT Ctrl::WindowProc(UINT message, WPARAM wParam, LPARAM lParam) {
	GuiLock __;
	eventid++;
//	LLOG("Ctrl::WindowProc(" << message << ") in " << ::Name(this) << ", focus " << (uk )::GetFocus());
	Ptr<Ctrl> _this = this;
	HWND hwnd = GetHWND();
...
//		return wantfocus ? 0 : DLGC_STATIC;
	...
	case WM_IME_COMPOSITION:
		...
		/*  // todo: SetCaretPos too
			...
			::SetCaretPos(p.x, p.y);
		*/
			auto ReadTxt = [&](i32 type) -> WTxt {
				i32 len = ImmGetCompositionTxtW (himc, type, NULL, 0);
				if(len > 0) {
					Buffer<char16> sw(len / 2);
					ImmGetCompositionTxtW(himc, type, sw, len);
					return ToUtf32(sw, len / 2);
				}
				return Null;
			};
			if(lParam & GCS_COMPSTR) {
				ShowPreedit(ReadTxt(GCS_COMPSTR), ImmGetCompositionTxt(himc, GCS_CURSORPOS, 0, 0));
			}
			if(lParam & GCS_RESULTSTR) {
				WTxt h = ReadTxt(GCS_RESULTSTR);
				for(wchar c : h)
					DispatchKey(c, 1);
				HidePreedit();
				SyncCaret();
	...
	case WM_NCDESTROY:
		...
		if(HasChildDeep(focusCtrlWnd) || this == ~focusCtrlWnd) {
			LLOG("WM_NCDESTROY: clearing focusCtrlWnd = " << DRX::Name(focusCtrlWnd));
			focusCtrlWnd = NULL;
			focusCtrl = NULL;
		}
		if(::GetFocus() == NULL) {
	...
	case WM_MOUSEACTIVATE:
		LLOG("WM_MOUSEACTIVATE " << Name() << ", focusCtrlWnd = " << DRX::Name(focusCtrlWnd) << ", raw = " << (uk )::GetFocus());
		if(!IsEnabled()) {
			if(lastActiveWnd && lastActiveWnd->IsEnabled()) {
				if(focusCtrl) { // this closes popup
					LLOG("WM_MOUSEACTIVATE -> ClickActivate for " << DRX::Name(lastActiveWnd));
					StopPreedit();
					lastActiveWnd->ClickActivateWnd();
				}
				else { // this makes child dialog active when clicked on disabled parent
					LLOG("WM_MOUSEACTIVATE -> ::SetFocus for " << DRX::Name(lastActiveWnd));
	...
	case WM_HELP:
		return TRUE;
	case WM_ACTIVATE:
		LLOG("WM_ACTIVATE " << Name() << ", wParam = " << (i32)wParam << ", focusCtrlWnd = " << DRX::Name(focusCtrlWnd) << ", raw = " << (uk )::GetFocus());
		ignorekeyup = true;
	case 0x031A: // WM_THEMECHANGED
		XpClear();
		break;
	case WM_SETFOCUS:
		LLOG("WM_SETFOCUS " << Name() << ", focusCtrlWnd = " << DRX::Name(focusCtrlWnd) << ", raw = " << (uk )::GetFocus());
		if(this != focusCtrlWnd || focusCtrl && focusCtrlWnd != focusCtrl->GetTopCtrl()) { // second condition fixes popup issue when clicking dialog parent
			if(IsEnabled()) {
				LLOG("WM_SETFOCUS -> ActivateWnd: this != focusCtrlWnd, this = "
					<< Name() << ", focusCtrlWnd = " << DRX::Name(focusCtrlWnd));
				ActivateWnd();
			}
			else {
...
//					if(!IsEnabled())
//						MessageBeep(MB_OK);
					LLOG("WM_SETFOCUS -> ::SetFocus for " << DRX::Name(focusCtrlWnd) << ", this: " << DRX::Name(this));
					::SetFocus(focusCtrlWnd->GetHWND());
				}
				else
				if(lastActiveWnd && lastActiveWnd->IsEnabled()) {
					LLOG("WM_SETFOCUS -> ::SetFocus for " << DRX::Name(lastActiveWnd));
					::SetFocus(lastActiveWnd->GetHWND());
				}
				else {
		...
		LLOG("//WM_SETFOCUS " << (uk )hwnd << ", focusCtrlWnd = " << DRX::Name(focusCtrlWnd) << ", raw = " << (uk )::GetFocus());
		return 0L;
	case WM_KILLFOCUS:
		LLOG("WM_KILLFOCUS " << (uk )(HWND)wParam << ", this = " << DRX::Name(this) << ", focusCtrlWnd = " << DRX::Name(focusCtrlWnd) << ", raw = " << (uk )::GetFocus());
		LLOG("Kill " << DRX::Name(CtrlFromHWND((HWND)wParam)));
		if(!CtrlFromHWND((HWND)wParam)) {
			LLOG("WM_KILLFOCUS -> KillFocusWnd: " << DRX::Name(this));
			KillFocusWnd();
		}
		LLOG("//WM_KILLFOCUS " << (uk )(HWND)wParam << ", focusCtrlWnd = " << DRX::Name(focusCtrlWnd) << ", raw = " << (uk )::GetFocus());
		StopPreedit();
		return 0L;
	case WM_ENABLE:
		if(!!wParam != enabled) {
