...
static i32 TwipDotsLim(i32 twips) { return minmax<i32>(TwipDots(twips), 0, MAX_DOTS); }

static Txt FromCTxt(tukk p, tukk *endptr = NULL)
{
    if(endptr) {
        tukk e = p;
        if(*e == '\"')
...
private:
    ...
    void          Flush(bool force, i32 itap);
    void          OpenTable(i32 level);
    void          FlushTable(i32 level);
    TOKEN         Fetch();
    void          Skip();
    ...
    bool          PassEndGroup(i32 level);
    bool          PassCmd(tukk cmd) { return PassIf(Token() == T_COMMAND && command == cmd); }
    bool          PassQ(tukk cmd)   { return PassIf(command == cmd); }

    void          SkipGroup()              { SkipGroup(stack.GetCount()); }
    void          SkipGroup(i32 level);
    i32           Level() const            { return stack.GetCount(); }

    void          ReadItem();
    void          ReadItemGroup(i32 level);
    void          ReadItemGroup()          { ReadItemGroup(Level()); }
    void          ReadText();
    void          ReadText(const WTxt& text);
    void          ReadChar(word ch)        { ReadText(WTxt(ch, 1)); }
    void          ReadCommand();
    ...
    void          ReadCharStyle();
    void          ReadCellBorder(i32& width);

    Txt        ReadBinHex(char& odd) const;
...
private:
    ...
    struct TableState {
        ...
        Vec< Array<Cell> > cells;
        i32 textcol;
        i32 stylecol;
    };

    CellInfo&     CellInfoAt(i32 i);
    Cell&         CellAt(TableState& ts, i32 i);
    void          SetCellMargin(Cell& cell, i32 Rect::*mbr);
...
RichText ParseRTF(tukk rtf) { return RTFParser(rtf).Run(); }
...
void RTFParser::FlushTable(i32 level)
{
    while(table_stack.GetCount() > level) {
        ...
        Index<i32> dot_index;
//      i32 pos = child.tableformat.lm;
        dot_index.Add(child.tableformat.lm);
        for(i32 r = 0; r < child.cells.GetCount(); r++) {
            Array<Cell>& rw = child.cells[r];
            for(i32 c = 0; c < rw.GetCount(); c++)
                dot_index.FindAdd(rw[c].info.end_dots);
        }
        Vec<i32> dot_order = dot_index.PickKeys();
        Sort(dot_order);
        RichTable table;
...
//      child.tableformat.after = state.format.after;
        ...
        for(i32 c = 1; c < dot_order.GetCount(); c++)
            table.AddColumn(dot_order[c] - dot_order[c - 1]);
        dot_index = pick(dot_order);
        i32 tbl_border = Null, tbl_grid = Null;
        ...
        for(i32 r = 0; r < child.cells.GetCount(); r++) {
            ...
            for(i32 c = 0; c < rw.GetCount(); c++) {
                ...
                cell.span.cy = 0;
                if(cell.merge_first) {
                    for(i32 m = r + 1; m < child.cells.GetCount(); m++) {
                ...
                for(i32 b = 0; b < __countof(border_width); b++) {
                    i32& out_wd = (outer_border[b] ? tbl_border : tbl_grid);
                ...
                if(cell.info.shading > 0) {
                    ...
                    i32 r = zero.GetR() + iscale(one.GetR() - zero.GetR(), cell.info.shading, 10000);
                    i32 g = zero.GetG() + iscale(one.GetG() - zero.GetG(), cell.info.shading, 10000);
                    i32 b = zero.GetB() + iscale(one.GetB() - zero.GetB(), cell.info.shading, 10000);
...
//          i32 pos = child.tableformat.lm;
            for(i32 c = 0; c < rw.GetCount(); c++) {
                ...
                for(i32 b = 0; b < __countof(border_width); b++) {
                    i32 tbl_wd = (outer_border[b] ? tbl_border : tbl_grid);
...
void RTFParser::Flush(bool force, i32 itap)
{
    if(!para.part.IsEmpty() || force) {
        i32 fi = state.first_indent, li = state.left_margin, ri = state.right_margin;
        if(state.format.bullet != RichPara::BULLET_NONE) {
            Swap(li, fi);
//          li += fi;
//          fi = -fi;
        }
        state.format.indent = minmax<i32>(fi, 0, MAX_DOTS);
        state.format.lm = minmax<i32>(li, 0, MAX_DOTS);
        state.format.rm = minmax<i32>(ri, 0, MAX_DOTS);
            ...
            switch(rtf++, *rtf++)
            {
                ...
                default: {
                    if(IsAlpha(*--rtf) || *rtf == '*' && rtf[1] == '\\' && IsAlpha(rtf[2])) {
                        ...
                        command = Txt(b, rtf);
                        if(IsDigit(*rtf) || *rtf == '-')
                            command_arg = strtol(rtf, (tuk *)&rtf, 10);
                        if(*rtf == ' ')
                            rtf++;
                        if(command == "uc")
...
bool RTFParser::PassEndGroup(i32 level)
{
    if(Token() == T_EOF)
        return true;
...
void RTFParser::SkipGroup(i32 level)
{
...
void RTFParser::ReadItemGroup(i32 level)
{
    while(!PassEndGroup(level))
        ReadItem();
}
...
void RTFParser::ReadText(const WTxt& text)
{
...
void RTFParser::ReadCharSet()
{
    if(PassQ("ansi")) {}
        ...
        for(i32 c = 0; c < __countof(charsets); c++)
...
/*          else if(PassText()) {
                Txt s = FromUnicode(text, charset);
                if(!s.IsEmpty() && *s.Last() == ';')
...
bool RTFParser::ReadField(tukk p)
{
    Index<Txt> symdef;
    while(*p)
        if((u8)*p <= ' ')
    ...
    if(symdef[0] == "SYMBOL" && symdef.GetCount() >= 2 && IsDigit(*symdef[1])) {
        ...
        if(code >= 0 && code < 255) {
            ...
            ReadText(WTxt(ToUnicode(code, state.charset), 1));
            return true;
        }
    }
    return false;
...
void RTFParser::ReadParaStyle()
{
    if(PassQ("par"))
        ...
        i32 pos = TwipDotSize(command_arg);
...
void RTFParser::ReadPict()
{
    ...
    while(!PassEndGroup()) {
        if(PassText())
            ...
            if(PassQ("picw"))            log_size.cx = minmax<i32>(command_arg, 0, 30000);
            else if(PassQ("pich"))       log_size.cy = minmax<i32>(command_arg, 0, 30000);
            else if(PassQ("picwgoal"))   out_size.cx = TwipDotSize(command_arg);
            else if(PassQ("pichgoal"))   out_size.cy = TwipDotSize(command_arg);
            else if(PassQ("picscalex"))  scaling.cx = minmax<i32>(command_arg, 1, 1000);
            else if(PassQ("picscaley"))  scaling.cy = minmax<i32>(command_arg, 1, 1000);
    ...
    DrawingDraw dd;
    RichObj object;
    LLOG("Pict format " << (i32)blip_type << ", data size: " << blip_data.GetCount());
    ...
    if(blip_type == DIB_BLIP || blip_type == PNG_BLIP || blip_type == JPEG_BLIP) {
        Image image = StreamRaster::LoadTxtAny(blip_data);
...
Txt RTFParser::ReadBinHex(char& odd) const
{
    i32 t = odd;
    u8 v = ctoi(odd);
    Txt out;
    for(wtukk s = text.Begin(); *s; s++) {
        u8 w = (u8)(*s >= '0' && *s <= '9' ? *s - '0'
            : *s >= 'A' && *s <= 'F' ? *s - 'A' + 10
            : *s >= 'a' && *s <= 'f' ? *s - 'a' + 10
            : 255);
...
void RTFParser::OpenTable(i32 level)
{
...
void RTFParser::ReadCellBorder(i32& width)
{
...
RTFParser::Cell& RTFParser::CellAt(TableState& ts, i32 i)
{
...
RTFParser::CellInfo& RTFParser::CellInfoAt(i32 i)
{
    return state.cellinfo.At(i, std_cell_info);
}

void RTFParser::SetCellMargin(Cell& out, i32 Rect::*mbr)
{
...
void RTFParser::ReadTableStyle()
{
    ...
    i32 itap = (state.nestprop ? state.itap : 1);
    if(PassQ("trowd")) {
        OpenTable(itap);
