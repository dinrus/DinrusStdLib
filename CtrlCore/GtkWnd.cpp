...
Vec<Ctrl::Win> Ctrl::wins;

Ptr<Ctrl>         Ctrl::activeCtrl;

bool              Ctrl::invalids;

i32 Ctrl::FindGtkId(i32 id)
{
...
i32 Ctrl::FindGtkWindow(GtkWidget *gtk)
{
	for(i32 i = 0; i < wins.GetCount(); i++)
		if(wins[i].gtk == gtk)
			return i;
...
i32 Ctrl::FindGdkWindow(GdkWindow *gdk)
{
	for(i32 i = 0; i < wins.GetCount(); i++)
		if(wins[i].gdk == gdk)
			return i;
...
void Ctrl::GetWorkArea(Array<Rect>& rc)
{
	...
	i32 n = gdk_display_get_n_monitors(s);
	rc.Clear();
	Vec<i32> netwa;
	for(i32 i = 0; i < n; i++) {
		GdkRectangle rr;
	...
	i32 n = gdk_screen_get_n_monitors(s);
	rc.Clear();
	Vec<i32> netwa;
	for(i32 i = 0; i < n; i++) {
		GdkRectangle rr;
...
Rect Ctrl::GetPrimaryWorkArea()
{
	...
	if (r.right == 0) {
		Array<Rect> rc;
		GetWorkArea(rc);
		i32 primary = gdk_screen_get_primary_monitor(gdk_screen_get_default());
...
{
	GuiLock __;
	LLOG("SetWndFocus0 " << drx::Name(this) << ", top: " << top);
	if(top) {
...
bool Ctrl::SweepConfigure(bool wait)
{
	...
	bool r = false;
	FetchEvents(wait);
	for(i32 i = 0; i < Events.GetCount() && this_; i++) {
		GEvent& e = Events[i];
		Top *top = GetTop();
