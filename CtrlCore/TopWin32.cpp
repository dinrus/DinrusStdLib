...
{
	GuiLock __;
	LLOG("TopWindow::SyncTitle0 " << DRX::Name(this));
	HWND hwnd = GetHWND();
	if(hwnd)
...
{
	GuiLock __;
	LLOG("TopWindow::DeleteIco " << DRX::Name(this));
	if(ico)
		DestroyIcon(ico);
	if(lico)
		DestroyIcon(lico);
...
{
	GuiLock __;
	LLOG("TopWindow::SyncCaption " << DRX::Name(this));
	if(fullscreen)
		return;
	HWND hwnd = GetHWND();
	if(hwnd) {
...
void TopWindow::CenterRect(HWND hwnd, i32 center)
{
	GuiLock __;
...
void TopWindow::Open(HWND hwnd)
{
	...
	IgnoreMouseUp();
	SyncCaption();
	LLOG("WindowStyles: " << WindowStyleAsTxt(style, exstyle));
#ifdef TARGET_WINCE
	if(!GetRect().IsEmpty())
#endif
	if(fullscreen) {
...
{
	GuiLock __;
	LLOG("TopWindow::Open(Ctrl) -> " << DRX::Name(owner));
...
{
	LLOG("TopWindow::Minimize " << DRX::Name(this));
	state = MINIMIZED;
	if(IsOpen())
#ifdef TARGET_WINCE
...
{
	LLOG("TopWindow::FullScreen " << DRX::Name(this));
	fullscreen = b;
...
{
	LLOG("TopWindow::Maximize " << DRX::Name(this));
	state = MAXIMIZED;
	if(IsOpen()) {
...
{
	GuiLock __;
	LLOG("TopWindow::Overlap " << DRX::Name(this));
	state = OVERLAPPED;
	if(IsOpen()) {
...
{
	GuiLock __;
	LLOG("TopWindow::Style " << DRX::Name(this));
	style = _style;
	if(GetHWND())
...
{
	GuiLock __;
	LLOG("TopWindow::ExStyle " << DRX::Name(this));
	exstyle = _exstyle;
	if(GetHWND())
...
{
	GuiLock __;
	LLOG("TopWindow::TopMost " << DRX::Name(this));
	HWND hwnd = GetHWND();
	if(hwnd)
