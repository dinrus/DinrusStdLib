...
void TopWindow::EventProc(XWindow& w, XEvent *event)
{
	...
	if(event->type == PropertyNotify && event->xproperty.atom == XAtom("_NET_WM_STATE")) {
		...
		Vec<i32> p = GetPropertyInts(GetWindow(), XAtom("_NET_WM_STATE"));
		if(FindIndex(p, (i32)XAtom("_NET_WM_STATE_HIDDEN")) >= 0) {
			state = MINIMIZED;
			LLOG("MINIMIZED");
		}
		else
		if(FindIndex(p, (i32)XAtom("_NET_WM_STATE_MAXIMIZED_HORZ")) >= 0 &&
		   FindIndex(p, (i32)XAtom("_NET_WM_STATE_MAXIMIZED_VERT")) >= 0) {
			state = MAXIMIZED;
			LLOG("MAXIMIZED");
		}
		else {
...
void TopWindow::DefSyncTitle()
{
	...
	if(IsOpen() && GetWindow()) {
		Window w = GetWindow();
		XStoreName(Xdisplay, w, title.ToTxt());
		XSetIconName(Xdisplay, w, title.ToTxt());
		Txt utf8title = FromUnicode(title, CHARSET_UTF8);
		XChangeProperty(Xdisplay, w, XAtom("_NET_WM_NAME"), XAtom("UTF8_TXT"),
		                8, PropModeReplace,
		                (u8k*)~utf8title, utf8title.GetLength());
		XChangeProperty(Xdisplay, w, XAtom("_NET_WM_ICON_NAME"), XAtom("UTF8_TXT"),
		                8, PropModeReplace,
		                (u8k*)~utf8title, utf8title.GetLength());
	}
}
...
void TopWindow::SyncTitle()
{
	...
	SetTimeCallback(0, OTVET(DefSyncTitle), TIMEID_DEFSYNCTITLE);
	LLOG("*SyncTitle: " << title);
}
...
//		                PropModeReplace, (u8k*)wina, n);
		...
		if(!icon.IsEmpty()) {
			i32 len = 0;
			Buffer<u64> data = PreperIcon(icon, len);
		...
		XChangeProperty(Xdisplay, w, XAtom("_MOTIF_WM_HINTS"), XAtom("_MOTIF_WM_HINTS"), 32,
		                PropModeReplace,
		                (u8*) &mwmhints, PROP_MWM_HINTS_ELEMENTS);
	}
}

Buffer<u64> TopWindow::PreperIcon(const Image& icon, i32& len) {
...
void TopWindow::Open(Ctrl *owner)
{
	...
	LLOG("OPEN " << Name() << " owner: " << DRX::Name(owner));
	IgnoreMouseUp();
	...
	Txt x = GetExeTitle().ToTxt();
	tukk progname = ~x;
	class_hint->res_name = (tuk )progname;
	class_hint->res_class = (tuk )progname;
	...
	if(xwin && visible) {
		...
		SetTimeCallback(500, OTVET(EndIgnoreTakeFocus));
		LLOG("SetWndFocus");
		SetWndFocus();
		for(i32 i = 0; i < 50; i++) {
	...
	PlaceFocus();
	StateH(OPEN);
	Vec<i32> fe = GetPropertyInts(xwin, XAtom("_NET_FRAME_EXTENTS"));
	if(fe.GetCount() >= 4 &&
	...
	ONCELOCK {
		...
		XChangeProperty(Xdisplay, wm_client_leader, XAtom("WM_CLIENT_LEADER"),
		                XA_WINDOW, 32, PropModeReplace, (u8 *)&wm_client_leader, 1);
		XChangeProperty(Xdisplay, wm_client_leader, XAtom("_NET_WM_PID"), XA_CARDINAL, 32,
		                PropModeReplace, (u8 *) &curr_pid, 1);
	}
