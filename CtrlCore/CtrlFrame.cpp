...
Ctrl::Frame Ctrl::AllocFrames(i32 alloc)
{
	Frame m;
...
//	memset(m.frames, 0, sz); _DBG_
	m.multi.alloc = alloc + (i32)((sz - sz0) / sizeof(Frame));
	return m;
}

void Ctrl::InsertFrame(i32 i, CtrlFrame& fr)
{
	GuiLock __;
	i32 fc = GetFrameCount();
	ASSERT(i <= fc);
	if(i == 0 && fc == 0) {
		frame.frame = &fr;
		multi_frame = false;
...
Ctrl& Ctrl::SetFrame(i32 i, CtrlFrame& fr) {
	GuiLock __;
...
i32  Ctrl::FindFrame(CtrlFrame& frm) const
{
	GuiLock __;
	i32 n = GetFrameCount();
	for(i32 i = 0; i < n; i++)
