...
Vec< Ptr<Ctrl> > Ctrl::defferedChildLostFocus;
...
bool Ctrl::DispatchKey(dword keycode, i32 count)
{
	GuiLock __;
	EventLevelDo ___;
...
//	RLOG("DispatchKey: focusCtrl = " << FormatIntHex((i32)~focusCtrl) << ", wnd = " << FormatIntHex((i32)~focusCtrlWnd) << ")");
	LLOG("DispatchKey " << keycode << " (0x" << Sprintf("%08x", keycode)
		<< ", " << GetKeyDesc(keycode) << "), count:" << count
		<< " focusCtrl:" << DRX::Name(focusCtrl) << " focusCtrlWnd:" << DRX::Name(focusCtrlWnd));
	if((keycode & K_KEYUP) && ignorekeyup)
	{
		ignorekeyup = false;
		return true;
	}
	for(i32 i = 0; i < keyhook().GetCount(); i++)
...
void Ctrl::DoDeactivate(Ptr<Ctrl> pfocusCtrl, Ptr<Ctrl> nfocusCtrl)
{
	...
	if(pfocusCtrl) {
		...
		if(ntop != ptop && !ptop->destroying) {
			LLOG("DoDeactivate " << DRX::Name(ptop) << " in favor of " << DRX::Name(ntop));
...
bool Ctrl::SetFocus0(bool activate)
{
	GuiLock __;
	if(IsUsrLog())
		UsrLogT(6, Txt().Cat() << "SETFOCUS " << Desc(this));
	LLOG("Ctrl::SetFocus " << Desc(this));
	LLOG("focusCtrlWnd " << DRX::Name(focusCtrlWnd));
	LLOG("Ctrl::SetFocus0 -> deferredSetFocus = NULL; was: " << DRX::Name(defferedSetFocus));
	...
	LLOG("SetFocus -> SetWndFocus: topwindow = " << DRX::Name(topwindow) << ", focusCtrlWnd = " << DRX::Name(focusCtrlWnd));
	...
	LLOG("SetFocus -> focusCtrl = this: " << FormatIntHex(this) << ", _this = " << FormatIntHex(~_this) << ", " << DRX::Name(_this));
	focusCtrl = _this;
...
bool Ctrl::SetFocus0(bool activate)
{
	...
	Ptr<Ctrl> topctrl = GetTopCtrl();
	Ptr<Ctrl> _this = this;
#ifdef TARGET_COCOA0 // hopefully no needed
	topwindow = topctrl;
#else
	if(!topwindow) topwindow = topctrl;
#endif
	LLOG("SetFocus -> SetWndFocus: topwindow = " << DRX::Name(topwindow) << ", focusCtrlWnd = " << DRX::Name(focusCtrlWnd));
	if(!topwindow->HasWndFocus() && !topwindow->SetWndFocus()) return false;// cxl 31.1.2004
#ifdef TARGET_OSX11 // ugly temporary hack - popups not behaving right in MacOS
...
void Ctrl::ActivateWnd()
{
	...
	Ptr<Ctrl> pfocusCtrl = focusCtrl;
	LLOG("About to set focus: " << DRX::Name(nfocusCtrl));
	...
	LLOG("Focus: " << DRX::Name(focusCtrl) << " FocusWnd:" << DRX::Name(focusCtrlWnd));
}
...
void Ctrl::DefferedFocusSync()
{
	...
	while(defferedChildLostFocus.GetCount() || defferedSetFocus) {
		LLOG("Ctrl::DeferredFocusSync, defferedSetFocus = " << DRX::Name(defferedSetFocus));
		Vec< Ptr<Ctrl> > b = pick(defferedChildLostFocus);
		defferedChildLostFocus.Clear();
		for(i32 i = 0; i < b.GetCount(); i++)
			if(b[i]) {
				LLOG("Ctrl::DeferredFocusSync -> ChildLostFocus " << DRX::Name(b[i]));
				b[i]->ChildLostFocus();
			}
		if(defferedSetFocus) {
			LLOG("Ctrl::DeferredFocusSync -> SetFocus " << DRX::Name(defferedSetFocus));
...
void Ctrl::SyncCaret() {
	...
	if(focusCtrl != caretCtrl) {
		LLOG("SyncCaret DO " << drx::Name(caretCtrl) << " -> " << drx::Name(focusCtrl));
		RefreshCaret();
		caretCtrl = focusCtrl;
		WndCaretTime = msecs();
		RefreshCaret();
	...
	if(key >= K_NUMPAD0 && key <= K_NUMPAD9)
		...
		for(i32 i = 0; KeyNames__[i].a; i++)
			if(KeyNames__[i].a == key) {
				desc << GetLngTxt(KeyNames__[i].b);
				return desc;
			}
		desc << Format("%04x", (i32)key);
	}
	return desc;
}
