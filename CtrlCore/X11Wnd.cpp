...
static i32 s_starttime;

void Ctrl::DoPaint(const Vec<Rect>& invalid)
{
	GuiLock __;
...
void  Ctrl::WndScrollView(const Rect& r, i32 dx, i32 dy)
{
	...
	i32 cx = r.Width() - abs(dx);
	i32 cy = r.Height() - abs(dy);
	...
	Vec<Rect> ur;
	if(xw)
		for(i32 i = 0; i < xw->invalid.GetCount(); i++)
			if(xw->invalid[i].Intersects(r))
				ur.Add(xw->invalid[i]);
	for(i32 i = 0; i < ur.GetCount(); i++)
...
Vec<dword> Ctrl::keyhot;
Vec<dword> Ctrl::modhot;
...
void Ctrl::ProcessEvent(XEvent *event)
{
	...
	if(event->type == PropertyNotify &&
	   (event->xproperty.atom == XAtom("_QT_SETTINGS_TIMESTAMP_") ||
		...
		LLOG("Property notify " << XAtomName(event->xproperty.atom)
		     << " " << (uk )event->xany.window);
		for(i32 i = 0; i < Xwindow().GetCount(); i++) {
			if(Xwindow().GetKey(i)) {
	...
	i32 q = xmap.Find(event->xany.window);
#ifdef _DEBUG
	bool eo = false;
...
void Ctrl::TimerAndPaint() {
	...
	for(i32 i = 0; i < Xwindow().GetCount(); i++) {
		...
		if(Xwindow().GetKey(i) && xw.exposed && xw.invalid.GetCount()) {
			if(xw.ctrl) {
				LLOG("..and paint " << DRX::Name(xw.ctrl));
				xw.ctrl->SyncScroll();
				Vec<Rect> x = pick(xw.invalid);
				xw.invalid.Clear();
				xw.ctrl->DoPaint(x);
			}
			else
...
void Ctrl::GuiSleep(i32 ms)
{
	...
	i32 level = LeaveGuiMutexAll(); // Give  a chance to nonmain threads to touch GUI things
...
void Ctrl::Create(Ctrl *owner, bool redirect, bool savebits)
{
	...
	if(cw.xic)
		XGetICVals(cw.xic, XNFilterEvents, &im_event_mask, NULL);
	...
	if(redirect) {
		i32 windowType = XInternAtom(Xdisplay, "_NET_WM_WINDOW_TYPE_POPUP_MENU", false);
		XChangeProperty(Xdisplay, w, XInternAtom(Xdisplay, "_NET_WM_WINDOW_TYPE", false), XA_ATOM, 32,
		                PropModeReplace, (u8 *)&windowType, 1);
	}
...
void Ctrl::WndDestroy()
{
	...
	if(focusWindow) {
		i32 q = Xwindow().Find(focusWindow);
		if(q >= 0) {
...
void  Ctrl::SetAlpha(u8 alpha)
{
	...
	if (!IsAlphaSupported() || GetParent() || !GetTop() || !hwnd)
		return;
	u32 opacity = (u32) 16843009 * alpha;
...
{
	GuiLock __;
	static bool b = XGetSelectionOwner(Xdisplay, XAtom(Txt().Cat() << "_NET_WM_CM_S" << Xscreenno)) != None;
	return b;
}
	...
	if(this != focusCtrlWnd) {
		if(IsEnabled()) {
			...
			if(w->last_active && w->last_active->IsOpen() && w->last_active->IsEnabled() &&
			   w->last_active != this) {
				LLOG("    activating last active: " << DRX::Name(w->last_active));
...
/*
	...
	if(xw.xic && xw.ctrl) {
		...
		XSetICVals(xw.xic, XNPreeditAttributes, preedit_attr, NULL);
		XFree(preedit_attr);
	}
*/
}
...
void Ctrl::Invalidate(XWindow& xw, const Rect& _r)
{
	GuiLock __;
	LTIMING("Invalidate");
	LLOG("Invalidate " << DRX::Name(xw.ctrl) << " " << _r);
...
void Ctrl::SyncNativeWindows(void)
{
	...
	for(i32 i = 0; i < xwindows.GetCount(); i++)
	{
		...
		if(xw.ctrl && xw.ctrl->GetParent() && w)
		{
			Window dummy;
			i32 x, y;
			u32 width, height, border, depth;
			XGetGeometry(Xdisplay, w, &dummy, &x, &y, &width, &height, &border, &depth);
			Rect r = xw.ctrl->GetRectInParentWindow();
			if( (x != r.left || y != r.top) && ((i32)width == r.Width() && (i32)height == r.Height()))
				XMoveWindow(Xdisplay, w, r.left, r.top);
			else if( (x == r.left && y == r.top) && ((i32)width != r.Width() || (i32)height != r.Height()))
