...
public:
	...
	BitmapInfo32__(i32 cx, i32 cy);
};

BitmapInfo32__::BitmapInfo32__(i32 cx, i32 cy)
{
...
HBITMAP CreateBitMask(const RGBA *data, Size sz, Size tsz, Size csz, RGBA *ct)
{
	...
	i32 linelen = (tsz.cx + 15) >> 4 << 1;
	Buffer<u8>  mask(tsz.cy * linelen, 0xff);
	u8 *m = mask;
	RGBA *ty = ct;
	const RGBA *sy = data;
	for(i32 y = 0; y < csz.cy; y++) {
		const RGBA *s = sy;
		RGBA *t = ty;
		for(i32 x = 0; x < csz.cx; x++) {
			if(s->a > 128) {
				*t = *s;
				m[x >> 3] &= ~(0x80 >> (x & 7));
			}
...
void SetSurface(HDC dc, i32 x, i32 y, i32 cx, i32 cy, const RGBA *pixels)
{
...
class DrawSurface : NoCopy {
	...
	void  Init(SystemDraw& w, i32 x, i32 y, i32 cx, i32 cy);
	RGBA* Line(i32 i) const { ASSERT(i >= 0 && i < size.cy); return (RGBA *)pixels + size.cx * (size.cy - i - 1); }
...
public:
	...
	i32         GetLineDelta() const    { return -size.cx; }

	DrawSurface(SystemDraw& w, const Rect& r);
	DrawSurface(SystemDraw& w, i32 x, i32 y, i32 cx, i32 cy);
	~DrawSurface();
};

void DrawSurface::Init(SystemDraw& w, i32 _x, i32 _y, i32 cx, i32 cy)
{
	LTIMING("DrawSurface");
	GuiLock __;
	dc = w.GetHandle();
	...
	hbmp = CreateDIBSection(dc, bi, DIB_RGB_COLORS, (uk *)&pixels, NULL, 0);
...
DrawSurface::DrawSurface(SystemDraw& w, i32 x, i32 y, i32 cx, i32 cy)
{
	Init(w, x, y, cx, cy);
}
...
#ifndef TARGET_WINCE
typedef BOOL (WINAPI *tAlphaBlend)(HDC hdcDest, i32 nXOriginDest, i32 nYOriginDest,
                                   i32 nWidthDest, i32 nHeightDest,
                                   HDC hdcSrc, i32 nXOriginSrc, i32 nYOriginSrc,
                                   i32 nWidthSrc, i32 nHeightSrc, BLENDFUNCTION blendFunction);
...
struct ImageSysData {
	...
	void Paint(SystemDraw& w, i32 x, i32 y, const Rect& src, Color c);
...
void ImageSysData::CreateHBMP(HDC dc, const RGBA *data)
{
	...
	RGBA *pixels;
	HBITMAP hbmp32 = CreateDIBSection(dcMem, bi, DIB_RGB_COLORS, (uk *)&pixels, NULL, 0);
	HDC hbmpOld = (HDC) ::SelectObject(dcMem, hbmp32);
	Copy(pixels, data, (i32)img.GetLength());
...
void ImageSysData::Paint(SystemDraw& w, i32 x, i32 y, const Rect& src, Color c)
{
	HDC dc = w.GetHandle();
	Size sz = img.GetSize();
	i32  len = sz.cx * sz.cy;
	...
	if(fnAlphaBlend() && IsNull(c) && !ImageFallBack &&
	   !(w.IsPrinter() && (GetDeviceCaps(dc, SHADEBLENDCAPS) & (SB_PIXEL_ALPHA|SB_PREMULT_ALPHA)) !=
		...
		if(!himg) {
			...
			BitmapInfo32__ bi(sz.cx, sz.cy);
			himg = CreateDIBSection(ScreenHDC(), bi, DIB_RGB_COLORS, (uk *)&section, NULL, 0);
			if(!himg) return; // Return on release.
			Copy(section, ~img, (i32)img.GetLength());
		}
		...
		for(i32 i = sr.top; i < sr.bottom; i++) {
			if(IsNull(c))
...
struct ImageSysDataMaker : LRUCache<ImageSysData, z64>::Maker {
	Image img;

	virtual z64  Key() const                      { return img.GetSerialId(); }
	virtual i32    Make(ImageSysData& object) const { object.Init(img); return (i32)img.GetLength(); }
};

void SystemDraw::SysDrawImageOp(i32 x, i32 y, const Image& img, const Rect& src, Color color)
{
	...
	ImageSysDataMaker m;
	static LRUCache<ImageSysData, z64> cache;
	static i32 Rsz;
	Rsz += (i32)img.GetLength();
...
void ImageDraw::Section::Init(i32 cx, i32 cy)
{
	GuiLock __;
...
ImageDraw::ImageDraw(i32 cx, i32 cy)
{
	size = Size(cx, cy);
	Init();
}
...
Image SystemDraw::Win32IconCursor(LPCSTR id, i32 iconsize, bool cursor)
{
	HICON icon;
	if(cursor)
	...
	if(cursor)
		img.SetAuxData(reinterpret_cast<zu64>(id));
	return img;
}
...
Image Win32Icon(LPCSTR id, i32 iconsize)
{
	return SystemDraw::Win32IconCursor(id, iconsize, false);
}

Image Win32Icon(i32 id, i32 iconsize)
{
...
HICON SystemDraw::IconWin32(const Image& img, bool cursor)
{
	...
	if(!ImageFallBack) {
		...
		HDC dcMem = ::CreateCompatibleDC(NULL);
		iconinfo.hbmColor = ::CreateDIBSection(dcMem, bi, DIB_RGB_COLORS, (uk *)&pixels, NULL, 0);
