...
{
	i32 div = HorzLayoutZoom(4);
	i32 cy = dir1.GetStdSize().cy;

	i32 bcx = GetTextSize(t_("Сравнить"), StdFont()).cx * 12 / 10 + 2 * div;
	...
	i32 bcy = max(cy, compare.GetStdSize().cy);
	...
	compare <<= OTVET(Compare);
	dir1 <<= OTVET(ClearFiles);
	dir2 <<= OTVET(ClearFiles);
	...
	auto SetupRevert = [=, this](Button& revert, EditTxt *dir) {
		revert.Disable();
		revert.SetLabel("Отменить");
		revert.SetImage(CtrlImg::undo());
		revert << [=, this] {
			Txt path = AppendFName(~*dir, files.GetCurrentName());
	...
	auto SetupRemove = [=, this](Button& remove, TextCompareCtrl *text, EditTxt *dir)
	{
	...
	WhenIcon = [](tukk path) -> Image { return NativePathIcon(path); };
	
	Title("Сравнить папки");
};

void DirDiffDlg::GatherFilesDeep(Index<Txt>& files, const Txt& base, const Txt& path)
{
	FindFile ff(AppendFName(AppendFName(base, path), "*.*"));
	while(ff) {
		Txt p = (path.GetCount() ? path + '/' : Txt()) + ff.GetName();
...
bool DirDiffDlg::FileEqual(const Txt& f1, const Txt& f2, i32& n)
{
	FileIn in1(f1);
	FileIn in2(f2);
	if(in1 && in2) {
...
void DirDiffDlg::Compare()
{
	...
	files.Clear();
	Vec<Txt> f = fs.PickKeys();
	Sort(f);
	Progress pi(t_("Сравниваются.."));
	pi.SetTotal(f.GetCount());
	
	Date dlim = IsNull(recent) ? Null : GetSysDate() - (i32)~recent;

	list.Clear();
	for(i32 i = 0; i < f.GetCount(); i++) {
		if(pi.StepCanceled())
			break;
		Txt p1 = AppendFName(~dir1, f[i]);
		Txt p2 = AppendFName(~dir2, f[i]);
		i32 n = NORMAL_FILE;
...
FileList::File DirDiffDlg::MakeFile(i32 i)
{
...
void DirDiffDlg::ShowResult()
{
	...
	Title(AsTxt(files.GetCount()) + " файлов");
...
Txt SplitLines(const Txt& s)
{
    TxtStream ss(s);
    WTxt result;
    while(!ss.IsEof()) {
        WTxt l = ExpandTabs(ss.GetLine().ToWTxt());
        i32 q = 0;
...
{
	Txt fn = files.GetCurrentName();
	Txt p1 = AppendFName(~dir1, fn);
	Txt p2 = AppendFName(~dir2, fn);

	diff.right.WhenHighlight = diff.left.WhenHighlight = [=, this](Vec<LineEdit::Highlight>& hl, const WTxt& ln) {
		DiffDlg::WhenHighlight(AppendFName(p1, files.GetCurrentName()), hl, ln);
	};
...
void DirDiffDlg::Backup(const Txt& path)
{
	i32 q = backup.Find(path);
...
bool drx::DirDiffDlg::HotKey(dword key)
{
	if(key == K_CTRL_F) {
		ActiveFocus(find);
		return true;
...
bool DirDiffDlg::Key(dword key, i32 count)
{
	bool left;
