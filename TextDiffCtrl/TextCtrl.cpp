...
TextCompareCtrl::TextCompareCtrl()
{
	...
	scroll.NoAutoHide();
	scroll.WhenScroll = OTVET(SelfScroll);
	maxwidth = 0;
	tabsize = 4;
	gutter_width = 0;
...
i32 TextCompareCtrl::GetLineNo(i32 y, i32& ii)
{
	ii = scroll.Get().y + y / letter.cy;
	return ii >= 0 && ii < lines.GetCount() ? lines[ii].number : Null;
}

void TextCompareCtrl::DoSelection(i32 y, bool shift)
{
	i32 ii;
	i32 i = GetLineNo(y, ii);
	if(!IsNull(i)) {
		if(shift)
			cursor = i;
		else
...
void TextCompareCtrl::LeftDown(Point pt, dword keyflags)
{
	...
	if(pt.x > sz.cx - gutter_width || HasCapture() && gutter_capture) {
		if(!HasCapture())
			SetCapture();
		i32 line = (pt.y * lines.GetCount()) / sz.cy;
		i32 page_lines = sz.cy / letter.cy;
...
void drx::TextCompareCtrl::LostFocus()
{
	Refresh();
}
...
{
	MenuBar b;
	b.Add(!IsNull(cursor), t_("Копировать"), CtrlImg::copy(), OTVET(Copy)).Key(K_CTRL_C);
	b.Execute();
}

bool TextCompareCtrl::Key(dword key, i32 repcnt)
{
	...
	switch(key) {
		...
		case K_SHIFT_F3: {
			bool found = false;
			i32 i = min(pos.y, lines.GetCount() - 1);
...
i32 TextCompareCtrl::GetMatchLen(wtukk s1, wtukk s2, i32 len)
{
	for(i32 q = 0; q < len; q++)
		if(s1[q] != s2[q])
			return q;
...
bool TextCompareCtrl::LineDiff(bool left, Vec<LineEdit::Highlight>& hln, Color eq_color,
                               ...
                               wtukk s2, i32 l2, i32 h2, i32 depth)
{
	...
	i32 p2 = 0;
	i32 matchlen = 0;
	i32 subscore = 0; // prefer matches that are not in words
	
	for(i32 pos1 = l1; pos1 < h1; pos1++)
		for(i32 pos2 = l2; pos2 < h2; pos2++) {
			i32 ml = GetMatchLen(s1 + pos1, s2 + pos2, min(h1 - pos1, h2 - pos2));
			if(ml && ml >= matchlen) {
				i32 l = pos1;
				i32 h = pos1 + matchlen;
				i32 sc = (l == 0 || IsAlNum(s1[l - 1]) != IsAlNum(s1[l])) +
...
void TextCompareCtrl::Paint(Draw& draw)
{
	...
	i32 lcnt = lines.GetCount();
	i32 first_line = offset.cy / letter.cy;
	i32 last_line = min(idivceil(sz.cy + offset.cy, letter.cy), lines.GetCount() - 1);

	if(gutter_width > 0)
	{
		i32 t = 0, b = 0;
		i32 gx = sz.cx - gutter_width;
		for(i32 i = 0; i < lcnt; i++)
			if(lines[i].level > 1) {
		...
		i32 total = letter.cy * lcnt;
		if(total <= 0)
			total = 1;
		i32 page_height = (sz.cy * sz.cy) / total;
		i32 ty = max(0, (sz.cy * offset.cy) / total);
		i32 by = min(sz.cy, ty + page_height);
	...
	WTxt test = "č"; // read text/paper colors from highlighting scheme using likely non-highlighted text
	Vec<LineEdit::Highlight> th;
	th.SetCount(2);
	th[0].ink = SColorText();
	...
	i32 n_width = show_line_number ? number_width : 0;
	if(show_line_number) {
		for(i32 i = first_line; i <= last_line; i++) {
			const Line& l = lines[i];
			i32 y = i * letter.cy - offset.cy;
	...
	for(i32 i = first_line; i <= last_line; i++) {
		...
		WTxt ln = l.text.ToWTxt();
		if(ln.GetCount() > 20000)
			ln.Trim(20000);
		ln = ExpandTabs(ln);
		Vec<LineEdit::Highlight> hln;
		hln.SetCount(ln.GetCount() + 1);
		for(i32 i = 0; i < hln.GetCount(); i++) {
		...
		if(!sel) {
			...
			if(show_diff_highlight) {
				WTxt ln_diff = l.text_diff.ToWTxt();
				ln_diff = ExpandTabs(ln_diff);
				if((z64)ln_diff.GetCount() * ln.GetCount() < 50000) {
					if(left)
...
void TextCompareCtrl::MouseWheel(Point pt, i32 zdelta, dword keyflags)
{
	scroll.WheelY(zdelta);
}

void TextCompareCtrl::SetCount(i32 c)
{
...
void TextCompareCtrl::AddCount(i32 c)
{
	lines.InsertN(lines.GetCount(), c);
	Layout();
}

void TextCompareCtrl::Set(i32 line, Txt text, bool diff, i32 number, i32 level, Txt text_diff, i32 number_diff)
{
	Line& l = lines.At(line);
	i32 tl = MeasureLength(text.ToWTxt());
	i32 lt = MeasureLength(l.text.ToWTxt());
...
void TextCompareCtrl::UpdateWidth()
{
	maxwidth = 0;
	for(i32 i = 0; i < lines.GetCount(); i++)
		maxwidth = max(maxwidth, MeasureLength(lines[i].text.ToWTxt()));
}

i32 TextCompareCtrl::MeasureLength(wtukk text) const
{
	i32 pos = 0;
	while(*text)
		if(*text++ == '\t')
...
bool TextCompareCtrl::IsSelected(i32 i) const
{
	i32 sell, selh;
...
bool TextCompareCtrl::GetSelection(i32& l, i32& h) const
{
	if(IsNull(cursor)) {
		l = h = -1;
		return false;
...
WTxt TextCompareCtrl::ExpandTabs(wtukk text) const
{
	WTxt out;
	for(wchar c; (c = *text++);)
		if(c == '\t')
...
{
	Point pos(0, 0);
	i32 ltop = minmax(scroll.Get().y, 0, lines.GetCount() - 1);
...
{
	i32 l = FindFieldIndex(lines, &Line::number,pos.x);
	if(l < 0)
		l = 0;
	SetSb(l + pos.y);
}
