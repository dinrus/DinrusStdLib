  ...
  : public EigenBase<Derived>
{
  public:
    ...
    enum {
      ...
      HasDirectAccess_ = (i32(Flags)&DirectAccessBit) ? 1 : 0 // workaround sunCC
      #endif
    };
    ...
    /** This is the "real scalar" type; if the \a Scalar type is already real numbers
      * (e.g. i32, float or double) then \a RealScalar is just the same as \a Scalar. If
...
#undef EIGEN_DOC_BLOCK_ADDONS_INNER_PANEL_IF
    ...
    Index outerSize() const { return (i32(Flags)&RowMajorBit) ? this->rows() : this->cols(); }
    /** \returns the size of the inner dimension according to the storage order,
      * i.e., the number of rows for a columns major matrix, and the number of cols otherwise */
    Index innerSize() const { return (i32(Flags)&RowMajorBit) ? this->cols() : this->rows(); }
  ...
  public:
    ...
    template<u32 UpLo> struct SelfAdjointViewReturnType { typedef SparseSelfAdjointView<Derived, UpLo> Type; };
    template<u32 UpLo> struct ConstSelfAdjointViewReturnType { typedef const SparseSelfAdjointView<const Derived, UpLo> Type; };

    template<u32 UpLo> inline 
