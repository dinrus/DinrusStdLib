...
template<i32 SrcMode,i32 DstMode,typename MatrixType,i32 DestOrder>
void permute_symm_to_symm(const MatrixType& mat, SparseMatrix<typename MatrixType::Scalar,DestOrder,typename MatrixType::StorageIndex>& _dest, const typename MatrixType::StorageIndex* perm = 0);

template<i32 Mode,typename MatrixType,i32 DestOrder>
void permute_symm_to_fullsymm(const MatrixType& mat, SparseMatrix<typename MatrixType::Scalar,DestOrder,typename MatrixType::StorageIndex>& _dest, const typename MatrixType::StorageIndex* perm = 0);

}

template<typename MatrixType, u32 Mode_> class SparseSelfAdjointView
  : public EigenBase<SparseSelfAdjointView<MatrixType,Mode_> >
{
  public:
    ...
    template<typename SrcMatrixType,i32 SrcMode>
    ...
    template<typename SrcMatrixType,u32 SrcMode>
...
struct Assignment<DstXprType, SrcXprType, Functor, SparseSelfAdjoint2Sparse>
{
  ...
  template<typename DestScalar,i32 StorageOrder>
  ...
  template<typename DestScalar,i32 StorageOrder,typename AssignFunc>
...
template<i32 Mode, typename SparseLhsType, typename DenseRhsType, typename DenseResType, typename AlphaType>
...
template<typename LhsView, typename Rhs, i32 ProductType>
...
template<typename Lhs, typename RhsView, i32 ProductType>
...
template<typename LhsView, typename Rhs, i32 ProductTag>
...
template<typename Lhs, typename RhsView, i32 ProductTag>
...
void permute_symm_to_fullsymm(const MatrixType& mat, SparseMatrix<typename MatrixType::Scalar,DestOrder,typename MatrixType::StorageIndex>& _dest, const typename MatrixType::StorageIndex* perm)
{
  ...
  enum {
    StorageOrderMatch = i32(Dest::IsRowMajor) == i32(MatrixType::IsRowMajor)
  };
...
template<i32 SrcMode_,i32 DstMode_,typename MatrixType,i32 DstOrder>
void permute_symm_to_symm(const MatrixType& mat, SparseMatrix<typename MatrixType::Scalar,DstOrder,typename MatrixType::StorageIndex>& _dest, const typename MatrixType::StorageIndex* perm)
{
  ...
  enum {
    ...
    StorageOrderMatch = i32(SrcOrder) == i32(DstOrder),
  ...
  for(StorageIndex j = 0; j<size; ++j)
  {
    ...
    for(MatIterator it(matEval,j); it; ++it)
    {
      ...
      if((i32(SrcMode)==i32(Lower) && i<j) || (i32(SrcMode)==i32(Upper) && i>j))
        continue;
                  
      StorageIndex ip = perm ? perm[i] : i;
      count[i32(DstMode)==i32(Lower) ? (std::min)(ip,jp) : (std::max)(ip,jp)]++;
    }
  }
  dest.outerIndexPtr()[0] = 0;
  for(Index j=0; j<size; ++j)
  ...
  for(StorageIndex j = 0; j<size; ++j)
  {
    ...
    for(MatIterator it(matEval,j); it; ++it)
    {
      ...
      Index k = count[i32(DstMode)==i32(Lower) ? (std::min)(ip,jp) : (std::max)(ip,jp)]++;
      dest.innerIndexPtr()[k] = i32(DstMode)==i32(Lower) ? (std::max)(ip,jp) : (std::min)(ip,jp);
      
      if(!StorageOrderMatch) std::swap(ip,jp);
      if( ((i32(DstMode)==i32(Lower) && ip<jp) || (i32(DstMode)==i32(Upper) && ip>jp)))
        dest.valuePtr()[k] = numext::conj(it.value());
      else
        dest.valuePtr()[k] = it.value();
    }
...
template<typename MatrixType, i32 Mode>
...
template<typename DstXprType, typename MatrixType, i32 Mode, typename Scalar>
struct Assignment<DstXprType, SparseSymmetricPermutationProduct<MatrixType,Mode>, internal::assign_op<Scalar,typename MatrixType::Scalar>, Sparse2Sparse>
{
  typedef SparseSymmetricPermutationProduct<MatrixType,Mode> SrcXprType;
  typedef typename DstXprType::StorageIndex DstIndex;
  template<i32 Options>
  ...
  template<typename DestType,u32 DestMode>
