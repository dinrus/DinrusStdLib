...
template <typename MatrixType_, i32 UpLo_, SparseFactorization_t Solver_, bool EnforceSquare_>
...
/** \ingroup AccelerateSupport_Module
  ...
  */
template <typename MatrixType, i32 UpLo = Lower>
...
class AccelerateImpl : public SparseSolverBase<AccelerateImpl<MatrixType_, UpLo_, Solver_, EnforceSquare_> > {
 ...
 public:
  ...
  AccelerateImpl() {
    ...
    auto check_flag_set = [](i32 value, i32 flag) { return ((value & flag) == flag); };
 ...
 private:
  ...
  void buildAccelSparseMatrix(const SparseMatrix<T>& a, AccelSparseMatrix& A, std::vector<long>& columnStarts) {
    ...
    structure.rowCount = static_cast<i32>(a.rows());
    structure.columnCount = static_cast<i32>(a.cols());
    structure.blockSize = 1;
    structure.columnStarts = columnStarts.data();
    structure.rowIndices = const_cast<i32*>(a.innerIndexPtr());
...
void AccelerateImpl<MatrixType_, UpLo_, Solver_, EnforceSquare_>::_solve_impl(const MatrixBase<Rhs>& b,
  ...
  xmat.columnCount = static_cast<i32>(x.cols());
  xmat.rowCount = static_cast<i32>(x.rows());
  xmat.columnStride = xmat.rowCount;
  xmat.data = x_ptr;

  AccelDenseMatrix bmat{};
  bmat.attributes = SparseAttributes_t();
  bmat.columnCount = static_cast<i32>(b.cols());
  bmat.rowCount = static_cast<i32>(b.rows());
