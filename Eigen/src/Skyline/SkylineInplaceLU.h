...
public:
     ...
     * flags \a flags. */
    SkylineInplaceLU(MatrixType& matrix, i32 flags = 0)
    ...
    void setOrderingMethod(i32 m) {
        m_flags = m;
    }

    i32 orderingMethod() const {
        return m_flags;
    }
...
template<typename MatrixType>
template<typename BDerived, typename XDerived>
bool SkylineInplaceLU<MatrixType>::solve(const MatrixBase<BDerived> &b, MatrixBase<XDerived>* x, i32k transposed) const {
    const size_t rows = m_lu.rows();
    const size_t cols = m_lu.cols();
