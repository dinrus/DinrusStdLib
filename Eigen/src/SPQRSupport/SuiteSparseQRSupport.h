  ...
  public:
    ...
    void compute(const MatrixType_& matrix)
    {
      ...
      if(m_useDefaultThreshold) 
      {
        ...
        for (i32 j = 0; j < mat.cols(); j++) max2Norm = numext::maxi(max2Norm, mat.col(j).norm());
...
//       dest = colsPermutation() * y.topRows(cols());
    ...
    void setSPQROrdering(i32 ord) { m_ordering = ord;}
  ...
  protected:
    ...
    i32 m_ordering; // Ordering method to use, see SPQR's manual
    i32 m_allow_tol; // Allow to use some tolerance during numerical factorization.
...
struct SPQR_QProduct : ReturnByValue<SPQR_QProduct<SPQRType,Derived> >
{
  ...
  void evalTo(ResType& res) const
  {
    cholmod_dense y_cd;
    cholmod_dense *x_cd; 
    i32 method = m_transpose ? SPQR_QTX : SPQR_QX; 
