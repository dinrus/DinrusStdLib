...
struct plan<float> {
  ...
  inline void forward2(complex_type* dst, complex_type* src, i32 n0, i32 n1) {
    if (m_plan == 0) {
  ...
  inline void inverse2(complex_type* dst, complex_type* src, i32 n0, i32 n1) {
    if (m_plan == 0) {
      MKL_LONG sizes[2] = {n0, n1};
      configure_descriptor(&m_plan, precision, DFTI_COMPLEX, 2, sizes);
    }
...
struct imklfft_impl {
  ...
  inline void fwd(Complex* dst, const Complex* src, i32 nfft) {
    MKL_LONG size = nfft;
  ...
  inline void fwd(Complex* dst, const Scalar* src, i32 nfft) {
    MKL_LONG size = nfft;
    get_plan(nfft, dst, src)
  ...
  inline void fwd2(Complex* dst, const Complex* src, i32 n0, i32 n1) {
    get_plan(n0, n1, dst, src)
        .forward2(complex_cast(dst), complex_cast(src), n0, n1);
  }

  // inverse complex-to-complex
  inline void inv(Complex* dst, const Complex* src, i32 nfft) {
    MKL_LONG size = nfft;
    get_plan(nfft, dst, src)
  ...
  inline void inv(Scalar* dst, const Complex* src, i32 nfft) {
    MKL_LONG size = nfft;
    get_plan(nfft, dst, src)
  ...
  // 2-d complex-to-complex
  inline void inv2(Complex* dst, const Complex* src, i32 n0, i32 n1) {
    get_plan(n0, n1, dst, src)
 ...
 private:
  ...
  inline plan<Scalar>& get_plan(i32 nfft, void* dst,
                                const void* src) {
    i32 inplace = dst == src ? 1 : 0;
    i32 aligned = ((reinterpret_cast<size_t>(src) & 15) |
  ...
  inline plan<Scalar>& get_plan(i32 n0, i32 n1, void* dst,
                                const void* src) {
    i32 inplace = (dst == src) ? 1 : 0;
    i32 aligned = ((reinterpret_cast<size_t>(src) & 15) |
                   (reinterpret_cast<size_t>(dst) & 15)) == 0
