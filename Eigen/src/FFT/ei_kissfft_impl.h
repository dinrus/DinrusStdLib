...
struct kiss_cpx_fft
{
  ...
  std::vector<i32> m_stageRadix;
  std::vector<i32> m_stageRemainder;
  std::vector<Complex> m_scratchBuf;
  bool m_inverse;

  inline void make_twiddles(i32 nfft, bool inverse)
  {
    using numext::sin;
    using numext::cos;
    m_inverse = inverse;
  ...
  template <typename Src_>
    inline
    void work( i32 stage,Complex * xout, const Src_ * xin, size_t fstride,size_t in_stride)
    {
      i32 p = m_stageRadix[stage];
      i32 m = m_stageRemainder[stage];
  ...
  inline
    void bfly2( Complex * Fout, const size_t fstride, i32 m)
    {
      for (i32 k=0;k<m;++k) {
  ...
  inline
    ...
    {
      Complex scratch[6];
      i32 negative_if_inverse = m_inverse * -2 +1;
  ...
  inline
        ...
        )
    {
      ...
      Complex * twiddles = &m_twiddles[0];
      Complex t;
      i32 Norig = static_cast<i32>(m_twiddles.size());
      ...
      for ( u=0; u<m; ++u ) {
        ...
        for ( q1=0 ; q1<p ; ++q1 ) {
          ...
          Fout[ k ] = scratchbuf[0];
          for (q=1;q<p;++q ) {
            twidx += static_cast<i32>(fstride) * k;
...
struct kissfft_impl
{
  ...
  inline
    void fwd( Complex * dst,const Complex *src,i32 nfft)
    {
      get_plan(nfft,false).work(0, dst, src, 1,1);
    }

  inline
    void fwd2( Complex * dst,const Complex *src,i32 n0,i32 n1)
    {
  ...
  inline
    void inv2( Complex * dst,const Complex *src,i32 n0,i32 n1)
    {
        EIGEN_UNUSED_VARIABLE(dst);
        EIGEN_UNUSED_VARIABLE(src);
        EIGEN_UNUSED_VARIABLE(n0);
  ...
  inline
    void fwd( Complex * dst,const Scalar * src,i32 nfft) 
    {
      if ( nfft&3  ) {
  ...
  inline
    void inv(Complex * dst,const Complex  *src,i32 nfft)
    {
  ...
  typedef std::map<i32,PlanData> PlanMap;

  PlanMap m_plans;
  std::map<i32, std::vector<Complex> > m_realTwiddles;
  std::vector<Complex> m_tmpBuf1;
  std::vector<Complex> m_tmpBuf2;

  inline
    i32 PlanKey(i32 nfft, bool isinverse) const { return (nfft<<1) | i32(isinverse); }

  inline
    PlanData & get_plan(i32 nfft, bool inverse)
    {
  ...
  inline
    Complex * real_twiddles(i32 ncfft2)
    {
      using std::acos;
      std::vector<Complex> & twidref = m_realTwiddles[ncfft2];// creates new if not there
      if ( (i32)twidref.size() != ncfft2 ) {
        twidref.resize(ncfft2);
        i32 ncfft= ncfft2<<1;
        Scalar pi =  acos( Scalar(-1) );
        for (i32 k=1;k<=ncfft2;++k) 
