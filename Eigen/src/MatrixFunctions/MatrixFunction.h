...
MatrixType MatrixFunctionAtomic<MatrixType>::compute(const MatrixType& A)
{
  ...
  for (Index s = 1; double(s) < 1.1 * double(rows) + 10.0; s++) { // upper limit is fairly arbitrary
    Fincr = m_f(avgEival, static_cast<i32>(s)) * P;
    F += Fincr;
    ...
    if (Fincr_norm < NumTraits<Scalar>::epsilon() * F_norm) {
      ...
      for (Index r = 0; r < rows; r++) {
        ...
        for (Index i = 0; i < rows; i++)
          mx = (std::max)(mx, std::abs(m_f(Ashifted(i, i) + avgEival, static_cast<i32>(s+r))));
        if (r != 0)
...
{ 
  ...
  typedef typename MatrixType::Scalar Scalar;
  static i32k Options = MatrixType::Options;
...
/** \ingroup MatrixFunctions_Module
  ...
  */
template <typename MatrixType, i32 IsComplex = NumTraits<typename internal::traits<MatrixType>::Scalar>::IsComplex>
...
{  
  ...
  static void run(const MatA& A, AtomicType& atomic, ResultType &result)
  {
    ...
    static i32k Rows = Traits::RowsAtCompileTime, Cols = Traits::ColsAtCompileTime;
    static i32k MaxRows = Traits::MaxRowsAtCompileTime, MaxCols = Traits::MaxColsAtCompileTime;
