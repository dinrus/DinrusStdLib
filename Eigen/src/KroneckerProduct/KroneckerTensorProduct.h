...
{
  i32k BlockRows = Rhs::RowsAtCompileTime,
...
void KroneckerProductSparse<Lhs,Rhs>::evalTo(Dest& dst) const
{
  ...
  // compute number of non-zeros per innervectors of dst
  {
    ...
    Matrix<i32,Dynamic,Dynamic,ColMajor> nnzAB = nnzB * nnzA.transpose();
...
struct traits<KroneckerProductSparse<Lhs_,Rhs_> >
{
  ...
  enum {
    ...
    EvalToRowMajor = (i32(LhsFlags) & i32(RhsFlags) & RowMajorBit),
    RemovedBits = ~(EvalToRowMajor ? 0 : RowMajorBit),

    Flags = ((i32(LhsFlags) | i32(RhsFlags)) & HereditaryBits & RemovedBits)
