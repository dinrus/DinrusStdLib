...
template<typename MatrixType_, i32 Options> class PastixLLT;
template<typename MatrixType_, i32 Options> class PastixLDLT;
...
namespace internal
{
  ...
  template<typename MatrixType_, i32 Options>
  ...
  inline void eigen_pastix(pastix_data_t **pastix_data, i32 pastix_comm, i32 n, i32 *ptr, i32 *idx, float *vals, i32 *perm, i32 * invp, float *x, i32 nbrhs, i32 *iparm, double *dparm)
  {
  ...
  inline void eigen_pastix(pastix_data_t **pastix_data, i32 pastix_comm, i32 n, i32 *ptr, i32 *idx, double *vals, i32 *perm, i32 * invp, double *x, i32 nbrhs, i32 *iparm, double *dparm)
  {
    if (n == 0) { ptr = NULL; idx = NULL; vals = NULL; }
    if (nbrhs == 0) {x = NULL; nbrhs=1;}
  ...
  inline void eigen_pastix(pastix_data_t **pastix_data, i32 pastix_comm, i32 n, i32 *ptr, i32 *idx, std::complex<float> *vals, i32 *perm, i32 * invp, std::complex<float> *x, i32 nbrhs, i32 *iparm, double *dparm)
  {
    if (n == 0) { ptr = NULL; idx = NULL; vals = NULL; }
    if (nbrhs == 0) {x = NULL; nbrhs=1;}
  ...
  inline void eigen_pastix(pastix_data_t **pastix_data, i32 pastix_comm, i32 n, i32 *ptr, i32 *idx, std::complex<double> *vals, i32 *perm, i32 * invp, std::complex<double> *x, i32 nbrhs, i32 *iparm, double *dparm)
  {
    if (n == 0) { ptr = NULL; idx = NULL; vals = NULL; }
    if (nbrhs == 0) {x = NULL; nbrhs=1;}
...
class PastixBase : public SparseSolverBase<Derived>
{
  ...
  protected:
    ...
    mutable i32 m_comm; // The MPI communicator identifier
    mutable Array<i32,IPARM_SIZE,1> m_iparm; // integer vector for the input parameters
    ...
    mutable i32 m_size; // Size of the matrix 
}; 
...
{                         
  ...
  m_size = internal::convert_index<i32>(mat.rows());
...
bool PastixBase<Base>::_solve_impl(const MatrixBase<Rhs> &b, MatrixBase<Dest> &x) const
{
  ...
  for (i32 i = 0; i < b.cols(); i++){
    m_iparm[IPARM_START_TASK]          = API_TASK_SOLVE;
    m_iparm[IPARM_END_TASK]            = API_TASK_REFINE;
  
    internal::eigen_pastix(&m_pastixdata, MPI_COMM_WORLD, internal::convert_index<i32>(x.rows()), 0, 0, 0,
...
/** \ingroup PaStiXSupport_Module
  ...
  */
template<typename MatrixType_, i32 UpLo_>
