...
struct transform_traits
{
  enum
  {
    ...
    HDim = Transform::HDim,
    Mode = Transform::Mode,
    IsProjective = (i32(Mode)==i32(Projective))
  };
};

template< typename TransformType,
          typename MatrixType,
          i32 Case = transform_traits<TransformType>::IsProjective ? 0
                   : i32(MatrixType::RowsAtCompileTime) == i32(transform_traits<TransformType>::HDim) ? 1
                   : 2,
          i32 RhsCols = MatrixType::ColsAtCompileTime>
...
template< typename Other,
          ...
          i32 Dim,
          i32 HDim,
          i32 OtherRows=Other::RowsAtCompileTime,
          i32 OtherCols=Other::ColsAtCompileTime>
...
template<typename Scalar_, i32 Dim_, i32 Mode_, i32 Options_>
...
template<i32 Mode> struct transform_make_affine;
...
public:
  ...
  enum {
    ...
    Rows = i32(Mode)==(AffineCompact) ? Dim : HDim
  };
  ...
  typedef Block<MatrixType,Dim,Dim,i32(Mode)==(AffineCompact) && (i32(Options)&RowMajor)==0> LinearPart;
  /** type of read reference to the linear part of the transformation */
  typedef const Block<ConstMatrixType,Dim,Dim,i32(Mode)==(AffineCompact) && (i32(Options)&RowMajor)==0> ConstLinearPart;
  /** type of read/write reference to the affine part of the transformation */
  typedef std::conditional_t<i32(Mode)==i32(AffineCompact),
                              MatrixType&,
  ...
  enum { TransformTimeDiagonalMode = ((Mode==i32(Isometry))?Affine:i32(Mode)) };
...
public:
  ...
  EIGEN_DEVICE_FUNC inline Transform()
  {
    ...
    internal::transform_make_affine<(i32(Mode)==Affine || i32(Mode)==Isometry) ? Affine : AffineCompact>::run(m_matrix);
  }
  ...
  template<i32 OtherMode,i32 OtherOptions>
  EIGEN_DEVICE_FUNC inline Transform(const Transform<Scalar,Dim,OtherMode,OtherOptions>& other)
  {
    check_template_params();
    // prevent conversions as:
    // Affine | AffineCompact | Isometry = Projective
    EIGEN_STATIC_ASSERT(internal::check_implication(OtherMode==i32(Projective), Mode==i32(Projective)),
                        YOU_PERFORMED_AN_INVALID_TRANSFORMATION_CONVERSION)

    // prevent conversions as:
    // Isometry = Affine | AffineCompact
    EIGEN_STATIC_ASSERT(internal::check_implication(OtherMode==i32(Affine)||OtherMode==i32(AffineCompact), Mode!=i32(Isometry)),
                        YOU_PERFORMED_AN_INVALID_TRANSFORMATION_CONVERSION)

    enum { ModeIsAffineCompact = Mode == i32(AffineCompact),
           OtherModeIsAffineCompact = OtherMode == i32(AffineCompact)
    };
  ...
  EIGEN_DEVICE_FUNC EIGEN_CONSTEXPR Index rows() const EIGEN_NOEXCEPT { return i32(Mode)==i32(Projective) ? m_matrix.cols() : (m_matrix.cols()-1); }
    ...
    operator * (const DiagonalBase<DiagonalDerived> &a, const Transform &b)
  {
    ...
    if (EIGEN_CONST_CONDITIONAL(Mode!=i32(AffineCompact)))
...
private:
  ...
  //
  template<i32 OtherMode,i32 OtherOptions> struct icc_11_workaround
  {
...
public:
  ...
  typedef std::conditional_t<i32(Mode)==Isometry,ConstLinearPart,const LinearMatrixType> RotationReturnType;
  ...
  {
    internal::transform_make_affine<i32(Mode)>::run(m_matrix);
  }
  ...
  /** \internal
    ...
    */
  EIGEN_DEVICE_FUNC inline Block<MatrixType,i32(Mode)==i32(Projective)?HDim:Dim,Dim> linearExt()
  { return m_matrix.template block<i32(Mode)==i32(Projective)?HDim:Dim,Dim>(0,0); }
  /** \internal
    * \returns the Dim x Dim linear part if the transformation is affine,
    *          and the HDim x Dim part for projective transformations.
    */
  EIGEN_DEVICE_FUNC inline const Block<MatrixType,i32(Mode)==i32(Projective)?HDim:Dim,Dim> linearExt() const
  { return m_matrix.template block<i32(Mode)==i32(Projective)?HDim:Dim,Dim>(0,0); }
  ...
  /** \internal
    ...
    */
  EIGEN_DEVICE_FUNC inline Block<MatrixType,i32(Mode)==i32(Projective)?HDim:Dim,1> translationExt()
  { return m_matrix.template block<i32(Mode)==i32(Projective)?HDim:Dim,1>(0,Dim); }
  /** \internal
    * \returns the translation part if the transformation is affine,
    *          and the last column for projective transformations.
    */
  EIGEN_DEVICE_FUNC inline const Block<MatrixType,i32(Mode)==i32(Projective)?HDim:Dim,1> translationExt() const
  { return m_matrix.template block<i32(Mode)==i32(Projective)?HDim:Dim,1>(0,Dim); }
...
/** Initializes \c *this from a QMatrix assuming the dimension is 2.
  ...
  */
template<typename Scalar, i32 Dim, i32 Mode,i32 Options>
...
{
  EIGEN_STATIC_ASSERT_VECTOR_SPECIFIC_SIZE(OtherDerived,i32(Dim))
  EIGEN_STATIC_ASSERT(Mode!=i32(Isometry), THIS_METHOD_IS_ONLY_FOR_SPECIFIC_TRANSFORMATIONS)
...
{
  EIGEN_STATIC_ASSERT_VECTOR_SPECIFIC_SIZE(OtherDerived,i32(Dim))
  if(EIGEN_CONST_CONDITIONAL(i32(Mode)==i32(Projective)))
...
{
  EIGEN_STATIC_ASSERT(i32(Dim)==2, YOU_MADE_A_PROGRAMMING_MISTAKE)
  EIGEN_STATIC_ASSERT(Mode!=i32(Isometry), THIS_METHOD_IS_ONLY_FOR_SPECIFIC_TRANSFORMATIONS)
...
namespace internal {
template<i32 Mode> struct transform_rotation_impl {
  template<typename TransformType>
...
struct transform_make_affine
{
  ...
  {
    static i32k Dim = MatrixType::ColsAtCompileTime-1;
...
template<typename TransformType, i32 Mode=TransformType::Mode>
...
template<typename Scalar, i32 Dim, i32 Options>
...
template<typename Other, i32 Mode, i32 Options, i32 Dim, i32 HDim>
...
template<typename Other, i32 Options, i32 Dim, i32 HDim>
...
template<i32 LhsMode,i32 RhsMode>
struct transform_product_result
{
  enum
  {
    Mode =
      (LhsMode == (i32)Projective    || RhsMode == (i32)Projective    ) ? Projective :
      (LhsMode == (i32)Affine        || RhsMode == (i32)Affine        ) ? Affine :
      (LhsMode == (i32)AffineCompact || RhsMode == (i32)AffineCompact ) ? AffineCompact :
      (LhsMode == (i32)Isometry      || RhsMode == (i32)Isometry      ) ? Isometry : Projective
  };
};

template< typename TransformType, typename MatrixType, i32 RhsCols>
...
struct transform_right_product_impl< TransformType, MatrixType, 1, RhsCols>
{
  ...
  static EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE ResultType run(const TransformType& T, const MatrixType& other)
  {
    ...
    typedef Block<ResultType, Dim, OtherCols, i32(MatrixType::RowsAtCompileTime)==Dim> TopLeftLhs;
...
template<typename Scalar, i32 Dim, i32 LhsMode, i32 LhsOptions, i32 RhsMode, i32 RhsOptions>
...
template<typename Scalar, i32 Dim, i32 LhsOptions, i32 RhsOptions>
