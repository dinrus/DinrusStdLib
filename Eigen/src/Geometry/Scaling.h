...
namespace internal
{
  ...
  // See bug 1412.
  template <typename Scalar, i32 Dim, i32 Mode>
  struct uniformscaling_times_affine_returntype
  {
    enum
    {
      NewMode = i32(Mode) == i32(Isometry) ? Affine : Mode
    };
...
public:
  ...
  template<i32 Dim, i32 Mode, i32 Options>
  inline typename
  ...
  template<typename Derived,i32 Dim>
