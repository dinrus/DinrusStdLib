...
template<typename Scalar, i32 Dim>
struct vector_int_pair
{
EIGEN_MAKE_ALIGNED_OPERATOR_NEW_IF_VECTORIZABLE_FIXED_SIZE(Scalar, Dim)
  typedef Matrix<Scalar, Dim, 1> VectorType;

  vector_int_pair(const VectorType &v, i32 i) : first(v), second(i) {}
...
struct get_boxes_helper<ObjectList, VolumeList, i32> {
  void operator()(const ObjectList &objects, i32, i32, VolumeList &outBoxes)
  {
 ...
 */
template<typename Scalar_, i32 Dim_, typename _Object> class KdBVH
{
public:
  enum { Dim = Dim_ };
  ...
  typedef i32 Index;
  typedef i32k *VolumeIterator; //the iterators are just pointers into the tree's vectors
  ...
  template<typename Iter> KdBVH(Iter begin, Iter end) { init(begin, end, 0, 0); } //i32 is recognized by init as not being an iterator type
  ...
  template<typename OIter, typename BIter> void init(OIter begin, OIter end, BIter boxBegin, BIter boxEnd)
  {
    ...
    i32 n = static_cast<i32>(objects.size());
  ...
  inline Index getRootIndex() const { return (i32)boxes.size() - 1; }
  ...
  { //inlining this function should open lots of optimization opportunities to the compiler
    ...
    i32 numBoxes = static_cast<i32>(boxes.size());
...
private:
  ...
  {
    VectorComparator(i32 inDim) : dim(inDim) {}
  ...
  void build(VIPairList &objCenters, i32 from, i32 to, const VolumeList &objBoxes, i32 dim)
  {
    eigen_assert(to - from > 1);
    if(to - from == 2) {
      boxes.push_back(objBoxes[objCenters[from].second].merged(objBoxes[objCenters[from + 1].second]));
      children.push_back(from + (i32)objects.size() - 1); //there are objects.size() - 1 tree nodes
      children.push_back(from + (i32)objects.size());
    }
    else if(to - from == 3) {
      i32 mid = from + 2;
  ...
  std::vector<i32> children; //children of x are children[2x] and children[2x+1], indices bigger than boxes.size() index into objects.
  VolumeList boxes;
  ObjectList objects;
};
