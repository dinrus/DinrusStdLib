...
template<typename Scalar, i32 StorageOrder, typename PivIndex, i32 SizeAtCompileTime=Dynamic>
struct partial_lu_impl
{
  static constexpr i32 UnBlockedBound = 16;
  static constexpr bool UnBlockedAtCompileTime = SizeAtCompileTime!=Dynamic && SizeAtCompileTime<=UnBlockedBound;
  static constexpr i32 ActualSizeAtCompileTime = UnBlockedAtCompileTime ? SizeAtCompileTime : Dynamic;
  // Remaining rows and columns at compile-time:
  static constexpr i32 RRows = SizeAtCompileTime==2 ? 1 : Dynamic;
  static constexpr i32 RCols = SizeAtCompileTime==2 ? 1 : Dynamic;
  ...
  static Index unblocked_lu(MatrixTypeRef& lu, PivIndex* row_transpositions, PivIndex& nb_transpositions)
  {
    ...
    {
      i32 rrows = internal::convert_index<i32>(rows-k-1);
      i32 rcols = internal::convert_index<i32>(cols-k-1);
...
{
  // the row permutation is stored as i32 indices, so just to be sure:
  eigen_assert(m_lu.rows()<NumTraits<i32>::highest());
