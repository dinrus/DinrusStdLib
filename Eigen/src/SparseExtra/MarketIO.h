...
namespace internal 
{
  ...
  template<> inline void GetMarketLine (const char* line, i32& i, i32& j, float& value)
  { std::sscanf(line, "%d %d %g", &i, &j, &value); }

  template<> inline void GetMarketLine (const char* line, i32& i, i32& j, double& value)
  { std::sscanf(line, "%d %d %lg", &i, &j, &value); }

  template<> inline void GetMarketLine (const char* line, i32& i, i32& j, std::complex<float>& value)
  { std::sscanf(line, "%d %d %g %g", &i, &j, &numext::real_ref(value), &numext::imag_ref(value)); }

  template<> inline void GetMarketLine (const char* line, i32& i, i32& j, std::complex<double>& value)
  ...
  inline void putMarketHeader(std::string& header,i32 sym)
  {
 ...
 */
inline bool getMarketHeader(const std::string& filename, i32& sym, bool& iscomplex, bool& isdense)
{
  sym = 0; 
  iscomplex = false;
  isdense = false;
 ...
 */
template<typename SparseMatrixType>
bool saveMarket(const SparseMatrixType& mat, const std::string& filename, i32 sym = 0)
{
  typedef typename SparseMatrixType::Scalar Scalar;
