...
inline i32 klu_solve(klu_symbolic *Symbolic, klu_numeric *Numeric, Index ldim, Index nrhs, double B [ ], klu_common *Common, double) {
   return klu_solve(Symbolic, Numeric, internal::convert_index<i32>(ldim), internal::convert_index<i32>(nrhs), B, Common);
}

inline i32 klu_solve(klu_symbolic *Symbolic, klu_numeric *Numeric, Index ldim, Index nrhs, std::complex<double>B[], klu_common *Common, std::complex<double>) {
   return klu_z_solve(Symbolic, Numeric, internal::convert_index<i32>(ldim), internal::convert_index<i32>(nrhs), &numext::real_ref(B[0]), Common);
}

inline i32 klu_tsolve(klu_symbolic *Symbolic, klu_numeric *Numeric, Index ldim, Index nrhs, double B[], klu_common *Common, double) {
   return klu_tsolve(Symbolic, Numeric, internal::convert_index<i32>(ldim), internal::convert_index<i32>(nrhs), B, Common);
}

inline i32 klu_tsolve(klu_symbolic *Symbolic, klu_numeric *Numeric, Index ldim, Index nrhs, std::complex<double>B[], klu_common *Common, std::complex<double>) {
   return klu_z_tsolve(Symbolic, Numeric, internal::convert_index<i32>(ldim), internal::convert_index<i32>(nrhs), &numext::real_ref(B[0]), 0, Common);
}

inline klu_numeric* klu_factor(i32 Ap [ ], i32 Ai [ ], double Ax [ ], klu_symbolic *Symbolic, klu_common *Common, double) {
   return klu_factor(Ap, Ai, Ax, Symbolic, Common);
}

inline klu_numeric* klu_factor(i32 Ap[], i32 Ai[], std::complex<double> Ax[], klu_symbolic *Symbolic, klu_common *Common, std::complex<double>) {
...
class KLU : public SparseSolverBase<KLU<MatrixType_> >
{
  ...
  public:
    ...
    typedef SparseMatrix<Scalar,ColMajor,i32> KLUMatrixType;
  ...
  protected:
    ...
    void analyzePattern_impl()
    {
      ...
      m_symbolic = klu_analyze(internal::convert_index<i32>(mp_matrix.rows()),
...
void KLU<MatrixType>::extractData() const
{
  if (m_extractedDataAreDirty)
  {
    ...
    i32 lnz, unz, rows, cols, nz_udiag;
...
bool KLU<MatrixType>::_solve_impl(const MatrixBase<BDerived> &b, MatrixBase<XDerived> &x) const
{
  ...
  x = b;
  i32 info = klu_solve(m_symbolic, m_numeric, b.rows(), rhsCols, x.const_cast_derived().data(), const_cast<klu_common*>(&m_common), Scalar());
