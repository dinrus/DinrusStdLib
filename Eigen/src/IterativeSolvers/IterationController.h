...
class IterationController
{
  protected :
    ...
    i32 m_noise;          ///< if noise > 0 iterations are printed
  ...
  public :
    ...
    IterationController(double r = 1.0E-8, i32 noi = 0, size_t mit = size_t(-1))
      : m_rhsn(1.0), m_maxiter(mit), m_noise(noi), m_resmax(r) { init(); }

    void operator ++(i32) { m_nit++; m_written = false; m_resadd += m_res; }
    void operator ++() { (*this)++; }

    bool first() { return m_nit == 0; }

    /* get/set the "noisyness" (verbosity) of the solvers */
    i32 noiseLevel() const { return m_noise; }
    void setNoiseLevel(i32 n) { m_noise = n; }
