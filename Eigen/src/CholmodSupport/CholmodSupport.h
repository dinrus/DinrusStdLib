...
/** Wraps the Eigen sparse matrix \a mat into a Cholmod sparse matrix object.
  ...
  */
template<typename Scalar_, i32 Options_, typename StorageIndex_>
cholmod_sparse viewAsCholmod(Ref<SparseMatrix<Scalar_,Options_,StorageIndex_> > mat)
{
  ...
  if (internal::is_same<StorageIndex_,i32>::value)
  {
...
template<typename Scalar_, i32 Options_, typename Index_>
...
template<typename Scalar_, i32 Options_, typename Index_, u32 UpLo>
...
/** Returns a view of the Cholmod sparse matrix \a cm as an Eigen sparse matrix.
  * The data are not copied but shared. */
template<typename Scalar, i32 Flags, typename StorageIndex>
...
// template specializations for i32 and long that call the correct cholmod method
...
EIGEN_CHOLMOD_SPECIALIZE0(i32, start)
EIGEN_CHOLMOD_SPECIALIZE0(i32, finish)

EIGEN_CHOLMOD_SPECIALIZE1(i32, free_factor, cholmod_factor*, L)
EIGEN_CHOLMOD_SPECIALIZE1(i32, free_dense,  cholmod_dense*,  X)
EIGEN_CHOLMOD_SPECIALIZE1(i32, free_sparse, cholmod_sparse*, A)

EIGEN_CHOLMOD_SPECIALIZE1(cholmod_factor*, analyze, cholmod_sparse, A)

template<typename StorageIndex_> inline cholmod_dense*  cm_solve         (i32 sys, cholmod_factor& L, cholmod_dense&  B, cholmod_common &Common) { return cholmod_solve     (sys, &L, &B, &Common); }
template<>                       inline cholmod_dense*  cm_solve<SuiteSparse_long>   (i32 sys, cholmod_factor& L, cholmod_dense&  B, cholmod_common &Common) { return cholmod_l_solve   (sys, &L, &B, &Common); }

template<typename StorageIndex_> inline cholmod_sparse* cm_spsolve       (i32 sys, cholmod_factor& L, cholmod_sparse& B, cholmod_common &Common) { return cholmod_spsolve   (sys, &L, &B, &Common); }
template<>                       inline cholmod_sparse* cm_spsolve<SuiteSparse_long> (i32 sys, cholmod_factor& L, cholmod_sparse& B, cholmod_common &Common) { return cholmod_l_spsolve (sys, &L, &B, &Common); }

template<typename StorageIndex_>
inline i32  cm_factorize_p       (cholmod_sparse*  A, double beta[2], StorageIndex_* fset, std::size_t fsize, cholmod_factor* L, cholmod_common &Common) { return cholmod_factorize_p   (A, beta, fset, fsize, L, &Common); }
template<>
inline i32  cm_factorize_p<SuiteSparse_long> (cholmod_sparse*  A, double beta[2], SuiteSparse_long* fset,          std::size_t fsize, cholmod_factor* L, cholmod_common &Common) { return cholmod_l_factorize_p (A, beta, fset, fsize, L, &Common); }
...
/** \ingroup CholmodSupport_Module
  ...
  */
template<typename MatrixType_, i32 UpLo_, typename Derived>
...
/** \ingroup CholmodSupport_Module
  ...
  */
template<typename MatrixType_, i32 UpLo_ = Lower>
