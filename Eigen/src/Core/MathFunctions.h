...
template<u32 n, i32 lower, i32 upper> struct meta_floor_log2_selector
{
  enum { middle = (lower + upper) / 2,
         value = (upper <= lower + 1) ? i32(meta_floor_log2_terminate)
               : (n < (1 << middle)) ? i32(meta_floor_log2_move_down)
               : (n==0) ? i32(meta_floor_log2_bogus)
               : i32(meta_floor_log2_move_up)
  };
};

template<u32 n,
         i32 lower = 0,
         i32 upper = sizeof(u32) * CHAR_BIT - 1,
         i32 selector = meta_floor_log2_selector<n, lower, upper>::value>
struct meta_floor_log2 {};

template<u32 n, i32 lower, i32 upper>
...
{
  enum { value = (n >= ((u32)(1) << (lower+1))) ? lower+1 : lower };
};
...
struct random_default_impl<Scalar, false, true>
{
  static inline Scalar run(const Scalar& x, const Scalar& y)
  {
    ...
    // ScalarX is the widest of ScalarU and u32.
    // We'll deal only with ScalarX and u32 below thus avoiding signed
...
#else
    enum { rand_bits = meta_floor_log2<(u32)(RAND_MAX)+1>::value,
           scalar_bits = sizeof(Scalar) * CHAR_BIT,
           shift = plain_enum_max(0, i32(rand_bits) - i32(scalar_bits)),
...
inline i32 log2(i32 x)
{
  eigen_assert(x>=0);
  u32 v(x);
  static i32k table[32] = { 0, 9, 1, 10, 13, 21, 2, 29, 11, 14, 16, 18, 22, 25, 3, 30, 8, 12, 20, 28, 15, 17, 24, 7, 19, 27, 23, 6, 26, 5, 4, 31 };
  v |= v >> 1;
  v |= v >> 2;
  v |= v >> 4;
  v |= v >> 8;
...
template <typename T, std::enable_if_t<!NumTraits<T>::IsComplex, i32> = 0>
...
template<> struct random_impl<bool>
{
  ...
  {
    return random<i32>(0,1)==0 ? false : true;
  }

  static inline bool run(const bool& a, const bool& b)
  {
    return random<i32>(a, b)==0 ? false : true;
  }
};
