...
template<typename Derived, i32 UnrollCount, i32 InnerSize>
struct all_unroller
{
  enum {
    IsRowMajor = (i32(Derived::Flags) & i32(RowMajor)),
...
template<typename Derived, i32 InnerSize>
...
EIGEN_DEVICE_FUNC inline bool DenseBase<Derived>::all() const
{
  ...
  enum {
    unroll = SizeAtCompileTime != Dynamic
          && SizeAtCompileTime * (i32(Evaluator::CoeffReadCost) + i32(NumTraits<Scalar>::AddCost)) <= EIGEN_UNROLLING_LIMIT,
  };
  Evaluator evaluator(derived());
  if(unroll)
    return internal::all_unroller<Evaluator, unroll ? i32(SizeAtCompileTime) : Dynamic, InnerSizeAtCompileTime>::run(evaluator);
  else
  {
...
EIGEN_DEVICE_FUNC inline bool DenseBase<Derived>::any() const
{
  ...
  Evaluator evaluator(derived());
  if(unroll)
    return internal::any_unroller<Evaluator, unroll ? i32(SizeAtCompileTime) : Dynamic, InnerSizeAtCompileTime>::run(evaluator);
  else
  {
    for(Index i = 0; i < derived().outerSize(); ++i)
      for(Index j = 0; j < derived().innerSize(); ++j)
