 ...
 : traits<MatrixType>
{
  ...
  enum {
    ...
    // need to cast the sizeof's from size_t to i32 explicitly, otherwise:
    // "error: no integral type can represent all of the enumerator values
    InnerStrideAtCompileTime = StrideType::InnerStrideAtCompileTime == 0
                             ? (MatrixTypeInnerStride == Dynamic
                               ? i32(Dynamic)
                               : i32(MatrixTypeInnerStride) * i32(sizeof(typename traits<MatrixType>::Scalar) / sizeof(Scalar)))
                             : i32(StrideType::InnerStrideAtCompileTime),

    OuterStrideAtCompileTime = StrideType::OuterStrideAtCompileTime == 0
                             ? (outer_stride_at_compile_time<MatrixType>::ret == Dynamic
                               ? i32(Dynamic)
                               : outer_stride_at_compile_time<MatrixType>::ret * i32(sizeof(typename traits<MatrixType>::Scalar) / sizeof(Scalar)))
