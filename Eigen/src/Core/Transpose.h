...
template<typename MatrixType,
  ...
  bool MatchPacketSize =
        (i32(MatrixType::RowsAtCompileTime) == i32(internal::packet_traits<typename MatrixType::Scalar>::size))
...
void BlockedInPlaceTranspose(MatrixType& m) {
  ...
  for (; row_start + PacketSize <= m.rows(); row_start += PacketSize) {
    for (i32 col_start = row_start; col_start + PacketSize <= m.cols(); col_start += PacketSize) {
      PacketBlock<Packet> A;
