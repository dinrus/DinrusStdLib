...
template< typename MatrixType, typename MemberOp, i32 Direction>
class PartialReduxExpr;
...
#define EIGEN_MAKE_PARTIAL_REDUX_FUNCTOR(MEMBER,COST,VECTORIZABLE,BINARYOP)                \
  ...
  struct member_##MEMBER {                                                                  \
    ...
    template<i32 Size> struct Cost { enum { value = COST }; };             \
...
template <i32 p, typename ResultType,typename Scalar>
struct member_lpnorm {
  typedef ResultType result_type;
  enum { Vectorizable = 0 };
  template<i32 Size> struct Cost
...
struct member_redux {
  ...
  template<i32 Size> struct Cost { enum { value = (Size-1) * functor_traits<BinaryOp>::Cost }; };
...
template<typename ExpressionType, i32 Direction> class VectorwiseOp
{
  ...
  public:
    ...
    template<i32 p> struct LpNormReturnType {
    ...
    * \sa DenseBase::mean() */
    ...
    template<i32 Factor> const Replicate<ExpressionType,isVertical*Factor+isHorizontal,isHorizontal*Factor+isVertical>
    EIGEN_DEVICE_FUNC
...
/////////// Geometry module ///////////
    ...
    typedef Block<const ExpressionType,
                  Direction==Vertical   ? i32(HNormalized_SizeMinusOne)
                                        : i32(internal::traits<ExpressionType>::RowsAtCompileTime),
                  Direction==Horizontal ? i32(HNormalized_SizeMinusOne)
                                        : i32(internal::traits<ExpressionType>::ColsAtCompileTime)>
            HNormalized_Block;
    typedef Block<const ExpressionType,
                  Direction==Vertical   ? 1 : i32(internal::traits<ExpressionType>::RowsAtCompileTime),
                  Direction==Horizontal ? 1 : i32(internal::traits<ExpressionType>::ColsAtCompileTime)>
            HNormalized_Factors;
