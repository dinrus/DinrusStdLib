...
template<typename PlainObjectType_, i32 Options_, typename StrideType_>
  ...
  : public traits<Map<PlainObjectType_, Options_, StrideType_> >
{
  ...
  template<typename Derived> struct match {
    enum {
      ...
      InnerStrideMatch = i32(StrideType::InnerStrideAtCompileTime)==i32(Dynamic)
                      || i32(StrideType::InnerStrideAtCompileTime)==i32(Derived::InnerStrideAtCompileTime)
                      || (i32(StrideType::InnerStrideAtCompileTime)==0 && i32(Derived::InnerStrideAtCompileTime)==1),
      OuterStrideMatch = IsVectorAtCompileTime
                      || i32(StrideType::OuterStrideAtCompileTime)==i32(Dynamic) || i32(StrideType::OuterStrideAtCompileTime)==i32(Derived::OuterStrideAtCompileTime),
      // NOTE, this indirection of evaluator<Derived>::Alignment is needed
      // to workaround a very strange bug in MSVC related to the instantiation
      // of has_*ary_operator in evaluator<CwiseNullaryOp>.
      // This line is surprisingly very sensitive. For instance, simply adding parenthesis
      // as "DerivedAlignment = (i32(evaluator<Derived>::Alignment))," will make MSVC fail...
      DerivedAlignment = i32(evaluator<Derived>::Alignment),
      AlignmentMatch = (i32(traits<PlainObjectType>::Alignment)==i32(Unaligned)) || (DerivedAlignment >= i32(Alignment)), // FIXME the first condition is not very clear, it should be replaced by the required alignment
...
public:
  ...
  EIGEN_DEVICE_FUNC EIGEN_CONSTEXPR inline Index outerStride() const
  {
    return StrideType::OuterStrideAtCompileTime != 0 ? m_stride.outer()
         ...
         : i32(Flags)&RowMajorBit ? this->cols()
         : this->rows();
  }
  ...
  EIGEN_DEVICE_FUNC bool construct(Expression& expr)
  {
    ...
    EIGEN_STATIC_ASSERT(
      ...
      || ( PlainObjectType::IsVectorAtCompileTime
            && ((i32(PlainObjectType::RowsAtCompileTime)==Eigen::Dynamic
              || i32(Expression::ColsAtCompileTime)==Eigen::Dynamic
              || i32(PlainObjectType::RowsAtCompileTime)==i32(Expression::ColsAtCompileTime))
            &&  (i32(PlainObjectType::ColsAtCompileTime)==Eigen::Dynamic
              || i32(Expression::RowsAtCompileTime)==Eigen::Dynamic
              || i32(PlainObjectType::ColsAtCompileTime)==i32(Expression::RowsAtCompileTime)))),
...
/** \class Ref
  ...
  */
template<typename PlainObjectType, i32 Options, typename StrideType> class Ref
...
template<typename TPlainObjectType, i32 Options, typename StrideType> class Ref<const TPlainObjectType, Options, StrideType>
...
//      std::cout << i32(StrideType::OuterStrideAtCompileTime) << " - " << i32(Derived::OuterStrideAtCompileTime) << "\n";
//      std::cout << i32(StrideType::InnerStrideAtCompileTime) << " - " << i32(Derived::InnerStrideAtCompileTime) << "\n";
