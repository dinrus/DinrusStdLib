...
void stable_norm_impl_inner_step(const VectorType &vec, RealScalar& ssq, RealScalar& scale, RealScalar& invScale)
{
  ...
  enum {
    CanAlign = (   (i32(VectorTypeCopyClean::Flags)&DirectAccessBit)
                || (i32(internal::evaluator<VectorTypeCopyClean>::Alignment)>0) // FIXME Alignment)>0 might not be enough
...
blueNorm_impl(const EigenBase<Derived>& _vec)
{
  ...
  static i32k ibeta = std::numeric_limits<RealScalar>::radix;  // base for floating-point numbers
  static i32k it    = NumTraits<RealScalar>::digits();  // number of base-beta digits in mantissa
  static i32k iemin = NumTraits<RealScalar>::min_exponent();  // minimum exponent
  static i32k iemax = NumTraits<RealScalar>::max_exponent();  // maximum exponent
