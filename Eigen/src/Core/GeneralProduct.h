...
template<i32 Rows, i32 Cols, i32 Depth> struct product_type_selector;

template<i32 Size, i32 MaxSize> struct product_size_category
{
  enum {
...
template<i32 M, i32 N>  struct product_type_selector<M,N,1>              { enum { ret = OuterProduct }; };
template<i32 M>         struct product_type_selector<M, 1, 1>            { enum { ret = LazyCoeffBasedProductMode }; };
template<i32 N>         struct product_type_selector<1, N, 1>            { enum { ret = LazyCoeffBasedProductMode }; };
template<i32 Depth>     struct product_type_selector<1,    1,    Depth>  { enum { ret = InnerProduct }; };
...
template<i32 Side, i32 StorageOrder, bool BlasCompatible>
...
template<typename Scalar,i32 Size,i32 MaxSize,bool Cond> struct gemv_static_vector_if;

template<typename Scalar,i32 Size,i32 MaxSize>
struct gemv_static_vector_if<Scalar,Size,MaxSize,false>
{
  EIGEN_STRONG_INLINE EIGEN_DEVICE_FUNC Scalar* data() { eigen_internal_assert(false && "should never be called"); return 0; }
};

template<typename Scalar,i32 Size>
...
template<i32 StorageOrder, bool BlasCompatible>
...
MatrixBase<Derived>::operator*(const MatrixBase<OtherDerived> &other) const
{
  ...
  enum {
    ProductIsValid =  Derived::ColsAtCompileTime==Dynamic
                   ...
                   || i32(Derived::ColsAtCompileTime)==i32(OtherDerived::RowsAtCompileTime),
