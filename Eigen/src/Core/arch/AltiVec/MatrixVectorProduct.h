...
#define GEMV_BUILDPAIR_MMA(dst, src1, src2) \
  __builtin_vsx_assemble_pair(&dst, (__vector u8)src2, (__vector u8)src1)
#else
#if (__GNUC__ <= 10)
#if (__GNUC_MINOR__ > 3)
#define GEMV_BUILDPAIR_MMA(dst, src1, src2) \
...
#else
#define GEMV_BUILDPAIR_MMA(dst, src1, src2) \
  __builtin_vsx_assemble_pair(&dst, (__vector u8)src1, (__vector u8)src2)
#endif
#else
#define GEMV_BUILDPAIR_MMA(dst, src1, src2) \
  __builtin_vsx_build_pair(&dst, (__vector u8)src1, (__vector u8)src2)
#endif
#endif
...
EIGEN_ALWAYS_INLINE void pger_vecMMA_acc(__vector_quad* acc, const RhsPacket& a, const LhsPacket& b)
{
    if (accumulate)
    {
        __builtin_mma_xvf32gerpp(acc, (__vector u8)a, (__vector u8)b);
    }
    else
    {
        __builtin_mma_xvf32ger(acc, (__vector u8)a, (__vector u8)b);
    }
}
...
EIGEN_ALWAYS_INLINE void pger_vecMMA_acc(__vector_quad* acc, __vector_pair& a, const LhsPacket& b)
{
    if (accumulate)
    {
        __builtin_mma_xvf64gerpp(acc, a, (__vector u8)b);
    }
    else
    {
        __builtin_mma_xvf64ger(acc, a, (__vector u8)b);
    }
}
#endif
...
EIGEN_ALWAYS_INLINE Scalar pset1_realimag(ResScalar& alpha, i32 which, i32 conj)
{
    return (which) ? ((conj) ? -alpha.real() : alpha.real()) : ((conj) ? -alpha.imag() : alpha.imag());
}

/** \internal set a vector from complex location */
template<typename Scalar, typename ResScalar, typename ResPacket, i32 which>
...
template<typename LhsPacket, typename RhsScalar, typename RhsPacket, typename PResPacket, bool ConjugateLhs, bool ConjugateRhs, i32 StorageOrder>
...
template<typename ScalarPacket, typename LhsPacket, typename RhsScalar, typename RhsPacket, typename PResPacket, typename ResPacket, bool ConjugateLhs, bool ConjugateRhs, i32 StorageOrder>
...
EIGEN_ALWAYS_INLINE void pger_vecMMA(__vector_quad* acc, RhsPacket& a, LhsPacket& b)
{
    ...
    {
        __builtin_mma_xvf32gernp(acc, (__vector u8)a, (__vector u8)b);
    }
    else {
        __builtin_mma_xvf32gerpp(acc, (__vector u8)a, (__vector u8)b);
    }
...
EIGEN_ALWAYS_INLINE void pger_vecMMA(__vector_quad* acc, __vector_pair& a, Packet2d& b)
{
    if (NegativeAccumulate)
    {
        __builtin_mma_xvf64gernp(acc, (__vector_pair)a, (__vector u8)b);
    }
    else {
        __builtin_mma_xvf64gerpp(acc, (__vector_pair)a, (__vector u8)b);
    }
}
...
template<typename RealPacket, typename LhsPacket, bool Conjugate, i32 StorageOrder>
...
template<typename ScalarPacket, typename LhsPacket, typename SLhsPacket, typename RhsScalar, typename ResPacket, bool ConjugateLhs, bool ConjugateRhs, i32 StorageOrder>
...
template<typename ScalarPacket, typename LhsScalar, typename LhsPacket, typename SLhsPacket, typename RhsScalar, typename RhsPacket, typename ResPacket, bool ConjugateLhs, bool ConjugateRhs, i32 StorageOrder> \
...
template <typename Scalar, typename ScalarPacket, i32 ResPacketSize, typename LhsPacket, typename RhsPacket, bool ConjugateLhs, bool ConjugateRhs>
...
template <typename Scalar, i32 N> struct ScalarBlock {
    Scalar scalar[N];
};
...
template<typename Index, typename LhsMapper, bool ConjugateLhs, typename RhsMapper, bool ConjugateRhs, i32 Version> \
