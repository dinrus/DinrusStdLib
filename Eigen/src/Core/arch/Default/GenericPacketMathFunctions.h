...
Packet pfrexp_generic_get_biased_exponent(const Packet& a) {
  ...
  static constexpr i32 mantissa_bits = numext::numeric_limits<Scalar>::digits - 1;
...
Packet pfrexp_generic(const Packet& a, Packet& exponent) {
  ...
  // To handle denormals, normalize by multiplying by 2^(i32(MantissaBits)+1).
  const Packet is_denormal = pcmp_lt(pabs(a), normal_min);
  EIGEN_CONSTEXPR ScalarUI scalar_normalization_offset = ScalarUI(MantissaBits + 1); // 24
  // The following cannot be constexpr because bfloat16(uint16_t) is not constexpr.
  const Scalar scalar_normalization_factor = Scalar(ScalarUI(1) << i32(scalar_normalization_offset)); // 2^24
...
Packet psincos_float(const Packet& _x)
{
  ...
  {
    i32k PacketSize = unpacket_traits<Packet>::size;
...
void veltkamp_splitting(const Packet& x, Packet& x_hi, Packet& x_lo) {
  typedef typename unpacket_traits<Packet>::type Scalar;
  EIGEN_CONSTEXPR i32 shift = (NumTraits<Scalar>::digits() + 1) / 2;
