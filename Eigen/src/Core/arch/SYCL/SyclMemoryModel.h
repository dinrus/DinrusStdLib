 ...
 * Given a size, creates a ubyte-typed buffer and returns a
...
struct RangeAccess {
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE self_t operator++(i32 i) {
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE ref_t operator[](i32 x) {
    return *(get_pointer() + x);
  }

  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE ref_t operator[](i32 x) const {
    return *(get_pointer() + x);
  }
