...
template <typename Scalar, i32 SVEVectorLength>
...
EIGEN_STRONG_INLINE PacketXi plset<PacketXi>(const numext::int32_t& a)
{
  ...
  for (i32 i = 0; i < packet_traits<numext::int32_t>::size; i++) c[i] = i;
...
EIGEN_DEVICE_FUNC inline void ptranspose(PacketBlock<PacketXi, N>& kernel) {
  i32 buffer[packet_traits<numext::int32_t>::size * N] = {0};
  i32 i = 0;
...
EIGEN_STRONG_INLINE PacketXf plset<PacketXf>(const float& a)
{
  ...
  for (i32 i = 0; i < packet_traits<float>::size; i++) c[i] = i;
