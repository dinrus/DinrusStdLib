...
class gemm_class {
  ...
  static constexpr i32 a_regs[] = {0, 1, 2, use_less_a_regs ? 0 : 3, use_less_a_regs ? 1 : 4, use_less_a_regs ? 2 : 5};
  static constexpr i32 b_regs[] = {6, use_less_b_regs ? 6 : 7};
  static constexpr i32 c_regs[] = {
      8, 16, 24, 9, 17, 25, 10, 18, 26, 11, 19, 27, 12, 20, 28, 13, 21, 29, 14, 22, 30, 15, 23, 31,
  };

  static constexpr i32 alpha_load_reg = 0;
  static constexpr i32 c_load_regs[] = {1, 2, 6};

  static constexpr i32 a_shift = 128;
  static constexpr i32 b_shift = 128;

  static constexpr i32 nelems_in_cache_line = is_f32 ? 16 : 8;
  static constexpr i32 a_prefetch_size = nelems_in_cache_line * 2;
  static constexpr i32 b_prefetch_size = nelems_in_cache_line * 8;
  ...
  static EIGEN_ALWAYS_INLINE constexpr i32 div_up(i32 a, i32 b) { return a == 0 ? 0 : (a - 1) / b + 1; }
  ...
  template <i32 j, i32 endX, i32 i, i32 endY, i32 nelems>
  ...
  template <i32 un, i32 max_b_unroll, i32 i, i32 um_vecs, i32 a_unroll, i32 b_unroll>
   ...
   * for (i32 un = 0; un < 8; un++) {
   *     if (b_unroll >= un + 1) {
   *         if (un == 4) co2 = co1 + 4 * ldc;
   *
   *         for (i32 i = 0; i < um_vecs; i++) {
  ...
  // load_c
  template <i32 i, i32 um_vecs, i32 idx, i32 nelems>
   ...
   *
   *              i32k um_vecs = div_up(a_unroll, nelems_in_cache_line);
  ...
  template <i32 pow, i32 a_unroll, i32 idx>
  ...
  template <i32 max_b_unroll, i32 a_unroll, i32 b_unroll>
  ...
  // compute
  template <i32 um, i32 um_vecs, i32 idx, i32 uk, bool fetch_x, bool ktail>
  EIGEN_ALWAYS_INLINE std::enable_if_t<(um > um_vecs)> compute(const Scalar *ao, const Scalar *bo, i32 &fetchA_idx,
                                                               i32 &fetchB_idx, vec &b_reg) {
    EIGEN_UNUSED_VARIABLE(ao);
  ...
  // load_a
  template <i32 um, i32 um_vecs, i32 uk, i32 nelems, bool ktail>
  ...
  template <i32 uk, i32 pow, i32 count, i32 um_vecs, i32 b_unroll, bool ktail, bool fetch_x, bool c_fetch>
  ...
  template <i32 uk, i32 max_b_unroll, i32 a_unroll, i32 b_unroll, bool ktail, bool fetch_x, bool c_fetch>
  EIGEN_ALWAYS_INLINE void innerkernel_1uk(const Scalar *&aa, const Scalar *const &ao, const Scalar *const &bo,
                                           Scalar *&co2, i32 &fetchA_idx, i32 &fetchB_idx) {
    i32k um_vecs = div_up(a_unroll, nelems_in_cache_line);
   ...
   *  for (i32 uk = 0; uk < kfactor; uk++) {
   *      i32 idx = 0;
   *
   *      for (pow = 1; pow < max_b_unroll << 1; pow <<= 1) {
   *          for (i32 count = 0; count < (pow + 1) / 2; count++) {
   *              auto &b_reg = zmm[b_regs[idx % 2]];
   *
   *              if (fetch_x && uk == 3 && idx == 0) prefetch_x(aa);
   *              if (fetch_x && uk == 3 && idx == 4) aa += 8;
  ...
  template <i32 a_unroll, i32 b_unroll, i32 k_factor, i32 max_b_unroll, i32 max_k_factor, bool c_fetch>
  ...
  template <i32 a_unroll, i32 b_unroll, i32 max_b_unroll>
  ...
  template <i32 a_unroll, i32 max_a_unroll, i32 max_b_unroll>
 ...
 public:
  ...
  template <i32 max_a_unroll, i32 max_b_unroll>
...
//     max_b_unroll: 8, 4, 2, 1
template <typename Scalar, i32 max_a_unroll, i32 max_b_unroll, bool is_alpha1, bool is_beta0, bool is_unit_inc>
...
template <bool ConjLhs_, bool ConjRhs_, i32 PacketSize_>
...
struct gemm_pack_rhs<Scalar, Index, DataMapper, 8, RowMajor, Conjugate, PanelMode> {
  ...
  EIGEN_DONT_INLINE void operator()(Scalar *blockB, const DataMapper &rhs, Index depth, Index cols, Index stride = 0,
    ...
    EIGEN_UNUSED_VARIABLE(offset);
    eigen_assert(((!PanelMode) && stride == 0 && offset == 0) || (PanelMode && stride >= depth && offset <= stride));
    const bool HasHalf = (i32)HalfPacketSize < (i32)PacketSize;
    const bool HasQuarter = (i32)QuarterPacketSize < (i32)HalfPacketSize;
    conj_if<NumTraits<Scalar>::IsComplex && Conjugate> cj;
    Index packet_cols8 = nr >= 8 ? (cols / 8) * 8 : 0;
    Index packet_cols4 = nr >= 4 ? (cols / 4) * 4 : 0;
    Index count = 0;
...
template <typename Scalar, typename Index, typename DataMapper, i32 mr, bool ConjugateLhs, bool ConjugateRhs>
