...
namespace internal {
template<typename MatrixType, i32 DiagIndex>
 ...
 : traits<MatrixType>
{
  ...
  enum {
    RowsAtCompileTime = (i32(DiagIndex) == DynamicIndex || i32(MatrixType::SizeAtCompileTime) == Dynamic) ? Dynamic
    ...
    ColsAtCompileTime = 1,
    MaxRowsAtCompileTime = i32(MatrixType::MaxSizeAtCompileTime) == Dynamic ? Dynamic
    ...
    Flags = (u32)MatrixTypeNested_::Flags & (RowMajorBit | MaskLvalueBit | DirectAccessBit) & ~RowMajorBit, // FIXME DirectAccessBit should not be handled by expressions
...
template<typename MatrixType, i32 DiagIndex_> class Diagonal
   : public internal::dense_xpr_base< Diagonal<MatrixType,DiagIndex_> >::type
{
  ...
  private:
    ...
    template<i32 LoadMode> typename MatrixType::PacketReturnType packet(Index) const;
    template<i32 LoadMode> typename MatrixType::PacketReturnType packet(Index,Index) const;
};
...
/** This is the const version of diagonal<i32>(). */
template<typename Derived>
template<i32 Index_>
EIGEN_DEVICE_FUNC
