...
/** \internal If the template parameter Value is Dynamic, this class is just a wrapper around a T variable that
  ...
  */
template<typename T, i32 Value> class variable_if_dynamic
{
  public:
...
/** \internal like variable_if_dynamic but for DynamicIndex
  */
template<typename T, i32 Value> class variable_if_dynamicindex
{
  public:
...
template<i32 Size, typename PacketType,
...
constexpr inline i32 compute_default_alignment_helper(i32 ArrayBytes, i32 AlignmentBytes) {
...
template<typename T, i32 Size> struct compute_default_alignment {
...
template<typename Scalar_, i32 Rows_, i32 Cols_,
         i32 Options_ = AutoAlign |
...
constexpr inline unsigned compute_matrix_flags(i32 Options) {
...
constexpr inline i32 size_at_compile_time(i32 rows, i32 cols) {
...
template<typename T, typename BaseClassType, i32 Flags> struct plain_matrix_type_dense;
...
template<typename T, i32 Flags> struct plain_matrix_type_dense<T,MatrixXpr,Flags>
{
...
template<typename T, i32 Flags> struct plain_matrix_type_dense<T,ArrayXpr,Flags>
{
...
template<typename Scalar_, i32 Rows_, i32 Cols_, i32 Options_, i32 MaxRows_, i32 MaxCols_>
...
/** \internal Determines how a given expression should be nested when evaluated multiple times.
  ...
  */
template<typename T, i32 n, typename PlainObject = typename plain_object_eval<T>::type> struct nested_eval
{
  enum {
    ...
    Evaluate = (i32(evaluator<T>::Flags) & EvalBeforeNestingBit) || (i32(CostEval) < i32(CostNoEval))
  };
...
template <typename LhsKind, typename RhsKind, i32 LhsOrder, i32 RhsOrder> struct cwise_promote_storage_order {
  enum { value = LhsOrder };
};

template <typename LhsKind, i32 LhsOrder, i32 RhsOrder>   struct cwise_promote_storage_order<LhsKind,Sparse,LhsOrder,RhsOrder>                { enum { value = RhsOrder }; };
template <typename RhsKind, i32 LhsOrder, i32 RhsOrder>   struct cwise_promote_storage_order<Sparse,RhsKind,LhsOrder,RhsOrder>                { enum { value = LhsOrder }; };
template <i32 Order>                                      struct cwise_promote_storage_order<Sparse,Sparse,Order,Order>                       { enum { value = Order }; };
...
/** \internal Specify the "storage kind" of multiplying an expression of kind A with kind B.
  ...
  * \endcode
  */
template <typename A, typename B, i32 ProductTag> struct product_promote_storage_type;

template <typename A, i32 ProductTag> struct product_promote_storage_type<A,                  A,                  ProductTag> { typedef A     ret;};
template <i32 ProductTag>             struct product_promote_storage_type<Dense,              Dense,              ProductTag> { typedef Dense ret;};
template <typename A, i32 ProductTag> struct product_promote_storage_type<A,                  Dense,              ProductTag> { typedef Dense ret; };
template <typename B, i32 ProductTag> struct product_promote_storage_type<Dense,              B,                  ProductTag> { typedef Dense ret; };

template <typename A, i32 ProductTag> struct product_promote_storage_type<A,                  DiagonalShape,      ProductTag> { typedef A ret; };
template <typename B, i32 ProductTag> struct product_promote_storage_type<DiagonalShape,      B,                  ProductTag> { typedef B ret; };
template <i32 ProductTag>             struct product_promote_storage_type<Dense,              DiagonalShape,      ProductTag> { typedef Dense ret; };
template <i32 ProductTag>             struct product_promote_storage_type<DiagonalShape,      Dense,              ProductTag> { typedef Dense ret; };

template <typename A, i32 ProductTag> struct product_promote_storage_type<A,                  SkewSymmetricShape, ProductTag> { typedef A ret; };
template <typename B, i32 ProductTag> struct product_promote_storage_type<SkewSymmetricShape, B,                  ProductTag> { typedef B ret; };
template <i32 ProductTag>             struct product_promote_storage_type<Dense,              SkewSymmetricShape, ProductTag> { typedef Dense ret; };
template <i32 ProductTag>             struct product_promote_storage_type<SkewSymmetricShape, Dense,              ProductTag> { typedef Dense ret; };
template <i32 ProductTag>             struct product_promote_storage_type<SkewSymmetricShape, SkewSymmetricShape, ProductTag> { typedef Dense ret; };

template <typename A, i32 ProductTag> struct product_promote_storage_type<A,                  PermutationStorage, ProductTag> { typedef A ret; };
template <typename B, i32 ProductTag> struct product_promote_storage_type<PermutationStorage, B,                  ProductTag> { typedef B ret; };
template <i32 ProductTag>             struct product_promote_storage_type<Dense,              PermutationStorage, ProductTag> { typedef Dense ret; };
template <i32 ProductTag>             struct product_promote_storage_type<PermutationStorage, Dense,              ProductTag> { typedef Dense ret; };
...
struct plain_row_type
{
  typedef Matrix<Scalar, 1, ExpressionType::ColsAtCompileTime,
                 i32(ExpressionType::PlainObject::Options) | i32(RowMajor), 1, ExpressionType::MaxColsAtCompileTime> MatrixRowType;
  typedef Array<Scalar, 1, ExpressionType::ColsAtCompileTime,
                 i32(ExpressionType::PlainObject::Options) | i32(RowMajor), 1, ExpressionType::MaxColsAtCompileTime> ArrayRowType;
...
template<typename T, i32 S> struct is_diagonal<DiagonalMatrix<T,S> >
{ enum { ret = true }; };
...
std::string demangle_traversal(i32 t)
{
...
std::string demangle_unrolling(i32 t)
{
...
std::string demangle_flags(i32 f)
{
  std::string res;
