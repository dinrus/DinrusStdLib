...
// Define portable (u)i32{32,64} types
#include <cstdint>
...
template<> struct is_arithmetic<i8>   { enum { value = true }; };
template<> struct is_arithmetic<u8> { enum { value = true }; };
template<> struct is_arithmetic<signed short>  { enum { value = true }; };
template<> struct is_arithmetic<unsigned short>{ enum { value = true }; };
template<> struct is_arithmetic<signed i32>    { enum { value = true }; };
template<> struct is_arithmetic<u32>  { enum { value = true }; };
...
template<> struct is_arithmetic<signed z64>   { enum { value = true }; };
template<> struct is_arithmetic<unsigned z64> { enum { value = true }; };
...
template<typename T, i32 N> struct array_size<const T (&)[N]> {
  enum { value = N };
};
template<typename T, i32 N> struct array_size<T (&)[N]> {
  enum { value = N };
};
...
/** \internal In short, it computes i32(sqrt(\a Y)) with \a Y an integer.
...
class meta_sqrt
{
    enum {
      ...
      NewInf = i32(TakeInf) ? InfX : i32(MidX),
      NewSup = i32(TakeInf) ? i32(MidX) : SupX
    };
  public:
    enum { ret = meta_sqrt<Y,NewInf,NewSup>::ret };
};

template<i32 Y, i32 InfX, i32 SupX>
...
/** \internal Computes the least common multiple of two positive integer A and B
  * at compile-time. 
  */
template<i32 A, i32 B, i32 K=1, bool Done = ((A*K)%B)==0, bool Big=(A>=B)>
struct meta_least_common_multiple
{
  enum { ret = meta_least_common_multiple<A,B,K+1>::ret };
};
template<i32 A, i32 B, i32 K, bool Done>
struct meta_least_common_multiple<A,B,K,Done,false>
{
  enum { ret = meta_least_common_multiple<B,A,K>::ret };
};
template<i32 A, i32 B, i32 K>
...
struct aligned_storage {
  struct type {
    EIGEN_ALIGN_TO_BOUNDARY(Align) u8 data[Len];
  };
};
...
inline constexpr i32 plain_enum_min(A a, B b) {
  ...
  return ((i32) a <= (i32) b) ? (i32) a : (i32) b;
}

/// \internal Gets the maximum of two values which may be integers or enums
template<typename A, typename B>
inline constexpr i32 plain_enum_max(A a, B b) {
  static_assert(is_int_or_enum_v<A>, "Argument a must be an integer or enum");
  static_assert(is_int_or_enum_v<B>, "Argument b must be an integer or enum");
  return ((i32) a >= (i32) b) ? (i32) a : (i32) b;
}
 ...
 */
template<typename A, typename B>
inline constexpr i32 min_size_prefer_dynamic(A a, B b) {
  static_assert(is_int_or_enum_v<A>, "Argument a must be an integer or enum");
  static_assert(is_int_or_enum_v<B>, "Argument b must be an integer or enum");
  if ((i32) a == 0 || (i32) b == 0) return 0;
  if ((i32) a == 1 || (i32) b == 1) return 1;
  if ((i32) a == Dynamic || (i32) b == Dynamic) return Dynamic;
 ...
 */
template<typename A, typename B>
inline constexpr i32 min_size_prefer_fixed(A a, B b) {
  static_assert(is_int_or_enum_v<A>, "Argument a must be an integer or enum");
  static_assert(is_int_or_enum_v<B>, "Argument b must be an integer or enum");
  if ((i32) a == 0 || (i32) b == 0) return 0;
  if ((i32) a == 1 || (i32) b == 1) return 1;
  if ((i32) a == Dynamic && (i32) b == Dynamic) return Dynamic;
  if ((i32) a == Dynamic) return (i32) b;
  if ((i32) b == Dynamic) return (i32) a;
  return plain_enum_min(a, b);
}

/// \internal see `min_size_prefer_fixed`. No need for a separate variant for MaxSizes here.
template<typename A, typename B>
inline constexpr i32 max_size_prefer_dynamic(A a, B b) {
  static_assert(is_int_or_enum_v<A>, "Argument a must be an integer or enum");
  static_assert(is_int_or_enum_v<B>, "Argument b must be an integer or enum");
  if ((i32) a == Dynamic || (i32) b == Dynamic) return Dynamic;
