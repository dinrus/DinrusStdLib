...
template<typename T,i32 XprSize,typename EnableIf=void>
...
// Turn a single index into something that looks like an array (i.e., that exposes a .size(), and operator[](i32) methods)
template<typename T, i32 XprSize>
...
template<i32 Size> struct get_compile_time_incr<AllRange<Size> > {
  enum { value = 1 };
};
