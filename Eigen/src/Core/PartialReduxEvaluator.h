...
{
  enum {
    OuterSize = i32(Evaluator::IsRowMajor) ? Evaluator::RowsAtCompileTime : Evaluator::ColsAtCompileTime,
...
template<typename Func, typename Evaluator,
         i32 Unrolling = packetwise_redux_traits<Func, Evaluator>::Unrolling
>
...
template< typename ArgType, typename MemberOp, i32 Direction>
  ...
  : evaluator_base<PartialReduxExpr<ArgType, MemberOp, Direction> >
{
  ...
  enum {
    TraversalSize = Direction==i32(Vertical) ? i32(ArgType::RowsAtCompileTime) :  i32(ArgType::ColsAtCompileTime)
  };
  typedef typename MemberOp::template Cost<i32(TraversalSize)> CostOpType;
  enum {
    CoeffReadCost = TraversalSize==Dynamic ? HugeCost
                  : TraversalSize==0 ? 1
                  : i32(TraversalSize) * i32(evaluator<ArgType>::CoeffReadCost) + i32(CostOpType::value),
    
    ArgFlags_ = evaluator<ArgType>::Flags,

    Vectorizable_ =  bool(i32(ArgFlags_)&PacketAccessBit)
                  && bool(MemberOp::Vectorizable)
                  && (Direction==i32(Vertical) ? bool(ArgFlags_&RowMajorBit) : (ArgFlags_&RowMajorBit)==0)
                  && (TraversalSize!=0),
  ...
  {
    EIGEN_INTERNAL_CHECK_COST_VALUE(TraversalSize==Dynamic ? HugeCost : (TraversalSize==0 ? 1 : i32(CostOpType::value)));
  ...
  PacketType packet(Index idx) const
  {
    ...
    typedef Block<const ArgTypeNestedCleaned,
                  Direction==Vertical ? i32(ArgType::RowsAtCompileTime) : i32(PacketSize),
                  Direction==Vertical ? i32(PacketSize) : i32(ArgType::ColsAtCompileTime),
