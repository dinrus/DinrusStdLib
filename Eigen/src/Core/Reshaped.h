...
template<typename XprType, i32 Rows, i32 Cols, i32 Order>
struct traits<Reshaped<XprType, Rows, Cols, Order> > : traits<XprType>
{
  ...
  enum{
    ...
    XpxStorageOrder = ((i32(traits<XprType>::Flags) & RowMajorBit) == RowMajorBit) ? RowMajor : ColMajor,
    ReshapedStorageOrder = (RowsAtCompileTime == 1 && ColsAtCompileTime != 1) ? RowMajor
                         : (ColsAtCompileTime == 1 && RowsAtCompileTime != 1) ? ColMajor
                         : XpxStorageOrder,
    HasSameStorageOrderAsXprType = (ReshapedStorageOrder == XpxStorageOrder),
    InnerSize = (ReshapedStorageOrder==i32(RowMajor)) ? i32(ColsAtCompileTime) : i32(RowsAtCompileTime),
    InnerStrideAtCompileTime = HasSameStorageOrderAsXprType
                             ? i32(inner_stride_at_compile_time<XprType>::ret)
                             : Dynamic,
    OuterStrideAtCompileTime = Dynamic,

    HasDirectAccess = internal::has_direct_access<XprType>::ret
                    && (Order==i32(XpxStorageOrder))
                    && ((evaluator<XprType>::Flags&LinearAccessBit)==LinearAccessBit),

    MaskPacketAccessBit = (InnerSize == Dynamic || (InnerSize % packet_traits<Scalar>::size) == 0)
                       && (InnerStrideAtCompileTime == 1)
                        ? PacketAccessBit : 0,
    //MaskAlignedBit = ((OuterStrideAtCompileTime!=Dynamic) && (((OuterStrideAtCompileTime * i32(sizeof(Scalar))) % 16) == 0)) ? AlignedBit : 0,
    FlagsLinearAccessBit = (RowsAtCompileTime == 1 || ColsAtCompileTime == 1) ? LinearAccessBit : 0,
    FlagsLvalueBit = is_lvalue<XprType>::value ? LvalueBit : 0,
    FlagsRowMajorBit = (ReshapedStorageOrder==i32(RowMajor)) ? RowMajorBit : 0,
...
template<typename XprType, i32 Rows, i32 Cols, i32 Order, bool HasDirectAccess> class ReshapedImpl_dense;

} // end namespace internal

template<typename XprType, i32 Rows, i32 Cols, i32 Order, typename StorageKind> class ReshapedImpl;

template<typename XprType, i32 Rows, i32 Cols, i32 Order> class Reshaped
...
// Evaluators
template<typename ArgType, i32 Rows, i32 Cols, i32 Order, bool HasDirectAccess> struct reshaped_evaluator;

template<typename ArgType, i32 Rows, i32 Cols, i32 Order>
...
//     OuterStrideAtCompileTime = Dynamic,
    ...
    FlagsRowMajorBit = (traits<XprType>::ReshapedStorageOrder==i32(RowMajor)) ? RowMajorBit : 0,
    FlagsDirectAccessBit =  HasDirectAccess ? DirectAccessBit : 0,
