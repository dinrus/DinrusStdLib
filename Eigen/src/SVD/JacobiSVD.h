...
template <typename MatrixType, i32 Options, bool IsComplex = NumTraits<typename MatrixType::Scalar>::IsComplex>
...
template<typename MatrixType, i32 QRPreconditioner, i32 Case>
...
template <typename MatrixType, i32 Options, i32 QRPreconditioner, i32 Case,
...
template <typename MatrixType, i32 Options>
 ...
 */
template <typename MatrixType_, i32 Options_>
class JacobiSVD : public SVDBase<JacobiSVD<MatrixType_, Options_> > {
 ...
 public:
   ...
   */
  EIGEN_DEPRECATED
  JacobiSVD(Index rows, Index cols, u32 computationOptions) {
  ...
  JacobiSVD(const MatrixType& matrix, u32 computationOptions) {
   ...
   */
  EIGEN_DEPRECATED
  JacobiSVD& compute(const MatrixType& matrix, u32 computationOptions) {
 ...
 private:
  void allocate(Index rows, Index cols, u32 computationOptions);
  JacobiSVD& compute_impl(const MatrixType& matrix, u32 computationOptions);
 ...
 protected:
  ...
  EIGEN_STATIC_ASSERT(!(ShouldComputeThinU && i32(QRPreconditioner) == i32(FullPivHouseholderQRPreconditioner)) &&
                          !(ShouldComputeThinU && i32(QRPreconditioner) == i32(FullPivHouseholderQRPreconditioner)),
                      "JacobiSVD: can't compute thin U or thin V with the FullPivHouseholderQR preconditioner. "
                      "Use the ColPivHouseholderQR preconditioner instead.")

  template <typename MatrixType__, i32 Options__, bool IsComplex_>
  friend struct internal::svd_precondition_2x2_block_to_be_real;
  template <typename MatrixType__, i32 Options__, i32 QRPreconditioner_, i32 Case_, bool DoAnything_>
...
template <typename MatrixType, i32 Options>
void JacobiSVD<MatrixType, Options>::allocate(Index rows, Index cols, u32 computationOptions) {
  if (Base::allocate(rows, cols, computationOptions)) return;

  eigen_assert(!(ShouldComputeThinU && i32(QRPreconditioner) == i32(FullPivHouseholderQRPreconditioner)) &&
               !(ShouldComputeThinU && i32(QRPreconditioner) == i32(FullPivHouseholderQRPreconditioner)) &&
               "JacobiSVD: can't compute thin U or thin V with the FullPivHouseholderQR preconditioner. "
               "Use the ColPivHouseholderQR preconditioner instead.");
