...
constexpr i32 get_qr_preconditioner(i32 options) { return options & QRPreconditionerBits; }

constexpr i32 get_computation_options(i32 options) { return options & ComputationOptionsBits; }

constexpr bool should_svd_compute_thin_u(i32 options) { return (options & ComputeThinU) != 0; }
constexpr bool should_svd_compute_full_u(i32 options) { return (options & ComputeFullU) != 0; }
constexpr bool should_svd_compute_thin_v(i32 options) { return (options & ComputeThinV) != 0; }
constexpr bool should_svd_compute_full_v(i32 options) { return (options & ComputeFullV) != 0; }

template<typename MatrixType, i32 Options>
void check_svd_options_assertions(u32 computationOptions, Index rows, Index cols) {
 ...
 : traits<Derived>
{
  ...
  typedef i32 StorageIndex;
  enum { Flags = 0 };
};

template <typename MatrixType, i32 Options_>
struct svd_traits : traits<MatrixType> {
  static constexpr i32 Options = Options_;
...
protected:
  ...
  bool allocate(Index rows, Index cols, u32 computationOptions);
...
template <typename Derived>
bool SVDBase<Derived>::allocate(Index rows, Index cols, u32 computationOptions) {
