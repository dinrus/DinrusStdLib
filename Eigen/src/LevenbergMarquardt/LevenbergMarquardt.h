...
template <typename Scalar_, i32 NX=Dynamic, i32 NY=Dynamic>
struct DenseFunctor
{
  ...
  i32k m_inputs, m_values;

  DenseFunctor() : m_inputs(InputsAtCompileTime), m_values(ValuesAtCompileTime) {}
  DenseFunctor(i32 inputs, i32 values) : m_inputs(inputs), m_values(values) {}

  i32 inputs() const { return m_inputs; }
  i32 values() const { return m_values; }

  //i32 operator()(const InputType &x, ValueType& fvec) { }
  // should be defined in derived classes
  
  //i32 df(const InputType &x, JacobianType& fjac) { }
  // should be defined in derived classes
};
...
struct SparseFunctor
{
  ...
  typedef SparseQR<JacobianType, COLAMDOrdering<i32> > QRSolver;
  enum {
    InputsAtCompileTime = Dynamic,
    ValuesAtCompileTime = Dynamic
  };
  
  SparseFunctor(i32 inputs, i32 values) : m_inputs(inputs), m_values(values) {}
...
class LevenbergMarquardt : internal::no_assignment_operator
{
  public:
    ...
    typedef PermutationMatrix<Dynamic,Dynamic,i32> PermutationType;
  public:
