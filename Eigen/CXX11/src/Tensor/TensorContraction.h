...
struct traits<TensorContractionOp<Dimensions, LhsXprType, RhsXprType, OutputKernelType> >
{
  ...
  // From NumDims below.
  static constexpr i32 NumDimensions = traits<LhsXprType>::NumDimensions + traits<RhsXprType>::NumDimensions - 2 * array_size<Dimensions>::value;
  static constexpr i32 Layout = traits<LhsXprType>::Layout;
...
struct traits<TensorEvaluator<const TensorContractionOp<Indices_, LeftArgType_, RightArgType_, OutputKernelType_>, Device_> > {
  ...
  // From NumDims below.
  static constexpr i32 NumDimensions = traits<LeftArgType_>::NumDimensions + traits<RightArgType_>::NumDimensions - 2 * array_size<Indices_>::value;
};
...
struct TensorContractionKernel {
  ...
  EIGEN_DEVICE_FUNC EIGEN_DONT_INLINE void invoke(
    ...
    static i32k kComputeStrideFromBlockDimensions = -1;
...
struct TensorContractionEvaluatorBase {
  ...
  TensorContractionEvaluatorBase(const XprType& op, const Device& device)
      : m_leftImpl(choose(Cond<static_cast<i32>(Layout) == static_cast<i32>(ColMajor)>(),
                          op.lhsExpression(), op.rhsExpression()), device),
        m_rightImpl(choose(Cond<static_cast<i32>(Layout) == static_cast<i32>(ColMajor)>(),
                           op.rhsExpression(), op.lhsExpression()), device),
        m_device(device),
        m_output_kernel(op.outputKernel()),
        m_result(NULL) {
    EIGEN_STATIC_ASSERT((static_cast<i32>(TensorEvaluator<LeftArgType, Device>::Layout) ==
         static_cast<i32>(TensorEvaluator<RightArgType, Device>::Layout)),
    ...
    if (static_cast<i32>(Layout) == static_cast<i32>(ColMajor)) {
      ...
      for (i32 i = 0; i < LDims; i++) {
        eval_left_dims[i] = m_leftImpl.dimensions()[i];
      }
      for (i32 i = 0; i < RDims; i++) {
        eval_right_dims[i] = m_rightImpl.dimensions()[i];
      }
      // We keep the pairs of contracting indices.
      for (i32 i = 0; i < ContractDims; i++) {
    ...
    for (i32 i = 0; i < ContractDims; i++) {
      for (i32 j = i + 1; j < ContractDims; j++) {
    ...
    for (i32 i = 0; i < ContractDims; i++) {
      ...
      if (i+1 < static_cast<i32>(internal::array_size<contract_t>::value)) {
    ...
    if (static_cast<i32>(Layout) == static_cast<i32>(RowMajor)) {
      for (i32 i = 0, j = NumDims - 1; i < j; i++, j--) {
    ...
    m_tensor_contraction_params.swapped_arguments = static_cast<i32>(Layout) == RowMajor;
  }
...
#define TENSOR_CONTRACTION_ASYNC_DISPATCH(METHOD, DONE, ALIGNMENT, ARGS, FN) \
  ...
  template <bool lhs_inner_dim_contiguous, bool rhs_inner_dim_contiguous, bool rhs_inner_dim_reordered, i32 Alignment>
  ...
  void evalGemv(Scalar* buffer) const {
    ...
    i32k lhs_alignment = LeftEvaluator::IsAligned ? Aligned : Unaligned;
    i32k rhs_alignment = RightEvaluator::IsAligned ? Aligned : Unaligned;
  ...
  EIGEN_DEVICE_FUNC void evalGemmPartialWithoutOutputKernel(
      Scalar* buffer, Index k_start, Index k_end, i32 num_threads) const {
  ...
  template <bool lhs_inner_dim_contiguous, bool rhs_inner_dim_contiguous, bool rhs_inner_dim_reordered, i32 Alignment, bool use_output_kernel>
  EIGEN_DEVICE_FUNC void evalGemmPartial(Scalar* buffer, Index k_start, Index k_end, i32 num_threads) const {
...
struct TensorEvaluator<const TensorContractionOp<Indices, LeftArgType, RightArgType, OutputKernelType>, Device> :
  ...
  // If we want to compute A * B = C, where A is LHS and B is RHS, the code
  // will pretend B is LHS and A is RHS.
  typedef std::conditional_t<Layout == static_cast<i32>(ColMajor), LeftArgType, RightArgType> EvalLeftArgType;
  typedef std::conditional_t<Layout == static_cast<i32>(ColMajor), RightArgType, LeftArgType> EvalRightArgType;
