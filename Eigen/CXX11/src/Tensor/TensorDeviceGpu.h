...
static i32k kGpuScratchSize = 1024;
...
class GpuDeviceProperties {
 public:
  ...
  EIGEN_STRONG_INLINE const gpuDeviceProp_t& get(i32 device) const {
...
EIGEN_ALWAYS_INLINE const gpuDeviceProp_t& GetGpuDeviceProperties(i32 device) {
...
class GpuStreamDevice : public StreamInterface {
 public:
  ...
  GpuStreamDevice(i32 device) : stream_(&default_stream), device_(device), scratch_(NULL), semaphore_(NULL) {}
  // Use the specified stream. Note that it's the
  // caller responsibility to ensure that the stream can run on
  // the specified device. If no device is specified the code
  // assumes that the stream is associated to the current gpu device.
  GpuStreamDevice(const gpuStream_t* stream, i32 device = -1)
  ...
  virtual void* scratchpad() const {
    if (scratch_ == NULL) {
      scratch_ = allocate(kGpuScratchSize + sizeof(u32));
    }
    return scratch_;
  }

  virtual u32* semaphore() const {
    if (semaphore_ == NULL) {
      char* scratch = static_cast<char*>(scratchpad()) + kGpuScratchSize;
      semaphore_ = reinterpret_cast<u32*>(scratch);
      gpuError_t err = gpuMemsetAsync(semaphore_, 0, sizeof(u32), *stream_);
      EIGEN_UNUSED_VARIABLE(err)
      gpu_assert(err == gpuSuccess);
    }
    return semaphore_;
...
struct GpuDevice {
  ...
  explicit GpuDevice(const StreamInterface* stream, i32 num_blocks) : stream_(stream), max_blocks_(num_blocks) {
    eigen_assert(stream);
  }
  ...
  EIGEN_STRONG_INLINE u32* semaphore() const {
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE void memset(void* buffer, i32 c, size_t n) const {
#ifndef EIGEN_GPU_COMPILE_PHASE
  ...
  EIGEN_STRONG_INLINE i32 getNumGpuMultiProcessors() const {
    return stream_->deviceProperties().multiProcessorCount;
  }
  EIGEN_STRONG_INLINE i32 maxGpuThreadsPerBlock() const {
    return stream_->deviceProperties().maxThreadsPerBlock;
  }
  EIGEN_STRONG_INLINE i32 maxGpuThreadsPerMultiProcessor() const {
    return stream_->deviceProperties().maxThreadsPerMultiProcessor;
  }
  EIGEN_STRONG_INLINE i32 sharedMemPerBlock() const {
    return static_cast<i32>(stream_->deviceProperties().sharedMemPerBlock);
  }
  EIGEN_STRONG_INLINE i32 majorDeviceVersion() const {
    return stream_->deviceProperties().major;
  }
  EIGEN_STRONG_INLINE i32 minorDeviceVersion() const {
    return stream_->deviceProperties().minor;
  }

  EIGEN_STRONG_INLINE i32 maxBlocks() const {
    return max_blocks_;
  }
