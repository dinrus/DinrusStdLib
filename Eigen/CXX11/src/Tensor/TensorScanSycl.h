...
struct ScanKernelFunctor {
  typedef cl::sycl::accessor<CoeffReturnType, 1, cl::sycl::access::mode::read_write, cl::sycl::access::target::local>
      LocalAccessor;
  static EIGEN_CONSTEXPR i32 PacketSize = ScanParameters<Index>::ScanPerThread / 2;
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE void operator()(cl::sycl::nd_item<1> itemID) const {
    ...
    for (Index loop_offset = 0; loop_offset < scanParameters.loop_range; loop_offset++) {
      ...
      EIGEN_UNROLL_LOOP
      for (i32 i = 0; i < ScanParameters<Index>::ScanPerThread; i++) {
      ...
      EIGEN_UNROLL_LOOP
      for (i32 packetIndex = 0; packetIndex < ScanParameters<Index>::ScanPerThread; packetIndex += PacketSize) {
...
struct ScanLauncher<Self, Reducer, Eigen::SyclDevice, vectorize> {
  ...
  void operator()(Self &self, EvaluatorPointerType data) const {
    ...
    Index panel_size = 1;
    if (static_cast<i32>(Self::Layout) == static_cast<i32>(ColMajor)) {
      for (i32 i = 0; i < consume_dim; i++) {
        non_scan_size *= dims[i];
      }
      for (i32 i = consume_dim + 1; i < Self::NumDims; i++) {
        panel_size *= dims[i];
      }
    } else {
      for (i32 i = Self::NumDims - 1; i > consume_dim; i--) {
        non_scan_size *= dims[i];
      }
      for (i32 i = consume_dim - 1; i >= 0; i--) {
        panel_size *= dims[i];
      }
    }
