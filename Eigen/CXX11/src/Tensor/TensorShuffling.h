...
struct TensorEvaluator<const TensorShufflingOp<Shuffle, ArgType>, Device>
{
        ...
        m_impl(op.expression(), device)
  {
    ...
    for (i32 i = 0; i < NumDims; ++i) {
      m_shuffle[i] = static_cast<i32>(shuffle[i]);
  ...
  template <i32 LoadMode, typename Self, bool ImplPacketAccess>
  struct PacketLoader {
  ...
  template<i32 LoadMode, typename Self>
  ...
  internal::TensorBlockResourceRequirements getResourceRequirements() const {
    static i32k inner_dim =
        Layout == static_cast<i32>(ColMajor) ? 0 : NumDims - 1;
 ...
 protected:
  ...
  bool m_is_identity;
  array<i32, NumDims> m_shuffle;
  array<Index, NumDims> m_inverseShuffle;  // TODO(ezhulenev): Make it i32 type.
    ...
    : public TensorEvaluator<const TensorShufflingOp<Shuffle, ArgType>, Device>
{
  ...
  template <i32 StoreMode> EIGEN_STRONG_INLINE
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE void writeBlock(
    ...
    for (i32 i = 0; i < NumDims; ++i) {
      dst_to_src_dim_map[i] = static_cast<i32>(this->m_inverseShuffle[i]);
    }
    TensorBlockIO::Copy(dst, src, dst_to_src_dim_map);
