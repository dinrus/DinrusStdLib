...
template <i32 ResultType> struct PartOf {
...
namespace internal {
template <typename FFT, typename XprType, i32 FFTResultType, i32 FFTDir>
...
template <typename FFT, typename XprType, i32 FFTResultType, i32 FFTDirection>
...
// Eval as rvalue
template <typename FFT, typename ArgType, typename Device, i32 FFTResultType, i32 FFTDir>
    ...
    for (size_t i = 0; i < m_fft.size(); ++i) {
      ...
      for (Index partial_index = 0; partial_index < m_size / line_len; ++partial_index) {
        ...
        if (stride == 1) {
          ...
          for (i32 j = 0; j < line_len; ++j, offset += stride) {
...
#define SQRT2DIV2 0.7071067811865476
 ...
 template <i32 Dir>
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE Index getBaseOffsetFromIndex(Index index, Index omitted_dim) const {
    ...
    if (static_cast<i32>(Layout) == static_cast<i32>(ColMajor)) {
      for (i32 i = NumDims - 1; i > omitted_dim; --i) {
