...
struct traits<TensorBroadcastingOp<Broadcast, XprType> > : public traits<XprType>
{
  ...
  static constexpr i32 NumDimensions = XprTraits::NumDimensions;
...
struct TensorEvaluator<const TensorBroadcastingOp<Broadcast, ArgType>, Device>
{
  ...
  static constexpr i32 NumDims = internal::array_size<typename TensorEvaluator<ArgType, Device>::Dimensions>::value;
  ...
  (Index index) const
  {
    ...
    if (outputOffset + PacketSize <= m_outputStrides[endDim]) {
      ...
      EIGEN_UNROLL_LOOP
      for (i32 i = 0, cur = 0; i < PacketSize; ++i, ++cur) {
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE PacketReturnType packetOneByN(Index index) const
  {
    ...
    const Index M = (static_cast<i32>(Layout) == static_cast<i32>(ColMajor)) ?
  ...
  block(TensorBlockDesc& desc, TensorBlockScratch& scratch,
    ...
    for (i32 i = params.inner_dim_count + 1; i < NumDims; ++i) {
 ...
 private:
  ...
  struct BlockBroadcastingParams {
    ...
    i32 inner_dim_count;   // count inner dimensions matching in size
    i32 bcast_dim;         // broadcasting dimension index
  ...
  blockBroadcastingParams(TensorBlockDesc& desc) const {
    ...
    for (i32 i = 0; i < params.inner_dim_count; ++i) {
      i32k dim = IsColMajor ? i : NumDims - i - 1;
      params.input_block_sizes[dim] = params.input_dims[dim];
    }
    for (i32 i = params.inner_dim_count; i < NumDims; ++i) {
      i32k dim = IsColMajor ? i : NumDims - i - 1;
    ...
    for (i32 i = 0; i < params.inner_dim_count; ++i) {
      ...
      i32k copy_dim = IsColMajor ? 2 * i : 2 * NumDims - 2 * i - 1;
      i32k broadcast_dim = IsColMajor ? copy_dim + 1 : copy_dim - 1;
    ...
    for (i32 i = 2 * params.inner_dim_count; i < 2 * NumDims; ++i) {
      i32k dim = IsColMajor ? i : 2 * NumDims - i - 1;
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE TensorBlock emptyBlock() const {
    ...
    for (i32 i = 0; i < NumDims; ++i) dimensions[i] = 0;
