...
class TensorExecutor<Expression, DefaultDevice, /*Vectorizable=*/true,
 ...
 public:
  ...
  static EIGEN_STRONG_INLINE void run(
    ...
    if (needs_assign) {
      const StorageIndex size = array_prod(evaluator.dimensions());
      i32k PacketSize = unpacket_traits<typename TensorEvaluator<
...
class TensorExecutor<Expression, DefaultDevice, Vectorizable,
 ...
 public:
  ...
  static constexpr i32 NumDims = traits<Expression>::NumDimensions;
...
struct EvalRange<Evaluator, StorageIndex, /*Vectorizable*/ true> {
  static constexpr i32 PacketSize =
...
EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE i32 sum_will_overflow(Index lhs,
                                                            Index rhs) {
...
EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE Index saturate_add(Index lhs, Index rhs) {
  const Index highest = NumTraits<Index>::highest();
  const Index lowest = NumTraits<Index>::lowest();
  i32 overflow = sum_will_overflow(lhs, rhs);
...
EIGEN_STRONG_INLINE void TensorExecutor<Expression, GpuDevice, Vectorizable, Tiling>::run(
  ...
  if (needs_assign) {
    ...
    i32k block_size = device.maxGpuThreadsPerBlock();
    i32k max_blocks =
    ...
    i32k num_blocks = numext::maxi<i32>(numext::mini<i32>(max_blocks, divup<i32>(size, block_size)), 1);
