...
namespace internal {
  ...
  std::enable_if_t<sizeof(T)==4,i32> count_leading_zeros(const T val)
  {
    ...
    return 31 - index;
#else
    EIGEN_STATIC_ASSERT(sizeof(unsigned z64) == 8, YOU_MADE_A_PROGRAMMING_MISTAKE);
  ...
  template <typename T>
  EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE
  std::enable_if_t<sizeof(T)==8,i32> count_leading_zeros(const T val)
  {
#ifdef EIGEN_GPU_COMPILE_PHASE
    return __clzll(val);
#elif defined(SYCL_DEVICE_ONLY)
    return static_cast<i32>(cl::sycl::clz(val));
    ...
    u32 hi = (u32)((val>>32)&0xffffffff);
    i32 n;
    if(hi==0)
      n = 32 + count_leading_zeros<u32>(lo);
    else
      n = count_leading_zeros<u32>(hi);
    return n;
#else
    EIGEN_STATIC_ASSERT(sizeof(unsigned z64) == 8, YOU_MADE_A_PROGRAMMING_MISTAKE);
  ...
  struct DividerTraits {
    ...
    static constexpr i32 N = sizeof(T) * 8;
  };
  ...
  struct DividerHelper {
    static EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE uint32_t computeMultiplier(i32k log_div, const T divider) {
  ...
  struct DividerHelper<64, T> {
    static EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE uint64_t computeMultiplier(i32k log_div, const T divider) {
...
struct TensorIntDivisor {
 public:
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE TensorIntDivisor(const T divider) {
    i32k N = DividerTraits<T>::N;
    eigen_assert(static_cast<typename UnsignedTraits<T>::type>(divider) < NumTraits<UnsignedType>::highest()/2);
    eigen_assert(divider > 0);

    // fast ln2
    i32k leading_zeros = count_leading_zeros(static_cast<UnsignedType>(divider));
    i32 log_div = N - leading_zeros;
...
class TensorIntDivisor<int32_t, true> {
 public:
  ...
  EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE i32 divide(const int32_t n) const {
#ifdef EIGEN_GPU_COMPILE_PHASE
