...
template <typename Index, typename InputDims, i32 NumKernelDims, i32 Layout>
class IndexMapper {
 public:
  IndexMapper(const InputDims& input_dims, const array<Index, NumKernelDims>& kernel_dims,
    ...
    if (static_cast<i32>(Layout) == static_cast<i32>(ColMajor)) {
      inputStrides[0] = 1;
      outputStrides[0] = 1;
      for (i32 i = 1; i < NumDims; ++i) {
      ...
      for (i32 i = static_cast<i32>(NumDims) - 2; i >= 0; --i) {
        inputStrides[i] = inputStrides[i + 1] * input_dims[i + 1];
        outputStrides[i] = outputStrides[i + 1] * dimensions[i + 1];
      }
    }
    ...
    const size_t offset = static_cast<i32>(Layout) == static_cast<i32>(ColMajor)
                              ? 0
    ...
    i32 written = static_cast<i32>(Layout) == static_cast<i32>(ColMajor)
                      ? NumKernelDims
                      : 0;
  ...
  EIGEN_STRONG_INLINE EIGEN_DEVICE_FUNC Index mapGpuInputPlaneToTensorInputOffset(Index p) const {
    Index inputIndex = 0;
    if (static_cast<i32>(Layout) == static_cast<i32>(ColMajor)) {
      for (i32 d = NumDims - 1; d > NumKernelDims; --d) {
 ...
 private:
  static constexpr i32 NumDims = internal::array_size<InputDims>::value;
...
struct traits<TensorConvolutionOp<Dimensions, InputXprType, KernelXprType> >
{
  ...
  static constexpr i32 NumDimensions = traits<InputXprType>::NumDimensions;
  static constexpr i32 Layout = traits<InputXprType>::Layout;
...
struct TensorEvaluator<const TensorConvolutionOp<Indices, InputArgType, KernelArgType>, Device>
{
  ...
  static constexpr i32 NumDims = internal::array_size<typename TensorEvaluator<InputArgType, Device>::Dimensions>::value;
  static constexpr i32 NumKernelDims = internal::array_size<Indices>::value;
  ...
  static constexpr i32 PacketSize = PacketType<CoeffReturnType, Device>::size;
  typedef StorageMemory<Scalar, Device> Storage;
  typedef typename Storage::Type EvaluatorPointerType;

  static constexpr i32 Layout = TensorEvaluator<InputArgType, Device>::Layout;
  enum {
    IsAligned = i32(TensorEvaluator<InputArgType, Device>::IsAligned) & i32(TensorEvaluator<KernelArgType, Device>::IsAligned),
    PacketAccess = i32(TensorEvaluator<InputArgType, Device>::PacketAccess) & i32(TensorEvaluator<KernelArgType, Device>::PacketAccess),
    BlockAccess = false,
  ...
  {
    EIGEN_STATIC_ASSERT((static_cast<i32>(TensorEvaluator<InputArgType, Device>::Layout) == static_cast<i32>(TensorEvaluator<KernelArgType, Device>::Layout)), YOU_MADE_A_PROGRAMMING_MISTAKE);
    ...
    if (static_cast<i32>(Layout) == static_cast<i32>(ColMajor)) {
      ...
      for (i32 i = NumKernelDims - 1; i >= 0; --i) {
        const Index index = op.indices()[i];
        const Index input_dim = input_dims[index];
        const Index kernel_dim = kernel_dims[i];
        const Index result_dim = input_dim - kernel_dim + 1;
  ...
  void evalTo(typename XprType::Scalar* buffer) {
    ...
    for (i32 i = 0; i < dimensions().TotalSize(); ++i) {
      buffer[i] += coeff(i);
    }
    cleanup();
  }
 ...
 private:
  ...
  EIGEN_DEVICE_FUNC void convolve(Index firstIndex, Index firstKernel, i32 DimIndex, CoeffReturnType& accum) const {
    for (i32 j = 0; j < m_kernelImpl.dimensions()[DimIndex]; ++j) {
  ...
  EIGEN_DEVICE_FUNC void convolvePacket(Index firstIndex, Index firstKernel, i32 DimIndex, Packet& accum) const {
    for (i32 j = 0; j < m_kernelImpl.dimensions()[DimIndex]; ++j) {
      const Index input = firstIndex + j * m_indexStride[DimIndex];
      const Index kernel = firstKernel + j * m_kernelStride[DimIndex];
      if (DimIndex > 0) {
...
template <i32 StaticKernelSize>
struct GetKernelSize {
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE i32 operator() (i32k /*kernelSize*/) const {
...
__global__ EIGEN_HIP_LAUNCH_BOUNDS_1024 void EigenConvolutionKernel1D(
    ...
    const internal::IndexMapper<Index, InputDims, 1, InputEvaluator::Layout>
        indexMapper,
    const float* __restrict kernel, i32k numPlanes, i32k numX,
    i32k maxX, i32k kernelSize, float* buffer) {
  ...
  i32k first_x = blockIdx.x * maxX;
  i32k last_x = (first_x + maxX < numX ? first_x + maxX : numX) - 1;
  i32k num_x_input = last_x - first_x + GetKernelSize<StaticKernelSize>()(kernelSize);
  i32k num_x_output = last_x - first_x + 1;

  i32k first_plane = blockIdx.y * blockDim.y;
  i32k plane_stride = blockDim.y * gridDim.y;

  for (i32 p = first_plane + threadIdx.y; p < numPlanes; p += plane_stride) {
    // Load inputs to shared memory
    i32k plane_input_offset = indexMapper.mapGpuInputPlaneToTensorInputOffset(p);
    i32k plane_kernel_offset = threadIdx.y * num_x_input;
    #pragma unroll
    for (i32 i = threadIdx.x; i < num_x_input; i += blockDim.x) {
      i32k tensor_index = plane_input_offset + indexMapper.mapGpuInputKernelToTensorInputOffset(i+first_x);
    ...
    i32k plane_output_offset = indexMapper.mapGpuOutputPlaneToTensorOutputOffset(p);

    #pragma unroll
    for (i32 i = threadIdx.x; i < num_x_output; i += blockDim.x) {
      i32k kernel_offset = plane_kernel_offset + i;
      float result = 0.0f;
      #pragma unroll
      for (i32 k = 0; k < GetKernelSize<StaticKernelSize>()(kernelSize); ++k) {
        result += s[k + kernel_offset] * kernel[k];
      }
      i32k tensor_index = plane_output_offset + indexMapper.mapGpuOutputKernelToTensorOutputOffset(i+first_x);
...
template <typename InputEvaluator, typename Index, typename InputDims,
          i32 StaticKernelSizeX, i32 StaticKernelSizeY>
__global__ EIGEN_HIP_LAUNCH_BOUNDS_1024 void EigenConvolutionKernel2D(
    ...
    const internal::IndexMapper<Index, InputDims, 2, InputEvaluator::Layout>
        indexMapper,
    const float* __restrict kernel, i32k numPlanes, i32k numX,
    i32k maxX, i32k numY, i32k maxY, i32k kernelSizeX,
    i32k kernelSizeY, float* buffer) {
#if defined(EIGEN_HIPCC)
  HIP_DYNAMIC_SHARED(float, s)
#else
  extern __shared__ float s[];
  ...
  i32k first_x = blockIdx.x * maxX;
  i32k last_x = (first_x + maxX < numX ? first_x + maxX : numX) - 1;
  i32k num_x_input = last_x - first_x + GetKernelSize<StaticKernelSizeX>()(kernelSizeX);
  i32k num_x_output = last_x - first_x + 1;

  i32k first_y = blockIdx.y * maxY;
  i32k last_y = (first_y + maxY < numY ? first_y + maxY : numY) - 1;
  i32k num_y_input = last_y - first_y + GetKernelSize<StaticKernelSizeY>()(kernelSizeY);
  i32k num_y_output = last_y - first_y + 1;

  i32k first_plane = blockIdx.z * blockDim.z;
  i32k plane_stride = blockDim.z * gridDim.z;

  for (i32 p = first_plane + threadIdx.z; p < numPlanes; p += plane_stride) {

    i32k plane_input_offset = indexMapper.mapGpuInputPlaneToTensorInputOffset(p);
    i32k plane_kernel_offset = threadIdx.z * num_y_input;

    // Load inputs to shared memory
    #pragma unroll
    for (i32 j = threadIdx.y; j < num_y_input; j += blockDim.y) {
      i32k input_offset = num_x_input * (j + plane_kernel_offset);
      #pragma unroll
      for (i32 i = threadIdx.x; i < num_x_input; i += blockDim.x) {
        i32k tensor_index = plane_input_offset + indexMapper.mapGpuInputKernelToTensorInputOffset(i+first_x, j+first_y);
    ...
    #pragma unroll
    for (i32 j = threadIdx.y; j < num_y_output; j += blockDim.y) {
      #pragma unroll
      for (i32 i = threadIdx.x; i < num_x_output; i += blockDim.x) {
        float result = 0.0f;
        #pragma unroll
        for (i32 l = 0; l < GetKernelSize<StaticKernelSizeY>()(kernelSizeY); ++l) {
          i32k kernel_offset = kernelSizeX * l;
          i32k input_offset = i + num_x_input * (j + l + plane_kernel_offset);
          #pragma unroll
          for (i32 k = 0; k < GetKernelSize<StaticKernelSizeX>()(kernelSizeX); ++k) {
            result += s[k + input_offset] * kernel[k + kernel_offset];
          }
        }
        i32k tensor_index = plane_output_offset + indexMapper.mapGpuOutputKernelToTensorOutputOffset(i+first_x, j+first_y);
        buffer[tensor_index] = result;
      }
    }
...
__global__ EIGEN_HIP_LAUNCH_BOUNDS_1024 void EigenConvolutionKernel3D(
  ...
  for (i32 p = 0; p < numPlanes; ++p) {

    i32k plane_input_offset = indexMapper.mapGpuInputPlaneToTensorInputOffset(p);
    i32k plane_kernel_offset = 0;
    ...
    for (size_t k = threadIdx.z; k < num_z_input; k += blockDim.z) {
      for (size_t j = threadIdx.y; j < num_y_input; j += blockDim.y) {
        for (size_t i = threadIdx.x; i < num_x_input; i += blockDim.x) {
          i32k tensor_index = plane_input_offset + indexMapper.mapGpuInputKernelToTensorInputOffset(i+first_x, j+first_y, k+first_z);
    ...
    for (size_t k = threadIdx.z; k < num_z_output; k += blockDim.z) {
      for (size_t j = threadIdx.y; j < num_y_output; j += blockDim.y) {
        for (size_t i = threadIdx.x; i < num_x_output; i += blockDim.x) {
          ...
          i32k tensor_index = plane_output_offset + indexMapper.mapGpuOutputKernelToTensorOutputOffset(i+first_x, j+first_y, k+first_z);
          buffer[tensor_index] = result;
        }
      }
    }
...
struct TensorEvaluator<const TensorConvolutionOp<Indices, InputArgType, KernelArgType>, GpuDevice>
{
  ...
  static constexpr i32 NumDims =  internal::array_size<typename TensorEvaluator<InputArgType, GpuDevice>::Dimensions>::value;
  static constexpr i32 NumKernelDims = internal::array_size<Indices>::value;
  typedef typename XprType::Index Index;
  typedef DSizes<Index, NumDims> Dimensions;
  typedef typename TensorEvaluator<KernelArgType, GpuDevice>::Dimensions KernelDimensions;

  static constexpr i32 Layout = TensorEvaluator<InputArgType, GpuDevice>::Layout;
  enum {
  ...
  {
    EIGEN_STATIC_ASSERT((static_cast<i32>(TensorEvaluator<InputArgType, GpuDevice>::Layout) == static_cast<i32>(TensorEvaluator<KernelArgType, GpuDevice>::Layout)), YOU_MADE_A_PROGRAMMING_MISTAKE);
  ...
  static constexpr i32 PacketSize = internal::unpacket_traits<PacketReturnType>::size;
  ...
  static u32 ceil(u32 num, u32 denom) {
    u32k rounded_toward_zero = num / denom;
  ...
  void executeEval(Scalar* data) const {
    ...
    i32k maxSharedMem = m_device.sharedMemPerBlock();
    i32k maxThreadsPerBlock = m_device.maxGpuThreadsPerBlock();
    i32k maxBlocksPerProcessor = m_device.maxGpuThreadsPerMultiProcessor() / maxThreadsPerBlock;
    i32k numMultiProcessors = m_device.getNumGpuMultiProcessors();
    i32k warpSize = 32;

    switch (NumKernelDims) {
      case 1: {
        i32k kernel_size = m_kernelImpl.dimensions().TotalSize();

        i32k numX = dimensions()[m_indices[0]];
        i32k numP = dimensions().TotalSize() / numX;
        i32 maxX;
        dim3 block_size;

        i32k single_stride_dim =
            static_cast<i32>(Layout) == static_cast<i32>(ColMajor)
                ? 0
                : m_inputImpl.dimensions().rank() - 1;
        if (m_indices[0] == single_stride_dim) {
          // Maximum the reuse
          i32k inner_dim = ((maxSharedMem / (sizeof(Scalar)) - kernel_size + 1 + 31) / 32) * 32;
          maxX = numext::mini<i32>(inner_dim, numX);
          i32k maxP = numext::mini<i32>(maxSharedMem / ((kernel_size - 1 + maxX) * sizeof(Scalar)), numP);
          block_size.x = numext::mini(maxThreadsPerBlock, maxX);
          block_size.y = numext::mini<i32>(maxThreadsPerBlock / block_size.x, maxP);
        }
        else {
          // Read as much as possible alongside the inner most dimension, that is the plane
          i32k inner_dim = maxSharedMem / ((warpSize + kernel_size) * sizeof(Scalar));
          i32k maxP = numext::mini<i32>(inner_dim, numP);
          maxX = numext::mini<i32>(maxSharedMem / (inner_dim * sizeof(Scalar)) - kernel_size + 1, numX);

          block_size.x = numext::mini(warpSize, maxX);
          block_size.y = numext::mini<i32>(maxThreadsPerBlock/block_size.x, maxP);
        }

        i32k shared_mem = block_size.y * (maxX + kernel_size - 1) * sizeof(Scalar);
        gpu_assert(shared_mem <= maxSharedMem);

        i32k num_x_blocks = ceil(numX, maxX);
        i32k blocksPerProcessor = numext::mini(maxBlocksPerProcessor, maxSharedMem / shared_mem);
        i32k num_y_blocks = ceil(numMultiProcessors * blocksPerProcessor, num_x_blocks);

        dim3 num_blocks(num_x_blocks, numext::mini<i32>(num_y_blocks, ceil(numP, block_size.y)));
      ...
      case 2: {
        i32k idxX =
            static_cast<i32>(Layout) == static_cast<i32>(ColMajor) ? 0 : 1;
        i32k idxY =
            static_cast<i32>(Layout) == static_cast<i32>(ColMajor) ? 1 : 0;
        i32k kernel_size_x = m_kernelImpl.dimensions()[idxX];
        i32k kernel_size_y = m_kernelImpl.dimensions()[idxY];

        i32k numX = dimensions()[m_indices[idxX]];
        i32k numY = dimensions()[m_indices[idxY]];
        i32k numP = dimensions().TotalSize() / (numX*numY);
        ...
        i32 inner_dim = ((static_cast<i32>(scaling_factor * kernel_size_x) - kernel_size_x + 1 + 32) / 32) * 32;
        i32k maxX = numext::mini<i32>(inner_dim, numX);
        i32k maxY = numext::mini<i32>(maxSharedMem / (sizeof(Scalar) * (maxX + kernel_size_x - 1)) - kernel_size_y + 1, numY);
        i32k maxP = numext::mini<i32>(maxSharedMem / ((kernel_size_x - 1 + maxX) * (kernel_size_y - 1 + maxY) * sizeof(Scalar)), numP);

        dim3 block_size;
        block_size.x = numext::mini(1024, maxX);
        block_size.y = numext::mini<i32>(1024/block_size.x, maxY);
        block_size.z = numext::mini<i32>(1024/(block_size.x*block_size.y), maxP);

        i32k shared_mem = block_size.z * (maxX + kernel_size_x - 1) * (maxY + kernel_size_y - 1) * sizeof(Scalar);
        gpu_assert(shared_mem <= maxSharedMem);

        i32k num_x_blocks = ceil(numX, maxX);
        i32k num_y_blocks = ceil(numY, maxY);
        i32k blocksPerProcessor = numext::mini(maxBlocksPerProcessor, maxSharedMem / shared_mem);
        i32k num_z_blocks = ceil(numMultiProcessors * blocksPerProcessor, num_x_blocks * num_y_blocks);

        dim3 num_blocks(num_x_blocks, num_y_blocks, numext::mini<i32>(num_z_blocks, ceil(numP, block_size.z)));
      ...
      case 3: {
        i32k idxX =
            static_cast<i32>(Layout) == static_cast<i32>(ColMajor) ? 0 : 2;
        i32k idxY =
            static_cast<i32>(Layout) == static_cast<i32>(ColMajor) ? 1 : 1;
        i32k idxZ =
            static_cast<i32>(Layout) == static_cast<i32>(ColMajor) ? 2 : 0;

        i32k kernel_size_x = m_kernelImpl.dimensions()[idxX];
        i32k kernel_size_y = m_kernelImpl.dimensions()[idxY];
        i32k kernel_size_z = m_kernelImpl.dimensions()[idxZ];

        i32k numX = dimensions()[m_indices[idxX]];
        i32k numY = dimensions()[m_indices[idxY]];
        i32k numZ = dimensions()[m_indices[idxZ]];
        i32k numP = dimensions().TotalSize() / (numX*numY*numZ);

        i32k maxX = numext::mini<i32>(128, numext::mini<i32>(maxSharedMem / (sizeof(Scalar) * kernel_size_y * kernel_size_z) - kernel_size_x + 1, numX));
        i32k maxY = numext::mini<i32>(128, numext::mini<i32>(maxSharedMem / (sizeof(Scalar) * (maxX + kernel_size_x - 1) * kernel_size_z) - kernel_size_y + 1, numY));
        i32k maxZ = numext::mini<i32>(128, numext::mini<i32>(maxSharedMem / (sizeof(Scalar) * (maxX + kernel_size_x - 1) * (maxY + kernel_size_y - 1)) - kernel_size_z + 1, numZ));

        dim3 block_size;
        block_size.x = numext::mini(32, maxX);
        block_size.y = numext::mini(32, maxY);
        block_size.z = numext::mini<i32>(1024/(block_size.x*block_size.y), maxZ);
        dim3 num_blocks(ceil(numX, maxX), ceil(numY, maxY), ceil(numZ, maxZ));

        i32k shared_mem = (maxX + kernel_size_x - 1) * (maxY + kernel_size_y - 1) * (maxZ + kernel_size_z - 1) * sizeof(Scalar);
        gpu_assert(shared_mem <= maxSharedMem);
