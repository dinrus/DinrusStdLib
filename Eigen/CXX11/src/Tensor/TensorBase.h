...
class TensorBase<Derived, ReadOnlyAccessors>
{
  public:
    ...
    static constexpr i32 NumDimensions = DerivedTraits::NumDimensions;
    ...
    template <i32 NanPropagation=PropagateFast>
    EIGEN_DEVICE_FUNC
  ...
  template<i32 NaNPropagation=PropagateFast, typename OtherDerived> EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE
  ...
  template<i32 NaNPropagation=PropagateFast, typename OtherDerived> EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE
    ...
    // Fourier transforms
    template <i32 FFTDataType, i32 FFTDirection, typename FFT> EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE
    ...
    template <typename Dims,i32 NanPropagation=PropagateFast> EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE
   ...
   EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE
    ...
    pad(const PaddingDimensions& padding) const {
      return TensorPaddingOp<const PaddingDimensions, const Derived>(derived(), padding, internal::scalar_cast_op<i32, Scalar>()(0));
    }
    template <typename PaddingDimensions> EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE
    const TensorPaddingOp<const PaddingDimensions, const Derived>
  ...
  protected:
    template <typename Scalar, i32 NumIndices, i32 Options, typename IndexType> friend class Tensor;
    template <typename Scalar, typename Dimensions, i32 Option, typename IndexTypes> friend class TensorFixedSize;
    // the Eigen:: prefix is required to workaround a compilation issue with nvcc 9.0
    template <typename OtherDerived, i32 AccessLevel> friend class Eigen::TensorBase;
    EIGEN_DEVICE_FUNC
    EIGEN_STRONG_INLINE const Derived& derived() const { return *static_cast<const Derived*>(this); }
};

template<typename Derived, i32 AccessLevel = internal::accessors_level<Derived>::value>
class TensorBase : public TensorBase<Derived, ReadOnlyAccessors> {
 public:
    ...
    template <typename Scalar, typename Dimensions, i32 Option, typename IndexTypes> friend class TensorFixedSize;
    // the Eigen:: prefix is required to workaround a compilation issue with nvcc 9.0
    template <typename OtherDerived, i32 OtherAccessLevel> friend class Eigen::TensorBase;
