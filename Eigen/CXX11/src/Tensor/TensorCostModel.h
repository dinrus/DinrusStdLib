...
class TensorOpCost {
 public:
  ...
  static EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE i32 MulCost() {
    return internal::functor_traits<
        internal::scalar_product_op<ArgType, ArgType> >::Cost;
  }
  template <typename ArgType>
  static EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE i32 AddCost() {
    return internal::functor_traits<internal::scalar_sum_op<ArgType> >::Cost;
  }
  template <typename ArgType>
  static EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE i32 DivCost() {
    return internal::functor_traits<
        internal::scalar_quotient_op<ArgType, ArgType> >::Cost;
  }
  template <typename ArgType>
  static EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE i32 ModCost() {
    return internal::functor_traits<internal::scalar_mod_op<ArgType> >::Cost;
  }
  template <typename SrcType, typename TargetType>
  static EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE i32 CastCost() {
    return internal::functor_traits<
...
class TensorCostModel {
 public:
  ...
  static i32k kDeviceCyclesPerComputeCycle = 1;

 // Costs in device cycles.
  static i32k kStartupCycles = 100000;
  static i32k kPerThreadCycles = 100000;
  static i32k kTaskSize = 40000;
  ...
  // coefficient.
  static EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE i32 numThreads(
      double output_size, const TensorOpCost& cost_per_coeff, i32 max_threads) {
    double cost = totalCost(output_size, cost_per_coeff);
    double threads = (cost - kStartupCycles) / kPerThreadCycles + 0.9;
    // Make sure we don't invoke undefined behavior when we convert to an i32.
    threads = numext::mini<double>(threads, GenericNumTraits<i32>::highest());
    return numext::mini(max_threads,
                        numext::maxi<i32>(1, static_cast<i32>(threads)));
  }
