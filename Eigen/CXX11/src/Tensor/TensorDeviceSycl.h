...
class QueueInterface {
 public:
  ...
  /// No callback is required here as both arguments are on the device
  /// and SYCL can handle the dependency.
  EIGEN_STRONG_INLINE void memset(void *data, i32 c, size_t n) const {
    static const auto write_mode = cl::sycl::access::mode::discard_write;
    if (n == 0) {
      return;
    }
    auto f = [&](cl::sycl::handler &cgh) {
      // Get a typed range accesser to ensure we fill each ubyte, in case
  ...
  EIGEN_STRONG_INLINE i32 majorDeviceVersion() const { return 1; }
