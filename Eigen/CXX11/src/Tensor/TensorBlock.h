...
template <typename Scalar, typename IndexType, i32 NumDims, i32 Layout>
class TensorBlockIO;
...
template <i32 Layout, typename IndexType, i32 NumDims>
...
template <i32 NumDims, typename IndexType = Eigen::Index>
class TensorBlockDescriptor {
 public:
  ...
  class DestinationBuffer {
   public:
    enum DestinationBufferKind : i32 {
      // The above explicit specification of "i32" as the enum basetype is
      // needed to get around a HIPCC link error ("the field type is not
      // amp-compatible")
      // which is issued for class members with the enum type.
      // TODO(rocm):
      // remove the "i32" basetype once HIPCC has been fixed to not error out
   ...
   private:
    ...
    template <i32 Layout, typename Scalar>
  ...
  IndexType dimension(i32 index) const { return m_dimensions[index]; }
  ...
  template <i32 Layout, typename Scalar, typename DstStridesIndexType>
...
template <i32 NumDims, i32 Layout, typename IndexType = Eigen::Index>
class TensorBlockMapper {
 ...
 public:
  ...
  blockDescriptor(IndexType block_index) const {
    static const bool isColMajor = Layout == static_cast<i32>(ColMajor);
    ...
    for (i32 i = NumDims - 1; i >= 0; --i) {
      i32k dim = isColMajor ? i : NumDims - i - 1;
...
class TensorBlockScratchAllocator {
 public:
  ...
  void* allocate(size_t size) {
    ...
    i32k num_allocations = static_cast<i32>(m_allocations.size());
...
template <typename Scalar, i32 NumDims, i32 Layout,
...
class TensorMaterializedBlock {
 public:
  ...
  EIGEN_STRONG_INLINE static TensorMaterializedBlock materialize(
    ...
    i32 num_matching_inner_dims = 0;
    for (i32 i = 0; i < NumDims; ++i) {
      i32 dim = is_col_major ? i : NumDims - i - 1;
    ...
    for (i32 i = num_matching_inner_dims + 1; i < NumDims; ++i) {
      i32 dim = is_col_major ? i : NumDims - i - 1;
...
class TensorBlockIO {
 ...
 public:
  typedef DSizes<IndexType, NumDims> Dimensions;
  typedef DSizes<i32, NumDims> DimensionsMap;
  ...
  static EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE IndexType Copy(
    ...
    {
      i32 inner_dim = IsColMajor ? 0 : NumDims - 1;
    ...
    i32 num_squeezable_dims = NumSqueezableInnerDims(dim_map);
    ...
    // effective inner dim.
    i32 num_size_one_inner_dims = 0;
    for (i32 i = 0; i < num_squeezable_dims; ++i) {
      i32k dst_dim = IsColMajor ? i : NumDims - i - 1;
    ...
    for (i32 i = num_size_one_inner_dims + 1; i < num_squeezable_dims; ++i) {
      i32k dst_dim = IsColMajor ? i : NumDims - i - 1;
    ...
    i32k at_least_1_dim = NumDims <= 1 ? 1 : NumDims - 1;
    array<BlockIteratorState, at_least_1_dim> it;

    // Initialize block iterator state. Squeeze away any dimension of size 1.
    i32 idx = 0;  // currently initialized iterator state index
    for (i32 i = num_size_one_inner_dims; i < NumDims - 1; ++i) {
      i32k dst_dim = IsColMajor ? i + 1 : NumDims - i - 2;
...
#undef COPY_INNER_DIM
  ...
  static EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE IndexType Copy(const Dst& dst,
    ...
    for (i32 i = 0; i < NumDims; ++i) dst_to_src_map[i] = i;
 ...
 private:
  ...
  static i32 NumSqueezableInnerDims(const DimensionsMap& dim_map) {
    i32 num_squeezable_dims = 0;
    for (i32 i = 0; i < NumDims; ++i) {
      i32k dim = IsColMajor ? i : NumDims - i - 1;
...
template <typename Scalar, i32 NumDims, typename TensorBlockExpr,
          typename IndexType = Eigen::Index>
