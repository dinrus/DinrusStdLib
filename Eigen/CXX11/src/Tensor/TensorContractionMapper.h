...
template <typename Scalar, typename Index, i32 side, typename Tensor,
          typename nocontract_t, typename contract_t, i32 packet_size,
          bool inner_dim_contiguous, bool inner_dim_reordered, i32 Alignment,
...
template<typename Scalar, typename Index, i32 side,
         typename Tensor,
         typename nocontract_t, typename contract_t,
         i32 packet_size, bool inner_dim_contiguous, i32 Alignment, template <class> class MakePointer_ = MakePointer>
class SimpleTensorContractionMapper {
  ...
  EIGEN_STRONG_INLINE Index computeIndex(Index row, Index col) const {
    ...
    Index linidx = 0;
    EIGEN_UNROLL_LOOP
    for (i32 i = static_cast<i32>(array_size<nocontract_t>::value) - 1; i > 0; i--) {
    ...
    if(array_size<contract_t>::value > 0) {
      EIGEN_UNROLL_LOOP
      for (i32 i = static_cast<i32>(array_size<contract_t>::value) - 1; i > 0; i--) {
...
template<typename Scalar, typename Index, i32 side,
         typename Tensor,
         typename nocontract_t, typename contract_t,
         i32 packet_size, bool inner_dim_contiguous,
         bool inner_dim_reordered, i32 Alignment, template <class> class MakePointer_>
class BaseTensorContractionMapper : public SimpleTensorContractionMapper<Scalar, Index, side, Tensor, nocontract_t, contract_t, packet_size, inner_dim_contiguous, Alignment, MakePointer_>
{
 public:
  ...
  template <typename PacketT,i32 AlignmentType>
  ...
  : public SimpleTensorContractionMapper<Scalar, Index, side, Tensor, nocontract_t, contract_t, 1, inner_dim_contiguous, Alignment, MakePointer_>
{
 public:
  ...
  template <typename PacketT,i32> EIGEN_DEVICE_FUNC
  EIGEN_STRONG_INLINE PacketT loadPacket(Index i, Index j) const {
...
template<typename Scalar, typename Index, i32 side,
         ...
         typename nocontract_t, typename contract_t,
         i32 packet_size,
         bool inner_dim_contiguous, bool inner_dim_reordered, i32 Alignment, template <class> class MakePointer_=MakePointer>
class TensorContractionSubMapper {
 public:
  ...
  EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE PacketT load(Index i) const {
    ...
    i32k ActualAlignment = (AlignmentType == Aligned) && (Alignment == Aligned) ? Aligned : Unaligned;
    if (UseDirectOffsets) {
...
template<typename Scalar_, typename Index, i32 side,
         typename Tensor,
         typename nocontract_t, typename contract_t,
         i32 packet_size,
         bool inner_dim_contiguous, bool inner_dim_reordered, i32 Alignment,  template <class> class MakePointer_=MakePointer>
...
template<typename Scalar_, typename Index_, i32 side_,
         typename Tensor_,
         typename nocontract_t_, typename contract_t_,
         i32 packet_size_,
         bool inner_dim_contiguous_, bool inner_dim_reordered_, i32 Alignment_,  template <class> class MakePointer_>
