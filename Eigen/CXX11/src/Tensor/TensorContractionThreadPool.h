...
struct TensorEvaluator<const TensorContractionOp<Indices, LeftArgType, RightArgType, OutputKernelType>, ThreadPoolDevice> :
  ...
  static constexpr i32 Layout = TensorEvaluator<LeftArgType, Device>::Layout;
  ...
  typedef std::conditional_t<
    static_cast<i32>(Layout) == static_cast<i32>(ColMajor), LeftArgType, RightArgType> EvalLeftArgType;
  typedef std::conditional_t<
    static_cast<i32>(Layout) == static_cast<i32>(ColMajor), RightArgType, LeftArgType> EvalRightArgType;

  static constexpr i32 LDims =
      internal::array_size<typename TensorEvaluator<EvalLeftArgType, Device>::Dimensions>::value;
  static constexpr i32 RDims =
      internal::array_size<typename TensorEvaluator<EvalRightArgType, Device>::Dimensions>::value;
  static constexpr i32 ContractDims = internal::array_size<Indices>::value;
  ...
  static constexpr i32 NumDims = LDims + RDims - 2 * ContractDims;
  ...
  template <i32 Alignment>
  void evalProduct(Scalar* buffer) const {
    evalProductImpl<NoCallback, Alignment>(buffer, NoCallback());
  }

  template <typename EvalToCallback, i32 Alignment>
  ...
  template <typename DoneCallback, i32 Alignment>
  void evalProductImpl(Scalar* buffer, DoneCallback done) const {
    ...
    i32 num_threads = TensorCostModel<ThreadPoolDevice>::numThreads(
        static_cast<double>(n) * m, cost, this->m_device.numThreads());
    i32 num_threads_by_k = numThreadsInnerDim(m, n, k);
    ...
    i32k num_worker_threads = this->m_device.numThreadsInPool();
...
#undef CONTEXT_ARGS
  ...
  class EvalParallelContext {
   public:
    ...
    EvalParallelContext(const Self* self, i32 num_threads, Scalar* buffer,
      ...
      if (parallelize_by_sharding_dim_only_) {
        i32k num_worker_threads = device_.numThreadsInPool();
   ...
   private:
    ...
    std::atomic<i32> num_thread_local_allocations_;
    i32k thread_local_capacity;
    ...
    class ThreadLocalBlocks {
     public:
      ...
      BlockType& block(i32 grain_index) {
    ...
    class ThreadLocalBlocksInitialize {
     ...
     public:
      ...
      void operator()(Blocks& blocks) {
        i32k n = ctx_.num_thread_local_allocations_.fetch_add(
     ...
     private:
      ...
      struct ThreadLocalBlocksAllocator</*pack_rhs=*/true, EvalCtx> {
        ...
        static void reuse(EvalCtx& ctx, i32 index, Blocks& blocks) {
    ...
    LhsBlock& packed_lhs(Index m, Index k, Index m1, bool use_thread_local) {
      if (use_thread_local) {
        ...
        return blocks.block(internal::convert_index<i32>(grain_index)); // FIXME better make ThreadLocalBlocks use Eigen::Index?
      } else {
  ...
  template <bool lhs_inner_dim_contiguous, bool rhs_inner_dim_contiguous,
            bool rhs_inner_dim_reordered, i32 Alignment>
  ...
  struct EvalShardedByInnerDimContext {
    EvalShardedByInnerDimContext(const Self* self, i32 num_threads,
    ...
    template <i32 Alignment>
    void run() {
      Barrier barrier(internal::convert_index<i32>(num_blocks));
   ...
   private:
    ...
    std::atomic<i32> num_pending_blocks;
    ...
    // Keep count of pending gemm tasks for each l0 range.
    MaxSizeVector<std::atomic<i32>> l0_state;  // [0, l0_ranges)
    ...
    void processBlock(Index block_idx, Index begin, Index end) {
      ...
      TENSOR_CONTRACTION_DISPATCH(
          ...
          (buf, begin, end,
           /*num_threads=*/internal::convert_index<i32>(num_blocks)));

      // Check if it was the last task in l0 range.
      const Index l0_index = block_idx / l0_size;
      i32k v = l0_state[l0_index].fetch_sub(1);
      eigen_assert(v >= 1);
    ...
    void evalAsync(Index start_block_idx, Index end_block_idx) {
      ...
      i32 v = num_pending_blocks.fetch_sub(1);
      eigen_assert(v >= 1);
    ...
    static Index blockSize(Index k, i32 num_threads) {
  ...
  Index coarsenM(Index m, Index n, Index bm, Index bn, Index bk, Index gn,
                 i32 num_threads, bool shard_by_col) const {
    Index gm = 1;
    Index gm1 = 1;
    Index nm0 = divup(m, bm);
    Index nm1 = nm0;
    for (;;) {
      ...
      if (gm1 > nm0) break;
      // Check the candidate.
      i32 res = checkGrain(m, n, bm, bn, bk, gm1, gn, gm, gn, num_threads,
                           shard_by_col);
      if (res < 0) break;
      nm1 = divup(nm0, gm1);
      if (res == 0) continue;
  ...
  Index coarsenN(Index m, Index n, Index bm, Index bn, Index bk, Index gm,
    ...
    for (;;) {
      ...
      if (gn1 > nn0) break;
      i32 res = checkGrain(m, n, bm, bn, bk, gm, gn1, gm, gn, num_threads,
                           shard_by_col);
      if (res < 0) break;
      nn1 = divup(nn0, gn1);
      if (res == 0) continue;
  ...
  // (oldgm, oldgn).
  i32 checkGrain(Index m, Index n, Index bm, Index bn, Index bk, Index gm,
                 Index gn, Index oldgm, Index oldgn, i32 num_threads,
  ...
  TensorOpCost contractionCost(Index m, Index n, Index bm, Index bn, Index bk,
    ...
    i32k packed_size = std::min<i32>(PacketType<LhsScalar, Device>::size,
                                          PacketType<RhsScalar, Device>::size);
    i32k output_packet_size = internal::unpacket_traits<PacketReturnType>::size;
  ...
  static bool shardByInnerDim(Index m, Index n, Index k, i32 num_threads,
                              i32 num_threads_by_k) {
  ...
  i32 numThreadsInnerDim(Index m, Index n, Index k) const {
    i32k output_packet_size = internal::unpacket_traits<PacketReturnType>::size;
    ...
    for (i32 nt = 2; nt <= this->m_device.numThreads(); nt += 2) {
