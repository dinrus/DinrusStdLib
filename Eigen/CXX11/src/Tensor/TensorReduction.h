 ...
 : traits<XprType>
{
  ...
  static constexpr i32 NumDimensions = XprTraits::NumDimensions - array_size<Dims>::value;
  static constexpr i32 Layout = XprTraits::Layout;
...
template <typename OutputDims> struct DimInitializer {
  ...
  static void run(const InputDims& input_dims,
    ...
    i32k NumInputDims = internal::array_size<InputDims>::value;
    i32 outputIndex = 0;
    i32 reduceIndex = 0;
    for (i32 i = 0; i < NumInputDims; ++i) {
      if (reduced[i]) {
...
template <typename ReducedDims, i32 NumTensorDims, i32 Layout>
...
template <typename ReducedDims, i32 NumTensorDims>
...
template <i32 DimIndex, typename Self, typename Op>
struct GenericDimReducer {
  static EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE void reduce(const Self& self, typename Self::Index firstIndex, Op& reducer, typename Self::CoeffReturnType* accum) {
    EIGEN_STATIC_ASSERT((DimIndex > 0), YOU_MADE_A_PROGRAMMING_MISTAKE);
    for (i32 j = 0; j < self.m_reducedDims[DimIndex]; ++j) {
...
struct GenericDimReducer<0, Self, Op> {
  static EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE void reduce(const Self& self, typename Self::Index firstIndex, Op& reducer, typename Self::CoeffReturnType* accum) {
    for (i32 j = 0; j < self.m_reducedDims[0]; ++j) {
...
template <i32 DimIndex, typename Self, typename Op, bool vectorizable = (Self::InputPacketAccess && Self::ReducerTraits::PacketAccess)>
...
struct FullReducer<Self, Op, ThreadPoolDevice, Vectorizable> {
  ...
  static void run(const Self& self, Op& reducer, const ThreadPoolDevice& device,
    ...
    Barrier barrier(internal::convert_index<u32>(numblocks));
...
template <i32 B, i32 N, typename S, typename R, typename I_>
__global__ EIGEN_HIP_LAUNCH_BOUNDS_1024 void FullReductionKernel(R, const S, I_, typename S::CoeffReturnType*, u32*);
...
template <i32 NPT, typename S, typename R, typename I_>
...
struct TensorReductionEvaluatorBase<const TensorReductionOp<Op, Dims, ArgType, MakePointer_>, Device>
{
  ...
  static constexpr i32 NumInputDims = internal::array_size<InputDimensions>::value;
  static constexpr i32 NumReducedDims = internal::array_size<Dims>::value;
  static constexpr i32 NumOutputDims = NumInputDims - NumReducedDims;
  ...
  static constexpr i32 NumPreservedStrides = max_n_1<NumOutputDims>::size;
...
static constexpr bool RunningOnGPU = false;
  ...
  static constexpr i32 Layout = TensorEvaluator<ArgType, Device>::Layout;
  enum {
    IsAligned = false,
      ...
      : m_impl(op.expression(), device), m_reducer(op.reducer()), m_result(NULL), m_device(device)
  {
    ...
    for (i32 i = 0; i < NumReducedDims; ++i) {
    ...
    if (NumInputDims > 0) {
      ...
      if (static_cast<i32>(Layout) == static_cast<i32>(ColMajor)) {
        input_strides[0] = 1;
        for (i32 i = 1; i < NumInputDims; ++i) {
          input_strides[i] = input_strides[i-1] * input_dims[i-1];
        }
      } else {
        input_strides.back() = 1;
        for (i32 i = NumInputDims - 2; i >= 0; --i) {
          input_strides[i] = input_strides[i + 1] * input_dims[i + 1];
        }
      }
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE CoeffReturnType coeff(Index index) const
  {
    ...
    if (ReducingInnerMostDims || RunningFullReduction) {
      const Index num_values_to_reduce =
        (static_cast<i32>(Layout) == static_cast<i32>(ColMajor)) ? m_preservedStrides[0] : m_preservedStrides[NumPreservedStrides - 1];
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE PacketReturnType packet(Index index) const
  {
    ...
    if (ReducingInnerMostDims) {
      ...
      const Index firstIndex = firstInput(index);
      i32k innermost_dim = (static_cast<i32>(Layout) == static_cast<i32>(ColMajor)) ? 0 : NumOutputDims - 1;
  ...
  private:
  template <i32, typename, typename> friend struct internal::GenericDimReducer;
  template <typename, typename, bool, bool> friend struct internal::InnerMostDimReducer;
  template <i32, typename, typename, bool> friend struct internal::InnerMostDimPreserver;
  template <typename S, typename O, typename D, bool V> friend struct internal::FullReducer;
#ifdef EIGEN_USE_THREADS
  template <typename S, typename O, bool V> friend struct internal::FullReducerShard;
#endif
#if defined(EIGEN_USE_GPU) && (defined(EIGEN_GPUCC))
  template <i32 B, i32 N, typename S, typename R, typename I_> KERNEL_FRIEND void internal::FullReductionKernel(R, const S, I_, typename S::CoeffReturnType*, u32*);
#if defined(EIGEN_HAS_GPU_FP16)
  ...
  template <i32 B, i32 N, typename S, typename R, typename I_> KERNEL_FRIEND void internal::FullReductionKernelHalfFloat(R, const S, I_, half*, internal::packet_traits<Eigen::half>::type*);
  template <i32 NPT, typename S, typename R, typename I_> KERNEL_FRIEND void internal::InnerReductionKernelHalfFloat(R, const S, I_, I_, half*);
#endif
  template <i32 NPT, typename S, typename R, typename I_> KERNEL_FRIEND void internal::InnerReductionKernel(R, const S, I_, I_, typename S::CoeffReturnType*);

  template <i32 NPT, typename S, typename R, typename I_> KERNEL_FRIEND void internal::OuterReductionKernel(R, const S, I_, I_, typename S::CoeffReturnType*);
#endif
 ...
 template <typename, typename, typename> friend struct internal::GenericReducer;
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE Index firstInput(Index index) const {
    ...
    Index startInput = 0;
    if (static_cast<i32>(Layout) == static_cast<i32>(ColMajor)) {
      for (i32 i = NumOutputDims - 1; i > 0; --i) {
      ...
      for (i32 i = 0; i < NumOutputDims - 1; ++i) {
        // This is index_i in the output tensor.
        const Index idx = index / m_outputStrides[i];
        startInput += idx * m_preservedStrides[i];
        index -= idx * m_outputStrides[i];
