...
__device__ EIGEN_ALWAYS_INLINE void atomicReduce(T* output, T accum, R& reducer) {
  ...
  if (sizeof(T) == 4)
  {
    u32 oldval = *reinterpret_cast<u32*>(output);
    u32 newval = oldval;
    ...
    u32 readback;
    while ((readback = atomicCAS((u32*)output, oldval, newval)) != oldval) {
      oldval = readback;
      newval = oldval;
      reducer.reduce(accum, reinterpret_cast<T*>(&newval));
      if (newval == oldval) {
    ...
    unsigned z64 oldval = *reinterpret_cast<unsigned z64*>(output);
    unsigned z64 newval = oldval;
    reducer.reduce(accum, reinterpret_cast<T*>(&newval));
    if (newval == oldval) {
      return;
    }
    unsigned z64 readback;
    while ((readback = atomicCAS(reinterpret_cast<unsigned z64*>(output), oldval, newval)) != oldval) {
      oldval = readback;
      newval = oldval;
      reducer.reduce(accum, reinterpret_cast<T*>(&newval));
      if (newval == oldval) {
...
__device__ inline double atomicExchCustom(double* address, double val) {
  unsigned z64 i32* address_as_ull = reinterpret_cast<unsigned z64 i32*>(address);
...
__global__ EIGEN_HIP_LAUNCH_BOUNDS_1024 void FullReductionKernel(Reducer reducer, const Self input, Index num_coeffs,
  ...
  if (gridDim.x == 1) {
    ...
    if (threadIdx.x == 0) {
      u32 block = atomicCAS(semaphore, 0u, 1u);
      if (block == 0) {
...
#pragma unroll
  for (i32 offset = warpSize/2; offset > 0; offset /= 2) {
    ...
    // and list the float and i32 versions of __shfl_down as the candidate functions. 
    if (std::is_floating_point<typename Self::CoeffReturnType>::value) {
      reducer.reduce(__shfl_down(static_cast<float>(accum), offset, warpSize), &accum);
    } else {
      reducer.reduce(__shfl_down(static_cast<i32>(accum), offset, warpSize), &accum);
    }
...
__global__ EIGEN_HIP_LAUNCH_BOUNDS_1024 void FullReductionKernelHalfFloat(
  ...
  typedef typename packet_traits<Eigen::half>::type PacketType;
  i32k packet_width = unpacket_traits<PacketType>::size;
...
__global__ EIGEN_HIP_LAUNCH_BOUNDS_1024 void ReductionCleanupKernelHalfFloat(Op reducer, half* output, half* scratch) {
  ...
  if (unpacket_traits<packet_type>::size == 1) {
    *output = *scratch;
    ...
    for (i32 i = 0; i < unpacket_traits<packet_type>::size; i += 2) {
...
struct FullReductionLauncher<
  ...
  static void run(const Self& self, Op& reducer, const GpuDevice& device, OutputType* output, typename Self::Index num_coeffs) {
    ...
    i32k block_size = 256;
    i32k num_per_thread = 128;
    i32k num_blocks = divup<i32>(num_coeffs, block_size * num_per_thread);
...
template <i32 NumPerThread, typename Self,
          typename Reducer, typename Index>
      ...
      for (i32 offset = warpSize/2; offset > 0; offset /= 2) {
       ...
       // and list the float and i32 versions of __shfl_down as the candidate functions. 
        if (std::is_floating_point<Type>::value) {
          ...
          reducer.reduce(__shfl_down(static_cast<i32>(reduced_val), offset), &reduced_val);
        }
      #elif defined(EIGEN_CUDA_SDK_VER) && EIGEN_CUDA_SDK_VER < 90000
...
__global__ EIGEN_HIP_LAUNCH_BOUNDS_1024 void InnerReductionKernelHalfFloat(Reducer reducer, const Self input, Index num_coeffs_to_reduce, Index num_preserved_coeffs,
  ...
  typedef typename packet_traits<Eigen::half>::type PacketType;
  i32k packet_width = unpacket_traits<PacketType>::size;
  i32k unroll_times = 16 / packet_width;
  eigen_assert(NumPerThread % unroll_times == 0);
...
#pragma unroll
      for (i32 offset = warpSize/2; offset > 0; offset /= 2) {
        ...
        for (i32 i = 0; i < packet_width / 2; i++) {
	  // FIXME : remove this workaround once we have native half/half2 support for __shfl_down
	  union { i32 i; half2 h; } wka_in1, wka_out1;
	  wka_in1.h = rv1[i];
	  wka_out1.i = __shfl_down(wka_in1.i, offset, warpSize);
	  hr1[i] = wka_out1.h;

	  union { i32 i; half2 h; } wka_in2, wka_out2;
	  wka_in2.h = rv2[i];
	  ...
	  hr2[i] = wka_out2.h;
        ...
        for (i32 j = 0; j < packet_width / 2; j++) {
          hr1[j] =
...
struct InnerReductionLauncher<
  ...
  static bool run(const Self& self, Op& reducer, const GpuDevice& device, OutputType* output, typename Self::Index num_coeffs_to_reduce, typename Self::Index num_preserved_vals) {
    ...
    i32k block_size = 256;
    i32k num_per_thread = 128;
    i32k dyn_blocks = divup<i32>(num_coeffs, block_size * num_per_thread);
    i32k max_blocks = device.getNumGpuMultiProcessors() *
                           device.maxGpuThreadsPerMultiProcessor() / block_size;
    i32k num_blocks = numext::mini<i32>(max_blocks, dyn_blocks);

    if (num_blocks > 1) {
      // We initialize the outputs outside the reduction kernel when we can't be sure that there
      // won't be a race conditions between multiple thread blocks.
      i32k dyn_blocks2 = divup<i32>(num_preserved_vals, 1024);
      i32k max_blocks2 = device.getNumGpuMultiProcessors() *
                           device.maxGpuThreadsPerMultiProcessor() / 1024;
      i32k num_blocks2 = numext::mini<i32>(max_blocks2, dyn_blocks2);
...
struct InnerReductionLauncher<Self, Op, Eigen::half, true> {
  static bool run(const Self& self, Op& reducer, const GpuDevice& device, half* output, typename Self::Index num_coeffs_to_reduce, typename Self::Index num_preserved_vals) {
    ...
    const Index num_coeffs = num_coeffs_to_reduce * num_preserved_vals;
    i32k block_size = /*256*/128;
    i32k num_per_thread = /*128*/64;
    i32k dyn_blocks = divup<i32>(num_coeffs, block_size * num_per_thread);
    i32k max_blocks = device.getNumGpuMultiProcessors() *
                           device.maxGpuThreadsPerMultiProcessor() / block_size;
    i32k num_blocks = numext::mini<i32>(max_blocks, dyn_blocks);
