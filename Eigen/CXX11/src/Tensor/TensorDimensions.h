...
// Dynamic size
template <typename DenseIndex, i32 NumDims>
struct DSizes : array<DenseIndex, NumDims> {
  typedef array<DenseIndex, NumDims> Base;
  static i32k count = NumDims;
  ...
  // larger type, e.g. allow to promote dimensions of type i32 to long.
  ...
  explicit DSizes(const Eigen::IndexList<FirstType, OtherTypes...>& dimensions) {
    for (i32 i = 0; i < dimensions.count; ++i) {
      (*this)[i] = dimensions[i];
    }
  }
...
template <typename IndexType, i32 NumDims>
...
template <typename DenseIndex, i32 NumDims> struct array_size<const DSizes<DenseIndex, NumDims> > {
  static const ptrdiff_t value = NumDims;
};
template <typename DenseIndex, i32 NumDims> struct array_size<DSizes<DenseIndex, NumDims> > {
  static const ptrdiff_t value = NumDims;
};
#ifndef EIGEN_EMULATE_CXX11_META_H
