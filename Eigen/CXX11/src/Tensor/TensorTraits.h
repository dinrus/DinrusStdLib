...
template<typename Scalar, i32 Options>
...
template<typename Scalar_, i32 NumIndices_, i32 Options_, typename IndexType_>
struct traits<Tensor<Scalar_, NumIndices_, Options_, IndexType_> >
{
  typedef Scalar_ Scalar;
  typedef Dense StorageKind;
  typedef IndexType_ Index;
  static constexpr i32 NumDimensions = NumIndices_;
  static constexpr i32 Layout = Options_ & RowMajor ? RowMajor : ColMajor;
  enum {
    Options = Options_,
...
template<typename Scalar_, typename Dimensions, i32 Options_, typename IndexType_>
struct traits<TensorFixedSize<Scalar_, Dimensions, Options_, IndexType_> >
{
  typedef Scalar_ Scalar;
  typedef Dense StorageKind;
  typedef IndexType_ Index;
  static constexpr i32 NumDimensions = array_size<Dimensions>::value;
  static constexpr i32 Layout = Options_ & RowMajor ? RowMajor : ColMajor;
  enum {
    Options = Options_,
    Flags = compute_tensor_flags<Scalar_, Options_>::ret | (is_const<Scalar_>::value ? 0: LvalueBit)
...
template<typename PlainObjectType, i32 Options_, template <class> class MakePointer_>
  ...
  : public traits<PlainObjectType>
{
  ...
  static constexpr i32 NumDimensions = BaseTraits::NumDimensions;
  static constexpr i32 Layout = BaseTraits::Layout;
  enum {
    Options = Options_,
...
template<typename Scalar_, i32 NumIndices_, i32 Options, typename IndexType_>
...
template<typename Scalar_, typename Dimensions, i32 Options, typename IndexType_>
...
template<typename PlainObjectType, i32 Options, template <class> class MakePointer>
...
template<typename T, i32 n=1, typename PlainObject = void> struct nested
{
