...
#define initResultRow(i)                        \
  ...
  internal::scalar_cast_op<i32, Scalar> conv;
  initResultRow(0);
  initResultRow(1);
  initResultRow(2);
  initResultRow(3);
...
#undef writeRow
  ...
  i32k max_i_write = numext::mini((i32)((m_size - base_m - threadIdx.y + 7) / 8), 8);
  i32k max_j_write = numext::mini((i32)((n_size - base_n - threadIdx.z + 7) / 8), 8);
...
#undef add_vals
  ...
  if (!CHECK_LHS_BOUNDARY && !CHECK_RHS_BOUNDARY) {
    ...
    // CHECK RHS
    /*
    i32 ncols_rem = fminf(n_size- horiz_base, 4);
    for (i32 i = 0; i < ncols_rem; i++) {
      output(lhs_vert, horiz_base + i) = results[i].x;
      output(lhs_vert + 1, horiz_base + i) = results[i].y;
      output(lhs_vert + 2, horiz_base + i) = results[i].z;
      output(lhs_vert + 3, horiz_base + i) = results[i].w;
...
\
    ...
    for (i32 koff = 0; koff < 32; koff ++) {
      ...
      i32 start_feature = (threadIdx.y / 4) * 8;
