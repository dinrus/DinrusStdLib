...
struct traits<TensorChippingOp<DimId, XprType> > : public traits<XprType>
{
  ...
  static constexpr i32 NumDimensions = XprTraits::NumDimensions - 1;
...
struct TensorEvaluator<const TensorChippingOp<DimId, ArgType>, Device>
{
  ...
  static constexpr i32 NumInputDims = internal::array_size<typename TensorEvaluator<ArgType, Device>::Dimensions>::value;
  static constexpr i32 NumDims = NumInputDims-1;
      ...
      : m_impl(op.expression(), device), m_dim(op.dim()), m_device(device)
  {
    ...
    if (static_cast<i32>(Layout) == static_cast<i32>(ColMajor)) {
      for (i32 i = 0; i < m_dim.actualDim(); ++i) {
        m_stride *= input_dims[i];
        m_inputStride *= input_dims[i];
      }
    } else {
      for (i32 i = NumInputDims-1; i > m_dim.actualDim(); --i) {
        m_stride *= input_dims[i];
        m_inputStride *= input_dims[i];
      }
    }
  ...
  costPerCoeff(bool vectorized) const {
    ...
    if ((static_cast<i32>(Layout) == static_cast<i32>(ColMajor) &&
         m_dim.actualDim() == 0) ||
        (static_cast<i32>(Layout) == static_cast<i32>(RowMajor) &&
         m_dim.actualDim() == NumInputDims - 1)) {
      cost += TensorOpCost::MulCost<Index>() + TensorOpCost::AddCost<Index>();
    } else if ((static_cast<i32>(Layout) == static_cast<i32>(ColMajor) &&
                m_dim.actualDim() == NumInputDims - 1) ||
               (static_cast<i32>(Layout) == static_cast<i32>(RowMajor) &&
                m_dim.actualDim() == 0)) {
 ...
 protected:
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE bool isInnerChipping() const {
    return IsInnerChipping ||
           (static_cast<i32>(Layout) == ColMajor && m_dim.actualDim() == 0) ||
           (static_cast<i32>(Layout) == RowMajor && m_dim.actualDim() == NumInputDims - 1);
  }

  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE bool isOuterChipping() const {
    return IsOuterChipping ||
           (static_cast<i32>(Layout) == ColMajor && m_dim.actualDim() == NumInputDims-1) ||
           (static_cast<i32>(Layout) == RowMajor && m_dim.actualDim() == 0);
  }
