...
struct TensorEvaluator<const TensorPaddingOp<PaddingDimensions, ArgType>, Device>
{
  ...
  static constexpr i32 NumDims = internal::array_size<PaddingDimensions>::value;
  ...
  block(TensorBlockDesc& desc, TensorBlockScratch& scratch,
    ...
    i32k inner_dim_idx = IsColMajor ? 0 : NumDims - 1;
    ...
    for (i32 i = NumDims - 1; i > 0; --i) {
      ...
      i32k stride_dim = IsColMajor ? dim : dim + 1;
    ...
    for (i32 i = 0; i < NumDims - 1; ++i) {
      i32k dim = IsColMajor ? i + 1 : NumDims - i - 2;
      it[i].count = 0;
    ...
    i32k squeeze_dim = IsColMajor ? inner_dim_idx + 1 : inner_dim_idx - 1;
    ...
    for (Index size = 0; size < output_size;) {
      ...
      for (i32 j = 1; j < NumDims; ++j) {
        i32k dim = IsColMajor ? j : NumDims - j - 1;
      ...
      for (i32 j = 0; j < NumDims - 1; ++j) {
        i32k dim = IsColMajor ? j + 1 : NumDims - j - 2;
 ...
 private:
  ...
  EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE bool isPaddingAtIndexForDim(
      Index index, i32 dim_index) const {
  ...
  void updateCostPerDimension(TensorOpCost& cost, i32 i, bool first) const {
