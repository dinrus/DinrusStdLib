...
struct traits<TensorConversionOp<TargetType, XprType> >
{
  ...
  static constexpr i32 NumDimensions = traits<XprType>::NumDimensions;
  static constexpr i32 Layout = traits<XprType>::Layout;
  enum { Flags = 0 };
...
template <typename TensorEvaluator, typename SrcPacket, typename TgtPacket, i32 SrcCoeffRatio, i32 TgtCoeffRatio>
...
struct PacketConverter<TensorEvaluator, SrcPacket, TgtPacket, 1, 1> {
  ...
  template<i32 LoadMode, typename Index>
...
struct PacketConverter<TensorEvaluator, SrcPacket, TgtPacket, 2, 1> {
  ...
  template<i32 LoadMode, typename Index>
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE TgtPacket packet(Index index) const {
    i32k SrcPacketSize = internal::unpacket_traits<SrcPacket>::size;
...
template <typename TensorEvaluator, typename SrcPacket, typename TgtPacket, i32 TgtCoeffRatio>
struct PacketConverter<TensorEvaluator, SrcPacket, TgtPacket, 1, TgtCoeffRatio> {
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE TgtPacket packet(Index index) const {
    ...
    if (m_impl.data() && (index + SrcPacketSize < m_maxIndex)) {
      ...
      i32k TgtPacketSize = internal::unpacket_traits<TgtPacket>::size;
...
template <typename SrcPacket, typename TargetPacket, i32 LoadMode, bool ActuallyVectorize, bool IsSameT>
struct PacketConv {
  typedef typename internal::unpacket_traits<SrcPacket>::type SrcType;
  typedef typename internal::unpacket_traits<TargetPacket>::type TargetType;

  static constexpr i32 PacketSize = internal::unpacket_traits<TargetPacket>::size;
...
template <typename SrcPacket, typename TargetPacket, i32 LoadMode, bool IsSameT>
struct PacketConv<SrcPacket, TargetPacket, LoadMode, true, IsSameT> {
  ...
  template <typename ArgType, typename Device>
  static EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE TargetPacket run(const TensorEvaluator<ArgType, Device>& impl, Index index) {
    i32k SrcCoeffRatio = internal::type_casting_traits<SrcType, TargetType>::SrcCoeffRatio;
    i32k TgtCoeffRatio = internal::type_casting_traits<SrcType, TargetType>::TgtCoeffRatio;
...
template <typename SrcPacket, typename TargetPacket, i32 LoadMode>
struct PacketConv<SrcPacket, TargetPacket, LoadMode, /*ActuallyVectorize=*/false, /*IsSameT=*/true> {
  ...
  static EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE TargetPacket run(const TensorEvaluator<ArgType, Device>& impl, Index index) {
    EIGEN_ALIGN_MAX std::remove_const_t<TargetType> values[PacketSize];
    for (i32 i = 0; i < PacketSize; ++i) values[i] = impl.coeff(index+i);
...
struct TensorEvaluator<const TensorConversionOp<TargetType, ArgType>, Device>
{
  ...
  packet(Index index) const {
    ...
    const bool Vectorizable =
        IsSameType
        ? TensorEvaluator<ArgType, Device>::PacketAccess
        : i32(TensorEvaluator<ArgType, Device>::PacketAccess) &
          i32(internal::type_casting_traits<SrcType, TargetType>::VectorizedCast);
