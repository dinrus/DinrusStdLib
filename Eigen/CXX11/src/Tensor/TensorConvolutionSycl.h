...
struct TensorEvaluator<const TensorConvolutionOp<Indices, InputArgType, KernelArgType>, Eigen::SyclDevice> {
  ...
  static constexpr i32 Layout = TensorEvaluator<InputArgType, Eigen::SyclDevice>::Layout;
  enum {
  ...
  TensorEvaluator(const XprType &op, const Eigen::SyclDevice &device)
      : m_inputImpl(op.inputExpression(), device),
        ...
        m_device(device) {
    EIGEN_STATIC_ASSERT((static_cast<i32>(TensorEvaluator<InputArgType, Eigen::SyclDevice>::Layout) ==
                         static_cast<i32>(TensorEvaluator<KernelArgType, Eigen::SyclDevice>::Layout)),
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE void executeEval(EvaluatorPointerType data) const {
    ...
    switch (NumKernelDims) {
      ...
      case 2: {
        auto kernel_index = std::array<size_t, 2>{static_cast<i32>(Layout) == static_cast<i32>(ColMajor) ? 0 : 1,
      ...
      case 3: {
        auto kernel_index = std::array<size_t, 3>{static_cast<i32>(Layout) == static_cast<i32>(ColMajor) ? 0 : 2,
