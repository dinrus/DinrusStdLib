...
struct type_list { constexpr static i32 count = sizeof...(tt); };

template<typename t, typename... tt>
struct type_list<t, tt...> { constexpr static i32 count = sizeof...(tt) + 1; typedef t first_type; };
 ...
 * equivalencies:
 *     constructor                                              result
 *     typename gen_numeric_list<i32, 5>::type                  numeric_list<i32, 0,1,2,3,4>
 *     typename gen_numeric_list_reversed<i32, 5>::type         numeric_list<i32, 4,3,2,1,0>
 *     typename gen_numeric_list_swapped_pair<i32, 5,1,2>::type numeric_list<i32, 0,2,1,3,4>
 *     typename gen_numeric_list_repeated<i32, 0, 5>::type      numeric_list<i32, 0,0,0,0,0>
 */
...
template<i32 n, typename x> struct take;
template<i32 n, typename a, typename... as> struct take<n, type_list<a, as...>> : concat<type_list<a>, typename take<n-1, type_list<as...>>::type> {};
template<i32 n>                             struct take<n, type_list<>>         { typedef type_list<> type; };
...
template<typename T, i32 n, T a, T... as> struct take<n, numeric_list<T, a, as...>> : concat<numeric_list<T, a>, typename take<n-1, numeric_list<T, as...>>::type> {};
// XXX The following breaks in gcc-11, and is invalid anyways.
// template<typename T, i32 n>               struct take<n, numeric_list<T>>           { typedef numeric_list<T> type; };
...
template<typename T, i32 n, T... ii>      struct h_skip_helper_numeric;
template<typename T, i32 n, T i, T... ii> struct h_skip_helper_numeric<T, n, i, ii...> : h_skip_helper_numeric<T, n-1, ii...> {};
template<typename T, T i, T... ii>        struct h_skip_helper_numeric<T, 0, i, ii...> { typedef numeric_list<T, i, ii...> type; };
template<typename T, i32 n>               struct h_skip_helper_numeric<T, n>           { typedef numeric_list<T> type; };
template<typename T>                      struct h_skip_helper_numeric<T, 0>           { typedef numeric_list<T> type; };

template<i32 n, typename... tt>             struct h_skip_helper_type;
template<i32 n, typename t, typename... tt> struct h_skip_helper_type<n, t, tt...> : h_skip_helper_type<n-1, tt...> {};
template<typename t, typename... tt>        struct h_skip_helper_type<0, t, tt...> { typedef type_list<t, tt...> type; };
template<i32 n>                             struct h_skip_helper_type<n>           { typedef type_list<> type; };
...
template<i32 n, typename a> struct skip { typedef decltype(h_skip<n>::helper(a())) type; };

template<i32 start, i32 count, typename a> struct slice : take<count, typename skip<start, a>::type> {};

/* list manipulation: retrieve single element from list */

template<i32 n, typename x> struct get;

template<i32 n, typename a, typename... as>               struct get<n, type_list<a, as...>>   : get<n-1, type_list<as...>> {};
template<typename a, typename... as>                      struct get<0, type_list<a, as...>>   { typedef a type; };

template<typename T, i32 n, T a, T... as>                        struct get<n, numeric_list<T, a, as...>>   : get<n-1, numeric_list<T, as...>> {};
template<typename T, T a, T... as>                               struct get<0, numeric_list<T, a, as...>>   { constexpr static T value = a; };

template<std::size_t n, typename T, T a, T... as> constexpr T       array_get(const numeric_list<T, a, as...>&) {
   return get<(i32)n, numeric_list<T, a, as...>>::value;
}
...
template<typename a, typename b> struct is_same_gf : is_same<a, b> { constexpr static i32 global_flags = 0; };
...
template<
  ...
  i32 default_flags = 0,
  bool last_check_positive = false,
  i32 last_check_flags = default_flags
>
...
template<
  ...
  i32 last_check_flags
>
struct contained_in_list_gf<test, check_against, type_list<EIGEN_TPL_PP_SPEC_HACK_USE(empty)>, default_flags, false, last_check_flags> { constexpr static bool value = false; constexpr static i32 global_flags = default_flags; };
...
{
  EIGEN_DEVICE_FUNC constexpr static EIGEN_STRONG_INLINE i32 run() { return Reducer::Identity; }
};
...
struct sum_op           {
  ...
  static constexpr i32 Identity = 0;
};
struct product_op       {
  template<typename A, typename B> EIGEN_DEVICE_FUNC constexpr static EIGEN_STRONG_INLINE auto run(A a, B b) -> decltype(a * b)   { return a * b;   }
  static constexpr i32 Identity = 1;
};
...
template<typename Array, i32... n>
constexpr EIGEN_STRONG_INLINE Array h_array_reverse(Array arr, numeric_list<i32, n...>)
{
...
{
  return h_array_reverse(arr, typename gen_numeric_list<i32, N>::type());
}
...
template<typename Op, typename A, typename B, std::size_t N, i32... n>
constexpr EIGEN_STRONG_INLINE array<decltype(Op::run(A(), B())),N> h_array_zip(array<A, N> a, array<B, N> b, numeric_list<i32, n...>)
{
...
{
  return h_array_zip<Op>(a, b, typename gen_numeric_list<i32, N>::type());
}

/* zip an array and reduce the result */

template<typename Reducer, typename Op, typename A, typename B, std::size_t N, i32... n>
constexpr EIGEN_STRONG_INLINE auto h_array_zip_and_reduce(array<A, N> a, array<B, N> b, numeric_list<i32, n...>) -> decltype(reduce<Reducer, typename id_numeric<i32,n,decltype(Op::run(A(), B()))>::type...>::run(Op::run(array_get<n>(a), array_get<n>(b))...))
{
  return reduce<Reducer, typename id_numeric<i32,n,decltype(Op::run(A(), B()))>::type...>::run(Op::run(array_get<n>(a), array_get<n>(b))...);
}

template<typename Reducer, typename Op, typename A, typename B, std::size_t N>
constexpr EIGEN_STRONG_INLINE auto array_zip_and_reduce(array<A, N> a, array<B, N> b) -> decltype(h_array_zip_and_reduce<Reducer, Op, A, B, N>(a, b, typename gen_numeric_list<i32, N>::type()))
{
  return h_array_zip_and_reduce<Reducer, Op, A, B, N>(a, b, typename gen_numeric_list<i32, N>::type());
}

/* apply stuff to an array */

template<typename Op, typename A, std::size_t N, i32... n>
constexpr EIGEN_STRONG_INLINE array<decltype(Op::run(A())),N> h_array_apply(array<A, N> a, numeric_list<i32, n...>)
{
...
{
  return h_array_apply<Op>(a, typename gen_numeric_list<i32, N>::type());
}

/* apply stuff to an array and reduce */

template<typename Reducer, typename Op, typename A, std::size_t N, i32... n>
constexpr EIGEN_STRONG_INLINE auto h_array_apply_and_reduce(array<A, N> arr, numeric_list<i32, n...>) -> decltype(reduce<Reducer, typename id_numeric<i32,n,decltype(Op::run(A()))>::type...>::run(Op::run(array_get<n>(arr))...))
{
  return reduce<Reducer, typename id_numeric<i32,n,decltype(Op::run(A()))>::type...>::run(Op::run(array_get<n>(arr))...);
}

template<typename Reducer, typename Op, typename A, std::size_t N>
constexpr EIGEN_STRONG_INLINE auto array_apply_and_reduce(array<A, N> a) -> decltype(h_array_apply_and_reduce<Reducer, Op, A, N>(a, typename gen_numeric_list<i32, N>::type()))
{
  return h_array_apply_and_reduce<Reducer, Op, A, N>(a, typename gen_numeric_list<i32, N>::type());
}

/* repeat a value n times (and make an array out of it
 * usage:
 *   array<i32, 16> = repeat<16>(42);
 */

template<i32 n>
struct h_repeat
{
  template<typename t, i32... ii>
  constexpr static EIGEN_STRONG_INLINE array<t, n> run(t v, numeric_list<i32, ii...>)
  {
    return {{ typename id_numeric<i32, ii, t>::type(v)... }};
  }
};

template<i32 n, typename t>
constexpr array<t, n> repeat(t v) { return h_repeat<n>::run(v, typename gen_numeric_list<i32, n>::type()); }
