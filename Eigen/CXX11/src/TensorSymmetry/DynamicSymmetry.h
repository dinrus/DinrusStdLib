...
class DynamicSGroup
{
  public:
    ...
    void add(i32 one, i32 two, i32 flags = 0);

    template<typename Gen_>
    inline void add(Gen_) { add(Gen_::One, Gen_::Two, Gen_::Flags); }
    inline void addSymmetry(i32 one, i32 two) { add(one, two, 0); }
    inline void addAntiSymmetry(i32 one, i32 two) { add(one, two, NegationFlag); }
    inline void addHermiticity(i32 one, i32 two) { add(one, two, ConjugationFlag); }
    inline void addAntiHermiticity(i32 one, i32 two) { add(one, two, NegationFlag | ConjugationFlag); }
    ...
    inline RV apply(const std::array<Index, N>& idx, RV initial, Args&&... args) const
    {
      ...
      for (std::size_t i = 0; i < size(); i++)
        initial = Op::run(h_permute(i, idx, typename internal::gen_numeric_list<i32, N>::type()), m_elements[i].flags, initial, std::forward<Args>(args)...);
      return initial;
    }
    ...
    inline i32 globalFlags() const { return m_globalFlags; }
  ...
  private:
    ...
    struct Generator {
      ...
      i32 two;
      i32 flags;
      constexpr inline Generator(i32 one_, i32 two_, i32 flags_) : one(one_), two(two_), flags(flags_) {}
    };
    ...
    template<typename Index, std::size_t N, i32... n>
    inline std::array<Index, N> h_permute(std::size_t which, const std::array<Index, N>& idx, internal::numeric_list<i32, n...>) const
    {
    ...
    inline GroupElement ge(Generator const& g) const
    {
      ...
      for (std::size_t k = 0; k < m_numIndices; k++) {
        if (k == (std::size_t)g.one)
          ...
          result.representation.push_back(i32(k));
      }
      return result;
    }
    ...
    inline i32 findElement(GroupElement e) const
    {
    ...
    void updateGlobalFlags(i32 flagDiffOfSameGenerator);
};
...
inline DynamicSGroup::GroupElement DynamicSGroup::mul(GroupElement g1, GroupElement g2) const
{
  ...
  for (std::size_t i = 0; i < m_numIndices; i++) {
    i32 v = g2.representation[g1.representation[i]];
    eigen_assert(v >= 0);
...
inline void DynamicSGroup::add(i32 one, i32 two, i32 flags)
{
  eigen_assert(one >= 0);
  eigen_assert(two >= 0);
...
inline void DynamicSGroup::updateGlobalFlags(i32 flagDiffOfSameGenerator)
{
