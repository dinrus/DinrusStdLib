...
class Barrier {
 public:
  Barrier(u32 count) : state_(count << 1), notified_(false) {
    eigen_plain_assert(((count << 1) >> 1) == count);
  }
  ~Barrier() { eigen_plain_assert((state_ >> 1) == 0); }

  void Notify() {
    u32 v = state_.fetch_sub(2, std::memory_order_acq_rel) - 2;
    if (v != 1) {
  ...
  void Wait() {
    u32 v = state_.fetch_or(1, std::memory_order_acq_rel);
    if ((v >> 1) == 0) return;
    std::unique_lock<std::mutex> l(mu_);
    while (!notified_) {
      cv_.wait(l);
 ...
 private:
  ...
  std::atomic<u32> state_;  // low bit is waiter flag
  bool notified_;
};
