...
class ThreadLocal {
 ...
 public:
  explicit ThreadLocal(i32 capacity)
      : ThreadLocal(capacity, internal::ThreadLocalNoOpInitialize<T>(),
                    internal::ThreadLocalNoOpRelease<T>()) {}

  ThreadLocal(i32 capacity, Initialize initialize)
      : ThreadLocal(capacity, std::move(initialize),
                    internal::ThreadLocalNoOpRelease<T>()) {}

  ThreadLocal(i32 capacity, Initialize initialize, Release release)
 ...
 private:
  ...
  std::atomic<i32> filled_records_;
