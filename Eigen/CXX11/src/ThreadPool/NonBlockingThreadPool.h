...
class ThreadPoolTempl : public Eigen::ThreadPoolInterface {
 public:
  ...
  ThreadPoolTempl(i32 num_threads, Environment env = Environment())
      : ThreadPoolTempl(num_threads, true, env) {}

  ThreadPoolTempl(i32 num_threads, bool allow_spinning,
  ...
  void ScheduleWithHint(std::function<void()> fn, i32 start,
                        i32 limit) override {
    ...
    if (pt->pool == this) {
      ...
      i32 num_queues = limit - start;
      i32 rnd = Rand(&pt->rand) % num_queues;
  ...
  i32 NumThreads() const EIGEN_FINAL { return num_threads_; }

  i32 CurrentThreadId() const EIGEN_FINAL {
 ...
 private:
  // Create a single atomic<i32> that encodes start and limit information for
  // each thread.
  ...
  static i32k kMaxPartitionBits = 16;
  static i32k kMaxThreads = 1 << kMaxPartitionBits;
  ...
  void AssertBounds(i32 start, i32 end) {
  ...
  inline unsigned GetStealPartition(i32 i) {
    return thread_data_[i].steal_partition.load(std::memory_order_relaxed);
  }

  void ComputeCoprimes(i32 N, MaxSizeVector<unsigned>* coprimes) {
    for (i32 i = 1; i <= N; i++) {
      unsigned a = i;
      unsigned b = N;
  ...
  struct PerThread {
    ...
    i32 thread_id;          // Worker thread index in pool.
#ifndef EIGEN_THREAD_LOCAL
  ...
  void WorkerLoop(i32 thread_id) {
    ...
    if (num_threads_ == 1) {
      ...
      while (!cancelled_) {
        Task t = q.PopFront();
        for (i32 i = 0; i < spin_count && !t.f; i++) {
