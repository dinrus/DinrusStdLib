...
i32 ToolWin::GetTitleCy() const
{
	return max(GetStdFontCy() + DPI(4), DPI(16));
}

i32 ToolWin::GetBorder() const
{
	return DPI(2);
}
...
Image ToolWin::FrameMouseEvent(i32 event, Point p, i32 zdelta, dword keyflags)
{
	if(GetParent())
		return Image::Arrow();
	switch(event) {
