...
i32 DockCont::DockContHandle::GetHandleSize(const DockableCtrl::Style &s) const
{
...
void DockCont::DockContHandle::Paint(Draw& w)
{
    if (IsShown() && dc) {
        ...
        if (!IsNull(s.title_font)) {
            ...
            w.Clip(r);
            WTxt text = IsNull(dc->GetGroup()) ? dc->GetTitle() : (WTxt)Format("%s (%s)", dc->GetTitle(), dc->GetGroup());
...
bool DockCont::Key(dword key, i32 count)
{
    if (!IsDocked())
...
void DockCont::Animate(Rect target, i32 ticks, i32 interval)
{
...
void DockCont::TabDragged(i32 ix)
{
    if (base->IsLocked())
        return;
    if (ix >= 0) {
        // Special code needed
        Val v = tabbar.GetKey(ix);
        if (IsDockCont(v)) {
...
void DockCont::TabContext(i32 ix)
{
    MenuBar         bar;
    DockContMenu    menu(base);
...
void DockCont::TabClosed0(Val v)
{
    if (IsDockCont(v)) {
        DockCont *c = ContCast(v);
        c->Remove();
...
void DockCont::TabsClosed(ValArray vv)
{
    for (i32 i = vv.GetCount()-1; i >= 0 ; --i)
        TabClosed0(vv[i]);
    Layout();
    if (tabbar.GetCount() == 1)
        RefreshLayout();
...
void DockCont::SortTabs(ValOrder &sorter)
{
    tabbar.SortTabVals(sorter);
}
...
void DockCont::SortTabsOnce(ValOrder &sorter)
{
    tabbar.SortTabValsOnce(sorter);
}
...
void DockCont::AutoHideAlign(i32 align)
{
    ASSERT(base);
    if (IsAutoHide()) {
...
void DockCont::AddFrom(DockCont& cont, i32 except)
{
    bool all = except < 0 || except >= cont.GetCount();
    i32 ix = GetCount();
    for (i32 i = cont.GetCount() - 1; i >= 0; i--)
        if (i != except) {
...
void DockCont::Clear()
{
    if (tabbar.GetCount()) {
        ...
        for (i32 i = 0; i < tabbar.GetCount(); i++)
            v[i] = tabbar.GetKey(i);
        tabbar.Clear();
...
void DockCont::SignalStateChange(i32 from, i32 to)
{
    for (i32 i = to; i >= from; i--) {
...
//  if (!handle.IsShown()) return;
    if (base) {
        Ctrl::LogPos lp;
        const DockableCtrl::Style& s = dc.GetStyle();
        i32 btnsize = handle.GetHandleSize(s) - 3;
...
void DockCont::SyncTabs(i32 align, bool text)
{
...
void DockCont::GetGroups(Vec<Txt>& groups)
{
    for (i32 i = 0; i < tabbar.GetCount(); i++) {
        Val v = tabbar.GetKey(i);
        if (IsDockCont(v))
...
i32 DockCont::GetDockAlign() const
{
    return base->GetDockAlign(*this);
}

i32 DockCont::GetAutoHideAlign() const
{
    return base->GetAutoHideAlign(*this);
}

bool DockCont::IsDockAllowed(i32 align, i32 dc_ix) const
{
...
bool DockCont::IsDockAllowed0(i32 align, const Val& v) const
{
    return IsDockCont(v) ? ContCast(v)->IsDockAllowed(align, -1) : base->IsDockAllowed(align, *DockCast(v));
}

DockableCtrl * DockCont::Get0(i32 ix) const
{
...
WTxt DockCont::GetTitle(bool force_count) const
{
    if ((IsTabbed() || force_count) && tabbar.GetCount() > 1)
        return (WTxt)Format("%s (%d/%d)", GetCurrent().GetTitle(), tabbar.GetCursor()+1, tabbar.GetCount());
...
void DockCont::ChildTitleChanged(Ctrl *child, WTxt title, Image icon)
{
    for (i32 i = 0; i < tabbar.GetCount(); i++)
...
void DockCont::Serialize(Stream& s)
{
    i32 ctrl = 'D';
    i32 cont = 'C';
    const Vec<DockableCtrl *>& dcs = base->GetDockableCtrls();
    ...
    if (s.IsStoring()) {
        ...
        for (i32 i = GetCount() - 1; i >= 0; i--) {
            if (IsDockCont(tabbar.GetKey(i)))
                ...
                i32 ix = base->FindDocker(dc);
                s / ctrl / ix;
            }
        }
        i32 cursor = tabbar.GetCursor();
        i32 groupix = tabbar.GetGroup();
        s / cursor / groupix;
    }
    else {
        i32 cnt;
...
void DockCont::DockContMenu::MenuDock(i32 align, DockableCtrl *dc)
{
    cont->Dock(align);
}
...
void DockCont::DockContMenu::MenuAutoHide(i32 align, DockableCtrl *dc)
{
...
void DockCont::Lock(bool lock)
{
    ...
    tabbar.WhenDrag         = lock ? Callback1<i32>() : OTVET(TabDragged);
    tabbar.WhenContext      = lock ? Callback1<i32>() : OTVET(TabContext);
...
DockCont::DockCont()
{
    ...
    tabbar.SortTabVals(Single<StdValOrder>());
    tabbar.SortTabs(false);

    tabbar.AutoHideMin(1);
    tabbar<<= OTVET(TabSelected);
    tabbar.WhenClose = OTVET(TabClosed);
    tabbar.WhenCloseSome = OTVET(TabsClosed);
    tabbar.SetBottom();

    WhenClose = OTVET(CloseAll);

    handle << close << autohide << windowpos;
    handle.WhenContext = OTVET(WindowMenu);
    handle.WhenLeftDrag = OTVET(MoveBegin);
    handle.WhenLeftDouble = OTVET(RestoreCurrent);
    close.Tip(t_("Закрыть"))                 <<= OTVET(CloseAll);
    autohide.Tip(t_("Автоматически скрыть"))         <<= OTVET(AutoHide);
    windowpos.Tip(t_("Оконное Меню"))    <<= OTVET(WindowMenu);
