...
/*
i32 DockPane::ClientToPos(const Size& p)
{
    i32 w = (width>>1)*pos.GetCount()-1;
    return minmax(vert ? 10000 * p.cy / (GetSize().cy - w) : 10000 * p.cx / (GetSize().cx - w), 0, 9999);
}
*/
void DockPane::CumulativePos(Vec<i32>& p) const
{
    for (i32 i = 1; i < p.GetCount()-1; i++)
        p[i] += p[i-1];
    p.Top() = 10000;
}

void DockPane::NonCumulativePos(Vec<i32>& p) const
{
    for (i32 i = p.GetCount()-1; i > 0; i--)
        p[i] -= p[i-1];
}

void DockPane::StartAnimate(i32 ix, Size sz, bool restore)
{
    if (restore) {
        ...
        i32 tsz = ClientToPos(sz);
        animpos <<= pos;
        if (tsz) {
            i32 msz = GetMinPos(ix);
            if (msz < 10000 && msz+tsz > 10000) {
                Ctrl *c = GetIndexChild(ix);
                i32 min = ClientToPos(c->GetMinSize());
                if (min < tsz) {
                    i32 std = ClientToPos(c->GetStdSize());
                    tsz = (std < tsz) ? std : min;
                }
            }
    ...
    animtick = 0;
    SetTimeCallback(-animinterval, OTVET(AnimateTick), TIMEID_ANIMATE);
    AnimateTick();
}

void DockPane::AnimateTick()
{
    for (i32 i = 0; i < animpos.GetCount(); i++)
...
void DockPane::SmartReposDown(Vec<i32>& p, i32 ix)
{
    i32 n = 0;
    i32 dif = 0;
    i32 maxsize = 0;
        ...
        while (dif > 1) {
            ...
            for (i32 i = 0; i < cnt; i++) {
                if (i != ix && p[i] < maxpos[i]) {
                    i32 t = max(1, min((p[i]*dif) / totalsize, maxpos[i] - p[i]));
                    p[i] += t;
                    sum += t;
                }
            }
...
void DockPane::SmartReposUp(Vec<i32>& p, i32 ix, i32 sz)
{
    i32 n = 0;
    i32 dif = 0;
    i32 minsize = 0;
    ...
    i32 totalsize = 0;
    i32 cnt = p.GetCount();
    i32 resizemin = max(10000 / (cnt*4), PosToClient(30));
    Vec<i32> minpos;
    Vec<i32> maxpos;
    maxpos.SetCount(cnt);
    minpos.SetCount(cnt);
...
void DockPane::SimpleRepos(Vec<i32>& p, i32 ix, i32 inc)
{
    i32 cnt = p.GetCount();
    if (cnt == 1) {
        p[0] = 10000;
...
i32 DockPane::GetMinPos(i32 notix)
{
    i32 n = 0;
    i32 msz = 0;
    for (Ctrl *c = GetFirstChild(); c; c = c->GetNext()) {
...
/*
i32 DockPane::FindIndex(Ctrl& child)
{
    i32 ix = 0;
    for (Ctrl *c = GetFirstChild(); c; c = c->GetNext()) {
...
Ctrl * DockPane::FindCtrl(i32 ix)
{
    Ctrl *c = GetFirstChild();
    for (i32 i = 0; i < ix && c; i++)
        c = c->GetNext();
    return c;
}
...
*/
void DockPane::Dock(Ctrl& newctrl, Size sz, i32 ps, bool animate, bool save)
{
    if (IsAnimating())
        EndAnimate();
    ...
    i32 tsz = cnt ? ClientToPos(sz) : 10000;
    ps = min(ps, cnt);
    if (ps >= 0) {
        if (ps && ps == cnt)
...
Rect DockPane::GetFinalAnimRect(Ctrl& ctrl)
{
    ...
    if (!IsAnimating())
        return ctrl.GetRect();
    i32 ix = GetChildIndex(&ctrl);
    ASSERT(ix >= 0 && ix < animpos.GetCount());

    Rect r = GetRect();
    i32 prev = ix ? animpos[ix-1] : 0;
    i32 width = GetSplitWidth();
    if (IsHorz()) {
