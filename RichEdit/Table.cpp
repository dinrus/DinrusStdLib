...
void RichEdit::SaveTableFormat(i32 table)
{
	AddUndo(MakeOne<UndoTableFormat>(text, table));
}

void RichEdit::SaveTable(i32 table)
{
...
void RichEdit::InsertTable()
{
	...
	i32 nx = minmax((i32)~dlg.columns, 1, 20);
	for(i32 q = nx; q--;)
		fmt.column.Add(1);
	if(dlg.header)
		fmt.header = 1;
	RichTable table;
	table.SetFormat(fmt);
	for(i32 i = 0; i < (dlg.header ? 2 : 1); i++)
		for(i32 j = 0; j < nx; j++) {
			RichText h;
...
template <class T>
struct CtrlRetrieveItemValNN : public CtrlRetriever::Item {
	Ctrl& ctrl;
	T&    value;
	...
	CtrlRetrieveItemValNN(Ctrl& ctrl, T& value) : ctrl(ctrl), value(value) {}
};

template <class T>
void Advn(CtrlRetriever& r, Ctrl& ctrl, T& value) {
	ctrl <<= value;
	r.Put(MakeOne<CtrlRetrieveItemValNN<T>>(ctrl, value));
}

void RichEdit::DestroyTable()
{
	AddUndo(MakeOne<UndoDestroyTable>(text, cursorp.table));
	i32 c = text.GetCellPos(cursorp.table, 0, 0).pos;
	text.DestroyTable(cursorp.table);
	Move(c);
}

wchar CharFilterEqualize(wchar c)
{
...
void RichEdit::TableProps()
{
	...
	Txt ratios;
	for(i32 i = 0; i < fmt.column.GetCount(); i++) {
		if(i)
			ratios << ':';
		ratios << "1";
	}
	...
	for(;;) {
		...
		default:
			...
			Point violator(0, 0);
			i32 vspan = 0;
			for(i32 rw = 0; valid && rw < fmt.header && rw < tbl.GetRows(); rw++)
				for(i32 co = 0; valid && co < tbl.GetColumns(); co++)
			...
			if(dlg.equalize) {
				Vec<Txt> r = Split((Txt)~dlg.ratios, ':');
				for(i32 i = 0; i < fmt.column.GetCount(); i++)
...
bool RichEdit::RemoveSpecial(i32 ll, i32 hh, bool back)
{
	NextUndo();
	i32 l = min(ll, hh);
	i32 h = max(ll, hh);
