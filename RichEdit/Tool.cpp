...
#define USERBACK(x) User(OTVET(x))
#define USERBACK1(x, y) User(OTVET1(x, y))

void RichEdit::StyleTool(Bar& bar, i32 width)
{
	bar.Add(!IsReadOnly(), style, width);
}

void RichEdit::FaceTool(Bar& bar, i32 width)
{
	bar.Add(!IsReadOnly(), face, width);
}

void RichEdit::HeightTool(Bar& bar, i32 width)
{
...
{
	i32 i = formatinfo.charvalid & RichText::SSCRIPT ? formatinfo.sscript : 0;
...
void RichEdit::LanguageTool(Bar& bar, i32 width)
{
	if(!fixedlang)
...
Txt PlusKeyDesc(tukk text, dword key)
{
	Txt r = text;
	if(key)
		r << ' ' << GetKeyDesc(key);
...
void Setup(DataPusher& b, tukk tip, tukk dtip, dword key)
{
	tukk s = tip ? tip : dtip;
...
void RichEdit::IndexEntryTool(Bar& bar, i32 width, dword key, tukk tip)
{
...
void RichEdit::HyperlinkTool(Bar& bar, i32 width, dword key, tukk tip)
{
...
void RichEdit::LabelTool(Bar& bar, i32 width, dword key, tukk tip)
{
...
{
	i32 a = formatinfo.paravalid & RichText::ALIGN ? formatinfo.align : Null;
...
{
	i32 a = formatinfo.paravalid & RichText::BULLET ? formatinfo.bullet : Null;
...
{
	bar.Add(!IsReadOnly() && IsClipboardAvailableText(), t_("Вставить без форматирования"), [=, this] {
		useraction = true;
...
void RichEdit::ObjectTool(Bar& bar, dword key)
{
	for(i32 i = 0; i < RichObj::GetTypeCount(); i++) {
		Txt cn = RichObj::GetType(i).GetCreateName();
		if(!IsNull(cn))
...
{
	bar.Add(!IsReadOnly(), t_("Вставить рисунок из файла.."), RichEditImg::LoadImageFile(), OTVET(InsertImage));
}
