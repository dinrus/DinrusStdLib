...
void RichEdit::FinishNF()
{
	...
	SetZsc();
	PageY top, bottom;
	i32 sell = min(cursor, anchor);
	i32 selh = max(cursor, anchor);
	if(tablesel)
		Refresh();
	else
...
void RichEdit::MoveNG(i32 newpos, bool select)
{
	...
	Finish();
	if(select)
		SetSelectionSource(Txt().Cat() << "text/QTF;Rich Text Format;text/rtf;application/rtf;"
		                   << ClipFmtsText());
	CancelMyPreedit();
}

void RichEdit::Move(i32 newpos, bool select)
{
	MoveNG(newpos, select);
	gx = cursorc.left;
}

void RichEdit::MoveUpDown(i32 dir, bool select, i32 pg)
{
	Rect page = pagesz;
	...
	i32 c = text.GetVertMove(min(GetLength(), cursor), gx, page, dir);
	if(c >= 0)
		MoveNG(c, select);
	else
...
void RichEdit::MovePageUpDown(i32 dir, bool select)
{
...
void RichEdit::MoveHomeEnd(i32 dir, bool select)
{
	i32 c = cursor;
...
void RichEdit::SelCell(i32 dx, i32 dy)
{
...
bool RichEdit::CursorKey(dword key, i32 count)
{
...
bool RichEdit::GetSelection(i32& l, i32& h) const
{
	if(IsSelection()) {
		l = min(anchor, cursor);
		h = max(anchor, cursor);
...
bool RichEdit::InSelection(i32& c) const
{
	i32 sell, selh;
...
void    RichEdit::GetWordAtCursorPos(i32& pos, i32& count)
{
	WTxt w;
	i32 c = cursor;
	pos = count = 0;
...
void RichEdit::GotoType(i32 type, Ctrl& l)
{
	Vec<RichValPos> f = text.GetValPos(pagesz, type);
	gototable.Clear();
	for(i32 i = 0; i < f.GetCount(); i++) {
		const RichValPos& q = f[i];
		i32 endpos = q.pos;
		if(type == RichText::INDEXENTRIES) {
			WTxt ie = text.GetRichPos(endpos).format.indexentry;
...
bool RichEdit::GotoLabel(Gate<const WTxt&> match)
{
	Vec<RichValPos> f = text.GetValPos(pagesz, RichText::LABELS);
	for(i32 i = 0; i < f.GetCount(); i++)
		if(match(f[i].data)) {
			Move(f[i].pos);
			return true;
...
bool RichEdit::GotoLabel(const Txt& lbl)
{
	return GotoLabel([=](const WTxt& data) { return data == WTxt(lbl); });
}
