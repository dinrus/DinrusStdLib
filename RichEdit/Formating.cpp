...
void RichEdit::ApplyFormat(dword charvalid, dword paravalid)
{
	...
	if(IsSelection()) {
		if(tablesel) {
			...
			i32 l = min(cursor, anchor);
			i32 h = max(cursor, anchor);
...
{
	NextUndo();
	formatinfo.language = (i32)~language;
...
void RichEdit::Language()
{
	...
	if(d.Run() != IDOK)
		return;
	formatinfo.language = (i32)~d.lang;
	ApplyFormat(RichText::LANGUAGE);
	SetFocus();
	if(!language.HasKey((i32)~d.lang)) {
		Vec<i32> h;
...
void RichEdit::IndentMark()
{
	RichRuler::Marker m;
	i32 l = formatinfo.lm;
	i32 r = cursorc.textpage.Width() - formatinfo.rm;
...
void RichEdit::ShowFormat()
{
	...
	if(formatinfo.charvalid & RichText::LANG)
		language <<= (i32)formatinfo.language;
	else
		language <<= Null;
...
void RichEdit::HighLightTab(i32 r)
{
...
{
	Txt s = formatinfo.indexentry.ToTxt();
	Txt s0 = s;
	WhenIndexEntry(s);
	if(s != s0) {
		formatinfo.indexentry = s.ToWTxt();
...
void RichEdit::TabAdd(i32 align)
{
	RichPara::Tab tab;
...
{
	NextUndo();
	i32 r = ruler.GetTrack() - 3;
	if(r >= 0)
		HighLightTab(r);
	CallbackArgTarget<i32> align;
	CallbackArgTarget<i32> fill;
	MenuBar menu;
	...
	if(r >= 0) {
		i32 f = formatinfo.tab[r].fillchar;
		menu.Separator();
	...
	if(!IsNull(align)) {
		SaveFormat();
		if(r >= 0) {
			formatinfo.tab[r].align = (i32)align;
			SetParaFormat(RichText::TABS);
		}
		else
			TabAdd(align);
	...
	if(!IsNull(fill) && r >= 0) {
		...
		if(r >= 0) {
			if(fill == -1)
				...
				formatinfo.tab[r].fillchar = (i32)fill;
			SetParaFormat(RichText::TABS);
		}
	}
	Finish();
...
void  RichEdit::SetBullet(i32 bullet)
{
	NextUndo();
...
void RichEdit::AdjustObjectSize()
{
	...
	for(;;) {
		...
		case IDNO:
			...
			break;
		case IDOK:
			if(!IsNull(d.width) && (i32)~d.width > 0)
				sz.cx = ~d.width;
			if(!IsNull(d.height) && (i32)~d.height > 0)
				sz.cy = ~d.height;
			obj.SetSize(sz);
			if(!IsNull(d.ydelta))
