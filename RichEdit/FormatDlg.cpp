...
{
	bool b = !IsNull(bullet) && !(i32)~bullet;
...
i32  ParaFormatting::ComputeIndent()
{
	if(!IsNull(bullet) && (i32)~bullet)
		return 150;
	if(IsNumbering()) {
		RichPara::NumberFormat f = GetNumbering();
		RichPara::Number n;
		static u8 n0[] = { 0, 37, 38, 48, 48, 37, 37 };
		static u8 n1[] = { 0, 17, 18, 12, 12, 17, 17 };
		if(f.number[0] && f.number[0] < 8)
			n.n[0] = n0[f.number[0]];
		if(f.number[1] && f.number[1] < 8)
			n.n[1] = n1[f.number[1]];
		for(i32 i = 2; i < 8; i++) {
			static u8 nn[] = { 0,  7,  8,  1,  1,  7,  7 };
...
void ParaFormatting::Set(i32 unit, const RichText::FormatInfo& formatinfo, bool baselevel)
{
	...
	if(RichText::NUMBERING & formatinfo.paravalid) {
		before_number <<= formatinfo.before_number.ToWTxt();
		after_number <<= formatinfo.after_number.ToWTxt();
	...
	if(RichText::TABS & formatinfo.paravalid)
		for(i32 i = 0; i < formatinfo.tab.GetCount(); i++)
...
dword ParaFormatting::Get(RichText::FormatInfo& formatinfo)
{
	...
	if(!IsNull(align)) {
		static i32 sw[] = { ALIGN_LEFT, ALIGN_CENTER, ALIGN_RIGHT, ALIGN_JUSTIFY };
		formatinfo.align = sw[(i32)~align];
		v |= RichText::ALIGN;
	}
	if(!IsNull(page)) {
	...
	if((RichText::TABS & formatinfo.paravalid) || tabs.GetCount()) {
		...
		for(i32 i = 0; i < tabs.GetCount(); i++) {
			...
			tab.align = (i32)tabs.Get(i, 1);
			tab.fillchar = (i32)tabs.Get(i, 2);
...
ParaFormatting::ParaFormatting()
{
	...
	tabs.AddColumn(t_("Тип"), 2).Edit(tabtype).SetConvert(tabtype).InsertVal(ALIGN_LEFT);
	tabs.AddColumn(t_("Заливка"), 1).Edit(tabfill).SetConvert(tabfill).InsertVal(0);
	tabs.ColumnWidths("103 89 78");
	tabs.Appending().Removing().NoAskRemove();
	tabs.WhenAcceptEdit = tabs.WhenArrayAction = OTVET(SetMod);
	...
	for(i32 i = 0; i < 8; i++) {
		...
		list.Add(RichPara::NUMBER_I, "I, II, III");
		list <<= OTVET(SetupIndent);
	}
	before_number <<=
	after_number <<=
	reset_number <<=
	bullet <<= OTVET(SetupIndent);

	newhdrftr <<= OTVET(NewHdrFtr);
	hdrftr <<= OTVET(EditHdrFtr);
	SyncHdrFtr();
...
{
	bar.Add(t_("Создать новый стиль.."), OTVET(Create))
	   .Key(K_INSERT);
	bar.Add(t_("Удалить стиль"), OTVET(Remove))
	   .Key(K_DELETE);
...
void RichEdit::DisplayDefault::Paint(Draw& w, const Rect& r, const Val& q, Color ink, Color paper, dword style) const
{
	Txt text = q;
	w.DrawRect(r, paper);
...
void StyleManager::Set(const RichText& text, const Uuid& current)
{
	...
	i32 q = list.Find(RichStyle::GetDefaultId());
	if(q >= 0)
...
void StyleManager::Set(tukk qtf)
{
...
{
	SaveStyle();
	for(i32 i = 0; i < dirty.GetCount(); i++) {
		Uuid id = dirty[i];
		i32 q = style.Find(id);
		if(q >= 0)
...
Txt StyleManager::GetQTF()
{
	return AsQTF(Get());
}

void StyleManager::Setup(const Vec<i32>& faces, i32 aunit)
{
	unit = aunit;
	height.Clear();
	for(i32 i = 0; RichEdit::fh[i]; i++)
		height.AddList(RichEdit::fh[i]);
	face.ClearList();
	SetupFaceList(face);
	for(i32 i = 0; i < faces.GetCount(); i++)
		face.Add(faces[i]);
}
...
StyleManager::StyleManager()
{
	...
	list.WhenEnterRow = OTVET(EnterStyle);
	list.WhenKillCursor = OTVET(SaveStyle);
	list.WhenBar = OTVET(Menu);
	list.WhenAcceptEdit = OTVET(ReloadNextStyles);
	ink.NotNull();
	face <<= height <<= italic <<= bold <<= underline <<= strikeout <<= OTVET(SetupFont);
	Vec<i32> ffs;
	Vec<i32> ff;
	ff.Add(Font::ARIAL);
	ff.Add(Font::ROMAN);
