...
void StyleKeysDlg::Init(const RichEdit& edit, RichEdit::StyleKey *key)
{
	...
	for(i32 i = 0; i < 20; i++) {
		...
		list.Add((i >= 10 ? "Shift+Alt+" : "Alt+") + AsTxt(i % 10));

		DropList& st = style.At(i);
		st.Add(Null);
		const RichStyles& ts = text.GetStyles();
		for(i32 j = 0; j < ts.GetCount(); j++) {
		...
		fc.Add(Null);
		for(i32 j = 0; j < edit.face.GetCount(); j++)
		...
		hg.Add(Null);
		for(i32 j = 0; j < edit.height.GetCount(); j++)
...
void RichEdit::ApplyStyleKey(i32 i)
{
	if(i < 0 || i >= 20)
		return;
	const StyleKey& k = stylekey[i];
	if(!IsNull(k.styleid)) {
		i32 q = style.FindKey(k.styleid);
		if(q >= 0) {
			style.SetIndex(q);
			Style();
		}
