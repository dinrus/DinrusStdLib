...
Txt sParseArgs(tukk cmd, const Vec<Txt> *pargs)
{
	while(*cmd && (u8) *cmd <= ' ') ++cmd;
...
Vec<WCHAR> sEnvtoWCHAR(tukk envptr)
{
	Vec<WCHAR> env;
...
#ifdef  flagWIN10
bool Win32CreateProcess(tukk cmdptr, tukk envptr, STARTUPINFOEX& si, PROCESS_INFORMATION& pi, tukk cd)
{
	Vec<WCHAR> cmd = ToSystemCharsetW(cmdptr);
	cmd.Add(0);
...
HANDLE WinPtyCreateProcess(tukk cmdptr, tukk envptr, tukk cd, winpty_t* hConsole)
{
	Vec<WCHAR> cmd = ToSystemCharsetW(cmdptr);
	cmd.Add(0);
...
bool PtyProcess::DoStart(tukk cmd, const Vec<Txt> *args, tukk env, tukk cd)
{
	...
	Txt command = sParseArgs(cmd, args);
	if(command.IsEmpty()) {
	...
	size_t listsize;  // FIXME: Use drx's allocators (or drx::Buffer<>) here.
...
void PtyProcess::Write(Txt s)
{
	...
	if(hInputWrite) {
		bool ret = true;
		dword n = 0;
		for(i32 wn = 0; ret && wn < wbuffer.GetLength(); wn += n) {
