...
Txt CParserPlus::ReadIdPlus() {
    if(!IsId())
        ThrowError("отсутствует ид");
    Txt result;
    tukk b = term;
    tukk p = b;
    while(iscidplus(*p))
    ...
    term = p;
    DoSpaces();
    return Txt(b, static_cast<i32>(p - b));
}

Drawing EquationDraw::Text(Txt text, bool italic, i32 offsetX, i32 offsetY, double betw) {
    Font fnt;

    i32 _pos1;
    if ((_pos1 = text.Find('_', 0)) >= 0) {
        i32 _pos2;
        Txt sub = text.Right(text.GetCount()-_pos1-1);
    ...
    i32 width = 0;
    WTxt wtext(text);
    for (i32 i = 0; i < wtext.GetCount(); ++i)
        width += fi.GetWidth(wtext[i]);

    width += 10;
    width = i32(width*betw);
...
Drawing EquationDraw::SubSup(Drawing &drwText, Drawing &drwSub, Drawing &drwSup) {
    ...
    i32 width = szText.cx + max(szSup.cx, szSub.cx);
    i32 deltay = szSup.cy/2;
...
Drawing EquationDraw::SubSup(Txt text, Txt sub, Txt sup) {
...
Drawing EquationDraw::SubSup(Drawing &drwText, Txt sub, Txt sup) {
    Drawing subdraw  = Text(sub, true);
    Drawing supdraw  = Text(sup, true);
...
Drawing EquationDraw::SubSupInv(Txt text, Txt sub, Txt sup) {
    Drawing textdraw = Text(text, true);
    Drawing subdraw  = Text(sub, true);
    Drawing supdraw  = Text(sup, true);
...
Drawing EquationDraw::SubSupInv(Drawing &drwText, Txt sub, Txt sup) {
    Drawing subdraw  = Text(sub, true);
    Drawing supdraw  = Text(sup, true);
...
Drawing EquationDraw::JoinCenter(Drawing &left, Drawing &right) {
    ...
    i32 width = szLeft.cx + szRight.cx;
    i32 height = max(szLeft.cy, szRight.cy);
...
Drawing EquationDraw::JoinFlex(Drawing &left, double betw1, Drawing &right, double betw2) {
    ...
    i32 width = i32(szLeft.cx*betw1 + szRight.cx*betw2);
    i32 height = max(szLeft.cy, szRight.cy);

    DrawingDraw dw(width, height);

    dw.DrawDrawing(0,               0, szLeft.cx,  height, left);
    dw.DrawDrawing(i32(szLeft.cx*betw1), 0, szRight.cx, height, right);
...
Drawing EquationDraw::NumDenom(Drawing &num, Drawing &denom) {
    ...
    i32 width = max(szNum.cx, szDenom.cx);
    i32 height = szNum.cy + szDenom.cy;
...
Drawing EquationDraw::Sqrt(Drawing &right) {
    ...
    i32 width = 2*szLeft.cx + szRight.cx;
    i32 height = max(szLeft.cy, szRight.cy);
...
Drawing EquationDraw::Integral(Drawing &data, Drawing &sub, Drawing &sup) {
    ...
    i32 sqWidth = i32(szLeft.cx*1.5);
    i32 width = sqWidth + szRight.cx;
    i32 height = szRight.cy;

    DrawingDraw dw(width, height);

    dw.DrawDrawing(0,       0, static_cast<i32>(szLeft.cx*1.5),  height, left);
...
Drawing EquationDraw::Summat(Drawing &data, Drawing &sub, Drawing &sup) {
    ...
    i32 sqWidth = i32(szLeft.cx*1.3);
    i32 width = sqWidth + szRight.cx;
    i32 height = szRight.cy;

    DrawingDraw dw(width, height);

    dw.DrawDrawing(0, 0, static_cast<i32>(szLeft.cx*1.1),  height, left);
    dw.DrawDrawing(sqWidth, 0, szRight.cx, height, right);
...
Drawing EquationDraw::Function(Txt function, Drawing &content) {
...
Txt EquationDraw::ReplaceSymbols(Txt var) {
    for (i32 i = 0; i < symbols.GetCount(); ++i) {
        Txt letter = ToLower(symbols.GetKey(i));
...
Txt EquationDraw::TermTrig(CParserPlus& p) {
