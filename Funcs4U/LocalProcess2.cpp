...
#include <drx/Funcs4U/LocalProcess2.h>
...
#endif
#ifdef TARGET_POSIX
    LLOG("\nLocalProcess::Free, pid = " << (i32)getpid());
...
#ifdef TARGET_WIN32
bool Win32CreateProcess2(tukk command, tukk envptr, STARTUPINFOW& si, PROCESS_INFORMATION& pi, tukk cd)
{ // provides conversion of charset for cmdline and envptr
    Vec<WCHAR> cmd = ToSystemCharsetW(command);
    cmd.Add(0);
    ...
    if(envptr) {
        ...
        while(envptr[len] || envptr[len + 1])
            len++;
        WTxt wenv(envptr, len + 1);
        env.Alloc(len + 2);
        memcpy(env, wenv, (len + 2) * sizeof(wchar));
    }
#endif
    return CreateProcessW(NULL, cmd, NULL, NULL, TRUE, NORMAL_PRIORITY_CLASS, (uk )envptr,
                          cd ? ToSystemCharsetW(cd).begin() : NULL, &si, &pi);
}
#endif

bool LocalProcess2::DoStart(tukk _command, const Vec<Txt> *arg, bool spliterr, tukk envptr, tukk cd)
{
...
#ifdef TARGET_POSIX
    if(arg) {
        i32 n = strlen(command) + 1;
        for(i32 i = 0; i < arg->GetCount(); i++)
            n += (*arg)[i].GetCount() + 1;
        cmd_buf.Alloc(n + 1);
        tuk p = cmd_buf;
        args.Add(p);
        i32 l = strlen(command) + 1;
        memcpy(p, command, l);
        p += l;
        for(i32 i = 0; i < arg->GetCount(); i++) {
            args.Add(p);
    ...
    Txt app_full = GetFileOnPath(args[0], getenv("PATH"), true);
    if(IsNull(app_full))
        return false;
    ...
    pid = fork();
#endif
    LLOG("\tfork, pid = " << (i32)pid << ", getpid = " << (i32)getpid());
    if(pid < 0)
        return false;
//      throw Exc(NFormat(t_("fork() error; error code = %d"), errno));
    ...
    if (doublefork) {
        pid_t pid2 = fork();
        LLOG("\tfork2, pid2 = " << (i32)pid2 << ", getpid = " << (i32)getpid());
        if (pid2 < 0) {
            LLOG("fork2 failed");
            Exit(1);
        }
...
//  rpipe[1] = wpipe[0] = -1;
    ...
    if(envptr) {
        ...
        env.Add(NULL);
        execve(app_full, args.Begin(), (tuk const *)env.Begin());
    }
    else
...
#ifdef TARGET_POSIX
bool LocalProcess2::DecodeExitCode(i32 status)
{
    if(WIFEXITED(status)) {
...
#undef SIGDEF
        ...
        exit_string << "\nПроцесс " << (WIFSIGNALED(status) ? "прерван" : "остановлен") << " по сигналу " << sig;
...
#ifdef TARGET_WIN32
Vec<DWORD> GetChildProcessList(DWORD processId) {
    Vec<DWORD> child, all, parents;
...
void TerminateChildProcesses(DWORD dwProcessId, UINT uExitCode) {
    Vec<DWORD> children = GetChildProcessList(dwProcessId);
    for (i32 i = 0; i < children.GetCount(); ++i) {
...
i32  LocalProcess2::GetExitCode() {
#ifdef TARGET_WIN32
    return IsRunning() ? (i32)Null : exit_code;
#endif
#ifdef TARGET_POSIX
    if(!IsRunning())
...
Txt LocalProcess2::GetExitMessage() {
#ifdef TARGET_POSIX
...
bool LocalProcess2::Read(Txt& res) {
    Txt dummy;
    return Read(res, dummy);
}

bool LocalProcess2::Read(Txt& reso, Txt& rese)
{
    ...
    for (i32 wp=0; wp<2;wp++) {
        i32 *pipe = wp ? epipe : wpipe;
        if (pipe[0] < 0) {
            LLOG("Pipe["<<wp<<"] closed");
            continue;
        }
        ...
        while((sv = select(pipe[0]+1, set, NULL, NULL, &tval)) > 0) {
            LLOG("Read() -> select");
            char buffer[1024];
            i32 done = read(pipe[0], buffer, sizeof(buffer));
            LLOG("Read(), read -> " << done);
            if(done > 0)
...
void LocalProcess2::Write(Txt s)
{
    if(convertcharset)
        s = ToSystemCharset(s);
#ifdef TARGET_WIN32
    if (hInputWrite) {
        bool ret = true;
        dword n;
        for(i32 wn = 0; ret && wn < s.GetLength(); wn += n) {
    ...
    if (rpipe[1] >= 0) {
        i32 ret=1;
        for(i32 wn = 0; (ret > 0 || errno == EINTR) && wn < s.GetLength(); wn += ret) {
            Txt ho = wreso;
            Txt he = wrese;
            wreso = wrese = Null;
            Read2(wreso, wrese);
...
i32 LocalProcess2::Finish(Txt& out)
{
    out.Clear();
    while(IsRunning()) {
        Txt h = Get();
