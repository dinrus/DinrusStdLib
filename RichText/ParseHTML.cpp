...
struct TrivialHtmlParser {
	...
	RichPara   para;
	RichText   richtext;
	i32        prev_margin = 0; // for margin collapsing
	i32        parent_tag_margin = 0; // for margin (de)collapsing
	bool       wastext = false;
	...
	struct Style : RichPara::Format {
		...
		i32  bullet_style = RichPara::BULLET_ROUND;
		bool pre = false;
	};

	static VecMap<Txt, Color>  named_color;
	static VecMap<Txt, Txt> div;
	static VecMap<Txt, Txt> span;
	
	void ReadStyle(Style& fmt, const Txt& style);
	void AddPara(const Style& fmt);
	void Parse(Style fmt);
	
	TrivialHtmlParser(tukk html);
};

VecMap<Txt, Color> TrivialHtmlParser::named_color;
VecMap<Txt, Txt> TrivialHtmlParser::div;
VecMap<Txt, Txt> TrivialHtmlParser::span;

TrivialHtmlParser::TrivialHtmlParser(tukk html)
	: p(html) {
	ONCELOCK {
		div
			("div", "")
		...
		Txt html_colors = ZDecompress(html_colors_z, sizeof(html_colors_z));
		CParser p(html_colors);
		while(!p.IsEof()) {
			Txt id = p.ReadId();
			dword n = p.ReadNumber(16);
			named_color.Add(id, Color((u8)(n >> 16), (u8)(n >> 8), (u8)(n)));
		}
	}
	p.PreserveAllWhiteSpaces();
	p.Relaxed();
	p.RegisterEntity("nbsp", WTxt(0xa0, 1).ToTxt());
}

void TrivialHtmlParser::ReadStyle(Style& fmt, const Txt& style)
{
	...
	try {
		...
		auto ReadLengthPar = [&](i32& tsize, double psize = 2000) {
			double l;
			if(ReadLength(l, psize))
				tsize = (i32)l;
		};
		auto ReadLengthParAdd = [&](i32& tsize, double psize = 2000) {
			i32 n = 0;
		...
		auto ReadColor = [&](Color& c) {
			if(p.IsId()) {
				i32 q = named_color.Find(p.ReadId());
				if(q >= 0) {
					c = named_color[q];
					return true;
				}
		...
		auto ReadLengthPar4 = [&](Rect& r) {
			...
			switch(m.GetCount()) {
			case 1: r.left = r.right = r.top = r.bottom = (i32)m[0]; break;
			case 2: r.left = r.right = (i32)m[1]; r.top = r.bottom = (i32)m[0]; break;
			case 3: r.left = r.right = (i32)m[1]; r.top = (i32)m[0]; r.bottom = (i32)m[2]; break;
			case 4: r.left = (i32)m[3]; r.right = (i32)m[1]; r.top = (i32)m[0]; r.bottom = (i32)m[2]; break;
			default:;
			}
		};
		while(!p.IsEof()) {
...
//	para.Dump();
	...
	LLOG("AddPara " << AsCTxt(para.GetText().ToTxt()) << ", before: " << para.format.before << ", after: " << para.format.after);
	richtext.Cat(para);
	para.part.Clear();
	wastext = false;
}
	...
	while(!p.IsEnd() && !p.IsEof()) {
		if(p.IsText()) {
			...
			if(tfmt.pre) {
				...
				if(text.GetCount()) {
					WTxt h = text.ToWTxt();
					for(wtukk s = h; *s;) // ignore more than 1 space between words
						if(*s == ' ') {
							s++;
							while(*s == ' ') s++;
							t.Cat(' ');
...
RichText ParseTrivialHtml(tukk html, Font base_font)
{
