...
RichTxt::Para::Para(const Para& src, i32)
: object(src.object, 1)
{
	length = src.length;
	styleid = src.styleid;
...
RichTxt::Part::Part(const Part& src, i32) {
	if(src.Is<Para>())
...
i32 RichTxt::GetPartLength(i32 pi) const
{
	return part[pi].Is<RichTable>() ? part[pi].Get<RichTable>().GetLength() : part[pi].Get<Para>().length;
}

bool             RichTxt::IsTable(i32 i) const  { return part[i].Is<RichTable>(); }
const RichTable& RichTxt::GetTable(i32 i) const { return part[i].Get<RichTable>(); }


i32 RichTxt::FindPart(i32& pos) const
{
	i32 pi = 0;
	while(pi < part.GetCount()) {
		i32 l = GetPartLength(pi) + 1;
		if(pos < l)
			break;
		pos -= l;
		pi++;
...
i32  RichTxt::GetPartPos(i32 pi) const
{
	i32 pos = 0;
	for(i32 i = 0; i < pi; i++)
...
void RichTxt::SetRefresh(i32 parti)
{
	switch(r_type) {
	case NONE:
		r_parti = parti;
...
void RichTxt::SetRefreshFrom(i32 parti)
{
	r_type = FROM;
	if(r_type == NONE)
		r_parti = parti;
...
void RichTxt::Put(i32 i, const RichPara& p, const RichStyle& s)
{
	if(i >= part.GetCount() || !IsPara(i))
		part.At(i).Create<Para>();
	Para& pp = part[i].Get<Para>();
	i32 numbering = p.format.GetNumberLevel();
...
void RichTxt::Put(i32 i, const RichPara& p, const RichStyles& s)
{
	Put(i, p, GetStyle(s, p.format.styleid));
}

void RichTxt::Set(i32 i, const RichPara& p, const RichStyles& s)
{
	Put(i, p, s);
	Invalidate();
}

void RichTxt::Insert(i32 i, const RichPara& p, const RichStyles& s)
{
	part.Insert(i);
	Set(i, p, s);
}

void RichTxt::RemovePart(i32 parti)
{
	part.Remove(parti);
	Invalidate();
}

void RichTxt::SetPick(i32 i, RichTable&& p)
{
...
RichPara RichTxt::Get(i32 parai, const RichStyle& style, bool usecache) const
{
...
RichPara RichTxt::Get(i32 parai, const Uuid& styleid, const RichStyles& s, bool usecache) const
{
...
RichPara RichTxt::Get(i32 parti, const RichStyles& s, bool usecache) const
{
	return Get(parti, part[parti].Get<Para>().styleid, s, usecache);
}

RichPara RichTxt::Get(i32 parai, const RichStyle& style) const
{
	return Get(parai, style, false);
}

RichPara RichTxt::Get(i32 parai, const Uuid& styleid, const RichStyles& s) const
{
	return Get(parai, styleid, s, false);
}

RichPara RichTxt::Get(i32 i, const RichStyles& s) const
{
...
i32 RichTxt::ComputeLength() const
{
	i32 length = part.GetCount() - 1;
	for(i32 i = 0; i < part.GetCount(); i++)
		length += GetPartLength(i);
	return length;
}

i32 RichTxt::GetLength() const
{ // expects i32 to be atomic, worst 'race' is that it gets computed more times...
	if(length < 0)
		length = ComputeLength();
	return length;
}

i32  RichTxt::GetTableCount() const
{ // expects i32 to be atomic, worst 'race' is that it gets computed more times...
	if(tabcount < 0) {
		i32 n = 0;
		for(i32 i = 0; i < part.GetCount(); i++)
...
bool RichTxt::EvaluateFields(const RichStyles& s, VecMap<Txt, Val>& vars)
{
	bool b = false;
	for(i32 i = 0; i < part.GetCount(); i++)
		if(IsTable(i)) {
...
void RichTxt::SetParaStyle(i32 i, const Uuid& id)
{
	ASSERT(IsPara(i));
	Para& p = part[i].Get<Para>();
	p.styleid = id;
...
void RichTxt::GetRichPos(i32 pos, RichPos& rp, i32 ti, i32 maxlevel, const RichStyles& st) const
{
	i32 p = pos;
	i32 pti = ti;
	for(i32 i = 0; i < part.GetCount(); i++) {
		i32 l = GetPartLength(i) + 1;
		if(pos < l) {
			if(IsTable(i)) {
...
RichCellPos RichTxt::GetCellPos(i32 table, i32 row, i32 column) const
{
	i32 pos = 0;
	for(i32 i = 0;; i++) {
		if(IsTable(i)) {
...
RichCellPos RichTxt::GetCellPos(i32 table, Point p) const
{
	return GetCellPos(table, p.y, p.x);
}

bool RichTxt::Iterate(UpdateIterator& r, i32 gpos, const RichStyles& s)
{
	for(i32 pi = 0; pi < part.GetCount(); pi++)
		if(IsTable(pi)) {
...
struct sIter__ : RichTxt::UpdateIterator {
	...
	virtual i32 operator()(i32 pos, RichPara& para) {
		return iter->operator()(pos, para) ? STOP : CONTINUE;
	}
};

bool RichTxt::Iterate(Iterator& r, i32 gpos, const RichStyles& s) const
{
	sIter__ it;
	it.iter = &r;
...
RichTxt::RichTxt(const RichTxt& src, i32)
{
	Init();
	part <<= src.part;
	length = src.length;
