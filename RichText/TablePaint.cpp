...
void RichTable::ExpandFrr(VecMap<i32, Rect>& frr, i32 pi, i32 l, i32 r, i32 t, i32 b)
{
                          ...
                          PaintInfo& pi, i32 pd, bool sel) const
{
	...
	i32 gridln = LineZoom(pi.zoom, format.grid);
	i32 ff2ln = gridln - gridln / 2;
	...
	if(py.page != clayout.py.page) // We are on the next page already
		pg = npg;
	for(i32 j = 0; j < format.column.GetCount();) {
		...
		if(pc.top) {
			...
			if(j) { // need to draw vertical grid line on the left
				...
				if(py.page == pyy.page)
					...
					for(i32 i = py.page + 1; i < pyy.page; i++)
...
void RichTable::Paint(PageDraw& pw, RichContext rc, const PaintInfo& _pi, bool baselevel) const
{
	...
	PaintInfo pi = _pi;
	i32 frameln = LineZoom(pi.zoom, format.frame);
	i32 gridln = LineZoom(pi.zoom, format.grid);
	Rect pg[2];
	Rect hpg(0, 0, 0, 0);
	RichContext hrc = rc;
	i32 hy = min(format.header, cell.GetCount());
	for(i32 pass = 0;; pass++) {
		Rect& pgr = pg[pass];
	...
	VecMap<i32, Rect> frr; // page -> table rectangle
	for(i32 i = 0; i < ny; i++)
	...
	for(i32 i = 0; i < frr.GetCount(); i++) { // add headers on subsequent pages and outer border
		pi.tablesel = 0;
		pi.sell = pi.selh = -1;
		i32 pgi = frr.GetKey(i);
		Rect r = frr[i];
...
i32 RichTable::GetWidth(const RichStyles& st) const
{
	Size sz = GetSize();
	Buffer<i32> col(sz.cx, 0);
...
RichCaret RichTable::GetCaret(i32 pos, RichContext rc) const
{
	ASSERT(pos >= 0);
...
i32  RichTable::GetPos(i32 x, PageY y, RichContext rc) const
{
	i32 nx = format.column.GetCount();
	i32 ny = cell.GetCount();
	const TabLayout& tab = Realize(rc);
...
RichHotPos  RichTable::GetHotPos(i32 x, PageY y, i32 tolerance, RichContext rc) const
{
	RichHotPos hp;
	...
	i32 ny = cell.GetCount();
	const TabLayout& tab = Realize(rc);
	i32 left = tab.first_page.left;
	i32 right = tab.first_page.right;
...
i32 RichTable::GetVertMove(i32 pos, i32 gx, RichContext rc, i32 dir) const
{
	if(cell.GetCount() == 0)
		return -1;
	i32 nx = format.column.GetCount();
	...
	for(;;) {
		...
		if(c.Reduce(rc1)) {
			i32 q = cell[cp.y][cp.x].text.GetVertMove(pos, gx, rc1, dir);
			if(q >= 0)
...
void RichTable::GatherValPos(Vec<RichValPos>& f, RichContext rc, i32 pos, i32 type) const
{
	ASSERT(pos >= 0);
	i32 nx = format.column.GetCount();
	i32 ny = cell.GetCount();
