...
void RichPara::Smh(Lines& lines, HeightInfo *th, i32 cx) const
{
...
RichPara::Tab RichPara::GetNextTab(i32 pos, i32 cx) const
{
	i32 tabi = -1;
	i32 dist = INT_MAX;
...
struct RichPara::StorePart {
	...
	void Store(Lines& lines, const Part& p, i32 pinc);
};

void RichPara::StorePart::Store(Lines& lines, const Part& part, i32 pinc)
{
	CharFormat& pfmt = lines.hformat.Add();
	pfmt = part.format;
	if(part.field && pinc) {
		for(i32 i = 0; i < part.fieldpart.GetCount(); i++)
...
static i32 CountChars(const Array<RichPara::Part>& part)
{
	i32 n = 0;
...
RichPara::Lines RichPara::FormatLines(i32 acx) const
{
	Lines lines;
	if(cacheid) {
		Mutex::Lock __(cache_lock);
	...
	wtuk s = lines.text;
	wtuk text = s;
	wtuk end = lines.text + lines.clen;
	wtuk space = NULL;
	i32 *w = lines.width;
	i32 cx = lines.first_indent;
	i32 rcx = lines.cx - format.lm - format.rm;
	bool withtabs = false;
	i32 scx = cx;
	while(s < end) {
		Tab t;
		if(*s == ' ') {
			...
			if(*s == '\n' || // fieldparts with newline
			   ...
			   *s == '\t' && (t.align == ALIGN_RIGHT ? t.pos - format.lm > rcx
				...
				Line& l = lines.line.Add();
				l.withtabs = withtabs;
				l.pos = (i32)(text - lines.text);
				if(space) {
					l.len = (i32)(space - text) + 1;
					l.cx = scx;
					text = s = space + 1;
				}
				else {
			...
			*s += t.fillchar;
			if(t.align == ALIGN_LEFT) {
				*w++ = t.pos - format.lm - cx;
				...
				i32 tcx = 0;
				i32 *tw = w + 1;
				for(wtuk ts = s + 1; ts < end && *ts != '\t'; ts++)
					tcx += *tw++;
				i32 ww = t.pos - format.lm - cx - (t.align == ALIGN_RIGHT ? tcx : tcx / 2);
				if(ww > 0) {
					*w++ = ww;
					cx += ww;
				}
...
void RichPara::Lines::Justify(const RichPara::Format& format)
{
	...
	for(i32 i = 0; i < line.GetCount() - 1; i++) {
		...
		if(!li.withtabs && li.len) {
			...
			s = beg;
			if(nspc) {
				i32 q = ((cx - format.lm - format.rm -
				          (i == 0 ? first_indent : next_indent) - li.cx) << 16)
				        / nspc;
				i32 *w = beg - ~text + width;
				i32 prec = 0;
				while(s < lim) {
					if(*s == ' ' || *s == 160) {
						*w += (prec + q) >> 16;
...
i32 RichPara::Lines::BodyHeight() const
{
	i32 sum = 0;
