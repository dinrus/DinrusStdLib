...
i32 RichTxt::GetWidth(const RichStyles& st) const
{
	i32 cx = 0;
...
void RichTxt::Sync0(const Para& pp, i32 parti, const RichContext& rc) const
{
	i32 cx = rc.page.Width();
	pp.ccx = cx;
	...
	if(~pp.header_qtf != ~p.format.header_qtf) { // we compare just pointers
		...
		drx::SetQTF(pp.header, pp.header_qtf);
	}
	if(~pp.footer_qtf != ~p.format.footer_qtf) { // we compare just pointers
		pp.footer_qtf = p.format.footer_qtf;
		drx::SetQTF(pp.footer, pp.footer_qtf);
	}
}

void RichTxt::Sync(i32 parti, const RichContext& rc) const {
...
bool RichTxt::BreaksPage(PageY py, const Para& pp, i32 i, const Rect& page) const
{
	i32 linecy = pp.linecy[i];
...
void RichTxt::Advance(i32 parti, RichContext& rc, RichContext& begin) const
{
	if(part[parti].Is<RichTable>()) {
		...
		if(rc.page.Height() < 30000) {
			...
			if(rc.py.y + pp.cy < rc.page.bottom)
				...
				for(i32 lni = 0; lni < pp.linecy.GetCount(); lni++) {
...
RichContext RichTxt::GetAdvanced(i32 parti, const RichContext& rc, RichContext& begin) const
{
	RichContext r = rc;
...
RichContext RichTxt::GetPartContext(i32 parti, const RichContext& rc0) const
{
	RichContext begin;
	RichContext rc = rc0;
	for(i32 i = 0; i < parti; i++)
		Advance(i, rc, begin);
	return rc;
}
...
{
	for(i32 pi = top.page; pi <= bottom.page; pi++)
...
RichCaret RichTxt::GetCaret(i32 pos, RichContext rc) const
{
	i32 parti = 0;
	if(pos > GetLength())
		pos = GetLength();
...
i32   RichTxt::GetPos(i32 x, PageY y, RichContext rc) const
{
	i32 parti = 0;
	i32 pos = 0;
...
RichHotPos RichTxt::GetHotPos(i32 x, PageY y, i32 tolerance, RichContext rc) const
{
	i32 parti = 0;
	i32 ti = 0;
	if(part.GetCount()) {
...
i32 RichTxt::GetVertMove(i32 pos, i32 gx, RichContext rc, i32 dir) const
{
	ASSERT(dir == -1 || dir == 1);
	if(GetPartCount() == 0)
		return -1;
	...
	while(pi < GetPartCount()) {
		i32 q = IsTable(pi) ? GetTable(pi).GetVertMove(p, gx, rc, dir)
...
void RichTxt::GatherValPos(Vec<RichValPos>& f, RichContext rc, i32 pos, i32 type) const
{
	i32 parti = 0;
	while(parti < part.GetCount()) {
		RichContext begin;
