...
RichTable::Layout RichTable::Realize(PageY py, RichContext arc, i32 ny, bool header) const
{ // create layout for first ny rows
	...
	Buffer<i32> column_left, column_right;
	column_left.Alloc(nx);
	column_right.Alloc(nx);

	i32 x = 0;
	i32 xx = arc.page.left;
	i32 dcx = arc.page.Width();
	for(i32 i = 0; i < nx; i++) {
		column_left[i] = xx;
		x += format.column[i];
	...
	i32 f2 = format.grid / 2;
	i32 ff2 = format.grid - f2;
		...
		for(i32 j = 0; j < nx;) {
			...
			if(pc.top) {
				...
				if(j + cell.hspan < nx - 1)
					pc.page_right -= ff2;
				pc.bottom = false;
				i32 ms = min(ny - 1, i + cell.vspan);
				for(i32 k = i + 1; k <= ms; k++) {
