...
RichPos RichText::GetRichPos(i32 pos, i32 maxlevel) const
{
	Mutex::Lock __(mutex);
	RichPos rp;
	rp.level = 0;
...
i32  RichText::AdjustCursor(i32 anchor, i32 cursor) const
{
	Mutex::Lock __(mutex);
	i32 d = anchor;
...
void RichText::Remove(i32 pos, i32 count)
{
	GetLength();
	i32 p = pos;
	i32 l = GetConstText(p).GetLength();
	count = min(l - p, count);
	RichTxt& txt = GetUpdateText(pos);
	i32 pos2 = pos + count;
	i32 pi = txt.FindPart(pos);
	i32 pi2 = txt.FindPart(pos2);
	if(pi == pi2) {
		RichPara pa, pa1;
		pa = txt.Get(pi, style);
		pa1 <<= pa;
...
void RichText::Insert(i32 pos, const RichText& p)
{
	MergeStyles(p);
	if(p.GetLength() == 0)
		return;
...
RichText RichText::Copy(i32 pos, i32 count) const
{
	Mutex::Lock __(mutex);
...
RichText::FormatInfo RichText::GetFormatInfo(i32 pos, i32 count) const
{
	Mutex::Lock __(mutex);
	...
	RichPara pa = txt.Get(pi, style);
	fmt = pa.format;
	i32 i = pa.FindPart(pos);
	if(i < pa.GetCount())
		(RichPara::CharFormat&)fmt = pa[i].format;
	fi.Set(fmt);
	i++;
	if(pi == pi2) {
		i32 i2 = pa.FindPart(pos2);
		while(i < i2)
...
void RichText::ApplyFormatInfo(i32 pos, const FormatInfo& fi, i32 count)
{
	RichTxt& txt = GetUpdateText(pos);

	i32 pos2 = min(txt.GetLength(), pos + count);
	i32 pi = txt.FindPart(pos);
	i32 pi2 = txt.FindPart(pos2);
...
void RichText::ReplaceStyle(i32 pos, const Uuid& id)
{
	RichTxt& txt = GetUpdateText(pos);
	i32 p = txt.FindPart(pos);
	RichPara pa = txt.Get(p, id, style);
	txt.Set(p, pa, style);
}

void RichText::ReStyle(i32 pos, const Uuid& id)
{
	RichTxt& txt = GetUpdateText(pos);
	i32 p = txt.FindPart(pos);
...
RichText::Formating RichText::SaveFormat(i32 pos, i32 count) const
{
	Mutex::Lock __(mutex);
	const RichTxt& txt = GetConstText(pos);
	count += pos;
...
void RichText::RestoreFormat(i32 pos, const RichText::Formating& info)
{
	RichTxt& txt = GetUpdateText(pos);
	i32 ii = 0;
...
RichText::RichText(const RichText& x, i32)
...
void RichTextLayoutTracer::TableRow(const Rect& page, PageY y, i32 i, const RichTable& table) {}
void RichTextLayoutTracer::EndTableRow(PageY y) {}
void RichTextLayoutTracer::TableCell(const Rect& page, PageY y, i32 i, i32 j, const RichTable& table, PageY npy) {}
