...
RichPara::~RichPara()
{
	if(cacheid && part.GetCount() && !incache) {
		...
		i32 total = 0;
		for(i32 i = 1; i < cache.GetCount(); i++) {
...
PaintInfo::PaintInfo()
{
	...
	textcolor = Null;
	DrawSelection = [] (Draw& w, i32 x, i32 y, i32 cx, i32 cy) {
		w.DrawRect(x, y, cx, cy, InvertColor);
	};
}

Txt RichPara::Number::AsText(const RichPara::NumberFormat& format) const
{
	Txt result;
	for(i32 i = 0; i < 8; i++)
		if(format.number[i]) {
...
i32 RichPara::NumberFormat::GetNumberLevel() const
{
	for(i32 i = 7; i >= 0; i--)
		if(number[i])
		   return i + 1;
...
void RichPara::Cat(const WTxt& s, const RichPara::CharFormat& f)
{
	cacheid = 0;
	part.Add();
	part.Top().text = s;
...
void RichPara::Cat(tukk s, const CharFormat& f)
{
	Cat(WTxt(s), f);
}

void RichPara::Cat(const RichObj& o, const RichPara::CharFormat& f)
{
	cacheid = 0;
	part.Add();
	part.Top().object = o;
...
void RichPara::Cat(Id field, const Txt& param, const RichPara::CharFormat& f)
{
	cacheid = 0;
	Part& p = part.Add();
	p.field = field;
...
Txt RichPara::Pack(const RichPara::Format& style, Array<RichObj>& obj) const
{
	TxtStream out;
	dword pattr = 0;
	...
	if(pattr & 0x400)  { Txt t = format.label; out % t; }
	if(pattr & 0x800)  out.Put(format.keepnext);
	if(pattr & 0x1000) out.Put(format.orphan);
	if(pattr & 0x2000) {
		Txt b = format.before_number, a = format.after_number;
		out % b % a;
	...
	if(pattr & 0x80000) {
		...
		if(format.newhdrftr) {
			Txt t = format.header_qtf;
			Txt f = format.footer_qtf;
			out % t % f;
		}
	}
...
void RichPara::UnpackParts(Stream& in, const RichPara::CharFormat& chrstyle,
                           Array<RichPara::Part>& part, const Array<RichObj>& obj,
                           i32& oi) {
	part.Add();
...
void RichPara::Unpack(const Txt& data, const Array<RichObj>& obj,
...
VecMap<Id, RichPara::FieldType *>& RichPara::fieldtype0()
{
	static VecMap<Id, RichPara::FieldType *> h;
	return h;
}
...
bool RichPara::EvaluateFields(VecMap<Txt, Val>& vars)
{
	bool b = false;
...
i32  RichPara::FindPart(i32& pos) const
{
	i32 pi = 0;
...
void RichPara::Dump()
{
	...
	for(i = 0; i < part.GetCount(); i++)
		LOG("Part[" << i << "] = " << AsCTxt(part[i].text.ToTxt()) << " "
		    << part[i].format);
	LOG(LOG_END << "---------");
}


Txt RichPara::CharFormat::ToTxt() const
{
	Txt out;
	...
	default: out << ", sscript(" << (i32)sscript << ")"; break;
	}
...
Txt RichPara::Format::ToTxt() const
{
	Txt out;
	if(!DRX::IsNull(label))
		out << "label <" << label << ">: ";
