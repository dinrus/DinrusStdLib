...
void DrawRoundCorner(Draw& w, i32 x, i32 y, i32 rx1, i32 ry1, i32 rx2, i32 ry2, Color color)
{
	if(rx1 == rx2 || ry1 == ry2) {
	...
	i32 steps = max(rx1, ry1, 1000);
	Vec<Point> vertex;
	for(i32 i = 0; i <= steps; i++) {
		double angle = M_PI / 2 * i / steps;
		vertex.Add() = Point(i32(rx1 * sin(angle) + x), i32(ry1 * cos(angle) + y));
	}
	for(i32 i = steps; i >= 0; i--) {
		double angle = M_PI / 2 * i / steps;
		vertex.Add() = Point(i32(rx2 * sin(angle) + x), i32(ry2 * cos(angle) + y));
	}
                    ...
                    Color color, bool round)
{
	...
	i32 h = rect.GetHeight() - fb.top - fb.bottom;
	i32 y = rect.top + border.top + margin.top;
	i32 v = rect.GetWidth() - fb.left - fb.right;
	i32 x = rect.left + border.left + margin.left;
...
void RichCell::DrawCell(Draw& w, i32 l, i32 r, i32 y, i32 yy, const Rect& border,
...
PageY RichCell::Align(const RichContext& rc, PageY npy) const
{
	...
	if(rc.py.page == npy.page) {
		i32 dx = npy.y - text.GetHeight(rc).y
...
void RichCell::Paint(PageDraw& pw, RichContext rc, PageY npy,
                     ...
                     i32 y, i32 ny, const PaintInfo& pi, bool select) const
{
	if(!Reduce(rc))
		return;
	...
	if(rc.py.page == npy.page)
		...
		for(i32 i = rc.py.page + 1; i < npy.page; i++)
...
RichCaret RichCell::GetCaret(i32 pos, RichContext rc, PageY pyy) const
{
	if(!Reduce(rc))
		return RichCaret();
	rc.py = Align(rc, pyy);
...
i32 RichCell::GetPos(i32 x, PageY y, RichContext rc, PageY pyy) const
{
	if(!Reduce(rc))
		return 0;
	rc.py = Align(rc, pyy);
...
RichHotPos  RichCell::GetHotPos(i32 x, PageY y, i32 tolerance, RichContext rc, PageY pyy) const
{
	if(!Reduce(rc))
		return RichHotPos();
	rc.py = Align(rc, pyy);
...
void RichCell::GatherValPos(Vec<RichValPos>& f, RichContext rc, PageY pyy, i32 pos, i32 type) const
{
	if(!Reduce(rc))
		return;
	rc.py = Align(rc, pyy);
...
RichCell::RichCell(const RichCell& src, i32)
{
	format = src.format;
	hspan = src.hspan;
	vspan = src.vspan;
