...
Date OciDecodeDate(u8k data[7]) {
	Date d;
	d.year = i32(data[0] - 100) * 100 + data[1] - 100;
	d.month = data[2];
	d.day = data[3];
	return d;
}
...
bool OciEncodeDate(u8 data[7], Date d) {
	if(IsNull(d))
		return false;
	i32 d100 = d.year / 100, m100 = d.year % 100;
	if(m100 < 0) {
		m100 += 100;
		d100--;
	}
...
Time OciDecodeTime(u8k data[7]) {
	Time t;
	t.year = i32(data[0] - 100) * 100 + data[1] - 100;
	t.month = data[2];
	t.day = data[3];
	t.hour = data[4] - 1;
	t.minute = data[5] - 1;
...
bool OciEncodeTime(u8 data[7], Time t) {
	if(IsNull(t))
		return false;
	i32 d100 = t.year / 100, m100 = t.year % 100;
	if(m100 < 0) {
		m100 += 100;
		d100--;
	}
...
Txt OciParseTxtError(tukk s) {
	Txt err = "Parse error: unterminated string ";
	i32 l = (i32)strlen(s);
	enum { SAMPLE = 10 };
	if(l <= SAMPLE)
		err.Cat(s, l);
...
Txt OciParseRefError(tukk s) {
	return Txt().Cat() << "Parse error: invalid ?% type specifier '" << *s << "'";
}
...
i32 OciParse(tukk statement, Txt& out, OciSqlConnection *conn, SqlSession *session) {
	...
	while(*s) {
		if(*s == '\'') {
			...
			if(!s) {
				if(session)
					session -> SetError(OciParseTxtError(b), statement);
				return -1;
			}
			cmd.Cat(b, i32(s - b));
		}
			...
			if(*s == '%') {
				...
				if(!s) {
					if(session)
						session -> SetError(Txt().Cat() << "Parse error: invalid ?% type specifier '"
						<< *p << "'", statement);
					return -1;
				}
...
i32 OracleRef::GetOraType() const {
	switch(ora_vtype) {
	case BOOL_V:
	case INT_V:      return SQLT_INT;
	case DOUBLE_V:   return SQLT_FLT;
	case WTXT_V:
	case TXT_V:   return SQLT_STR;
	case DATE_V:
...
i32 OracleRef::GetMaxLen() const {
	switch(ora_vtype) {
	case BOOL_V:
	case INT_V:       return sizeof(i32);
	case DOUBLE_V:    return sizeof(double);
	case WTXT_V:
	case TXT_V:    return OCI_TXT_MAXLEN + 1;
	case DATE_V:
	case TIME_V:      return 7;
	case ORA_BLOB_V:  return sizeof(uk );
	case ORA_CLOB_V:  return sizeof(uk );
	default: NEVER(); return 0;
	}
}
...
bool OraclePerformScript(const Txt& text, StatementExecutor& exe, Gate<i32, i32> progress_canceled)
{
	bool error = false;
	Txt statement;
	bool body = false;
	...
	TxtStream in(text);
	while(!in.IsEof()) {
		if(progress_canceled((i32)in.GetPos(), (i32)in.GetSize()))
			return false;
		Txt line = in.GetLine();
		bool exec = false;
...
static Vec<Txt> FetchList(Sql& cursor)
{
	Vec<Txt> out;
	while(cursor.Fetch())
		out.Add(cursor[0]);
...
{
	Vec<Txt> out;
	Txt udb = Nvl(database, cursor.GetUser());
...
Vec<Txt> OracleSchemaReservedWords()
{
	...
	for(i32 i = 0; i < __countof(rwlist); i++)
		out[i] = rwlist[i];
	return out;
}
