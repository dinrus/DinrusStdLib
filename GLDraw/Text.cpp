...
Image RenderGlyphByPainter2(Point at, double angle, i32 chr, Font font, Color color, Size sz)
{
	ImageBuffer ib(sz);
	BufferPainter sw(ib);
	sw.Clear(RGBAZero());
...
i32 texture_glyph_cache_max = 64*1024*1024;
...
struct sGlyphTextureMaker : LRUCache<GLTexture>::Maker {
	...
	virtual Txt Key() const {
		...
		RawCat(h, angle);
		RawCat(h, color);
		return static_cast<Txt>(h);
	}
	virtual i32 Make(GLTexture& object) const {
		GL_TIMING("Do glyph");
