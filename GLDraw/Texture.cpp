...
static LRUCache<ImageGLData, Tuple<zu64, zu64, dword> > sTextureCache;
static bool sReset;
...
struct ImageGLDataMaker : LRUCache<ImageGLData, Tuple<zu64, zu64, dword> >::Maker {
	Image         img;
	zu64        context;
	dword         flags;

	virtual Tuple<zu64, zu64, dword>  Key() const                      { return MakeTuple(img.GetSerialId(), context, flags); }
	virtual i32                           Make(ImageGLData& object) const  { object.Init(img, flags); return img.GetLength(); }
};

i32 max_texture_memory = 4 * 1024 * 768;
i32 max_texture_count = 1000;

GLuint GetTextureForImage(dword flags, const Image& img, zu64 context)
{
