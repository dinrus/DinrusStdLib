...
void CheckError()
{
    ...
    if( gl_error != GL_NO_ERROR ) {
        Panic(Txt().Cat() << " Ошибка OpenGL: " << gl_error);
    }
}
...
void GLDraw::FlushPutRect()
{
	...
	GLubyte *c = color;
	GLushort *n = index;
	for(i32 i = 0; i < put_rect.GetCount(); i++) {
...
void GLDraw::Init(Size sz, zu64 context_)
{
	SDraw::Init(sz);
	...
	gl_image.Use();
	static i32 uni1 = gl_image.GetUniform("u_projection");
	GLOrtho(0, (float)sz.cx, (float)sz.cy, 0, 0.0f, 1.0f, uni1);
	gl_image_colored.Use();
	static i32 uni2 = gl_image_colored.GetUniform("u_projection");
	GLOrtho(0, (float)sz.cx, (float)sz.cy, 0, 0.0f, 1.0f, uni2);
	gl_rect.Use();
	static i32 uni3 = gl_rect.GetUniform("u_projection");
