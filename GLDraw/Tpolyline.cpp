...
void Polyline(GLTriangles& tr, const Vec<Pointf>& p, double width, Color color, double alpha, bool close)
{
	...
	if(p.GetCount() && close && p[0] != p.Top())
		m = 0;
	width *= 0.5;
	i32 fv11, fv12, pv21, pv22;
	for(i32 i = 0; i < p.GetCount() - m; i++) {
		Pointf p1 = p[i];
		Pointf p2 = p[i + 1 < p.GetCount() ? i + 1 : 0];
		Pointf un = width * Normalize(Orthogonal(p2 - p1));
		
		i32 v11 = tr.Vertex(p1 + un, color, alpha);
		i32 v12 = tr.Vertex(p1 - un, color, alpha);
		i32 v21 = tr.Vertex(p2 + un, color, alpha);
		i32 v22 = tr.Vertex(p2 - un, color, alpha);
