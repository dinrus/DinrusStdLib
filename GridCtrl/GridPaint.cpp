...
void GridCtrl::Paint(Draw &w)
{
	...
	i32 en = IsShowEnabled() ? 0 : GD::READONLY;
	...
	if(w.IsPainting(r) && total_cols > 1)
	{
		...
		i32 firstcol = indicator ? 0 : (fixed_cols > 1 ? 1 : firstVisCol);
		if(firstCol < 0) firstCol = 0;
		i32 jfc = chameleon ? 0 : firstCol;
				...
				if(summary_row && i == 0)
				{
					...
					if(w.IsPainting(x, y, cx, cy))
					{
						...
						if(hi.sop != SOP_NONE && !hi.sopfrm.IsEmpty() && !IsNull(it.val))
							...
							s = IsTxt(it.val) ? it.val : StdConvert().Format(it.val);
		...
		if(rx < sz.cx || chameleon)
		{
			...
			if(summary_row)
			{
				...
				display->Paint(w, rx, y + 1, cx, GD_HDR_HEIGHT - 1, Val(""), style, fg, bg, stdfont, false, 0, 0);
			}
		}
...
//
		...
		for(i = max(firstRow, fixed_rows); i < total_rows; i++)
		{
			...
			for(j = max(firstCol, fixed_cols); j < total_cols; j++)
			{
				...
				if(w.IsPainting(x, y, cx, cy))
				{
					...
					Font fnt = StdFont();
					GridDisplay* gd;
					Val val = GetItemVal(it, id, hi, vi);
	...
	if(moving_header && fixed_top_click && curSplitCol >= 0)
	{
		...
		i32 cx = hitems[cp].nWidth() / 2;
		i32 x = lastcol ? hitems[curSplitCol].nLeft(sbx) + cx
	...
	{
		i32 dy = curSplitRow == lastVisRow ? -2 : -1;
		i32 y = curSplitRow >= 0 ? vitems[curSplitRow].nBottom(sby) + dy : 0;
		if(y >= fixed_height - 1)
