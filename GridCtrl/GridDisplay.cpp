...
void GridDisplay::SetTheme(i32 th)
{
	theme = th;
	hdrhigh = Contrast(vhdr[theme](), 230);
}

WTxt GridDisplay::GetStdConvertedVal(const Val &v) const
{
	return IsTxt(v) ? v : StdConvert().Format(v);
}

void GridDisplay::Paint(Draw &w, i32 x, i32 y, i32 cx, i32 cy, const Val &val, dword style,
						Color &fg, Color &bg, Font &fnt, bool found, i32 fs, i32 fe)
{
	real_size.cx = 0;
	real_size.cy = 0;
	...
	i32 al = style & GD::ALIGN ? style & GD::ALIGN : align;
	...
	if(cx > lm + rm && cy > tm + bm)
	{
		...
		if(!(style & GD::NOTEXT))
		{
			DrawText(w, nx, nx, ny, ncx, ncy, al, GetStdConvertedVal(val), fnt, fg, bg, found, fs, fe, style & GD::WRAP);
		}
...
void GridDisplay::PaintFixed(Draw &w, bool firstx, bool firsty, i32 x, i32 y, i32 cx, i32 cy, const Val &val, dword style, Font &fnt,
		                     bool indicator, bool moved, i32 sortmode, i32 sortcol, i32 sortcnt, bool horizontal)
{
	real_size.cx = 0;
	real_size.cy = 0;
	...
	if(chameleon)
	{
		...
		i32 ht = HeaderCtrl::StyleDefault().gridadjustment;
	...
	if(sortmode > 0)
	{
		...
		i32 yf = y + (cy - isz.cy) / 2;
		i32 xf = tx + 2;
		tx = xf + isz.cx + 1;

		if(sortcol > 0 && sortcnt > 1)
		{
			Txt tcol = AsTxt(sortcol);
	...
	if(cx > lm + rm && cy > tm + bm)
	{
		...
		DrawText(w, tx, nx, ny, ncx, ncy, al, GetStdConvertedVal(val), fnt, fg, SColorPaper, 0, 0, 0, style & GD::WRAP);

		w.End();
	}
}

i32 GridDisplay::GetLinesCount(i32 cx, const wchar* s, const Font& font, bool wrap)
{
	if(*s == 0)
		return 0;
	...
	while(*p)
	{
		...
		if(nextline || *(p + 1) == '\0')
		{
			if(p - e > 0 && wrap)
			{
				i32 tcx = GetTextSize(e, font, i32(p - e)).cx;
				if(tcx > cx)
				{
					lines += tcx / ccx;
					if(tcx % ccx > 0)
...
void GridDisplay::DrawText(Draw &w, i32 mx, i32 x, i32 y, i32 cx, i32 cy, i32 align, wtukk s, const Font &font, const Color &fg, const Color &bg, bool found, i32 fs, i32 fe, bool wrap)
{
	if(*s == 0)
		return;

	i32 tcy = font.Info().GetHeight();
...
//	i32 enters = 0;
	...
	while(true)
	{
		...
		if(nextline || endtext)
		{
			...
			i32 tx = x;
			tsz = GetTextSize(t, font, i32(p - t));
			...
			if(tsz.cx > gcx)
			{
				...
				while(e < p)
				{
					i32 tcx = GetTextSize(e, font, 1).cx;
					size += tcx;
					if(size > gcx)
						break;
					e++;
			...
			if(found)
			{
				...
				if(fs <= che && fe >= chs)
				{
					i32 scx = GetTextSize(t, font, max(chs, fs) - chs).cx;
					i32 ecx = GetTextSize(t, font, min(che, fe) - chs + 1).cx;
			...
			if(dots)
			{
				...
				if(p > t)
				{
					...
					w.DrawText(max(mx, tx), ty, t, font, tfg, i32(p - t));
					w.End();
				}
			}
			else
