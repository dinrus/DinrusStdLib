...
#define TFILE <drx/GridCtrl/GridCtrl.t>
...
GridCtrl::GridCtrl() : holder(*this)
{
	...
	sbx.Horz();
	sby.Vert();
	sbx.WhenScroll = OTVET(Scroll);
	sby.WhenScroll = OTVET(Scroll);
	sbx.SetLine(5);
	...
	WhenMenuBar = OTVET(StdMenuBar);
	WhenToolBar = OTVET(StdToolBar);

	find <<= OTVET(DoFind);

	StdAppend = OTVET(DoAppend);
	StdRemove = OTVET(DoRemove);
	StdInsert = OTVET(DoInsertBefore);
	StdDuplicate = OTVET(DoDuplicate);
	StdEdit = OTVET(DoEdit);
	...
	find.WhenBar = OTVET(FindOptsBar);
...
void GridCtrl::StdToolBar(Bar &bar)
{
	...
	if(editing)
	{
		...
		if(accepting)
			bar.Add(isedit, t_("Принять"), GridImg::Commit(), OTVET(DoEndEdit));
		if(canceling)
			bar.Add(isedit, t_("Отмена"), GridImg::Cancel(), OTVET(DoCancelEdit));
	}
	...
	if(moving)
	{
		...
		bar.Add(c, t_("Поднять"), GridImg::MoveUp(), OTVET(DoSwapUp));
		bar.Add(c, t_("Опустить"), GridImg::MoveDn(), OTVET(DoSwapDown));
	}
...
void GridCtrl::FindBar(Bar &bar, i32 cx)
{
	bar.Add(find, cx);
}

void GridCtrl::InfoBar(Bar &bar, i32 cx)
{
	bar.Add(info, cx);
}

void GridCtrl::SetToolBarInfo(Txt inf)
{
	info.SetLabel(inf);
}
...
{
	bar.Add(RowFormat(t_("Первый %s")), GridImg::FirstRec(), OTVET(DoGoBegin));
	bar.Add(RowFormat(t_("Предыдущий %s")), GridImg::PrevRec(), OTVET(DoGoPrev));
	bar.Add(RowFormat(t_("Следующий %s")), GridImg::NextRec(), OTVET(DoGoNext));
	bar.Add(RowFormat(t_("Последний %s")), GridImg::LastRec(), OTVET(DoGoEnd));
}

GridCtrl& GridCtrl::SetToolBar(bool b, i32 align, i32 frame)
{
	RemoveFrame(bar);
...
{
	bar.Add(t_("Немедленный поиск"), OTVET1(SetFindOpts, 0)).Check(search_immediate);
	bar.Add(t_("Скрыть ряды"), OTVET1(SetFindOpts, 1)).Check(search_hide);
	bar.Add(t_("Подсветить найденные ячейки"), OTVET1(SetFindOpts, 2)).Check(search_highlight);
	bar.Add(t_("Регистрочувствительно"), OTVET1(SetFindOpts, 3)).Check(search_case);
}

void GridCtrl::SetFindOpts(i32 n)
{
	switch(n)
	{
		case 0:
			...
			{
				find <<= OTVET(Nothing);
				find.WhenEnter = OTVET(DoFind);
			}
			else
			{
				find <<= OTVET(DoFind);
				find.WhenEnter = OTVET(Nothing);
			}
			break;
		case 1:
...
void GridCtrl::StdMenuBar(Bar &bar)
{
	...
	if(inserting)
	{
		...
		{
			bar.Add(c, t_("Вставить перед"), OTVET(DoInsertBefore))
			   .Image(GridImg::InsertBefore())
			   .Help(RowFormat(t_("Вставить новый %s в эту таблицу перед текущим")))
			   .Key(K_INSERT);
			bar.Add(c, t_("Вставить после"), OTVET(DoInsertAfter))
	...
	if(duplicating)
	{
		bar.Add(c, t_("Дублировать"), OTVET(DoDuplicate))
...
{
	bool c = IsCursor();
	bar.Add(c && curpos.y > fixed_rows, t_("Поднять"), OTVET(DoSwapUp))
	   .Image(GridImg::MoveUp())
	   .Key(K_CTRL_UP);
	bar.Add(c && curpos.y >= fixed_rows && curpos.y < total_rows - 1, t_("Опустить"), OTVET(DoSwapDown))
...
{
	bar.Add(total_rows > fixed_rows, RowFormat(t_("Выделить все")), OTVET(DoSelectAll))
...
{
	bar.Add(t_("Колонки"), OTVET(ColumnList));
}

void GridCtrl::ColumnList(Bar &bar)
{
	i32 cnt = 0;
	for(i32 i = fixed_cols; i < total_cols; i++)
	...
	{
		if(!hitems[i].index)
			bar.Add((Txt) items[0][hitems[i].id].val, OTVET1(MenuHideColumn, i))
...
void GridCtrl::ClipboardMenu(Bar &bar)
{
	...
	bar.Add(t_("Копировать"), OTVET(DoCopy)).Image(CtrlImg::copy()).Key(K_CTRL_C).Enable(s && copy_allowed);
	if(cut_allowed)
		bar.Add(t_("Вырезать"), OTVET(Nothing)).Image(CtrlImg::cut()).Key(K_CTRL_X).Enable(s && cut_allowed);
	if(paste_allowed)
		bar.Add(t_("Вставить"), OTVET(DoPaste)).Image(CtrlImg::paste()).Key(K_CTRL_V).Enable(c && paste_allowed && IsClipboardAvailable());
	if(extra_paste)
		bar.Add(t_("Вставить как"), OTVET(PasteAsMenu));
}
...
void GridCtrl::PasteAsMenu(Bar &bar)
{
	...
	bar.Add(t_("приставлен"), OTVET(DoPasteAppendedRows)).Key(K_CTRL_E).Enable(s && paste_allowed);
	bar.Add(t_("вставлен"), OTVET(DoPasteInsertedRows)).Key(K_CTRL_I).Enable(c && paste_allowed && s);
}
...
void GridCtrl::SetClipboard(bool all, bool silent)
{
	...
	for(i32 i = fixed_rows; i < total_rows; i++)
	{
		...
		for(i32 j = fixed_cols; j < total_cols; j++)
		{
			if(all || row_selected || IsSelected(i, j, false))
			{
				...
				body += GetStdConvertedColumn(j, d.v).ToTxt() + '\t';

				sc[j] = 1;
			}
		}

		i32 cnt = body.GetCount();
		if(cnt > 0 && body[cnt - 1] == '\t')
			body.Remove(cnt - 1);
	}
	...
	{
		for(i32 i = 0; i < sc.GetCount(); i++)
			if(sc[i] >= 0)
				header += hitems[i].GetName() + '\t';

		i32 cnt = header.GetCount();
...
void GridCtrl::Paste(i32 mode)
{
	...
	Txt s = FromUnicode(ReadClipboardUnicodeText());
	...
	if(mode == 1)
	{
		i32 dy = is_gc ? gc.maxpos.y - gc.minpos.y + 1
		               : lines.GetCount();
		Insert0(curpos.y, dy);
		curpos.y += dy;
	}
	...
	if(!is_gc)
	{
		if(is_tc)
		{
			...
			for(i32 i = 0; i < lines.GetCount(); i++)
			{
				Txt line = TrimRight(lines[i]);
				Vec<Txt> cells = drx::Split(line, '\t', false);
				for(i32 j = 0; j < cells.GetCount(); j++)
				{
					i32 r = i;
					i32 c = j;
	...
	else if(!select_row && gc.shiftmode)
	{
		...
		for(i32 i = 0; i < gc.data.GetCount(); i++)
		{
	...
	{
		i32 pr = gc.data[0].row - gc.minpos.y;
		bool new_row = false;

		for(i32 i = 0; i < gc.data.GetCount(); i++)
		{
			i32 r = gc.data[i].row - gc.minpos.y;
			i32 c = gc.data[i].col - gc.minpos.x;
...
void GridCtrl::DrawLine(bool iniLine, bool delLine)
{
	...
	{
		i32 sx = resize_paint_mode == 1 ? fixed_width : 0;
		i32 sy = resize_paint_mode == 1 ? fixed_height : 0;
		ViewDraw w(this);
		Size sz = GetSize();
...
Val GridCtrl::GetItemVal(const Item& it, i32 id, const ItemRect& hi, const ItemRect& vi)
{
	Val val = hi.IsConvertion() && vi.IsConvertion()
...
Val GridCtrl::GetAttrTextVal(const Val& val)
{
	if(IsType<AttrText>(val))
	{
		const AttrText& t = ValTo<AttrText>(val);
		return t.text;
	}
	return val;
}

void GridCtrl::GetItemAttrs(const Item& it, const Val& value, i32 r, i32 c, const ItemRect& vi, const ItemRect& hi, dword& style, GridDisplay*& gd, Color& fg, Color& bg, Font& fnt)
{
	if(!IsNull(vi.fg))
		fg = vi.fg;
	...
	const Val& val = IsNull(value) ? it.val : value;
...
GridCtrl::Item& GridCtrl::GetItemSize(i32 &r, i32 &c, i32 &x, i32 &y, i32 &cx, i32 &cy, bool &skip, bool relx, bool rely)
{
	i32 idx = hitems[c].id;
	i32 idy = vitems[r].id;
...
void GridCtrl::DrawHorzDragLine(Draw &w, i32 pos, i32 cx, i32 size, Color c)
{
	//w.DrawRect(pos, 0, cx / 2, size - 1, Color(100, 100, 100, 100);
	DrawFrame(w, pos + 1, 1, cx / 2 - 2, size - 2, c);
	DrawFrame(w, pos, 0, cx / 2, size, c);
//	DrawFatFrame(w, x, 0,ďż˝ i32 cxďż˝ i32 cyďż˝ Color colorďż˝ i32 n)
}

void GridCtrl::DrawVertDragLine(Draw &w, i32 pos, i32 size, i32 dx, Color c)
{
	w.DrawRect(1 + dx, pos, size - dx - 1, 2, c);
}

Val GridCtrl::GetStdConvertedVal(const Val& v) const
{
	if(IsTxt(v))
	{
		return v;
	...
	{
		const AttrText& t = ValTo<AttrText>(v);
		return t.text;
	}
	else
...
Val GridCtrl::GetConvertedColumn(i32 col, const Val &v) const
{
	Convert *conv = edits[col].convert;
	return conv ? conv->Format(v) : v;
}

Val GridCtrl::GetStdConvertedColumn(i32 col, const Val &v) const
{
	Val val = GetConvertedColumn(col, v);
	return GetStdConvertedVal(val);
}

Txt GridCtrl::GetTxt(Id id) const
{
	i32 c = aliases.Get(id);
	return GetStdConvertedColumn(c, Get0(rowidx, c));
}

GridCtrl::ItemRect& GridCtrl::InsertColumn(i32 col, tukk name, i32 size, bool idx)
{
	i32 id;
...
GridCtrl::ItemRect& GridCtrl::AddColumn(tukk name, i32 size, bool idx)
{
...
GridCtrl::ItemRect& GridCtrl::AddColumn(const Id id, tukk name, i32 size, bool idx)
{
	return AddColumn(name ? name : (tukk ) ~id, size, idx).Alias(id);
}

GridCtrl::ItemRect& GridCtrl::AddColumn(const Txt& name, i32 size, bool idx)
{
	return AddColumn((tukk ) name, size, idx);
}

void GridCtrl::RemoveColumn(i32 n, i32 count)
{
	n += fixed_cols;
	if(count < 0)
		count = total_cols - n;
...
GridCtrl::ItemRect& GridCtrl::AddRow(i32 n, i32 size)
{
	Append0(n, size);
	return GetRow();
}
...
void GridCtrl::SetRowCount(i32 n, i32 size)
{
	Clear();
	Append0(n, size);
}

void GridCtrl::SetColCount(i32 n, i32 size)
{
	Reset();
	for(i32 i = 0; i < n; i++)
...
void GridCtrl::SyncPopup()
{
	if(!IsPopUp() && popups)
	{
		...
		i32 c = GetMouseCol(p, !fc, fc);
		i32 r = GetMouseRow(p, !fr, fr);
		...
		if(valid_pos && !ctrl)
		{
			...
			if(it.rcx > 0 || it.rcy > 0)
			{
				...
				Val val = GetStdConvertedVal(r == 0 ? it.val : GetItemVal(it, hi.id, hi, vi));
				...
				if(new_cell)
				{
					...
					i32 x = hi.npos + p0.x - p.x - 1 - sbx.Get() * i32(!fc);
					i32 y = vi.npos + p0.y - p.y - 1 - sby.Get() * i32(!fr);
					...
					i32 margin = popup.gd->lm + popup.gd->rm;
					i32 cx = min(600, min((i32) (scr.right * 0.4), max(it.rcx + margin + 2, hi.nsize + 1)));
					i32 lines = popup.gd->GetLinesCount(cx - margin - 2, WTxt(val), popup.fnt, true);
					i32 cy = max(lines * Draw::GetStdFontCy() + popup.gd->tm + popup.gd->bm + 2, vi.nsize + 1);
					if(fr && r == 0)
					{
						y++;
						cy--;
...
void GridCtrl::State(i32 reason)
{
	if(reason == OPEN)
	{
		Init();
...
void GridCtrl::ChildAction(Ctrl *child, i32 event)
{
...
void GridCtrl::ChildMouseEvent(Ctrl *child, i32 event, Point p, i32 zdelta, dword keyflags)
{
	ChildAction(child, event);
	Ctrl::ChildMouseEvent(child, event, p, zdelta, keyflags);
}

void GridCtrl::ChildFrameMouseEvent(Ctrl *child, i32 event, Point p, i32 zdelta, dword keyflags)
{
	ChildAction(child, event);
...
Rect GridCtrl::GetItemRect(i32 r, i32 c, bool hgrid, bool vgrid, bool hrel, bool vrel)
{
	i32 dx = sbx + (hrel ? fixed_width : 0);
	i32 dy = sby + (vrel ? fixed_height : 0);
	...
	return Rect(left, top, right - (i32) vgrid, bottom - (i32) hgrid);
}

Rect& GridCtrl::AlignRect(Rect &r, i32 i)
{
	Rect c(r);
	i32 align = hitems[i].calign;
	i32 sx = hitems[i].sx;
	i32 sy = hitems[i].sy;
...
void GridCtrl::SetFixedRows(i32 n)
{
	if(n >= 0 && n <= total_rows)
	{
		LG(0, "SetFixedRows");
...
void GridCtrl::SetFixedCols(i32 n)
{
	if(n >= 0 && n < total_cols)
	{
		LG(0, "SetFixedCols");
...
void GridCtrl::Set0(i32 r, i32 c, const Val &val_, bool paste)
{
	Val val = val_;
	if(c > total_cols - 1)
		return;
...
void GridCtrl::Set(i32 r, i32 c, const Val &val)
{
	Set0(r + fixed_rows, c + fixed_cols, val);
}

void GridCtrl::Set(i32 r, Id id, const Val &val)
{
	Set0(r + fixed_rows, aliases.Get(id), val);
}

void GridCtrl::Set(i32 r, tukk s, const Val &val)
{
	Set0(r + fixed_rows, aliases.Get(s), val);
}

void GridCtrl::Set(i32 c, const Val &val)
{
	Set0(rowidx, c + fixed_cols, val);
}

void GridCtrl::Set(Id id, const Val &val)
{
	Set0(rowidx, aliases.Get(id), val);
}

void GridCtrl::Set(i32 r, const Vec<Val> &v, i32 data_offset /* = 0*/, i32 column_offset /* = 0*/)
{
	r += fixed_rows;
	vitems[r].operation = GridOperation::UPDATE;
	i32 cnt = min(v.GetCount(), total_cols - fixed_cols);
	i32 r0 = vitems[r].id;
	i32 c = fixed_cols + column_offset;
	for(i32 i = data_offset; i < cnt; i++)
		items[r0][c++].val = v[i];

	RefreshRow(r, false, 0);
}

void GridCtrl::Set(const Vec<Val> &v, i32 data_offset /* = 0*/, i32 column_offset /* = 0*/)
{
	i32 r = rowidx - fixed_rows;
	Set(r, v, data_offset, column_offset);
}

void GridCtrl::SetAny(i32 r, i32 c, const Val &val)
{
	Set0(r, c, val);
}

void GridCtrl::SetRaw(i32 r, i32 c, const Val &val)
{
	items[r][c].val = val;
}

void GridCtrl::SetIndicator(i32 r, const Val &val)
{
	Set0(r, 0, val);
}

void GridCtrl::SetCtrl(i32 r, i32 c, Ctrl& ctrl)
{
	r += fixed_rows;
	c += fixed_cols;
...
void GridCtrl::SetCtrl(i32 c, Ctrl& ctrl)
{
	c += fixed_cols;
...
void GridCtrl::ClearCtrl(i32 r, i32 c)
{
...
void GridCtrl::SetCtrlVal(i32 r, i32 c, const Val &val)
{
	c += fixed_cols;
	i32 ri = vitems[r].id;
	Ctrl * ctrl = items[ri][c].ctrl;
...
void GridCtrl::SetCtrlVal(i32 c, const Val &val)
{
	SetCtrlVal(rowidx, c, val);
}

void GridCtrl::SetLast(i32 c, const Val &val)
{
	c += fixed_cols;
...
void GridCtrl::SetFixed(i32 r, i32 c, const Val &val)
{
	items[r][c + 1].val = val;
	Refresh();
}

Val GridCtrl::GetFixed(i32 r, i32 c) const
{
	return items[vitems[r].id][c + fixed_cols].val;
}

Val GridCtrl::GetFixed(i32 c) const
{
	return items[0][c + fixed_cols].val;
}

Val GridCtrl::Get0(i32 r, i32 c) const
{
	r = vitems[r].id;
...
Vec<Val> GridCtrl::ReadCol(i32 n, i32 start_row, i32 end_row) const
{
	Vec<Val> v;
	i32 idx = hitems[n < 0 ? colidx : n + fixed_cols].id;
	...
	for(i32 i = start_row; i <= end_row; i++)
		v.Add(items[i][idx].val);

	return v;
}

Vec<Val> GridCtrl::ReadRow(i32 n, i32 start_col, i32 end_col) const
{
	Vec<Val> v;
	i32 idy = vitems[n < 0 ? rowidx : n + fixed_rows].id;
	...
	for(i32 i = start_col; i <= end_col; i++)
		v.Add(items[idy][i].val);

	return v;
}

Vec< Vec<Val> > GridCtrl::GetVals()
{
	Vec< Vec<Val> > v;

	i32 rows_cnt = total_rows - fixed_rows;
	i32 cols_cnt = total_cols - fixed_cols;

	v.SetCount(rows_cnt);

	for(i32 i = 0; i < rows_cnt; i++)
	{
		v[i].SetCount(cols_cnt);

		for(i32 j = 0; j < cols_cnt; j++)
		{
			const Val &val = items[i + fixed_rows][j + fixed_cols].val;
			if(IsType<AttrText>(val))
			{
				const AttrText& t = ValTo<AttrText>(val);
				v[i][j] = t.text;
...
void GridCtrl::SetVals(const Vec< Vec<Val> >& v)
{
	i32 rows_cnt = v.GetCount();

	if(rows_cnt <= 0)
		return;

	i32 cols_cnt = v[0].GetCount();

	i32 tc = total_cols - fixed_cols;
	if(cols_cnt > tc)
		cols_cnt = tc;
...
GridCtrl& GridCtrl::Add(const Vec<Val> &v, i32 offset, i32 count, bool hidden)
{
	Append0(1, hidden ? 0 : GD_ROW_HEIGHT);

	i32 cnt = min(count < 0 ? v.GetCount() : count,
	              total_cols - fixed_cols);

	i32 r0 = total_rows - 1;
	i32 r = vitems[r0].id;
	for(i32 i = offset; i < cnt; i++)
...
GridCtrl::ItemRect& GridCtrl::GetColumn(i32 n)
{
...
GridCtrl::ItemRect& GridCtrl::GetRow(i32 n)
{
...
i32 GridCtrl::GetCurrentRow() const
{
...
GridCtrl::Item& GridCtrl::GetCell(i32 n, i32 m)
{
	return items[vitems[n + fixed_rows].id][hitems[m + fixed_cols].id];
}

GridCtrl::Item& GridCtrl::GetCell(i32 n, Id id)
{
...
Image GridCtrl::CursorImage(Point p, dword keyflags)
{
	...
	if(resizing_cols && curSplitCol >= 0 || resizeCol)
	{
		...
		{
			i32 idy = GetMouseRow(p, true, p.y < fixed_height, true);
			if(idy >= 0)
			{
	...
	else if(resizing_rows && curSplitRow >= 0 || resizeRow)
	{
		...
		{
			i32 idx = GetMouseCol(p, true, p.x < fixed_width, true);
			if(idx >= 0)
			{
...
GridCtrl::CurState GridCtrl::SetCursor0(i32 x, i32 y, i32 opt, i32 dirx, i32 diry)
{
	return SetCursor0(Point(x, y), opt, dirx, diry);
}

GridCtrl::CurState GridCtrl::SetCursor0(Point p, i32 opt, i32 dirx, i32 diry)
{
	CurState cs;
	if(!row_changing)
	{
	...
	if(!highlight && mouse_valid)
	{
		...
		i32 fc = max(fixed_cols, firstVisCol);
		i32 lc = lastVisCol;
		i32 fr = max(fixed_rows, firstVisRow);
		i32 lr = lastVisRow;
		...
		while(true)
		{
			...
			if(newvalid)
			{
				i32 idx = hitems[tmpcur.x].id;
				i32 idy = vitems[tmpcur.y].id;
...
i32 GridCtrl::GetFixedWidth()
{
	return GetWidth(fixed_cols);
}

i32 GridCtrl::GetFixedHeight()
{
	return GetHeight(fixed_rows);
}

i32 GridCtrl::GetFirst0(Vec<ItemRect> &its, i32 total, i32 sb, i32 p)
{
	i32 l = 0;
	i32 r = total - 1;
	...
	while(l <= r)
	{
		...
		i32 p0 = its[i].nLeft(sb);
		i32 p1 = its[i].nRight(sb);
...
i32 GridCtrl::GetFirstVisCol(i32 p)
{
	return total_cols <= 2 ? fixed_cols : GetFirst0(hitems, total_cols, sbx, p);
}

i32 GridCtrl::GetFirstVisRow(i32 p)
{
	return total_rows <= 1 ? fixed_rows : GetFirst0(vitems, total_rows, sby, p);
}

GridCtrl& GridCtrl::SetColWidth(i32 n, i32 width, bool recalc /* = true */)
{
...
GridCtrl& GridCtrl::SetRowHeight(i32 n, i32 height, bool recalc)
{
...
bool GridCtrl::SetDiffItemSize(bool horizontal, RectItems &its, i32 n, i32 diff, bool newsize)
{
	if(diff == 0)
		return false;
	...
	i32 resize_mode = horizontal ? resize_col_mode : resize_row_mode;

	if(resize_mode > 0 && diff > 0)
	{
		bool ismin = true;
		for(i32 i = n + 1; i < (horizontal ? total_cols : total_rows); i++)
			if(!its[i].IsMin())
			{
				ismin = false;
				break;
...
void GridCtrl::Recalc(bool horizontal, RectItems &its, i32 n, double size, double diff)
{
	its[n].size = size;

	Size sz = GetSize();
	i32 cnt = horizontal ? total_cols : total_rows;
	i32 maxsize = horizontal ? sz.cx : sz.cy;
	i32 tcnt = cnt;
...
void GridCtrl::CalcIntPos(RectItems &its, i32 n, i32 maxsize, i32 cnt, i32 resize_mode, bool renumber)
{
	its[0].npos = 0;
	...
	for(i32 i = (renumber ? 1 : max(1, n)); i <= cnt ; i++)
	{
	...
	if(resize_mode > 0)
	{
		i32 size = maxsize - its[last_vis].npos;
...
bool GridCtrl::UpdateSizes()
{
	...
	i32 prev_summary_height = summary_height;

	summary_height = summary_row ? GD_HDR_HEIGHT : 0;

	size_changed = prev_summary_height != summary_height;

	i32 new_fixed_width  = fixed_cols ? hitems[fixed_cols - 1].nRight() : 0;
	i32 new_fixed_height = fixed_rows ? vitems[fixed_rows - 1].nRight() : 0;
...
bool GridCtrl::Recalc(bool horizontal, RectItems &its, i32 resize_mode)
{
	Size sz = GetSize();
	...
	i32 fixed = horizontal ? fixed_cols : fixed_rows;
	i32 cnt = horizontal ? total_cols : total_rows;
	i32 tcnt = cnt;
	...
	{
		i32 cs = horizontal ? sz.cx - fixed_width
		                    : sz.cy - fixed_height;

		//i32 cs = horizontal ? sz.cx : sz.cy;
...
bool GridCtrl::RecalcCols(i32 mode)
{
	return Recalc(true, hitems, mode);
}

bool GridCtrl::RecalcRows(i32 mode)
{
	return Recalc(false, vitems, mode);
}

i32 GridCtrl::GetSplitCol(const Point &p, i32 splitSize, bool full)
{
	if(total_cols < 2)
		return -1;
	...
	i32 tc = splitSize >= 0 ? (resize_col_mode == 0 ? 0 : 1) : 0;

	i32 fc = lastVisCol - tc;
	i32 lc = resizing_fixed_cols ? 1 : firstVisCol;
	...
	else
	{
		...
		for(i32 i = fc; i >= lc; i--)
		{
			...
			i32 x = hitems[c].nLeft(diff) + hitems[c].nWidth() / 2;
			if(p.x >= x)
...
i32 GridCtrl::GetSplitRow(const Point &p, i32 splitSize, bool full)
{
	if(total_rows < 2)
		return -1;
	...
	i32 tr = splitSize >= 0 ? (resize_row_mode == 0 ? 0 : 1) : 0;

	i32 fr = lastVisRow - tr;
	i32 lr = p.y < fixed_height && resizing_fixed_rows ? 0 : firstVisRow;
	...
	else
	{
		...
		for(i32 i = fr; i >= lr; i--)
		{
			...
			i32 y = vitems[c].nTop(diff) + vitems[c].nHeight() / 2;
			if(p.y >= y)
...
bool GridCtrl::IsValidCursor(const Point &p, i32 fc, i32 lc, i32 fr, i32 lr) const
{
...
bool GridCtrl::IsValidCursor(i32 c) const
{
	c += fixed_rows;
	return c >= fixed_rows && c < total_rows;
}

bool GridCtrl::IsRowEditable(i32 r)
{
	if(r < 0)
		r = curpos.y;
	else
...
bool GridCtrl::IsRowClickable(i32 r /* = -1*/)
{
	if(r < 0)
		r = curpos.y;
	else
...
GridCtrl& GridCtrl::Indicator(bool b, i32 size)
{
	if(size < 0)
		size = GD_IND_WIDTH;
	indicator = b;
...
void GridCtrl::RefreshRow(i32 n, bool relative, bool fixed)
{
	if(!ready)
		return;
...
void GridCtrl::RefreshCol(i32 n, bool relative, bool fixed)
{
	if(!ready)
		return;
...
void GridCtrl::RefreshRows(i32 from, i32 to, bool relative, bool fixed)
{
	if(!ready)
		return;
	if(relative)
...
void GridCtrl::RefreshFrom(i32 from)
{
	Size sz = GetSize();
	i32 y = 0;
...
void GridCtrl::RefreshItem(i32 r, i32 c, bool relative)
{
	if(!ready)
		return;
	if(relative)
...
i32 GridCtrl::GetIdCol(i32 id, bool checkall) const
{
	for(i32 i = checkall ? 1 : fixed_cols; i < total_cols; i++)
	{
		if(id == hitems[i].id)
			return i;
	}
...
i32 GridCtrl::GetIdRow(i32 id, bool checkall) const
{
	for(i32 i = checkall ? 0 : fixed_rows; i < total_rows; i++)
	{
		if(id == vitems[i].id)
			return i;
	}
...
i32 GridCtrl::GetNextRow(i32 n)
{
	n += fixed_rows;
	for(i32 i = n + 1; i < total_rows; i++)
		if(!vitems[i].hidden)
			return i - fixed_rows;
	return -1;
}

i32 GridCtrl::GetPrevRow(i32 n)
{
	n += fixed_rows;
	for(i32 i = n - 1; i >= fixed_rows; i--)
		if(!vitems[i].hidden)
			return i - fixed_rows;
	return -1;
}
...
i32 GridCtrl::Find(const Val &v, i32 col, i32 start_from, i32 opt) const
{
	for(i32 i = fixed_rows + start_from; i < total_rows; i++)
	{
...
i32 GridCtrl::Find(const Val &v, Id id, i32 opt) const
{
	return Find(v, aliases.Get(id) - fixed_cols, 0, opt);
}

i32 GridCtrl::FindInRow(const Val& v, i32 row, i32 start_from) const
{
	for(i32 i = fixed_cols + start_from; i < total_cols; i++)
	{
...
i32 GridCtrl::FindCurrent(Id id, i32 opt) const
{
	i32 col = aliases.Get(id) - fixed_cols;
	return Find(Get(col), col, 0, opt);
}

i32 GridCtrl::Find(const Val &v0, Id id0, const Val&v1, Id id1, i32 opt) const
{
	i32 col0 = aliases.Get(id0);
	i32 col1 = aliases.Get(id1);
...
i32 GridCtrl::FindCurrent(Id id0, Id id1, i32 opt) const
{
	i32 col0 = aliases.Get(id0);
	i32 col1 = aliases.Get(id1);

	const Val& val0 = items[vitems[rowidx].id][col0].val;
	const Val& val1 = items[vitems[rowidx].id][col1].val;

	return Find(val0, id0, val1, id1, opt);
}

void GridCtrl::UpdateDefaults(i32 ri)
{
	for(i32 i = 1; i < total_cols; i++)
	...
	if(focused_ctrl)
	{
		...
		if(it.modified)
		{
			...
			if(updates)
			{
				...
				LGR(2, Format("[oldval : %s]", AsTxt(rowbkp[focused_ctrl_id])));
				LGR(2, Format("[newval : %s]", AsTxt(v)));
				#endif
				WhenUpdateCell();
...
void GridCtrl::UpdateCtrls(i32 opt /*= UC_CHECK_VIS | UC_SHOW | UC_CURSOR */)
{
	if(!valid_cursor)
		return;
...
void GridCtrl::SyncCtrls(i32 row, i32 col)
{
	if(!genr_ctrls)
		return;
	...
	i32 js = row < 0 ? 0 : row;
	i32 je = row < 0 ? total_rows : row + 1;

	i32 is = col < 0 ? 1 : col;
	i32 ie = col < 0 ? total_cols : col + 1;
...
void GridCtrl::SyncSummary()
{
	...
	else
	{
		for(i32 i = fixed_cols; i < total_cols; i++)
		{
			...
			for(i32 j = fixed_rows; j < total_rows; j++)
			{
...
void GridCtrl::SetCtrlFocus(i32 col)
{
	oldcur.x = curpos.x;
...
i32 GridCtrl::GetFocusedCtrlIndex()
{
	for(i32 i = 1; i < total_cols; i++)
	{
		i32 id = hitems[i].id;
...
void GridCtrl::Split(i32 state, bool sync)
{
...
i32 GridCtrl::GetResizePanelHeight() const
{
	return (resize_panel.GetHeight() + 2) * resize_panel_open;
}

Txt GridCtrl::GetColumnName(i32 n) const
{
	return hitems[GetIdCol(n + fixed_cols)].GetName();
}

Id GridCtrl::GetColumnId(i32 n) const
{
	return aliases.GetKey(n + fixed_cols);
}

void GridCtrl::SwapCols(i32 n, i32 m)
{
	if(m == n ||
...
bool GridCtrl::CanMoveCol(i32 n, i32 m)
{
	if(m == n || m == n - 1 ||
	   n < 0 || n > total_cols - 1 ||
	   m < 0 || m > total_cols - 1)
...
void GridCtrl::MoveCol(i32 n, i32 m)
{
	LG(0, "%d->%d", n, m);
...
bool GridCtrl::MoveRow(i32 n, i32 m, bool repaint)
{
	LG(0, "%d->%d", n, m);
...
void GridCtrl::MoveRows(i32 n, bool onerow)
{
	if(selected_rows && !onerow)
	{
		...
		for(i32 i = total_rows - 1; i >= fixed_rows; i--)
			if(vitems[i].IsSelect())
			{
				vitems.Remove(i);
				if(i < n)
...
bool GridCtrl::SwapRows(i32 n, i32 m, bool repaint)
{
	if(isedit || m == n ||
...
void GridCtrl::SwapDown(i32 cnt)
{
	i32 yp = 0;
	bool first = false;
	bool repaint = false;
	...
	i32 cy = GetSize().cy - bar.GetSize().cy;
...
GridCtrl& GridCtrl::ColoringMode(i32 m)
{
	coloring_mode = m;
	return *this;
}
...
void GridCtrl::ClearRow(i32 r, i32 column_offset)
{
	if(r < 0)
		r = rowidx;
	else
		r -= fixed_rows;
	for(i32 i = fixed_cols + column_offset; i < total_cols; i++)
...
void GridCtrl::Clear(bool columns)
{
	...
	i32 nrows = columns ? 1 : fixed_rows;
...
i32 GridCtrl::SetCursorId(i32 id)
{
	id += fixed_rows;
	for(i32 i = fixed_rows; i < total_rows; i++)
	{
...
i32 GridCtrl::GetCursor(bool rel) const
{
	if(rel)
...
i32 GridCtrl::GetPrevCursor(bool rel) const
{
	if(rel)
...
i32 GridCtrl::GetCursor(i32 uid) const
{
	for(i32 i = fixed_rows; i < total_rows; i++)
		if(vitems[i].uid == uid)
			return i - fixed_rows;
...
i32 GridCtrl::GetRowId() const
{
	return valid_cursor ? vitems[curpos.y].id - fixed_rows : -1;
}

i32 GridCtrl::GetColId() const
{
	return valid_cursor ? hitems[curpos.x].id - fixed_cols: -1;
}

i32 GridCtrl::GetRowId(i32 n) const { return vitems[n + fixed_rows].id - fixed_rows; }
i32 GridCtrl::GetColId(i32 n) const { return hitems[n + fixed_cols].id - fixed_cols; }

i32 GridCtrl::GetColUId() const
{
	return valid_cursor ? hitems[curpos.x].uid : -1;
}

i32 GridCtrl::GetRowUId() const
{
	return valid_cursor ? vitems[curpos.y].uid : -1;
}

i32 GridCtrl::FindCol(i32 id) const
{
	for(i32 i = fixed_cols; i < total_cols; i++)
		if(hitems[i].id == id)
			return i - fixed_cols;
...
i32 GridCtrl::FindCol(const Id& id) const
{
	for(i32 i = fixed_cols; i < total_cols; i++)
...
i32 GridCtrl::FindCol(const Txt& s) const
{
	for(i32 i = fixed_cols; i < total_cols; i++)
...
i32 GridCtrl::FindRow(i32 id) const
{
	for(i32 i = fixed_rows; i < total_rows; i++)
		if(vitems[i].id == id)
			return i - fixed_rows;
...
bool GridCtrl::Go0(i32 jump, bool scroll, bool goleft, bool ctrlmode)
{
	if(IsEmpty())
		return false;
	...
	i32 opt = /*ctrls*/ ctrlmode ? CU_CTRLMODE : 0;
	...
	switch(jump)
	{
		...
		case GO_NEXT:
		{
			...
			i32 b = vitems[curpos.y].nBottom(sby);
			i32 r = sz.cy - summary_height;
		...
		case GO_PREV:
		{
			...
			i32 t = vitems[curpos.y].nTop(sby + fixed_height);
		...
		case GO_PAGEUP:
		{
			...
			i32 yn = vitems[c].nTop() - sz.cy;
			i32 ya = vitems[c].nTop(sby);
			...
			{
				i32 yc = vitems[c].nTop();
				i32 yt = vitems[cp].nTop(sby);
				i32 yb = vitems[cp].nBottom(sby);
...
void GridCtrl::GoCursorLeftRight()
{
	...
	{
		i32 l = hitems[curpos.x].nLeft(sbx + fixed_width);
		i32 r = hitems[curpos.x].nRight(sbx);
		i32 w = GetSize().cx;
	...
	if(resize_row_mode == 0)
	{
		i32 t = vitems[curpos.y].nTop(sby + fixed_height);
		i32 b = vitems[curpos.y].nBottom(sby);
		i32 h = GetSize().cy - summary_height;
...
Ctrl * GridCtrl::GetCtrl(i32 r, i32 c, bool check_visibility, bool hrel, bool vrel, bool check_edits)
{
	i32 idx = hrel ? fixed_cols + c : hitems[c].id;
	i32 idy = vrel ? fixed_rows + r : vitems[r].id;
...
Ctrl * GridCtrl::GetCtrl(i32 r, i32 c)
{
	return GetCtrl(r + fixed_rows, c, true, true, false);
}

Ctrl * GridCtrl::GetCtrlAt(i32 r, i32 c)
{
	return GetCtrl(r + fixed_rows, c, false, true, false);
}

Ctrl * GridCtrl::GetCtrl(i32 c)
{
...
void GridCtrl::GoTo(i32 r, bool setcursor, bool scroll)
{
	r += fixed_rows;
...
void GridCtrl::GoTo(i32 r, i32 c, bool setcursor, bool scroll)
{
	c += fixed_cols;
	r += fixed_rows;
...
i32 GridCtrl::GetCount() const      { return total_rows - fixed_rows; }
i32 GridCtrl::GetFixedCount() const { return fixed_rows;              }
i32 GridCtrl::GetTotalCount() const { return total_rows;              }

i32 GridCtrl::GetVisibleCount() const
{
	i32 cnt = 0;
	for(i32 i = fixed_rows; i < total_rows; i++)
		if(!vitems[i].hidden)
...
GridCtrl& GridCtrl::SetColsMin(i32 size)
{
	for(i32 i = 1; i < total_cols; i++)
		hitems[i].min = size;
...
GridCtrl& GridCtrl::SetColsMax(i32 size)
{
	for(i32 i = 0; i < total_rows; i++)
		hitems[i].max = size;
...
void GridCtrl::Repaint(bool do_recalc_cols /* = false*/, bool do_recalc_rows /* = false*/, i32 opt)
{
	if(do_recalc_cols)
	{
		if(ready)
...
GridCtrl& GridCtrl::ResizeColMode(i32 m)
{
	resize_col_mode = m;
	recalc_cols = true;
	RefreshLayout();
...
GridCtrl& GridCtrl::ResizeRowMode(i32 m)
{
	resize_row_mode = m;
	recalc_rows = true;
	RefreshLayout();
...
void GridCtrl::Insert0(i32 row, i32 cnt /* = 1*/, bool recalc /* = true*/, bool refresh /* = true*/, i32 size /* = GD_ROW_HEIGHT*/)
{
	i32 id;
...
bool GridCtrl::Remove0(i32 row, i32 cnt /* = 1*/, bool recalc /* = true*/, bool refresh /* = true*/, bool whens /* = true*/)
{
	if(cnt < 0)
		return false;
	...
	for(i32	i = 0; i < cnt; i++)
	{
		...
		i32 id = vitems[rowidx].id;
		i32 op = vitems[rowidx].operation;
...
i32 GridCtrl::Append0(i32 cnt, i32 size, bool refresh)
{
	if(size < 0)
		size = GD_ROW_HEIGHT;
	...
	i32 j = total_rows;
	i32 k = j;
	i32 n = j > 0 ? vitems[j - 1].n + (i32) vitems[j - 1].hidden : 0;
	for(i32 i = 0; i < cnt; i++)
	{
...
bool GridCtrl::Duplicate0(i32 row, i32 cnt, bool recalc, bool refresh)
{
	i32 id;
	i32 nrow = row + cnt;
	...
	for(i32 i = 0; i < cnt; i++)
	{
		...
		i32 oid = vitems[row + i].id;
		for(i32 j = 1; j < total_cols; j++)
...
i32 GridCtrl::Append(i32 cnt, bool refresh, i32 height)
{
	return Append0(cnt, height, refresh);
}

void GridCtrl::Insert(i32 i, i32 cnt)
{
	Insert0(fixed_rows + i, cnt);
}

void GridCtrl::Remove(i32 i, i32 cnt)
{
	Remove0(i < 0 ? rowidx : fixed_rows + i, cnt);
}

void GridCtrl::RemoveFirst(i32 cnt /* = 1*/)
{
	Remove0(fixed_rows, min(total_rows - fixed_rows, cnt));
}

void GridCtrl::RemoveLast(i32 cnt /* = 1*/)
{
	Remove0(total_rows - cnt, min(total_rows - fixed_rows, cnt));
}

void GridCtrl::Duplicate(i32 i, i32 cnt)
{
...
void GridCtrl::DoRemove()
{
	...
	else
	{
		...
		for(i32 i = minRowSelected; i <= maxRowSelected; i++)
		{
			i32 rid = remove_hides ? i : minRowSelected + not_removed;
...
GridCtrl& GridCtrl::ShowRow(i32 n, bool refresh)
{
	if(!vitems[n].hidden)
		return *this;
...
GridCtrl& GridCtrl::HideRow(i32 n, bool refresh)
{
	if(n < 0)
		n = rowidx;
...
GridCtrl& GridCtrl::ShowColumn(i32 n, bool refresh)
{
	if(!hitems[n].hidden)
		return *this;
...
GridCtrl& GridCtrl::HideColumn(i32 n, bool refresh)
{
	if(hitems[n].hidden)
		return *this;
...
void GridCtrl::MenuHideColumn(i32 n)
{
	if(hitems[n].hidden)
		ShowColumn(n);
	else
...
i32 GridCtrl::ShowMatchedRows(const WTxt &f)
{
	if(f.IsEmpty())
	{
		if(search_highlight)
	...
	for(i32 i = fixed_rows; i < total_rows; i++)
	{
		...
		for(i32 j = fixed_cols; j < total_cols; j++)
		{
			...
			if(Match(f, (WTxt) GetStdConvertedColumn(idh, it.val), s, e))
			{
				first_matched_row = i;
				rowfnd = i;
...
bool GridCtrl::Match(const WTxt &f, const WTxt &s, i32 &fs, i32 &fe)
{
	i32 i = 0;
...
i32 GridCtrl::GetMinRowSelected()
{
	i32 i = fixed_rows;
...
i32 GridCtrl::GetMaxRowSelected()
{
	i32 i = total_rows - 1;
...
void GridCtrl::UpdateVisColRow(bool col)
{
	if(col)
	{
		...
		for(i32 i = total_cols - 1; i >= fixed_cols; i--)
			if(!hitems[i].hidden)
			{
				lastVisCol = i;
				break;
...
void GridCtrl::JoinCells(i32 left, i32 top, i32 right, i32 bottom, bool relative)
{
	i32 fc = relative ? fixed_cols : 0;
	i32 fr = relative ? fixed_rows : 0;
	...
	i32 idx = hitems[left].id;
	i32 idy = vitems[top].id;
	i32 cx = right - left;
	i32 cy = bottom - top;
...
void GridCtrl::JoinFixedCells(i32 left, i32 top, i32 right, i32 bottom)
{
	JoinCells(left, top, right, bottom, false);
}

void GridCtrl::JoinRow(i32 n, i32 left, i32 right)
{
	if(n < 0)
		n = rowidx;
...
void GridCtrl::JoinRow(i32 left, i32 right)
{
...
void GridCtrl::UpdateJoins(i32 row, i32 col, i32 cnt)
{
	if(row >= 0)
	{
		for(i32 i = 0; i < joins.GetCount(); i++)
		{
			JoinRect& jr = joins[i];

			i32 top = jr.r.top;
			i32 bottom = jr.r.bottom;

			if(row > top && row < bottom) // new row is inside cell rect, idy doesn't change but cy does
			{
				for(i32 r = jr.r.top; r <= jr.r.bottom; r++)
				{
					for(i32 c = jr.r.left; c <= jr.r.right; c++)
					{
						if(r < row || r > row)
						{
							Item& it = GetItem(r, c);
...
void GridPopUp::MouseWheel(Point p, i32 zdelta, dword flags)
{
...
void GridPopUp::PopUp(Ctrl *owner, i32 x, i32 y, i32 width, i32 height)
{
...
void GridCtrl::UpdateHighlighting(i32 mode, Point p)
{
	...
	{
		i32 col = GetMouseCol(p, true, true, false);
		i32 row = hrow = GetMouseRow(p, false, true, true);
...
Txt GridCtrl::GetColumnWidths()
{
	Txt s;
	for(i32 i = fixed_cols; i < total_cols; i++)
	{
		s += AsTxt(hitems[i].nsize);
		if(i < total_cols - 1)
			s += " ";
	}
	return s;
...
void GridCtrl::ColumnWidths(tukk s)
{
	Vec<Txt> w = DRX::Split(s, ' ');
	for(i32 i = 0; i < min(w.GetCount(), GetColumnCount()); i++)
		hitems[i + fixed_cols].Width(atoi(w[i]));
}

void GridCtrl::SetDisplay(i32 r, i32 c, GridDisplay& gd)
{
...
bool GridFind::Key(dword key, i32 count)
{
...
void GridPopUpHeader::PopUp(Ctrl *owner, i32 x, i32 y, i32 width, i32 height)
{
...
void GridButton::State(i32 reason)
{
	if(reason == CLOSE)
		img = 0;
}
...
void GridButton::SetButton(i32 b)
{
	n = b;
}
...
{
	i32 h = max(16, Draw::GetStdFontCy()) + 5;
	Height(h);
...
#define E__AddF0(I) \
GridCtrl& GridCtrl::Add(__List##I(E__Val)) { \
	i32 q = GetCount(); \
	__List##I(E__Addv0); \
	return *this; \
}
...
#define E__AddF1(I) \
GridCtrl::ItemRect& GridCtrl::AddRow(__List##I(E__Val)) { \
	i32 q = GetCount(); \
	ItemRect& ir = AddRow(); \
	__List##I(E__Addv1); \
	return ir; \
