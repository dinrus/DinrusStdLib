#ifndef _GridCtrl_GridUtils_h_
#define _GridCtrl_GridUtils_h_

#define BIT(x) (1 << x)

inline bool IsSet(i32 s, i32 b)
{
	return s & b;
}

inline void BitSet(dword &k, dword v, bool s)
{
	if(s) k |= v; else  k &= ~v;
}

inline bool BitInverse(dword &k, dword v)
{
	bool s = k & v;
	BitSet(k, v, !s);
	return s;
}

inline i32 Distance(const Point &p0, const Point &p1)
{
	return max(abs(p0.x - p1.x), abs(p0.y - p1.y));
}

inline i32 Round(double a)
{
#if defined(COMPILER_MSC) && defined(CPU_32)
	i32 retval;
	__asm fld a
	__asm fistp retval
	return retval;
#else
	return (i32) a;
#endif
}

#ifdef flagDEBUG
#define LG LogGui
#else
inline void LG_Dummy_Nop__(i32 level, tukk fmt, ...) {} // needed to silence CLANG warnings
inline void LG_Dummy_Nop__(tukk fmt, ...) {} // about unused value
#define LG LG_Dummy_Nop__
#endif
#define LGR LogGui

extern LineEdit *dlog;
extern i32       dlev;

void LogGui(tukk fmt, ...);
void LogGui(i32 level, tukk fmt, ...);

void LogCon(tukk fmt, ...);
void LogCon(i32 level, tukk fmt, ...);

#endif
