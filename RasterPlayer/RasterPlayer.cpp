...
#include <drx/RasterPlayer/RasterPlayer.h>
...
Rect Fit(const Size &frame, const Size &object) {
	...
	if (frame.cx/(double)frame.cy > objectAspect) {
		...
		return Rect(i32(x), 0, i32(x + objectAspect*frame.cy), frame.cy);
	} else {
		double y = (frame.cy - frame.cx/objectAspect)/2.;
		return Rect(0, i32(y), frame.cx, i32(y + frame.cx/objectAspect));
	}
}
...
bool RasterPlayer::LoadBuffer(const Txt &buffer) {
	if (buffer.IsEmpty())
		return false;
	TxtStream str(buffer);
	...
	Image previous;
	Rect r;
	for (i32 i = 0; i < raster->GetPageCount(); ++i) {
		if (previous) {
...
bool RasterPlayer::Load(const Txt &fileName) {
	FileIn in(fileName);
