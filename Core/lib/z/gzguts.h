...
typedef struct {
    ...
    tuk path;             /* path or fd for error messages */
    u32 size;          /* buffer size, zero if not allocated yet */
    u32 want;          /* requested buffer size, default is GZBUFSIZE */
    u8* in;      /* input buffer (double-sized when writing) */
    u8* out;     /* output buffer (double-sized when reading) */
...
/* shared functions */
void ZLIB_INTERNAL gz_error OF((gz_statep, i32, tukk ));
#if defined UNDER_CE
