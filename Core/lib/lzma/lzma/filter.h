 ...
 * \param[out]  out_pos     The next u8 will be written to out[*out_pos].
 *                          *out_pos is updated only if encoding succeeds.
 * \param       out_size    Size of the out buffer; the first u8 into
 ...
 *                          Set to NULL to use malloc() and free().
 * \param       in          Beginning of the input buffer
 * \param       in_pos      The next u8 will be read from in[*in_pos].
 *                          *in_pos is updated only if decoding succeeds.
 * \param       in_size     Size of the input buffer; the first u8 that
 ...
 * \param       out_size    out[out_size] is the first u8 to not write.
 *
 * \return      Possible lzma_ret values:
 *              - LZMA_OK: Encoding was successful.
 ...
 *                          Set to NULL to use malloc() and free().
 * \param       in          Beginning of the input buffer
 * \param[out]  in_pos      The next u8 will be read from in[*in_pos].
 *                          *in_pos is updated only if decoding succeeds.
 * \param       in_size     Size of the input buffer; the first u8 that
 *                          won't be read is in[in_size].
 *
 ...
 * i32 pos;
 * tukk msg = lzma_str_to_filters(str, &pos, filters, 0, NULL);
 * if (msg != NULL) {
 ...
 *                          an i32 to make it straightforward to use this
 ...
 */
extern LZMA_API(tukk ) lzma_str_to_filters(
		tukk str, i32 *error_pos, lzma_filter *filters,
...
extern LZMA_API(lzma_ret) lzma_str_from_filters(
		tuk* str, const lzma_filter *filters, uint32_t flags,
		const lzma_allocator *allocator)
		lzma_nothrow lzma_attr_warn_unused_result;
...
extern LZMA_API(lzma_ret) lzma_str_list_filters(
		tuk* str, lzma_vli filter_id, uint32_t flags,
		const lzma_allocator *allocator)
		lzma_nothrow lzma_attr_warn_unused_result;
