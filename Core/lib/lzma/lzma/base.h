 ...
 * sizeof(enum) can be different than sizeof(i32), and sizeof(enum) may
...
typedef struct {
	 ...
	 */
	uk (LZMA_API_CALL *alloc)(uk opaque, size_t nmemb, size_t size);
	 ...
	 */
	void (LZMA_API_CALL *free)(uk opaque, uk ptr);
 ...
 */
typedef struct {
	const uint8_t *next_in; /**< Pointer to the next input u8. */
