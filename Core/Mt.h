...
struct MtInspector {
	...
	MtInspector(tukk s, i32 n = -1) { name = s; number = n; locked = blocked = 0; }
	~MtInspector();
};
...
public:
	...
	bool        Priority(i32 percent); // 0 = lowest, 100 = normal
	void        StackSize(i32 bytes)           { stack_size = bytes; }
...
public:
	bool Wait(Mutex& m, i32 timeout_ms = -1);
	void Signal();
	void Broadcast();
...
for(static ::drx::Mutex o_ss_; !o_b_.load(std::memory_order_acquire);) \
	for(::drx::Mutex::Lock o_ss_lock__(o_ss_); !o_b_.load(std::memory_order_acquire); \
	    o_b_.store(true, std::memory_order_release))

#define ONCELOCK \
for(static ::drx::OnceFlag o_b_; !o_b_.load(std::memory_order_acquire);) ONCELOCK_(o_b_)
...
for(bool i_b_ = true; i_b_;) \
	for(static DRX::Mutex i_ss_; i_b_;) \
		for(DRX::Mutex::Lock i_ss_lock__(i_ss_); i_b_; i_b_ = false)
...
for(DRX::H_l_ i_ss_lock__(cs); i_ss_lock__.b; i_ss_lock__.b = false)
