...
template <class K, class TT, class Less> class SortedVecMap;

struct InVectorSlave__ {
	virtual void Clear() = 0;
	virtual void Count(i32 n) = 0;
	virtual void Split(i32 blki, i32 nextsize) = 0;
	virtual void AddFirst() = 0;
	virtual void Insert(i32 blki, i32 pos) = 0;
	virtual void Join(i32 blki) = 0;
	virtual void Remove(i32 blki, i32 pos, i32 n) = 0;
	virtual void RemoveBlk(i32 blki, i32 n) = 0;
	virtual void Index(i32 blki, i32 n) = 0;
...
public:
	...
	template <class K, class TT, class Less> friend class SortedVecMap;
...
private:
	...
	InVectorSlave__ *Slave()          { return (InVectorSlave__ *)((u8 *)this + slave); }
	void SetSlave(InVectorSlave__ *s) { slave = (u16)((u8 *)s - (u8 *)this); }

	void SetCache(i32 blki, i32 offset) const;
	void ClearCache() const;
	i32  FindBlock0(i32& pos, i32& off) const;
	i32  FindBlock(i32& pos, i32& off) const;
	i32  FindBlock(i32& pos) const;
	void SetBlkPar();
	
	template <class L>
	i32  FindUpperBound(const T& val, const L& less, i32& off, i32& pos) const;

	template <class L>
	i32  FindLowerBound(const T& val, const L& less, i32& off, i32& pos) const;

	void Reindex();
	void Index(i32 q, i32 n);
	void Count(i32 n)                               { count += n; }
	void Join(i32 blki);

	bool JoinSmall(i32 blki);
	T   *Insert0(i32 ii, i32 blki, i32 pos, i32 off, const T *val);
	T   *Insert0(i32 ii, const T *val);
	
	void Reset();

	void SetIter(ConstIterator& it, i32 ii) const;
	...
	void     Insert_(i32 ii, const Range& r, bool def);

#ifdef flagIVTEST
	void Check(i32 blki, i32 offset) const;
#endif
public:
	T&       Insert(i32 i)                          { return *Insert0(i, NULL); }
	T&       Insert(i32 i, const T& x)              { return *Insert0(i, &x); }
	template <class Range>
	void     InsertRange(i32 i, const Range& r)     { Insert_(i, r, false); }
	void     InsertN(i32 i, i32 count)              { Insert_(i, ConstRange<T>(count), true); }
	...
	void     AddN(i32 n)                            { InsertN(GetCount(), n); }
	
	i32      GetCount() const                       { return count; }
	bool     IsEmpty() const                        { return GetCount() == 0; }

	void     Trim(i32 n)                            { Remove(n, GetCount() - n); }
	void     SetCount(i32 n);
	void     Clear();

	T&       At(i32 i)                              { if(i >= GetCount()) SetCount(i + 1); return (*this)[i]; }

	void     Shrink();

	void     Set(i32 i, const T& x, i32 count);
	T&       Set(i32 i, const T& x)                 { Set(i, x, 1); return (*this)(i); }

	void     Swap(i32 i1, i32 i2)                   { DRX::Swap((*this)[i1], (*this)[i2]); }

	void     Drop(i32 n = 1)                        { ASSERT(n <= GetCount()); Trim(GetCount() - n); }
	...
	template <class L>
	i32 FindUpperBound(const T& val, const L& less) const { i32 off, pos; FindUpperBound(val, less, off, pos); return off + pos; }
	i32 FindUpperBound(const T& val) const                { return FindUpperBound(val, StdLess<T>()); }

	template <class L>
	i32 FindLowerBound(const T& val, const L& less) const { i32 off, pos; FindLowerBound(val, less, off, pos); return off + pos; }
	i32 FindLowerBound(const T& val) const                { return FindLowerBound(val, StdLess<T>()); }

	template <class L>
	i32 InsertUpperBound(const T& val, const L& less);
	i32 InsertUpperBound(const T& val)                    { return InsertUpperBound(val, StdLess<T>()); }
	
	template <class L>
	i32 Find(const T& val, const L& less) const;
	i32 Find(const T& val) const                         { return Find(val, StdLess<T>()); }
	...
	void Reserve(i32) {} // Does nothing, but needed for unified interface (e.g. StreamContainer)
#ifdef DEPRECATED
	typedef T        ValType;
	ConstIterator    GetIter(i32 pos) const         { ConstIterator it; SetIter(it, pos); return it; }
	Iterator         GetIter(i32 pos)               { Iterator it; SetIter(it, pos); return it; }
#endif
};
...
public:
	force_inline i32 GetIndex() const              { return i32(ptr - begin) + offset; }
	...
	force_inline ConstIterator  operator++(i32)    { ConstIterator t = *this; ++*this; return t; }
	force_inline ConstIterator  operator--(i32)    { ConstIterator t = *this; --*this; return t; }

	force_inline ConstIterator& operator+=(i32 d);
	ConstIterator& operator-=(i32 d)               { return operator+=(-d); }

	ConstIterator operator+(i32 d) const           { ConstIterator t = *this; t += d; return t; }
	ConstIterator operator-(i32 d) const           { return operator+(-d); }

	i32  operator-(const ConstIterator& x) const   { return GetIndex() - x.GetIndex(); }
	...
	const T *operator->() const                    { return ptr; }
	const T& operator[](i32 i) const               { ConstIterator h = *this; h += i; return *h; }
...
public:
	...
	Iterator& operator+=(i32 d)                    { ConstIterator::operator+=(d); return *this; }
	Iterator& operator-=(i32 d)                    { return operator+=(-d); }

	Iterator operator+(i32 d) const                { Iterator t = *this; t += d; return t; }
	Iterator operator-(i32 d) const                { return operator+(-d); }

	i32  operator-(const Iterator& x) const        { return B::GetIndex() - x.GetIndex(); }

	T& operator*()                                 { return *(T*)B::ptr; }
	T *operator->()                                { return (T*)B::ptr; }
	T& operator[](i32 i)                           { Iterator h = *this; h += i; return *h; }
...
private:
	...
	T&       Get(i32 i) const       { return *(T *)iv[i]; }
	void     Delete(IVIter i, i32 count);
	void     Delete(i32 i, i32 count);
	void     Init(i32 i, i32 count);
	...
	template <class K, class V, class L> friend class SortedVecMap;

public:
	T&       Insert(i32 i, T *newt)                 { iv.Insert(i, newt); return *newt; }
	T&       Insert(i32 i)                          { return Insert(i, new T); }
	T&       Insert(i32 i, const T& x)              { return Insert(i, new T(x)); }
	template<class TT, class... Args>
	TT&      InsertCreate(i32 i, Args&&... args)    { TT *q = new TT(std::forward<Args>(args)...); Insert(i, q); return *q; }

	void     InsertN(i32 i, i32 count);
	template <class Range>
	...
	i32      GetCount() const                       { return iv.GetCount(); }
	bool     IsEmpty() const                        { return GetCount() == 0; }
	...
	template <class L>
	i32 FindUpperBound(const T& val, const L& less) const  { return iv.FindUpperBound((T*)&val, ALess<L>(less)); }
	i32 FindUpperBound(const T& val) const                 { return FindUpperBound(val, StdLess<T>()); }

	template <class L>
	i32 FindLowerBound(const T& val, const L& less) const  { return iv.FindLowerBound((T*)&val, ALess<L>(less)); }
	i32 FindLowerBound(const T& val) const                 { return FindLowerBound(val, StdLess<T>()); }

	template <class L>
	i32 InsertUpperBound(const T& val, const L& lss)       { return iv.InsertUpperBound(new T(val), ALess<L>(lss)); }
	i32 InsertUpperBound(const T& val)                     { return InsertUpperBound(val, StdLess<T>()); }
	
	template <class L>
	i32 Find(const T& val, const L& less) const            { return iv.Find((T*)&val, ALess<L>(less)); }
	i32 Find(const T& val) const                           { return Find(val, StdLess<T>()); }
...
public:
	force_inline i32 GetIndex() const              { return it.GetIndex(); }
	...
	force_inline ConstIterator& operator+=(i32 d)  { it += d; return *this; }
	ConstIterator& operator-=(i32 d)               { return operator+=(-d); }
...
public:
	...
	const T *operator->() const                    { return (T *)*B::it; }
	const T& operator[](i32 i) const               { Iterator h = *this; h += i; return *h; }
...
class SortedIndex : MoveableAndDeepCopyOption< SortedIndex<T, Less> > {
	...
	template <class K, class TT, class Lss> friend class SortedVecMap;
	template <class K, class TT, class Lss, class Data> friend class SortedAMap;

public:
	i32           Add(const T& x)                  { return iv.InsertUpperBound(x, Less()); }
	i32           FindAdd(const T& key);
	SortedIndex&  operator<<(const T& x)           { Add(x); return *this; }
	
	i32           FindLowerBound(const T& x) const { return iv.FindLowerBound(x, Less()); }
	i32           FindUpperBound(const T& x) const { return iv.FindUpperBound(x, Less()); }

	i32           Find(const T& x) const           { return iv.Find(x, Less()); }
	i32           FindNext(i32 i) const;
	i32           FindLast(const T& x) const;
	i32           FindPrev(i32 i) const;

	void          Remove(i32 i)                    { iv.Remove(i); }
	void          Remove(i32 i, i32 count)         { iv.Remove(i, count); }
	i32           RemoveKey(const T& x);

	const T&      operator[](i32 i) const          { return iv[i]; }
	i32           GetCount() const                 { return iv.GetCount(); }
	bool          IsEmpty() const                  { return iv.IsEmpty(); }
	void          Clear()                          { iv.Clear(); }

	void          Trim(i32 n)                      { return iv.Trim(n); }
	void          Drop(i32 n = 1)                  { iv.Drop(n); }
	...
	SortedIndex()                                        {}
	SortedIndex(const SortedIndex& s, i32) : iv(s.iv, 1) {}
	
	void Swap(SortedIndex& a)                        { iv.Swap(a.iv); }

	void     Serialize(Stream& s)                               { iv.Serialize(s); }
	void     Xmlize(XmlIO& xio, tukk itemtag = "key")    { iv.Xmlize(xio, itemtag); }
...
#ifdef DEPRECATED
	ConstIterator    GetIter(i32 pos) const         { return iv.GetIter(pos); }
...
protected:
	...
	T&       At(i32 i) const                        { return (T&)value.data[i]; }

public:
	i32      FindLowerBound(const K& k) const       { return key.FindLowerBound(k); }
	i32      FindUpperBound(const K& k) const       { return key.FindUpperBound(k); }
	...
	const T& Get(const K& k, const T& d) const      { i32 i = Find(k); return i >= 0 ? At(i) : d; }

	T       *FindPtr(const K& k)                    { i32 i = Find(k); return i >= 0 ? &At(i) : NULL; }
	const T *FindPtr(const K& k) const              { i32 i = Find(k); return i >= 0 ? &At(i) : NULL; }

	const T& operator[](i32 i) const                { return At(i); }
	T&       operator[](i32 i)                      { return At(i); }
	...
	void     Remove(i32 i)                          { key.Remove(i); }
	void     Remove(i32 i, i32 count)               { key.Remove(i, count); }
	i32      RemoveKey(const K& k)                  { return key.RemoveKey(k); }
	...
	i32      Compare(const SortedAMap& b) const     { return CompareMap(*this, b); }
...
struct Slaved_InVector__ : InVectorSlave__ {
	...
	virtual void Count(i32 n)                     { data.count += n; }
	virtual void Insert(i32 blki, i32 pos);
	virtual void Split(i32 blki, i32 nextsize);
	virtual void AddFirst();
	virtual void RemoveBlk(i32 blki, i32 n)       { data.data.Remove(blki, n); }
	virtual void Join(i32 blki)                   { data.Join(blki); }
	virtual void Remove(i32 blki, i32 pos, i32 n) { data.data[blki].Remove(pos, n); }
	virtual void Index(i32 blki, i32 n)           { data.Index(blki, n); }
	virtual void Reindex()                        { data.Reindex(); }
//	virtual void Serialize(Stream& s)             { data.Serialize(s); }
	virtual void Shrink()                         { data.Shrink(); }

	T& Get(i32 blki, i32 i) const                 { return *(T*)&data.data[blki][i]; }
};

template <class K, class T, class Less = StdLess<K> >
class SortedVecMap : public SortedAMap<K, T, Less, Slaved_InVector__<T> >,
                        public MoveableAndDeepCopyOption<SortedVecMap<K, T, Less> > {
...
public:
	...
	i32      FindAdd(const K& k)                    { return B::key.FindAdd(k); }
	i32      FindAdd(const K& k, const T& init);

	T&       GetAdd(const K& k)                     { return B::At(FindAdd(k)); }
	T&       GetAdd(const K& k, const T& x)         { return B::At(FindAdd(k, x)); }

	SortedVecMap& operator()(const K& k, const T& v) { Add(k, v); return *this; }

	SortedVecMap()                               { B::SetSlave(); }
	SortedVecMap(SortedVecMap&&);
	SortedVecMap& operator=(SortedVecMap&&);
	SortedVecMap(const SortedVecMap& s, i32);
	
	SortedVecMap(std::initializer_list<std::pair<K, T>> init) : SortedVecMap() { for(const auto& i : init) Add(i.first, i.second); }

	void     Swap(SortedVecMap& x);
	...
	const InVector<T>& GetVals() const            { return B::value.data; }

	friend void    Swap(SortedVecMap& a, SortedVecMap& b) { a.Swap(b); }
	...
	STL_SORTED_MAP_COMPATIBILITY(SortedVecMap<K _cm_ T _cm_ Less>)

#ifdef DEPRECATED
	typedef T                                       ValType;
	Iterator         GetIter(i32 pos)               { return B::value.data.GetIter(pos); }
	ConstIterator    GetIter(i32 pos) const         { return B::value.data.GetIter(pos); }
#endif
};
...
struct Slaved_InArray__ : InVectorSlave__ {
	...
	virtual void Clear()                          { data.Clear(); }
	virtual void Count(i32 n)                     { data.iv.count += n; }
	virtual void Insert(i32 blki, i32 pos);
	virtual void Split(i32 blki, i32 nextsize);
	virtual void AddFirst();
	virtual void RemoveBlk(i32 blki, i32 n)       { data.iv.data.Remove(blki, n); }
	virtual void Join(i32 blki)                   { data.iv.Join(blki); }
	virtual void Remove(i32 blki, i32 pos, i32 n);
	virtual void Index(i32 blki, i32 n)           { data.iv.Index(blki, n); }
...
//	virtual void Serialize(Stream& s)             { data.iv.Serialize(s); }
	...
	T& Get(i32 blki, i32 i) const                 { return *(T*)data.iv.data[blki][i]; }
	T *Detach(i32 i)                              { T *x = data.iv[i]; data.iv[i] = NULL; return x; }
};
...
public:
	...
	i32      FindAdd(const K& k)                  { B::value.res = NULL; return B::key.FindAdd(k); }
	i32      FindAdd(const K& k, const T& init);

	T&       GetAdd(const K& k)                   { return B::At(FindAdd(k)); }
	T&       GetAdd(const K& k, const T& x)       { return B::At(FindAdd(k, x)); }

	T       *Detach(i32 i)                        { T *x = B::value.Detach(i); B::Remove(i); return x; }
	T       *PopDetach()                          { return Detach(B::GetCount() - 1); }

	const InArray<T>& GetVals() const           { return B::value.data; }
	...
	SortedArrayMap(const SortedArrayMap& s, i32);
