...
class CoRoutineT {
    ...
    template<typename U>
    struct ReturnVal : PromiseType<CoRoutineT, ReturnVal<U>>
    {
    ...
    template<typename U>
    struct VoidVal : PromiseType<CoRoutineT, VoidVal<U>>
    {
        void return_void() {}
    };
    
    template<typename U>
    struct CurrentVal : PromiseType<CoRoutineT, CurrentVal<U>>
    {
        template<std::convertible_to<U> L>
...
public:
    using promise_type =
        typename std::conditional<
            std::is_void_v<T>,
                VoidVal<T>, typename std::conditional<
                    R == CoRoutineType::Routine,
                        ReturnVal<T>, CurrentVal<T>>::type>::type;
    ...
    public: // C++20 iterator tags...
        ...
        IteratorT& operator++(i32)      { cogen.Rethrow(); cogen.co.resume(); return *this; }
