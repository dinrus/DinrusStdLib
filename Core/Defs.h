#define STD_NEWDELETE 1
//Используем собственную кучу, но операторы new - от стандартной библиотеки,
//поскольку происходит накладка и появление дубликатов (см. heap.cpp).
//Остальные параметры, вроде бы, не конфликтуют...
 #define MEMORY_SHRINK 0
 //Сжатие памяти также работает сомнительно.
 
 //Флагом USEMALLOC при необходимости можно вообще отключить использование собственного
 //управления памятью.
...
#define __BREAK__        (*(volatile i32 *)0 = 0) // kill(getpid(), SIGTRAP)
...
#define VERIFY(x)        ((x) ? (void)0 : ::drx::AssertFailed(__FILE__, __LINE__, #x))

#ifdef _DEBUG

#define ASSERT_(x, msg)  ((x) ? (void)0 : ::drx	::AssertFailed(__FILE__, __LINE__, msg))
...
#define __countof(a)          i32(sizeof(a) / sizeof(a[0]) )

#define ASTXT_(x)  #x
#define ASTXT(x)   ASTXT_(x)
...
#else
inline void InitBlockBegin__(tukk , i32) {}
inline void InitBlockEnd__(tukk , i32) {}
#endif

struct Callinit {
	Callinit(void (*fn)(), tukk cpp, i32 line) { InitBlockBegin__(cpp, line); fn(); InitBlockEnd__(cpp, line); }
	Callinit(void (*fn)())                            { fn(); }
};
...
#define INITBLOCK \
static void COMBINE(MK__s, _fn)(); static DRX::Callinit MK__s(COMBINE(MK__s, _fn), __FILE__, __LINE__); \
static void COMBINE(MK__s, _fn)()

#define INITBLOCK_(x) \
static void COMBINE(x, _fn)(); static DRX::Callinit x(COMBINE(x, _fn), __FILE__, __LINE__); \
static void COMBINE(x, _fn)()


#define EXITBLOCK \
static void COMBINE(MK__s, _fn)(); static DRX::Callexit MK__s(COMBINE(MK__s, _fn)); \
static void COMBINE(MK__s, _fn)()

#define EXITBLOCK_(x) \
static void COMBINE(x, _fn)(); static DRX::Callexit x(COMBINE(x, _fn)); \
static void COMBINE(x, _fn)()
...
#ifdef DEPRECATED
#define INITCODE(x) \
static void COMBINE(MK__s, _fn)() { x } static DRX::Callinit MK__s(COMBINE(MK__s, _fn), __FILE__, __LINE__);

#define EXITCODE(x) \
static void COMBINE(MK__s, _fn)() { x } static DRX::Callexit MK__s(COMBINE(MK__s, _fn));
#endif
...
struct m128 {
	...
	static m128 Zero()             { m128 a; a.i[0] = a.i[1] = 0; return a; }
};
...
#ifdef COMPILER_MSC
	#define I64(c) ((z64)COMBINE(c, i64))
#else
	#define I64(c) ((z64)COMBINE(c, LL))
#endif
...
#define MAKEWORD(l, h)   ((word)  (((u8) (l)) | ((word) ((u8) (h))) << 8))
#define MAKELONG(l, h)   ((dword) (((word) (l)) | ((dword) ((word) (h))) << 16))

#endif

#define MAKEQWORD(l, h)  ((qword) (((dword) (l)) | ((qword) ((dword) (h))) << 32))
#define HIDWORD(a)       (dword)(((zu64)a) >> 32)
#define LODWORD(a)       dword(a)

#define OFFSETOF(clss, mbr) ((i32)(uintptr_t)&(((clss *)1)->mbr) - 1)
...
#define BINARY(i, f) \
extern "C" u8 *i; \
extern "C" i32 COMBINE(i, _length);

#define BINARY_ARRAY(i, x, f) \
extern "C" u8 *i[]; \
extern "C" i32 COMBINE(i, _length)[]; \
extern "C" i32 COMBINE(i, _count);

#define BINARY_MASK(i, m) \
extern "C" u8 *i[]; \
extern "C" i32 COMBINE(i, _length)[]; \
extern "C" i32 COMBINE(i, _count); \
extern "C" tuk COMBINE(i, _files)[];
...
i32k  INT_NULL           =   INT_MIN;
const i64  INT64_NULL         =   INT64_MIN;
const z64  INT64_NULL2        =   INT64_MIN;

constexpr double DOUBLE_NULL    =    -std::numeric_limits<double>::infinity();

class Nuller {
public:
	operator i32() const                { return INT_NULL; }
	operator i64() const                { return INT64_NULL; }
	operator z64() const                { return INT64_NULL2; }
...
template<> inline bool  IsNull(i32k& i)     { return i == INT_NULL; }
template<> inline bool  IsNull(const z64& i)     { return i == INT64_NULL2; }
template<> inline bool  IsNull(const i64& i)     { return i == INT64_NULL; }
...
void GetSystemMemoryStatus(zu64& total, zu64& available);
...
#else
inline tukk FromSysChrSet(tukk s) { return s; }
inline tukk ToSysChrSet(tukk s) { return s; }
#endif
...
#else
inline void __LOGF__(tukk format, ...);
#endif
