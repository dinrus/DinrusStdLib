			   ...
			   __syscall_arg_t nr)
{
  ...
  i32 ch = atomic_load_relaxed (&pd->cancelhandling);
...
{
  i32 r = __internal_syscall_cancel (a1, a2, a3, a4, a5, a6,
...
__syscall_do_cancel (void)
{
  ...
  i32 oldval = atomic_load_relaxed (&self->cancelhandling);
  while (1)
    {
      i32 newval = oldval | CANCELSTATE_BITMASK;
      if (oldval == newval)
	break;
