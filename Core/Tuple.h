...
struct TupleN<1, A>
{
	...
	i32   Compare(const TupleN& x) const      { return SgnCompare(a, x.a); }

	void  ToHash(CombineHash& h) const        { h << a; }
	void  ToTxt(Txt& r) const           { r << AsTxt(a); }
	
	void  Serialize(Stream& s)                { s % a; }
	
	i32   GetCount() const                    { return 1; }

	Val Get(i32 i) const                    { ASSERT(i == 0); return a; }
	void  Set(i32 i, const Val& v)          { ASSERT(i == 0); a = v; }
...
#define TUPLE_N_METHODS(M, I) \
	...
	i32  Compare(const TupleN& x) const       { i32 q = Base::Compare(x); return q ? q : SgnCompare(M, x.M); } \
 \
	void ToHash(CombineHash& h) const         { Base::ToHash(h); h << M; } \
	void ToTxt(Txt& r) const            { Base::ToTxt(r); r << ", " << M; } \
	 \
	void Serialize(Stream& s)                 { Base::Serialize(s); s % M; } \
	 \
	i32   GetCount() const                    { return I + 1; } \
 \
	Val Get(i32 i) const                    { if(i == I) return M; return Base::Get(i); } \
	void  Set(i32 i, const Val& v)          { if(i == I) M = v; else Base::Set(i, v); } \
 \
	TupleN() {} \
...
public:
	...
	i32  GetCount() const                      { return Base::GetCount(); }

	bool operator==(const Tuple& x) const     { return Base::operator==(x); }
	bool operator!=(const Tuple& x) const     { return !operator==(x); }
	
	i32  Compare(const Tuple& x) const        { return Base::Compare(x); }
	bool operator<=(const Tuple& x) const     { return Compare(x) <= 0; }
	bool operator>=(const Tuple& x) const     { return Compare(x) >= 0; }
	bool operator<(const Tuple& x) const      { return Compare(x) < 0; }
	bool operator>(const Tuple& x) const      { return Compare(x) > 0; }

	hash_t GetHashVal() const               { CombineHash h; Base::ToHash(h); return h; }
	
	void Serialize(Stream& s)                 { Base::Serialize(s); }

	Txt ToTxt() const                   { Txt h = "("; Base::ToTxt(h); h << ")"; return h; }
	
	Val Get(i32 i) const                    { return Base::Get(i); }
	void  Set(i32 i, const Val& v)          { return Base::Set(i, v); }
	ValArray GetArray() const               { ValArray va; for(i32 i = 0; i < GetCount(); i++) va.Add(Get(i)); return va; }
	void  SetArray(const ValArray& va)      { for(i32 i = 0; i < va.GetCount(); i++) Set(i, va[i]); }
...
inline T *FindTuple(T *x, i32 n, const U& key) {
	while(n--) {
		if(x->a == key)
			return x;
		x++;
