...
void memset8__(uk t, i16x8 data, size_t len);

inline
void memset8(uk p, u8 data, size_t count)
{
	u8 *t = (u8 *)p;
	if(count < 2) {
		if(count)
...
inline
void memset16(uk p, word data, size_t count)
{
	word *t = (word *)p;
	if(count < 2) {
		if(count)
...
inline
void memset32(uk p, dword data, size_t count)
{
	dword *t = (dword *)p;
	if(count < 4) {
		if(count & 2) {
...
inline
void memset64(uk p, qword data, size_t count)
{
	qword *t = (qword *)p;
	if(count < 2) {
		if(count)
...
inline
void memset128(uk t, m128 data, size_t count)
{
	if(count)
		memset8__(t, i16x8().Load(&data), 16 * count);
}

void memcpy8__(uk p, ukk q, size_t count);

inline
void memcpy8(uk p, ukk q, size_t count)
{
	u8 *t = (u8 *)p;
	u8 *s = (u8 *)q;
	if(count <= 4) {
	...
	if(count <= 16) {
		...
		*(zu64 *)t = *(zu64 *)s;
		*(zu64 *)(t + count - 8) = *(zu64 *)(s + count - 8);
		return;
	}
	if(count <= 32) { // improves Txt::LSet
...
inline
void memcpy16(uk p, ukk q, size_t count)
{
	word *t = (word *)p;
	word *s = (word *)q;
	if(count <= 4) {
...
inline
void memcpy32(uk p, ukk q, size_t count)
{
	dword *t = (dword *)p;
	dword *s = (dword *)q;
...
inline
void memcpy64(uk p, ukk q, size_t count)
{
	qword *t = (qword *)p;
	qword *s = (qword *)q;
...
inline
void memcpy128(uk p, ukk q, size_t count)
{
...
template <class T>
void memcpy_t(uk t, const T *s, size_t count)
{
#ifdef CPU_X86
	if((sizeof(T) & 15) == 0)
...
force_inline
bool memeq8__(ukk p, ukk q, size_t count)
{
	ASSERT(count >= 16);
	u8k *t = (u8 *)p;
	u8k *s = (u8 *)q;
...
force_inline
bool inline_memeq8_aligned(ukk p, ukk q, size_t count)
{
	u8k *t = (u8k *)p;
	u8k *s = (u8k *)q;
	if(count >= 16) // 15..31 is the most important range for Txt, make it fastest
...
force_inline
bool inline_memeq16_aligned(ukk p, ukk q, size_t count)
{
...
force_inline
bool inline_memeq32_aligned(ukk p, ukk q, size_t count)
{
...
force_inline
bool inline_memeq64_aligned(ukk p, ukk q, size_t count)
{
...
force_inline
bool inline_memeq128_aligned(ukk t, ukk s, size_t count)
{
	return count == 0 || memeq8__(t, s, 16 * count);
}

bool memeq8(ukk p, ukk q, size_t count);
bool memeq16(ukk p, ukk q, size_t count);
bool memeq32(ukk p, ukk q, size_t count);
bool memeq64(ukk p, ukk q, size_t count);
bool memeq128(ukk p, ukk q, size_t count);

#else

template <class T>
void memset__(uk p, T data, size_t count)
{
	T *t = (T *)p;
	while(count >= 8) {
...
inline
void memset128(uk p, m128 data, size_t count)
{
...
#if defined(CPU_LE)
force_inline
i32 inline_memcmp_aligned(tukk a, tukk b, size_t count)
{
#ifdef CPU_64
	while(count >= 8) {
		zu64 a64 = *(zu64 *)a;
