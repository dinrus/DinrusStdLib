...
{ // optimised version for fundamental types, for Vec
	i32 n = cont.GetCount();
	s / n;
	if(n < 0) {
		s.LoadError();
	...
	if(s.IsLoading()) {
		cont.Clear();
		cont.Reserve(min(n, i32(256*1024 / sizeof(T)))); // protect against invalid streams...

		while(n > 0) {
			i32 count = min(n, 65536);
			i32 q = cont.GetCount();
...
template <>
inline void StreamContainer(Stream& s, Vec<u8>& cont)
{
	StreamContainerRaw(s, cont);
}

template <>
inline void StreamContainer(Stream& s, Vec<word>& cont)
{
	StreamContainerRaw(s, cont);
}

template <>
inline void StreamContainer(Stream& s, Vec<i16>& cont)
{
	StreamContainerRaw(s, cont);
}

template <>
inline void StreamContainer(Stream& s, Vec<dword>& cont)
{
	StreamContainerRaw(s, cont);
}

template <>
inline void StreamContainer(Stream& s, Vec<i32>& cont)
{
	StreamContainerRaw(s, cont);
}

template <>
inline void StreamContainer(Stream& s, Vec<zu64>& cont)
{
	StreamContainerRaw(s, cont);
}

template <>
inline void StreamContainer(Stream& s, Vec<float>& cont)
{
	StreamContainerRaw(s, cont);
}

template <>
inline void StreamContainer(Stream& s, Vec<double>& cont)
{
	StreamContainerRaw(s, cont);
}

template <class T>
T * Vec<T>::RawAlloc(i32& n)
{
	size_t sz0 = n * sizeof(T);
	size_t sz = sz0;
	uk q = MemoryAllocSz(sz);
	n += (i32)((sz - sz0) / sizeof(T));
	return (T *)q;
}

template <class T>
bool Vec<T>::ReAlloc(i32 newalloc)
{
	...
	alloc = newalloc == INT_MAX ? INT_MAX // maximum alloc reached
	        : (i32)((sz - sz0) / sizeof(T) + newalloc); // adjust alloc to real memory size
	if(vector && newvector)
...
template <class T>
void Vec<T>::ReAllocF(i32 newalloc)
{
	uk prev = vector;
...
{
	if(alloc == INT_MAX)
		Panic("В контейнере слишком много элементов!");
#ifdef _DEBUG
	return ReAlloc(max(alloc + 1, alloc >= INT_MAX / 2 ? INT_MAX : 2 * alloc));
#else
	return ReAlloc(max(alloc + 1, alloc >= i32((z64)2 * INT_MAX / 3) ? INT_MAX : alloc + (alloc >> 1)));
#endif
}
...
#ifdef DEPRECATED
template <class T>
i32  Vec<T>::GetIndex(const T& item) const
{
...
template <class T>
void Vec<T>::__DeepCopy(const Vec& src) {
...
template <class T>
void Vec<T>::SetCount(i32 n, const T& init) {
	ASSERT(n >= 0);
	if(n == items) return;
	if(n < items)
		Destroy(vector + n, vector + items);
...
template <class T>
void Vec<T>::SetCountR(i32 n, const T& init) {
	ASSERT(n >= 0);
	if(n == items) return;
	if(n < items)
		Destroy(vector + n, vector + items);
...
template <class T>
void Vec<T>::Remove(i32 q, i32 count) {
	ASSERT(q >= 0 && q <= items - count && count >= 0);
	if(count == 0) return;
	Destroy(vector + q, vector + q + count);
	memmove((uk )(vector + q), (uk )(vector + q + count), (items - q - count) * sizeof(T));
	items -= count;
}

template <class T>
void Vec<T>::Remove(i32k *sorted_list, i32 n)
{
	if(!n) return;
	i32 pos = *sorted_list;
	i32 npos = pos;
...
template <class T>
void Vec<T>::Remove(const Vec<i32>& v)
{
	Remove(v, v.GetCount());
}
...
void Vec<T>::RemoveIf(Condition c)
{
	i32 i = 0;
...
template <class T>
void Vec<T>::RawInsert(i32 q, i32 count)
{
	ASSERT(count >= 0);
	ASSERT(q >= 0 && q <= items);
	if(!count) return;
	if(items + count > alloc) {
		T *newvector = RawAlloc(alloc = max(alloc + count, i32(alloc + ((unsigned)alloc >> 1))));
		if(vector) {
		...
		memmove((uk )(vector + q + count), (uk )(vector + q), (items - q) * sizeof(T));
	items += count;
}

template <class T>
void Vec<T>::InsertN(i32 q, i32 count) {
	ASSERT(count >= 0);
	ASSERT(q >= 0 && q <= items);
	RawInsert(q, count);
...
template <class T>
void Vec<T>::Insert(i32 q, const T& x, i32 count) {
	if(!count) return;
...
template <class T>
void Vec<T>::Insert(i32 q, const Vec& x, i32 offset, i32 count) {
...
template <class T>
void Vec<T>::Insert(i32 i, std::initializer_list<T> init)
{
	RawInsert(i, (i32)init.size());
	T *t = vector + i;
...
void Vec<T>::InsertRange(i32 i, const Range& r)
{
...
template <class T>
void Vec<T>::Insert(i32 q, const Vec& x) {
	if(!x.GetCount()) return;
	Insert(q, x, 0, x.GetCount());
}

template <class T>
void Vec<T>::Insert(i32 i, Vec<T>&& v) {
...
template <class T>
void Vec<T>::InsertSplit(i32 i, Vec<T>& v, i32 from)
{
	ASSERT(!vector || v.vector != vector && from <= v.GetCount());
	i32 n = v.GetCount() - from;
	if(n) {
		RawInsert(i, n);
...
template <class T>
void Vec<T>::Set(i32 i, const T& x, i32 count) {
	ASSERT(i >= 0 && count >= 0);
	if(count == 0) return;
...
template <class T>
template <class Range>
void Vec<T>::SetRange(i32 i, const Range& r)
{
...
#ifdef DRX
template <class T>
void Vec<T>::Xmlize(XmlIO& xio, tukk itemtag)
{
	XmlizeContainer(xio, itemtag, *this);
}

template <class T>
void Vec<T>::Jsonize(JsonIO& jio)
{
	JsonizeArray<Vec<T>>(jio, *this);
}

template <class C>
Txt AsTxtArray(const C& v)
{
	Txt r;
	r << '[';
...
template <class T>
void Array<T>::SetCount(i32 n) {
	ASSERT(n >= 0);
	i32  lc = vector.GetCount();
	Del(vector.begin() + n, vector.end());
	vector.SetCount(n);
...
template <class T>
void Array<T>::SetCount(i32 n, const T& init) {
	ASSERT(n >= 0);
	i32  lc = vector.GetCount();
	Del(vector.begin() + n, vector.end());
	vector.SetCount(n);
...
template <class T>
void Array<T>::SetCountR(i32 n, const T& init) {
	ASSERT(n >= 0);
	i32  lc = vector.GetCount();
	Del(vector.begin() + n, vector.end());
	vector.SetCountR(n);
...
#ifdef DEPRECATED
template <class T>
i32  Array<T>::GetIndex(const T& item) const {
	for(PointerType const *ptr = vector.begin(); ptr < vector.end(); ptr++)
		if(*ptr == (PointerType)&item) return (i32)(ptr - vector.begin());
	return -1;
}
#endif

template <class T>
void Array<T>::Move(i32 i1, i32 i2) {
...
template <class T>
void Array<T>::Remove(i32 i, i32 count) {
...
template <class T>
void Array<T>::Remove(i32k *sorted_list, i32 n)
{
	i32k *q = sorted_list;
	i32k *e = sorted_list + n;
	while(q < e) {
...
template <class T>
void Array<T>::Remove(const Vec<i32>& sorted_list)
{
...
template <class T>
void Array<T>::Set(i32 i, const T& x, i32 count) {
	ASSERT(i >= 0 && count >= 0);
	if(i + count >= GetCount())
		SetCountR(i + count);
...
template <class T>
void Array<T>::InsertN(i32 i, i32 count) {
	ASSERT(i >= 0 && count >= 0);
...
template <class T>
void Array<T>::Insert(i32 i, const T& x, i32 count) {
	vector.InsertN(i, count);
...
template <class T>
template <class Range>
void Array<T>::InsertRange(i32 i, const Range& r)
{
...
template <class T>
void Array<T>::Insert(i32 i, const Array& x) {
	Insert(i, x, 0, x.GetCount());
}

template <class T>
void Array<T>::Insert(i32 i, const Array& x, i32 offset, i32 count) {
	vector.InsertN(i, count);
...
template <class T>
void Array<T>::Insert(i32 i, std::initializer_list<T> init)
{
	vector.InsertN(i, (i32)init.size());
	for(const auto& q : init)
		vector[i++] = new T(q);
}
...
#ifdef DRX
template <class T>
void Array<T>::Xmlize(XmlIO& xio, tukk itemtag)
{
	XmlizeContainer(xio, itemtag, *this);
}
...
template <class T>
void BiVec<T>::ReAlloc(i32 newalloc) {
...
template <class T>
void BiVec<T>::DeepCopy0(const BiVec& src) {
	alloc = items = src.items;
	vector = alloc ? (T *) new u8[alloc * sizeof(T)] : NULL;
	if(items) {
		i32 end = src.start + src.items;
		if(end <= src.alloc)
...
template <class T>
void BiVec<T>::Serialize(Stream& s) {
	i32 n = items;
	s / n;
	if(n < 0)
		s.LoadError();
...
template <class T>
Txt BiVec<T>::ToTxt() const
{
	return AsTxtArray(*this);
}

template <class T>
BiVec<T>::BiVec(std::initializer_list<T> init)
{
	start = items = alloc = 0; vector = NULL;

	start = 0;
	alloc = items = (i32)init.size();
	if(!alloc) {
		vector = NULL;
		return;
	}
...
void BiArray<T>::Serialize(Stream& s) {
	...
	if(s.IsLoading()) {
		...
		for(i32 i = 0; i < bv.GetCount(); i++)
			s % operator[](i);
}

template <class T>
Txt BiArray<T>::ToTxt() const
{
	return AsTxtArray(*this);
}
...
inline
void   Bits::Set(i32 i, dword bits, i32 count)
{
...
inline
dword Bits::Get(i32 i, i32 count)
{
	ASSERT(i >= 0 && count >= 0 && count <= 32);
	if(!count) // note: this also avoids problem with (dword)-1 >> 32 being undefined
		return 0;
...
inline
void Bits::Set64(i32 i, zu64 bits, i32 count)
{
	if(count > 32) {
...
inline
zu64 Bits::Get64(i32 i, i32 count)
{
	if(count > 32) {
		dword l = Get(i, 32);
...
inline
void Bits::SetN(i32 i, bool b, i32 count)
{
	ASSERT(i >= 0);
	if(!count) // note: this also avoids problem with (dword)-1 >> 32 being undefined
		return;
	...
	i32 n = min(32 - i, count); // number of bits to dword boundary (or count if smaller)
