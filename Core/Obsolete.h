...
i32 IterCompare(I a, I a_end, I b, I b_end, const C& compare)
{
	for(;;) {
		if(a >= a_end)
			return b < b_end ? -1 : 0;
...
i32 BinFindCompIndex(I begin, I end, const K& key, const X& comp)
{
...
inline i32 BinFindCompIndex(const C& container, const K& key, const X& comp)
{
...
template <class C>
i32 MinIndex(const C& c) { return FindMin(c); }

template <class C>
i32 MaxIndex(const C& c) { return FindMax(c); }

template <class C>
typename C::ValType Sum0(const C& c) { return Sum(c); }
...
i32 FindFieldIndex(const C& container, F field, const O& object, const E& equal)
{
	i32 i = 0;
...
i32 FindFieldIndex(const C& container, F field, const O& object)
...
template <class C>
i32 FindMin(const C& c, i32 pos, i32 count)
{
...
inline i32 BinFindIndex(const C& container, const K& key, const L& less)
{
	return BinFindIndex(container.Begin(), container.End(), key, less);
}

template <class C, class K>
inline i32 BinFindIndex(const C& container, const K& key)
{
	typedef typename C::ValType VT;
...
i32 FindLowerBound(const C& v, i32 pos, i32 count, const T& val, const L& less)
{
...
template <class C, class T, class L>
i32 FindUpperBound(const C& v, i32 pos, i32 count, const T& val, const L& less)
{
...
template <class C, class T, class L>
i32 FindBinary(const C& v, const T& val, i32 pos, i32 count, const L& less)
{
...
i32 FindBinary(const C& v, const T& val, i32 pos, i32 count)
{
...
template <class T>
struct StdCmp {
	i32 operator()(const T& a, const T& b) const {
...
{
	StableSortCmp(c.begin(), c.end(), StdCmp<typename T::ValType>());
}


template <class I, class T, class Cmp>
inline void __StableSortOrderCmp(i32 *ibegin, i32 *iend, I data, const Cmp& cmp, const T *)
{
...
template <class I, class Cmp>
inline Vec<i32> GetStableSortOrderCmp(I begin, I end, const Cmp& cmp)
{
	Vec<i32> index;
	index.SetCount((i32)(end - begin));
...
inline Vec<i32> GetStableSortOrderCmp(const C& container, const Cmp& cmp)
{
	return GetStableSortOrderCmp(container.begin(), container.end(), cmp);
}

template <class C>
inline Vec<i32> GetStableSortOrderCmp(const C& container)
{
	typedef typename C::ValType V;
