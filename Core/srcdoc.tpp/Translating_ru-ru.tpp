...
[s0; &]
[s0; // csCZ(`"datum[@(0.0.255) `\v]Květen`")  Wrong: Do not translate 
`"date`" prefix!&]
[s0; // csCZ(`"date[@(0.0.255) `\v]Květen`")  Wrong: Do not copy `"date`" 
prefix!&]
[s0; &]
[s0; csCZ(`"Květen`") // Correct]}}&]
...
`+`+ Library support »]&]
[s0; [@(0.0.255) -|-|][_^http`:`/`/www`.ultimatepp`.org`/forums`/index`.php`?t`=thread`&frm`_id`=74`&^@(0.0.255) U
`+`+ Libraries and DinrusIDE: i18n, Unicode and Internationalization]&]
