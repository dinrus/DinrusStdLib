...
struct DLLTYPE{
	...
	DRX::Txt          GetLibName() const        { return libname; }
	void              SetLibName(tukk n) { if(handle) Free(); libname = n; checked = false; }

	bool              Load();
	bool              Load(tukk n)       { SetLibName(n); return Load(); }
	void              Force();
	void              Free();
