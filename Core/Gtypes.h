...
struct Size_ : Moveable< Size_<T> > {
	...
	bool          IsNullInstance() const       { return DRX::IsNull(cx); }
	...
	Size_&        operator/=(Size_ p)          { cx  /= p.cx; cy  /= p.cy; return *this; }
	Size_&        operator/=(T t)              { cx  /= t;    cy  /= t;    return *this; }
	Size_&        operator<<=(i32 sh)          { cx <<= sh;   cy <<= sh;   return *this; }
	Size_&        operator>>=(i32 sh)          { cx >>= sh;   cy >>= sh;   return *this; }
	...
	friend Size_  operator/(Size_ a, Size_ b)  { return a /= b; }
	friend Size_  operator/(Size_ a, T b)      { return a /= b; }
	friend Size_  operator<<(Size_ a, i32 sh)  { return a <<= sh; }
	friend Size_  operator>>(Size_ a, i32 sh)  { return a >>= sh; }
	...
	hash_t        GetHashVal() const         { return CombineHash(cx, cy); }
	...
	Size_(const Size_<i32>& sz)  : cx((T)sz.cx), cy((T)sz.cy) {}
	Size_(const Size_<i16>& sz)  : cx((T)sz.cx), cy((T)sz.cy) {}
	Size_(const Size_<double>& sz)  : cx((T)sz.cx), cy((T)sz.cy) {}
	Size_(const Size_<z64>& sz) : cx((T)sz.cx), cy((T)sz.cy) {}

	Size_(const Point_<T>& pt) : cx(pt.x), cy(pt.y) {}

	Size_(const Nuller&)                       { cx = cy = Null; }

	operator Val() const                     { return FitsSvoVal<Size_>() ? SvoToVal(*this) : RichToVal(*this); }
	Size_(const Val& src)                    { *this = src.Get<Size_>(); }
	...
	i32  Compare(const Size_&) const           { NEVER(); return 0; }
	...
	explicit Size_(LONG lParam) { cx = (T)(i16)LOWORD(lParam); cy = (T)(i16)HIWORD(lParam); }
#endif
};

template <class T>
Txt Size_<T>::ToTxt() const {
	Txt ss;
	return ss << '(' << cx << ", " << cy << ')';
}
...
struct Point_ : Moveable< Point_<T> > {
	...
	bool          IsNullInstance() const            { return DRX::IsNull(x); }
	...
	Point_&       operator/=(Point_ p)              { x  /= p.x;  y  /= p.y; return *this; }
	Point_&       operator/=(T t)                   { x  /= t;    y  /= t;    return *this; }
	Point_&       operator<<=(i32 sh)               { x <<= sh;   y <<= sh;   return *this; }
	Point_&       operator>>=(i32 sh)               { x >>= sh;   y >>= sh;   return *this; }
	...
	friend Point_ operator/(Point_ a, Sz b)         { return a /= b; }
	friend Point_ operator/(Point_ a, T b)          { return a /= b; }
	friend Point_ operator<<(Point_ a, i32 sh)      { return a <<= sh; }
	friend Point_ operator>>(Point_ a, i32 sh)      { return a >>= sh; }
	...
	hash_t        GetHashVal() const              { return CombineHash(x, y); }
	...
	Point_(const Point_<i32>& pt) : x((T)pt.x), y((T)pt.y) {}
	Point_(const Point_<i16>& pt) : x((T)pt.x), y((T)pt.y) {}
	Point_(const Point_<double>& pt) : x((T)pt.x), y((T)pt.y) {}
	Point_(const Point_<z64>& pt) : x((T)pt.x), y((T)pt.y) {}

	Point_(const Size_<T>& sz)  : x(sz.cx), y(sz.cy) {}

	Point_(const Nuller&)                           { x = y = Null; }

	operator Val() const                          { return FitsSvoVal<Point_>() ? SvoToVal(*this) : RichToVal(*this); }
	Point_(const Val& src)                        { *this = src.Get<Point_>(); }
	...
	i32  Compare(const Point_&) const               { NEVER(); return 0; }
	i32  PolyCompare(const Val&) const            { NEVER(); return 0; }
	...
	explicit Point_(LONG lParam)                    { x = (T)(i16)LOWORD(lParam); y = (T)(i16)HIWORD(lParam); }
#endif
};

template <class T>
Txt Point_<T>::ToTxt() const {
	Txt ss;
	return ss << '[' << x << ", " << y << ']';
}
...
struct Rect_ : Moveable< Rect_<T> > {
	...
	hash_t GetHashVal() const                             { return CombineHash(left, top, right, bottom); }
	...
	Rect_(const Rect_<i32>& r) { Set((T)r.left, (T)r.top, (T)r.right, (T)r.bottom); }
	Rect_(const Rect_<i16>& r) { Set((T)r.left, (T)r.top, (T)r.right, (T)r.bottom); }
	Rect_(const Rect_<double>& r) { Set((T)r.left, (T)r.top, (T)r.right, (T)r.bottom); }
	Rect_(const Rect_<z64>& r) { Set((T)r.left, (T)r.top, (T)r.right, (T)r.bottom); }

	Rect_(const Nuller&)             { SetNull(); }

	operator Val() const           { return RichToVal(*this); }
	Rect_(const Val& src)          { *this = src.Get<Rect_>(); }
	...
	i32      Compare(const Rect_&) const           { NEVER(); return 0; }
	i32      PolyCompare(const Val&) const       { NEVER(); return 0; }
...
template <class T>
Txt Rect_<T>::ToTxt() const
{
	Txt str;
	return str << AsTxt(TopLeft()) << " - " << AsTxt(BottomRight())
	           << " : " << AsTxt(Size());
}

typedef Size_<i32>     Size;
typedef Point_<i32>    Point;
typedef Rect_<i32>     Rect;

typedef Size_<i16>   Size16;
typedef Point_<i16>  Point16;
typedef Rect_<i16>   Rect16;

typedef Size_<double>  Sizef;
typedef Point_<double> Pointf;
typedef Rect_<double>  Rectf;

typedef Size_<z64>   Size64;
typedef Point_<z64>  Point64;
typedef Rect_<z64>   Rect64;
...
template<> inline dword ValTypeNo(const Size*)   { return SIZE_V; }
template<> inline dword ValTypeNo(const Size16*) { return SIZE16_V; }
template<> inline dword ValTypeNo(const Size64*) { return SIZE64_V; }
template<> inline dword ValTypeNo(const Sizef*)  { return SIZEF_V; }
...
template<> inline dword ValTypeNo(const Point*)   { return POINT_V; }
template<> inline dword ValTypeNo(const Point16*) { return POINT16_V; }
template<> inline dword ValTypeNo(const Point64*) { return POINT64_V; }
template<> inline dword ValTypeNo(const Pointf*)  { return POINTF_V; }
...
template<> inline dword ValTypeNo(const Rect*)   { return RECT_V; }
template<> inline dword ValTypeNo(const Rect16*) { return RECT16_V; }
template<> inline dword ValTypeNo(const Rect64*) { return RECT64_V; }
template<> inline dword ValTypeNo(const Rectf*)  { return RECTF_V; }

Rect    RectC(i32 x, i32 y, i32 cx, i32 cy);
Rect16  Rect16C(i16 x, i16 y, i16 cx, i16 cy);
...
{
	sz.cx = i32(sz.cx * a.cx);
	sz.cy = i32(sz.cy * a.cy);
	return sz;
}
...
{
	sz.cx = i16(sz.cx * a.cx);
	sz.cy = i16(sz.cy * a.cy);
	return sz;
}
...
inline Rect RectC(i32 x, i32 y, i32 cx, i32 cy) {
	return Rect(x, y, x + cx, y + cy);
}

inline Rect16  Rect16C(i16 x, i16 y, i16 cx, i16 cy) {
...
Size iscale(Size a,  i32 b, i32 c);
Size iscalefloor(Size a,  i32 b, i32 c);
Size iscaleceil(Size a,  i32 b, i32 c);
Size idivfloor(Size a,  i32 b);
Size idivceil(Size a,  i32 b);
...
Size        GetRatioSize(Size stdsize, i32 cx, i32 cy);
Size        GetFitSize(Size objsize, i32 cx, i32 cy);
