...
template <typename CHAR, typename BYTE, typename UINT, i32 base>
force_inline
const CHAR *ScanUint0(UINT& result, const CHAR *s, bool& overflow)
{
	...
	if(c1 < 0 || c1 >= base)
		return NULL;
	UINT n = 0;
	i32k base2 = base * base;
	i32k base3 = base * base2;
	i32k base4 = base * base3;
	for(;;) {
		UINT n0 = n;
...
template <typename CHAR, typename BYTE, typename UINT, typename INT, i32 base>
force_inline
const CHAR *ScanInt(INT& result, const CHAR *s, bool& overflow)
{
	SkipSpaces__<CHAR, BYTE>(s);
	i32 sgn = ScanSgn__<CHAR, BYTE>(s);
	UINT uresult;
...
{
	return Txt::Make(12, [&](tuk s) { return utoa32(w, s); });
}

inline
Txt FormatInt(i32 i)
{
	return Txt::Make(12, [&](tuk s) {
		if(IsNull(i))
			return 0;
		if(i < 0) {
			*s++ = '-';
