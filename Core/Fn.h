...
constexpr i32 findarg(const T& x, const K& k)
{
	return x == k ? 0 : -1;
}

template <class T, class K, typename... L>
constexpr i32 findarg(const T& sel, const K& k, const L& ...args)
{
	if(sel == k)
		return 0;
	i32 q = findarg(sel, args...);
	return q >= 0 ? q + 1 : -1;
}

template <class T>
constexpr tukk decode_chr_(const T& sel, tukk def)
{
	return def;
}

template <class T, class K, typename... L>
constexpr tukk decode_chr_(const T& sel, const K& k, tukk v, const L& ...args)
{
	return sel == k ? v : decode_chr_(sel, args...);
}

template <class T, class K, typename... L>
constexpr tukk decode(const T& sel, const K& k, tukk v, const L& ...args)
{
...
constexpr A get_i(i32 i, const A& p0, const T& ...args)
{
	A list[] = { p0, (A)args... };
	return list[clamp(i, 0, (i32)sizeof...(args))];
}

template <typename P, typename... T>
constexpr const P *get_i(i32 i, const P* p0, const T& ...args)
{
...
template <class C, typename... Args>
i32 scatter_n(i32 n, const C& c, Args& ...args)
{
	if(n < (i32)sizeof...(args))
		return 0;
...
template <class C, typename... Args>
i32 scatter(const C& c, Args& ...args)
{
