...
static tukk DbgFormat(tuk b, DbgBlkHeader *p)
{
	snprintf(b, 100, "--memory-breakpoint__ %u ", (u32)~(p->serial ^ (uintptr_t)p));
	return b;
}

static void DbgHeapPanic(tukk text, DbgBlkHeader *p)
{
	char h[256];
	char b[100];
	strcpy(h, text);
	strcat(h, DbgFormat(b, p));
	HeapPanic(h, p + 1, (i32)(uintptr_t)p->size);
}
...
void  DbgSet(DbgBlkHeader *p, size_t size)
{
	...
	if(s_allocbreakpoint && s_allocbreakpoint == serial_number)
		__BREAK__;
	dbg_live.Insert(p);
	Poke32le((u8 *)(p + 1) + p->size, p->serial);
}
...
#ifdef LOGAF
	char h[200];
	sprintf(h, "ALLOCATED %d at %p - %p", size, p + 1, (u8 *)(p + 1) + size);
	DLOG(h);
#endif
	return p + 1;
}
...
{
	if((dword)Peek32le((u8 *)(p + 1) + p->size) != p->serial)
...
bool MemoryTryRealloc_(uk ptr, size_t& newsize);
...
uk MemoryAlloc32()             { return MemoryAlloc(32); }
void  MemoryFree32(uk ptr)     { return MemoryFree(ptr); }
...
void MemoryDumpLeaks()
{
	...
	i32 n = 0;
	while(p != &dbg_live) {
		dword serial = (u32)~(p->serial ^ (uintptr_t)p);
		if(p->serial && (!sIgnoreNonMainLeaks || serial >= serial_main_begin && serial < serial_main_end)) {
			...
			DbgFormat(b, p);
			VppLog() << '\n' << b << ": ";
			HexDump(VppLog(), p + 1, (i32)(uintptr_t)p->size, 64);
			if(++n > 16) {
