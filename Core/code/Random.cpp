...
static force_inline uint64_t s_rotl(const uint64_t x, i32 k) {
	return (x << k) | (x >> (64 - k)); // GCC/CLANG/MSC happily optimize this
}

force_inline
static zu64 sNext(zu64 *s)
{
...
never_inline
static void sSeed(zu64 *s)
{
#ifdef TARGET_POSIX
	i32 fd = open("/dev/urandom", O_RDONLY);
	if(fd != -1) {
		IGNORE_RESULT(
			read(fd, s, 4 * sizeof(zu64))
		);
		close(fd);
	}
#else
...
zu64 Random64(zu64 n) {
	ASSERT(n);
	zu64 *s = sState();
	zu64 mask = ((zu64)1 << SignificantBits64(n)) - 1;
	zu64 r;
	do
		r = sNext(s) & mask;
	while(r >= n);
