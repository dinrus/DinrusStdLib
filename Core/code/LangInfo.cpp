...
extern i32  LanguageList[];
extern tukk LangInfoList[];
...
WTxt DefaultGetIndexLetter(wtukk text, i32)
{
	return IsLetter(*text) ? WTxt(text, 1) : WTxt(Null);
}

static i32 LangCompareDigits(wtukk &a, wtukk &b, wtukk e1, wtukk e2)
{
	wtukk p1 = a, p2 = b;
	while(p1 < e1 && *p1 == '0')
		p1++;
...
i32 DefaultLanguageCompare(wtukk a, i32 a_length, wtukk b, i32 b_length, i32)
{
	i32  little = 0, middle = 0;
	wtukk p1 = a, e1 = a + a_length, p2 = b, e2 = b + b_length;
	...
	while(p1 < e1 && p2 < e2)
	{
		...
		i32 level1 = (IsLetter(c1) ? 3 : IsDigit(c1) ? 2 : c1 == ' ' ? 0 : 1);
		i32 level2 = (IsLetter(c2) ? 3 : IsDigit(c2) ? 2 : c2 == ' ' ? 0 : 1);
		if(level1 != level2)
		...
		{ // digits
			wtukk dp1 = --p1, dp2 = --p2;
			i32 res = LangCompareDigits(dp1, dp2, e1, e2);
			if(res)
				return res;
			p1 = dp1;
			p2 = dp2;
...
WTxt CSCZGetIndexLetter(wtukk s, i32)
{
	wchar temp[3];
...
i32 CSCZLanguageCompare(wtukk a, i32 a_length, wtukk b, i32 b_length, i32)
{
	i32  little = 0, middle = 0;
	wtukk p1 = a, e1 = a + a_length, p2 = b, e2 = b + b_length;
...
static dword sGetLanguageDetails(i32 language, Txt *english_name, Txt *native_name)
{
	i32 q = 0;
	u8 cs = GetLNGCharset(language);
	language &= ~LNGC_(0, 0, 0, 0, ~0);
	for(i32k *ptr = LanguageList; *ptr; ptr++, q++)
		if(*ptr == language) {
			tukk f = LangInfoList[q];
			tukk a = strchr(f, '\t');
			tukk b = strchr(a + 1, '\t');
			if(english_name)
				*english_name = Txt(f, a);
			if(native_name)
				*native_name = ToCharset(cs, Txt(a + 1, b), CHARSET_UTF8);
...
void LangInfo::Set(i32 lang_)
{
	...
	Txt langtext = LNGAsText(language);
	Txt ltext = Format("%c%c_%c%c", ToLower((char)langtext[0]), ToLower((char)langtext[1]), ToUpper((char)langtext[3]), ToUpper((char)langtext[4]));
	Txt oldloc = setlocale(LC_ALL, NULL);
...
#endif
	for(i32 i = 0; i < 12; i++) {
		month[i] = month_names[i].ToTxt();
		smonth[i] = short_month_names[i].ToTxt();
	}
	for(i32 i = 0; i < 7; i++) {
		day[i] = day_names[i].ToTxt();
		sday[i] = short_day_names[i].ToTxt();
	}
}

Txt LangInfo::ToTxt() const
{
	Txt out;
...
static tukk NlsFindDigits(tukk src, Txt& dest)
{
	if(*src && !IsDigit(*src))
	{
		tukk start = src;
		while(*++src && !IsDigit(*src))
			;
		dest.Cat(start, (i32)(src - start));
	}
	return src;
}

static tukk NlsCopyDigits(tukk src, Txt& dest, Txt thousands)
{
...
static Txt NlsFormatRaw(tukk n, Txt thousands, Txt decimals)
{
	if(*n == 0)
		return Null;
	Txt result;
...
Txt LangInfo::FormatInt(i32 value) const
{
	if(IsNull(value))
		return Null;
	Txt dest;
	Txt is = DRX::FormatInt(value);
	tukk p = NlsCopyDigits(is, dest, thousand_separator);
	if(*p)
		dest.Cat(p);
	return dest;
}

Txt LangInfo::FormatDouble(double value, i32 digits, i32 FD_flags, i32 fill_exp) const
{
	if(IsNull(value))
		return Null;
	return NlsFormatRaw(DRX::FormatDouble(value, digits, FD_flags),
	                    FD_flags & FD_NOTHSEPS ? Txt() : thousand_separator,
	                    FD_flags & FD_COMMA ? "," : decimal_point);
}

Txt LangInfo::FormatDate(Date date) const
{
	return DRX::FormatDate(date, date_format, language);
}

Txt LangInfo::FormatTime(Time time) const
{
	return DRX::FormatTime(time, time_format, language);
}

ArrayMap<i32, LangInfo>& LangMap()
{
	static ArrayMap<i32, LangInfo> x;
	return x;
}

StaticMutex sLangInfoMutex;

static std::atomic<const LangInfo *> sCurrentLangInfo;

const LangInfo& GetLangInfo(i32 lang)
{
	Mutex::Lock __(sLangInfoMutex);
	if(!lang)
		lang = GetCurrentLanguage();
	lang = SetLNGCharset(lang, 0);
	ArrayMap<i32, LangInfo>& m = LangMap();
	i32 q = m.Find(lang);
	if(q >= 0)
		return m[q];
	LangInfo& f = m.Add(lang);
	f.Set(lang);
	return f;
}
...
void SyncLngInfo__()
{
	sCurrentLangInfo = &GetLangInfo(GetCurrentLanguage());
}
