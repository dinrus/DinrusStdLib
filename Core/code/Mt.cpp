...
{ // Стопор, синхронизующий инициализацию других примитивов.
	static Mutex m;
	return m;
}
...
bool Thread::Run(Function<void ()> _cb, bool noshutdown)
{
	...
	handle = (HANDLE)_beginthreadex(0, stack_size ? stack_size : 1024*1024, sThreadRoutine, p, STACK_SIZE_PARAM_IS_A_RESERVATION, ((u32 *)(&thread_id)));
#else
	handle = (HANDLE)_beginthreadex(0, stack_size, sThreadRoutine, p, 0, ((u32 *)(&thread_id)));
#endif
#endif
#ifdef TARGET_POSIX
	if(stack_size) {
...
i32 Thread::Wait()
{
	...
	else
	{
		if(WaitForSingleObject(handle, INFINITE) != WAIT_OBJECT_0)
			return Null;
		out = GetExitCodeThread(handle, &exit) ? i32(exit) : i32(Null);
	}
	Detach();
#endif
#ifdef TARGET_POSIX
	...
	if(pthread_join(handle, &thread_return))
		...
		out = (i32)(intptr_t)thread_return;
	handle = 0;
#endif
	return out;
}

bool Thread::Priority(i32 percent)
{
	ASSERT(IsOpen());
#ifdef TARGET_WIN32
	i32 prior;
	...
	if(pthread_getschedparam(handle, &policy, &param))
		return false;
	i32 percent_min = 0, percent_max = 200;
	if(percent <= 25) {
...
void Semaphore::Release(i32 n)
{
	ReleaseSemaphore(handle, n, NULL);
}

bool Semaphore::Wait(i32 timeout_ms)
{
...
bool ConditionVariable::Wait(Mutex& m, i32 timeout_ms)
{
	if(InitializeConditionVariable)
		...
		static thread_local u8 buffer[sizeof(WaitingThread)]; // only one Wait per thread is possible
...
bool StartAuxThread(auxthread_t (auxthread__ *fn)(uk ptr), uk ptr)
{
#ifdef TARGET_WIN32
	HANDLE handle;
