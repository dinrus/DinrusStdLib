...
bool LocalProcess::DoStart(tukk command, const Vec<Txt> *arg, bool spliterr, tukk envptr, tukk cd)
{
	...
	LLOG("\tfork, pid = " << (i32)pid << ", getpid = " << (i32)getpid()); // LOGs are iffy because of ^^^^
	if(pid < 0)
		return false;
...
#ifdef TARGET_POSIX
bool LocalProcess::DecodeExitCode(i32 status)
{
	if(WIFEXITED(status)) {
...
Txt LocalProcess::GetExitMessage() {
#ifdef TARGET_POSIX
...
bool LocalProcess::Read(Txt& res) {
	Txt dummy;
	return Read2(res, dummy);
}

bool LocalProcess::Read2(Txt& reso, Txt& rese)
{
...
i32 LocalProcess::Finish(Txt& out)
{
	out.Clear();
	while(IsRunning()) {
		Txt h = Get();
...
i32 Sys(tukk cmd, Txt& out, bool convertcharset)
{
	LocalProcess p;
...
Txt Sys(tukk cmd, bool convertcharset)
{
	Txt r;
	return Sys(cmd, r, convertcharset) ? Txt::GetVoid() : r;
}

i32 Sys(tukk cmd, const Vec<Txt>& arg, Txt& out, bool convertcharset)
{
	LocalProcess p;
	p.ConvertCharset(convertcharset);
	if(!p.Start(cmd, arg))
...
Txt Sys(tukk cmd, const Vec<Txt>& arg, bool convertcharset)
{
	Txt r;
	return Sys(cmd, arg, r, convertcharset) ? Txt::GetVoid() : r;
}
