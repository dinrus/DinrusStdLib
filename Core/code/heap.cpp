...
#define LLOG(x) //  DLOG(x) // LOG((uk )this << ' ' << x)

word Heap::HPAGE = 16 * 256; // 16MB default value
word Heap::sys_block_limit = 16 * 256; // 16MB default value
i32  Heap::max_free_hpages = 1; // default value
i32  Heap::max_free_lpages = 2; // default value
i32  Heap::max_free_spages = 256; // default value (1MB)
...
MemoryOptions::~MemoryOptions()
{
	...
	Heap::sys_block_limit = (word)clamp((i32)sys_block_limit / 4, 16, (i32)Heap::HPAGE);
...
{
	LLOG("MoveLargePage " << asTxt(ml) << " to " << asTxt(to_heap));
	ml->Unlink();
	ml->Link(to_heap->large);
	LBlkHeader *h = ml->GetFirst();
	for(;;) {
		LLOG("Large block " << asTxt(h) << " size " << AsTxt(h->GetSize() * 256) << (h->IsFree() ? " free" : ""));
		h->heap = to_heap;
		if(h->IsFree()) {
...
i32 Heap::CheckFree(FreeLink *l, i32 k, bool pg)
{
	char h[200];
	i32 n = 0;
	...
	while(l) {
		if(l->next) {
			...
			if(pg && lp != page) {
				sprintf(h, "Неверный блок списка очистки у 0x%p sz: %d (вне страницы) (-> 0x%p)", l, Ksz(k), l->next);
				Panic(h);
			}
			if((4096 - ((uintptr_t)(l->next) & (uintptr_t)4095)) % Ksz(k) != 0) {
				sprintf(h, "Неверный блок списка очистки у 0x%p sz: %d (неверный адрес)", l, Ksz(k));
				Panic(h);
			}
			if(lp->klass != k) {
				sprintf(h, "Неверный блок списка очистки у 0x%p sz: %d (следщ klass несовпадает)", l, Ksz(k));
				Panic(h);
			}
		}
...
#endif
	DLink *m = lempty->next; //lempty взят неясно откуда! Поэтому MEMORY_SHRINK = 0
	while(m != lempty) {
		DLink *q = m;
		m = m->next;
		q->Unlink();
...
#if defined(DRX_HEAP) && !defined(STD_NEWDELETE)
#include <new>
...
uk operator new(size_t size)                                    { uk ptr = DRX::MemoryAlloc(size); return ptr; }
void operator  delete(uk ptr) noexcept(true)                    { DRX::MemoryFree(ptr); }

uk operator new[](size_t size)                                  { uk ptr = DRX::MemoryAlloc(size); return ptr; }
void operator  delete[](uk ptr) noexcept(true)                  { DRX::MemoryFree(ptr); }

uk operator new(size_t size, const std::nothrow_t&) noexcept    { uk ptr = DRX::MemoryAlloc(size); return ptr; }
void operator  delete(uk ptr, const std::nothrow_t&) noexcept   { DRX::MemoryFree(ptr); }
