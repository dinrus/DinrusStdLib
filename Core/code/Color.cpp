...
Color HsvColorf(double h, double s, double v)
{
	...
	return Color(min(i32(r * 255), 255), min(i32(g * 255), 255), min(i32(b * 255), 255));
}
...
template <>
Txt AsTxt(const RGBA& c)
{
	return Format("RGBA(%d, %d, %d, %d)", (i32)c.r, (i32)c.g, (i32)c.b, (i32)c.a);
}
...
Txt AsTxt(const Color& c) {
	...
	if(c.GetRaw() & 0x80000000)
		return Format("Color(%d, 0)", i32(c.GetRaw() & ~0x80000000));
...
static wchar sCharFilterNoDigit(wchar c)
{
	return IsDigit(c) ? 0 : c;
}

static wchar sCharFilterHex(wchar c)
{
...
{
	Vec<Txt> h = Split(s, sCharFilterNoDigit);
	...
	Txt hex = Filter(s, sCharFilterHex);
	if(hex.GetCount() == 6 || hex.GetCount() == 8) {
		dword w = (dword)ScanInt64(~hex, NULL, 16);
		return Color(u8(w >> 16), u8(w >> 8), u8(w));
	}
	return Null;
}

Color Blend(Color c1, Color c2, i32 alpha)
{
	i32 a = (alpha >> 7) + alpha;
...
//	static double c[3] = { 0.21, 0.72, 0.07 }; // physiologically correct values
	...
	i32k middle = 155; // this value represents gamma-like feature, imbalance of perception of dark vs bright
	...
	return Color((i32)v[0], (i32)v[1], (i32)v[2]);
}
