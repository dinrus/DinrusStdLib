...
Txt Uuid::ToTxt() const
{
	return Format(*this);
}

Txt Uuid::ToTxtWithDashes() const
{
	return FormatWithDashes(*this);
}

Txt Dump(const Uuid& id) {
	return "UUID: " + Format(id);
}

struct UuidValGenClass : ValGen
{
	virtual Val Get() {
