#include "../Core.h"

#ifdef TARGET_WIN32
#include <winsock2.h>
#include <ws2tcpip.h>
#endif

#ifdef TARGET_POSIX
#include <arpa/inet.h>
#endif

namespace drx {

#ifdef TARGET_WIN32
#pragma comment(lib, "ws2_32.lib")
#endif

#define LLOG(x)  // LOG("TCP " << x)

IpAddrInfo::Entry IpAddrInfo::pool[IpAddrInfo::COUNT];

Mutex IpAddrInfoPoolMutex;

void IpAddrInfo::EnterPool()
{
	IpAddrInfoPoolMutex.Enter();
}

void IpAddrInfo::LeavePool()
{
	IpAddrInfoPoolMutex.Leave();
}

i32 sGetAddrInfo(tukk host, tukk port, i32 family, addrinfo **result)
{
	if(!host || !*host)
		return EAI_NONAME;
	addrinfo hints;
	memset(&hints, 0, sizeof(addrinfo));
	const static i32 FamilyToAF[] = { AF_UNSPEC, AF_INET, AF_INET6 };
	hints.ai_family = FamilyToAF[(family > 0 && family < __countof(FamilyToAF)) ? family : 0];
	hints.ai_socktype = SOCK_STREAM;
	hints.ai_protocol = IPPROTO_TCP;

	return getaddrinfo(host, port, &hints, result);
}

auxthread_t auxthread__ IpAddrInfo::Thread(uk ptr)
{
	Entry *entry = (Entry *)ptr;
	EnterPool();
	if(entry->status == WORKING) {
		char host[1025];
		char port[257];
		i32 family = entry->family;
		strcpy(host, entry->host);
		strcpy(port, entry->port);
		LeavePool();
		addrinfo *result;
		if(sGetAddrInfo(host, port, family, &result) == 0 && result) {
			EnterPool();
			if(entry->status == WORKING) {
				entry->addr = result;
				entry->status = RESOLVED;
			}
			else {
				freeaddrinfo(result);
				entry->status = EMPTY;
			}
		}
		else {
			EnterPool();
			if(entry->status == CANCELED)
				entry->status = EMPTY;
			else
				entry->status = FAILED;
		}
	}
	LeavePool();
	return 0;
}

bool IpAddrInfo::Execute(const Txt& host, i32 port, i32 family)
{
	Clear();
	entry = exe;
	addrinfo *result;
	entry->addr = sGetAddrInfo(~host, ~AsTxt(port), family, &result) == 0 ? result : NULL;
	entry->status = entry->addr ? RESOLVED : FAILED;
	return entry->addr;
}

void IpAddrInfo::Start()
{
	if(entry)
		return;
	EnterPool();
	for(i32 i = 0; i < COUNT; i++) {
		Entry *e = pool + i;
		if(e->status == EMPTY) {
			entry = e;
			e->addr = NULL;
			if(host.GetCount() > 1024 || port.GetCount() > 256)
				e->status = FAILED;
			else {
				e->status = WORKING;
				e->host = host;
				e->port = port;
				e->family = family;
				StartAuxThread(&IpAddrInfo::Thread, e);
			}
			break;
		}
	}
	LeavePool();
}

void IpAddrInfo::Start(const Txt& host_, i32 port_, i32 family_)
{
	Clear();
	port = AsTxt(port_);
	host = host_;
	family = family_;
	Start();
}

bool IpAddrInfo::InProgress()
{
	if(!entry) {
		Start();
		return true;
	}
	EnterPool();
	i32 s = entry->status;
	LeavePool();
	return s == WORKING;
}

addrinfo *IpAddrInfo::GetResult() const
{
	EnterPool();
	addrinfo *ai = entry ? entry->addr : NULL;
	LeavePool();
	return ai;
}

void IpAddrInfo::Clear()
{
	EnterPool();
	if(entry) {
		if(entry->status == RESOLVED && entry->addr)
			freeaddrinfo(entry->addr);
		if(entry->status == WORKING)
			entry->status = CANCELED;
		else
			entry->status = EMPTY;
		entry = NULL;
	}
	LeavePool();
}

IpAddrInfo::IpAddrInfo()
{
	TcpSocket::Init();
	entry = NULL;
}

#ifdef TARGET_POSIX

#define SOCKERR(x) x

tukk TcpSocketErrorDesc(i32 code)
{
	return strerror(code);
}

i32 TcpSocket::GetErrorCode()
{
	return errno;
}

#else

#define SOCKERR(x) WSA##x

tukk TcpSocketErrorDesc(i32 code)
{
	static Tuple<i32, tukk > err[] = {
		{ WSAEINTR,                 "Прерванный вызов функции." },
		{ WSAEACCES,                "Отказ в разрешении." },
		{ WSAEFAULT,                "Неверный адрес." },
		{ WSAEINVAL,                "Неверный аргумент." },
		{ WSAEMFILE,                "Слишком много открытых файлов." },
		{ WSAEWOULDBLOCK,           "Ресурс временно недоступен." },
		{ WSAEINPROGRESS,           "Операция уже в прогрессе." },
		{ WSAEALREADY,              "Операция уже в прогрессе." },
		{ WSAENOTSOCK,              "Операция TcpSocket не над сокетом." },
		{ WSAEDESTADDRREQ,          "Требуется адрес назначения." },
		{ WSAEMSGSIZE,              "Сообщение слишком длинное." },
		{ WSAEPROTOTYPE,            "Неверный тип протокола для сокета." },
		{ WSAENOPROTOOPT,           "Неверная опция протокола." },
		{ WSAEPROTONOSUPPORT,       "Протокол не поддерживается." },
		{ WSAESOCKTNOSUPPORT,       "Тип TcpSocket не поддерживается." },
		{ WSAEOPNOTSUPP,            "Операция не поддерживается." },
		{ WSAEPFNOSUPPORT,          "Семейство протоколов не поддерживается." },
		{ WSAEAFNOSUPPORT,          "Семейство адресов не поддерживается семейством протоколов." },
		{ WSAEADDRINUSE,            "Адрес уже используется." },
		{ WSAEADDRNOTAVAIL,         "Не удаётся присвоить заданный адрес." },
		{ WSAENETDOWN,              "Сета легла." },
		{ WSAENETUNREACH,           "Сеть недоступна." },
		{ WSAENETRESET,             "Сеть сбросила подключение при сбросе." },
		{ WSAECONNABORTED,          "ПО вызвало разрыв связи." },
		{ WSAECONNRESET,            "Пир сбросил соединение." },
		{ WSAENOBUFS,               "Нет места в буфере." },
		{ WSAEISCONN,               "TcpSocket уже подключен." },
		{ WSAENOTCONN,              "TcpSocket не подключен." },
		{ WSAESHUTDOWN,             "Не удаётся отправка после шатдауна сокета." },
		{ WSAETIMEDOUT,             "Таймаут соединения." },
		{ WSAECONNREFUSED,          "Соединение отклонено." },
		{ WSAEHOSTDOWN,             "Хост лёг." },
		{ WSAEHOSTUNREACH,          "Нет маршрута к хосту." },
		{ WSAEPROCLIM,              "Слишком много процессов." },
		{ WSASYSNOTREADY,           "Сетевая подсистема недоступна." },
		{ WSAVERNOTSUPPORTED,       "Версия Winsock.dll вне диапазона." },
		{ WSANOTINITIALISED,        "Удачный WSAStartup пока ещё не выполнен." },
		{ WSAEDISCON,               "В прогрессе благосклонный шатдаун!" },
		{ WSATYPE_NOT_FOUND,        "Тип класса не найден." },
		{ WSAHOST_NOT_FOUND,        "Хост не найден." },
		{ WSATRY_AGAIN,             "Неавторитативный хост не найден." },
		{ WSANO_RECOVERY,           "Это невосстановимая ошибка." },
		{ WSANO_DATA,               "Имя валидно, нет записи данных запрошенного типа." },
		{ WSASYSCALLFAILURE,        "Неудачный системный вызов." },
	};
	const Tuple<i32, tukk > *x = FindTuple(err, __countof(err), code);
	return x ? x->b : "Неизвестный код ошибки.";
}

i32 TcpSocket::GetErrorCode()
{
	return WSAGetLastError();
}

#endif

void TcpSocketInit()
{
#ifdef TARGET_WIN32
	ONCELOCK {
		WSADATA wsadata;
		WSAStartup(MAKEWORD(2, 2), &wsadata);
	}
#endif
#ifdef TARGET_POSIX
	signal(SIGPIPE, SIG_IGN);
#endif
}

void TcpSocket::Init()
{
	TcpSocketInit();
}

void TcpSocket::Reset()
{
	LLOG("Reset");
	is_eof = false;
	socket = INVALID_SOCKET;
	ipv6 = false;
	ptr = end = buffer;
	is_error = false;
	is_abort = false;
	is_timeout = false;
	mode = NONE;
	ssl.Clear();
	sslinfo.Clear();
	cert = pkey = Null;
#if defined(TARGET_WIN32) || defined(TARGET_BSD)
	connection_start = Null;
#endif
	ssl_start = Null;
}

TcpSocket::TcpSocket()
{
	ClearError();
	Reset();
	timeout = global_timeout = start_time = Null;
	waitstep = 10;
	asn1 = false;
}

bool TcpSocket::SetupSocket()
{
#ifdef TARGET_WIN32
	connection_start = msecs();
	u_long arg = 1;
	if(ioctlsocket(socket, FIONBIO, &arg)) {
		SetSockError("ioctlsocket(FIO[N]BIO)");
		return false;
	}
#else
	#ifdef TARGET_BSD
		connection_start = msecs();
	#endif
	if(fcntl(socket, F_SETFL, (fcntl(socket, F_GETFL, 0) | O_NONBLOCK))) {
		SetSockError("fcntl(O_[NON]BLOCK)");
		return false;
	}
#endif
	return true;
}

bool TcpSocket::Open(i32 family, i32 type, i32 protocol)
{
	Init();
	Close();
	ClearError();
	if((socket = ::socket(family, type, protocol)) == INVALID_SOCKET) {
		SetSockError("open");
		return false;
	}
	LLOG("TcpSocket::Data::Open() -> " << (i32)socket);
	return SetupSocket();
}

bool TcpSocket::Listen(i32 port, i32 listen_count, bool ipv6_, bool reuse, uk addr)
{
	Close();
	Init();
	Reset();

	ipv6 = ipv6_;
	if(!Open(ipv6 ? AF_INET6 : AF_INET, SOCK_STREAM, 0))
		return false;
	sockaddr_in sin;
#ifdef TARGET_WIN32
	SOCKADDR_IN6 sin6;
	if(ipv6 && IsWinVista())
#else
	sockaddr_in6 sin6;
	if(ipv6)
#endif
	{
		Zero(sin6);
		sin6.sin6_family = AF_INET6;
		sin6.sin6_port = htons(port);
		sin6.sin6_addr = addr?(*(in6_addr*)addr):in6addr_any;
	}
	else {
		Zero(sin);
		sin.sin_family = AF_INET;
		sin.sin_port = htons(port);
		sin.sin_addr.s_addr = addr?(*(u32*)addr):htonl(INADDR_ANY);
	}
	if(reuse) {
		i32 optval = 1;
		setsockopt(socket, SOL_SOCKET, SO_REUSEADDR, (tukk )&optval, sizeof(optval));
	}
	if(bind(socket, ipv6 ? (const sockaddr *)&sin6 : (const sockaddr *)&sin,
	        ipv6 ? sizeof(sin6) : sizeof(sin))) {
		SetSockError(Format("bind(port=%d)", port));
		return false;
	}
	if(listen(socket, listen_count)) {
		SetSockError(Format("listen(port=%d, count=%d)", port, listen_count));
		return false;
	}
	return true;
}

bool TcpSocket::Listen(const IpAddrInfo& addr, i32 port, i32 listen_count, bool ipv6, bool reuse)
{
	addrinfo *a = addr.GetResult();
	return Listen(port, listen_count, ipv6, reuse, &(((sockaddr_in*)a->ai_addr)->sin_addr.s_addr));
}

bool TcpSocket::Accept(TcpSocket& ls)
{
	Close();
	Init();
	Reset();
	ASSERT(ls.IsOpen());
	i32 et = GetEndTime();
	for(;;) {
		i32 h = ls.GetTimeout();
		bool b = ls.Timeout(timeout).Wait(WAIT_READ, et);
		ls.Timeout(h);
		if(!b) // timeout
			return false;
		socket = accept(ls.GetSOCKET(), NULL, NULL);
		if(socket != INVALID_SOCKET)
			break;
		if(!WouldBlock() && GetErrorCode() != SOCKERR(EINTR)) { // In prefork condition, Wait is not enough, as other process can accept
			SetSockError("accept");
			return false;
		}
	}
	mode = ACCEPT;
	return SetupSocket();
}

Txt TcpSocket::GetPeerAddr() const
{
	if(!IsOpen())
		return Null;
	sockaddr_in addr;
	socklen_t l = sizeof(addr);
	if(getpeername(socket, (sockaddr *)&addr, &l) != 0)
		return Null;
	if(l > sizeof(addr))
		return Null;
#ifdef TARGET_WIN32
	return inet_ntoa(addr.sin_addr);
#else
	char h[200];
	return inet_ntop(AF_INET, &addr.sin_addr, h, 200);
#endif
}

void TcpSocket::NoDelay()
{
	ASSERT(IsOpen());
	i32 __true = 1;
	LLOG("NoDelay(" << (i32)socket << ")");
	if(setsockopt(socket, IPPROTO_TCP, TCP_NODELAY, (tukk )&__true, sizeof(__true)))
		SetSockError("setsockopt(TCP_NODELAY)");
}

void TcpSocket::Linger(i32 msecs)
{
	ASSERT(IsOpen());
	linger ls;
	ls.l_onoff = !IsNull(msecs) ? 1 : 0;
	ls.l_linger = !IsNull(msecs) ? (msecs + 999) / 1000 : 0;
	if(setsockopt(socket, SOL_SOCKET, SO_LINGER, reinterpret_cast<tukk >(&ls), sizeof(ls)))
		SetSockError("setsockopt(SO_LINGER)");
}

void TcpSocket::Attach(SOCKET s)
{
	Close();
	socket = s;
}

bool TcpSocket::RawConnect(addrinfo *arp)
{
	if(!arp) {
		SetSockError("подключение", -1, "не найдено");
		return false;
	}
	Txt err;
	for(i32 pass = 0; pass < 2; pass++) {
		addrinfo *rp = arp;
		while(rp) {
			if(rp->ai_family == AF_INET == !pass && // Try to connect IPv4 in the first pass
			   Open(rp->ai_family, rp->ai_socktype, rp->ai_protocol)) {
				if(connect(socket, rp->ai_addr, (i32)rp->ai_addrlen) == 0 ||
				   GetErrorCode() == SOCKERR(EINPROGRESS) || GetErrorCode() == SOCKERR(EWOULDBLOCK)
				) {
					mode = CONNECT;
					return true;
				}
				if(err.GetCount())
					err << '\n';
				err << TcpSocketErrorDesc(GetErrorCode());
				Close();
			}
			rp = rp->ai_next;
		}
    }
	SetSockError("подключение", -1, Nvl(err, "не удалось"));
	return false;
}


bool TcpSocket::Connect(IpAddrInfo& info)
{
	LLOG("Connect addrinfo");
	Init();
	Reset();
	addrinfo *result = info.GetResult();
	return RawConnect(result);
}

bool TcpSocket::Connect(tukk host, i32 port)
{
	LLOG("Connect(" << host << ':' << port << ')');
	Close();
	Init();
	Reset();
	IpAddrInfo info;
	if(!info.Execute(host, port)) {
		SetSockError(Format("getaddrinfo(%s) неудачно", host));
		return false;
	}
	return Connect(info);
}

bool TcpSocket::WaitConnect()
{
	if(WaitWrite()) {
		i32 optval = 0;
		socklen_t optlen = sizeof(optval);
		if (getsockopt(GetSOCKET(), SOL_SOCKET, SO_ERROR, (char*)&optval, &optlen) == 0) {
			if (optval == 0)
				return true;
			else {
				SetSockError("wait connect", -1, Nvl(Txt(TcpSocketErrorDesc(optval)), "завершён неудачно"));
				return false;
			}
		}
	}
	return false;
}

void TcpSocket::RawClose()
{
	LLOG("close " << (i32)socket);
	if(socket != INVALID_SOCKET) {
		i32 res;
#if defined(TARGET_WIN32)
		res = closesocket(socket);
#elif defined(TARGET_POSIX)
		res = close(socket);
#else
	#error Unsupported platform
#endif
		if(res && !IsError())
			SetSockError("close");
		socket = INVALID_SOCKET;
	}
}

void TcpSocket::Close()
{
	if(ssl)
		ssl->Close();
	else
		RawClose();
	ssl.Clear();
}

bool TcpSocket::WouldBlock()
{
	i32 c = GetErrorCode();
#ifdef TARGET_POSIX
#ifdef TARGET_BSD
		if(c == SOCKERR(ENOTCONN) && !IsNull(connection_start) && msecs(connection_start) < 20000)
			return true;
#endif
	return c == SOCKERR(EWOULDBLOCK) || c == SOCKERR(EAGAIN);
#endif
#ifdef TARGET_WIN32
	if(c == SOCKERR(ENOTCONN) && !IsNull(connection_start) && msecs(connection_start) < 20000) {
		LLOG("ENOTCONN issue");
		return true;
	}
	return c == SOCKERR(EWOULDBLOCK);
#endif
}

i32 TcpSocket::RawSend(ukk buf, i32 amount)
{
#ifdef TARGET_LINUX
	i32 res = send(socket, (tukk )buf, amount, MSG_NOSIGNAL);
#else
	i32 res = send(socket, (tukk )buf, amount, 0);
#endif
	if(res < 0 && WouldBlock())
		res = 0;
	else
	if(res == 0 || res < 0)
		SetSockError("send");
	return res;
}

i32 TcpSocket::Send(ukk buf, i32 amount)
{
	if(SSLHandshake())
		return 0;
	return ssl ? ssl->Send(buf, amount) : RawSend(buf, amount);
}

void TcpSocket::Shutdown()
{
	ASSERT(IsOpen());
	if(shutdown(socket, SD_SEND))
		SetSockError("shutdown(SD_SEND)");
}

Txt TcpSocket::GetHostName()
{
	Init();
	char buffer[256];
	gethostname(buffer, __countof(buffer));
	return buffer;
}

bool TcpSocket::IsGlobalTimeout()
{
	if(!IsNull(global_timeout) && msecs() - start_time > global_timeout) {
		SetSockError("wait", ERROR_GLOBAL_TIMEOUT, "таймаут");
		return true;
	}
	return false;
}

bool TcpSocket::RawWait(dword flags, i32 end_time)
{ // wait till end_time
	LLOG("RawWait end_time: " << end_time << ", current time " << msecs() << ", to wait: " << end_time - msecs());
	is_timeout = false;
	if((flags & WAIT_READ) && ptr != end)
		return true;
	if(socket == INVALID_SOCKET)
		return false;
	for(;;) {
		if(IsError() || IsAbort())
			return false;
		i32 to = end_time - msecs();
		if(WhenWait)
			to = waitstep;
		timeval *tvalp = NULL;
		timeval tval;
		if(end_time != INT_MAX || WhenWait) {
			to = max(to, 0);
			tval.tv_sec = to / 1000;
			tval.tv_usec = 1000 * (to % 1000);
			tvalp = &tval;
			LLOG("RawWait timeout: " << to);
		}
		fd_set fdsetr[1], fdsetw[1], fdsetx[1];;
		FD_ZERO(fdsetr);
		if(flags & WAIT_READ)
			FD_SET(socket, fdsetr);
		FD_ZERO(fdsetw);
		if(flags & WAIT_WRITE)
			FD_SET(socket, fdsetw);
		FD_ZERO(fdsetx);
		FD_SET(socket, fdsetx);
		i32 avail = select((i32)socket + 1, fdsetr, fdsetw, fdsetx, tvalp);
		LLOG("Wait select avail: " << avail);
		if(avail < 0 && GetErrorCode() != SOCKERR(EINTR)) {
			SetSockError("wait");
			return false;
		}
		if(avail > 0) {
		#if defined(TARGET_WIN32) || defined(TARGET_BSD)
			connection_start = Null;
		#endif
			return true;
		}
		if(IsGlobalTimeout() || to <= 0 && timeout) {
			is_timeout = true;
			return false;
		}
		WhenWait();
		if(timeout == 0) {
			is_timeout = true;
			return false;
		}
	}
}

TcpSocket& TcpSocket::GlobalTimeout(i32 ms)
{
	start_time = msecs();
	global_timeout = ms;
	return *this;
}

bool TcpSocket::Wait(dword flags, i32 end_time)
{
	return ssl ? ssl->Wait(flags, end_time) : RawWait(flags, end_time);
}

i32  TcpSocket::GetEndTime() const
{ // Compute time limit for operation, based on global timeout and per-operation timeout settings
	i32 o = min(IsNull(global_timeout) ? INT_MAX : start_time + global_timeout,
	            IsNull(timeout) ? INT_MAX : msecs() + timeout);
#if defined(TARGET_WIN32) || defined(TARGET_BSD)
	if(GetErrorCode() == SOCKERR(ENOTCONN) && !IsNull(connection_start))
		if(msecs(connection_start) < 20000)
			o = connection_start + 20000;
#endif
	return o;
}

bool TcpSocket::Wait(dword flags)
{
	return Wait(flags, GetEndTime());
}

i32 TcpSocket::Put(ukk s_, i32 length)
{
	LLOG("Put " << socket << ": " << length);
	ASSERT(IsOpen());
	tukk s = (tukk )s_;
	if(length < 0 && s)
		length = (i32)strlen(s);
	if(!s || length <= 0 || IsError() || IsAbort())
		return 0;
	done = 0;
	bool peek = false;
	i32 end_time = GetEndTime();
	while(done < length) {
		if(peek && !Wait(WAIT_WRITE, end_time))
			return done;
		peek = false;
		i32 count = Send(s + done, length - done);
		if(IsError() || timeout == 0 && count == 0 && peek)
			return done;
		if(count > 0)
			done += count;
		else
			peek = true;
	}
	LLOG("//Put() -> " << done);
	return done;
}

bool TcpSocket::PutAll(ukk s, i32 len)
{
	if(Put(s, len) != len) {
		if(!IsError())
			SetSockError("GePutAll", -1, "таймаут");
		return false;
	}
	return true;
}

bool TcpSocket::PutAll(const Txt& s)
{
	if(Put(s) != s.GetCount()) {
		if(!IsError())
			SetSockError("GePutAll", -1, "таймаут");
		return false;
	}
	return true;
}

i32 TcpSocket::RawRecv(uk buf, i32 amount)
{
	i32 res = recv(socket, (tuk )buf, amount, 0);
	if(res == 0)
		is_eof = true;
	else
	if(res < 0 && WouldBlock())
		res = 0;
	else
	if(res < 0)
		SetSockError("recv");
	LLOG("recv(" << socket << "): " << res << " bytes: "
	     << AsCTxt((tuk )buf, (tuk )buf + min(res, 16))
	     << (res ? "" : IsEof() ? ", EOF" : ", WOULDBLOCK"));
	return res;
}

i32 TcpSocket::Recv(uk buffer, i32 maxlen)
{
	if(SSLHandshake())
		return 0;
	return ssl ? ssl->Recv(buffer, maxlen) : RawRecv(buffer, maxlen);
}

void TcpSocket::ReadBuffer(i32 end_time)
{
	ptr = end = buffer;
	if(Wait(WAIT_READ, end_time))
		end = buffer + Recv(buffer, BUFFERSIZE);
}

bool TcpSocket::IsEof() const
{
	return is_eof && ptr == end || IsAbort() || !IsOpen() || IsError();
}

i32 TcpSocket::Get_()
{
	if(!IsOpen() || IsError() || IsEof() || IsAbort())
		return -1;
	ReadBuffer(GetEndTime());
	return ptr < end ? (u8)*ptr++ : -1;
}

i32 TcpSocket::Peek_(i32 end_time)
{
	if(!IsOpen() || IsError() || IsEof() || IsAbort())
		return -1;
	ReadBuffer(end_time);
	return ptr < end ? (u8)*ptr : -1;
}

i32 TcpSocket::Peek_()
{
	return Peek_(GetEndTime());
}

i32 TcpSocket::Get(uk buffer, i32 count)
{
	LLOG("Get " << count);

	if(!IsOpen() || IsError() || IsEof() || IsAbort())
		return 0;

	i32 l = (i32)(end - ptr);
	done = 0;
	if(l > 0) {
		if(l < count) {
			memcpy(buffer, ptr, l);
			done += l;
			ptr = end;
		}
		else {
			memcpy(buffer, ptr, count);
			ptr += count;
			return count;
		}
	}
	i32 end_time = GetEndTime();
	while(done < count && !IsError() && !IsEof()) {
		if(!Wait(WAIT_READ, end_time))
			break;
		i32 part = Recv((tuk )buffer + done, count - done);
		if(part > 0)
			done += part;
		if(timeout == 0)
			break;
	}
	return done;
}

Txt TcpSocket::Get(i32 count)
{
	if(count == 0)
		return Null;
	TxtBuf out(count);
	i32 done = Get(out, count);
	if(!done && IsEof())
		return Txt::GetVoid();
	out.SetLength(done);
	return Txt(out);
}

bool  TcpSocket::GetAll(uk buffer, i32 len)
{
	if(Get(buffer, len) == len)
		return true;
	if(!IsError())
		SetSockError("GetAll", -1, "таймаут");
	return false;
}

Txt TcpSocket::GetAll(i32 len)
{
	Txt s = Get(len);
	if(s.GetCount() != len) {
		if(!IsError())
			SetSockError("GetAll", -1, "таймаут");
		return Txt::GetVoid();
	}
	return s;
}

Txt TcpSocket::GetLine(i32 maxlen)
{
	LLOG("GetLine " << maxlen << ", iseof " << IsEof());
	Txt ln;
	i32 end_time = GetEndTime();
	for(;;) {
		if(IsEof())
			return Txt::GetVoid();
		i32 c = Peek(end_time);
		if(c < 0) {
			if(!IsError()) {
				if(msecs() > end_time)
					SetSockError("GetLine", -1, "таймаут");
				else
					continue;
			}
			return Txt::GetVoid();
		}
		Get();
		if(c == '\n')
			return ln;
		if(ln.GetCount() >= maxlen) {
			if(!IsError())
				SetSockError("GetLine", -1, "исчерпана максимальная длина");
			return Txt::GetVoid();
		}
		if(c != '\r')
			ln.Cat(c);
	}
}

void TcpSocket::SetSockError(tukk context, i32 code, tukk errdesc)
{
	errorcode = code;
	errordesc.Clear();
	if(socket != INVALID_SOCKET)
		errordesc << "socket(" << (i32)socket << ") / ";
	errordesc << context << ": " << errdesc;
	is_error = true;
	LLOG("ERROR " << errordesc);
}

void TcpSocket::SetSockError(tukk context, tukk errdesc)
{
	SetSockError(context, GetErrorCode(), errdesc);
}

void TcpSocket::SetSockError(tukk context)
{
	SetSockError(context, TcpSocketErrorDesc(GetErrorCode()));
}

TcpSocket::SSL *(*TcpSocket::CreateSSL)(TcpSocket& socket);

bool TcpSocket::StartSSL()
{
	ASSERT(IsOpen());
	if(!CreateSSL) {
		SetSockError("StartSSL", -1, "Отсутствует поддержка SSL (Core/SSL)");
		return false;
	}
	if(!IsOpen()) {
		SetSockError("StartSSL", -1, "Сокет закрытый");
		return false;
	}
	if(mode != CONNECT && mode != ACCEPT) {
		SetSockError("StartSSL", -1, "Сокет неподключенный");
		return false;
	}
	ssl = (*CreateSSL)(*this);
	if(!ssl->Start()) {
		ssl.Clear();
		return false;
	}
	ssl_start = msecs();
	SSLHandshake();
	return true;
}

dword TcpSocket::SSLHandshake()
{
	if(ssl && (mode == CONNECT || mode == ACCEPT)) {
		dword w = ssl->Handshake();
		if(w) {
			if(msecs(ssl_start) > 20000) {
				SetSockError("Рукопожатие ssl", ERROR_SSLHANDSHAKE_TIMEOUT, "таймаут");
				return false;
			}
			if(IsGlobalTimeout())
				return false;
			Wait(w);
			return ssl->Handshake();
		}
	}
	return 0;
}

void TcpSocket::SSLCertificate(const Txt& cert_, const Txt& pkey_, bool asn1_)
{
	cert = cert_;
	pkey = pkey_;
	asn1 = asn1_;
}

void TcpSocket::SSLServerNameIndication(const Txt& name)
{
	sni = name;
}

void TcpSocket::Clear()
{
	ClearError();
	if(IsOpen())
		Close();
	Reset();
}

i32 SocketWaitEvent::Wait(i32 timeout)
{
	FD_ZERO(read);
	FD_ZERO(write);
	FD_ZERO(exception);
	i32 maxindex = -1;
	for(i32 i = 0; i < socket.GetCount(); i++) {
		const Tuple<i32, dword>& s = socket[i];
		if(s.a >= 0) {
			const Tuple<i32, dword>& s = socket[i];
			if(s.b & WAIT_READ)
				FD_SET(s.a, read);
			if(s.b & WAIT_WRITE)
				FD_SET(s.a, write);
			FD_SET(s.a, exception);
			maxindex = max(s.a, maxindex);
		}
	}
	timeval *tvalp = NULL;
	timeval tval;
	if(!IsNull(timeout)) {
		tval.tv_sec = timeout / 1000;
		tval.tv_usec = 1000 * (timeout % 1000);
		tvalp = &tval;
	}
	return select(maxindex + 1, read, write, exception, tvalp);
}

dword SocketWaitEvent::Get(i32 i) const
{
	i32 s = socket[i].a;
	if(s < 0)
		return 0;
	dword events = 0;
	if(FD_ISSET(s, read))
		events |= WAIT_READ;
	if(FD_ISSET(s, write))
		events |= WAIT_WRITE;
	if(FD_ISSET(s, exception))
		events |= WAIT_IS_EXCEPTION;
	return events;
}

SocketWaitEvent::SocketWaitEvent()
{
	FD_ZERO(read);
	FD_ZERO(write);
	FD_ZERO(exception);
}

}