...
static Tuple<tukk , tukk > sEXT_MIME[] = {
	{ "txt", "text/plain" },
	{ "html", "text/html" },
	{ "jpeg", "image/jpeg" },
	{ "eml", "message/rfc822" },
...
static const Index<Txt>& sEXT()
{
	static Index<Txt> x;
	ONCELOCK {
		for(i32 i = 0; i < __countof(sEXT_MIME); i++)
			x.Add(sEXT_MIME[i].a);
	}
	return x;
}

static const Index<Txt>& sMIME()
{
	static Index<Txt> x;
	ONCELOCK {
		for(i32 i = 0; i < __countof(sEXT_MIME); i++)
