...
thread_local i32     CoWork::worker_index = -1;
...
void CoWork::Pool::InitThreads(i32 nthreads)
{
...
void CoWork::Pool::ExitThreads()
{
	...
	for(i32 i = 0; i < threads.GetCount(); i++)
		threads[i].Wait();
	threads.Clear();
	lock.Enter();
	quit = false;
...
i32 CoWork::GetPoolSize()
{
	i32 n = GetPool().threads.GetCount();
...
void CoWork::Pool::ThreadRun(i32 tno)
{
...
void CoWork::Pipe(i32 stepi, Function<void ()>&& fn)
{
