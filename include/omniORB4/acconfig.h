/* include/omniORB4/acconfig_pre.h.  Generated from acconfig_pre.h.in by configure.  */
/* include/omniORB4/acconfig_pre.h.in.  Generated from configure.ac by autoheader.  */

/* Define if building universal (internal helper macro) */
/* #undef OMNI_AC_APPLE_UNIVERSAL_BUILD */

/* Define to 1 if using 'alloca.c'. */
/* #undef OMNI_C_ALLOCA */

/* define if you want to avoid use of alloca */
/* #undef OMNI_DISABLE_ALLOCA */

/* define if you want to disable atomic operations */
/* #undef OMNI_DISABLE_ATOMIC_OPS */

/* define if you want to disable IPv6 support */
/* #undef OMNI_DISABLE_IPV6 */

/* define if you want to disable long double support */
/* #undef OMNI_DISABLE_LONGDOUBLE */

/* define if you want mutexes to be traced */
/* #undef OMNI_ENABLE_LOCK_TRACES */

/* define to enable ziop */
#define OMNI_ENABLE_ZIOP /**/

/* define to support zlib in ziop */
#define OMNI_ENABLE_ZIOP_ZLIB /**/

/* define to support zstd in ziop */
#define OMNI_ENABLE_ZIOP_ZSTD /**/

/* define if gettimeofday() takes a timezone argument */
#define OMNI_GETTIMEOFDAY_TIMEZONE /**/

/* Define to 1 if you have 'alloca', as a function or macro. */
#define OMNI_HAVE_ALLOCA 1

/* Define to 1 if <alloca.h> works. */
#define OMNI_HAVE_ALLOCA_H 1

/* define if bool is a built-in type */
#define OMNI_HAVE_BOOL /**/

/* define if the compiler can catch exceptions by base class */
#define OMNI_HAVE_CATCH_BY_BASE /**/

/* define if the compiler supports const_cast<> */
#define OMNI_HAVE_CONST_CAST /**/

/* define if the compiler supports covariant return types */
#define OMNI_HAVE_COVARIANT_RETURNS /**/

/* define if the compiler supports dynamic_cast<> */
#define OMNI_HAVE_DYNAMIC_CAST /**/

/* Define to 1 if you have the <errno.h> header file. */
#define OMNI_HAVE_ERRNO_H 1

/* define if the compiler supports exceptions */
#define OMNI_HAVE_EXCEPTIONS /**/

/* Define to 1 if you have the <fcntl.h> header file. */
#define OMNI_HAVE_FCNTL_H 1

/* Define to 1 if you have the 'getaddrinfo' function. */
#define OMNI_HAVE_GETADDRINFO 1

/* Define to 1 if you have the 'gethostname' function. */
#define OMNI_HAVE_GETHOSTNAME 1

/* Define to 1 if you have the 'getnameinfo' function. */
#define OMNI_HAVE_GETNAMEINFO 1

/* Define to 1 if you have the 'getopt' function. */
#define OMNI_HAVE_GETOPT 1

/* Define to 1 if you have the 'getpid' function. */
#define OMNI_HAVE_GETPID 1

/* Define to 1 if you have the 'gettimeofday' function. */
#define OMNI_HAVE_GETTIMEOFDAY 1

/* Define to 1 if you have the 'gmtime' function. */
#define OMNI_HAVE_GMTIME 1

/* Define to 1 if you have the 'gmtime_r' function. */
#define OMNI_HAVE_GMTIME_R 1

/* Define to 1 if you have the <ifaddrs.h> header file. */
#define OMNI_HAVE_IFADDRS_H 1

/* Define to 1 if you have the 'inet_ntop' function. */
#define OMNI_HAVE_INET_NTOP 1

/* Define to 1 if you have the <inttypes.h> header file. */
#define OMNI_HAVE_INTTYPES_H 1

/* Define to 1 if you have the 'isinf' function. */
#define OMNI_HAVE_ISINF 1

/* Define to 1 if you have the 'isinff' function. */
#define OMNI_HAVE_ISINFF 1

/* Define to 1 if you have the 'isinfl' function. */
#define OMNI_HAVE_ISINFL 1

/* Define to 1 if you have the 'isnan' function. */
#define OMNI_HAVE_ISNAN 1

/* Define to 1 if you have the 'isnanf' function. */
#define OMNI_HAVE_ISNANF 1

/* Define to 1 if you have the 'isnanl' function. */
#define OMNI_HAVE_ISNANL 1

/* Define if you have the `IsNANorINF' function */
/* #undef OMNI_HAVE_ISNANORINF */

/* Define to 1 if you have the 'localtime' function. */
#define OMNI_HAVE_LOCALTIME 1

/* Define to 1 if you have the 'localtime_r' function. */
#define OMNI_HAVE_LOCALTIME_R 1

/* define if the compiler supports member constants */
#define OMNI_HAVE_MEMBER_CONSTANTS /**/

/* define if the compiler implements namespaces */
#define OMNI_HAVE_NAMESPACES /**/

/* Define to 1 if you have the 'nanosleep' function. */
#define OMNI_HAVE_NANOSLEEP 1

/* Define to 1 if you have the <nan.h> header file. */
/* #undef OMNI_HAVE_NAN_H */

/* Define to 1 if you have the <netdb.h> header file. */
#define OMNI_HAVE_NETDB_H 1

/* Define to 1 if you have the 'poll' function. */
#define OMNI_HAVE_POLL 1

/* Define to 1 if you have the 'rand_r' function. */
#define OMNI_HAVE_RAND_R 1

/* define if the compiler supports reinterpret_cast<> */
#define OMNI_HAVE_REINTERPRET_CAST /**/

/* Define to 1 if you have the 'sigaction' function. */
#define OMNI_HAVE_SIGACTION 1

/* Define to 1 if you have the <signal.h> header file. */
#define OMNI_HAVE_SIGNAL_H 1

/* Define to 1 if you have the 'sigvec' function. */
/* #undef OMNI_HAVE_SIGVEC */

/* define if SIG_IGN is available */
#define OMNI_HAVE_SIG_IGN /**/

/* Define to 1 if you have the 'snprintf' function. */
#define OMNI_HAVE_SNPRINTF 1

/* define if the compiler supports ISO C++ standard library */
#define OMNI_HAVE_STD /**/

/* Define to 1 if you have the <stdint.h> header file. */
#define OMNI_HAVE_STDINT_H 1

/* Define to 1 if you have the <stdio.h> header file. */
#define OMNI_HAVE_STDIO_H 1

/* Define to 1 if you have the <stdlib.h> header file. */
#define OMNI_HAVE_STDLIB_H 1

/* Define to 1 if you have the 'strcasecmp' function. */
#define OMNI_HAVE_STRCASECMP 1

/* Define to 1 if you have the 'strdup' function. */
#define OMNI_HAVE_STRDUP 1

/* Define to 1 if you have the 'strerror' function. */
#define OMNI_HAVE_STRERROR 1

/* Define to 1 if you have the 'strftime' function. */
#define OMNI_HAVE_STRFTIME 1

/* Define to 1 if you have the 'stricmp' function. */
/* #undef OMNI_HAVE_STRICMP */

/* Define to 1 if you have the <strings.h> header file. */
#define OMNI_HAVE_STRINGS_H 1

/* Define to 1 if you have the <string.h> header file. */
#define OMNI_HAVE_STRING_H 1

/* Define to 1 if you have the 'strncasecmp' function. */
#define OMNI_HAVE_STRNCASECMP 1

/* Define to 1 if you have the 'strtoul' function. */
#define OMNI_HAVE_STRTOUL 1

/* Define to 1 if you have the 'strtoull' function. */
#define OMNI_HAVE_STRTOULL 1

/* Define to 1 if you have the 'strtouq' function. */
#define OMNI_HAVE_STRTOUQ 1

/* Define to 1 if the system has the type 'struct lifconf'. */
/* #undef OMNI_HAVE_STRUCT_LIFCONF */

/* Define to 1 if the system has the type 'struct sockaddr_in6'. */
#define OMNI_HAVE_STRUCT_SOCKADDR_IN6 1

/* Define to 1 if 'sin_len' is a member of 'struct sockaddr_in'. */
/* #undef OMNI_HAVE_STRUCT_SOCKADDR_IN_SIN_LEN */

/* Define to 1 if 'sin_zero' is a member of 'struct sockaddr_in'. */
#define OMNI_HAVE_STRUCT_SOCKADDR_IN_SIN_ZERO 1

/* Define to 1 if the system has the type 'struct sockaddr_storage'. */
#define OMNI_HAVE_STRUCT_SOCKADDR_STORAGE 1

/* define if __sync_add_and_fetch and __sync_sub_and_fetch are available */
#define OMNI_HAVE_SYNC_ADD_AND_FETCH /**/

/* Define to 1 if you have the <sys/if.h> header file. */
/* #undef OMNI_HAVE_SYS_IF_H */

/* Define to 1 if you have the <sys/ioctl.h> header file. */
#define OMNI_HAVE_SYS_IOCTL_H 1

/* Define to 1 if you have the <sys/param.h> header file. */
#define OMNI_HAVE_SYS_PARAM_H 1

/* Define to 1 if you have the <sys/poll.h> header file. */
#define OMNI_HAVE_SYS_POLL_H 1

/* Define to 1 if you have the <sys/stat.h> header file. */
#define OMNI_HAVE_SYS_STAT_H 1

/* Define to 1 if you have the <sys/time.h> header file. */
#define OMNI_HAVE_SYS_TIME_H 1

/* Define to 1 if you have the <sys/types.h> header file. */
#define OMNI_HAVE_SYS_TYPES_H 1

/* Define to 1 if you have the 'uname' function. */
#define OMNI_HAVE_UNAME 1

/* Define to 1 if you have the <unistd.h> header file. */
#define OMNI_HAVE_UNISTD_H 1

/* Define to 1 if you have the 'vprintf' function. */
#define OMNI_HAVE_VPRINTF 1

/* Define to 1 if you have the 'vsnprintf' function. */
#define OMNI_HAVE_VSNPRINTF 1

/* Define to 1 if you have the <zlib.h> header file. */
#define OMNI_HAVE_ZLIB_H 1

/* Define to 1 if you have the <zstd.h> header file. */
#define OMNI_HAVE_ZSTD_H 1

/* define if long is the same type as int */
/* #undef OMNI_LONG_IS_INT */

/* Define to the address where bug reports for this package should be sent. */
#define OMNI_PACKAGE_BUGREPORT "bugs@omniorb-support.com"

/* Define to the full name of this package. */
#define OMNI_PACKAGE_NAME "omniORB"

/* Define to the full name and version of this package. */
#define OMNI_PACKAGE_STRING "omniORB 4.3.3"

/* Define to the one symbol short name of this package. */
#define OMNI_PACKAGE_TARNAME "omniorb"

/* Define to the home page for this package. */
#define OMNI_PACKAGE_URL ""

/* Define to the version of this package. */
#define OMNI_PACKAGE_VERSION "4.3.3"

/* define if base constructors have to be fully qualified */
/* #undef OMNI_REQUIRES_FQ_BASE_CTOR */

/* The size of 'bool', as computed by sizeof. */
#define OMNI_SIZEOF_BOOL 1

/* The size of 'char', as computed by sizeof. */
#define OMNI_SIZEOF_CHAR 1

/* The size of 'double', as computed by sizeof. */
#define OMNI_SIZEOF_DOUBLE 8

/* The size of 'float', as computed by sizeof. */
#define OMNI_SIZEOF_FLOAT 4

/* The size of 'int', as computed by sizeof. */
#define OMNI_SIZEOF_INT 4

/* The size of 'long', as computed by sizeof. */
#define OMNI_SIZEOF_LONG 8

/* The size of 'long double', as computed by sizeof. */
#define OMNI_SIZEOF_LONG_DOUBLE 16

/* The size of 'long long', as computed by sizeof. */
#define OMNI_SIZEOF_LONG_LONG 8

/* The size of 'short', as computed by sizeof. */
#define OMNI_SIZEOF_SHORT 2

/* The size of 'unsigned char', as computed by sizeof. */
#define OMNI_SIZEOF_UNSIGNED_CHAR 1

/* The size of 'void*', as computed by sizeof. */
#define OMNI_SIZEOF_VOIDP 8

/* The size of 'wchar_t', as computed by sizeof. */
#define OMNI_SIZEOF_WCHAR_T 4

/* Define to the type of getsockname's third argument */
#define OMNI_SOCKNAME_SIZE_T socklen_t

/* If using the C implementation of alloca, define if you know the
   direction of stack growth for your system; otherwise it will be
   automatically deduced at runtime.
	STACK_DIRECTION > 0 => grows toward higher addresses
	STACK_DIRECTION < 0 => grows toward lower addresses
	STACK_DIRECTION = 0 => direction of growth unknown */
/* #undef OMNI_STACK_DIRECTION */

/* Define to 1 if all of the C89 standard headers exist (not just the ones
   required in a freestanding environment). This macro is provided for
   backward compatibility; new code need not use it. */
#define OMNI_STDC_HEADERS 1

/* define if you want to use CFNetwork */
/* #undef OMNI_USE_CFNETWORK_CONNECT */

/* Define WORDS_BIGENDIAN to 1 if your processor stores words with the most
   significant byte first (like Motorola and SPARC, unlike Intel). */
#if defined AC_APPLE_UNIVERSAL_BUILD
# if defined __BIG_ENDIAN__
#  define OMNI_WORDS_BIGENDIAN 1
# endif
#else
# ifndef WORDS_BIGENDIAN
/* #  undef OMNI_WORDS_BIGENDIAN */
# endif
#endif

/* Define as 'unsigned int' if <stddef.h> doesn't define. */
/* #undef size_t */
