...
СИЭКСП i32 getentropy(uk buffer, size_t length);
СИЭКСП void explicit_bzero(uk s, size_t n)__THROW __nonnull ((1))
...
СИЭКСП i32 close_range(u32 first, u32 last, i32 flags)__THROW;
СИЭКСП i32 close_from(u32 first);

////////////////// arc4random - находится в libdrx_unwind /////////
/*
...
/*
 * Генерирует и возвращает случайное 32-битное число
 */
СИЭКСП u32 arc4random(void);

/*
 * Генерирует и возвращает униформно случайное 32-битное качество
 * с верхней границей 'upper_bound'
 */
СИЭКСП u32 arc4random_uniform(u32 upper_bound);

/*
 * Генерирует 'n' случайных байтов и помещает их в 'buf'.
 */
СИЭКСП void arc4random_buf(uk buf, size_t n);

/*
 * Генерирует случайный UUID
 *
 * n должно быть длиной не менее 16 байтов.
 */
СИЭКСП void randuuid(u8* b, size_t n);
