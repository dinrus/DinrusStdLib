/* cairo - библиотека векторной графики с дисплеем и выводом на печать
 *
 ...
 * cairo_t:
 *
 * A #cairo_t содержит текущее состояние отображающего устройства,
 * включая координаты пока ещё не прорисованных фигур.
 *
 * Контексты Cairo, названные как объекты #cairo_t, занимают центральное
 * место в cairo и всякое рисование посредством cairo всегда выполняется
 * в объект #cairo_t.
 *
 * Управление памятью #cairo_t выполняется посредством
 * cairo_reference() и cairo_destroy().
 *
 * Since: 1.0
 **/
 ...
 * cairo_surface_t:
 *
 * #cairo_surface_t представляет изображение, либо как цель операции отрисовки,
 *либо как источник,- при отрисовке на другой поверхности.
 * Для рисования в #cairo_surface_t создаётся контекст cairo
 * с целевой поверхностью, посредством cairo_create().
 *
 * Есть разные подтипы #cairo_surface_t для различных отрисовывающих бэкэндов;
 * например, cairo_image_surface_create() создаёт рисунок bitmap  в памяти.
 * Тип поверхности опрашивается посредством cairo_surface_get_type().
 *
 * Начальное содержимое поверхности после создания зависит от способа её
 * создания. Если cairo создаёт поверхность и поддерживающее хранилище для
 * пользователя, изначально она будет очищена; например,
 * cairo_image_surface_create() и cairo_surface_create_similar().
 * Альтернативно, если пользователь передаёт ссылку на какое-то плддерживающее
 * хранилище и хочет, чтобы cairo обернул его в #cairo_surface_t, тогда содержимое
 * не изменяется; например, cairo_image_surface_create_for_data() и
 * cairo_xlib_surface_create().
 *
 * Управление памятью #cairo_surface_t выполняется посредством
 * cairo_surface_reference() и cairo_surface_destroy().
 *
 * Since: 1.0
 **/
 ...
 * cairo_device_t:
 *
 * #cairo_device_t представляет интерфейс драйвера для операций отрисовки
 * на #cairo_surface_t.  Для разных отрисовочных бэкэндов есть различные
 * типы #cairo_device_t.
 *
 * Опросить тип устройства можно посредством cairo_device_get_type().
 *
 * Управление памятью #cairo_device_t выполнется посредством
 * cairo_device_reference() и cairo_device_destroy().
 *
 * Since: 1.10
 **/
