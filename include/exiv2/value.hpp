...
class EXIV2API Value {
 public:
  ...
  /// @param byteOrder Applicable u8 order (little or big endian).
  /// @return 0 if successful.
  virtual int read(const u8* buf, size_t len, ByteOrder byteOrder) = 0;
   ...
   */
  virtual int setDataArea(const u8* buf, size_t len);
  //@}
  ...
  */
  virtual size_t copy(u8* buf, ByteOrder byteOrder) const = 0;
...
class EXIV2API DataValue : public Value {
 public:
  ...
  DataValue(const u8* buf, size_t len, ByteOrder byteOrder = invalidByteOrder, TypeId typeId = undefined);

  //! @name Manipulators
  //@{
  int read(const u8* buf, size_t len, ByteOrder byteOrder = invalidByteOrder) override;
  ...
  /*!
    ...
    @note The u8 order is required by the interface but not used by this
  ...
  */
  size_t copy(u8* buf, ByteOrder byteOrder = invalidByteOrder) const override;
...
/*!
  ...
  This can be a plain Ascii string or a multiple u8 encoded string. It is
...
/*!
  ...
  This class is for null terminated single u8 Ascii strings.
...
/*!
  ...
  This can be a plain Ascii string or a multiple u8 encoded string. The
...
class EXIV2API CommentValue : public StringValueBase {
  ...
  */
  ...
  int read(const u8* buf, size_t len, ByteOrder byteOrder) override;
  //@}
  ...
  size_t copy(u8* buf, ByteOrder byteOrder) const override;
  /*!
...
class ValueType : public Value {
 ...
 public:
  ...
  ValueType();
  //! Constructor.
  ValueType(const u8* buf, size_t len, ByteOrder byteOrder, TypeId typeId = getType<T>());
  //! Constructor.
  ...
  /*!
    @brief Set the data area. This method copies (clones) the buffer
           pointed to by buf.
   */
  int setDataArea(const u8* buf, size_t len) override;
  //@}
 ...
 */
template <typename T>
T getValue(const u8* buf, ByteOrder byteOrder);
// Specialization for a 2 u8 unsigned short value.
template <>
inline uint16_t getValue(const u8* buf, ByteOrder byteOrder) {
  return getUShort(buf, byteOrder);
}
// Specialization for a 4 u8 unsigned long value.
template <>
inline uint32_t getValue(const u8* buf, ByteOrder byteOrder) {
  return getULong(buf, byteOrder);
}
// Specialization for an 8 u8 unsigned rational value.
template <>
inline URational getValue(const u8* buf, ByteOrder byteOrder) {
  return getURational(buf, byteOrder);
}
// Specialization for a 2 u8 signed short value.
template <>
inline int16_t getValue(const u8* buf, ByteOrder byteOrder) {
  return getShort(buf, byteOrder);
}
// Specialization for a 4 u8 signed long value.
template <>
inline int32_t getValue(const u8* buf, ByteOrder byteOrder) {
  return getLong(buf, byteOrder);
}
// Specialization for an 8 u8 signed rational value.
template <>
inline Rational getValue(const u8* buf, ByteOrder byteOrder) {
  return getRational(buf, byteOrder);
}
// Specialization for a 4 u8 float value.
template <>
inline float getValue(const u8* buf, ByteOrder byteOrder) {
  return getFloat(buf, byteOrder);
}
// Specialization for a 8 u8 double value.
template <>
inline double getValue(const u8* buf, ByteOrder byteOrder) {
 ...
 */
template <typename T>
size_t toData(u8* buf, T t, ByteOrder byteOrder);
/*!
  @brief Specialization to write an unsigned short to the data buffer.
         Return the number of bytes written.
 */
template <>
inline size_t toData(u8* buf, uint16_t t, ByteOrder byteOrder) {
 ...
 */
template <>
inline size_t toData(u8* buf, uint32_t t, ByteOrder byteOrder) {
 ...
 */
template <>
inline size_t toData(u8* buf, URational t, ByteOrder byteOrder) {
 ...
 */
template <>
inline size_t toData(u8* buf, int16_t t, ByteOrder byteOrder) {
 ...
 */
template <>
inline size_t toData(u8* buf, int32_t t, ByteOrder byteOrder) {
 ...
 */
template <>
inline size_t toData(u8* buf, Rational t, ByteOrder byteOrder) {
 ...
 */
template <>
inline size_t toData(u8* buf, float t, ByteOrder byteOrder) {
 ...
 */
template <>
inline size_t toData(u8* buf, double t, ByteOrder byteOrder) {
...
template <typename T>
ValueType<T>::ValueType(const u8* buf, size_t len, ByteOrder byteOrder, TypeId typeId) : Value(typeId) {
...

{
  if (rhs.sizeDataArea_ > 0) {
    pDataArea_ = new u8[rhs.sizeDataArea_];
...
template <typename T>
int ValueType<T>::read(const u8* buf, size_t len, ByteOrder byteOrder) {
  value_.clear();
...
template <typename T>
size_t ValueType<T>::copy(u8* buf, ByteOrder byteOrder) const {
  size_t offset = 0;
...
template <typename T>
int ValueType<T>::setDataArea(const u8* buf, size_t len) {
  u8* tmp = nullptr;
  if (len > 0) {
    tmp = new u8[len];
