...
struct EXIV2API Photoshop {
  ...
  static bool isIrb(const u8* pPsData);
  ...
  static bool valid(const u8* pPsData, size_t sizePsData);
  ...
  static int locateIrb(const u8* pPsData, size_t sizePsData, uint16_t psTag, const u8** record, uint32_t& sizeHdr,
                       uint32_t& sizeData);

  /// @brief Forwards to locateIrb() with \em psTag = \em iptc_
  static int locateIptcIrb(const u8* pPsData, size_t sizePsData, const u8** record, uint32_t& sizeHdr,
                           uint32_t& sizeData);

  /// @brief Forwards to locatePreviewIrb() with \em psTag = \em preview_
  static int locatePreviewIrb(const u8* pPsData, size_t sizePsData, const u8** record, uint32_t& sizeHdr,
                              uint32_t& sizeData);
  ...
  static DataBuf setIptcIrb(const u8* pPsData, size_t sizePsData, const IptcData& iptcData);
};
}  // namespace Exiv2
