...
class EXIV2API IptcData {
 public:
  ...
  static void printStructure(std::ostream& out, const Slice<u8*>& bytes, size_t depth);
  //@}
...
class EXIV2API IptcParser {
 public:
   ...
   */
  static int decode(IptcData& iptcData, const u8* pData, size_t size);
 ...
 private:
  // Constant data
  static constexpr u8 marker_ = 0x1C;  // Dataset marker
