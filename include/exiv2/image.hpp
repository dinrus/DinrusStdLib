...
class EXIV2API Image {
 public:
  ...
  /*!
    @brief Set the u8 order to encode the Exif metadata in.
    ...
    already contains Exif metadata, the u8 order of the existing data
    is used. If u8 order is not set when writeMetadata() is called,
    little-endian u8 order (II) is used by default.
   */
  ...
  //@{
  /*!
    @brief Return the u8 order in which the Exif metadata of the image is
...
class EXIV2API ImageFactory {
 ...
 public:
   ...
   */
  static Image::UniquePtr open(const u8* data, size_t size);
  /*!
    @brief Create an Image subclass of the appropriate type by reading
  ...
  /*!
    ...
    @return %Image type or Image::none if the type is not recognized.
   */
  static ImageType getType(const u8* data, size_t size);
  /*!
...
EXIV2API void append(Exiv2::Blob& blob, const u8* buf, size_t len);
