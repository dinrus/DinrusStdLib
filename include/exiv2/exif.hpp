...
class EXIV2API Exifdatum : public Metadatum {
 ...
 public:
   ...
   */
  int setDataArea(const u8* buf, size_t len) const;
  //@}
 ...
 public:
  ...
  /*!
    ...
    @note  The JPEG image inserted as thumbnail image should not
           ...
           application that comes with OS X for one.) - David Harvey.
   */
  void setJpegThumbnail(const u8* buf, size_t size, URational xres, URational yres, uint16_t unit);
#ifdef EXV_ENABLE_FILESYSTEM
  /*!
  ...
  /*!
    ...
    @note  Additional existing Exif thumbnail tags are not modified.
   */
  void setJpegThumbnail(const u8* buf, size_t size);
  /*!
...
class EXIV2API ExifParser {
 public:
  /*!
    @brief Decode metadata from a buffer \em pData of length \em size
           ...
           The buffer must start with a TIFF header. Return u8 order
  ...
  */
  static ByteOrder decode(ExifData& exifData, const u8* pData, size_t size);
  /*!
  ...
  */
  static WriteMethod encode(Blob& blob, const u8* pData, size_t size, ByteOrder byteOrder, ExifData& exifData);
  /*!
