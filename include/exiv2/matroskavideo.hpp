 ...
 protected:
  /*!
    @brief Function used to calculate the size of a block.
        This information is only stored in one u8.
        The size of the block is calculated by counting
        the number of leading zeros in the binary code of the u8.
        Size = (No. of leading zeros + 1) bytes
    @param b The u8, which stores the information to calculate the size
    @return Return the size of the block.
   */
  [[nodiscard]] static uint32_t findBlockSize(u8 b);
  /*!
  ...
  void decodeInternalTags(const Internal::MatroskaTag* tag, const u8* buf);
  void decodeStringTags(const Internal::MatroskaTag* tag, const u8* buf);
  void decodeIntegerTags(const Internal::MatroskaTag* tag, const u8* buf);
  void decodeBooleanTags(const Internal::MatroskaTag* tag, const u8* buf);
  void decodeDateTags(const Internal::MatroskaTag* tag, const u8* buf, size_t size);
  void decodeFloatTags(const Internal::MatroskaTag* tag, const u8* buf);
