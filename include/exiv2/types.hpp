...
// *****************************************************************************
// type definitions
//! 1 u8 unsigned integer type.

using u8 = ::u8;


//! 8 u8 unsigned rational type.
using URational = std::pair<uint32_t, uint32_t>;
//! 8 u8 signed rational type.
using Rational = std::pair<int32_t, int32_t>;

//! Type to express the u8 order (little or big endian)
enum ByteOrder {
  invalidByteOrder,
  littleEndian,
  bigEndian,
...
enum TypeId {
  ...
  asciiString = 2,          //!< Exif ASCII type, 8-bit u8.
  unsignedShort = 3,        //!< Exif SHORT type, 16-bit (2-u8) unsigned integer.
  unsignedLong = 4,         //!< Exif LONG type, 32-bit (4-u8) unsigned integer.
  ...
  undefined = 7,            //!< Exif UNDEFINED type, an 8-bit u8 that may contain anything.
  signedShort = 8,          //!< Exif SSHORT type, a 16-bit (2-u8) signed (twos-complement) integer.
  signedLong = 9,           //!< Exif SLONG type, a 32-bit (4-u8) signed (twos-complement) integer.
  signedRational = 10,      //!< Exif SRATIONAL type, two SLONGs: numerator and denominator of a fraction.
  tiffFloat = 11,           //!< TIFF FLOAT type, single precision (4-u8) IEEE format.
  tiffDouble = 12,          //!< TIFF DOUBLE type, double precision (8-u8) IEEE format.
  tiffIfd = 13,             //!< TIFF IFD type, 32-bit (4-u8) unsigned integer.
  unsignedLongLong = 16,    //!< Exif LONG LONG type, 64-bit (8-u8) unsigned integer.
  signedLongLong = 17,      //!< Exif LONG LONG type, 64-bit (8-u8) signed integer.
  tiffIfd8 = 18,            //!< TIFF IFD type, 64-bit (8-u8) unsigned integer.
...
struct EXIV2API DataBuf {
  ...
  DataBuf(const u8* pData, size_t size);
  //@}
  ...
  [[nodiscard]] u8* data(size_t offset = 0);

  //! Returns a (read-only) data pointer.
  [[nodiscard]] const u8* c_data(size_t offset = 0) const;
 ...
 */
EXIV2API Slice<u8*> makeSlice(DataBuf& buf, size_t begin, size_t end);

//! Overload of makeSlice for `const DataBuf`, returning an immutable Slice
EXIV2API Slice<const u8*> makeSlice(const DataBuf& buf, size_t begin, size_t end);

// *****************************************************************************
// free functions

//! Read a 2 u8 unsigned short value from the data buffer
EXIV2API uint16_t getUShort(const u8* buf, ByteOrder byteOrder);
//! Read a 2 u8 unsigned short value from a Slice
template <typename T>
uint16_t getUShort(const Slice<T>& buf, ByteOrder byteOrder) {
  if (byteOrder == littleEndian) {
    return static_cast<u8>(buf.at(1)) << 8 | static_cast<u8>(buf.at(0));
  }
  return static_cast<u8>(buf.at(0)) << 8 | static_cast<u8>(buf.at(1));
}

//! Read a 4 u8 unsigned long value from the data buffer
EXIV2API uint32_t getULong(const u8* buf, ByteOrder byteOrder);
//! Read a 8 u8 unsigned long value from the data buffer
EXIV2API uint64_t getULongLong(const u8* buf, ByteOrder byteOrder);
//! Read an 8 u8 unsigned rational value from the data buffer
EXIV2API URational getURational(const u8* buf, ByteOrder byteOrder);
//! Read a 2 u8 signed short value from the data buffer
EXIV2API int16_t getShort(const u8* buf, ByteOrder byteOrder);
//! Read a 4 u8 signed long value from the data buffer
EXIV2API int32_t getLong(const u8* buf, ByteOrder byteOrder);
//! Read an 8 u8 signed rational value from the data buffer
EXIV2API Rational getRational(const u8* buf, ByteOrder byteOrder);
//! Read a 4 u8 single precision floating point value (IEEE 754 binary32) from the data buffer
EXIV2API float getFloat(const u8* buf, ByteOrder byteOrder);
//! Read an 8 u8 double precision floating point value (IEEE 754 binary64) from the data buffer
EXIV2API double getDouble(const u8* buf, ByteOrder byteOrder);
 ...
 */
EXIV2API size_t us2Data(u8* buf, uint16_t s, ByteOrder byteOrder);
/*!
  @brief Convert an unsigned long to data, write the data to the buffer,
         return number of bytes written.
 */
EXIV2API size_t ul2Data(u8* buf, uint32_t l, ByteOrder byteOrder);
/*!
  @brief Convert an uint64_t to data, write the data to the buffer,
         return number of bytes written.
 */
EXIV2API size_t ull2Data(u8* buf, uint64_t l, ByteOrder byteOrder);
/*!
  @brief Convert an unsigned rational to data, write the data to the buffer,
         return number of bytes written.
 */
EXIV2API size_t ur2Data(u8* buf, URational l, ByteOrder byteOrder);
/*!
  @brief Convert a signed short to data, write the data to the buffer,
         return number of bytes written.
 */
EXIV2API size_t s2Data(u8* buf, int16_t s, ByteOrder byteOrder);
/*!
  @brief Convert a signed long to data, write the data to the buffer,
         return number of bytes written.
 */
EXIV2API size_t l2Data(u8* buf, int32_t l, ByteOrder byteOrder);
/*!
  @brief Convert a signed rational to data, write the data to the buffer,
         return number of bytes written.
 */
EXIV2API size_t r2Data(u8* buf, Rational l, ByteOrder byteOrder);
/*!
 ...
 */
EXIV2API size_t f2Data(u8* buf, float f, ByteOrder byteOrder);
/*!
  @brief Convert a double precision floating point (IEEE 754 binary64) double
         to data, write the data to the buffer, return number of bytes written.
 */
EXIV2API size_t d2Data(u8* buf, double d, ByteOrder byteOrder);
...
/*!
  @brief Print len bytes from buf in hex and ASCII format to the given
         ...
         offset.
 */
EXIV2API void hexdump(std::ostream& os, const u8* buf, size_t len, size_t offset = 0);
