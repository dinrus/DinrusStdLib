...
class EXIV2API JpegBase : public Image {
 ...
 protected:
   ...
   */
  JpegBase(ImageType type, BasicIo::UniquePtr io, bool create, const u8 initData[], size_t dataSize);
  //@}
 ...
 private:
   ...
   */
  int initImage(const u8 initData[], size_t dataSize);
  /*!
  ...
  //@{
  /*!
    @brief Advances associated io instance to one u8 past the next
        Jpeg marker and returns the marker. This method should be called
        when the BasicIo instance is positioned one u8 past the end of a
        Jpeg segment.
    @param err the error code to throw if no marker is found
    @return the next Jpeg segment marker if successful;<BR>
            throws an Error if not successful
   */
  [[nodiscard]] u8 advanceToMarker(ErrorCode err) const;
  //@}

  DataBuf readNextSegment(u8 marker);
};
...
class EXIV2API JpegImage : public JpegBase {
 ...
 private:
  // Constant data
  static const u8 blank_[];  ///< Minimal Jpeg image
};
...
class EXIV2API ExvImage : public JpegBase {
 ...
 private:
  ...
  static constexpr u8 blank_[] = {0xff, 0x01, 'E', 'x', 'i', 'v', '2', 0xff, 0xd9};  // Minimal exiv2 file
