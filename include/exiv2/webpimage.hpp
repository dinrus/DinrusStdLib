...
class EXIV2API WebPImage : public Image {
 ...
 private:
  ...
  static size_t getHeaderOffset(const u8* data, size_t data_size, const u8* header, size_t header_size);

  static bool equalsWebPTag(const Exiv2::DataBuf& buf, const char* str);
  void debugPrintHex(u8* data, size_t size);
  ...
  /* Misc. */
  static constexpr u8 WEBP_PAD_ODD = 0;
