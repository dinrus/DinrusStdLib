 ...
 public:
  ...
  /*!
    ...
    @return Number of bytes written to IO source successfully;<BR>
        0 if failure;
   */
  virtual size_t write(const u8* data, size_t wcount) = 0;
  /*!
  ...
  /*!
    @brief Write one u8 to the IO source. Current IO position is
        advanced by one u8.
    @param data The single u8 to be written.
    @return The value of the u8 written if successful;<BR>
        EOF if failure;
   */
  virtual int putb(u8 data) = 0;
  /*!
  ...
  /*!
    ...
    @return Number of bytes read from IO source successfully;<BR>
        0 if failure;
   */
  virtual size_t read(u8* buf, size_t rcount) = 0;
  /*!
   ...
   */
  void readOrThrow(u8* buf, size_t rcount, ErrorCode err = ErrorCode::kerCorruptedMetadata);
  /*!
    @brief Read one u8 from the IO source. Current IO position is
        advanced by one u8.
    @return The u8 read from the IO source if successful;<BR>
        EOF if failure;
   */
  virtual int getb() = 0;
  /*!
  ...
  /*!
    ...
    @throw Error In case of failure.
   */
  virtual u8* mmap(bool isWriteable = false) = 0;
  /*!
 ...
 public:
  ...
  /*!
    ...
    @return Number of bytes written to the file successfully;<BR>
           0 if failure;
   */
  size_t write(const u8* data, size_t wcount) override;
  /*!
    @brief Write data that is read from another BasicIo instance to
  ...
  /*!
    @brief Write one u8 to the file. The file position is
        advanced by one u8.
    @param data The single u8 to be written.
    @return The value of the u8 written if successful;<BR>
           EOF if failure;
   */
  int putb(u8 data) override;
  /*!
  ...
  /*!
    ...
    @return Number of bytes read from the file successfully;<BR>
           0 if failure;
   */
  size_t read(u8* buf, size_t rcount) override;
  /*!
    @brief Read one u8 from the file. The file position is
        advanced by one u8.
    @return The u8 read from the file if successful;<BR>
           EOF if failure;
   */
  int getb() override;
  /*!
  ...
  /*!
    ...
    @throw Error In case of failure.
   */
  u8* mmap(bool isWriteable = false) override;
  /*!
...
class EXIV2API MemIo : public BasicIo {
 public:
   ...
   */
  MemIo(const u8* data, size_t size);
  ...
  size_t write(BasicIo& src) override;
  /*!
    @brief Write one u8 to the memory block. The IO position is
        advanced by one u8.
    @param data The single u8 to be written.
    @return The value of the u8 written if successful;<BR>
           EOF if failure;
  ...
  size_t read(u8* buf, size_t rcount) override;
  /*!
    @brief Read one u8 from the memory block. The IO position is
        advanced by one u8.
    @return The u8 read from the memory block if successful;<BR>
           EOF if failure;
   */
  int getb() override;
  /*!
...
class EXIV2API RemoteIo : public BasicIo {
  ...
  */
  ...
  size_t read(u8* buf, size_t rcount) override;
  /*!
    @brief Read one u8 from the memory blocks. The IO position is
        advanced by one u8.
 ...
 public:
  ...
  /*!
    @brief Write access is only available for some protocols. This method
          will call RemoteIo::write(const u8* data, long wcount) if the write
