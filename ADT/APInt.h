...
#include <drx/llvm/Support/MathExtras.h>
#include <cassert>
#include <climits>
#include <cstring>
#include <optional>
...
public:
  ...
  /// constructor.
  APInt(unsigned numBits, unsigned numWords, u64k bigVal[]);
  ...
  APInt(unsigned numBits, StringRef str, u8 radix);
  ...
  bool isMask(unsigned numBits) const {
    ...
    assert(numBits <= BitWidth && "numBits вне диапазона");
    if (isSingleWord())
  ...
  APInt relativeLShr(i32 RelativeShift) const {
    return RelativeShift > 0 ? lshr(RelativeShift) : shl(-RelativeShift);
  }

  /// relative logical shift left
  APInt relativeLShl(i32 RelativeShift) const {
    return relativeLShr(-RelativeShift);
  }

  /// relative arithmetic shift right
  APInt relativeAShr(i32 RelativeShift) const {
    return RelativeShift > 0 ? ashr(RelativeShift) : shl(-RelativeShift);
  }

  /// relative arithmetic shift left
  APInt relativeAShl(i32 RelativeShift) const {
  ...
  void setBit(unsigned BitPosition) {
    assert(BitPosition < BitWidth && "BitPosition вне диапазона");
  ...
  void setBitsWithWrap(unsigned loBit, unsigned hiBit) {
    assert(hiBit <= BitWidth && "hiBit вне диапазона");
    assert(loBit <= BitWidth && "loBit вне диапазона");
    if (loBit < hiBit) {
      setBits(loBit, hiBit);
      return;
    }
  ...
  void setBits(unsigned loBit, unsigned hiBit) {
    ...
    if (loBit < APINT_BITS_PER_WORD && hiBit <= APINT_BITS_PER_WORD) {
      u64 mask = WORDTYPE_MAX >> (APINT_BITS_PER_WORD - (hiBit - loBit));
      mask <<= loBit;
      if (isSingleWord())
        U.VAL |= mask;
      else
  ...
  static unsigned getNumWords(unsigned BitWidth) {
    return ((u64)BitWidth + APINT_BITS_PER_WORD - 1) / APINT_BITS_PER_WORD;
  }
  ...
  u64 getZExtValue() const {
    if (isSingleWord())
      return U.VAL;
    assert(getActiveBits() <= 64 && "Too many bits for u64");
    return U.pVal[0];
  }
  ...
  std::optional<u64> tryZExtValue() const {
    return (getActiveBits() <= 64) ? std::optional<u64>(getZExtValue())
                                   : std::nullopt;
  };
  ...
  static unsigned getBitsNeeded(StringRef str, u8 radix);
  ...
  static unsigned getSufficientBitsNeeded(StringRef Str, u8 Radix);
  ...
  float bitsToFloat() const {
    return BitsToFloat(static_cast<u32>(getWord(0)));
  }
  ...
  static i32 tcExtractBit(const WordType *, unsigned bit);
  ...
  static i32 tcMultiplyPart(WordType *dst, const WordType *src,
  ...
  /// operands.
  static i32 tcMultiply(WordType *, const WordType *, const WordType *,
                        unsigned);
  ...
  static i32 tcDivide(WordType *lhs, const WordType *rhs, WordType *remainder,
  ...
  static i32 tcCompare(const WordType *, const WordType *, unsigned);
...
private:
  ...
  APInt &clearUnusedBits() {
    ...
    u64 mask = WORDTYPE_MAX >> (APINT_BITS_PER_WORD - WordBits);
    if (LLVM_UNLIKELY(BitWidth == 0))
      mask = 0;
  ...
  void fromString(unsigned numBits, StringRef str, u8 radix);
  ...
  i32 compare(const APInt &RHS) const LLVM_READONLY;

  /// Signed comparison. Returns -1, 0, or 1 if this APInt is less than, equal
  /// to, or greater than RHS.
  i32 compareSigned(const APInt &RHS) const LLVM_READONLY;
...
void LoadIntFromMemory(APInt &IntVal, u8k *Src, unsigned LoadBytes);
