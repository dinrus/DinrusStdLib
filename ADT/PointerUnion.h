...
#include <drx/llvm/Support/Casting.h>
#include <drx/llvm/Support/PointerLikeTypeTraits.h>
#include <algorithm>
#include <cassert>
#include <cstddef>
#include <cstdint>
...
namespace pointer_union_detail {
  ...
  /// This is ceil(log2(n)).
  constexpr i32 bitsRequired(unsigned n) {
    return n > 1 ? 1 + bitsRequired((n + 1) / 2) : 0;
  }

  template <typename... Ts> constexpr i32 lowBitsAvailable() {
    return std::min<i32>({PointerLikeTypeTraits<Ts>::NumLowBitsAvailable...});
  }
  ...
  public:
    static inline uk getAsVoidPointer(uk P) { return P; }
    static inline uk getFromVoidPointer(uk P) { return P; }
    static constexpr i32 NumLowBitsAvailable = lowBitsAvailable<PTs...>();
  };

  template <typename Derived, typename ValTy, i32 I, typename ...Types>
  class PointerUnionMembers;

  template <typename Derived, typename ValTy, i32 I>
  ...
  template <typename Derived, typename ValTy, i32 I, typename Type,
            typename ...Types>
  ...
  public:
    ...
    Derived &operator=(Type V) {
      this->Val = ValTy(
          const_cast<uk >(PointerLikeTypeTraits<Type>::getAsVoidPointer(V)),
          I);
...
///    PointerUnion<i32*, float*> P;
///    P = (i32*)0;
///    printf("%d %d", P.is<i32*>(), P.is<float*>());  // prints "1 0"
///    X = P.get<i32*>();     // ok.
///    Y = P.get<float*>();   // runtime assertion failure.
///    Z = P.get<double*>();  // compile time failure.
///    P = (float*)0;
///    Y = P.get<float*>();   // ok.
///    X = P.get<i32*>();     // runtime assertion failure.
///    PointerUnion<i32*, i32*> Q; // compile time failure.
template <typename... PTs>
class PointerUnion
    : public pointer_union_detail::PointerUnionMembers<
          PointerUnion<PTs...>,
          PointerIntPair<
              uk , pointer_union_detail::bitsRequired(sizeof...(PTs)), i32,
...
public:
  ...
  uk getOpaqueValue() const { return this->Val.getOpaqueValue(); }
  static inline PointerUnion getFromOpaqueValue(uk VP) {
    PointerUnion V;
...
struct PointerLikeTypeTraits<PointerUnion<PTs...>> {
  static inline uk getAsVoidPointer(const PointerUnion<PTs...> &P) {
    return P.getOpaqueValue();
  }

  static inline PointerUnion<PTs...> getFromVoidPointer(uk P) {
    return PointerUnion<PTs...>::getFromOpaqueValue(P);
  }

  // The number of bits available are the min of the pointer types minus the
  // bits needed for the discriminator.
  static constexpr i32 NumLowBitsAvailable = PointerLikeTypeTraits<decltype(
