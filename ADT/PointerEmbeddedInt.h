...
template <typename IntT, i32 Bits = sizeof(IntT) * CHAR_BIT>
...
// types.
template <typename IntT, i32 Bits>
struct PointerLikeTypeTraits<PointerEmbeddedInt<IntT, Bits>> {
  using T = PointerEmbeddedInt<IntT, Bits>;

  static inline uk getAsVoidPointer(const T &P) {
    return reinterpret_cast<uk >(P.Value);
  }

  static inline T getFromVoidPointer(uk P) {
    return T(reinterpret_cast<uintptr_t>(P), typename T::RawValueTag());
  }

  static inline T getFromVoidPointer(ukk P) {
    return T(reinterpret_cast<uintptr_t>(P), typename T::RawValueTag());
  }

  static constexpr i32 NumLowBitsAvailable = T::Shift;
};
