...
protected:
  ...
  SmallVectorBase(uk FirstEl, size_t TotalCapacity)
      : BeginX(FirstEl), Capacity(TotalCapacity) {}

  /// This is a helper for \a grow() that's out of line to reduce code
  /// duplication.  This function will report a fatal error if it can't grow at
  /// least to \p MinSize.
  uk mallocForGrow(uk FirstEl, size_t MinSize, size_t TSize,
                      size_t &NewCapacity);

  /// This is an implementation of the grow() method which only works
  /// on POD-like data types and is out of line to reduce code duplication.
  /// This function will report a fatal error if it cannot increase capacity.
  void grow_pod(uk FirstEl, size_t MinSize, size_t TSize);
  ...
  uk replaceAllocation(uk NewElts, size_t TSize, size_t NewCapacity,
                          size_t VSize = 0);
...
using SmallVectorSizeType =
    std::conditional_t<sizeof(T) < 4 && sizeof(uk ) >= 8, u64,
                       u32>;
...
protected:
  ...
  uk getFirstEl() const {
    return const_cast<uk >(reinterpret_cast<ukk >(
        reinterpret_cast<tukk >(this) +
  ...
  bool isReferenceToRange(ukk V, ukk First, ukk Last) const {
  ...
  bool isReferenceToStorage(ukk V) const {
    return isReferenceToRange(V, this->begin(), this->end());
  }

  /// Return true if First and Last form a valid (possibly empty) range in this
  /// vector's storage.
  bool isRangeInStorage(ukk First, ukk Last) const {
    // Use std::less to avoid UB.
    std::less<> LessThan;
  ...
  /// NewSize.
  bool isSafeToReferenceAfterResize(ukk Elt, size_t NewSize) {
    // Past the end.
  ...
  void assertSafeToReferenceAfterResize(ukk Elt, size_t NewSize) {
  ...
  /// vector by N.
  void assertSafeToAdd(ukk Elt, size_t N = 1) {
...
protected:
  ...
  template <typename... ArgTypes> T &growAndEmplaceBack(ArgTypes &&... Args) {
    ...
    ::new ((uk )(NewElts + this->size())) T(std::forward<ArgTypes>(Args)...);
...
public:
  void push_back(const T &Elt) {
    ...
    ::new ((uk )this->end()) T(*EltPtr);
    this->set_size(this->size() + 1);
  }

  void push_back(T &&Elt) {
    T *EltPtr = reserveForParamAndGetAddress(Elt);
    ::new ((uk )this->end()) T(::std::move(*EltPtr));
    this->set_size(this->size() + 1);
  }
...
protected:
  ...
  static constexpr bool TakesParamByValue = sizeof(T) <= 2 * sizeof(uk );
  ...
  static void uninitialized_copy(
    ...
    if (I != E)
      memcpy(reinterpret_cast<uk >(Dest), I, (E - I) * sizeof(T));
  }
...
public:
  void push_back(ValueParamT Elt) {
    const T *EltPtr = reserveForParamAndGetAddress(Elt);
    memcpy(reinterpret_cast<uk >(this->end()), EltPtr, sizeof(T));
    this->set_size(this->size() + 1);
  }
...
public:
  ...
  template <typename... ArgTypes> reference emplace_back(ArgTypes &&... Args) {
    ...
    ::new ((uk )this->end()) T(std::forward<ArgTypes>(Args)...);
    this->set_size(this->size() + 1);
    return this->back();
  }
...
extern template class llvm::SmallVectorBase<u32>;
#if SIZE_MAX > UINT32_MAX
extern template class llvm::SmallVectorBase<u64>;
#endif
