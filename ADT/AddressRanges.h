...
public:
  AddressRange() {}
  AddressRange(u64 S, u64 E) : Start(S), End(E) {
    assert(Start <= End);
  }
  u64 start() const { return Start; }
  u64 end() const { return End; }
  u64 size() const { return End - Start; }
  u64 empty() const { return size() == 0; }
  bool contains(u64 Addr) const { return Start <= Addr && Addr < End; }
...
public:
  ...
  std::optional<T> getRangeThatContains(u64 Addr) const {
...
protected:
  typename Collection::const_iterator find(u64 Start, u64 End) const {
    if (Start >= End)
      return Ranges.end();
