...
#include <drx/ADT/GenericUniformityInfo.h>

#include <drx/ADT/SmallPtrSet.h>
#include <drx/ADT/SparseBitVector.h>
#include <drx/ADT/StringExtras.h>
...
public:
  ...
  void printDefs(raw_ostream &Out) {
    ...
    for (i32 BlockIdx = (i32)CyclePOT.size() - 1; BlockIdx >= 0; --BlockIdx) {
  ...
  std::unique_ptr<DivergenceDescriptorT> computeJoinPoints() {
    ...
    i32 FloorIdx = CyclePOT.size() - 1;
    const BlockT *FloorLabel = nullptr;
    i32 DivTermIdx = CyclePOT.getIndex(&DivTermBlock);
    ...
    for (const auto *SuccBlock : successors(&DivTermBlock)) {
      ...
      FloorIdx = std::min<i32>(FloorIdx, SuccIdx);
    }
    ...
    while (true) {
      ...
      if (const auto *BlockCycle = getReducibleParent(Block)) {
        ...
        for (auto *BlockCycleExit : BlockCycleExits) {
          ...
          LoweredFloorIdx =
              std::min<i32>(LoweredFloorIdx, CyclePOT.getIndex(BlockCycleExit));
        }
      } else {
        for (const auto *SuccBlock : successors(Block)) {
          CausedJoin |= visitEdge(*SuccBlock, *Label);
          LoweredFloorIdx =
              std::min<i32>(LoweredFloorIdx, CyclePOT.getIndex(SuccBlock));
        }
      }
