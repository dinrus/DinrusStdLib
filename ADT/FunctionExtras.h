...
#include <drx/ADT/PointerUnion.h>
#include <drx/ADT/STLForwardCompat.h>
#include <drx/llvm/Support/MemAlloc.h>
...
/// - unique_function<i32() const> has a const operator().
///   It can only hold functions which themselves have a const operator().
/// - unique_function<i32()> has a non-const operator().
...
protected:
  static constexpr size_t InlineStorageSize = sizeof(uk ) * 3;
  ...
  struct IsSizeLessThanThresholdT<
      T, std::enable_if_t<sizeof(T) <= 2 * sizeof(uk )>> : std::true_type {};
  ...
  using CallPtrT = ReturnT (*)(uk CallableAddr,
                               AdjustedParamT<ParamTs>... Params);
  using MovePtrT = void (*)(uk LHSCallableAddr, uk RHSCallableAddr);
  using DestroyPtrT = void (*)(uk CallableAddr);
  ...
  union StorageUnionT {
    ...
    mutable std::aligned_storage_t<InlineStorageSize, alignof(uk )>
        InlineStorage;
  } StorageUnion;
  ...
  uk getCalleePtr() const {
    return isInlineStorage() ? getInlineStorage() : getOutOfLineStorage();
  }
  uk getInlineStorage() const { return &StorageUnion.InlineStorage; }
  uk getOutOfLineStorage() const {
  ...
  void setOutOfLineStorage(uk Ptr, size_t Size, size_t Alignment) {
    StorageUnion.OutOfLineStorage = {Ptr, Size, Alignment};
  }

  template <typename CalledAsT>
  static ReturnT CallImpl(uk CallableAddr,
                          AdjustedParamT<ParamTs>... Params) {
  ...
  static void MoveImpl(uk LHSCallableAddr, uk RHSCallableAddr) noexcept {
    new (LHSCallableAddr)
        CallableT(std::move(*reinterpret_cast<CallableT *>(RHSCallableAddr)));
  }

  template <typename CallableT>
  static void DestroyImpl(uk CallableAddr) noexcept {
  ...
  UniqueFunctionBase(CallableT Callable, CalledAs<CalledAsT>) {
    ...
    uk CallableAddr = getInlineStorage();
