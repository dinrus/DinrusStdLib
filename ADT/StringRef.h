...
#include <drx/ADT/DenseMapInfo.h>
#include <drx/ADT/STLFunctionalExtras.h>
#include <drx/ADT/iterator_range.h>
...
namespace llvm {
  ...
  bool getAsSignedInteger(StringRef Str, unsigned Radix, z64 &Result);

  bool consumeUnsignedInteger(StringRef &Str, unsigned Radix,
                              unsigned z64 &Result);
  bool consumeSignedInteger(StringRef &Str, unsigned Radix, z64 &Result);

  /// StringRef - Представляет константную ссылку на текст, т.е. массив
  /// символов и длину, которая не требует нулевого окончания.
  ///
  /// Этот класс не владеет текстовыми данными, он предназначен для использования
  /// в тех ситуациях, когда данные находятся в другом буфере, лайфтайм у которого
  /// больше, чем у этого StringRef. По этой причине в целом небезопасно
  /// хранить в памяти StringRef.
  class LLVM_GSL_POINTER StringRef {
  public:
    static constexpr size_t npos = ~size_t(0);

    using iterator = tukk;
    using const_iterator = tukk;
    using size_type = size_t;

  private:
    /// Начало текста, во внешнем буфере.
    tukk Data = nullptr;
    ...
    static i32 compareMemory(tukk Lhs, tukk Rhs, size_t Length) {
  ...
  public:
    ...
    /*implicit*/ constexpr StringRef(tukk Str)
        : Data(Str), Length(Str ?
    ...
    /*implicit*/ constexpr StringRef(tukk data, size_t length)
    ...
    u8k* bytes_begin() const {
      return reinterpret_cast<u8k* >(begin());
    }
    u8k* bytes_end() const {
      return reinterpret_cast<u8k* >(end());
    }
    iterator_range<u8k*> bytes() const {
    ...
    /// terminated).
    [[nodiscard]] tukk data() const { return Data; }
    ...
    [[nodiscard]] StringRef copy(Allocator &A) const {
      ...
      if (empty())
        return StringRef();
      tuk S = A.template Allocate<char>(Length);
    ...
    /// the \p RHS.
    [[nodiscard]] i32 compare(StringRef RHS) const {
      // Check the prefix for a mismatch.
      if (i32 Res = compareMemory(Data, RHS.Data, std::min(Length, RHS.Length)))
        return Res < 0 ? -1 : 1;
    ...
    [[nodiscard]] i32 compare_insensitive(StringRef RHS) const;

    /// compare_numeric - Compare two strings, treating sequences of digits as
    /// numbers.
    [[nodiscard]] i32 compare_numeric(StringRef RHS) const;
    ...
    template <typename T> bool getAsInteger(unsigned Radix, T &Result) const {
      if constexpr (std::numeric_limits<T>::is_signed) {
        ...
        unsigned z64 ULLVal;
        // The additional cast to unsigned z64 is required to avoid the
        // Visual C++ warning C4805: '!=' : unsafe mix of type 'bool' and type
        // 'unsigned __int64' when instantiating getAsInteger with T = bool.
        if (getAsUnsignedInteger(*this, Radix, ULLVal) ||
            static_cast<unsigned z64>(static_cast<T>(ULLVal)) != ULLVal)
          return true;
        Result = ULLVal;
      }
      return false;
    ...
    template <typename T> bool consumeInteger(unsigned Radix, T &Result) {
      if constexpr (std::numeric_limits<T>::is_signed) {
        ...
        if (consumeSignedInteger(*this, Radix, LLVal) ||
            static_cast<z64>(static_cast<T>(LLVal)) != LLVal)
          return true;
        Result = LLVal;
      } else {
        unsigned z64 ULLVal;
    ...
    void split(SmallVectorImpl<StringRef> &A,
               StringRef Separator, i32 MaxSplit = -1,
    ...
    /// \param MaxSplit - The maximum number of times the string is split.
    /// \param KeepEmpty - True if empty substring should be added.
    void split(SmallVectorImpl<StringRef> &A, char Separator, i32 MaxSplit = -1,
               bool KeepEmpty = true) const;
  ...
  private:
    constexpr StringLiteral(tukk Str, size_t N) : StringRef(Str, N) {
    }
...
#pragma clang diagnostic ignored "-Wgcc-compat"
        __attribute((enable_if(__builtin_strlen(Str) == N - 1,
                               "неполноценный строковый литерал")))
#pragma clang diagnostic pop
  ...
  template <> struct DenseMapInfo<StringRef, void> {
    static inline StringRef getEmptyKey() {
      return StringRef(
          reinterpret_cast<tukk >(~static_cast<uintptr_t>(0)), 0);
    }

    static inline StringRef getTombstoneKey() {
      return StringRef(
          reinterpret_cast<tukk >(~static_cast<uintptr_t>(1)), 0);
    }
