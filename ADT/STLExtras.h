...
#include <drx/ADT/STLForwardCompat.h>
#include <drx/ADT/STLFunctionalExtras.h>
#include <drx/ADT/identity.h>
#include <drx/ADT/iterator.h>
#include <drx/ADT/iterator_range.h>
#include <drx/llvm/Config/abi-breaking.h>
...
class Callable {
  ...
  static_assert(!std::is_pointer_v<value_type>,
                "Указатели на не-функции невызываемы.");
...
public:
  ...
  template <typename... Pn,
            std::enable_if_t<std::is_invocable_v<T, Pn...>, i32> = 0>
  decltype(auto) operator()(Pn &&...Params) {
    return (*Obj)(std::forward<Pn>(Params)...);
  }

  template <typename... Pn,
            std::enable_if_t<std::is_invocable_v<T const, Pn...>, i32> = 0>
...
public:
  ...
  template < // clang-format off
    typename FnPtrOrRef,
    std::enable_if_t<
      !std::is_same_v<remove_cvref_t<FnPtrOrRef>, Callable>, i32
    > = 0
  > // clang-format on
...
/// \code
///   i32 A[] = { 1, 2, 3, 4 };
///   auto R = make_filter_range(A, [](i32 N) { return N % 2 == 1; });
///   // R contains { 1, 3 }.
/// \endcode
///
...
public:
  ...
  decltype(auto) operator*() const {
    assert(BaseT::wrapped() != End && "Нельзя рассыливать конечный итератор!");
...
#if LLVM_ENABLE_ABI_BREAKING_CHECKS
    assert(!IsEarlyIncremented && "Нельзя рассыливать дважды!");
...
#if LLVM_ENABLE_ABI_BREAKING_CHECKS
    assert(IsEarlyIncremented && "Нельзя инкрементировать перед рассыливанием!");
...
#if LLVM_ENABLE_ABI_BREAKING_CHECKS
    assert(!LHS.IsEarlyIncremented && "Нельзя сравнивать после рассыливания!");
#endif
...
public:
  ...
  ZipType &operator--() {
    static_assert(Base::IsBidirectional,
                  "Все внутренние итераторы должны быть хотя бы двунаправленными.");
...
detail::zippy<detail::zip_first, T, U, Args...> zip_equal(T &&t, U &&u,
  ...
  assert(all_equal({std::distance(adl_begin(t), adl_end(t)),
         ...
         "Итерируемые не одинаковой длины");
...
detail::zippy<detail::zip_first, T, U, Args...> zip_first(T &&t, U &&u,
  ...
  assert(std::distance(adl_begin(t), adl_end(t)) <=
             std::min({std::distance(adl_begin(u), adl_end(u)),
                       std::distance(adl_begin(args), adl_end(args))...}) &&
         "Первый итерируемый не является самым короткимt");
...
class concat_iterator
  ...
  template <size_t... Ns> void increment(std::index_sequence<Ns...>) {
    ...
    llvm_unreachable("Попытка инкрементировать конечный concat-итератор!");
  }
  ...
  template <size_t... Ns> ValueT &get(std::index_sequence<Ns...>) const {
    ...
    llvm_unreachable("Попытка получить указатель от конечного concat-итератора!");
  }
...
detail::concat_range<ValueT, RangeTs...> concat(RangeTs &&... Ranges) {
  static_assert(sizeof...(RangeTs) > 1,
                "Нужно конкатенировать несколько диапазонов!");
...
public:
  ptrdiff_t operator-(const indexed_accessor_iterator &rhs) const {
    assert(base == rhs.base && "несовместимые итераторы");
...
public:
  ...
  ReferenceT operator[](size_t Index) const {
    assert(Index < size() && "неполноценный индекс у диапазона значений");
    return DerivedT::dereference_iterator(base, static_cast<ptrdiff_t>(Index));
  }
  ReferenceT front() const {
    assert(!empty() && "ожидается непустой диапазон");
    return (*this)[0];
  }
  ...
  DerivedT slice(size_t n, size_t m) const {
    assert(n + m <= size() && "неполноценные указатели размера");
    return DerivedT(offset_base(base, n), m);
  }

  /// Drop the first n elements.
  DerivedT drop_front(size_t n = 1) const {
    assert(size() >= n && "Роняется большее число элементов, чем существует");
...
/// overload candidates.
template <i32 N> struct rank : rank<N - 1> {};
...
/// auto a = visitor(42);    // `a` is now "i32".
...
template<typename T>
inline i32 array_pod_sort_comparator(ukk P1, ukk P2) {
...
template<typename T>
inline i32 (*get_array_pod_sort_comparator(const T &))
...
inline void array_pod_sort(
  ...
  qsort(&*Start, NElts, sizeof(*Start),
        reinterpret_cast<i32 (*)(ukk , ukk )>(Compare));
}
  ...
  for (auto *A : Range) {
    ...
    if (PRC.second) {
      assert(PRC.first == nullptr &&
             "Неконсистентные возвратные значения в find_singleton_nested.");
      return PRC;
    }
    if (PRC.first) {
      if (RC) {
