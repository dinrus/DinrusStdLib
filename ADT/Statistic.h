...
#include <drx/llvm/Config/llvm-config.h>
...
public:
  ...
  constexpr TrackingStatistic(tukk DebugType, tukk Name,
                              tukk Desc)
      : DebugType(DebugType), Name(Name), Desc(Desc), Value(0),
        Initialized(false) {}

  tukk getDebugType() const { return DebugType; }
  tukk getName() const { return Name; }
  tukk getDesc() const { return Desc; }

  u64 getValue() const { return Value.load(std::memory_order_relaxed); }

  // Allow use of this class as the value itself.
  operator u64() const { return getValue(); }

  const TrackingStatistic &operator=(u64 Val) {
  ...
  const TrackingStatistic &operator+=(u64 V) {
    if (V == 0)
      return *this;
  ...
  void updateMax(u64 V) {
    u64 PrevMax = Value.load(std::memory_order_relaxed);
...
class NoopStatistic {
public:
  NoopStatistic(tukk  /*DebugType*/, tukk  /*Name*/,
                tukk  /*Desc*/) {}

  u64 getValue() const { return 0; }

  // Allow use of this class as the value itself.
  operator u64() const { return 0; }

  const NoopStatistic &operator=(u64 Val) { return *this; }

  const NoopStatistic &operator++() { return *this; }

  u64 operator++(i32) { return 0; }

  const NoopStatistic &operator--() { return *this; }

  u64 operator--(i32) { return 0; }

  const NoopStatistic &operator+=(u64k &V) { return *this; }
...
/// completes.
std::vector<std::pair<StringRef, u64>> GetStatistics();
