...
public:
  ...
  void set(typename HelperT::template Lookup<N>::PointerT Pointer) {
    uk V = HelperT::template Lookup<N>::TraitsT::getAsVoidPointer(Pointer);
...
protected:
  uk getVoidPtr() const {
    return reinterpret_cast<uk >(getOpaqueValue() & HelperT::PointerMask);
  }
};
...
struct PointerSumTypeHelper : MemberTs... {
  ...
  enum : u64 {
    PointerMask = static_cast<u64>(-1) << NumTagBits,
    TagMask = ~PointerMask
  };
