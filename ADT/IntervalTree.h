...
private:
  ...
  static void printList(raw_ostream &OS, IntervalReferences &IntervalSet,
    ...
    tukk Format = HexFormat ? "[0x%08x,0x%08x] " : "[%2d,%2d] ";
    if (Size) {
  ...
  void printNode(raw_ostream &OS, unsigned Level, IntervalNode *Node,
                 bool HexFormat = true) {
    tukk Format = HexFormat ? "MP:0x%08x " : "MP:%2d ";
  ...
  IntervalNode *createTree(unsigned &IntervalsSize, i32 PointsBeginIndex,
                           i32 PointsEndIndex, i32 ReferencesBeginIndex,
                           i32 ReferencesSize) {
    ...
    for (i32 Index = ReferencesBeginIndex; Index < ReferencesRightIndex;) {
