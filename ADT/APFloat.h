...
#include <drx/ADT/APInt.h>
#include <drx/ADT/ArrayRef.h>
#include <drx/ADT/FloatingPointMode.h>
#include <drx/llvm/Support/ErrorHandling.h>
#include <memory>
...
#define APFLOAT_DISPATCH_ON_SEMANTICS(METHOD_CALL)                             \
  do {                                                                         \
    ...
    llvm_unreachable("Неожиданная семантика");                                  \
  } while (false)
...
struct APFloatBase {
  ...
  static u32 semanticsPrecision(const fltSemantics &);
  static ExponentType semanticsMinExponent(const fltSemantics &);
  static ExponentType semanticsMaxExponent(const fltSemantics &);
  static u32 semanticsSizeInBits(const fltSemantics &);
  static u32 semanticsIntSizeInBits(const fltSemantics&, bool);
...
public:
  ...
  opStatus convertToInteger(MutableArrayRef<integerPart>, u32, bool,
                            roundingMode, bool *) const;
  opStatus convertFromAPInt(const APInt &, bool, roundingMode);
  opStatus convertFromSignExtendedInteger(const integerPart *, u32,
                                          bool, roundingMode);
  opStatus convertFromZeroExtendedInteger(const integerPart *, u32,
                                          bool, roundingMode);
  ...
  u32 convertToHexString(tuk dst, u32 hexDigits,
  ...
  ///   Inf -> \c IEK_Inf
  ///
  friend i32 ilogb(const IEEEFloat &Arg);

  /// Returns: X * 2^Exp for integral exponents.
  friend IEEEFloat scalbn(IEEEFloat X, i32 Exp, roundingMode);

  friend IEEEFloat frexp(const IEEEFloat &X, i32 &Exp, roundingMode);
...
private:
  ...
  void shiftSignificandLeft(u32);
  lostFraction shiftSignificandRight(u32);
  u32 significandLSB() const;
  u32 significandMSB() const;
  ...
  bool roundAwayFromZero(roundingMode, lostFraction, u32) const;
  opStatus convertToSignExtendedInteger(MutableArrayRef<integerPart>,
                                        u32, bool, roundingMode,
                                        bool *) const;
  opStatus convertFromUnsignedParts(const integerPart *, u32,
                                    roundingMode);
  ...
  tuk convertNormalToHexString(tuk , u32, bool,
                                 roundingMode) const;
  opStatus roundSignificandWithExponent(const integerPart *, u32, i32,
                                        roundingMode);
...
i32 ilogb(const IEEEFloat &Arg);
IEEEFloat scalbn(IEEEFloat X, i32 Exp, IEEEFloat::roundingMode);
IEEEFloat frexp(const IEEEFloat &Val, i32 &Exp, IEEEFloat::roundingMode RM);
...
public:
  ...
  opStatus convertToInteger(MutableArrayRef<integerPart> Input,
                            u32 Width, bool IsSigned, roundingMode RM,
                            bool *IsExact) const;
  opStatus convertFromAPInt(const APInt &Input, bool IsSigned, roundingMode RM);
  opStatus convertFromSignExtendedInteger(const integerPart *Input,
                                          u32 InputSize, bool IsSigned,
                                          roundingMode RM);
  ...
  friend DoubleAPFloat scalbn(const DoubleAPFloat &X, i32 Exp, roundingMode);
  friend DoubleAPFloat frexp(const DoubleAPFloat &X, i32 &Exp, roundingMode);
...
public:
  ...
  /// Returns a float which is bitcasted from an all one value i32.
  ///
  ...
  friend i32 ilogb(const APFloat &Arg) { return ilogb(Arg.getIEEE()); }
  friend APFloat scalbn(APFloat X, i32 Exp, roundingMode RM);
  friend APFloat frexp(const APFloat &X, i32 &Exp, roundingMode RM);
  friend IEEEFloat;
...
inline APFloat scalbn(APFloat X, i32 Exp, APFloat::roundingMode RM) {
...
inline APFloat frexp(const APFloat &X, i32 &Exp, APFloat::roundingMode RM) {
  if (APFloat::usesLayout<detail::IEEEFloat>(X.getSemantics()))
