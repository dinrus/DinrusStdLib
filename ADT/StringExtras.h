...
#include <drx/ADT/APSInt.h>
#include <drx/ADT/ArrayRef.h>
#include <drx/ADT/SmallString.h>
#include <drx/ADT/StringRef.h>
#include <drx/ADT/Twine.h>
#include <cassert>
#include <cstddef>
#include <cstdint>
#include <cstdlib>
...
/// null string.
inline std::vector<StringRef> toStringRefArray(tukk const *Strings) {
...
inline StringRef toStringRef(ArrayRef<u8> Input) {
  return StringRef(reinterpret_cast<tukk >(Input.begin()), Input.size());
}

/// Construct a string ref from an array ref of unsigned chars.
inline ArrayRef<u8> arrayRefFromStringRef(StringRef Input) {
...
inline bool isASCII(char C) { return static_cast<u8>(C) <= 127; }
...
inline std::string utohexstr(u64 X, bool LowerCase = false,
                             unsigned Width = 0) {
  char Buffer[17];
  tuk BufPtr = std::end(Buffer);

  if (X == 0) *--BufPtr = '0';

  for (unsigned i = 0; Width ? (i < Width) : X; ++i) {
    u8 Mod = static_cast<u8>(X) & 15;
...
inline std::string toHex(ArrayRef<u8> Input, bool LowerCase = false) {
...
inline bool tryGetFromHex(StringRef Input, std::string &Output) {
  ...
  tuk OutputPtr = const_cast<tuk >(Output.data());
  if (Input.size() % 2 == 1) {
    u8 Hex = 0;
...
namespace detail {
template <typename N>
inline bool to_float(const Twine &T, N &Num, N (*StrTo)(tukk , tuk *)) {
...
inline std::string utostr(u64 X, bool isNeg = false) {
  char Buffer[21];
  tuk BufPtr = std::end(Buffer);
...
inline size_t join_one_item_size(tukk S) { return S ? ::strlen(S) : 0; }
