...
///      unsigned Value;
///    public:
///      MyNode(tukk N, unsigned V) : Name(N), Value(V) {}
///       ...
...
public:
  ...
  private:
    ...
    uk NextInFoldingSetBucket = nullptr;

  public:
    Node() = default;

    // Accessors
    uk getNextInBucket() const { return NextInFoldingSetBucket; }
    void SetNextInBucket(uk N) { NextInFoldingSetBucket = N; }
  };
...
protected:
  ...
  /// faster.
  Node *FindNodeOrInsertPos(const FoldingSetNodeID &ID, uk &InsertPos,
                            const FoldingSetInfo &Info);

  /// InsertNode - Insert the specified node into the folding set, knowing that
  /// it is not already in the folding set.  InsertPos must be obtained from
  /// FindNodeOrInsertPos.
  void InsertNode(Node *N, uk InsertPos, const FoldingSetInfo &Info);
};
...
public:
  ...
  void AddPointer(ukk Ptr) {
    // Note: this adds pointers to the hash using sizes and endianness that
    // depend on the host. It doesn't matter, however, because hashing on
    // pointer values is inherently unstable. Nothing should depend on the
    // ordering of nodes in the folding set.
    static_assert(sizeof(uintptr_t) <= sizeof(unsigned z64),
  ...
  void AddInteger(unsigned long I) {
    if (sizeof(long) == sizeof(i32))
      AddInteger(unsigned(I));
    else if (sizeof(long) == sizeof(z64)) {
      AddInteger((unsigned z64)I);
    } else {
      llvm_unreachable("unexpected sizeof(long)");
    }
  }
  void AddInteger(z64 I) { AddInteger((unsigned z64)I); }
  void AddInteger(unsigned z64 I) {
    AddInteger(unsigned(I));
...
public:
  ...
  /// return it.  If not, return the insertion token that will make insertion
  /// faster.
  T *FindNodeOrInsertPos(const FoldingSetNodeID &ID, uk &InsertPos) {
  ...
  /// it is not already in the folding set.  InsertPos must be obtained from
  /// FindNodeOrInsertPos.
  void InsertNode(T *N, uk InsertPos) {
...
protected:
  ...
  FoldingSetIteratorImpl(uk *Bucket);
...
public:
  explicit FoldingSetIterator(uk *Bucket) : FoldingSetIteratorImpl(Bucket) {}
  ...
  FoldingSetIterator operator++(i32) {        // Postincrement
...
protected:
  ...
  explicit FoldingSetBucketIteratorImpl(uk *Bucket);

  FoldingSetBucketIteratorImpl(uk *Bucket, bool) : Ptr(Bucket) {}

  void advance() {
    uk Probe = static_cast<FoldingSetNode*>(Ptr)->getNextInBucket();
...
public:
  explicit FoldingSetBucketIterator(uk *Bucket) :
    FoldingSetBucketIteratorImpl(Bucket) {}

  FoldingSetBucketIterator(uk *Bucket, bool) :
  ...
  FoldingSetBucketIterator operator++(i32) {      // Postincrement
