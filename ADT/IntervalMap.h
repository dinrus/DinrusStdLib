...
#include <drx/llvm/Support/RecyclingAllocator.h>
#include <algorithm>
#include <cassert>
#include <iterator>
#include <new>
...
public:
  ...
  i32 adjustFromLeftSib(unsigned Size, NodeBase &Sib, unsigned SSize, i32 Add) {
    if (Add > 0) {
...
void adjustSiblingSizes(NodeT *Node[], unsigned Nodes,
  ...
  // Move elements right.
  for (i32 n = Nodes - 1; n; --n) {
    if (CurSize[n] == NewSize[n])
      continue;
    for (i32 m = n - 1; m != -1; --m) {
      i32 d = Node[n]->adjustFromLeftSib(CurSize[n], *Node[m], CurSize[m],
                                         NewSize[n] - CurSize[n]);
      CurSize[m] -= d;
      CurSize[n] += d;
  ...
  for (unsigned n = 0; n != Nodes - 1; ++n) {
    ...
    for (unsigned m = n + 1; m != Nodes; ++m) {
      i32 d = Node[m]->adjustFromLeftSib(CurSize[m], *Node[n], CurSize[n],
                                        CurSize[n] -  NewSize[n]);
      CurSize[m] += d;
      CurSize[n] -= d;
      // Keep going if the current node was exhausted.
...
class NodeRef {
  struct CacheAlignedPointerTraits {
    ...
    static constexpr i32 NumLowBitsAvailable = Log2CacheLine;
  };
...
class Path {
  ...
  struct Entry {
    ...
    Entry(uk Node, unsigned Size, unsigned Offset)
...
public:
  ...
  void setRoot(uk Node, unsigned Size, unsigned Offset) {
    path.clear();
  ...
  void replaceRoot(uk Root, unsigned Size, IdxPair Offsets);
...
branchRoot(unsigned Position) {
  ...
  const unsigned Nodes = (unsigned)RootLeaf::Capacity / (unsigned)Leaf::Capacity + 1;
