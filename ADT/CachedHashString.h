...
public:
  ...
  CachedHashStringRef(StringRef S, u32 Hash)
      : P(S.data()), Size(S.size()), Hash(Hash) {
    assert(S.size() <= std::numeric_limits<u32>::max());
  }

  StringRef val() const { return StringRef(P, Size); }
  tukk data() const { return P; }
...
class CachedHashString {
  ...
  static tuk getEmptyKeyPtr() { return DenseMapInfo<tuk >::getEmptyKey(); }
  static tuk getTombstoneKeyPtr() {
    return DenseMapInfo<tuk >::getTombstoneKey();
  }
  ...
  CachedHashString(ConstructEmptyOrTombstoneTy, tuk EmptyOrTombstonePtr)
...
public:
  explicit CachedHashString(tukk S) : CachedHashString(StringRef(S)) {}
  ...
  CachedHashString(StringRef S, u32 Hash)
