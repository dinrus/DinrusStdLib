...
protected:
  ...
  static uk allocateWithKey(size_t EntrySize, size_t EntryAlign,
                               StringRef Key, AllocatorTy &Allocator);
};

// Define out-of-line to dissuade inlining.
template <typename AllocatorTy>
uk StringMapEntryBase::allocateWithKey(size_t EntrySize, size_t EntryAlign,
                                          StringRef Key,
  ...
  uk Allocation = Allocator.Allocate(AllocSize, EntryAlign);
  assert(Allocation && "Unhandled out-of-memory");

  // Copy the string information.
  tuk Buffer = reinterpret_cast<tuk >(Allocation) + EntrySize;
  if (KeyLength > 0)
...
public:
  ...
  tukk getKeyData() const {
    return reinterpret_cast<tukk >(this + 1);
  }
  ...
  static StringMapEntry &GetStringMapEntryFromKeyData(tukk keyData) {
    tuk ptr = const_cast<tuk >(keyData) - sizeof(StringMapEntry<ValueTy>);
  ...
  template <typename AllocatorTy> void Destroy(AllocatorTy &allocator) {
    ...
    this->~StringMapEntry();
    allocator.Deallocate(static_cast<uk >(this), AllocSize,
