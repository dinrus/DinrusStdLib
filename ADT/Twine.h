...
namespace llvm {
  ...
  class Twine {
    ...
    enum NodeKind : u8 {
      ...
      /// An u32 value, to render as an unsigned decimal integer.
      DecUIKind,

      /// An i32 value, to render as a signed decimal integer.
      DecIKind,
      ...
      /// A pointer to an unsigned z64 value, to render as an unsigned
      /// decimal integer.
      DecULLKind,

      /// A pointer to a z64 value, to render as a signed decimal integer.
      DecLLKind,
  ...
  public:
    ...
    /// Delete the implicit conversion from nullptr as Twine(tukk )
    /// cannot take nullptr.
    ...
    /// Construct from a i8.
    explicit Twine(i8 Val) : LHSKind(CharKind) {
      LHS.character = static_cast<char>(Val);
    }

    /// Construct from an u8.
    explicit Twine(u8 Val) : LHSKind(CharKind) {
      LHS.character = static_cast<char>(Val);
    }
    ...
    explicit Twine(i32 Val) : LHSKind(DecIKind) {
      LHS.decI = Val;
    }

    /// Construct a twine to print \p Val as an unsigned decimal integer.
    explicit Twine(u64k &Val) : LHSKind(DecULKind) {
      LHS.decUL = &Val;
    }
    ...
    /// Construct a twine to print \p Val as an unsigned decimal integer.
    explicit Twine(zu64k &Val) : LHSKind(DecULLKind) {
      LHS.decULL = &Val;
    }

    /// Construct a twine to print \p Val as a signed decimal integer.
    explicit Twine(z64k &Val) : LHSKind(DecLLKind) {
      LHS.decLL = &Val;
    }
    ...
    /*implicit*/ Twine(tukk LHS, const StringRef &RHS)
    ...
    /*implicit*/ Twine(const StringRef &LHS, tukk RHS)
    ...
    static Twine utohexstr(u64k &Val) {
      Child LHS, RHS;
      LHS.uHex = &Val;
      RHS.twine = nullptr;
  ...
  inline Twine operator+(tukk LHS, const StringRef &RHS) {
  ...
  inline Twine operator+(const StringRef &LHS, tukk RHS) {
    return Twine(LHS, RHS);
  }
