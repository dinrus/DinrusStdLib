topic "Класс EditFile";
[0 $$1,0#96390100711032703541132217272105:end]
[i448;a25;kKO9;2 $$2,0#37138531426314131252341829483380:class]
[l288;2 $$3,0#27521748481378242620020725143825:desc]
[b42;2 $$4,4#13035079074754324216151401829390:normal]
[i448;a25;kKO9;2 $$5,0#37138531426314131252341829483370:item]
[H6;0 $$6,0#05600065144404261032431302351956:begin]
[l288;i1121;b17;O9;~~~.1408;2 $$7,0#10431211400427159095818037425705:param]
[ $$0,0#00000000000000000000000000000000:Default]
[{_}%EN-US 
[ {{10000@3 [s0; [*@7;4 Класс EditFile]]}}&]
[s1; &]
[s2;:EditFile`:`:class:%- [@(0.0.255)3 class][3 _][*3 EditFile][3 _:_][@(0.0.255)3 public][3 _][*^topic`:`/`/Controls4U`/src`/EditFileFolder`_ru`-ru`#EditFileFolder`:`:class^@3;3 E
ditFileFolder]&]
[s0;2 &]
[s0; [2 В дизайнере выкладок выглядит так:]&]
[s0;2 &]
[s3; 
@@image:1525&131
()
&]
[s0;2 &]
[s0; [2 В программе так:]&]
[s0;2 &]
[s0;l288; [2 
@@image:1525&131
()
]&]
[s0;2 &]
[s4; EditFile `- поле для ввода пути к файлу.&]
[s4; &]
[s4; При нажатии левой кнопки открывается 
FileSel для выбора файла. Опции для настройки 
EditFile похожи на опции настройки FileSel.&]
[s1; &]
[s0;2 &]
[ {{10000F(128)G(128)@1 [s0; [*2 Детали Конструктора]]}}&]
[s1;%- &]
[s5;:EditFile`:`:EditFile`(`):%- [* EditFile]()&]
[s3; Инициализует EditFile.&]
[s1; &]
[s0;%- &]
[ {{10000F(128)G(128)@1 [s0; [*2 Список Публичных Членов]]}}&]
[s1;%- &]
[s0; [2 Они унаследованы от ][^topic`:`/`/Controls4U`/src`/EditFileFolder`_ru`-ru^2 E
ditFileFolder]&]
[s0; ]]