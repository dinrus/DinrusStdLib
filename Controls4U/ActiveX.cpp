...
OleStr::~OleStr() {
    SysFreeTxt(var.bstrVal);
}

bool OleStr::Set(const Txt str) {
    var.vt = VT_BSTR;
...
bool CBSTR::Set(const Txt str) {
    return BSTRSet(str, bstr);
}

DHCtrlActiveX::DHCtrlActiveX(CLSID clsid, const Txt name, bool status) :
...
uk DHCtrlActiveX::QueryInterface(const IID iid) {
    uk ret;
    if (!oleObj || !status)
        return NULL;
...
bool DHCtrlActiveX::Attach(HWND hwd) {
    ...
    oleObj->SetHostNames((const wchar_t *)name.ToWTxt().Begin(), 0);
...
HRESULT STDMETHODCALLTYPE AXStorage::OpenStream(const OLECHAR *, uk , DWORD , DWORD , IStream **) {
    throw Exc(t_("Функция не реализована"));
}
...
HRESULT STDMETHODCALLTYPE AXStorage::EnumElements(DWORD , uk , DWORD , IEnumSTATSTG **) {
    throw Exc(t_("Функция не реализована"));
}
