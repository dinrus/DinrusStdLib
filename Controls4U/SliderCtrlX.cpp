...
i32 SliderCtrlX::ClientToSlider(i32 p) const
/***********************************************************************************************
...
void    SliderCtrlX::DrawTick( Draw &w, MAJORMINOR Type, HOVE Orientation, i32 nPos, i32 nVal )
/***********************************************************************************************
...
{
    Size size = GetSize();
    i32     siz = Orientation == HORZ ? size.cy : size.cx;
    i32     nMajorSize = (i32)( m_nMajorTickSize / 100.0f * siz * .5 + 0.5f );
    i32     nMinorSize = (i32)( m_nMinorTickSize / 100.0f * siz * .5 + 0.5f );
    i32     nMajorWidth = 3;
    i32     TickBottomMaj = 0, TickBottomMin = 0, TickTop = 0;
    ...
    if( Orientation == HORZ ) {
        if( Type == MAJOR ) {
            w.DrawLine( max( nPos, nMajorWidth >> 1 ),
                (i32)( size.cy * .5 + TickTop),
                max( nPos, nMajorWidth >> 1 ),
                (i32)(size.cy * .5 + TickBottomMaj),
                nMajorWidth );

            Size sz = GetTextSize( txt, fnt );
            i32 nTextPos = nPos - (i32)( sz.cx / 2.0f + 0.5f );
            ...
            w.DrawLine( nPos,
                (i32)( size.cy * .5 + TickTop),
                nPos,
                (i32)(size.cy * .5 + TickBottomMin ) );
        }
    }
    else { // vert
        if( Type == MAJOR ) {
            w.DrawLine( (i32)( size.cx * .5 + TickTop),
                max( nPos, nMajorWidth >> 1 ),
                (i32)(size.cx * .5 + TickBottomMaj),
                max( nPos, nMajorWidth >> 1 ),
                nMajorWidth );

            Size sz = GetTextSize( txt, fnt );
            i32 nTextPos = nPos - (i32)( sz.cy / 2.0f + 0.5f );
            nTextPos = min( max( 0, nTextPos ), size.cy - sz.cy );
            w.DrawText( 0, nTextPos, txt );
        }
        else {
            w.DrawLine( (i32)(size.cx / 2 + TickTop),
                nPos,
                (i32)(size.cx * .5 + TickBottomMin),
                nPos );
        }
    }
}

Val  SliderCtrlX::GetData() const
/***********************************************************************************************
...
Val SliderCtrlX::GetData( i32 nIndex ) const
/***********************************************************************************************
 * reads the slider[n] value
 * virtual - public
 **********************************************************************************************/
{
    return m_vVals[nIndex];
}
...
i32  SliderCtrlX::HoVe(i32  x, i32  y) const
/***********************************************************************************************
...
i32& SliderCtrlX::HoVeR(i32& x, i32& y) const
/***********************************************************************************************
 * returns &x for Hslider or &y for Vslider
 * private
 **********************************************************************************************/
...
bool SliderCtrlX::Key(dword key, i32 repcnt)
/***********************************************************************************************
 ...
 **********************************************************************************************/
{
    ...
    SetWantFocus();
    i32 thumbPos = SliderToClient(m_vVals[0]);
    i32 p = HoVe(pos.x, pos.y);

    if(IsNull(thumbPos)) {
        SetVal( ClientToSlider(p) );
        WhenSlideFinish();
        UpdateActionRefresh();
    }
...
SliderCtrlX& SliderCtrlX::MinMax(i32 _min, i32 _max)
/***********************************************************************************************
 ...
 **********************************************************************************************/
{
    if(m_nMin != _min || m_nMax != _max) {
        ...
        m_nMax = _max;
        if(!IsNull(m_vVals[0])) {
            i32 v = minmax(m_vVals[0], Min(), Max());
            if(m_vVals[0] != v) {
                SetVal( v );
                Update();
            }
 ...
 **********************************************************************************************/
{
    ...
    if(IsVert()) { // Vertical slider
        ...
        if (IsInverted())
            ...
            w.DrawRect(half - BORDER1+1, SliderToClient(m_vVals[0])+1,
                   border2-2, size.cy-SliderToClient(m_vVals[0])-BORDER1-2,
                   m_FillColor);
        else
            // draw the fill where top is the min value
            w.DrawRect(half - BORDER1+1, BORDER1+1,
                   border2-2, SliderToClient(m_vVals[0])-BORDER1-2,
                   m_FillColor);
    }
    else { // Horz slider
        // Draw Line Border
        ...
        if (IsInverted())
            ...
            w.DrawRect(SliderToClient(m_vVals[0]), half - BORDER1+2, size.cx-SliderToClient(m_vVals[0])-2, border2-3, m_FillColor);
        else
            // draw the fill where right is the min value
            w.DrawRect(BORDER1, half - BORDER1+2, SliderToClient(m_vVals[0])-2, border2-3, m_FillColor);
    }
    ...
    // Draw thumbs
    if(IsVert()) { // Vertical slider
        for( i32 i = m_vVals.GetCount() - 1 ; i >= 0 ; i-- ) {
            if(!IsNull(m_vVals[i])) {
                w.DrawImage((size.cx - m_ThumbSize.cx ) >> 1,
                    SliderToClient(m_vVals[i]) - ( m_ThumbSize.cy >> 1 ),
        ...
        for( i32 i = m_vVals.GetCount() - 1 ; i >= 0 ; i-- ) {
            if(!IsNull(m_vVals[i])) {
                w.DrawImage(SliderToClient(m_vVals[i]) - ( m_ThumbSize.cx >> 1 ),
...
void SliderCtrlX::SetData(const Val& v)
/***********************************************************************************************
...
SliderCtrlX& SliderCtrlX::SetThumbType(i32 n)
/***********************************************************************************************
...
Val SliderCtrlX::SetVal( const Val& v, i32 nIndex /*= 0 */ )
/***********************************************************************************************
 ...
 **********************************************************************************************/
{
    ...
    return m_vVals.At(nIndex);
}

i32 SliderCtrlX::SliderToClient(i32 v) const
/***********************************************************************************************
 * CONVERTS SLIDER VALUES TO Canvas coordinates
 * private
 **********************************************************************************************/
