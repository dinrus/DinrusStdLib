...
public:
	...
	virtual bool            Fetch();
	virtual void            GetColumn(i32 i, Ref r) const;
	virtual void            Cancel();
	virtual SqlSession&     GetSession() const { ASSERT(session); return *session; }
	virtual Txt          GetUser() const { ASSERT(session); return session->user; }
...
Array< Tuple2<Txt, Txt> > ODBCSession::EnumDSN()
{
	Array< Tuple2<Txt, Txt> > out;
	try {
		SQLHENV MIenv;
		SQLAllocHandle(SQL_HANDLE_ENV, SQL_NULL_HANDLE, &MIenv);
		SQLSetEnvAttr(MIenv, SQL_ATTR_ODBC_VERSION, (uk ) SQL_OV_ODBC3, 0);
		SQLRETURN ret;
		char l_dsn[256];
		char l_desc[256];
		short i32 l_len1,l_len2,l_next;
		...
		while(SQL_SUCCEEDED(ret = SQLDataSources(MIenv, l_next, (SQLCHAR *)l_dsn, sizeof(l_dsn),
			...
			Tuple2<Txt, Txt>& listdsn = out.Add();
			listdsn.a = l_dsn;
			listdsn.b = l_desc;
...
bool ODBCSession::Connect(tukk cs)
{
...
ODBCSession::ODBCSession()
{
	...
	if(henv)
		SQLSetEnvAttr(henv, SQL_ATTR_ODBC_VERSION, (uk ) SQL_OV_ODBC3, 0);
	tlevel = 0;
	Dialect(MSSQL);
	tmode = NORMAL;
}
...
void ODBCSession::SetTransactionMode(i32 mode)
{
	tmode = mode;
...
Txt ODBCSession::Savepoint()
{
	NEVER();
	return "";
}

void ODBCSession::RollbackTo(const Txt& savepoint)
{
	NEVER();
}

Vec<Txt> ODBCSession::EnumUsers()
{
	return Vec<Txt>();
}

Vec<Txt> ODBCSession::EnumDatabases()
{
	return Vec<Txt>();
}

Vec<Txt> ODBCSession::EnumTables(Txt database)
{
	return Vec<Txt>();
}

Vec<Txt> ODBCSession::EnumViews(Txt database)
{
	return Vec<Txt>();
}

Vec<Txt> ODBCSession::EnumSequences(Txt database)
{
	return Vec<Txt>();
}

Vec<Txt> ODBCSession::EnumPrimaryKeys(Txt database, Txt table)
{
	return Vec<Txt>();
}

Txt ODBCSession::EnumRowID(Txt database, Txt table)
{
	return "";
}

bool   ODBCPerformScript(const Txt& text, StatementExecutor& executor, Gate<i32, i32> progress_canceled)
{
	tukk p = text;
	while(*p) {
		Txt cmd;
		...
		if(progress_canceled(i32(p - text.Begin()), text.GetLength()))
			return false;
...
:	session(session_)
{
	LLOG("ODBCConnection " << (uk )this << " " << (uk )session);
	rowcount = rowi = 0;
	number_i = 0;
	num64_i = 0;
	text_i = 0;
...
void ODBCConnection::SetParam(i32 i, const Val& r)
{
	param.At(i) = r;
/*
	Param& p = param.At(i);
	...
	if(IsNumber(r)) {
		if(r.Is<z64>()) {
			...
			p.data = Txt((tuk )&x, sizeof(x));
			p.li = sizeof(x);
		}
		else {
			double x = r;
			if(x >= INT_MIN && x < INT_MAX && (i32)x == x) {
				i64 h = (i32)x;
				p.ctype = SQL_C_SLONG;
				p.sqltype = SQL_INTEGER;
				p.data = Txt((tuk )&h, sizeof(h));
				p.li = sizeof(h);
			}
			else {
...
tukk ODBCReadTxt(tukk s, Txt& stmt)
{
...
{
	LLOG("Execute " << (uk )this << " " << (uk )session << " " << statement);
	...
	while(s < statement.End())
		if(*s == '\'' || *s == '\"')
			...
			if(*s == '?') {
				...
				Val v = param[pi++];
				if(session->charset >= 0 && IsTxt(v))
					v = ToCharset(session->charset, (Txt)v, CHARSET_DEFAULT, '?');
				if(v.GetType() == 34)
...
/*
		...
		bparam = pick(param);
		param.Clear();
		for(i32 i = 0; i < bparam.GetCount(); i++) {
			Param& p = bparam[i];
...
i32 ODBCConnection::GetRowsProcessed() const
{
...
{
	LLOG("Fetch0 " << (uk )this << " " << (uk )session);
	i32 ret = SQLFetch(session->hstmt);
	...
	for(i32 i = 0; i < info.GetCount(); i++) {
		...
		case DATE_V:
			...
			if(li != SQL_NULL_DATA) {
				...
				m.day = (u8)tm.day;
				m.hour = (u8)tm.hour;
				m.minute = (u8)tm.minute;
				m.second = (u8)tm.second;
				if(type == DATE_V)
					v = (Date)m;
				else
					v = m;
...
void ODBCConnection::GetColumn(i32 i, Ref r) const
{
	LLOG("GetColumn " << (uk )this << " " << (uk )session);
	r.SetVal(fetchrow[i]);
}

void ODBCConnection::FetchAll()
{
	LLOG("FetchAll " << (uk )this);
	rowcount = 0;
	rowi = 0;
	number.Clear();
	num64.Clear();
...
Txt ODBCConnection::ToTxt() const
{
	return statement;
}

Val ODBCConnection::GetInsertedId() const
{
	Sql sql(GetSession());
