...
u8    HighlightSetup::hilite_scope = 0;
u8    HighlightSetup::hilite_ifdef = 1;
u8    HighlightSetup::hilite_bracket = 1;
...
i32     HighlightSetup::indent_amount = 4;

#define HL_COLOR(x, a, b)    #x,
static tukk s_hl_color[] = {
#include "hl_color.i"
};
#undef  HL_COLOR

#define HL_COLOR(a, x, b)    x,
static tukk s_hl_name[] = {
#include "hl_color.i"
};
#undef  HL_COLOR
...
const HlStyle& HighlightSetup::GetHlStyle(i32 i)
{
	InitOnce();
	return hl_style[i];
}

tukk HighlightSetup::GetHlName(i32 i)
{
	InitOnce();
	return s_hl_name[i];
}

bool HighlightSetup::HasHlFont(i32 i)
{
	InitOnce();
	return s_hl_font[i];
}

void  HighlightSetup::SetHlStyle(i32 i, Color c, bool bold, bool italic, bool underline)
{
	InitOnce();
...
void HighlightSetup::LoadHlStyles(tukk s)
{
	CParser p(s);
	try {
		while(!p.IsEof()) {
...
Txt HighlightSetup::StoreHlStyles()
{
	Txt r;
	for(i32 i = 0; i < HL_COUNT; i++) {
...
void HighlightSetup::DarkTheme(bool host)
{
	...
	SetHlStyle(INK_CONST_STRING,          Color(170, 255, 200));
	...
	SetHlStyle(PAPER_BLOCK1,              Color(56, 85, 0));
	SetHlStyle(PAPER_BLOCK2,              Color(27, 75, 26));
	SetHlStyle(PAPER_BLOCK3,              Color(85, 42, 0));
	SetHlStyle(PAPER_BLOCK4,              Color(113, 42, 150));
