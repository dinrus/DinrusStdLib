...
inline wtukk strnext(wtukk p, wtukk end, i32 ch) {
	while(p < end) {
		if(*p == ch) return p;
		p++;
	}
...
wtukk isstmt(wtukk p) {
	static tukk stmt[] = {
		"if", "else", "while", "do", "for"
	};
	for(tukk *q = stmt; q < stmt + __countof(stmt); q++) {
		tukk k = *q;
		wtukk s = p;
		for(;;) {
			if(*k== '\0') {
...
void CSyntax::Grounding(wtukk b, wtukk e)
{
...
void CSyntax::ScanSyntax(wtukk ln, wtukk e, i32 line, i32 tab_size)
{
	CTIMING("ScanSyntax");
	Grounding(ln, e);
	if(!linecont) {
				...
				if(id[2] == 'i' && id[3] == 'f') {
					if(ifstack.GetCount() == 0) {
						...
						if(ifstate.state == IfState::IF || ifstate.state == IfState::ELIF) {
							...
							ifstate.iftext = WTxt().Cat() << sReadLn(ln) << ", " << ifstate.iftext;
						}
						else
							ifstate.state = IfState::ELSE_ERROR;
					}
