...
//			break;
	case SDL_TEXTINPUT: {
		...
		WTxt text = event->text.text;
		for(i32 i = 0; i < text.GetCount(); i++) {
			i32 c = text[i];
			if(c != 127)
				Ctrl::DoKeyFB(c, 1);
		}
	...
	case SDL_MOUSEBUTTONDOWN: {
			...
			i32 bi = event->button.button;
			dword ct = SDL_GetTicks();
			if(isdblclick[bi] && (abs(i32(ct) - i32(lastbdowntime[bi])) < 400))
			{
				switch(bi)
				{
...
Uint32 WakeCb(Uint32 interval, uk param)
{
