...
struct ButtonDecomposer {
	...
	void Do() {
		Size sz = src.GetSize();
		i32 a = min(5, min(sz.cx, sz.cy) / 5);
		maxdiff = gdiff = gcount = 0;
		color = SColorDisabled();
		dst = src;
		ImageBuffer b(dst);
		for(i32 y = a; y < sz.cy - a; y++) {
			RGBA *p = b[y];
			RGBA c1 = p[a - 1];
			RGBA c2 = p[sz.cx - a + 1];
			i32 n = sz.cx - 2 * a;
...
bool EqLine(const Image& m, i32 l1, i32 l2, i32 x, i32 width)
{
	return !memcmp(m[l1] + x, m[l2] + x, width * sizeof(RGBA));
}

bool EqLine(const Image& m, i32 l1, i32 l2)
{
...
Image RecreateAlpha(const Image& overwhite, const Image& overblack)
{
	...
	Premultiply(r);
	return static_cast<Image>(r);
}

i32 ImageMargin(const Image& _m, i32 p, i32 dist)
{
