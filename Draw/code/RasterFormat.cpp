...
static i32 sMaskPos16(dword mask)
{
	if(mask > 0xffff)
		return 0;
	i32 pos = 0;
...
i32 RasterFormat::IsRGBA() const
{
	return (type & 31) == RASTER_32PREMULTIPLIED && bpos == 0 && gpos == 1 && rpos == 2 && apos == 3;
}

static u8 bits[16] = { 1, 2, 4, 8, 16, 16, 24, 32, 32, 32 };

i32 RasterFormat::GetByteCount(i32 cx) const
{
	i32 b = bits[type & 15];
	return (cx * b + 7) >> 3;
}

i32 RasterFormat::GetBpp() const
{
	return bits[type & 15];
}
...
i32 RasterFormat::GetColorCount() const
{
	static i32 colors[16] = { 2, 4, 16, 256, 256, 65536, 256 * 256 * 256, 256 * 256 * 256, 256 * 256 * 256 };
	return colors[type & 15];
}

i32 RasterFormat::GetPaletteCount() const
{
	static i32 colors[16] = { 2, 4, 16, 256 };
	return (type & 15) < 4 ? colors[type & 15] : 0;
}

void RasterFormat::TailBits(RGBA *t, u8k *src, i32 cx, u8 andm, u8 shift, const RGBA *palette)
{
	if(cx) {
		u8 c = *src;
		while(cx > 0) {
...
void RasterFormat::TailBitsMSB1st(RGBA *t, u8k *src, i32 cx, u8 shift1, u8 andm, u8 shift, const RGBA *palette)
{
	if(cx) {
		u8 c = *src;
		while(cx > 0) {
...
void RasterFormat::Read(RGBA *t, u8k *s, i32 cx, const RGBA *palette) const
{
	switch(type) {
	case RASTER_1:
		{
	...
	case RASTER_16:
		{
			...
			while(t < e) {
				word w = Peek16le(s);
				t->a = 255;
				t->r = u8((w & rmask) << rpos >> 8);
				t->g = u8((w & gmask) << gpos >> 8);
				t->b = u8((w & bmask) << bpos >> 8);
				s += 2;
				t++;
			}
		}
