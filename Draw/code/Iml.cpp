...
{
	Vec<ImageIml> img;
	Txt data = ZDecompress(ptr, len);
	tukk s = data;
	while(s + 6 * 2 + 1 <= data.End()) {
		ImageIml& m = img.Add();
		ImageBuffer ib(Peek16le(s + 1), Peek16le(s + 3));
		m.flags = u8(*s) & 0x3f;
		ib.SetResolution(decode(u8(*s) >> 6, 0, IMAGE_RESOLUTION_STANDARD,
...
void Iml::Set(i32 i, const Image& img)
{
	map[i].image = img;
	map[i].loaded = true;
}
...
ImageIml Iml::GetRaw(i32 mode, i32 i)
{
...
ImageIml Iml::GetRaw(i32 mode, const Txt& id)
{
...
Image MakeImlImage(const Txt& id, Function<ImageIml(i32, const Txt& id)> GetRaw, dword global_flags)
{
	Image image;
	i32 mode = IsUHDMode() * GUI_MODE_UHD + IsDarkTheme() * GUI_MODE_DARK;
	if(mode == GUI_MODE_NORMAL)
		image = GetRaw(GUI_MODE_NORMAL, id).image;
	else {
		auto Mode = [&](dword m, tukk s) { return mode & m ? Txt(s) : Txt(); };
...
Image Iml::Get(i32 i)
{
	...
	if(!m.loaded) {
		Mutex::Lock __(sImlLock);
		if(!m.loaded) {
			m.image = MakeImlImage(GetId(i), [&](i32 mode, const Txt& id) { return GetRaw(mode, id); }, global_flags);
			m.loaded = true;
		}
	}
	return m.image;
...
void Iml::AddData(u8k *s, i32 len, i32 count, i32 mode)
{
...
void Iml::AddId(i32 mode1, tukk name)
{
...
void Iml::ResetAll()
{
	for(i32 i = 0; i < GetImlCount(); i++)
		GetIml(i).Reset();
}

static StaticCriticalSection sImgMapLock;

static VecMap<Txt, Iml *>& sImgMap()
{
	static VecMap<Txt, Iml *> x;
	return x;
}

void Register(tukk imageclass, Iml& list)
{
#ifdef flagCHECKINIT
