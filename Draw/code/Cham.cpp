...
namespace drx {
inline void LOGPNG(tukk name, const Image& m)
{
...
void  ChDraw(Draw& w, i32 x, i32 y, i32 cx, i32 cy, const Image& img, const Rect& src, Color ink)
{
	LTIMING("ChDraw");
	if(cx > 0 && cy > 0) {
...
void  ChDraw(Draw& w, i32 x, i32 y, const Image& img, const Rect& src, Color ink)
{
...
Val ChLookWith(const Val& look, const Image& img, Point offset)
{
	sChLookWith x;
	x.look = look;
	x.img = img;
...
Val ChLookWith(const Val& look, const Image& img, Color color, Point offset)
{
	sChLookWith x;
	x.look = look;
	x.img = img;
...
Val ChLookWith(const Val& look, const Image& img, Color (*color)(i32 i), i32 i, Point offset)
{
	sChLookWith x;
	x.look = look;
	x.img = img;
...
void ChLookWith(Val *look, const Image& image, const Color *color, i32 n)
{
	for(i32 i = 0; i < n; i++)
		look[i] = ChLookWith(look[i], image, color[i]);
}

Val sChOp(Draw& w, const Rect& r, const Val& v, i32 op, Color ink = Null);
...
Val ChBorder(const ColorF *colors, const Val& face)
...
static void sDrawScrollbarThumb(Draw& w, i32 x, i32 y, i32 cx, i32 cy, const Image& m, bool vert)
{
	if(cx <= 0)
		return;
	Size isz = m.GetSize();
	...
	i32 ecx = isz.cy / 2; // size of 'ending' (can be circular)
	i32 ccx = isz.cy / 2; // size of central part, there is sometimes some sort of 'handle'
	i32 sl = (isz.cx - 2 * ecx - ccx) / 2; // size of left stretched part source
	i32 sr = isz.cx - 2 * ecx - ccx - sl; // size of right stretched part source

	i32 tel = ecx; // left ending target size
	i32 l = (cx - 2 * ecx - ccx) / 2; // size of left stretched part
	i32 tccx = ccx; // target center size
	i32 r = cx - 2 * ecx - ccx - l; // size of right stretched part
	i32 ter = ecx; // size of right stretched part
...
Val StdChLookFn(Draw& w, const Rect& r, const Val& v, i32 op, Color ink)
{
	if(IsType<sChLookWith>(v)) {
		const sChLookWith& x = ValTo<sChLookWith>(v);
		if(op == LOOK_PAINT) {
			LOGPNG(AsTxt(x.img.GetSerialId()), x.img);
			ChPaint(w, r, x.look);
	...
	if(IsType<sChBorder>(v)) {
		sChBorder b = ValTo<sChBorder>(v);
		i32 n = (i32)(intptr_t)*b.border;
		switch(op) {
		case LOOK_PAINT:
	...
	if(IsType<Image>(v)) {
		...
		i32 top = minmax(sz.cy / 2, 0, isz.cy);
		i32 bottom = minmax(sz.cy - top, 0, isz.cy);
		i32 yy = isz.cy - bottom;
		i32 left = minmax(sz.cx / 2, 0, isz.cx);
		i32 right = minmax(sz.cx - left, 0, isz.cx);
		i32 xx = isz.cx - right;
		if(!r.IsEmpty()) {
			...
			if(op == LOOK_PAINT) {
				...
				if(tile) {
					...
					static VecMap<z64, bool> single_color_body_cache;
					z64 key = img.GetSerialId();
					bool single_color_body;
					i32 q = single_color_body_cache.Find(key);
					if(q < 0) {
...
typedef Val (*ChPainterFn)(Draw& w, const Rect& r, const Val& v, i32 op, Color ink);

Vec<ChPainterFn>& sChps()
{
	static Vec<ChPainterFn> x;
	return x;
}

void ChLookFn(Val (*fn)(Draw& w, const Rect& r, const Val& v, i32 op, Color ink))
{
...
Vec<sStyleCh>& sChStyle()
{
	static Vec<sStyleCh> x;
	return x;
}
...
void ChReset()
{
	if(dark_theme__) dark_theme__ = false;
	for(i32 i = 0; i < sChStyle().GetCount(); i++)
		*sChStyle()[i].status = 0;
...
Val sChOp(Draw& w, const Rect& r, const Val& v, i32 op, Color ink)
{
	if(r.right < r.left || r.bottom < r.top)
		return Rect(0, 0, 0, 0);
	Val q;
	if(!IsNull(v))
		for(i32 i = sChps().GetCount() - 1; i >= 0; i--) {
...
void ChPaint(Draw& w, const Rect& r, const Val& look, Color ink)
{
	sChOp(w, r, look, LOOK_PAINT, ink);
}

void ChPaint(Draw& w, i32 x, i32 y, i32 cx, i32 cy, const Val& look, Color ink)
{
	sChOp(w, RectC(x, y, cx, cy), look, LOOK_PAINT, ink);
}

void ChPaintEdge(Draw& w, const Rect& r, const Val& look, Color ink)
{
	sChOp(w, r, look, LOOK_PAINTEDGE, ink);
}

void ChPaintEdge(Draw& w, i32 x, i32 y, i32 cx, i32 cy, const Val& look, Color ink)
{
	sChOp(w, RectC(x, y, cx, cy), look, LOOK_PAINTEDGE, ink);
}

void ChPaintBody(Draw& w, const Rect& r, const Val& look, Color ink)
{
...
void ChPaintBody(Draw& w, i32 x, i32 y, i32 cx, i32 cy, const Val& look, Color ink)
{
...
void  Override(Iml& target, Iml& source, bool colored)
{
	for(i32 i = 0; i < target.GetCount(); i++) {
		i32 q = source.Find(target.GetId(i));
		if(q >= 0) {
