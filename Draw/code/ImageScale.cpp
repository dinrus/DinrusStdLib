...
static u8k *GetStretchCurve()
{
	static u8 cache[COUNT_STRETCH_CURVE];
	ONCELOCK {
		for(i32 i = 0; i < COUNT_STRETCH_CURVE; i++)
		{
			enum { HALF = COUNT_STRETCH_CURVE >> 1 };
			double a = (i <= HALF ? i / double(HALF) : (COUNT_STRETCH_CURVE - i) / double(HALF));
			double o = pow(a, 0.85);
			cache[i] = minmax<i32>((i32)((i <= HALF ? o : 2 - o) * 128), 0, 255);
		}
	}
	return cache;
}

Vec<dword> AAGetMap(i32& dmin, i32& dmax, i32 dclipmin, i32 dclipmax,
	                   i32 smin, i32 smax, i32 sclipmin, i32 sclipmax, i32 times, i32 avail)
{
	Vec<dword> map;
	...
	i32 dw = dmax - dmin, sw = smax - smin, spos;
	if(sw > 0)
	{
		i32 x0 = dmin;
		if(smin < sclipmin)
	...
	i32 count = min(dclipmax, dmax) - dmin;
	if(smin < sclipmin)
		smin = sclipmin;
	if(smax > sclipmax)
		smax = sclipmax;
	if(smax <= smin || count <= 0)
		return map;
	i32 span = min(tabs(sw) % dw ? idivceil(tabs(sw), dw) + 1 : tabs(sw) / dw, smax - smin);
	bool bigseg = (span >= MAXAA);
	i32 segment = (bigseg ? MAXAA : span);
	i32 segstep = span / segment;
	...
	i32 sendoffset = (smax - (segment - 1) * segstep - 1) * times;
	i32 last = 0;
	...
	{ // size reduction
		i32 sbegin = smin * dw, send = smax * dw, aw = tabs(sw);
		for(spos += min(sw, 0); --count >= 0; spos += sw)
		{
			i32 pb = max(spos, sbegin), pe = min(spos + aw, send);
			i32 total = pe - pb, left = avail;
			i32 start = idivfloor(pb, dw), end = idivceil(pe, dw) - 1, rem = pb % dw;
//			DUMP(start);
//			DUMP(end);
			if(pb >= send)
			{
			...
			{ // 1 source pixel only
//				ASSERT(!bigseg);
				i32 scomp = minmax(start + segment - smax, 0, start - smin);
			...
			{
				i32 delta = (dw - rem) * left / total;
				if(!delta)
					start++;
				i32 scomp = minmax(start + span - smax, 0, start - smin);
				last += *out++ = (start - scomp) * times - last;
				if(!bigseg)
				{
					i32 i = scomp;
...
//			LOG("-> " << map[rec] << " + " << map[rec + 1]);
	...
	{ // size inflation
		static u8k *curve = GetStretchCurve();
		ASSERT(segment == 2 && !bigseg);
		i32 sbegin = smin * dw, send = (smax - 1) * dw;
...
#ifdef _DEBUG
	ASSERT(out == map.End());
	i32 offs = 0, step = map[MAP_BLOCK], segspan = (map[MAP_SEGMENT] - 1) * map[MAP_STEP] + 1;
	for(i32 t = 0; t < (i32)map[MAP_COUNT]; t++)
	{
...
void BltAASet2Fix(RGBA *dest, const dword *src1, dword w1, const dword *src2, dword w2, i32 count)
{
	while(count--) {
		dest->b = u8((src1[0] * w1 + src2[0] * w2) >> 16);
		dest->g = u8((src1[1] * w1 + src2[1] * w2) >> 16);
		dest->r = u8((src1[2] * w1 + src2[2] * w2) >> 16);
		dest->a = u8((src1[3] * w1 + src2[3] * w2) >> 16);
		dest++;
		src1 += 4;
		src2 += 4;
	}
...
{
	while(count--) {
		dest->b = u8((src1[0] * w1 + src2[0] * w2 + src3[0] * w3) >> 16);
		dest->g = u8((src1[1] * w1 + src2[1] * w2 + src3[1] * w3) >> 16);
		dest->r = u8((src1[2] * w1 + src2[2] * w2 + src3[2] * w3) >> 16);
		dest->a = u8((src1[3] * w1 + src2[3] * w2 + src3[3] * w3) >> 16);
		dest++;
		src1 += 4;
		src2 += 4;
		src3 += 4;
...
void BltAASet4Fix(RGBA *dest, const dword *src1, const dword *src2,
                              const dword *src3, const dword *src4, i32 count)
{
	while(count--) {
		dest->b = u8((src1[0] + src2[0] + src3[0] + src4[0]) >> 10);
		dest->g = u8((src1[1] + src2[1] + src3[1] + src4[1]) >> 10);
		dest->r = u8((src1[2] + src2[2] + src3[2] + src4[2]) >> 10);
		dest->a = u8((src1[3] + src2[3] + src3[3] + src4[3]) >> 10);
		dest++;
		src1 += 4;
		src2 += 4;
		src3 += 4;
...
void BltAAFix2(RGBA *dest, const dword *src, i32 count)
{
...
const RGBA *RescaleImage::GetLine(i32 ii)
{
	if(cache[0].ii == ii)
		return cache[0].line;
	if(cache[1].ii == ii)
...
void RescaleImage::Get(RGBA *tgt)
{
	...
	if(bigseg) {
		...
		i32 endoff = offset + segment;
		for(i32 next = first + full; next < endoff; next++) {
			if(full >= segment)
				first++;
			else
				full++;
...
bool Rescale(RasterEncoder& tgt, Size tsz, Raster& src, const Rect& src_rc,
             Gate<i32, i32> progress)
{
	tgt.Create(tsz, src);
	RescaleImage rs;
	rs.Create(tsz, src, src_rc);
	for(i32 i = 0; i < tsz.cy; i++) {
...
Image Rescale(const Image& src, Size sz, const Rect& src_rc, Gate<i32, i32> progress)
{
...
Image Rescale(const Image& src, Size sz, Gate<i32, i32> progress)
{
	return Rescale(src, sz, src.GetSize(), progress);
}

Image Rescale(const Image& src, i32 cx, i32 cy, Gate<i32, i32> progress)
{
