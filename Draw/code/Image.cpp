...
i32 ImageBuffer::ScanKind() const
{
	const RGBA *s = pixels;
...
void ImageBuffer::Create(i32 cx, i32 cy)
{
	ASSERT(cx >= 0 && cy >= 0);
	size.cx = cx;
	size.cy = cy;
...
{
	dots.cx = i32(600.*size.cx/dpi.cx);
	dots.cy = i32(600.*size.cy/dpi.cy);
}

Size ImageBuffer::GetDPI() 
{
	return Size(dots.cx ? i32(600.*size.cx/dots.cx) : 0, dots.cy ? i32(600.*size.cy/dots.cy) : 0);
}
...
Size Image::GetDPI() const
{
	Size size = GetSize();
	Size dots = GetDots();
	return data ?  Size(i32(600.*size.cx/dots.cx), i32(600.*size.cy/dots.cy)): Size(0, 0);
}

i32 Image::GetResolution() const
{
	return data ? data->buffer.GetResolution() : IMAGE_RESOLUTION_NONE;
}

i32 Image::GetKindNoScan() const
{
	return data ? data->buffer.GetKind() : IMAGE_ALPHA;
}

i32 Image::Data::GetKind()
{
	i32 k = buffer.GetKind();
	if(k != IMAGE_UNKNOWN)
		return k;
	k = buffer.ScanKind();
	buffer.SetKind(k);
...
void Image::Serialize(Stream& s)
{
	...
	i32 version = spot2.x || spot2.y; // version 1 only if we need 2nd spot, to improve BW compatibility
	s / version;
	Size sz = GetSize();
	Point p = GetHotSpot();
	Size dots = GetDots();
	...
	if(sz.cx < 0 || sz.cy < 0)
		s.LoadError();
	z64 len = (z64)sz.cx * (z64)sz.cy * (z64)sizeof(RGBA);
	if(s.IsLoading()) {
		if(len) {
			ImageBuffer b;
			if(len < 6 * 1024 * 1024) {
				b.Create(sz);
				if(!s.GetAll(~b, (i32)len)) {
					Clear();
					s.LoadError();
					return;
				}
...
Txt Image::ToTxt() const
{
	return Txt("Изображение ").Cat() << GetSize();
}
...
void Image::SetAuxData(zu64 adata)
{
	if(data)
		data->aux_data = adata;
}

zu64 Image::GetAuxData() const
{
	return data ? data->aux_data : 0;
}

static void sMultiply(ImageBuffer& b, i32 (*op)(RGBA *t, const RGBA *s, size_t len))
{
...
static Image sMultiply(const Image& img, i32 (*op)(RGBA *t, const RGBA *s, size_t len))
{
	i32 k = img.GetKind();
...
Txt StoreImageAsTxt(const Image& img)
{
	if(img.GetKind() == IMAGE_EMPTY)
		return Null;
	i32 type = img.GetKind() == IMAGE_OPAQUE ? 3 : 4;
	...
	Buffer<u8> b(type * img.GetLength());
	u8 *t = b;
	if(type == 3)
		while(s < e) {
			*t++ = s->r;
...
Image  LoadImageFromTxt(const Txt& src)
{
	if(src.GetLength() < 13)
		return Null;
	TxtStream ss(src);
	i32 type = ss.Get();
	i32 resolution = decode(type & 0xc0, 0x40, IMAGE_RESOLUTION_STANDARD, 0x80, IMAGE_RESOLUTION_UHD, 0);
	type &= 0x3f;
	Size sz;
	sz.cx = ss.Get16le();
	sz.cy = ss.Get16le();
