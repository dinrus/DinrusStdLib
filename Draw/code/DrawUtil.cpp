...
void Union(Vec<Rect>& rr, const Rect& add)
{
	...
	Vec<Rect> r;
	r.Add(add);
	for(i32 i = 0; i < rr.GetCount() && r.GetCount(); i++)
		Subtract(r, rr[i]);
...
void AddRefreshRect(Vec<Rect>& invalid, const Rect& _r, double ctolerance)
{
	Vec<Rect> inv;
	Rect r = _r;
	i32 ra = r.Width() * r.Height();
	for(i32 i = 0; i < invalid.GetCount(); i++) {
	...
	Vec<Rect> rs;
	rs.Add(r);
	for(i32 i = 0; i < inv.GetCount(); i++) {
		bool ch = false;
		Vec<Rect> rs1 = Subtract(rs, inv[i], ch);
		if(ch) rs = pick(rs1);
	}
...
void DrawBorder(Draw& w, i32 x, i32 y, i32 cx, i32 cy, const ColorF *c) {
	if(!c) return;
	i32 n = (i32)(uintptr_t)*c;
	c++;
	while(n--) {
		if(cx <= 0 || cy <= 0)
			break;
...
void DrawRect(Draw& w, i32 x, i32 y, i32 cx, i32 cy, const Image& img, bool ra) {
	w.Clip(x, y, cx, cy);
	Size sz = img.GetSize();
	for(i32 a = ra ? x : x / sz.cx * sz.cx; a < x + cx; a += sz.cx)
		for(i32 b = ra ? y : y / sz.cy * sz.cy ; b < y + cy; b += sz.cy)
			w.DrawImage(a, b, img);
	w.End();
}
...
void DrawDragLine(Draw& w, bool horz, i32 x, i32 y, i32 len, i32 n, i32k *pattern, Color color, i32 animation)
{
	if(len <= 0)
		return;
	if(horz)
...
void DrawTiles(Draw& w, i32 x, i32 y, i32 cx, i32 cy, const Image& img) {
	w.Clip(x, y, cx, cy);
	Size sz = img.GetSize();
	for(i32 a = x; a < x + cx; a += sz.cx)
		for(i32 b = y; b < y + cy; b += sz.cy)
			w.DrawImage(a, b, img);
	w.End();
}
...
void DrawHighlightImage(Draw& w, i32 x, i32 y, const Image& img, bool highlight,
...
Image (*render_glyph)(i32 cx, i32 x, Font font, i32 chr, i32 py, i32 pcy, Color fg, Color bg);
