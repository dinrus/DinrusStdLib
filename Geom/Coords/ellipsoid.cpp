...
const Index<i32>& GisEllipsoid::EnumEPSG()
{
	static Index<i32> eem;
	if(eem.IsEmpty())
	{
		eem.Reserve(__countof(epsg_ellipsoids));
		for(i32 i = 0; i < __countof(epsg_ellipsoids); i++)
			eem.Add(epsg_ellipsoids[i].code);
	}
	return eem;
}

GisEllipsoid GisEllipsoid::GetEPSG(i32 code)
{
	i32 c = EnumEPSG().Find(code);
	GisEllipsoid out;
	if(c >= 0)
	{
