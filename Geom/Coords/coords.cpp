...
GisCoords::Arg GisCoords::Arg::Edit(double& v, Txt ident, Txt name, Txt help_topic, double min, double max, bool not_null)
{
	Arg out;
	out.vtype = DOUBLE_V;
	out.ident = ident;
...
GisCoords::Arg GisCoords::Arg::Angle(double& v, Txt ident, Txt name, Txt help_topic, double min, double max, bool not_null)
{
	Arg out;
	out.vtype = DOUBLE_V;
	out.ident = ident;
...
GisCoords::Arg GisCoords::Arg::Edit(Txt& v, Txt ident, Txt name, Txt help_topic, bool not_null)
{
	Arg out;
	out.vtype = TXT_V;
	out.ident = ident;
...
GisCoords::Arg GisCoords::Arg::Edit(Date& d, Txt ident, Txt name, Txt help_topic, bool not_null)
{
	Arg out;
	out.vtype = DATE_V;
	out.ident = ident;
...
GisCoords::Arg GisCoords::Arg::Edit(Time& t, Txt ident, Txt name, Txt help_topic, bool not_null)
{
	Arg out;
	out.vtype = TIME_V;
	out.ident = ident;
...
GisCoords::Arg GisCoords::Arg::DropList(Txt& v, Txt ident, Txt name, Txt help_topic, Txt variants, bool not_null)
{
	Arg out;
	out.vtype = TXT_V;
	out.ident = ident;
...
{
	virtual i32        GetType()                               { return UNKNOWN_V; }
	virtual Val      GetVal(ukk x)                 { return *(const bool *)x ? 1 : 0; }
	virtual void       SetVal(uk x, const Val& v)       { *(bool *)x = !DRX::IsNull(v) && (double)v; }
	virtual void       SetNull(uk x)                        { *(bool *)x = false; }
...
GisCoords::Arg GisCoords::Arg::Option(bool& b, Txt ident, Txt name, Txt help_topic)
{
	Arg out;
	out.vtype = INT_V;
	out.ident = ident;
...
GisCoords::Arg GisCoords::Arg::Option3(i32& b, Txt ident, Txt name, Txt help_topic)
{
	Arg out;
	out.vtype = INT_V;
	out.ident = ident;
...
i32 GisCoords::Data::GetBranchCount() const
{
	return 1;
}

Array<Pointf> GisCoords::Data::LonLatBoundary(const Rectf& lonlat_extent, i32 b) const
{
	Array<Pointf> out;
	out.SetCount(5);
...
double GisCoords::Data::ProjectDeviation(Pointf lonlat1, Pointf lonlat2, i32 branch) const
{
...
double GisCoords::Data::ProjectRatio(Pointf lonlat, i32 branch) const
{
...
bool GisCoords::Data::IsIdentity(const Data *data) const
{
	...
	if(args.GetCount() != args2.GetCount())
		return false;
	for(i32 i = 0; i < args.GetCount(); i++)
		if(args[i].name != args2[i].name
		|| args[i].vtype != args2[i].vtype
		|| Val(args[i].ref) != Val(args2[i].ref))
			return false;
	return true;
}
