...
Txt FormatDegree(double degrees, i32 decimals, bool spaces = true);
...
#ifdef REF_DEBUG
	i32            GetAllocIndex() const { return allocindex; }
#endif//REF_DEBUG
...
public:
	...
	dword           GetHashVal() const                   { return DRX::GetHashVal((unsigned)(uintptr_t)t); }
	...
	Txt          ToTxt() const                       { return t ? AsTxt(*t) : Txt("NULL"); }
...
{
public:
	ConvertDegree(i32 decimals = 3, bool not_null = true, double min = -360, double max = +360);
	virtual ~ConvertDegree();

	virtual Val    Format(const Val& v) const;
	virtual Val    Scan(const Val& v) const;
	virtual i32      Filter(i32 c) const;

	ConvertDegree&   Decimals(i32 d)                 { decimals = d; return *this; }
	i32              GetDecimals() const             { return decimals; }
...
public:
	...
	void           Dump(double xmin, double xmax, i32 steps) const;
};

class GisInverse : public GisFunction
{
public:
	GisInverse(double xmin, double xmax, const GisFunction& fn, i32 sections, double epsilon);
...
public:
	GisInterpolator() {}
//	Interpolator(Callback2<double, double&> calc, double xmin, double xmax, i32 min_depth, i32 max_depth, double epsilon)
//	{ Create(calc, xmin, xmax, min_depth, max_depth, epsilon); }
	GisInterpolator(double xmin, double xmax, const GisFunction& fn, i32 buckets, i32 sections, i32 samples) { Create(xmin, xmax, fn, buckets, sections, samples); }

	void           Clear()                      { bucket_index.Clear(); abc.Clear(); }
	bool           IsEmpty() const              { return bucket_index.IsEmpty(); }

//	void           Create(Callback2<double, double&> calc, double xmin, double xmax, i32 min_depth, i32 max_depth, double epsilon);
	void           Create(double xmin, double xmax, const GisFunction& fn, i32 buckets, i32 sections, i32 samples);
	void           CreateInverse(double xmin, double xmax, const GisFunction& fn, i32 buckets, i32 sections, i32 samples);
	Txt         CreateDump(double xmin, double xmax, const GisFunction& fn, i32 buckets, i32 sections, i32 samples, i32 check);
	Txt         CreateInverseDump(double xmin, double xmax, const GisFunction& fn, i32 buckets, i32 sections, i32 samples, i32 check);
	Txt         Dump(const GisFunction& fn, i32 check);
...
/*
	...
	Vec<double> abc;
*/
//	Vec<Bucket> bucket_list;
	Vec<unsigned> bucket_index;
	Vec<double> abc;
//	Rectf     extent;
//	double         divisor;
//	Vec<double> y;
...
public:
	...
	const Gis2DPolynome *GetBlock(i32 x, i32 y) const;
	const Gis2DPolynome *GetBlock(Point pt) const { return GetBlock(pt.x, pt.y); }

	i32    SizeOf() const;

public:
	BiVec< BiArray<Gis2DPolynome> > block_rows;
	Sizef block_size;
	Rect  block_span;
	Rect  block_limit;
};
...
public:
	...
	static const Index<i32>&  EnumEPSG();
	static GisEllipsoid       GetEPSG(i32 code);
...
public:
	...
	struct Arg
	{
		...
		static Arg Edit(double& v, Txt ident, Txt name, Txt help_topic = Null, double min = Null, double max = Null, bool not_null = true);
		static Arg Angle(double& v, Txt ident, Txt name, Txt help_topic = Null, double min = -2 * M_PI, double max = +2 * M_PI, bool not_null = true);
		static Arg Edit(Txt& v, Txt ident, Txt name, Txt help_topic = Null, bool not_null = true);
		static Arg Edit(Date& d, Txt ident, Txt name, Txt help_topic = Null, bool not_null = true);
		static Arg Edit(Time& t, Txt ident, Txt name, Txt help_topic = Null, bool not_null = true);
		static Arg DropList(Txt& v, Txt ident, Txt name, Txt help_topic, Txt variants, bool not_null = true);
		static Arg Option(bool& b, Txt ident, Txt name, Txt help_topic = Null);
		static Arg Option3(i32& b, Txt ident, Txt name, Txt help_topic = Null);

		Arg&       Cond(Txt ident, Val value) { cond_ident = ident; cond_value = value; return *this; }
	...
	struct ConstArg
	{
		ConstArg(Txt ident, Txt name, Val value, Txt help_topic = Null)
	...
	public:
		...
		virtual i32                   GetBranchCount() const;
		virtual Array<Pointf>         LonLatBoundary(const Rectf& lonlat_extent, i32 branch) const;
		...
		virtual Pointf                Project(Pointf lonlat, i32 branch) const = 0;
		virtual Rectf                 ProjectExtent(const Rectf& lonlat_extent) const;
		Pointf                        Project3D(Pointf3 xyz, i32 branch) const        { return Project(ellipsoid.From3D(xyz), branch); }
		virtual double                ProjectDeviation(Pointf lonlat1, Pointf lonlat2, i32 branch) const;
		virtual double                ProjectRatio(Pointf lonlat, i32 branch) const;
...
//		virtual Txt                ToTxt() const = 0;
	...
	GisCoords(const Val& v)                                                                  { if(IsTypeRaw<GisCoords>(v)) *this = ValTo<GisCoords>(v); }
	...
	i32                   GetBranchCount() const                                               { return data->GetBranchCount(); }
	Array<Pointf>         LonLatBoundary(const Rectf& lonlat_extent, i32 branch) const         { return data->LonLatBoundary(lonlat_extent, branch); }
	...
	Pointf                Project(Pointf lonlat, i32 branch) const                             { return data->Project(lonlat, branch); }
	Rectf                 ProjectExtent(const Rectf& lonlat_extent) const                      { return data->ProjectExtent(lonlat_extent); }
	Pointf                Project3D(Pointf3 xyz, i32 branch) const                             { return data->Project3D(xyz, branch); }
	double                ProjectDeviation(Pointf lonlat1, Pointf lonlat2, i32 branch) const   { return data->ProjectDeviation(lonlat1, lonlat2, branch); }
	double                ProjectRatio(Pointf lonlat, i32 branch) const                        { return data->ProjectRatio(lonlat, branch); }
...
//	Txt                ToTxt() const                                                     { return data->ToTxt(); }
	Txt                GetIdent() const                                                     { return !!data ? data->ident : Txt::GetVoid(); }
	i32                   GetEPSGCode() const                                                  { return !!data ? data->epsg_code : (i32)Null; }
	Txt                GetName() const                                                      { return !!data ? data->name : Txt::GetVoid(); }
	Txt                GetDescription() const                                               { return !!data ? data->description : Txt::GetVoid(); }
	Txt                GetCoordsys() const                                                  { return !!data ? data->coordsys : Txt::GetVoid(); }
	...
	static const Vec<i32>& EnumEPSG();
	static GisCoords      GetEPSG(i32 code);
