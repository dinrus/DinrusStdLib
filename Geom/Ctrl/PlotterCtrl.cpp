...
PlotterCtrl::ViewPlot::ViewPlot(PlotterCtrl& ctrl, i32 extra_gap)
: viewdraw(&ctrl)
{
...
PlotterCtrl::ViewPlot::ViewPlot(PlotterCtrl& ctrl, Pointf scale, Pointf delta, i32 extra_gap)
: viewdraw(&ctrl)
{
...
PlotterCtrl::ViewPlot::ViewPlot(PlotterCtrl& ctrl, const Matrixf& preform, i32 extra_gap)
: viewdraw(&ctrl)
{
	Plotter::Set(viewdraw, ctrl.scale, ctrl.delta, extra_gap);
...
, reset_push(false)
{
	...
	horz_in  .SetImage(PlotterImg::view_zoom_in())       .NoWantFocus() <<= OTVET(UserZoomInX);
	horz_out .SetImage(PlotterImg::view_zoom_out())      .NoWantFocus() <<= OTVET(UserZoomOutX);
	horz_full.SetImage(PlotterImg::view_zoom_horz_full()).NoWantFocus() <<= OTVET(UserZoomFullX);
	vert_in  .SetImage(PlotterImg::view_zoom_in())       .NoWantFocus() <<= OTVET(UserZoomInY);
	vert_out .SetImage(PlotterImg::view_zoom_out())      .NoWantFocus() <<= OTVET(UserZoomOutY);
	vert_full.SetImage(PlotterImg::view_zoom_vert_full()).NoWantFocus() <<= OTVET(UserZoomFullY);
	full     .SetImage(PlotterImg::view_zoom_full_icon()).NoWantFocus() <<= OTVET(UserZoomFull);
	AddFrame(*this);
	hscroll <<= OTVET(OnHScroll);
	vscroll <<= OTVET(OnVScroll);
	hscroll.WhenLeftClick = vscroll.WhenLeftClick = OTVET(DoSetFocus);
	hscroll.SetLine(50);
	vscroll.SetLine(50);
}
...
{
	i32 box = (show_scroll ? ScrollBarSize() : 0);
	Rect out = rc;
	rc.right -= box;
	rc.bottom -= box;
	i32 sbx = (IsAspectLocked() ? 0 : box);
...
void PlotterCtrl::AdjustPos(Point scpos, i32 xymask)
{
	if(!IsNull(extent))
	{
		Pointf d = delta;
...
void PlotterCtrl::ZoomX(double min, double max, bool add_gap)
{
	if(min != max)
	{
		i32 gl = (add_gap ? gap.left : 0), gr = (add_gap ? gap.right : 0);
		i32 s = GetSize().cx;
...
{
	if(min != max) {
		i32 gt = (add_gap ? gap.top : 0), gb = (add_gap ? gap.bottom : 0);
		i32 s = GetSize().cy;
...
{
	tcb_refresh.KillSet(1, OTVET(Refresh0));
}

void PlotterCtrl::Paint(Draw& draw)
{
	i32 level = draw.GetCloffLevel();
	try {
...
bool PlotterCtrl::Key(dword key, i32 repcnt)
{
	bool dd_key = false;
	if(drag_drop) {
		lock_drag_drop = true;
...
void PlotterCtrl::MouseWheel(Point p, i32 zdelta, dword keyflags)
{
	if(keyflags & K_SHIFT) {
		...
		i32 zlim = minmax(zdelta, -1200, +1200);
...
{
	mouse_pos = FromClient(DRX::GetMousePos() - Size(GetScreenView().TopLeft()));
	RefreshPos();
	WhenMousePos();
}
...
{
	bar.AddMenu(t_("Увеличение по гориз."), PlotterImg::view_zoom_in(), OTVET(OnViewZoomInX))
...
{
	bar.AddMenu(t_("Уменьшение по гориз.."), PlotterImg::view_zoom_out(), OTVET(OnViewZoomOutX))
...
{
	bar.AddMenu(t_("Полный зум гориз."), PlotterImg::view_zoom_horz_full(), OTVET(OnViewZoomFullX))
...
{
	bar.AddMenu(t_("Увеличение по верт."), PlotterImg::view_zoom_in(), OTVET(OnViewZoomInY))
...
{
	bar.AddMenu(t_("Уменьшение по верт."), PlotterImg::view_zoom_out(), OTVET(OnViewZoomOutY))
...
{
	bar.AddMenu(t_("Полный зум верт.."), PlotterImg::view_zoom_vert_full(), OTVET(OnViewZoomFullY))
...
{
	bar.Add(t_("Уменьшение"), PlotterImg::view_zoom_out(), OTVET(OnViewZoomOut))
...
{
	bar.Add(t_("Увеличение"), PlotterImg::view_zoom_in(), OTVET(OnViewZoomIn))
...
{
	bar.Add(!aspect, t_("Сомкнуть аспектное соотношение"), PlotterImg::view_aspect_lock(), OTVET(OnViewAspectLock))
		.Check(aspect_lock)
...
{
	bar.Add(t_("Панирование"), PlotterImg::view_pan(), OTVET(OnViewPan))
