...
{
	dialog.chamfer.Enable((i32)~dialog.miter == PathStyle::MITER_SHARP);
...
static double CalcDecadicStep(double scale, i32 minfine, i32 mincoarse, i32& fine)
{
	...
	i32 exp = ffloor(log10(mu));
	double lo = pow(10.0, exp);
	i32 tc = (lo * 5 <= mu ? 5 : lo * 2 <= mu ? 2 : 1);
...
{
	i32 version = StreamHeading(stream, 4, 4, 4, "PathEditorCtrl::Setup");
	if(version >= 1)
	{
...
: style(0)
{
	...
	zoom_horz_in   <<= OTVET(OnViewZoomHorzIn);
	zoom_horz_out  <<= OTVET(OnViewZoomHorzOut);
	zoom_vert_in   <<= OTVET(OnViewZoomVertIn);
	zoom_vert_out  <<= OTVET(OnViewZoomVertOut);
	zoom_horz_full <<= OTVET(OnViewZoomHorzFull);
	zoom_vert_full <<= OTVET(OnViewZoomVertFull);
	zoom_full      <<= OTVET(OnViewZoomFull);
...
//	sample_size.SetRect(LayoutZoom(80, 0));
	sample_size <<= 40;
	sample_size <<= OTVET(OnSampleSize);
	sample_width_tag.SetLabel("Љншka: ");
	sample_width.NotNull().MinMax(0, MAX_WIDTH);
	sample_width <<= OTVET(OnSampleWidth);
	sample_width <<= 2;
...
//	hruler.Units(1, 5);
	...
	vruler.MinMax(-5, 5);
//	vruler.Units(1, 5);
	hscroll <<= vscroll <<= OTVET(OnScroll);
...
void PathEditorCtrl::FrameLayout(Rect& rc)
{
	...
	Size avail = rc.Size();
	i32 box = ScrollBarSize();
	i32 rsz = RulerCtrl::GetStdWidth();
	if(setup.do_ruler)
		avail -= rsz;
...
void PathEditorCtrl::UpdateScroll()
{
	...
	i32 hpos = GAP - fround(hruler.GetDelta());
	i32 vpos = GAP + fround(style->width * vruler.GetScale() / 2 - vruler.GetDelta());
	Layout();
	hscroll = hpos;
	vscroll = vpos;
	OnScroll();
...
void PathEditorCtrl::OnScroll()
{
	...
	if(!vscroll.GetRect().IsEmpty())
		dy = GAP + style->width * vruler.GetScale() / 2 - (i32)vscroll;
...
void PathEditorCtrl::Paint(Draw& draw)
{
	...
	AreaTool area;
	PathTool path;
	for(i32 i = 0; i < traces.GetCount(); i++)
	{
		const PathStyle::Trace& trace = traces[i];
		i32 s = (selection.Find(i) >= 0 ? i == selection.Top() ? 2 : 1 : 0);
	...
	if(!selection.IsEmpty())
	{
		...
		for(i32 i = 0; i < __countof(list); i++)
...
i32 PathEditorCtrl::AddObject(const PathStyle::Trace& trace)
{
	i32 i = traces.GetCount();
	traces.Add(trace);
	RefreshObject(i);
	Update();
	UpdateSample();
...
void PathEditorCtrl::SetObject(i32 i, const PathStyle::Trace& trace)
{
	RefreshObject(i);
	traces[i] = trace;
	RefreshObject(i);
...
void PathEditorCtrl::RefreshObject(i32 item)
{
...
bool PathEditorCtrl::WriteClipboard()
{
	if(!IsSelection())
		return false; // no-op
	PathStyle::Clip clip;
	for(i32 i = 0; i < selection.GetCount(); i++)
...
bool PathEditorCtrl::ReadClipboard()
{
	...
	{
		i32 r = PromptYesNoCancel("Vybranй objekty pшesahujн limit danэ љншkou ибry. "
			"Chcete љншku ибry upravit?");
		if(r < 0)
			return true;
		setwd = !!r;
	}
	...
	{
		i32 r = PromptYesNoCancel("Vybranй objekty pшesahujн limit danэ dйlkou segmentu. "
...
Rectf PathEditorCtrl::GetSelectionExtent() const
{
	...
	for(i32 i = 1; i < selection.GetCount(); i++)
		rc |= traces[selection[i]].GetExtent();
	return rc;
}

i32 PathEditorCtrl::GetSelectionLeader() const
{
	return selection.IsEmpty() ? -1 : selection.Top();
}

void PathEditorCtrl::AddSelection(const Vec<i32>& list)
{
	i32 old = GetSelectionLeader();
...
void PathEditorCtrl::XorSelection(const Vec<i32>& list)
{
	i32 old_leader = GetSelectionLeader();
	for(i32 i = 0; i < list.GetCount(); i++)
	{
		RefreshObject(list[i]);
		i32 pos = selection.Find(list[i]);
		if(pos >= 0)
			selection.Remove(pos);
		else
			selection.Add(list[i]);
	}
	i32 new_leader = GetSelectionLeader();
...
void PathEditorCtrl::RemoveSelection(const Vec<i32>& list)
{
	i32 count = selection.GetCount();
	for(i32 i = 0; i < list.GetCount(); i++)
	{
		i32 pos = selection.Find(list[i]);
		if(pos >= 0)
...
void PathEditorCtrl::SetSelection(const Vec<i32>& list)
{
	ClearSelection();
...
bool PathEditorCtrl::Key(dword key, i32 repcnt)
{
	Size shift(0, 0);
	switch(key)
	{
...
void PathEditorCtrl::RightDown(Point pt, dword keyflags)
{
	...
	MenuBar::Execute(OTVET(ToolEdit));
}
...
void PathEditorCtrl::Drag(Point pt, Point last, Point next, dword keyflags)
{
	...
	Pointf up = ClientToSnap(pt);
//	i32 rop = SetROP2(plotter.draw, R2_NOTXORPEN);
...
i32 PathEditorCtrl::FindObject(Pointf pt) const
{
	enum { TOLERANCE = 10 };
...
Vec<i32> PathEditorCtrl::FindObject(const Rectf& rc) const
{
	Vec<i32> list;
	for(i32 i = 0; i < traces.GetCount(); i++)
...
i32 PathEditorCtrl::GetTrackStyle(Pointf pt, Pointf& track_start) const
{
...
i32 PathEditorCtrl::GetDragSize(Pointf pt) const
{
...
void PathEditorCtrl::ToolEdit(Bar& bar)
{
	...
	ToolEditDelete(bar);
	bar.Separator();
//	bar.Add("Zбkladnн", OTVET(ToolEditBasic))
...
{
	bar.Add(IsSelection(), "Kopнrovat", CtrlImg::copy(), OTVET(OnEditCopy))
...
{
	bar.Add(IsSelection(), "Vyjmout", CtrlImg::cut(), OTVET(OnEditCut))
...
{
	bar.Add("Vloћit", CtrlImg::paste(), OTVET(OnEditPaste))
...
{
	bar.Add("Vybrat vљe", OTVET(OnEditSelectAll))
		.Key(K_CTRL_A)
		.Help("Oznaиit vљechny ъseky jako vybranй");
}
...
{
	bar.Add(IsSelection(), "Barva...", PathImg::edit_color(), OTVET(OnEditColor))
		.Key(K_CTRL_R)
...
{
	bar.Add(IsSelection(), "Smazat", CtrlImg::remove(), OTVET(OnEditDelete))
		.Key(K_DELETE)
...
{
	RefreshSelection();
	Vec<i32> sel = selection.PickKeys();
	selection.Clear();
	Sort(sel);
	while(!sel.IsEmpty())
...
{
	bar.Add("Zvмtљit", PathImg::view_zoom_in(), OTVET(OnViewZoomIn))
		.Check(edit_mode == EDIT_ZOOM)
		.Help("Zvмtљit vybranй mнsto nebo oblast");
}
...
{
	bar.Add("Zmenљit", PathImg::view_zoom_out(), OTVET(OnViewZoomOut))
		.Help("Zmenљit mмшнtko zobrazenн stylu ибry");
}
...
{
	bar.Add("Podle okna", PathImg::view_zoom_full(), OTVET(OnViewZoomFull))
...
{
	bar.Add("Horiz. zvмtљit", OTVET(OnViewZoomHorzIn))
		.Help("Zvмtљit mмшнtko vodorovnй osy");
}

void PathEditorCtrl::OnViewZoomHorzIn()
{
	i32 half = GetSize().cx >> 1;
...
{
	bar.Add("Horiz. zmenљit", OTVET(OnViewZoomHorzOut))
		.Help("Zmenљit mмшнtko vodorovnй osy");
}
...
{
	bar.Add("Vert. zvмtљit", OTVET(OnViewZoomVertIn))
		.Help("Zvмtљit mмшнtko svislй osy");
}

void PathEditorCtrl::OnViewZoomVertIn()
{
	i32 half = GetSize().cy >> 1;
...
{
	bar.Add("Vert. zmenљit", OTVET(OnViewZoomVertOut))
		.Help("Zmenљit mмшнtko svislй osy");
}
...
{
	bar.Add("Posunout", PathImg::view_pan(), OTVET(OnViewPan))
		.Check(edit_mode == EDIT_PAN)
		.Help("Posunout myљн zobrazenэ vэшez");
}
...
{
	bar.Add("Mшнћka", PathImg::setup_grid(), OTVET(OnSetupGrid))
		.Check(setup.do_grid)
		.Help("Zobrazit / skrэt pomocnou mшнћku");
}
...
{
	bar.Add("Pravнtko", PathImg::setup_ruler(), OTVET(OnSetupRuler))
...
{
	bar.Add("Osa", PathImg::setup_axis(), OTVET(OnSetupAxis))
		.Check(setup.do_axis)
...
{
	bar.Add("Pшichytit do mшнћky", PathImg::setup_snap(), OTVET(OnSetupSnap))
		.Check(setup.do_snap)
		.Help("Pшichytit souшadnice myљi do mшнћky");
}
...
{
	bar.Add("Styl ибry", OTVET(OnSetupStyle))
...
{
	bar.Add("Editor", PathImg::setup_setup(), OTVET(OnSetupSetup))
...
public:
	...
	bool              Run(PathStyle& style, const Txt& title);
	void              Serialize(Stream& stream);
	static ConfigItem& config();

	void              Rescan() { tool_bar.Set(OTVET(ToolRoot)); menu_bar.Set(OTVET(ToolRoot)); }
...
bool RunDlgPathEditor(PathStyle& style, const Txt& title) { return DlgPathEditor().Run(style, title); }
...
DlgPathEditor::DlgPathEditor()
{
	...
	coords.SetReadOnly();
	menu_bar.Set(OTVET(ToolRoot));
	tool_bar.Set(OTVET(ToolRoot));
	dialog << path.SizePos();
	dialog.Sizeable().Zoomable();
	dialog.SetMinSize(Size(300, 200));
	path.WhenRescan = OTVET(Rescan);
}

bool DlgPathEditor::Run(PathStyle& style, const Txt& title)
{
	dialog.Title(Nvl(title, "Styl ибry").ToWTxt());
	ReadConfigSelf();
	path.SetOwner(style);
...
{
	bar.Add("Uloћit a zavшнt", CtrlImg::save(), OTVET(OnEditSave))
		.Key(K_CTRL_S)
		.Help("Zavшнt editor a pouћнt nadefinovanэ styl ибry");
}
...
template <class T>
bool WithPopupEdit<T>::Key(dword key, i32 repcnt)
{
	if(key == K_ENTER)
	{
		if(this->Accept())
...
PathStyleMapCtrl::PathStyleMapCtrl()
{
	...
	scroll.WhenScroll = OTVET(OnScroll);
	AddFrame(scroll);
	WhenBar = OTVET(ToolLocal);
}
...
bool PathStyleMapCtrl::Key(dword key, i32 repcnt)
{
	i32 c = cursor;
	i32 pg = max(1, GetSize().cy / pos_add.cx - 1);
	switch(key)
	{
	case K_LEFT:     c--; break;
...
Rect PathStyleMapCtrl::GetEditRect(i32 right, i32 bottom)
{
	Size size = PathImg::rename().GetSize();
	return RectC(right - size.cx, bottom - size.cy, size.cx, size.cy);
}

bool PathStyleMapCtrl::InEditRect(i32 right, i32 bottom, Point mouse)
{
...
i32 PathStyleMapCtrl::ClientToIndex(Point pt) const
{
...
Size PathStyleMapCtrl::ClientToRange(const Rect& rc) const
{
	if(rc.IsEmpty())
		return Size(-1, -1);
	i32 top = idivfloor(rc.top - gap_offset.y + scroll_pos, pos_add.cy);
	i32 bottom = idivfloor(rc.bottom - 1 - gap_offset.y + scroll_pos, pos_add.cy);
	i32 left = idivfloor(rc.left - gap_offset.x, pos_add.cx);
	i32 right = idivfloor(rc.right - 1 - gap_offset.x, pos_add.cx);
...
Rect PathStyleMapCtrl::IndexToClient(i32 i) const
{
...
void PathStyleMapCtrl::RightDown(Point pt, dword keyflags)
{
	...
	if(IsValid(i))
		SetCursor(i);
	SetWantFocus();
	MenuBar::Execute(this, WhenBar, DRX::GetMousePos());
}

void PathStyleMapCtrl::SetCursor(i32 c)
{
	RefreshItem(cursor);
...
void PathStyleMapCtrl::RefreshItem(i32 i)
{
...
{
	bar.Add(IsCursor(), "Upravit", OTVET(OnEdit));
}
...
{
	bar.Add(IsCursor(), "Pшejmenovat", OTVET(OnRename));
}
...
void PathStyleMapCtrl::OnRename()
{
	...
	while(pedit.PopUp(this, rc))
	{
		...
		{ // duplicate name
			Exclamation("Styl ибry [* \1" + n + "\1] jiћ v systйmu existuje. Zadejte prosнm jinэ nбzev.");
			continue;
		}
		i32 x = map->GetSort(i);
		map->Rename(x, n);
...
{
	bar.Add("Duplikovat", OTVET(OnCopy));
}
...
void PathStyleMapCtrl::OnCopy()
{
	...
	{
		i32 c = GetCursor();
		Txt prefix = map->GetSortName(c);
		i32 l = prefix.GetLength();
		...
		Txt q = map->GetUniqueName(prefix);
		map->Set(q, new_style);
		map->Touch();
		Layout();
		SetCursor(q);
...
void PathStyleMapCtrl::OnExport()
{
	if(map->IsEmpty())
	{
		PromptOK("V systйmu nebyly nalezeny ћбdnй styly иar.");
		return;
	}
	FileSelector fsel;
...
void PathStyleMapCtrl::OnImport()
{
	...
	if(!fsel.ExecuteOpen("Importovat styly иar"))
		return;
	Txt imp = LoadFile(recent = ~fsel);
	if(imp.IsVoid())
	{
		PromptOK("Nelze naинst soubor [* \1" + ~fsel + "\1].");
		return;
	}
	i32 old_count = map->GetCount();
	Txt old_name = map->GetName();
	try
	{
		map->Import(imp);
	}
...
public:
	...
	bool                 Run(PathStyleMap& map, Txt& style, bool editor = false, bool read_only = false);
...
private:
	...
	PathStyleMap        *map;
//	const Vec<PathStyleMap *> *maps;
//	Vec<i32> map_index;
};

bool RunDlgPathStyleMap(PathStyleMap& map, Txt& style, bool editor, bool read_only)
...
//	dialog.browser.WhenRename = OTVET(UpdateMaps);
	dialog.tab.Hide();
//	dialog.tab <<= OTVET(OnTab);
}

bool DlgPathStyleMap::Run(PathStyleMap& m, Txt& style, bool editor, bool read_only)
{
	if(editor) {
		dialog.ok.Hide();
		dialog.cancel.SetLabel(t_("Отмена"));
	}
...
//	UpdateMaps();
	...
	while(dialog.Run() == IDOK) {
		i32 c = dialog.browser.GetCursor();
		if(c >= 0) {
...
void DlgPathStyleMap::UpdateMaps()
{
	...
	dialog.tab.Clear();
	i32 new_tab = -1;
	for(i32 i = 0; i < map_index.GetCount(); i++)
	{
		i32 x = map_index[i];
