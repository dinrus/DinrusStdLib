...
/*
static bool ContainsPoints(const Point *poly, i32 count, Point P)
{
	if(count <= 2 || IsNull(P))
		return false;
	bool in = false;
	...
	for(Point B; --count >= 0; A = B)
	{
		...
		{
			i32 s = (B.x - A.x) * (P.y - A.y) - (B.y - A.y) * (P.x - A.x);
			if(ap.cy > 0)
				s = -s;
			if(s == 0)
				return true;
...
*/
static inline i32 PolyCompare(Pointf P, Pointf A, Pointf B, Size ap, Size bp)
{
	if((ap.cx < 0 && bp.cx < 0) || (ap.cy > 0 && bp.cy > 0) || (ap.cy < 0 && bp.cy < 0))
		return -1;
	if((ap.cx | ap.cy) == 0 || (bp.cx | bp.cy) == 0)
...
{
	PolyPart(Rectf rc, i32 first, i32 begin, i32 end)
		: Rectf(rc), first(first), begin(begin), end(end) {}
	i32 first, begin, end;
};

static Rectf SplitPoly(const Array<Pointf>& polygon, const Vec<i32>& polyend, bool closed,
	Array<PolyPart>& parts)
{
	...
	Rectf sum = Null;
	i32 i = 0;
	for(i32 p = 0; p < polyend.GetCount(); p++)
	{
		i32 e = polyend[p], f = (closed ? e - 1 : i++);
		while(i < e)
		{
...
i32 ContainsPoints(const Array<Pointf>& polygon, const Vec<i32>& polyend, const Array<Pointf>& points)
{
	...
	for(i32 p = 0; p < points.GetCount(); p++)
	{
		...
		bool in = false;
		for(i32 pp = 0; pp < parts.GetCount(); pp++)
		{
			...
			Size ap(cmp(A -> x, P.x), cmp(A -> y, P.y));
			for(i32 j = part.begin; j < part.end; j++)
			{
...
i32 ContainsPoints(const Array<Pointf>& polygon, const Array<Pointf>& points)
{
	if(points.IsEmpty() || polygon.GetCount() <= 1)
		return CMP_OUT;
	Vec<i32> polyend;
...
i32 ContainsPoint(const Array<Pointf>& polygon, const Vec<i32>& polyend, Pointf pt)
{
...
i32 ContainsPoint(const Array<Pointf>& polygon, Pointf pt)
{
...
i32 ContainsPoly(const Array<Pointf>& chkpoly,
	const Array<Pointf>& polygon, const Vec<i32>& polyend, bool closed)
{
	...
	for(i = 0; i < pp.GetCount(); i++)
	{
		...
		for(j = 0; j < cp.GetCount(); j++)
		{
			...
			if(api && bpi)
			{
				...
				for(i32 ia = api.begin; ia < api.end; ia++)
				{
					const Pointf *B = &polygon[ia];
					Pointf AB = *B - *A;
					const Pointf *C = C0;
					for(i32 ib = bpi.begin; ib < bpi.end; ib++)
					{
//						RTIMING("Contains(Poly / Poly) -> cross");
						const Pointf *D = &chkpoly[ib];
						Pointf CD = *D - *C, AC = *C - *A, BD = *D - *B;
						i32 acd = sgn(AC % CD) + 1, bcd = sgn(BD % CD) + 1;
						i32 cab = sgn(AC % AB) + 1, dab = sgn(BD % AB) + 1;
						static i32k clss[3] = { 0376104000, 0104104104, 0000104376 };
...
i32 ContainsPoly(const Array<Pointf>& chkpoly, const Array<Pointf>& polygon, bool closed)
{
...
struct PIUtils
{
	typedef i32             PointList;
	typedef i32             Iterator;
	typedef i32             ConstIterator;
};

template <>
struct PIUtils<i32>
{
	typedef Vec<Point>   PointList;
	typedef Point          *Iterator;
	typedef const Point    *ConstIterator;

	static i32              tscale(i32 x, i32 y, i32 z)  { return iscale(x, y, z); }
	static Size             tscale(Size x, i32 y, i32 z) { return iscale(x, y, z); }
	...
	static i32              MAXVAL()                     { return INT_MAX; }
	static i32              MulVector(Size a, Size b)    { return a.cx * b.cy - a.cy * b.cx; }
};
...
{
	PolygonIterator(Rect_<T> clip, i32 max_trace_points);

	void                    Add(typename PIUtils<T>::ConstIterator vertices, i32 vertex_count, i32k *counts, i32 count_count);
	void                    Run();
	...
	void                    Step(T ymax);
	void                    Flush(i32 index, T ypos);
	void                    CheckTraces();

	void                    DumpTraces(T y) const;
	void                    DumpSegments() const;
	void                    DumpDone(i32 i) const;

	Rect_<T>                clip;
	T                       ymin, ymax;
	Rect_<T>                src_clip;
	i32                     max_trace_points;
	Array<Trace>            traces;
	Vec< typename PIUtils<T>::PointList > segments;
	typename PIUtils<T>::PointList out_vertices;
	Vec<i32>             out_counts;
};

void SplitPolygon(const Point *vertices, i32 vertex_count, i32k *counts, i32 count_count,
	Vec<Point>& out_vertices, Vec<i32>& out_counts, Rect clip, i32 max_trace_points)
{
	POLY_TIMING("SplitPolygon");
	PolygonIterator<i32> pi(clip, max_trace_points);
...
void SplitPolygon(const Vec<Point>& vertices, const Vec<i32>& counts,
	Vec<Point>& out_vertices, Vec<i32>& out_counts, Rect clip, i32 max_trace_points)
{
...
void SplitPolygon(Array<Pointf>::ConstIterator vertices, i32 vertex_count, i32k *counts, i32 count_count,
	Array<Pointf>& out_vertices, Vec<i32>& out_counts, const Rectf& clip, i32 max_trace_points)
{
	POLY_TIMING("SplitPolygon");
...
void SplitPolygon(const Array<Pointf>& vertices, const Vec<i32>& counts,
	Array<Pointf>& out_vertices, Vec<i32>& out_counts, const Rectf& clip, i32 max_trace_points)
{
	SplitPolygon(vertices.Begin(), vertices.GetCount(), counts.Begin(), counts.GetCount(),
		out_vertices, out_counts, clip, max_trace_points);
...
template <class T>
PolygonIterator<T>::PolygonIterator(Rect_<T> clip, i32 max_trace_points)
...
template <class T>
void PolygonIterator<T>::Add(typename PIUtils<T>::ConstIterator vertices, i32 vertex_count, i32k *counts, i32 count_count)
{
	...
	for(; --count_count >= 0; vertices += *counts++)
	{
		...
		do
		{
			...
			seg1.SetCount(degen1 ? 2 : (i32)(ue - ub) + 1 + (ue > ub ? 0 : *counts));
			...
			seg2.SetCount(degen2 ? 2 : (i32)(ue - ub) + 1 + (ue > ub ? 0 : *counts));
...
{
	for(i32 i = 0; i < segments.GetCount(); i++)
	{
		Txt seglog;
		seglog << "[" << i << "]: " << segments[i][0];
		for(i32 j = 1; j < segments[i].GetCount(); j++)
			seglog << " -> " << segments[i][j];
		RLOG(seglog);
	}
}
...
void PolygonIterator<T>::Run()
{
	...
	ymin = segments[0][0].y;
	for(i32 s = 0; s < segments.GetCount();)
	{
		T y = segments[s][0].y;
		Step(y);
		i32 t = 0;
...
#endif//POLY_LOGGING
			for(i32 flush = (i1 + 1) | 1; flush <= i2; flush += 2)
				Flush(flush, y);
...
void PolygonIterator<T>::Step(T ymax)
{
	...
	while(ymin < ymax && !traces.IsEmpty())
	{
		...
		i32 nremove = 0;
		bool hseg = false;
		for(i32 t = 0; t < traces.GetCount(); t++)
		{
			Trace& p = traces[t];
		...
		{
			ASSERT(!(nremove & 1));
			i32 t = traces.GetCount();
			if(t == nremove)
			{
				for(i32 i = 1; i < traces.GetCount(); i += 2)
					Flush(i, ymin);
				traces.Clear();
				return;
			}
...
#endif//POLY_LOGGING
				for(i32 flush = i2 | 1; flush >= i1; flush -= 2)
					Flush(flush, ymin);
				if(i2 == i1 + 1)
					traces.Remove(i1, 2);
				else
		...
		{ // horizontal segments present, check for horizontal swaps
			...
			for(i32 t = 1; t < traces.GetCount(); t++)
			{
				Trace *q = &traces[t];
...
template <class T>
void PolygonIterator<T>::Flush(i32 index, T ypos)
{
	...
	i32 count = out_vertices.GetCount();
#if POLY_LOGGING
	...
	if(*qb == p.done[0] && qb < qe)
		qb++;
	out_vertices.SetCountR(out_vertices.GetCount() + (i32)(qe - qb));
	...
	else
	{
		...
		for(i32 i = count; i < out_vertices.GetCount(); i++)
		{
			seg << ' ' << out_vertices[i];
...
template <class T>
void PolygonIterator<T>::DumpDone(i32 i) const
{
	Txt out;
	out << "done[" << i << "]:";
	for(i32 t = 0; t < traces[i].done.GetCount(); t++)
		out << ' ' << traces[i].done[t];
	RLOG(out);
}
