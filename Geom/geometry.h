...
//inline i32    ScalarProduct(Size a, Size b) { return a.cx * b.cx + a.cy * b.cy; }
//inline i32    VectorProduct(Size a, Size b) { return a.cx * b.cy - a.cy * b.cx; }
//inline i32    Squared(Size a)             { return a.cx * a.cx + a.cy * a.cy; }
...
i32             ContainsPoly  (const Array<Pointf>& chkpoly, const Array<Pointf>& polygon, const Vec<i32>& polyend, bool closed);
...
Vec<i32>     ConvexHullOrder(const Vec<Point>& pt);
Vec<i32>     ConvexHullOrder(const Array<Pointf>& pt);
...
void            SplitPolygon  (const Point *vertices, i32 vertex_count, i32k *counts, i32 count_count,
	Vec<Point>& out_vertices, Vec<i32>& out_counts, Rect clip = Null, i32 max_trace_points = 8000);
...
void            SplitPolygon  (Array<Pointf>::ConstIterator vertices, i32 vertex_count, i32k *counts, i32 count_count,
	Array<Pointf>& out_vertices, Vec<i32>& out_counts, const Rectf& clip = Null, i32 max_trace_points = 8000);
...
/*
class VecCurve : public Vec<VecVertex>
{
public:
	typedef Vec<VecVertex> Base;

	VecCurve();
	VecCurve(const VecCurve& another, i32);
	...
	VecArc     Segment(i32 i) const;

	Vec<Pointf> Check() const; // find self-intersections; empty return value = OK
...
public:
	i32    count; // number of intersections
	double t[2], u[2];
...
public:
	...
	VecArcIterator& Level(i32 _level)              { level = _level; return *this; }
...
public:
	...
	i32             level;     // maximum # segments = 2 ^ level
...
protected:
	...
	i32             last_clip; // clipping status of last vertex

	i32             GetClip(Pointf point) const;
	void            Recurse(Pointf to, double l, double h, i32 depth, i32 next_flclip);
};
