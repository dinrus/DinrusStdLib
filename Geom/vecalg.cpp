...
Rectf GetBoundingBox(const Array<Pointf>& vertices)
{
	...
	for(i32 i = 1; i < vertices.GetCount(); i++)
		SetUnion(rc, vertices[i]);
	return rc;
}

Rect GetBoundingBox(const Point *vertices, i32 vertex_count)
{
	if(vertex_count == 0)
		return Null;
...
Rect GetBoundingBox(const Vec<Point>& vertices)
{
...
public:
	typedef typename C::ValType P;
	ConvexHullGenerator(const C& points) : points(points) {}
	
	Vec<i32> Generate();

	bool        operator () (i32 i, i32 j) const;
	
private:
	void        Recurse(i32 ib, i32 ie, i32& ue, i32& le);
	i32         Stitch(i32 lb, i32 le, i32 rb, i32 re, bool is_lower);
...
template <class C, class VP>
Vec<i32> ConvexHullGenerator<C, VP>::Generate()
{
	...
	i32k *b = lower.GetIter(lb);
	i32k *e = lower.GetIter(le);
	for(i32 *o = out.GetIter(ue); e > b; *o++ = *--e)
		;
	return out;
}

template <class C, class VP>
bool ConvexHullGenerator<C, VP>::operator () (i32 i, i32 j) const
{
	const P& a = points[i];
	const P& b = points[j];
...
template <class C, class VP>
void ConvexHullGenerator<C, VP>::Recurse(i32 ib, i32 ie, i32 &ue, i32& le)
{
	if(ie - ib <= 2) {
		ue = le = ie;
		return;
...
template <class C, class VP>
i32 ConvexHullGenerator<C, VP>::Stitch(i32 lb, i32 le, i32 rb, i32 re, bool is_lower)
{
	Vec<i32>& index = (is_lower ? lower : upper);
	for(;;) {
...
Vec<i32> ConvexHullOrder(const Vec<Point>& points)
{
	ConvexHullGenerator<Vec<Point>, z64> generator(points);
	return generator.Generate();
}

Vec<i32> ConvexHullOrder(const Array<Pointf>& points)
{
...
void VecArcIterator::Go()
{
	...
	i32 end_clip = (IsNull(clip) ? 0 : GetClip(arc.B));
	arclen = 0;
	...
	{ // divide in two halves
		...
		i32 centre_clip = IsNull(clip) ? 0 : GetClip(centre);
...
void VecArcIterator::Recurse(Pointf next, double l, double h, i32 depth, i32 next_clip)
{
...
i32 VecArcIterator::GetClip(Pointf point) const
{
...
double VecLine::Distance(Pointf point, double* arg) const
{
	if(IsNull(point) || IsNull(*this))
		return Null;
	return DRX::Distance(point, A, B, arg);
}
...
Rectf VecArc::GetBoundingBox() const
{
	...
	Pointf C = DRX::Mid(A, B) + DRX::Right(B - A) * ((bulge - r) / l);
	i32 q1 = (A.x < C.x) ^ (A.y < C.y ? 3 : 0);
	i32 q2 = (B.x < C.x) ^ (B.y < C.y ? 3 : 0);
	if(r < 0) // swap quadrants
	{
		i32 temp = q2; q2 = q1; q1 = temp;
		r = -r;
	}
...
{
	Pointf result = DRX::Mid(P, Q);
	if(l > Vec_tolerance)
		result += DRX::Right(Q - P) * (h / l);
	return result;
}
...
//	double k4 = k2 * k2;
	...
	if(denom > Vec_tolerance)
		C += DRX::Right(Q - P) * (4 * l2 * (1 - t) / denom);
	return C;
}
...
{
	centre = DRX::Mid(P, Q) + DRX::Right(Q - P) * h / l;
	ll = sqrt(l * l / 4 + h * h);
	hh = h * ll / (2 * ll + l);
}
...
VecCurve::VecCurve(const VecCurve& another, i32)
	: Base(another, 0)
{
}
...
Txt VecCurve::Dump() const
{
	if(IsEmpty())
		return "Null(VecCurve)";
	Txt dump = '(' + (*this)[0].Dump();
	for(i32 i = 1; i < GetCount(); i++)
		dump << " -> " << (*this)[i].Dump();
	return dump + ')';
}
...
Vec<Pointf> VecCurve::Check() const
{
	Vec<Pointf> errors;
	i32 count = GetCount();
	if(count < 2)
...
VecArc VecCurve::Segment(i32 i) const
{
	i32 count = GetCount();
	ASSERT(i >= 0 && i < count);
	i32 j = i + 1;
...
void VecArcInfo::Set(Pointf _C, Pointf _A, Pointf _B, bool anticlockwise)
{
	...
	bow = A | B;
	alpha = DRX::Bearing(A - C);
	beta = DRX::Bearing(B - C);
	radius = B | C;
	circle = (A == B);
...
void VecArcInfo::Set(Pointf _P, Pointf _Q, Pointf X)
{
	...
	double x2  = DRX::Squared(X);
	Pointf     AX  = X - _P;
	Pointf     BX  = X - _Q;
	double m1x = (x2 - DRX::Squared(_P)) / 2;
	double m1y = (x2 - DRX::Squared(_Q)) / 2;
	double det = AX % BX;
...
void VecArcInfo::Set(Pointf _P, Pointf _Q, double _bulge)
{
	...
	C      = DRX::Mid(A, B) + DRX::Right(B - A) * ((_bulge - radius) / bow);
	alpha  = DRX::Bearing(A - C);
	beta   = DRX::Bearing(B - C);
...
Pointf VecArcInfo::CentreOfMass() const
{
	if(IsCircle())
		return C;
	Pointf M = DRX::Mid(A, B);
	if(!IsCurved())
		return M;
	double extrusion = bow / (beta - alpha) - radius + fabs(bulge);
	return M + UnitV(IsReversed() ? DRX::Left(M) : DRX::Right(M)) * extrusion;
}
...
{
	if(IsCircle() || (IsCurved() && ContainsBearing(DRX::Bearing(C, point))))
	{
		double distance = (point | C) + radius;
		if(farthest)
			*farthest = point + DRX::Length(C - point, radius);
		return distance;
	}
...
bool VecArcInfo::Crosses(const Rectf& R) const
{
	...
	if(!IsCurved())
		return DRX::Crosses(R, A, B);

	if(R.Contains(A) || R.Contains(B))
		return true;

	if(!DRX::Crosses(R, C, radius))
		return false;
...
double VecArcInfo::GetArg(Pointf point) const
{
	...
	return GetArg(DRX::Bearing(point, C));
}
...
{
	return DRX::Bearing(GetStartDir());
}
...
VecArcInfo VecArcInfo::Offset(double dist) const
{
	if(dist == 0)
		return *this;
	Pointf last = A + DRX::UnitV(DRX::Right(GetStartDir())) * dist;
	Pointf next = B + DRX::UnitV(DRX::Right(GetEndDir())) * dist;
	if(IsCurved())
...
void VecIntersection::Remove(i32 index)
{
