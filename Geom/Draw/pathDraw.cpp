...
static tukk ParseCTxt(tukk p, Txt& out)
{
	try
	{
		CParser parser(p);
		out = parser.ReadOneTxt();
...
static Txt FormatPrec(double d)
{
...
Txt PathStyle::Trace::Encode() const
{
	Txt out;
...
tukk PathStyle::Trace::Decode(tukk s)
{
...
i32 PathStyle::Trace::GetTrackStyle(Pointf pt, Pointf tolerance, Pointf& start) const
{
...
Image PathStyle::Trace::GetTrackCursor(i32 style)
{
	switch(style)
	{
...
PathStyle::Trace& PathStyle::Trace::Track(Pointf delta, i32 style)
{
...
{
	i32 version = StreamHeading(stream, 1, 1, 1, "PathStyle::Clip");
	if(version >= 1)
		stream % traces;
}
...
Txt PathStyleMap::Export() const
{
	Txt out;
	out << "# PathStyleMap version 1\n"
		"$" << AscTxt(name) << "\n";
	for(i32 i = 0; i < map.GetCount(); i++)
		out << AscTxt(map.GetKey(i)) << '=' << map[i].Encode() << '\n';
	return out;
}

void PathStyleMap::Import(Txt s, bool update_existing)
{
	i32 line = 1;
	try
	{
		for(tukk p = s; *p;)
			...
			{ // style name
				if(*++p != '\"')
					throw Exc(Txt().Cat() << "oèekáván znak \", nalezeno: " << TxtSample(p, 10));
				p = ParseCTxt(p + 1, name);
			}
			else if(*p == '\"')
			{
				Txt sn;
				p = ParseCTxt(p + 1, sn);
				if(*p != '=')
					throw Exc(Txt().Cat() << "oèekáváno '=', nalezeno: " << TxtSample(p, 10));
				if(IsNull(sn))
					throw Exc(Txt().Cat() << "název stylu je prázdný");
				for(p++; *p && *p != '\n' && (u8)*p <= ' '; p++)
					;
				PathStyle ps;
				p = ps.Decode(p);
				if(!p)
					throw Exc(Txt().Cat() << "neplatná definice stylu '" << name << "'");
				if(map.Find(sn) < 0 || update_existing)
					Set(sn, ps);
			}
			else
				throw Exc(Txt().Cat() << "neplatný zaèátek øádku: " << TxtSample(p, 10));
	}
	catch(Exc e)
	{
		throw Exc(Txt().Cat() << "øádek " << line << ": " << e);
	}
}

const Vec<i32>& PathStyleMap::GetSort() const
{
	if(sort.IsEmpty() && !map.IsEmpty())
		sort = GetSortOrder(map.GetKeys(), GetLangInfo());
	return sort;
}

PathStyle PathStyleMap::Scan(Txt name, const PathStyle& dflt) const
{
	if(IsNull(name))
	...
	PathStyle ps;
	tukk end = ps.Decode(name);
	if(!end || *end)
		return PathStyle(dflt, 0);
	return ps;
}

i32 PathStyleMap::FindSortName(Txt name, i32 exclude) const
{
...
Txt PathStyleMap::GetUniqueName(Txt prefix) const
{
	i32 max_index = -1;
	i32 pl = prefix.GetLength();
	for(i32 i = 0; i < GetCount(); i++)
	{
		Txt s = map.GetKey(i);
...
static void RecurseArc(Vec<Point>& out, Pointf begin, Pointf end, double l, double h, i32 maxvert)
{
...
static Vec<Point>& GetArcPath(Vec<Point>& out, Point begin, Point end, double height, i32 maxvert = 256)
{
	RecurseArc(out, begin, end, hypot(begin.x - end.x, begin.y - end.y), height, maxvert);
	return out;
}

static Vec<Point> GetArcPath(Point begin, Point end, double height, i32 maxvert = 256)
{
	Vec<Point> out;
	return GetArcPath(out, begin, end, height, maxvert);
}

void PathDraw::Output::AddNext(const Vec<Point>& list)
{
	Point old = vertices.Top();
	i32 c = vertices.GetCount();
...
void PathDraw::Output::AddThick(void (*ln)(Output& out, Point a, Point b), Point start, const Vec<Point>& list)
{
	if(!list.IsEmpty())
	{
...
Point *PathDraw::Output::AddSeg(i32 segments, i32 seg_count)
{
	i32 c = vertices.GetCount();
	i32 n = counts.GetCount();
	if(c > 0 && counts[n - 1] == 1)
		c--, n--;
...
Point *PathDraw::Output::AddSeg(i32 count)
{
	i32 c = vertices.GetCount();
	i32 n = counts.GetCount();
	if(n > 0 && counts[n - 1] == 1)
...
*/
	...
	else
	{
		...
		t.vline = (t.width <= 1);
		t.hline = 0;
		i32 lw = t.left_bottom - t.left_top;
		i32 rw = t.right_bottom - t.right_top;
		i32 sw = 0;
...
/*			if(t.width > 0)
			{
				t.start++;
				t.width = max<i32>(t.width - 2, 1);
			}
*/		}
		else if(draw->Pixels() && lw <= THICK && rw <= THICK)
		{
			sw = 0;
			t.hline = PathDraw_Output_Thick[max<i32>(0, lw - 1)]
				[max<i32>(0, rw - 1)];
...
void PathDraw::Set(Draw& _draw, const PathStyle& _style, Color _color, double _width, double _dash, bool _closed)
{
	...
	Index<i32> overlap_begin, overlap_end;
...
void PathDraw::MoveToFull(Point p)
{
	...
	if(pos[2] != Null)
		...
		case PART_SEGMENT:
			if(closed)
			{
				for(i32 i = 0; i < __countof(close_line); i++)
...
{
	double tl = 0;
	for(i32 i = 0; i < split_buffer.GetCount(); i++)
...
{
	CALC_MITER();
	for(i32 i = 0; i < segment_lines.GetCount(); i++)
	{
...
void PathDraw::LineToRaw()
{
	...
	if(!traces.IsEmpty())
	{
		...
		for(i32 o = open_traces.GetCount(); --o >= 0;)
		{
...
{
	for(i32 i = 0; i < outputs.GetCount(); i++)
		outputs[i].Flush();
}

void PathDraw::RecurseArc(Point next, i32 length, i32 bulge, i32 levels)
{
...
void PathDraw::ArcTo(Point pt, i32 bulge)
{
	ASSERT(!IsNull(raw_pos[0]));
	RecurseArc(pt, (i32)Length(pt - pos[0]), bulge, 10);
}

void PathDraw::Circle(Point pt, i32 radius)
{
...
void PathStyleDisplay::Paint(Draw& draw, const Rect& rc, const Val& v, Color i, Color p, dword s) const
{
	draw.DrawRect(rc, p);
	if(IsNull(v) || IsTxt(v))
	{
