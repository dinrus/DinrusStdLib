...
void CalcBitMasks(i32 bpp, const dword *in_cmasks, dword cmask[], i32 cshift24[], i32 cbits[])
{
...
static void PixelZStreamInfo(Stream& stream, Size& size, Size& dot_size, Point& hotspot, i32& raw_bpp, bool& mono)
{
	enum { VERSION = 1 };
	i32 version = VERSION;
...
private:
	...
	i32             pixel_row_bytes;
	i32             alpha_row_bytes;
	i32             pixel_block_size;
	i32             alpha_block_size;
	i32             line_number;
	Txt          block;
	i32             block_offset;
	Vec<u8>    pixel;
	Vec<u8>    alpha;
	Vec<RGBA>    rgba_palette;
	RasterFormat    format;
};
...
bool ZImageRaster::Create()
{
	...
	if(stream.IsLoading())
		dir.SetCount(count);
	for(i32 i = 0; i < count && !stream.IsError(); i++)
		stream % dir[i];
	if(stream.IsError() || dir.IsEmpty())
		return false;
	item = dir[0];

	Buffer<i32> offsets(dir.GetCount() + 1);
	for(i32 i = 0; i <= dir.GetCount(); i++)
		stream % offsets[i];
	z64 base = stream.GetPos();
	...
	if(item.raw_bpp) {
		i32 bpp = (item.raw_bpp < 0 ? 24 : item.raw_bpp);
		if(bpp <= 8) {
			stream % palette;
			rgba_palette.SetCount(1 << bpp, RGBAZero());
			for(i32 i = 0; i < palette.GetCount() && i < rgba_palette.GetCount(); i++)
		...
		for(i32 i = 0; i < item.size.cy;) {
			i32 e = min(item.size.cy, i + pixel_block_size);
			Txt part;
			stream % part;
			Txt dpart = ZDecompress(part);
			if(dpart.IsVoid()) {
...
/*
	if(item.alpha_bpp) {
		...
		for(i32 i = 0; i < item.size.cy;) {
			i32 e = min(item.size.cy, i + alpha_block_size);
			Txt part;
			stream % part;
			Txt dpart = ZDecompress(part);
			if(dpart.IsVoid()) {
...
Raster::Line ZImageRaster::GetLine(i32 ln)
{
	RGBA *line = new RGBA[item.size.cx];
/*
	if(item.alpha_bpp) {
		u8k *ao = alpha.GetIter(ln * alpha_row_bytes);
		u8 active = *ao++;
		u8 avail = 8;
		RGBA *out = line;
		for(i32 width = item.size.cx; --width >= 0; out++) {
			if(!avail) {
				active = *ao++;
				avail = 8;
			}
	...
	if(item.raw_bpp) {
		u8k *po = pixel.GetIter(ln * pixel_row_bytes);
		RGBA *out = line;
		if(item.raw_bpp == -3) {
			for(i32 width = item.size.cx; --width >= 0; out++, po += 3) {
				out->b = po[0];
				out->g = po[1];
				out->r = po[2];
			}
			...
			u8 shift = item.raw_bpp;
			u8 per_byte = 8 / item.raw_bpp;
			u8 active = 0;
			u8 avail = 0;
			RGBA zero = RGBAZero();
			for(i32 width = item.size.cx; --width >= 0; out++) {
			...
			u8 bshift = cshift24[2];
			u8 bmask = (-256 >> cbits[2]) & 0xFF;
			u8 gshift = cshift24[1];
			u8 gmask = (-256 >> cbits[1]) & 0xFF;
			u8 rshift = cshift24[0];
			u8 rmask = (-256 >> cbits[0]) & 0xFF;

			if(item.raw_bpp == 16) {
				for(i32 width = item.size.cx; --width >= 0; out++, po += 2) {
					u16 w = Peek16le(po);
					out->r = u8((w << rshift) >> 24) & rmask;
					out->g = u8((w << gshift) >> 24) & gmask;
					out->b = u8((w << bshift) >> 24) & bmask;
				}
			}
			else {
				for(i32 width = item.size.cx; --width >= 0; out++, po += 4) {
					u32 w = Peek32le(po);
					out->r = u8((w << rshift) >> 24) & rmask;
					out->g = u8((w << gshift) >> 24) & gmask;
					out->b = u8((w << bshift) >> 24) & bmask;
...
void RasterCopy(RasterEncoder& dest, Raster& src, const Rect& src_rc)
{
	...
	for(i32 y = src_rc.top; y < src_rc.bottom; y++)
...
void ImageWriter::WriteLineRaw(u8k *s)
{
...
Raster::Line ImageBufferRaster::GetLine(i32 line)
{
...
static z64 Unpack64(dword i)
{
	if(!(i & 0x80000000))
		return i;
	return z64(i & 0x7fffffff) << 8;
}

static dword CeilPack64(z64 i)
{
	if(i < 0x7fffffff)
		return (dword)i;
...
static void Mask1Blt(u8 *dest, u8k *src, u8k *mask, i32 count)
{
	while(count >= 4)
	{
...
/*
static void StreamAlphaBlend(Stream& stream, Rect& dest, Rect& src, i32& alpha,
...
/*
static void DrawAlphaBlend(Draw& draw, Rect dest, Rect src, i32 alpha, AlphaArray& image, Color blend_bgnd)
{
	ASSERT(alpha >= 0);
...
static i32 GetMaskInfo(const RGBA *rgba, i32 count)
{
	if(count == 0)
		return 0;
	if(rgba->a == 255) {
...
static Txt EncodeMask(const ImageBuffer& mask, bool write_size)
{
	TxtBuf out;
	if(write_size) {
		char temp[4];
...
static void DecodeMask(ImageBuffer& mask, Txt s, bool read_size)
{
	Size size = mask.GetSize();
	u8k *p = s;
	if(read_size) {
...
/*
static Txt EncodeMask(const RawImage& mask)
{
	ASSERT(mask.bpp == 8);
	Txt out;
	i32 full = 0;
	...
	for(i32 i = 0; i < size.cy; i++)
	{
		u8k *p = mask.GetUpScan(i), *e = p + size.cx;
		i32 start = out.GetLength();
		while(p < e)
		{
			u8k *b = p;
...
static void DecodeMask(RawImage& mask, const Txt& s)
{
	ASSERT(mask.bpp == 8);
	Size size = mask.GetSize();
	mask.Set(1);
	...
	for(i32 i = 0; i < size.cy && *p; i++)
	{
		u8 *d = mask.GetUpScan(i), *e = d + size.cx;
		while(*p >= 2 && d < e)
		{
			i32 n1 = *p++ - 2;
			if(e - d <= n1)
...
HRRInfo::HRRInfo(const Rectf& log_rect_, const Rectf& map_rect_,
	i32 levels_, Color background_, i32 method_, i32 quality_,
...
{
	i32 outver = (stream.IsStoring() && !mono && method != METHOD_ZIM && method != METHOD_BZM ? 4 : 5);
	i32 version = StreamHeading(stream, outver, 2, 5, "HRRInfo");
	if(version >= 2)
...
Vec<i32> HRRInfo::EnumQualities(i32 method)
{
	Vec<i32> out;
	switch(method)
	{
...
VecMap<i32, Txt> HRRInfo::GetPackMap()
{
	VecMap<i32, Txt> out;
	Vec<i32> methods = EnumMethods();
	for(i32 m = 0; m < methods.GetCount(); m++)
	{
		Vec<i32> qualities = EnumQualities(methods[m]);
		if(qualities.IsEmpty())
			qualities.Add(0);
		for(i32 q = 0; q < qualities.GetCount(); q++)
			out.FindAdd(Pack(methods[m], qualities[q]), GetName(methods[m], qualities[q]));
	}
	return out;
}

Txt HRRInfo::GetName(i32 method, i32 quality)
{
	Txt out;
	switch(method)
	{
...
double HRRInfo::GetEstimatedFileSize(i32 _levels, i32 method, i32 quality)
{
	i32 images = 0;
	for(i32 i = 0; i < _levels; i++)
		images += 1 << (2 * i);
	i32 dir_size = images * sizeof(i32) // offset table
...
HRR::HRR(tukk path, bool read_only)
{
	Open(path, read_only);
}
...
bool HRR::Open(tukk path, bool read_only)
{
	Close();
...
static i32 GetImageSize(Size sz)
{
	return sizeof(Image) + 32 + sz.cx * sz.cy * sizeof(RGBA);
}

inline static i32 GetImageSize(const Image& im) { return GetImageSize(im.GetSize()); }

void HRR::FlushCache(i32 limit)
{
...
static Color BlendColor(Color a, i32 percent, Color b)
{
	return Color(
...
static i32 StopMsec(i32 start = 0)
{
	return GetTickCount() - start;
}

static void DrawAlphaImage(Draw& draw, Rect dest, Image img, Rect src, i32 alpha)
{
	if(alpha <= 0)
		return;
	alpha += alpha >> 7;
...
void HRR::Paint(Draw& draw, Rect dest, Rectf src,
	i32 alpha, i32 max_pixel, Color mono_black, Color mono_white, Color blend_bgnd)
{
...
HRR::Cursor::Cursor(HRR& owner_, const Rectf& extent_, double measure_,
	i32 alpha_, Color mono_black_, Color mono_white_, Color blend_bgnd_)
: owner(owner_)
, extent(extent_)
, measure(measure_)
, alpha(alpha_)
...
bool HRR::Cursor::Fetch(Rectf& part)
{
	for(;;) {
		...
		i32 layer_offset = 4 * (block.x + block.y * total);
		i32 pixel_offset = 0, mask_offset = 0;
			...
			if(mask_offset) {
				...
				if(owner.version < 5) {
					...
					if(cimg >= 0)
						...
						i32 csize = owner.size_cache.Find(pixel_offset);
						if(csize < 0) {
							if(owner.size_cache.GetCount() >= 10000)
								owner.size_cache.Clear();
							z64 pixpos = Unpack64(pixel_offset);
...
//					new_image.alpha = PixelArray::Mono(sz);
			...
			i32 new_len = new_image.GetLength() * sizeof(RGBA);
	...
	if(!info.mono) {
		...
		if(decoder.IsEmpty()) {
			draw.DrawText(cdest.left, cdest.top,
				Txt().Cat() << "Неподдерживаемая кодировка HRR: " << info.GetMethod(), StdFont());
			draw.End();
...
//				Stream64Stream pixel_stream(stream, Unpack64(pixel_offset));
				...
				if(pixel_offset) {
					...
					if(new_image.IsEmpty()) {
						Txt warn = NFormat("Не удалось загрузить блок [%d, %d].", x, y);
				...
				if(mask_offset && use_alpha) {
					...
					if(version < 5) {
						...
						if(pixel_offset)
							...
							i32 csize = size_cache.Find(pixel_offset);
							if(csize < 0) {
	...
	//								Stream64Stream pixel_stream(stream, pixpos);
			...
			i32 cimg = image_cache.Find(pixel_mask);
			if(cimg < 0)
				continue;
/*
			if(cimg < 0) {
...
i32 HRR::GetProgressCount(i32 levels, bool downscale)
{
	ASSERT(levels > 0);
	i32 images = 0;
	if(downscale)
...
bool HRR::Create(const HRRInfo& _info, tukk path)
{
...
static void StreamHRRTxt(Stream& stream, Txt& string)
{
	i32 version = 1, len = string.GetLength();
	stream / version / len;
	...
	if(stream.IsStoring())
		stream.SerializeRaw((u8 *)(tukk )string, len);
	else {
		TxtBuf stringbuf(len);
		stream.SerializeRaw((u8 *)~stringbuf, len);
		string = stringbuf;
	}
}
...
void HRR::Serialize()
{
	i32 outver = (stream.IsStoring() && (info.mono || info.method >= info.METHOD_ZIM) ? 5 : 4);
...
//				stream.SerializeRaw((u8 *)pixel_directory[l].Begin(),
	...
	if(map_offset && version > 3) {
		...
		if(stream.IsStoring() || mappos >= 0 && mappos < stream.GetSize()) {
			...
			for(i32 i = 0; i < count; i++)
			{
				...
				StreamHRRTxt(stream, key);
				StreamHRRTxt(stream, val);
				if(stream.IsLoading())
					map.Add(key, val);
			}
		}
...
void HRR::Write(Writeback drawback, bool downscale)
{
	...
	if(!encoder)
		throw Exc(Txt().Cat() << "Неподдерживаемая кодировка HRR: " << info.GetMethod());
...
Matrixf HRR::GetPixMapMatrix(i32 level, i32 x, i32 y) const
{
	double fac = 1 << level;
...
z64 HRR::GetFileWriteSize() const
{
	ASSERT(stream.IsOpen());
	return stream.GetSize();
}

Rectf HRR::GetLogBlockRect(i32 level, const Rect& rc) const
{
...
bool HRR::Write(Writeback drawback, bool downscale, i32 level, i32 px, i32 py,
...
//		TIMING("HRR::Write / save (indirect)");
		if(info.mono || IsNull(info.background)) {
			i32 kind = GetMaskInfo(block.block, block.block.GetLength());
			if(kind && !info.mono) {
				i32 pixoff = CeilPack64(stream.GetPos());
...
//				pixel_directory[level][lin] = pixoff;
			...
			if(kind == 2 || (kind == 1 && info.mono)) {
				Txt s = EncodeMask(block.block, version >= 5);
				ASSERT(s.GetLength() >= 4);
				i32 maskoff = CeilPack64(stream.GetPos());
...
//			pixel_directory[level][lin] = pixoff;
			...
			while(stream.GetSize() < pixpos)
				stream.Put(0, (i32)min<z64>(pixpos - stream.GetSize(), 1 << 24));
			stream.Seek(pixpos);
			//Stream64Stream pixstream(stream, pixpos);
			format.Save(stream, ImageBufferRaster(block.block));
		}
...
void HRR::SetMap(Txt key, Txt value)
{
	if(IsNull(value))
	{
		i32 i = map.Find(key);
