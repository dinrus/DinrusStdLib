...
static void DrawFatPolyPolygonOutline(Draw& draw,
	const Point *vertices, i32 vertex_counts,
	i32k *polygon_counts, i32 polygon_count_counts,
	Color color, i32 width)
{
	if(IsNull(color))
		return;
	if(width <= 1)
	...
	enum { BATCH = 4000 };
	i32 nsegs = min<i32>(BATCH, (vertex_counts + polygon_count_counts + 1) * width);
	i32 batch = 2 * nsegs;
	Buffer<Point> buffer(batch);
	Buffer<i32> counts(nsegs);
	...
	{
		i32 vert = *polygon_counts;
		i32 vw = vert * width;
		i32 pc = 2 * vw;
		Point prev = *vertices;
		for(const Point *next = vertices + vert; next-- > vertices; prev = *next)
		{
			i32 dx = next->x - prev.x, dy = next->y - prev.y;
...
static void DrawFatPolyPolygonOutline(Draw& draw,
	const Vec<Point>& vertices, const Vec<i32>& counts,
	Color color, i32 width)
{
...
LineDraw::LineDraw(Draw& draw, LineStyle pattern, Color color, i32 width, double dash)
{
	Set(draw, pattern, color, width, dash);
}

void LineDraw::Set(Draw& _draw, LineStyle pattern, Color color, i32 width, double dash)
{
	draw = &_draw;
...
void LineDraw::RecurseArc(Point next, i32 length, i32 bulge, i32 levels)
{
	...
	{ // bisect arc
		...
		i32 ll = (i32)Length(centre - next);
		i32 hh = iscale(bulge, ll, 2 * ll + length);
		RecurseArc(centre, ll, hh, levels);
		RecurseArc(next, ll, hh, levels);
	}
}

void LineDraw::ArcTo(Point p, i32 bulge, i32 levels)
{
	if(empty)
		return;
	if(p == last)
	...
	{
		RecurseArc(p, (i32)Length(p - last), bulge, levels);
		return;
	}
	Point c = PointfToPoint(info.C);
	i32 rad = fround(info.radius);
...
void LineDraw::Circle(Point centre, i32 radius)
{
	if(empty)
		return;
...
void Plotter::Set(Draw& _draw, const Rectf& _src, const Rectf& _dest, i32 flags, i32 reserve, double meter)
{
	Rectf src = _src;
...
void Plotter::Set(Draw& _draw, Sizef _scale, Pointf _delta, i32 reserve, double meter)
{
	Set(_draw, Matrixf(Pointf(_scale.cx, 0), Pointf(0, _scale.cy), _delta), reserve, meter);
}

void Plotter::Set(Draw& _draw, const Matrixf& matrix, i32 reserve, double meter)
{
...
void Plotter::Set(const Plotter& info, i32 reserve)
{
	ASSERT(info.draw);
...
{
	return Point((i32)(pt.x * physical.x.x + physical.a.x), (i32)(pt.y * physical.y.y + physical.a.y));
}

static void PaintRectPart(Draw& draw, i32 x, i32 y, i32 w, i32 h)
{
	if(SystemDraw *sdraw = dynamic_cast<SystemDraw *>(&draw)) {
...
void PaintDragHorzLine(Draw& draw, const Rect& rc, Color c1, Color c2, Color bgnd, i32 mingap)
{
	Size sz = rc.Size();
	if(SystemDraw *sdraw = dynamic_cast<SystemDraw *>(&draw)) {
#if defined(TARGET_WIN32)
...
#elif defined(GUI_X11)
		XGCVals gcv_old, gcv_new;
		XGetGCVals(Xdisplay, sdraw->GetGC(), GCForeground | GCFunction, &gcv_old);
...
#endif
		i32 whites = (sz.cx - 2 * mingap + DRAG_STEP) / (2 * DRAG_STEP);
		...
		{
			i32 rem = sz.cx - whites * 2 * DRAG_STEP + DRAG_STEP;
			i32 lrem = rem >> 1, rrem = (rem + 1) >> 1;
			PaintRectPart(*sdraw, rc.left, rc.top, lrem, sz.cy);
			PaintRectPart(*sdraw, rc.right - rrem, rc.top, rrem, sz.cy);
			i32 i;
			i32 start = rc.left + lrem - DRAG_STEP;
			for(i = 1; i < whites; i++)
...
void PaintDragVertLine(Draw& draw, const Rect& rc, Color c1, Color c2, Color bgnd, i32 mingap)
{
	Size sz = rc.Size();
	if(SystemDraw *sdraw = dynamic_cast<SystemDraw *>(&draw)) {
#if defined(TARGET_WIN32)
		...
		XChangeGC(Xdisplay, sdraw->GetGC(), GCForeground | GCFunction, &gcv_new);
#endif
		i32 whites = (sz.cy - 2 * mingap + DRAG_STEP) / (2 * DRAG_STEP);
...
void PaintDragRect(Draw& draw, const Rect& rc, Color c1, Color c2, Color bgnd, i32 width)
{
	if(rc.IsEmpty())
		return;
...
void PlotterTool::Line(const Array<Pointf>& points)
{
	if(!points.IsEmpty()) {
		bool line = false;
		for(i32 i = 0; i < points.GetCount(); i++)
			if(IsNull(points[i]))
				line = false;
			else {
...
void PlotterTool::ArcTo(Pointf pt, double bulge, i32 levels)
{
	...
	else
	{
		...
		arc.Precision(plotter.logprec).ArcTo(OTVET(ArcToRaw));
		if(clip_arcs)
...
i32  PathTool::segments = 0;
i32  PathTool::drawn = 0;
...
bool PathTool::Set(const Plotter& _info, const Txt& pattern, Color color, i32 width, double dash)
{
...
bool PathTool::SetExtent(const Rectf& rc)
{
	if(!dummy && rc.Intersects(plotter.logclip)) {
		...
		MoveToRaw = (simple ? OTVET(MoveToRawSimple) : OTVET(MoveToRawClip));
		LineToRaw = (simple ? OTVET(LineToRawSimple) : OTVET(LineToRawClip));
		return within;
	}
	else {
		MoveToRaw = OTVET(MoveToRawDummy);
		LineToRaw = OTVET(LineToRawDummy);
		return false;
	}
}
...
void PathTool::ClearExtent()
{
	if(dummy) {
		...
		MoveToRaw = OTVET(MoveToRawClip);
		LineToRaw = OTVET(LineToRawClip);
		simple = false;
		pathdraw.ClearExtent();
	}
}
...
//	RTIMING("AreaTool::LineToRawClip");
	...
	for(i32 t = vc; t < vertices.GetCount(); t++)
...
void AreaTool::Flush()
{
	...
	i32 add = vertices.GetCount() - begin_index;
	ASSERT(add >= 0);
...
AreaTool& AreaTool::Horz(i32 x)
{
	i32 count = vertices.GetCount() - begin_index;
	ASSERT(count > 0);
...
bool AreaTool::Set(const Plotter& _info, Color _fill_color, zu64 _fill_pattern,
	const Txt& outline_pattern, Color outline_color, i32 outline_width, double outline_dash)
{
	...
	plotter = Plotter(_info, i32(outline_width * path.width) + 2);
...
//		RTIMING("AreaTool::Fill(pattern)");
		...
		i32 old_rop = SetROP2(hdc, R2_MASKPEN);
		HGDIOBJ old_brush = SelectObject(hdc, fill_pattern.GetBrush(Null, Null));
		i32 old_color = SetTextColor(hdc, Black);
		i32 old_bk = SetBkColor(hdc, White);
...
//		RTIMING("AreaTool::Outline()");
		if(ghost_lines.IsEmpty())
		{
			...
			for(i32 i = 0; i < counts.GetCount(); i++)
			{
...
//				outline.LineTo(*b);
		...
		{
			i32 gl = 0;
			i32 ngl = (gl >= ghost_lines.GetCount() ? vertices.GetCount() : ghost_lines[gl]);
			i32 v = 0;
			for(i32 i = 0; i < counts.GetCount(); i++)
			{
				i32 nv = v + counts[i];
...
{
public:
	virtual void Paint(Draw& draw, const Vec<Point>& pt) {}
	virtual i32  GetSize() const                            { return 0; }
};
...
{
public:
	ImageMarker(const Image& img, i32 size, Color color, bool antialias, bool ignore_hotspot);

	virtual void  Paint(Draw& draw, const Vec<Point>& pt);
	virtual i32   GetSize() const { return linear_size; }
...
ImageMarker::ImageMarker(const Image& srcimg, i32 size_, Color color_, bool antialias_, bool ignore_hotspot_)
...
void ImageMarker::Paint(Draw& draw, const Vec<Point>& pts)
{
	if(img.IsEmpty())
		return;
	for(i32 t = 0; t < pts.GetCount(); t++) {
		Point pt = pts[t] - hotspot;
		draw.DrawImage(Rect(pt, outsize), img);
	}
}

One<MarkTool::Marker> MarkTool::Picture(Image pic, i32 size, Color color, bool antialias, bool ignore_hotspot)
{
...
{
public:
	AreaMarker(i32 size, Color color, Color outline, i32 outline_width)
	: size(size), color(color), outline(outline), outline_width(outline_width)
	{ out_size = (size + outline_width) >> 1; }

	virtual i32  GetSize() const { return out_size; }
...
{
public:
	SquareMarker(i32 size, Color color, Color outline, i32 outline_width)
	: AreaMarker(size, color, outline, outline_width) {}

	virtual void Paint(Draw& draw, const Vec<Point>& pt);
};

void SquareMarker::Paint(Draw& draw, const Vec<Point>& pt)
{
	Vec<Point> outpt;
	i32 obj = pt.GetCount();
...
One<MarkTool::Marker> MarkTool::Square(i32 size, Color color, Color outline, i32 outline_width)
{
...
{
public:
	Square45Marker(i32 size, Color color, Color outline, i32 outline_width)
	: AreaMarker(size, color, outline, outline_width) {}

	virtual void Paint(Draw& draw, const Vec<Point>& pt);
};

void Square45Marker::Paint(Draw& draw, const Vec<Point>& pt)
{
	Vec<Point> outpt;
	i32 obj = pt.GetCount();
	outpt.SetCount(4 * obj);
...
One<MarkTool::Marker> MarkTool::Square45(i32 size, Color color, Color outline, i32 outline_width)
{
...
{
public:
	TriangleMarker(i32 size, Color color, Color outline, i32 outline_width)
	: AreaMarker(size, color, outline, outline_width) {}

	virtual void Paint(Draw& draw, const Vec<Point>& pt);
};

void TriangleMarker::Paint(Draw& draw, const Vec<Point>& pt)
{
	i32 half = size >> 1;
	Vec<Point> outpt;
	i32 obj = pt.GetCount();
...
One<MarkTool::Marker> MarkTool::Triangle(i32 size, Color color, Color outline, i32 outline_width)
{
...
{
public:
	NablaMarker(i32 size, Color color, Color outline, i32 outline_width)
	: AreaMarker(size, color, outline, outline_width) {}

	virtual void Paint(Draw& draw, const Vec<Point>& pt);
};

void NablaMarker::Paint(Draw& draw, const Vec<Point>& pt)
{
	i32 half = size >> 1;
	Vec<Point> outpt;
	i32 obj = pt.GetCount();
...
One<MarkTool::Marker> MarkTool::Nabla(i32 size, Color color, Color outline, i32 outline_width)
{
...
{
public:
	CircleMarker(i32 size, Color color, Color outline, i32 outline_width);

	virtual void Paint(Draw& draw, const Vec<Point>& pt);
};

CircleMarker::CircleMarker(i32 size, Color color, Color outline, i32 outline_width)
: AreaMarker(size, color, Nvl(outline, color), IsNull(outline) ? 0 : outline_width)
{
}

void CircleMarker::Paint(Draw& draw, const Vec<Point>& pt)
{
	i32 half = size >> 1;
	for(i32 t = 0; t < pt.GetCount(); t++)
		draw.DrawEllipse(RectC(pt[t].x - half, pt[t].y - half, size, size),
			color, outline_width, Nvl(outline, color));
}

One<MarkTool::Marker> MarkTool::Circle(i32 size, Color color, Color outline, i32 outline_width)
{
...
{
public:
	CrossMarker(i32 size, Color outline, i32 outline_width)
	: AreaMarker(size, Null, outline, outline_width) {}

	virtual void Paint(Draw& draw, const Vec<Point>& pt);
};

void CrossMarker::Paint(Draw& draw, const Vec<Point>& pt)
{
	i32 half = size >> 1;
	Vec<Point> out;
...
One<MarkTool::Marker> MarkTool::Cross(i32 size, Color outline, i32 outline_width)
{
...
{
public:
	DiamondMarker(i32 size, Color color, Color outline, i32 outline_width);
...
DiamondMarker::DiamondMarker(i32 size, Color color, Color outline, i32 outline_width)
: AreaMarker(size, color, outline, outline_width)
{
	y2 = size >> 1;
	x2 = size * 17 >> 5;
...
void DiamondMarker::Paint(Draw& draw, const Vec<Point>& pt)
{
	Vec<Point> out;
...
One<MarkTool::Marker> MarkTool::Diamond(i32 size, Color color, Color outline, i32 outline_width)
{
...
{
public:
	HexagonMarker(i32 size, Color color, Color outline, i32 outline_width);
...
HexagonMarker::HexagonMarker(i32 size, Color color, Color outline, i32 outline_width)
: AreaMarker(size, color, outline, outline_width)
{
	y2 = size >> 1;
	x2 = size * 19 >> 5;
...
void HexagonMarker::Paint(Draw& draw, const Vec<Point>& pt)
{
	Vec<Point> out;
...
One<MarkTool::Marker> MarkTool::Hexagon(i32 size, Color color, Color outline, i32 outline_width)
{
...
{
public:
	StarMarker(i32 size, Color color, Color outline, i32 outline_width);

	virtual void Paint(Draw& draw, const Vec<Point>& pt);

private:
	i32 x2, x4, X2, X4, y2, y4, Y2, Y4, dy;
};

StarMarker::StarMarker(i32 size, Color color, Color outline, i32 outline_width)
: AreaMarker(size, color, outline, outline_width)
{
	y2 = size >> 1;
	x2 = size * 18 >> 5;
...
void StarMarker::Paint(Draw& draw, const Vec<Point>& pt)
{
	Vec<Point> out;
...
One<MarkTool::Marker> MarkTool::Star(i32 size, Color color, Color outline, i32 outline_width)
{
...
{
public:
	ThickCrossMarker(i32 size, Color color, Color outline, i32 outline_width);
...
ThickCrossMarker::ThickCrossMarker(i32 size, Color color, Color outline, i32 outline_width)
: AreaMarker(size, color, outline, outline_width)
{
	s2 = size >> 1;
	s6 = size / 6;
...
void ThickCrossMarker::Paint(Draw& draw, const Vec<Point>& pt)
{
	Vec<Point> out;
...
One<MarkTool::Marker> MarkTool::ThickCross(i32 size, Color color, Color outline, i32 outline_width)
{
...
{
public:
	LetterMarker(char ascii, i32 angle, Font font, Color color, Color outline);
...
LetterMarker::LetterMarker(char ascii, i32 angle, Font font, Color color, Color outline)
: ascii(ascii)
, angle(angle)
, font(font)
, color(color)
...
i32 LetterMarker::GetSize() const
{
	Font raw = font;
	if(raw.GetHeight() >= 0)
	...
	return fceil(hypot(info[(u8)ascii], info.GetHeight())) >> 1;
}

static void DrawTextWithMask(Draw& data, Draw& mask, i32 x, i32 y, char letter, i32 angle, Font font, Color cdata, Color cmask)
{
	data.DrawText(x, y, angle, &letter, font, cdata, 1);
	mask.DrawText(x, y, angle, &letter, font, cmask, 1);
}

void LetterMarker::Paint(Draw& draw, const Vec<Point>& pt)
{
...
One<MarkTool::Marker> MarkTool::Letter(char ascii, i32 angle, Font font, Color color, Color outline)
{
	return new LetterMarker(ascii, angle, font, color, outline);
}

One<MarkTool::Marker> MarkTool::StandardMarker(i32 type, i32 size, Color color, Color outline, i32 outline_width)
{
	switch(type)
	{
...
Size TextTool::Type::GetTextSize(const Txt& s) const
{
	i32 wd = 0;
	u8k *b = s;
...
void TextTool::Set(const Plotter& info, Font _f, Color _c, Color _o, double _a, Alignment _x, Alignment _y, bool _flip, i32 _aprec)
{
	cache_size = 0;
	font_index = 0;
	plotter    = info;
...
void TextTool::SetFont(Font _font)
{
	if((font_index = font_map.Find(font = _font)) < 0)
	{
		...
		for(i32 c = 256; --c >= 0; out[c] = fi.GetWidth(c))
			;
		type.height = fi.GetHeight();
	}
}

void TextTool::PutFlip(const Txt& text, Pointf pt, double angle)
{
...
void TextTool::Put(const Txt& text, Pointf pt, double angle)
{
	if(IsNull(pt.x) || IsNull(pt.y))
		return;
	Size size = font_map[font_index].GetTextSize(text);
	i32 delta = size.cx + size.cy;
	...
	i32 apart = fround(angle * angle_prec / (2 * M_PI)) % angle_prec;
...
void TextTool::Flush()
{
	...
	for(i32 i = 0; i < cache.GetCount(); i++)
	{
		...
		for(i32 t = 0; t < items.GetCount(); t++)
		{
