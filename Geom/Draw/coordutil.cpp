...
static void ExpandSegmentUp(Vec<Point>& src, Vec<Point>& dest, const LinearSegmentTree::Node *node, Point smin, Point dmin)
{
	if(node) {
...
static void ExpandSegmentDown(Vec<Point>& src, Vec<Point>& dest, const LinearSegmentTree::Node *node, Point smax, Point dmax)
{
	if(node) {
...
static void PaintSplit(ImageBuffer& out, const Image& in,
	...
	const SegmentTreeInfo& info, Color background, i32 index)
{
	if(planar.split) {
		index <<= 2;
...
static void PaintTreeSplit(Draw& draw, const PlanarSegmentTree::Node& planar,
	const LinearSegmentTree::Node *left, const LinearSegmentTree::Node *top,
	const LinearSegmentTree::Node *right, const LinearSegmentTree::Node *bottom, i32 index)
{
	if(planar.split) {
		index <<= 2;
		...
		for(i32 i = 1; i < bdest.GetCount(); i++)
...
static void PlotLine(PlotterTool& tool, GisTransform transform, Pointf src1, Pointf src2, i32 level, double maxdev)
{
	double dev;
...
/*
void PlotLine(PlotterTool& tool, GisCoords proj, Pointf lonlat1, Pointf lonlat2, i32 branch)
{
...
void PlotLineSafe(PlotterTool& tool, GisCoords proj, Pointf lonlat1, Pointf lonlat2, i32 branch)
{
	i32 nseg = minmax<i32>((i32)fpabsmax(lonlat2 - lonlat1), 1, 360);
	tool.MoveTo(proj.Project(lonlat1, branch));
	for(i32 i = 1; i <= nseg; i++)
		tool.LineTo(proj.Project(lonlat1 + (lonlat2 - lonlat1) * (i / double(nseg)), branch));
}

void PlotRectSafe(PlotterTool& tool, GisCoords proj, const Rectf& lonlat, i32 branch)
{
