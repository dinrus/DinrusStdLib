...
#define MINMAX0(x, hi) ((x) < 0 ? 0 : (x) > (hi) ? (hi) : (i32)(x))
...
struct SideSegment
{
	...
	void  operator ++ ()      { src16 += src16add; dest16 += dest16add; }
	void  operator += (i32 i) { src16 += i * src16add; dest16 += i * dest16add; }
};

static SideSegment CalcSideSegment(Point d1, Point d2, Point s1, Point s2, i32 y1)
{
	if(d2.y < d1.y) {
		Swap(d1, d2);
		Swap(s1, s2);
	...
	SideSegment sgm;
	i32 ht = max<i32>(d2.y - d1.y, 1);
	sgm.dest16 = ((z64)d1.x << 16) + 0x8000;
	sgm.dest16add = (z64)(d2.x - d1.x) * 0x10000 / ht;
	sgm.src16 = ((Poz64)s1 << 16) + (z64)0x8000;
	sgm.src16add = (Size64)(s2 - s1) * (z64)0x10000 / (z64)ht;
	if((y1 -= d1.y) > 0)
		sgm += y1;
	return sgm;
}
...
EXITBLOCK {
	RLOG("LinearSection: #pixels = " << linsec_pixels << ", " << linsec_msecs << " msecs, "
		<< linsec_pixels / max<z64>(1000 * linsec_msecs, 1) << " MPixels/s");
}
#endif

typedef void (*TransformScanProc)(RGBA *dest, i32 count, Poz64 sl16, Size64 sd16,
	const RGBA *src, Size src_size, i32 enterclip, i32 leaveclip);

static void TransformScanOpaqueNearest(RGBA *dest, i32 count, Poz64 sl16, Size64 sd16,
	const RGBA *src, Size src_size, i32 enterclip, i32 leaveclip)
{
	i32 xmax = (src_size.cx << 16) - 0x10001;
	i32 ymax = (src_size.cy << 16) - 0x10001;
	for(i32 i = enterclip; --i >= 0; dest++, sl16 += sd16)
		RGBA_SET(*dest, src[(MINMAX0(sl16.x, xmax) >> 16)
			+ ((i32)MINMAX0(sl16.y, ymax) >> 16) * src_size.cx]);
	if(enterclip < count) {
		Point sl16i(sl16);
		Size sd16i(sd16);
		for(i32 i = leaveclip - enterclip; --i >= 0; dest++, sl16i += sd16i) {
			ASSERT(sl16i.x >= 0 && sl16i.x <= xmax && sl16i.y >= 0 && sl16i.y <= ymax);
			RGBA_SET(*dest, src[(sl16i.x >> 16) + (sl16i.y >> 16) * src_size.cx]);
		}
		sl16 = sl16i;
		for(i32 i = count - leaveclip; --i >= 0; dest++, sl16 += sd16)
			RGBA_SET(*dest, src[(MINMAX0(sl16.x, xmax) >> 16)
				+ ((i32)MINMAX0(sl16.y, ymax) >> 16) * src_size.cx]);
	}
}

static void TransformScanAlphaNearest(RGBA *dest, i32 count, Poz64 sl16, Size64 sd16,
	const RGBA *src, Size src_size, i32 enterclip, i32 leaveclip)
{
	i32 xmax = (src_size.cx << 16) - 0x10001;
	i32 ymax = (src_size.cy << 16) - 0x10001;
...
static void TransformScanOpaqueBilinear(RGBA *dest, i32 count, Poz64 sl16, Size64 sd16,
	const RGBA *src, Size src_size, i32 enterclip, i32 leaveclip)
{
	i32 xmax = (src_size.cx << 16) - 0x10001;
	i32 ymax = (src_size.cy << 16) - 0x10001;
	for(; --count >= 0; dest++, sl16 += sd16) {
		i32 x = MINMAX0(sl16.x, xmax) >> 8;
		i32 y = MINMAX0(sl16.y, ymax) >> 8;
		const RGBA *row1 = &src[(y >> 8) * src_size.cx + (x >> 8)];
		const RGBA *row2 = row1 + src_size.cx;
		i32 t1, t2;
		t1 = row1[0].r * 256 + (row1[1].r - row1[0].r) * (u8)x;
		t2 = row2[0].r * 256 + (row2[1].r - row2[0].r) * (u8)x;
		dest->r = (t1 * 256 + (t2 - t1) * (u8)y) >> 16;
		t1 = row1[0].g * 256 + (row1[1].g - row1[0].g) * (u8)x;
		t2 = row2[0].g * 256 + (row2[1].g - row2[0].g) * (u8)x;
		dest->g = (t1 * 256 + (t2 - t1) * (u8)y) >> 16;
		t1 = row1[0].b * 256 + (row1[1].b - row1[0].b) * (u8)x;
		t2 = row2[0].b * 256 + (row2[1].b - row2[0].b) * (u8)x;
		dest->b = (t1 * 256 + (t2 - t1) * (u8)y) >> 16;
		dest->a = 255;
	}
}

static void TransformScanAlphaBilinear(RGBA *dest, i32 count, Poz64 sl16, Size64 sd16,
	const RGBA *src, Size src_size, i32 enterclip, i32 leaveclip)
{
	i32 xmax = (src_size.cx << 16) - 0x10001;
	i32 ymax = (src_size.cy << 16) - 0x10001;
	for(; --count >= 0; dest++, sl16 += sd16) {
		...
		t1 = row1[0].r * 256 + (row1[1].r - row1[0].r) * (u8)x;
		t2 = row2[0].r * 256 + (row2[1].r - row2[0].r) * (u8)x;
		u8 r = (t1 * 256 + (t2 - t1) * (u8)y) >> 16;
		t1 = row1[0].g * 256 + (row1[1].g - row1[0].g) * (u8)x;
		t2 = row2[0].g * 256 + (row2[1].g - row2[0].g) * (u8)x;
		u8 g = (t1 * 256 + (t2 - t1) * (u8)y) >> 16;
		t1 = row1[0].b * 256 + (row1[1].b - row1[0].b) * (u8)x;
		t2 = row2[0].b * 256 + (row2[1].b - row2[0].b) * (u8)x;
		u8 b = (t1 * 256 + (t2 - t1) * (u8)y) >> 16;
		t1 = row1[0].a * 256 + (row1[1].a - row1[0].a) * (u8)x;
		t2 = row2[0].a * 256 + (row2[1].a - row2[0].a) * (u8)x;
		if(u8 a = (t1 * 256 + (t2 - t1) * (u8)y) >> 16) {
			if(a > dest->a) dest->a = a;
			i32 coef = a * 256 / dest->a;
...
static i32 TransformSection(ImageBuffer& dest, const RGBA *src, Size src_size,
	SideSegment& left, SideSegment& right, Rect clip, TransformScanProc scanproc)
{
	i32 xmax = (src_size.cx << 16) - 0x10001;
	i32 ymax = (src_size.cy << 16) - 0x10001;
	i32 npixels = 0;
	for(i32 y = clip.top; y < clip.bottom; ++y, ++left, ++right) {
		i32 dl = (i32)(left.dest16 >> 16), n = (i32)((right.dest16 >> 16) - dl + 1);
		if(n <= 0)
			continue;
		Size64 sd16 = (right.src16 - left.src16) / (z64)n;
		Poz64 sl16 = left.src16;
		if(dl < clip.left) {
			n += dl - clip.left;
			sl16 += sd16 * (z64)(clip.left - dl);
			dl = clip.left;
		}
		if(dl + n > clip.right)
			n = clip.right - dl;
		i32 enterclip = 0;
		i32 leaveclip = n;
		Poz64 sr16 = sl16 + sd16 * (n - 1);
		if(sd16.cx > 0) {
			if(sl16.x < 0)
				enterclip = (i32)(~sl16.x / sd16.cx) + 1;
			if(sr16.x > xmax)
				leaveclip = n - 1 - (i32)((sr16.x - xmax - 1) / sd16.cx);
		}
		else if(sd16.cx < 0) {
			if(sl16.x > xmax)
				enterclip = (i32)((sl16.x - xmax - 1) / -sd16.cx) + 1;
			if(sr16.x < 0)
				leaveclip = n - 1 - (i32)(~sr16.x / -sd16.cx);
		}
		else {
			sl16.x = minmax<z64>(sl16.x, 0, xmax);
			if(src_size.cy == 1)
				enterclip = n;
		}
		...
		if(sd16.cy > 0) {
			if(sl16.y < 0)
				enterclip = max(enterclip, (i32)(~sl16.y / sd16.cy) + 1);
			if(sr16.y > ymax)
				leaveclip = min(leaveclip, n - 1 - (i32)((sr16.y - ymax - 1) / sd16.cy));
		}
		else if(sd16.cy < 0) {
			if(sl16.y > ymax)
				enterclip = max(enterclip, (i32)((sl16.y - ymax - 1) / -sd16.cy) + 1);
			if(sr16.y < 0)
				leaveclip = min(leaveclip, n - 1 - (i32)(~sr16.y / -sd16.cy));
		}
		else {
			sl16.y = minmax<z64>(sl16.y, 0, ymax);
			if(src_size.cy == 1)
				enterclip = n;
		}
...
static i32 TransformSet(ImageBuffer& dest, RGBA color, SideSegment& left, SideSegment& right, Rect clip)
{
	i32 npixels = 0;
	for(i32 y = clip.top; y < clip.bottom; ++y, ++left, ++right) {
		i32 dl = (i32)(left.dest16 >> 16), n = (i32)((right.dest16 >> 16) - dl + 1);
...
static TransformScanProc GetTransformProc(i32 kind, bool interpolate)
{
...
//		ASSERT(src_pixel.GetRect().Contains(srcpos[i]));
		sides[i].dest16 = Null;
		ybrk[i] = destpos[i].y;
		for(i32 j = i; --j >= 0 && ybrk[j] > ybrk[j + 1]; Swap(ybrk[j], ybrk[j + 1]))
			;
	}
	ybrk[3]++;

	for(i = 0; i < 3; i++) {
		i32 top = ybrk[i], bottom = ybrk[i + 1];
		if(top >= clip.bottom)
			return;
		if(bottom <= clip.top)
			continue;
		Rect subclip(clip.left, max(top, clip.top), clip.right, min(bottom, clip.bottom));
		i32 half = subclip.Height() >> 1;
		i32 active[4];
		z64 mid[4];
		i32 acount = 0;
		...
		for(i32 s = 0; s < 4; s++) {
			...
			if(prev.y < subclip.bottom && next.y > subclip.top) {
				...
				active[acount] = s;
				for(i32 t = acount++; --t >= 0 && mid[t] > mid[t + 1]; Swap(mid[t], mid[t + 1]), Swap(active[t], active[t + 1]))
					;
			}
			prev = destpos[s];
		}
		...
		z64 dt = sar->dest16 - sal->dest16, dw = sar->dest16add - sal->dest16add, db = dt + dw * (subclip.Height() - 1);
		i32 ycross = Null;
		Rect crossclip;
		if(dt >= +0x10000 && db <= -0x10000 || dt <= -0x10000 && db >= +0x10000) {
			ycross = (i32)(subclip.top - dt / dw);
			crossclip = subclip;
...
Image BilinearRotate(const Image& m, i32 angle)
{
	Size isz = m.GetSize();
	Point center = isz / 2;
