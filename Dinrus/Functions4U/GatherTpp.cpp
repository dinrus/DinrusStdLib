...
#include <Dinrus/PdfDraw/PdfDraw.h>
//#include <ide/Browser/Browser.h>

#include <Dinrus/Functions4U/Functions4U.h>
#include "GatherTpp.h"
#include <Dinrus/Functions4U/Html/htmld.h>
...
String GatherTpp::GatherTopics(const char *topic, String& _title)
{
    ...
    if(q < 0) {
        ...
        if(IsNull(t)) {
            ...
            p.text = String("{{1f1t0/50b0/50@(240.240.240) [<A2 ") + t_("Эта страница ещё не переведена") +
...
String GatherTpp::Www(const char *topic, int lang, String topicLocation) {
    ...
    if (www != "index.html")
        return www;
    return GatherTopics(String().Cat() << topicLocation << topic << "$" << "ru-ru");
}
