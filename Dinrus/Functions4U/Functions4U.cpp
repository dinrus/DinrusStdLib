...
#include <Dinrus/Functions4U/Functions4U.h>
...
#define TFILE <Dinrus/Functions4U/Functions4U.t>
...
String SeasonName(int iseason) {
    static const char *season[] = {t_("зима"), t_("весна"), t_("лето"), t_("осень")};
            ...
            if(ff.IsFile()) {
                ...
                /*
                    ...
                    if (fp != NULL) {
                        ...
                        errorList.Add(AppendFileNameX(dir, ff.GetName()) + String(": ") + t_("Невозможно открыть файл"));
                }
            }
        } while (ff.Next());
    }
...
bool FileDiffArray::Apply(String toFolder, String fromFolder, EXT_FILE_FLAGS flags)
{
    for (int i = 0; i < diffList.GetCount(); ++i) {
        ...
        if (!ok) {
            String strError = t_("Нельзя изменить ") + dest;
        ...
        case 'n': case 'u':
            ...
            if (!ok) {
                String strError = t_("Невозможно создать ") + dest;
#if defined(PLATFORM_WIN32) || defined (PLATFORM_WIN64)
        ...
        case 'd':
            ...
            if (!ok) {
                String strError = t_("Нельзя удалить") + String(" ") + dest;
#if defined(PLATFORM_WIN32) || defined (PLATFORM_WIN64)
                strError += ". " + WinLastError();
#endif
                SetLastError(strError);
