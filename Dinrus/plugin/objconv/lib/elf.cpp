...
void CELF<ELFSTRUCTURES>::Dump(int options) {
   if (options & DUMP_FILEHDR) {
      // File header
      printf("\nДамп файла ELF %s", FileName);
      printf("\n-----------------------------------------------");
      printf("\nРазмер файла: %i", GetDataSize());
      printf("\nЗаголовок файла:");
      printf("\nКласс файла: %s, Кодировка данных: %s, Версия ELF %i, ABI: %s, Версия ABI %i",
   ...
   if (options & DUMP_SECTHDR) {
      ...
      for (uint32 sc = 0; sc < NSections; sc++) {
         ...
         printf("\n%2i Имя: %-18s Тип: %s", sc, SecStringTable + namei,
            Lookup(ELFSectionTypeNames, sheader.sh_type));
         if (sheader.sh_flags) {
            printf("\n  Флаги: 0x%X:", uint32(sheader.sh_flags));
         ...
         if (sheader.sh_addr) {
            printf("\n  Адрес: 0x%X", uint32(sheader.sh_addr));
         }
         if (sheader.sh_offset || sheader.sh_size) {
            printf("\n  Смещение файла: 0x%X, Размер: 0x%X",
               uint32(sheader.sh_offset), uint32(sheader.sh_size));
         }
         if (sheader.sh_addralign) {
            printf("\n  Алиновка: 0x%X", uint32(sheader.sh_addralign));
         }
         if (sheader.sh_entsize) {
            printf("\n  Размер записи: 0x%X", uint32(sheader.sh_entsize));
            switch (sheader.sh_type) {
            case SHT_DYNAMIC:
               printf("\n  Таблица строк: %i", sheader.sh_link);
               break;
            case SHT_HASH:
               printf("\n  Таблица символов: %i", sheader.sh_link);
               break;
            case SHT_REL: case SHT_RELA:
               printf("\n  Таблица символов: %i, Секция релокаций: %i",
                  sheader.sh_link, sheader.sh_info);
               break;
            case SHT_SYMTAB: case SHT_DYNSYM:
               printf("\n  Таблица строк символов: %i, Первый глобальный символ: %i",
                  sheader.sh_link, sheader.sh_info);
               break;
            default:
               if (sheader.sh_link) {
                  printf("\n  Компоновка: %i", sheader.sh_link);
               }
               if (sheader.sh_info) {
                  printf("\n  Информация: %i", sheader.sh_info);
               }
            }
         }
         ...
         if ((sheader.sh_type==SHT_SYMTAB || sheader.sh_type==SHT_DYNSYM) && (options & DUMP_SYMTAB)) {
            ...
            for (symi = 0; symtab < symtabend; symtab += entrysize, symi++) {
               ...
               if (*(strtab + sym.st_name)) {
                  printf("\n  %2i Имя: %s,", symi, strtab + sym.st_name);}
               else {
                  printf("\n  %2i Без имени,", symi);}
               if (sym.st_value || type == STT_OBJECT || type == STT_FUNC || type == STT_GNU_IFUNC || int16(sym.st_shndx) < 0)
                  printf(" Значение: 0x%X", uint32(sym.st_value));
               if (sym.st_size)  printf(" Размер: %i", uint32(sym.st_size));
               if (sym.st_other) printf(" Другое: 0x%X", sym.st_other);
               if (int16(sym.st_shndx) >= 0) printf(" Секция: %i", sym.st_shndx);
               else { // Special segment values
                  switch (int16(sym.st_shndx)) {
                  case SHN_ABS:
                     printf(" Абсолютный,"); break;
                  case SHN_COMMON:
                     printf(" Общий,"); break;
                  case SHN_XINDEX:
                     printf(" Индекс в экстра-таблице,"); break;
                  default:
                     printf(" Секция: 0x%X", sym.st_shndx);
                  }
               }
               if (sym.st_type || sym.st_bind) {
                  printf(" Тип: %s, Подвязка: %s",
