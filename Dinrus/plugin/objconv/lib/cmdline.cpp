...
void CCommandLineInterpreter::ReportStatistics() {
    ...
    if (DebugInfo == CMDL_DEBUG_STRIP) {
        printf ("\n%3i Отладочных секций удалено", CountDebugSectionsRemoved);
    }
    if (ExeptionInfo == CMDL_EXCEPTION_STRIP) {
        printf ("\n%3i Секций исключений удалено", CountExceptionSectionsRemoved);
    }
    if ((DebugInfo == CMDL_DEBUG_STRIP || ExeptionInfo == CMDL_EXCEPTION_STRIP)
        && CountUnusedSymbolsHidden) {
            printf ("\n%3i Неиспользуемых ссылок на внешние символы скрыто", CountUnusedSymbolsHidden);
    }

    if (Underscore || SegmentDot || SymbolList.GetNumEntries()) {
        if (CountUnderscoreConversions || Underscore) {
            printf ("\n%3i Измений вводящих подчёркиваний у имён символов", CountUnderscoreConversions);
        }
        if (CountSectionDotConversions || SegmentDot) {
            printf ("\n%3i Изменений вводящих символов у имён секций", CountSectionDotConversions);
        }
        if (CountSymbolNameChanges) {
            printf ("\n%3i Имён символов изменено", CountSymbolNameChanges);
        }
        if (CountSymbolNameAliases) {
            printf ("\n%3i Публичных имён символов заалиасено", CountSymbolNameAliases);
        }
        if (CountSymbolsWeakened) {
            printf ("\n%3i Публичных имён символов сделано слабыми", CountSymbolsWeakened);
        }
        if (CountSymbolsMadeLocal) {
            printf ("\n%3i Публичных или внешних имён символов сделано локальными", CountSymbolsMadeLocal);
        }
        if (SymbolChangeEntries && !CountSymbolNameChanges && !CountSymbolNameAliases && !CountSymbolsWeakened && !CountSymbolsMadeLocal) {
            printf ("\n    Символы, требующие изменения, не обнаружены");
        }
    }
}
...
void CCommandLineInterpreter::Help() {
    ...
    printf("\n-nu        Изменить подчёркивания в именах символов на дефолт для целевого формата.");
    printf("\n-nu-       Удалить подчёркивания из имён символов.");
    printf("\n-nu+       Добавить подчёркивания в имена символов.");
    printf("\n-nd        Заменить точку/подчёркивание в именах секций.");
    printf("\n-nr:N1:N2  Заменить имя символа N1 на N2.");
    printf("\n-np:N1:N2  Заменить префикс символа с N1 на N2.");
    printf("\n-ns:N1:N2  Заменить суффикс символа с N1 на N2.");
    printf("\n-ar:N1:N2  Сделать алиас N2 для существующего публичного имени N1.");
    printf("\n-ap:N1:N2  Заменить префикс символа и сохранить старое имя как алиас.");
    printf("\n-as:N1:N2  Заменить суффикс символа и сохранить старое имя как алиас.");
    printf("\n-nw:N1     Сделать имя публичного символа N1 слабым (только для ELF и MAC64).");
    printf("\n-nl:N1     Сделать имя публичного символа N1 локальным (невидимым).\n");
    //printf("\n-ds        Strip Debug info.");    // default if input and output are different formats
    //printf("\n-dp        Preserve Debug info, even if it is incompatible.");
    printf("\n-xs        Убрать информацию об обработке исключений и прочую несовместимую информацию.");  // default if input and output are different formats. Hides unused symbols
    printf("\n-xp        Сохранить информацию об обработке исключений и прочую несовместимую информацию.\n");

    printf("\n-lx        Извлечь из библиотеки все члены.");
    printf("\n-lx:N1:N2  Извлечь член N1 из библиотеки в файл N2.");
    printf("\n-ld:N1     Удалить член N1 из библиотеки.");
    printf("\n-la:N1:N2  Добавить объектный файл N1 в библиотеку как член N2.");
    printf("\n           Альтернатива: -lib ИМЯ_БИБЛИОТЕКИ ИМЕНА_ФАЙЛОВ_ОБЪЕКТОВ.\n");

    printf("\n-vN        Опции детальности. Значения N:");
    printf("\n           0: Молча, 1: Выводить имена и типы файлов, 2: Сообщать о преобразованиях.");

    printf("\n-wdNNN     Отключить предупреждение NNN.");
    printf("\n-weNNN     Считать предупреждение NNN за ошибку. -wex: считать ошибками все предупреждения.");
    printf("\n-edNNN     Отключить ошибку номер NNN.");
    printf("\n-ewNNN     Считать ошибку номер NNN за предупреждение.\n");

    printf("\n-h         Вывести эту справку на экран.\n");

    printf("\n@RFILE     Читать дополнительные опции из респонс-файла RFILE.\n");
    printf("\n\nПример:");
