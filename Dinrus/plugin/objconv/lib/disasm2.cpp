...
// Определение текстов ошибок.
SIntTxt AsmErrorTexts[] = {
    {1,         "Инструкция длинее 15 байт."},
    {2,         "Префикс блокировки для этого опкода запрещён"},
    {4,         "Нелегальный опкод."},
    {8,         "Нелегальные операнды для этого опкода."},
    {0x10,      "Инструкция простирается за пределы блока кода."},
    {0x20,      "Префикс после префикса REX запрещён."},
    {0x40,      "Эта инструкция недопустима в 64-битном режиме."},
    {0x80,      "Инструкция несофазна со следующим ярлыком."},
    {0x100,     "Попытка применения R13 в качестве базового регистра без вымещения."},
    {0x200,     "Регистры 8 - 15 разрешены только в 64-битном режиме (Проигнорировано)."},
    {0x400,     "Префикс REX недопустим у инструкций с байтом DREX."},
    {0x800,     "VEX имеет X бит, но отсутствует байт SIB (Возможно, проигнорирован)"},
    {0x1000,    "Источник релокации не соответствует адресу или полю операнда."},
    {0x2000,    "Накладывающиеся релокации."},
    {0x4000,    "Не похоже на код."}, // Consecutive bytes of 0 found
    {0x8000,    "VEX.L бит здесь недопустим."},
    {0x10000,   "VEX.mmmm биты вне диапазона."},
    {0x80000,   "Внутренняя ошибка в таблице опкодов в opcodes.cpp"}
};

// Тексты предупреждений 1: Warnings about conditions that could be intentional and suboptimal code
SIntTxt AsmWarningTexts1[] = {
    {1,          "Промежуточный операнд можно уменьшить расширением знака."},
    {2,          "Промежуточный операнд можно уменьшить расширением нуля."},
    {4,          "Вымещение нуля можно убрать."},
    {8,          "Displacement можно уменьшить расширением знака."},
    {0x10,       "Байт SIB здесь неуместен."},
    {0x20,       "Для регистрового операнда существует более краткая инструкция."},
    {0x40,       "Префикс, изменяющий длину, вызывает задержки у процессоров Intel."},
    {0x80,       "Префикса размера адреса следует избегать."},
    {0x100,      "Один и тот же префикс несколько раз подряд."},
    {0x200,      "Префикс полноценный, не неуместен."},
    {0x400,      "Бит или байт префикса в этом контексте не играет роли."},
    {0x800,      "Противоречащие префиксы."},
    {0x1000,     "Отсутствует необходимый префикс"},
    {0x2000,     "У адреса есть фактор масштаба, но нет индексного регистра."},
    {0x4000,     "Адрес не является rip-относительным."},
    {0x8000,     "Абсолютный адрес памяти без релокации."},
    {0x10000,    "Необычный тип релокации для этого операнда."},
    {0x20000,    "Указатель на инструкцию обрезан префиксом размера операнда."},
    {0x40000,    "Стэковый указатель обрезан префиксом размера адреса."},
    {0x80000,    "Джамп или вызов сегмента данных запрещён."},
    {0x100000,   "Недокументированый опкод."},
    {0x200000,   "Неизвестный опкод, зарезервированный для расширений в последующем."},
    {0x400000,   "Операнд памяти алинован неправильно. Удар по производительности."},
    {0x800000,   "Ошибка алиновки. Операнд памяти должен быть алинован."},
    {0x1000000,  "Многобайтный NOP. Замените на ALIGN."},
...
// Тексты предупреждений 2: Warnings about possible misinterpretation; serious warnings
...
// Указания типов релокаций в комментариях:
