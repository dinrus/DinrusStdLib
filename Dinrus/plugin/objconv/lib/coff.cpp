...
SIntTxt COFF32RelNames[] = {
   {COFF32_RELOC_ABS,     "Абсолютная"},         // Ignored
   {COFF32_RELOC_DIR32,   "Direct32"},         // 32-bit absolute virtual address
   {COFF32_RELOC_IMGREL,  "Относительно образа"},   // 32-bit image relative virtual address
   {COFF32_RELOC_SECTION, "Section16"},        // 16-bit section index in file
   {COFF32_RELOC_SECREL,  "Относительно секции"}, // 32-bit section-relative
   {COFF32_RELOC_SECREL7, "7 бит относительно секции"}, // 7-bit section-relative
   {COFF32_RELOC_TOKEN,   "Сема CLR"},        // CLR token
   {COFF32_RELOC_REL32,   "Относительно EIP"}      // 32-bit relative to end of address field
};

SIntTxt COFF64RelNames[] = {
   {COFF64_RELOC_ABS,     "Игнорируется"},           // Ignored
   {COFF64_RELOC_ABS64,   "64-битный абсолютный"},  // 64 bit absolute virtual address
   {COFF64_RELOC_ABS32,   "32-битны абсолютный"},  // 32 bit absolute virtual address
   {COFF64_RELOC_IMGREL,  "Относительно образа"},   // 32 bit image-relative
   {COFF64_RELOC_REL32,   "Относительно RIP"},     // 32 bit, RIP-relative
   {COFF64_RELOC_REL32_1, "Относительно RIP-1"},   // 32 bit, relative to RIP - 1. For instruction with immediate byte operand
   {COFF64_RELOC_REL32_2, "Относительно RIP-2"},   // 32 bit, relative to RIP - 2. For instruction with immediate word operand
   {COFF64_RELOC_REL32_3, "Относительно RIP-3"},   // 32 bit, relative to RIP - 3. Useless
   {COFF64_RELOC_REL32_4, "Относительно RIP-4"},   // 32 bit, relative to RIP - 4. For instruction with immediate dword operand
   {COFF64_RELOC_REL32_5, "Относительно RIP-5"},   // 32 bit, relative to RIP - 5. Useless
   {COFF32_RELOC_SECTION, "Индекс секции"},    // 16-bit section index in file
   {COFF64_RELOC_SECREL,  "Относительно секции"}, // 32-bit section-relative
   {COFF64_RELOC_SECREL7, "7-битный относительно секции"},//  7-bit section-relative
   {COFF64_RELOC_TOKEN,   "Сема CLR"},        // 64 bit absolute virtual address without inline addend
   {COFF64_RELOC_SREL32,  "32-битный зависимый от спана"},        //
   {COFF64_RELOC_PAIR,    "пара после зависимого от спана"}, //
   {COFF64_RELOC_PPC_REFHI,"старшие 16 из 32 бит абс"},    //
   {COFF64_RELOC_PPC_REFLO,"младшие 16 из 32 бит абс"},     //
   {COFF64_RELOC_PPC_PAIR, "пара после старших 16"},       //
   {COFF64_RELOC_PPC_SECRELO,"младшие 16 из 32 битной секции релятивно"},
   {COFF64_RELOC_PPC_GPREL,  "16 бит относительно GP"},     //
   {COFF64_RELOC_PPC_TOKEN,  "Сема CLR"}               //
};

// Machine names

SIntTxt COFFMachineNames[] = {
   {0,     "Любая/неизвестная"},     // Any machine/unknown
   {0x184, "Alpha"},           // Alpha AXP
   {0x1C0, "Arm"},             // Arm
   {0x284, "Alpha 64-битная"},    // Alpha AXP 64 bit
...
void CCOFF::Dump(int options) {
   ...
   if (options & DUMP_FILEHDR) {
      // File header
      printf("\nДамп фала PE/COFF %s", FileName);
      printf("\n-----------------------------------------------");
      printf("\nРазмер файла: %i", GetDataSize());
      printf("\nЗаголовок файла:");
      printf("\nМашина: %s", Lookup(COFFMachineNames,FileHeader->Machine));
      printf("\nВремяДата: 0x%08X", FileHeader->TimeDateStamp);
      printf(" - %s", timestring(FileHeader->TimeDateStamp));
      printf("\nЧисло секций: %2i", FileHeader->NumberOfSections);
      printf("\nЧисло символов:  %2i", FileHeader->NumberOfSymbols);
      printf("\nРазмер опционального заголовка: %i", FileHeader->SizeOfOptionalHeader);
      printf("\nФлаги: 0x%04X", FileHeader->Flags);

      // May be removed:
      printf("\nСмещение таблицы символов: 0x%X", FileHeader->PSymbolTable);
      printf("\nСмещение таблицы строк: 0x%X", FileHeader->PSymbolTable + FileHeader->NumberOfSymbols * SIZE_SCOFF_SymTableEntry);
      printf("\nСмещение заголовков секций: 0x%X", (uint32)sizeof(SCOFF_FileHeader) + FileHeader->SizeOfOptionalHeader);
      ...
      if (OptionalHeader) {
         ...
         if (OptionalHeader->h32.Magic != COFF_Magic_PE64) {
            ...
            printf("\nМагическое число: 0x%X", OptionalHeader->h32.Magic);
            printf("\nРазмер кода: 0x%X", OptionalHeader->h32.SizeOfCode);
            printf("\nРазмер неинициализированных данных: 0x%X", OptionalHeader->h32.SizeOfInitializedData);
            printf("\nАдрес точки входа: 0x%X", OptionalHeader->h32.AddressOfEntryPoint);
            printf("\nОснование кода: 0x%X", OptionalHeader->h32.BaseOfCode);
            printf("\nОснование данных: 0x%X", OptionalHeader->h32.BaseOfData);
            printf("\nОснование образа: 0x%X", OptionalHeader->h32.ImageBase);
            printf("\nАлиновка секций: 0x%X", OptionalHeader->h32.SectionAlignment);
            printf("\nАлиновка файла: 0x%X", OptionalHeader->h32.FileAlignment);
            printf("\nРазмер образа: 0x%X", OptionalHeader->h32.SizeOfImage);
            printf("\nРазмер заголовков: 0x%X", OptionalHeader->h32.SizeOfHeaders);
            printf("\nХарактеристики Dll: 0x%X", OptionalHeader->h32.DllCharacteristics);
            printf("\nРазмер stack reserve: 0x%X", OptionalHeader->h32.SizeOfStackReserve);
            printf("\nnРазмер stack commit: 0x%X", OptionalHeader->h32.SizeOfStackCommit);
            printf("\nnРазмер heap reserve: 0x%X", OptionalHeader->h32.SizeOfHeapReserve);
            printf("\nnРазмер heap commit: 0x%X", OptionalHeader->h32.SizeOfHeapCommit);
         }
         else {
            // 64 bit optional header
            printf("\nМагическое число: 0x%X", OptionalHeader->h64.Magic);
            printf("\nРазмер кода: 0x%X", OptionalHeader->h64.SizeOfCode);
            printf("\nРазмер неинициализованных данных: 0x%X", OptionalHeader->h64.SizeOfInitializedData);
            printf("\nАдрес точки входа: 0x%X", OptionalHeader->h64.AddressOfEntryPoint);
            printf("\nОснование кода: 0x%X", OptionalHeader->h64.BaseOfCode);
            printf("\nОснование образа: 0x%08X%08X", HighDWord(OptionalHeader->h64.ImageBase), uint32(OptionalHeader->h64.ImageBase));
            printf("\nАлиновка секций: 0x%X", OptionalHeader->h64.SectionAlignment);
            printf("\nАВлиновка файла: 0x%X", OptionalHeader->h64.FileAlignment);
            printf("\nРазмер образа: 0x%X", OptionalHeader->h64.SizeOfImage);
            printf("\nРазмер заголовков: 0x%X", OptionalHeader->h64.SizeOfHeaders);
            printf("\nХарактеристики Dll: 0x%X", OptionalHeader->h64.DllCharacteristics);
            printf("\nРазмер stack reserve: 0x%08X%08X", HighDWord(OptionalHeader->h64.SizeOfStackReserve), uint32(OptionalHeader->h64.SizeOfStackReserve));
            printf("\nРазмер stack commit: 0x%08X%08X", HighDWord(OptionalHeader->h64.SizeOfStackCommit), uint32(OptionalHeader->h64.SizeOfStackCommit));
            printf("\nРазмер heap reserve: 0x%08X%08X", HighDWord(OptionalHeader->h64.SizeOfHeapReserve), uint32(OptionalHeader->h64.SizeOfHeapReserve));
            printf("\nРазмер heap commit: 0x%08X%08X", HighDWord(OptionalHeader->h64.SizeOfHeapCommit), uint32(OptionalHeader->h64.SizeOfHeapCommit));
         }
         // Data directories
         ...
         for (i = 0; i < NumImageDirs; i++) {
            if (GetImageDir(i, &dir)) {
               printf("\nДиректория %2i, %s:\n  Адрес 0x%04X, Размер 0x%04X, Секция %i, Смещение 0x%04X",
                  i, dir.Name,
   ...
   if (options & (DUMP_SECTHDR | DUMP_SYMTAB | DUMP_RELTAB)) {
      for (j = 0; j < (uint32)NSections; j++) {
         ...
         printf("\n\n%2i Секция %s", j+1, GetSectionName(SectionHeader->Name));

         //printf("\nFile offset of header: 0x%X", (int)((int8*)SectionHeader-buffer));
         printf("\nВиртуальный размер: 0x%X", SectionHeader->VirtualSize);
         if (SectionHeader->VirtualAddress) {
            printf("\nВиртуальный адрес: 0x%X", SectionHeader->VirtualAddress);}
         if (SectionHeader->PRawData || SectionHeader->SizeOfRawData) {
            printf("\nРазмер сырых данных: 0x%X", SectionHeader->SizeOfRawData);
            printf("\nУказатель на сырые данные: 0x%X", SectionHeader->PRawData);
         }
         printf("\nХарактеристики: ");
         ...
         if ((options & DUMP_RELTAB) && SectionHeader->NRelocations > 0) {
            printf("\nЗаписи релокаций: %i", SectionHeader->NRelocations);
            printf("\nУказатель на записи релокаций: 0x%X", SectionHeader->PRelocations);
         ...
         if (SectionHeader->NLineNumbers > 0) {
            printf("\nЗаписи номеров строк: %i", SectionHeader->NLineNumbers);
            printf("  Указатель на номера строк: %i\nСтрок:", SectionHeader->PLineNumbers);
...
void CCOFF::PrintSegmentCharacteristics(uint32 flags) {
   ...
   if (flags & PE_SCN_ALIGN_MASK) {
      ...
      printf(", Алиновано по 0x%X", a); n++;
   }
   if (n == 0) printf("Отсутствует");
}
...
void CCOFF::PrintSymbolTable(int symnum) {
   ...
   while (isym < NumberOfSymbols) {
      ...
      if (Symtab.p->s.StorageClass == COFF_CLASS_FILE && Symtab.p->s.NumAuxSymbols > 0) {
         printf("\n  Имя файла: %s", GetFileName(Symtab.p));
      }
      // Increment point
      s0 = Symtab.p;
      ...
      while (jsym < s0->s.NumAuxSymbols && isym + jsym < NumberOfSymbols) {
         ...
         if (s0->s.StorageClass == COFF_CLASS_EXTERNAL
            ...
            printf("\n  Вспомогательное определение функции:");
            ...
            printf("\n  Вспомогательное определение .bf/.ef:");
            printf("\n  Номер строки в исходнике: %i",
            ...
            printf("\n  Вспомогательное определение слабого внешнего символа:");
            printf("\n  Индекс Symbol2: %i, Характеристики: 0x%X",
            ...
            printf("\n  Вспомогательная запись определения секции:");
            printf("\n  Длина: %i, Число релокаций: %i, Число номеров строк: %i, Контрольная сумма 0x%X,"
               "\n  Число: %i, Секция: %i",
            ...
            // This is section definition aux record
            printf("\n  Вспомогательная запись определения алиаса:");
            printf("\n  индекс символа: %i, ", sa->weak.TagIndex);
            switch (sa->weak.Characteristics) {
            case IMAGE_WEAK_EXTERN_SEARCH_NOLIBRARY:
                printf("без поиска в библиотеке"); break;
            case IMAGE_WEAK_EXTERN_SEARCH_LIBRARY:
                printf("поиск в библиотеке"); break;
            case IMAGE_WEAK_EXTERN_SEARCH_ALIAS:
                printf("символ-алиас"); break;
            default:
                printf("неизвестные характеристики 0x%X", sa->weak.Characteristics);
            }
         }
         else {
            // Unknown aux record type
            printf("\n  Неизвестный тип вспомогательной записи %i", s0->s.StorageClass);
         }
         Symtab.b += SIZE_SCOFF_SymTableEntry;
         jsym++;
      }
...
void CCOFF::PrintImportExport() {
   ...
   if (GetImageDir(0, &dir)) {
      ...
      for (i = 0; i < NumExports; i++) {
         ...
         printf("\n  Порядковый номер %3i, Адрес %6X, Имя %s",
      ...
      while (ImportEntry->DLLNameRVA) {
         ...
         while (LookupEntry[0]) {
            if (LookupEntry[Is64bit] < 0) {
               // Imported by ordinal
               printf("\n  Порядковый номер %i", uint16(LookupEntry[0]));
            }
            else {
