...
// Класс для интерпретации командной строки (КС)
class CCommandLineInterpreter {
public:
   CCommandLineInterpreter();                // Дефолтный конструктор
   ~CCommandLineInterpreter();               // Деструктор
   void ReadCommandLine(int argc, char * argv[]);// Чтение и интерпретация КС
   int  SymbolChange(char const * oldname, char const ** newname, int symtype); // Проверка на необходимость изменения символа
   int  SymbolIsInList(char const * name);   // Проверка наличия символа в SymbolList
   int  SymbolChangesRequested();            // Вид изменения символа, затребованный в КС
   void ReportStatistics();                  // Отчёт о статистике, изменениях имён и проч.
   void CountDebugRemoved();                 // Инкрементирует CountDebugSectionsRemoved
   void CountExceptionRemoved();             // Инкрементирует CountExceptionSectionsRemoved
   void CountSymbolsHidden();                // Инкрементирует CountUnusedSymbolsHidden
   SSymbolChange const * GetMemberToAdd();   // Получает имена объектных файлов, добавляемые в библиотеку
   void CheckExtractSuccess();               // Проверка - найдены ли члены библиотеки для извлечения
   void CheckSymbolModifySuccess();          // Проверка - найдены ли символы для изменения
   char * InputFile;                         // Имя вводного файла
   char * OutputFile;                        // Имя выводимого файла
   int    InputType;                         // Тип вводного файла (обнаруживается по файлу)
   int    OutputType;                        // Тип вывода (тип файла или дамп)
