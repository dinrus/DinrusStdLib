...
bool PathEditorCtrl::ReadClipboard()
{
	...
	{
		int r = PromptYesNoCancel("Vybranй objekty pшesahujн limit danэ љншkou ибry. "
...
{
	bar.Add("Vloћit", CtrlImg::paste(), THISBACK(OnEditPaste))
...
{
	bar.Add("Vybrat vљe", THISBACK(OnEditSelectAll))
		.Key(K_CTRL_A)
...
{
	bar.Add("Zvмtљit", PathImg::view_zoom_in(), THISBACK(OnViewZoomIn))
...
{
	bar.Add("Zmenљit", PathImg::view_zoom_out(), THISBACK(OnViewZoomOut))
...
{
	bar.Add("Horiz. zvмtљit", THISBACK(OnViewZoomHorzIn))
...
{
	bar.Add("Horiz. zmenљit", THISBACK(OnViewZoomHorzOut))
...
{
	bar.Add("Vert. zvмtљit", THISBACK(OnViewZoomVertIn))
...
{
	bar.Add("Vert. zmenљit", THISBACK(OnViewZoomVertOut))
...
{
	bar.Add("Mшнћka", PathImg::setup_grid(), THISBACK(OnSetupGrid))
		.Check(setup.do_grid)
...
{
	bar.Add("Pшichytit do mшнћky", PathImg::setup_snap(), THISBACK(OnSetupSnap))
		.Check(setup.do_snap)
...
{
	bar.Add("Uloћit a zavшнt", CtrlImg::save(), THISBACK(OnEditSave))
		.Key(K_CTRL_S)
