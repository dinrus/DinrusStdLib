...
Value ConvertInt::Scan(const Value& text) const {
	...
	return ErrorValue(UPP::Format(t_("Число должно входить в диапазон между %d и %d."), minval, maxval));
}
...
Value ConvertDouble::Scan(const Value& txt) const {
	...
	return ErrorValue(UPP::Format(t_("Число должно входить в диапазон между %g и %g."), minval, maxval));
}
...
Value ConvertDate::Scan(const Value& text) const {
	...
	Date maxval = GetMax();
	if(m >= minval && m <= maxval) return v;
	return ErrorValue(t_("Дата должна быть в диапазоне между ") + UPP::Format(minval) + t_("\v и ") + UPP::Format(maxval) + ".");
}
...
Value ConvertTime::Scan(const Value& text) const
{
	...
	Time maxval = GetMax();
	if(m >= minval && m <= maxval) return v;
	return ErrorValue(t_("Время должно быть в дипазоне между ") + UPP::Format(minval) + t_("\v и ") + UPP::Format(maxval) + ".");
}
