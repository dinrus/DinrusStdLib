...
static void *ssh_malloc(size_t size, void **abstract)
{
	...
	*aptr++ = (int64)alloc;
	UPP_SSH_alloc += alloc;
	MLOG("DINRUS_SSH_MALLOC(" << (int64)size << ", alloc " << alloc << ") -> " << FormatIntHex(aptr) << ", total = " << (int64) UPP_SSH_alloc);
	return aptr;
}
...
static void ssh_free(void *ptr, void **abstract)
{
	...
	UPP_SSH_alloc -= *aptr;
	MLOG("DINRUS_SSH_FREE(" << ptr << ", alloc " << *aptr << "), total = " << (int64) UPP_SSH_alloc);
	MemoryFree(aptr);
}
...
static void *ssh_realloc(void *ptr, size_t size, void** abstract)
{
	...
	if((int64)(size + sizeof(int64)) <= *aptr) {
		MLOG("DINRUS_SSH_REALLOC(" << ptr << ", " << (int64)size << ", alloc " << *aptr << ") -> keep same block");
		return ptr;
	}
	size_t newalloc = size + sizeof(int64);
	int64 *newaptr = (int64 *)MemoryAllocSz(newalloc);
	if(!newaptr) {
		MLOG("DINRUS_SSH_REALLOC(" << ptr << ", " << (int64)size << ", alloc " << newalloc << ") -> fail");
		return NULL;
	}
	*newaptr++ = newalloc;
	memcpy(newaptr, ptr, min<int>((int)(*aptr - sizeof(int64)), (int)size));
	UPP_SSH_alloc += newalloc - *aptr;
	MLOG("DINRUS_SSH_REALLOC(" << ptr << ", " << (int64)size << ", alloc " << newalloc << ") -> "
