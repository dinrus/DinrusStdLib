...
#endif
{
	...
	*aptr++ = (int64)alloc;
	UPP_SSL_alloc += alloc;
	LLOG("DINRUS_SSL_MALLOC(" << (int64)size << ", size " << size
         << ", " << file << " " << line
...
#endif
{
	...
	if((int64)(size + sizeof(int64)) <= *aptr) { // СДЕЛАТЬ: Do we really want this?
		LLOG("DINRUS_SSL_REALLOC(" << ptr << ", " << (int64)size << ", alloc " << *aptr << ") -> keep same block" << ", thread: " << Thread::GetCurrentId());
		return ptr;
	}
	size_t newalloc = size + sizeof(int64);
	int64 *newaptr = (int64 *)MemoryAllocSz(newalloc);
	if(!newaptr) {
		LLOG("DINRUS_SSL_REALLOC(" << ptr << ", " << (int64)size << ", alloc " << newalloc << ") -> fail" << ", thread: " << Thread::GetCurrentId());
		return NULL;
	}
	*newaptr++ = newalloc;
	memcpy(newaptr, ptr, min<int>((int)(*aptr - sizeof(int64)), (int)size));
	UPP_SSL_alloc += newalloc - *aptr;
	LLOG("DINRUS_SSL_REALLOC(" << ptr << ", " << (int64)size << ", alloc " << newalloc
	     << ", " << file << " " << line
