...
{
	if(!a.IsNumber())
		ThrowError(String().Cat() << "ожидался номер для '" << oper << "', а получен " << Lims(a.ToString()));
	return a.GetNumber();
}
...
{
	if(!a.IsNumber())
		ThrowError(String().Cat() << "ожидалось целое для '" << oper << "', а получен " << Lims(a.ToString()));
	return a.GetInt64();
}
