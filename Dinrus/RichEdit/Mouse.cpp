...
void RichEdit::StdBar(Bar& menu)
{
	...
	if(GetSelection(l, h)) {
		...
		if(objectpos >= 0) {
			...
			bool b = sz.cx || sz.cy;
			menu.Add(t_("Позиция объекта.."), THISBACK(AdjustObjectSize));
			menu.Separator();
			...
			menu.Add(t_("3 pt выше"), THISBACK1(SetObjectYDelta, -3)).Check(IsObjectDelta(delta, -3));
			menu.Add(t_("2 pt выше"), THISBACK1(SetObjectYDelta, -2)).Check(IsObjectDelta(delta, -2));
			menu.Add(t_("1 pt выше"), THISBACK1(SetObjectYDelta, -1)).Check(IsObjectDelta(delta, -1));
			menu.Add(t_("Базовая линия"), THISBACK1(SetObjectYDelta, 0)).Check(IsObjectDelta(delta, 0));
			menu.Add(t_("1 pt ниже"), THISBACK1(SetObjectYDelta, 1)).Check(IsObjectDelta(delta, 1));
			menu.Add(t_("2 pt ниже"), THISBACK1(SetObjectYDelta, 2)).Check(IsObjectDelta(delta, 2));
			menu.Add(t_("3 pt ниже"), THISBACK1(SetObjectYDelta, 3)).Check(IsObjectDelta(delta, 3));
			menu.Separator();
			CopyTool(menu);
			CutTool(menu);
		}
			...
			if(ft) {
				...
				if(!w.IsEmpty() && !SpellWord(w, w.GetLength(),
					...
					menu.Add(t_("добавить в словарь пользователя"), THISBACK(AddUserDict));
					menu.Separator();
				}
				PasteTool(menu);
