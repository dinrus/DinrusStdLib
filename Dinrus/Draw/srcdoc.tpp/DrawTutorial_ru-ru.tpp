...
[s3; Содержание&]
[s0; &]
[s0; [^topic`:`/`/Draw`/srcdoc`/DrawTutorial`_ru`-ru`#1^ 1. Основные 
операции черчения]&]
[s0; [^topic`:`/`/Draw`/srcdoc`/DrawTutorial`_ru`-ru`#2^ 2. Смещения 
и обрезка]&]
[s0; [^topic`:`/`/Draw`/srcdoc`/DrawTutorial`_ru`-ru`#3^ 3. Шрифты 
и метрика шрифтов]&]
[s0; [^topic`:`/`/Draw`/srcdoc`/DrawTutorial`_ru`-ru`#4^ 4. DrawingDraw]&]
[s0; [^topic`:`/`/Draw`/srcdoc`/DrawTutorial`_ru`-ru`#5^ 5. ImageDraw]&]
[s0; [^topic`:`/`/Draw`/srcdoc`/DrawTutorial`_ru`-ru`#6^ 6. Печать]&]
[s0; &]
[s3;:1: [@(128.0.255) 1. Основные операции черчения]&]
[s5; Класс Draw `- это класс`-основа, представляющий
 графический вывод. Он намеренно разработан 
с тем, чтобы содержать довольно ограниценный
 набор лёгких в испоьзовании примитивов 
черчения. В отличие от большинства 
других, подобных ему, классов, из других 
конкурирующих инструментариев, операции 
черчения в U`+`+ [*/ безсостоятельны:] отсутствуют
 отдельные настройки, например, ширины 
линии, цвета пера и проч., все необходимые 
атрибуты рисования являются параметрами 
...
&]
[s5; &]
[s5; Этот пример демонстрирует на деле 
весь диапазон операций черчения.&]
[s5; &]
[s3;:2: [@(128.0.255) 2. Смещения и обрезка]&]
[s5; Можно смещать позицию черчения и 
обрезать операции черчения до указанных 
прямоугольников. Смещение и обрезка 
в U`+`+ являются операциями на стэке 
`- метод [*/ End] восстанавливает предыдущее 
состояние. В частности важна комбинированна
я операция над смещением/клипом [*/ Clipoff], 
которая фактически создаёт новый 
Draw для подобласти текущей области 
Draw.&]
[s7; &]
[s7; #include <CtrlLib/CtrlLib.h>&]
[s7; &]
...
&]
[s5; &]
[s3;:3: [@(128.0.255) 3. Шрифты и метрика шрифтов]&]
[s5; [* Font`- ] это простой тип значения, описывающий 
шрифт: он содержит индекс типофаса 
(typeface), высоту шрифта и дополнительные 
атрибуты (например, флаг `"курсив`"(italic)). 
Font также предоставляет информацию 
о метрике (его `"измерениях`") шрифта 
 и отдельных символов.&]
[s5; Наиболее важной информацией в Font 
является значение [* GetAscent] `- расстояние 
от основания (baseline) до вершины символа, 
значение [* GetDescent] `- расстояние от основания 
до низа ячейки символа (высота ячейки 
символа `- [* GetHeight] `- простая сумма этих 
двух значений) и ширина индивидуального 
символа, которую можно получить с 
помощью оператора Font`'а [* operator`[`]] (где 
индекс в UNICODE).&]
[s5; Для получения индексного списка 
доступных типофасов (typefaces) и их названий, 
соответственно используются статические 
методы [* GetFaceCount] и [* GetFaceName] из класса 
Font.&]
[s5; Позиция, заданная в [* DrawText,] определяет 
верхний левый угол первой буквы текста.&]
[s5; Символьный спейсинг, отличающийся 
от дефолта, можно задать массивом 
Си с целочисленными ширинами символов.&]
[s7; &]
[s7; #include <CtrlLib/CtrlLib.h>&]
[s7; &]
[s7; using namespace Upp;&]
[s7; &]
[s7; struct MyApp : TopWindow `{&]
[s7; -|DropList font`_list;&]
[s7; &]
[s7; -|[1 virtual void Paint(Draw`& w) override `{]&]
[s7; -|-|const String text `= `"Programming is fun`";&]
[s7; -|-|Font fnt(`~font`_list, 60);&]
[s7; -|-|&]
[s7; -|-|w.DrawRect(GetSize(), White);&]
[s7; -|-|w.Offset(50, 50);&]
[s7; -|-|int x `= 0;&]
[s7; -|-|Vector<int> dx;&]
[s7; -|-|for(char letter : text) `{&]
[s7; -|-|-|int width `= fnt`[letter`];&]
...
[s7; -|-|-|font`_list.Add(i, [* Font`::GetFaceName(i)]);&]
[s7; -|-|Add(font`_list.TopPos(0, MINSIZE).LeftPosZ(0, 200));&]
[s7; -|-|font`_list <<`= 0;&]
[s7; -|-|font`_list << `[`=`] `{ NewFont(); `};&]
[s7; -|`}&]
[s7; `};&]
[s7; &]
[s7; GUI`_APP`_MAIN&]
...
[s7; &]
[s0; &]
[s5; В этом уроке мы использовали [* operator`~], 
чтобы получить текущее значение, 
сохранённое в [* font`_list]. Альтернативно, 
можно использовать функцию GetData() 
для большей подробности. Дополнительную 
информацию на эту тему можно найти 
в разделе [*^topic`:`/`/CtrlLib`/srcdoc`/Tutorial`_ru`-ru`#17^ Значен
ие виджета] из урока по ГИП.&]
[s5; Конечный результат работы программы 
показан ниже:&]
[s0; &]
[s0;= 
@@image:2103&725
...
&]
[s5; &]
[s3;:4: [@(128.0.255) 4. DrawingDraw]&]
[s5; Объект[*  Drawing](Чертёж) содержит набор 
операций по черчению и фактически 
представляет векторный рисунок со 
сглаженным ремасштабированием (smooth 
rescaling). [* DrawingDraw] служит как целевой 
Draw при создании Drawing. Drawing можно отрисовать 
на любой Draw target, используя метод [* DrawDrawing]:&]
[s7; &]
[s7; #include <CtrlLib/CtrlLib.h>&]
[s7; &]
[s7; using namespace Upp;&]
...
&]
[s5; &]
[s3;:5: [@(128.0.255) 5. ImageDraw]&]
[s5; [* ImageDraw] предоставляет Draw target (`"цель 
черчения`"), которая создаёт объект 
Image (Рисунок). Чтобы задать компонент 
альфа, используется Draw target, возвращаемый 
методом [* Alpha] и [* GrayColor], для определения 
значения (если вы никогда не использовали 
Alpha, она полагается равной 255 для всего 
Image, в противном случае устанавливается 
в 0, когда Alpha вызывается впервые).&]
[s7; &]
[s7; #include <CtrlLib/CtrlLib.h>&]
[s7; &]
[s7; using namespace Upp;&]
...
&]
[s5; &]
[s3;:6: [@(128.0.255) 6. Печать]&]
[s5; Printing (Печать) весьма похожа на рисование 
на экране. Чтобы получить Draw target для 
принтера, нужно использовать [* PrinterJob] 
(фича из CtrlLib). Каждая печатаемая страница 
должна начинаться с вызова метода 
[* StartPage] и заканчиваться с помощью [* EndPage]. 
Координаты черчения при печати задаются 
в [*/ dots (точках). ]Точка определена как 
1/600 дюйма (фактически, это пиксель 
на принтере в 600 dpi).&]
[s7; &]
[s7; #include <CtrlLib/CtrlLib.h>&]
[s7; &]
[s7; using namespace Upp;&]
...
[s7; &]
[s5; &]
[s3; [@3 Рекомендуемые уроки:]&]
[s5; Если желаете узнать больше, у нас 
есть несколько уроков, которые вам 
могут пригодиться:&]
[s5;l160;i150;O0; [^topic`:`/`/Draw`/srcdoc`/ImgTutorial`_ru`-ru^ Урок 
Image] `- сюда помещены вещи, относящиеся 
к изображениям. Показано как создавать, 
использовать и внедрять иконки прямо 
в приложение.]]