...
void Report::PaintHF(Draw& w, i32 y, tukk qtf, i32 i)
{
...
Txt  Report::FormatHF(tukk s, i32 pageno)
{
	Txt result;
	while(*s) {
		if(s[0] == '$' && s[1] == '$') {
...
Report& Report::Footer(tukk qtf, i32 spc)
{
	footer = qtf;
	footerspc = spc;
...
void Report::Put(const RichText& txt, uk context)
{
	PageY py(pagei, y);
...
void Report::Put(tukk qtf)
{
	Put(ParseQTF(qtf));
}

bool Report::ChoosePrinter(tukk jobname)
{
	printerjob.Create();
...
bool Report::ChooseDefaultPrinter(tukk jobname)
{
	printerjob.Create();
	printerjob->Name(jobname);
