...
void  SqlOption::SetData(const Val& data) {
	Txt s = data;
...
Val SqlOption::GetData() const {
	if(IsThreeState() && IsNull(Get()))
		return Txt();
	return Get() ? "1" : "0";
}

Val SqlNOption::GetData() const
{
	if(Get()) return "1";
	return Null;
}
...
{
	Vec<Txt> col = GetSchColumns(~table);
	for(Ctrl *q = dlg.GetFirstChild(); q; q = q->GetNext()) {
		Txt id = ToUpper(q->GetLayoutId());
